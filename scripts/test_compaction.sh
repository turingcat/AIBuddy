...
if [ -n "$COMPACTION_PROVIDER" ]; then
  ...
  export HEYBUDDY_PROVIDER="$COMPACTION_PROVIDER"
fi
if [ -n "$COMPACTION_MODEL" ]; then
  echo "Using override model: $COMPACTION_MODEL"
  export HEYBUDDY_MODEL="$COMPACTION_MODEL"
fi
...
validate_compaction() {
  ...
  local session_json=$($HEYBUDDY_BIN session export --format json --session-id "$session_id" 2>&1)
...
(cd "$TESTDIR" && "$HEYBUDDY_BIN" run --with-builtin developer --text "list files and read hello.txt" 2>&1) | tee "$OUTPUT"
...
if ! command -v jq &> /dev/null; then
  ...
  SESSION_ID=$("$HEYBUDDY_BIN" session list --format json 2>/dev/null | jq -r '.[0].id' 2>/dev/null)
  ...
  if [ -z "$SESSION_ID" ] || [ "$SESSION_ID" = "null" ]; then
    ...
    (cd "$TESTDIR" && "$HEYBUDDY_BIN" run --resume --session-id "$SESSION_ID" --text "Please compact this conversation" 2>&1) | tee -a "$OUTPUT"
...
export HEYBUDDY_AUTO_COMPACT_THRESHOLD=0.005
...
(cd "$TESTDIR" && "$HEYBUDDY_BIN" run --text "$LONG_RESPONSE_PROMPT" 2>&1) | tee "$OUTPUT"
...
if ! command -v jq &> /dev/null; then
  ...
  if [ -z "$SESSION_ID" ] || [ "$SESSION_ID" = "null" ]; then
    ...
    (cd "$TESTDIR" && "$HEYBUDDY_BIN" run --resume --session-id "$SESSION_ID" --text "hi again" 2>&1) | tee -a "$OUTPUT"
...
unset HEYBUDDY_AUTO_COMPACT_THRESHOLD
...
if ! (cd "$PROXY_DIR" && uv sync 2>&1 | tee "$PROXY_SETUP_LOG"); then
  ...
  if [ "$PROXY_READY" != "true" ]; then
    ...
    export HEYBUDDY_PROVIDER_SKIP_BACKOFF=true
    export HEYBUDDY_PROVIDER=anthropic
    export HEYBUDDY_MODEL=claude-haiku-4-5
    ...
    (cd "$TESTDIR" && "$HEYBUDDY_BIN" run --text "hello world" 2>&1) | tee "$OUTPUT"
    ...
    unset HEYBUDDY_PROVIDER_SKIP_BACKOFF
    unset HEYBUDDY_PROVIDER
    unset HEYBUDDY_MODEL
    unset UV_INDEX_URL
  fi
fi
