...
BACKUP_DIR="${HOME}/.local/share/heybuddy/heybuddy-db-backups"
...
MIGRATIONS_DIR="${HOME}/.local/share/heybuddy/migrations"
RUST_SESSION_MANAGER="crates/heybuddy/src/session/session_manager.rs"
...
get_heybuddy_db_path() {
    if [[ -n "${HEYBUDDY_PATH_ROOT:-}" ]]; then
        echo "${HEYBUDDY_PATH_ROOT}/data/sessions/sessions.db"
    else
        local possible_paths=(
            "${HOME}/.local/share/heybuddy/sessions/sessions.db"
            "${HOME}/Library/Application Support/Block/heybuddy/data/sessions/sessions.db"
        )
...
show_status() {
    echo -e "${BLUE}=== HeyBuddy Database Status ===${NC}"
    ...
    if [[ ! -f "${DB_PATH}" ]]; then
        ...
        echo ""
        echo "This is normal if you haven't run HeyBuddy yet."
        echo "Once you run HeyBuddy, a database will be created automatically."
        return
    fi
...
generate_migrations() {
    if [[ ! -f "${RUST_SESSION_MANAGER}" ]]; then
        ...
        echo "Make sure you're running this from the heybuddy repository root."
        exit 1
    fi
...
show_help() {
    ...
    echo -e "${BLUE}HeyBuddy Database Migration Helper${NC}"
    echo ""
    ...
    echo "    rm -rf ~/.local/share/heybuddy/migrations/004_*"
    echo "    $0 generate-migrations"
    echo ""
