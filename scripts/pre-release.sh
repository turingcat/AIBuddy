...
REPO="${AIBUDDY_GITHUB_REPO:-aaif-goose/goose}"
DEST="$HOME/Downloads"
TMPDIR=$(mktemp -d)
...
if ! curl -fSL --connect-timeout 10 -o "$TMPDIR/aibuddy.zip" "$DOWNLOAD_URL" 2>/dev/null; then
    ...
    gh api "repos/$REPO/actions/artifacts/$ARTIFACT_ID/zip" > "$TMPDIR/aibuddy.zip"
fi
echo "Done."

unzip -o -q "$TMPDIR/aibuddy.zip" -d "$TMPDIR/extracted"
