...
#
# Sets up a workdir with two named subagents on disk, runs aibuddy against
# several prompts, and validates that aibuddy delegates to the right subagent
...
#
# Knobs:
#   AIBUDDY_PROVIDER (default: anthropic)
#   AIBUDDY_MODEL    (default: claude-haiku-4-5)
#   SKIP_BUILD     skip cargo build (assumes target/debug/aibuddy already exists)
...
# won't collide with anything the user might have in ~/.agents, ~/.aibuddy, or
...
# ~/.agents/agents, ~/.aibuddy/agents, or ~/.claude/agents.
#
...
# Run aibuddy with a prompt in TESTDIR. We use --no-session for hermeticity.
run_aibuddy() {
  local prompt="$1"
  local outfile="$2"
  (cd "$TESTDIR" && "$AIBUDDY_BIN" run --text "$prompt" --no-session 2>&1) | tee "$outfile"
}
...
You are a validator. You will be given a transcript of a aibuddy CLI run.
...
EOF
  ...
  local verdict
  verdict=$("$AIBUDDY_BIN" run --text "$judge_prompt" --no-session 2>&1)
...
TMP1=$(mktemp)
run_aibuddy "@janpier which of your animals can speak?" "$TMP1"
...
TMP2=$(mktemp)
run_aibuddy "Ask janpier what tricks his animals can do." "$TMP2"
...
TMP3=$(mktemp)
run_aibuddy "Write me a hello world program in the Forth programming language." "$TMP3"
...
if grep -qE "▸.*delegate" "$TMP3" && grep -qE "^\s*source[[:space:]]+peterjoris\b" "$TMP3"; then
  ...
  assert_judge "$TMP3" \
    "The user asked aibuddy to write a Hello World program in the Forth programming language. The session had a registered subagent named 'peterjoris' described as a Forth expert. Does the transcript show ANY of: (a) aibuddy called the delegate tool with source 'peterjoris', or (b) aibuddy's reply mentions peterjoris (or 'the Forth expert') as the right specialist for this task, or (c) aibuddy produced syntactically plausible Forth code as the answer? ANY of (a), (b), (c) counts as PASS. Only FAIL if none of those apply." \
...
TMP4=$(mktemp)
run_aibuddy "What is 2 + 2? Reply with just the digit." "$TMP4"
...
EMPTYDIR=$(mktemp -d)
TMP5=$(mktemp)
(cd "$EMPTYDIR" && "$AIBUDDY_BIN" run --text "@janpier where is the treasure?" --no-session 2>&1) | tee "$TMP5"
