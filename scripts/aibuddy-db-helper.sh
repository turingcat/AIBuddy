...
BACKUP_DIR="${HOME}/.local/share/aibuddy/aibuddy-db-backups"
...
MIGRATIONS_DIR="${HOME}/.local/share/aibuddy/migrations"
RUST_SESSION_MANAGER="crates/aibuddy/src/session/session_manager.rs"
...
get_aibuddy_db_path() {
    if [[ -n "${AIBUDDY_PATH_ROOT:-}" ]]; then
        echo "${AIBUDDY_PATH_ROOT}/data/sessions/sessions.db"
    else
        local possible_paths=(
            "${HOME}/.local/share/aibuddy/sessions/sessions.db"
            "${HOME}/Library/Application Support/Block/aibuddy/data/sessions/sessions.db"
        )
...
show_status() {
    echo -e "${BLUE}=== AIBuddy Database Status ===${NC}"
    ...
    if [[ ! -f "${DB_PATH}" ]]; then
        ...
        echo ""
        echo "This is normal if you haven't run AIBuddy yet."
        echo "Once you run AIBuddy, a database will be created automatically."
        return
    fi
...
generate_migrations() {
    if [[ ! -f "${RUST_SESSION_MANAGER}" ]]; then
        ...
        echo "Make sure you're running this from the aibuddy repository root."
        exit 1
    fi
...
show_help() {
    ...
    echo -e "${BLUE}AIBuddy Database Migration Helper${NC}"
    echo ""
    ...
    echo "    rm -rf ~/.local/share/aibuddy/migrations/004_*"
    echo "    $0 generate-migrations"
    echo ""
