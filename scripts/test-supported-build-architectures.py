...
class SupportedBuildArchitecturesTest(unittest.TestCase):
    ...
    def test_windows_portable_repack_uses_an_existing_installer_artifact(self) -> None:
        ...
        self.assertIn("376a13e7c41cc5528b6088d0dd16ec1b323a8d37", workflow)
        self.assertIn("cmake --build", workflow)
        self.assertIn('"$INNOEXTRACT" --extract', workflow)
        self.assertIn("extract_status=$?", workflow)
        self.assertIn("ARCHITECTURE='${{ inputs.architecture }}'", workflow)
        self.assertIn("extracted/app/HeyBuddy.exe", workflow)
        self.assertIn("extracted/app/resources/bin/goose.exe", workflow)
        self.assertIn("extracted/app/resources/bin/uv.exe", workflow)
        self.assertIn("extracted/app/resources/bin/uvx.exe", workflow)
