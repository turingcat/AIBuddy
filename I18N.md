# Internationalization (i18n) — HeyBuddy Desktop UI

This document describes the i18n infrastructure for the HeyBuddy Desktop UI (`ui/desktop/`).
...
2. **`HEYBUDDY_LOCALE`** — explicit override from env/app config when the desktop setting is
   System Default
...
1. Copy `src/i18n/messages/en.json` to a new file, e.g., `src/i18n/messages/xx.json`.
