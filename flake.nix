{
  description = "aibuddy - An AI agent CLI";
  ...
  outputs = { self, nixpkgs, flake-utils, rust-overlay }:
    flake-utils.lib.eachSystem [ "x86_64-linux" "aarch64-linux" "aarch64-darwin" ] (system:
      let
        ...
        cargoToml = builtins.fromTOML (builtins.readFile ./crates/aibuddy-cli/Cargo.toml);
      ...
      in
      {
        packages.default = pkgs.rustPlatform.buildRustPackage {
          ...
          cargoBuildFlags = [ "--package" "aibuddy-cli" ];
          ...
          checkPhase = ''
            ...
            # Run tests for aibuddy-cli package only
            cargo test --package aibuddy-cli --release
          '';
        ...
        devShells.default = pkgs.mkShell {
          ...
          shellHook = ''
            echo "aibuddy development environment"
            echo "Rust version: $(rustc --version)"
            echo ""
            echo "Commands:"
            echo "  nix build           - Build aibuddy CLI"
            echo "  nix run             - Run aibuddy CLI"
            echo "  cargo build -p aibuddy-cli - Build with cargo"
            echo "  cargo run -p aibuddy-cli   - Run with cargo"
          '';
        };
      }
    );
