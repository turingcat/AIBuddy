...
| `OIDC_AUDIENCE` | The audience your workflow requests (e.g. `aibuddy-oidc-proxy`) |
...
steps:
  - name: Get OIDC token
    ...
    with:
      script: |
        const token = await core.getIDToken('aibuddy-oidc-proxy');
  ...
  - name: Call API through proxy
    ...
    run: aibuddy run --recipe my-recipe.yaml
```
