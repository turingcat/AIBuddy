...
description: Chat with heybuddy through Telegram from any device.
---

The Telegram Gateway lets you interact with heybuddy through Telegram, enabling remote access from any device where Telegram is available.
...
The Gateway connects your Telegram account to heybuddy through a secure pairing process. Once paired, you can send messages to a Telegram bot that forwards them to heybuddy, and receive formatted responses back in Telegram.
...
1. [Configure heybuddy](/docs/getting-started/providers) with a provider and model.
...
Use the heybuddy CLI to start the gateway and pair your Telegram account.
...
heybuddy gateway start telegram --bot-token "$TELEGRAM_BOT_TOKEN"
```
...
```bash
heybuddy gateway pair telegram
```

Open your bot in Telegram and send it the six-character pairing code within five minutes. The bot confirms when pairing is complete, and you can then chat with heybuddy through Telegram.
...
Once paired, you can:
- Send messages to heybuddy and receive responses
- Get formatted code blocks with syntax highlighting
- Continue conversations across multiple sessions
- Access your configured heybuddy extensions

## Troubleshooting

### Bot not responding
- Verify the bot token is correct.
- Check that the `heybuddy gateway start` command is still running.
...
- The gateway converts heybuddy's markdown to Telegram-compatible formatting.
