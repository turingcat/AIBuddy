...
description: Chat with aibuddy through Telegram from any device.
---

The Telegram Gateway lets you interact with aibuddy through Telegram, enabling remote access from any device where Telegram is available.
...
The Gateway connects your Telegram account to aibuddy through a secure pairing process. Once paired, you can send messages to a Telegram bot that forwards them to aibuddy, and receive formatted responses back in Telegram.
...
1. [Configure aibuddy](/docs/getting-started/providers) with a provider and model.
...
Use the aibuddy CLI to start the gateway and pair your Telegram account.
...
aibuddy gateway start telegram --bot-token "$TELEGRAM_BOT_TOKEN"
```
...
```bash
aibuddy gateway pair telegram
```

Open your bot in Telegram and send it the six-character pairing code within five minutes. The bot confirms when pairing is complete, and you can then chat with aibuddy through Telegram.
...
Once paired, you can:
- Send messages to aibuddy and receive responses
- Get formatted code blocks with syntax highlighting
- Continue conversations across multiple sessions
- Access your configured aibuddy extensions

## Troubleshooting

### Bot not responding
- Verify the bot token is correct.
- Check that the `aibuddy gateway start` command is still running.
...
- The gateway converts aibuddy's markdown to Telegram-compatible formatting.
