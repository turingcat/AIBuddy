...
description: Use built-in diagnostics, report bugs, and request new features with heybuddy's integrated support tools.
---
...
heybuddy provides several built-in features to help you get support, report issues, and request new functionality. This guide covers the diagnostics system, bug reporting, and feature request tools.
...
<Tabs groupId="interface">
  ...
  <TabItem value="cli" label="heybuddy CLI">
    Use the session diagnostics command to generate a troubleshooting bundle. For complete details and all available options, see the [CLI Commands guide](/docs/guides/heybuddy-cli-commands#session-diagnostics-options).

    ```sh
    # Generate diagnostics for a specific session
    heybuddy session diagnostics --session-id <session_id>

    # Interactive selection (prompts you to choose a session)
    heybuddy session diagnostics

    # Save to a custom location
    heybuddy session diagnostics --session-id <session_id> --output /path/to/diagnostics.json
    ```
...
The feature request system helps you suggest improvements and new functionality for heybuddy.
...
## Error Recovery with "Ask heybuddy"

When certain types of error occur in heybuddy Desktop (such as failures to activate extensions), you'll see an `Ask heybuddy` button in the error notification. This feature lets you quickly troubleshoot the issue with heybuddy's help:

1. When the error occurs, an `Ask heybuddy` button appears in the error notification
2. Click the button to send the error details to heybuddy in a chat prompt
3. heybuddy provides diagnostic suggestions and potential solutions
