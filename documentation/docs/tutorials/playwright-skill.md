...
description: Use heybuddy with the Playwright CLI to generate automated tests using natural language
---
...
With the [Playwright CLI](https://github.com/microsoft/playwright-cli), heybuddy can navigate websites, click buttons, fill forms, and turn those interactions into Playwright tests, all from natural language. Unlike the Playwright MCP, which sends the full page structure to the LLM on every request, Playwright CLI stores the accessibility tree locally. That means faster responses, lower costs, and no issues with large pages.

## Why as a Skill?

LLMs may not be trained on Playwright's CLI, so when asking an agent to use it, it may hallucinate commands and arguments resulting in errors and wasted tokens. The [Playwright CLI Skill](https://github.com/microsoft/playwright-cli/blob/main/skills/playwright-cli/SKILL.md) teaches heybuddy how to use the CLI and when to invoke specific commands.
...
3. Choose `heybuddy` when asked which agent to install to
...
In heybuddy, enable the [Summon extension](/docs/mcp/summon-mcp) to load Agent Skills within sessions.
...
Give heybuddy a single prompt that describes what you want to test:

```
Using the Playwright CLI skill, open heybuddy-docs.ai, click on the Docs menu, click on Context Engineering, 
...
1. Opens the browser: `playwright-cli open heybuddy-docs.ai`
...
test('navigate to Using Skills guide via docs menu', async ({ page }) => {
  ...
  await expect(page).toHaveTitle(/heybuddy/);
...
heybuddy can even run the test for you to make sure it works as expected. If Playwright is already set up, just ask it to run the test. If not, heybuddy can install Playwright for you and then run the test.

## Viewing the Video

To see a video of what happened, prompt heybuddy:

```
Show me the video
```

heybuddy will use the CLI to open the recorded video, so you can see exactly what happened during the session.

## Viewing the Trace

To debug or review what happened, prompt heybuddy:
...
When you have heybuddy running several browser tasks at once, it can be hard to keep track of what's happening. The visual dashboard gives you a bird's eye view of all your active browser sessions, letting you watch progress in real time or jump in and take control when needed.

```
Show playwright dashboard
```

From here you can see live previews of every browser heybuddy is controlling. Click into any session to watch it full-size, or take over the mouse and keyboard yourself if heybuddy needs a hand. Press `Escape` when you're done and heybuddy picks up right where you left off.

## Full Capabilities

Want to know what else the Playwright skill can do? Ask heybuddy:
