---
title: Ralph Loop
description: Run heybuddy in a loop with fresh context per iteration and cross-model review
---

Ralph Loop, based on [Geoffrey Huntley's Ralph Wiggum technique](https://ghuntley.com/ralph/), is an iterative development pattern that keeps heybuddy working on a task until it's genuinely complete.
...
- [Install the heybuddy CLI](/docs/getting-started/installation) because the Ralph Loop runs via the terminal.
...
```bash
mkdir -p ~/.config/heybuddy/recipes

curl -sL https://raw.githubusercontent.com/aaif-goose/goose/main/documentation/src/pages/recipes/data/recipes/ralph-loop.sh -o ~/.config/heybuddy/recipes/ralph-loop.sh
curl -sL https://raw.githubusercontent.com/aaif-goose/goose/main/documentation/src/pages/recipes/data/recipes/ralph-work.yaml -o ~/.config/heybuddy/recipes/ralph-work.yaml
curl -sL https://raw.githubusercontent.com/aaif-goose/goose/main/documentation/src/pages/recipes/data/recipes/ralph-review.yaml -o ~/.config/heybuddy/recipes/ralph-review.yaml

chmod +x ~/.config/heybuddy/recipes/ralph-loop.sh
```
...
```bash
~/.config/heybuddy/recipes/ralph-loop.sh "Create a simple browser using Electron and React"
```
...
```bash
~/.config/heybuddy/recipes/ralph-loop.sh ./prd.md
```
:::
...
|--------|-------------|
| Worker model | The model that does the actual coding work. Defaults to `HEYBUDDY_MODEL` if set. |
| Worker provider | The provider for the worker model (e.g., `openai`, `anthropic`). Defaults to `HEYBUDDY_PROVIDER` if set. |
...
The terminal will show heybuddy moving through the worker and reviewer phases. Each iteration starts with a fresh session to keep the context clean. Here's what a successful run looks like:
...
▶ WORK PHASE
... (heybuddy creates initial implementation) ...

▶ REVIEW PHASE
... (heybuddy reviews the work) ...
...
▶ WORK PHASE
... (heybuddy addresses feedback) ...

▶ REVIEW PHASE
... (heybuddy reviews again) ...
...
Ralph Loop uses files in `.heybuddy/ralph/` to persist state between iterations. This is how the worker knows what to do and the reviewer knows what was done, even though each iteration starts with fresh context.
...
#   RALPH_RECIPE_DIR      - Recipe directory (default: ~/.config/heybuddy/recipes)
#

set -e

INPUT="$1"
RECIPE_DIR="${RALPH_RECIPE_DIR:-$HOME/.config/heybuddy/recipes}"
...
prompt_for_settings() {
    local default_model="${HEYBUDDY_MODEL:-}"
    local default_provider="${HEYBUDDY_PROVIDER:-}"
...
for i in $(seq 1 "$MAX_ITERATIONS"); do
    ...
    HEYBUDDY_PROVIDER="$WORKER_PROVIDER" HEYBUDDY_MODEL="$WORKER_MODEL" heybuddy run --recipe "$RECIPE_DIR/ralph-work.yaml" || {
    ...
    HEYBUDDY_PROVIDER="$REVIEWER_PROVIDER" HEYBUDDY_MODEL="$REVIEWER_MODEL" heybuddy run --recipe "$RECIPE_DIR/ralph-review.yaml" || {
...
instructions: |
  ...
  1. cat .heybuddy/ralph/task.md (YOUR TASK)
  2. cat .heybuddy/ralph/iteration.txt 2>/dev/null || echo "1"
  3. cat .heybuddy/ralph/review-feedback.txt 2>/dev/null
  ...
  - If task is complete: echo "done" > .heybuddy/ralph/work-complete.txt
  - Always write a summary: echo "what I did" > .heybuddy/ralph/work-summary.txt

prompt: |
  ## Ralph Work Phase
  
  Read your task from: .heybuddy/ralph/task.md
  
  1. Read the task: `cat .heybuddy/ralph/task.md`
  2. Check iteration: `cat .heybuddy/ralph/iteration.txt 2>/dev/null || echo "1"`
  3. Check for review feedback: `cat .heybuddy/ralph/review-feedback.txt 2>/dev/null`
  4. List existing files: `ls -la`
  5. Do the work (address feedback if any, otherwise make progress)
  6. Write summary: `echo "summary" > .heybuddy/ralph/work-summary.txt`
  7. If complete: `echo "done" > .heybuddy/ralph/work-complete.txt`
...
instructions: |
  ...
  OUTPUT:
  If approved: echo "SHIP" > .heybuddy/ralph/review-result.txt
  If needs work: 
    echo "REVISE" > .heybuddy/ralph/review-result.txt
    echo "specific feedback" > .heybuddy/ralph/review-feedback.txt

prompt: |
  ## Ralph Review Phase
  
  1. Read the task: `cat .heybuddy/ralph/task.md`
  2. Read work summary: `cat .heybuddy/ralph/work-summary.txt`
  3. Check if complete: `cat .heybuddy/ralph/work-complete.txt 2>/dev/null`
  4. Examine the actual files created/modified
  5. Run verification (tests, build, etc.)
  6. Decide: SHIP or REVISE
  
  If SHIP: `echo "SHIP" > .heybuddy/ralph/review-result.txt`
  If REVISE: 
    `echo "REVISE" > .heybuddy/ralph/review-result.txt`
    `echo "specific feedback" > .heybuddy/ralph/review-feedback.txt`
