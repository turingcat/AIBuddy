---
description: Integrate aibuddy with MLflow to observe and evaluate agent performance
---

# Observability with MLflow

This tutorial covers how to integrate aibuddy with MLflow to trace your aibuddy sessions and understand how the agent is performing.
...
## Configure aibuddy to export OTLP to MLflow

aibuddy exports OpenTelemetry data over OTLP/HTTP. Point the exporter to MLflow's OTLP endpoint:
...
```bash
pip install mlflow
mlflow experiments create --experiment-name "aibuddy-traces"
...
## Run aibuddy with MLflow enabled

Start aibuddy normally. With the OTLP environment variables set, aibuddy will automatically export traces to MLflow:

```bash
aibuddy session
```

Open the MLflow UI at `http://localhost:5000` and navigate to the **Traces** tab to see detailed traces of your aibuddy session, including LLM calls, tool executions, and token usage.

![aibuddy trace in MLflow](../assets/guides/mlflow-aibuddy-tracing.png)
...
- [MLflow aibuddy integration guide](https://mlflow.org/docs/latest/genai/tracing/integrations/listing/goose.html)
