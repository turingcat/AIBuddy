---
title: Using aibuddy in Headless Mode for Automation
description: aibuddy Headless Mode
---

# Automate Development Tasks with aibuddy Headless Mode

*Run AI-powered engineering workflows in CI/CD pipelines, servers, and batch processing scenarios*

The ability to automate complex engineering tasks without human intervention has been huge, but let's take it to the next level with AI. aibuddy's "headless" mode enables developers to harness the full power of AI automation in server environments, CI/CD pipelines, and batch processing scenarios where interactive sessions simply aren't feasible.

## What is Headless Mode?

Headless mode is aibuddy's non-interactive execution environment, designed for automated scenarios where human intervention isn't available (or wanted).
...
aibuddy run --with-builtin developer -t "Check system logs for errors in the last 24 hours, identify performance bottlenecks, and generate a maintenance report"

# Container optimization
aibuddy run --no-session -t "Analyze the Dockerfile, optimize for smaller image size, and update the build process documentation"

# Cloud resource audit
aibuddy run -t "Review our AWS infrastructure configuration, identify cost optimization opportunities, and create a migration plan for underutilized resources"
```
...
- name: AI-Powered Code Review
  run: |
    aibuddy run --with-builtin developer \
...
aibuddy run --debug -t "Examine the failing test suite, identify the root cause of failures, implement fixes, and ensure all tests pass"

# Automated documentation updates
aibuddy run -t "Review code changes and update the README.md and API documentation to reflect new features and modifications"
```
...
for repo in repo1 repo2 repo3; do
  cd $repo
  aibuddy run --no-session -t "Update all dependencies to latest stable versions, run tests, and create a PR if changes are needed"
  cd ..
done

# Database migration automation
aibuddy run -t "Analyze the current database schema, generate migration scripts for the new requirements, and create rollback procedures"
```
...
0 2 * * * /usr/local/bin/aibuddy run --no-session -t "Run comprehensive security audit, check for vulnerabilities, and email report to security team"

# Weekly dependency updates
0 9 * * 1 /usr/local/bin/aibuddy run -t "Check for outdated dependencies, create update PRs for non-breaking changes, and schedule review for major updates"
```
...
aibuddy run -t "Analyze the test failures in the latest CI run, identify the root cause, and create a fix with appropriate unit tests"

# Better: Even more detailed
aibuddy run -t "Review the failed tests in tests/integration/, identify why the authentication middleware is failing, implement a fix that maintains backward compatibility, and add regression tests"
```
...
```bash
export AIBUDDY_CONTEXT_STRATEGY=summarize
export AIBUDDY_MAX_TURNS=50
export AIBUDDY_MODE=auto
export AIBUDDY_DISABLE_SESSION_NAMING=true
```

The `CONTEXT_STRATEGY` and `MAX_TURNS` settings help manage conversation limits, while `AIBUDDY_MODE` set to `auto` allows for non-interactive execution. `AIBUDDY_DISABLE_SESSION_NAMING` avoids the extra background model call used to generate a session name and keeps the default "CLI Session" name.
...
```bash
#!/bin/bash
if ! aibuddy run --no-session -t "Run security audit and fix critical issues"; then
    echo "aibuddy automation failed - manual intervention required"
    exit 1
fi
```
...
[Recipes](/docs/guides/recipes/) are aibuddy's powerful way to define reusable, parameterized workflows. In headless mode, recipes become even more valuable as they enable sophisticated automation scenarios.
...
aibuddy run --recipe automation-recipe.yaml
...
aibuddy run --recipe main-workflow.yaml \
...
**What this means**: aibuddy cannot ask for clarification, approval, or additional input during execution. If it's unsure of what to do, the prompt result will usually show you a question like "How should I proceed?".

**Impact**: If instructions are ambiguous or if unexpected situations arise, aibuddy will make its best judgment based on available context, which might not always align with your intentions.
...
aibuddy run -t "Fix the issues"

# Better: Specific and actionable
aibuddy run -t "Fix the TypeScript compilation errors in src/components/, ensure all imports are correct, and update any deprecated API calls to use the latest syntax"
```
...
**What this means**: aibuddy cannot prompt for permission to use potentially risky tools or operations.
...
```bash
export AIBUDDY_MODE=auto  # Automatically approve safe operations
# or configure specific tool permissions in your config
```

### 4. Context Decision Automation

**What this means**: When conversation context limits are reached, aibuddy automatically applies the configured strategy without user input.
...
```bash
export AIBUDDY_CONTEXT_STRATEGY=summarize  # Usually the best choice for automation
export AIBUDDY_MAX_TURNS=100  # Prevent runaway execution
```
...
export AIBUDDY_MODEL=gpt-4o

# Output control
export AIBUDDY_CLI_MIN_PRIORITY=0.2  # Reduce verbose output
```
...
export AIBUDDY_PLANNER_MODEL=gpt-4o

# Security and permissions
export AIBUDDY_ALLOWLIST=https://company.com/allowed-extensions.json
```

## The Future of Automated Development

aibuddy's headless mode represents more than just a feature -- it's a shift toward truly automated development workflows powered by AI. We can remove human intervention in routine tasks so teams can focus on high-value work while AI handles the repetitive, time-consuming operations that slow us down.

Whether you're looking to streamline your CI/CD pipelines, automate server maintenance, or handle bulk operations across multiple repositories, aibuddy's headless mode provides the foundation for building sophisticated, reliable automation workflows.

**Start your automation journey today:**

1. **Install aibuddy** and configure your environment variables
