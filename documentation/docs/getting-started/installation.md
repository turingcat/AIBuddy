...
<Tabs>
  <TabItem value="mac" label="macOS" default>
    Choose to install the Desktop and/or CLI version of aibuddy:

    <Tabs groupId="interface">
      <TabItem value="ui" label="aibuddy Desktop" default>
        Install aibuddy Desktop directly from the browser or with [Homebrew](https://brew.sh/).
        ...
        <div style={{ marginTop: '1rem' }}>
          ...
          2. Run the executable file to launch the aibuddy Desktop application.

          :::tip Updating aibuddy
          It's best to periodically [update aibuddy](/docs/guides/updating-aibuddy).
          :::
        </div>
        ...
        ```bash
        brew install --cask block-aibuddy
        ```
        ---
        <div style={{ marginTop: '1rem' }}>
          :::info Permissions
          If you're on an Apple Mac M3 and the aibuddy Desktop app shows no window on launch, check and update the following:

          Ensure the `~/.config` directory has read and write access.

          aibuddy needs this access to create the log directory and file. Once permissions are granted, the app should load correctly. For steps on how to do this, refer to the  [Known Issues Guide](/docs/troubleshooting/known-issues#macos-permission-issues)
          :::
        </div>
      </TabItem>
      <TabItem value="cli" label="aibuddy CLI">
        Install aibuddy directly from the browser or with [Homebrew](https://brew.sh/).

        <h3 style={{ marginTop: '1rem' }}>Option 1: Install via Download script</h3>
        Run the following command to install the latest version of aibuddy on macOS:

        ```sh
        curl -fsSL https://github.com/aaif-goose/goose/releases/download/stable/download_cli.sh | bash
        ```
        This script will fetch the latest version of aibuddy and set it up on your system.
        ...
        :::tip Updating aibuddy
        It's best to keep aibuddy updated. To update aibuddy, run:
        ```sh
        aibuddy update
        ```
        :::
        ...
        ```bash
        brew install block-aibuddy-cli
        ```
      </TabItem>
    </Tabs>
  </TabItem>
  ...
  <TabItem value="linux" label="Linux">
    ...
    <Tabs groupId="interface">
      <TabItem value="ui" label="aibuddy Desktop" default>
        Install aibuddy Desktop directly from the browser.
      ...
      <TabItem value="cli" label="aibuddy CLI">
        Run the following command to install the aibuddy CLI on Linux:
  ...
  <TabItem value="windows" label="Windows">
    ...
    <Tabs groupId="interface">
      ...
      <TabItem value="cli" label="aibuddy CLI">
        To install aibuddy natively on Windows, you need one of the following environments:
        ...
        **PowerShell Installation: Standard**
        ...
        ```
        Then run the script to install aibuddy:
        ```powershell
        .\download_cli.ps1
        ```

        :::info Windows PATH Setup
        If you see a warning that aibuddy is not in your PATH, you need to add aibuddy to your PATH:
        ...
        After updating your PATH, you can run `aibuddy` commands from any directory.
        :::

        <details>
        <summary>Install via Windows Subsystem for Linux (WSL)</summary>

          We recommend running the aibuddy CLI natively on Windows, but you can use WSL if you prefer a Linux-like environment.
          ...
          3. Run the aibuddy installation script:
          ```bash
          curl -fsSL https://github.com/aaif-goose/goose/releases/download/stable/download_cli.sh | bash
          ```
          :::tip
          ...
          If needed, add aibuddy to your path:
...
## Set LLM Provider
aibuddy works with [supported LLM providers][providers] that give aibuddy the AI intelligence it needs to understand your requests. On first use, you'll be prompted to configure your preferred provider.

<Tabs groupId="interface">
  <TabItem value="ui" label="aibuddy Desktop" default>
    On the welcome screen the first time you open aibuddy, choose how to configure a provider:
  ...
  <TabItem value="cli" label="aibuddy CLI">
    ...
    [aibuddy opens the browser and prints details]
  ...
  Then run `aibuddy configure` again. aibuddy will detect the environment variable and display:
...
<Tabs groupId="interface">
    <TabItem value="ui" label="aibuddy Desktop" default>
        Starting a session in the aibuddy Desktop is straightforward. After choosing your provider, you'll see the session interface ready for use.

        Type your questions, tasks, or instructions directly into the input field, and aibuddy will get to work immediately.
    </TabItem>
    <TabItem value="cli" label="aibuddy CLI">
...
## Pin a aibuddy version in CI/CD
In CI/CD (and other automated, non-interactive environments), pin a specific version with `AIBUDDY_VERSION` to make installs reproducible and avoid 404s when downloading the aibuddy CLI binary assets if the `stable` release tag doesn’t include them.
...
If you're packaging aibuddy for a Linux distribution or creating a custom build, you can generate Unix manpages from the CLI command definitions:

```bash
just generate-manpages
```

This creates ROFF-formatted manpages in `target/man/` (e.g., `aibuddy.1`, `aibuddy-session.1`) that can be installed to `/usr/share/man/man1/` to provide offline documentation via the `man` command.

Manpage generation requires the aibuddy source repository and is intended for distribution packagers preparing packages for Fedora, Debian, and other Linux distributions. See the [generate_manpages.rs source](https://github.com/aaif-goose/goose/blob/main/crates/goose-cli/src/bin/generate_manpages.rs) for implementation details.

## Additional Resources

You can also configure Extensions to extend aibuddy's functionality, including adding new ones or toggling them on and off. For detailed instructions, visit the [Using Extensions Guide][using-extensions].
