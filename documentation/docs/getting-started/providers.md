...
aibuddy is compatible with a wide range of LLM providers, allowing you to choose and integrate your preferred model.
...
|-----------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [Amazon Bedrock](https://aws.amazon.com/bedrock/)                           | Offers a variety of foundation models, including Claude, Jurassic-2, and others. **AWS environment variables must be set in advance, not configured through `aibuddy configure`**                                           | Credential auth: `AWS_PROFILE`, or `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_REGION`<br /><br />Bearer token auth: `AWS_BEARER_TOKEN_BEDROCK` and `AWS_REGION`, `AWS_DEFAULT_REGION`, or `AWS_PROFILE` |
...
| [OpenAI](https://platform.openai.com/api-keys)                              | Provides gpt-4o, o1, and other advanced language models. Also supports OpenAI-compatible endpoints (e.g., self-hosted LLaMA, vLLM, KServe). **o1-mini and o1-preview are not supported because aibuddy uses tool calling.** | `OPENAI_API_KEY`, `OPENAI_HOST` (optional), `OPENAI_ORGANIZATION` (optional), `OPENAI_PROJECT` (optional), `OPENAI_CUSTOM_HEADERS` (optional)                                       |
...
| [Ramalama](https://ramalama.ai/)                                            | Local model using native [OCI](https://opencontainers.org/) container runtimes, [CNCF](https://www.cncf.io/) tools, and supporting models as OCI artifacts. Ramalama API is a compatible alternative to Ollama and can be used with the aibuddy Ollama provider. Supports Qwen, Llama, DeepSeek, and other open-source models. **Because this provider runs locally, you must first [download and run a model](#local-llms).**  | `OLLAMA_HOST`                                                                                                                                                                       |
| [Routstr](https://routstr.com/)                                             | OpenAI-compatible aggregator that fronts dozens of upstream providers (Anthropic, OpenAI, Google, DeepSeek, Llama, …) behind a single API. Authenticate with an `sk-...` bearer issued by your Routstr instance — payment is handled outside aibuddy.                                                                                                                                                                       | `ROUTSTR_API_KEY`, `ROUTSTR_HOST` (optional, default `https://api.routstr.com`)                                                                                                     |
...
aibuddy supports [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) agents as providers. ACP providers pass aibuddy extensions through to the agent as MCP servers.

| Provider                                                                    | Description                                                                                                                                                                                                               | Requirements                                                                                                                                                                          |
|-----------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [Claude ACP](https://github.com/agentclientprotocol/claude-agent-acp) (`claude-acp`) | Uses Claude Code via ACP. Passes aibuddy extensions to the agent as MCP servers. | `npm install -g @agentclientprotocol/claude-agent-acp`, active Claude Code subscription |
| [Codex ACP](https://github.com/agentclientprotocol/codex-acp) (`codex-acp`) | Uses OpenAI Codex via ACP. Passes aibuddy extensions to the agent as MCP servers. | `npm install -g @agentclientprotocol/codex-acp`, active ChatGPT Plus/Pro subscription or OpenAI API credits |
...
To configure your chosen provider, see available options, or select a model, visit the `Models` tab in aibuddy Desktop or run `aibuddy configure` in the CLI.

<Tabs groupId="interface">
  <TabItem value="ui" label="aibuddy Desktop" default>
  **First-time users:**

  On the welcome screen the first time you open aibuddy, you have these options:
  ...
  <Tabs groupId="setup">
    ...
    3. aibuddy will automatically detect your provider and configure the connection.
    ...
    2. aibuddy will open a browser window for you to sign in with the credentials of your active ChatGPT Plus or Pro subscription.
    3. Authorize aibuddy to access your ChatGPT subscription.
    4. When you return to aibuddy Desktop, you're ready to begin your first session.
    </TabItem>
    ...
    You'll receive $10 in free credits the first time you automatically authenticate with Tetrate through aibuddy. This offer is available to both new and existing Tetrate users.
    :::
    1. Choose `Agent Router by Tetrate`.
    2. aibuddy will open a browser window for you to authenticate with Tetrate, or create a new account if you don't have one already.
    3. When you return to aibuddy Desktop, you're ready to begin your first session.
    </TabItem>

    <TabItem value="openrouter" label="OpenRouter">
    1. Choose `Automatic setup with OpenRouter`.
    2. aibuddy will open a browser window for you to authenticate with OpenRouter, or create a new account if you don't have one already.
    3. When you return to the aibuddy Desktop, you're ready to begin your first session.
    </TabItem>

    <TabItem value="others" label="Other Providers">
    1. If you have a specific provider you want to use with aibuddy, and an API key from that provider, choose `Other Providers`.
  ...
  **To start over with provider and model configuration:**
  ...
  :::note
  `aibuddy configure` doesn't support entering custom model names. To use a model not in the provider's list, use aibuddy Desktop or edit the `AIBUDDY_MODEL` variable in your [`config.yaml`](/docs/guides/config-files) directly.
  :::

  :::tip
  Set the model for an individual session using the [`run` command](/docs/guides/aibuddy-cli-commands#run-options):

  ```bash
  aibuddy run --model claude-sonnet-4-0 -t "initial prompt"
  ```
  :::
...
Create custom providers to connect to services that aren't [already supported](#available-providers) or customize how you connect to them. Custom providers appear in aibuddy's provider list and can be selected like any other provider.
...
<Tabs groupId="interface">
  <TabItem value="ui" label="aibuddy Desktop" default>
    ...
    :::info Custom Headers
    Currently, custom headers can't be defined in aibuddy Desktop. As a workaround, edit the provider configuration file after creation.
    :::
  ...
  <TabItem value="cli" label="aibuddy CLI">
    ...
    4. Follow the prompts to enter the provider details:
       ...
       - **Authentication Required**: Answer "Yes" if your provider needs an API key, or "No" if authentication is not required
         - If Yes: Choose how aibuddy should obtain the credential:
           ...
           - **Command (refreshable)**: You'll be prompted for a **command** (and optional arguments) that aibuddy runs to fetch the credential, plus a **refresh interval** in seconds. Use this for short-lived credentials issued by an IdP or key vault, so aibuddy can refresh them automatically instead of requiring a restart when they expire. See [Command-Based Authentication](#command-based-authentication) below.
  ...
  <TabItem value="config" label="Config File">
    ...
    - macOS/Linux: `~/.config/aibuddy/custom_providers/`
    - Windows: `%APPDATA%\Block\aibuddy\config\custom_providers\`
    ...
    aibuddy session start --provider custom_corp_api
    ```

    :::tip Keychain Key Storage
    If you want to store the API key in the `aibuddy` keychain, update the provider in aibuddy Desktop and enter the key. This provides secure, persistent storage and allows aibuddy to connect natively to the provider.
    :::
...
Instead of a static `api_key_env`, a custom provider can be configured to run a command to obtain its credential. This is useful for short-lived credentials issued by an IdP or key vault: aibuddy re-runs the command to refresh the credential instead of requiring a restart when it expires.
...
- **`cwd`** (optional): Working directory for the command, and the base a relative `command` path is resolved against. Defaults to aibuddy's current directory.

The command's trimmed standard output is used as the credential. It must exit successfully and print a non-empty value; on failure, aibuddy surfaces an error rather than silently reusing a stale credential. The command inherits aibuddy's full environment, since the same user configures aibuddy and writes the script.

<Tabs groupId="interface">
  <TabItem value="cli" label="aibuddy CLI" default>

    In `aibuddy configure`, choose **Command (refreshable)** when prompted for how to obtain credentials for a custom provider (see [above](#configure-custom-provider)).
...
Your changes are available in your next aibuddy session.
...
<Tabs groupId="interface">
  ...
  <TabItem value="config" label="Config File">
    ...
    :::tip
    If the provider's API key is stored in the keychain, use aibuddy CLI to remove the custom provider. This also removes the stored API key.
    :::
...
## Using aibuddy for Free

aibuddy is a free and open source AI agent that you can start using right away, but not all supported [LLM Providers][providers] provide a free tier.

Below, we outline a couple of free options and how to get started with them.

:::warning Limitations
These free options are a great way to get started with aibuddy and explore its capabilities. However, you may need to upgrade your LLM for better performance.
:::


### Groq
Groq provides free access to open source (open weight) models with high-speed inference. To use Groq with aibuddy, you need an API key from [Groq Console](https://console.groq.com/keys).
...
To set up Groq with aibuddy, follow these steps:
...
### EmpirioLabs AI
[EmpirioLabs AI](https://empiriolabs.ai/) provides access to frontier open and proprietary chat models through a single OpenAI-compatible API with streaming. To use EmpirioLabs with aibuddy, you need an API key from [EmpirioLabs](https://platform.empiriolabs.ai/dashboard/api-keys).
...
The full live catalog is available at `https://api.empiriolabs.ai/v1/models`. For the complete list of EmpirioLabs models configured in aibuddy, see [empiriolabs.json](https://github.com/aaif-goose/goose/blob/main/crates/goose/src/providers/declarative/empiriolabs.json). For more details, see the [EmpirioLabs documentation](https://docs.empiriolabs.ai).

To set up EmpirioLabs with aibuddy, follow these steps:
...
### FuturMix
[FuturMix](https://futurmix.ai/) is a unified AI gateway providing access to models from Anthropic, Google, OpenAI, and DeepSeek through an OpenAI-compatible API. To use FuturMix with aibuddy, you need an API key from [FuturMix](https://futurmix.ai/).
...
To set up FuturMix with aibuddy, follow these steps:
...
### Novita AI
[Novita AI](https://novita.ai/) provides access to 90+ open-source models via an OpenAI-compatible API with competitive pricing. To use Novita AI with aibuddy, you need an API key from [Novita AI](https://novita.ai/settings#key-management).
...
To set up Novita AI with aibuddy, follow these steps:
...
### Routstr
[Routstr](https://routstr.com/) is an OpenAI-compatible aggregator that fronts dozens of upstream providers behind a single API. Payment is handled by the Routstr instance itself, so all aibuddy needs is the `sk-...` bearer that instance issues you. To use Routstr with aibuddy, pick an instance (the default is `https://api.routstr.com`) and obtain an API key from its payment flow.
...
`/v1/models` is queried at configure time, so the full catalogue your Routstr instance exposes is available in the model picker. For the static defaults shipped with aibuddy, see [routstr.json](https://github.com/aaif-goose/goose/blob/main/crates/goose/src/providers/declarative/routstr.json).

To set up Routstr with aibuddy, follow these steps:
...
### SayGM
[SayGM](https://saygm.com/) provides TEE-backed private inference via an OpenAI-compatible API. Model routing and prices are determined at runtime per request. To use SayGM with aibuddy, you need an API key from [SayGM](https://saygm.com/).
...
To set up SayGM with aibuddy, follow these steps:
...
### Google Gemini
Google Gemini provides a free tier. To start using the Gemini API with aibuddy, you need an API Key from [Google AI studio](https://aistudio.google.com/app/apikey).

To set up Google Gemini with aibuddy, follow these steps:
...
aibuddy is a local AI agent, and by using a local LLM, you keep your data private, maintain full control over your environment, and can work entirely offline without relying on cloud access. However, please note that local LLMs require a bit more set up before you can use one of them with aibuddy.

:::warning Limited Support for models without tool calling
aibuddy extensively uses tool calling, so models without it can only do chat completion. If using models without tool calling, all aibuddy [extensions must be disabled](/docs/getting-started/using-extensions#enablingdisabling-extensions).
:::
...
<Tabs groupId="local-llms">
    ...
    <Tabs groupId="ollama-models">
      <TabItem value="ramalala" label="Ramalala">
        ...
        2. In a terminal, run any Ollama [model supporting tool-calling](https://ollama.com/search?c=tools) or [GGUF format HuggingFace Model](https://huggingface.co/search/full-text?q=%22tools+support%22+%2B+%22gguf%22&type=model):
          ...
          The `--runtime-args="--jinja"` flag is required for Ramalama to work with the aibuddy Ollama provider.
          ...
          3. In a separate terminal window, configure with aibuddy:
          ...
          5. Choose `Ollama` as the model provider since Ramalama is API compatible and can use the aibuddy Ollama provider
          ...
          :::tip Context Length
          If you notice that aibuddy is having trouble using extensions or is ignoring [.aibuddyhints](/docs/guides/context-engineering/using-aibuddyhints), it is likely that the model's default context length of 2048 tokens is too low. Use `ramalama serve` to set the `--ctx-size, -c` option to a [higher value](https://github.com/containers/ramalama/blob/main/docs/ramalama-serve.1.md#--ctx-size--c).
          :::

      </TabItem>
      <TabItem value="deepseek" label="DeepSeek-R1">
        The native `DeepSeek-r1` model doesn't support tool calling, however, we have a [custom model](https://ollama.com/michaelneale/deepseek-r1-goose) you can use with aibuddy.
        ...
        ```sh
        ollama run michaelneale/deepseek-r1-aibuddy
        ```
        ...
        │
        ◇  Enter a model from that provider:
        │  michaelneale/deepseek-r1-aibuddy
        │
        ◇  Welcome! You're all set to explore and utilize my capabilities. Let's get started on solving your problems together!
        │
        └  Configuration saved successfully
  ...
  <TabItem value="lmstudio" label="LM Studio">
    ...
    4. Configure aibuddy to use LM Studio:
    ...
    :::tip Model Name
    Make sure the model name you enter in aibuddy matches the model identifier shown in LM Studio's server panel.
    :::
  </TabItem>
  <TabItem value="atomic-chat" label="Atomic Chat">
    ...
    4. Configure aibuddy to use Atomic Chat:
    ...
    :::tip Model Name
    Make sure the model name you enter in aibuddy matches the model identifier shown for your server in Atomic Chat. If the API listens on a different origin than `http://localhost:1337`, set `ATOMIC_CHAT_HOST` in aibuddy to match (scheme, host, and port only).
    :::
  </TabItem>
  <TabItem value="docker" label="Docker Model Runner" default>
    ...
    4. Configure aibuddy to use Docker Model Runner, using the OpenAI API compatible endpoint:
    ...
    9. Finally configure the model available in Docker Model Runner to be used by aibuddy: `hf.co/unsloth/gemma-3n-e4b-it-gguf:q6_k`
...
aibuddy ignores reserved request fields it already manages, such as `model`, `messages`, `stream`, and `stream_options`. Other OpenRouter-specific top-level fields are passed through the shared OpenAI-compatible request parameter handling.

## GitHub Copilot Authentication

GitHub Copilot uses a device flow for authentication, so no API keys are required:

1. Run [`aibuddy configure`](#configure-provider-and-model) and select **GitHub Copilot**
...
5. When you return to aibuddy, GitHub Copilot will be available as a provider in both CLI and Desktop.

## Azure OpenAI Authentication

aibuddy supports three authentication methods for Azure OpenAI:

1. **Entra ID Bearer Token** - Uses a pre-acquired Microsoft Entra access token from `AZURE_OPENAI_AD_TOKEN`, sent as `Authorization: Bearer <token>`. aibuddy skips Azure CLI and token acquisition entirely, which suits enterprise deployments where only short-lived tokens are exposed to the runtime (e.g. obtained via `az account get-access-token --resource https://cognitiveservices.azure.com --query accessToken --output tsv`)
...
- Configure with `aibuddy configure` and select Azure OpenAI, leaving the API key field empty

This method simplifies authentication and enhances security for enterprise environments.

## Multi-Model Configuration

Beyond single-model setups, aibuddy supports [multi-model configurations](/docs/guides/multi-model/) that can use different models and providers for specialized tasks:
...
<Tabs groupId="interface">
  ...
  <TabItem value="cli" label="aibuddy CLI">
    When you run `aibuddy configure` and select a Muse Spark model, you'll be prompted to choose a thinking effort:
    ...
    You can also set this globally with the `AIBUDDY_THINKING_EFFORT` environment variable (`off`, `low`, `medium`, `high`, or `max`).
  </TabItem>
</Tabs>
...
<Tabs groupId="interface">
  ...
  <TabItem value="cli" label="aibuddy CLI">
    **Interactive configuration:**
    ...
    When you run `aibuddy configure` and select a Gemini 3 model, you'll be prompted to choose a thinking level:
...
Some models expose their internal reasoning or "chain of thought" as part of their response. aibuddy automatically captures this reasoning output and makes it available to you. The following models and providers support reasoning output:
...
<Tabs groupId="interface">
  ...
  <TabItem value="cli" label="aibuddy CLI">
    Reasoning output is **hidden by default** in the CLI. To display it, set the `AIBUDDY_CLI_SHOW_THINKING` environment variable:
...
If you have any questions or need help with a specific provider, feel free to reach out to us on [Discord](https://discord.gg/n8R5VaWDAn) or on the [aibuddy repo](https://github.com/aaif-goose/goose).
