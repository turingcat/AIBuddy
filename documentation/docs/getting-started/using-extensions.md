...
Extensions are add-ons that provide a way to extend the functionality of aibuddy by connecting with applications and tools you already use in your workflow. These extensions can be used to add new features, access data and resources, or integrate with other systems.

Extensions are based on the [Model Context Protocol (MCP)](https://github.com/modelcontextprotocol), so you can connect
aibuddy to a wide ecosystem of capabilities.

aibuddy automatically checks external extensions for known malware before activation. If a malicious package is detected, the [extension will be blocked](/docs/troubleshooting/known-issues#malicious-package-detected) with a clear error message.

:::tip Tutorials
Check out the [step-by-step tutorials](/docs/category/mcp-servers) for adding and using various aibuddy extensions
:::

## Built-in Extensions
aibuddy includes several built-in extensions you can start using out of the box:
...
- [Memory](/docs/mcp/memory-mcp): Teaches aibuddy to remember your preferences as you use it.
- [Tutorial](/docs/mcp/tutorial-mcp): Provides interactive tutorials for learning about aibuddy.
- [Auto Visualiser](/docs/mcp/autovisualiser-mcp): Automatically generates graphical data visualizations in conversations.

:::warning Access Control
aibuddy operates autonomously by default. Combined with the Developer extension's tools, this means aibuddy can execute commands and modify files without your approval. If you want more control over this behavior, you can configure the [aibuddy permission mode](/docs/guides/managing-tools/aibuddy-permissions) and [tool permissions](/docs/guides/managing-tools/tool-permissions). See [Configuring Access Controls](/docs/mcp/developer-mcp#configuring-access-controls) for a brief overview.
:::
...
- [Top of Mind](/docs/mcp/tom-mcp): Inject persistent instructions into aibuddy's working memory every turn
...
:::info
aibuddy's built-in extensions are MCP servers in their own right. If you'd like
to use the MCP servers included with aibuddy with any other agent, you are free to do so.
:::

## Discovering Extensions

aibuddy provides a [central directory][extensions-directory] of extensions that you can install and use. 

You can also add any other [MCP Server](#mcp-servers) as a aibuddy extension, even if it's not listed in our directory.
...
You can install any MCP server as a aibuddy extension. 

Many MCP servers work better when they understand your active workspace. aibuddy supports [MCP Roots](/docs/guides/mcp-roots), which lets roots-aware extensions see your current session working directory automatically.
...
<Tabs groupId="interface">
  ...
  4. On the `Add custom extension` modal, enter the necessary details
     ...
     - The `Timeout` field lets you set how long aibuddy should wait for a tool call from this extension to complete
  5. Click `Add` button
  ...
  3. Choose the type of extension you'd like to add:
      - `Built-In Extension`: Use an extension that comes pre-installed with aibuddy.
...
Extensions can be installed using aibuddy's deep link protocol. The URL format varies based on the extension type:
...
Required parameters:
- `cmd`: The base command to run, one of `jbang`, `npx`, `uvx`, `aibuddyd`, or `docker`
...
### Config Entry
For advanced users, you can also directly edit the config file (`~/.config/aibuddy/config.yaml`) to add, remove, or update an extension:
...
- `client_secret_key`: the name of an env/secret key holding the client
  secret, resolved from `envs`/`env_keys` or aibuddy's secret store (`aibuddy
...
`AIBUDDY_OAUTH_CALLBACK_PORT` if the authorization server only allows a
...
<Tabs groupId="interface">
  ...
  1. Run the following command to open up aibuddy's configurations:
      ```sh
      aibuddy configure
      ```
...
The Smart Extension Recommendation system in aibuddy automatically identifies and suggests relevant extensions based on your tasks and needs. This section explains how to use this feature effectively and understand its capabilities and limitations.

When you request a task, aibuddy checks its enabled extensions and their tools to determine if it can fulfill the request. If not, it suggests or enables additional extensions as needed. You can also request specific extensions by name.
...
aibuddy automatically detects when an extension is needed based on your task requirements. Here's an example of how aibuddy identifies and enables a needed extension during a conversation:
...
■  aibuddy would like to enable the following extension, do you approve?
// highlight-start
...
aibuddy responds to explicit requests for extensions, allowing users to manually enable specific tools they need. Here's an example of how aibuddy handles a direct request to enable an extension:
...
aibuddy relies on extension properties to determine how to handle an extension. You can edit these properties if you want to change the extension's display settings and behavior, such as the name, timeout, or environment variables.
...
You can start a tailored aibuddy session with specific extensions directly from the CLI. 
...
```bash
aibuddy session --with-builtin "{extension_id}"
```

For example, to enable the Developer and Computer Controller extensions and start a session, you'd run:

```bash
aibuddy session --with-builtin "developer,computercontroller"
```

Or alternatively:

```bash
aibuddy session --with-builtin developer --with-builtin computercontroller
```
...
```bash
aibuddy session --with-extension "{extension command}" --with-extension "{another extension command}"
```
...
```bash
aibuddy session --with-extension "uvx mcp-server-fetch"
```
...
```bash
aibuddy session --with-extension "VAR=value command arg1 arg2"
```

For example, to start a session with the [GitHub extension](https://github.com/github/github-mcp-server), you'd run:

```bash
aibuddy session --with-extension "GITHUB_PERSONAL_ACCESS_TOKEN=<YOUR_TOKEN> npx -y @modelcontextprotocol/server-github"
```
...
```bash
aibuddy session --with-streamable-http-extension "{extension URL}" --with-streamable-http-extension "{another extension URL}"
```

For example, to start a session with a Streamable HTTP extension, you'd run:

```bash
aibuddy session --with-streamable-http-extension "https://example.com/streamable"
```

### Extensions in Containers

aibuddy can use the `--container` flag to run extensions inside your Docker containers for devcontainer workflows. See [Running Extensions in Docker Containers](/docs/tutorials/aibuddy-in-docker#running-extensions-in-docker-containers) for details.

## Developing Extensions

aibuddy extensions are implemented with MCP, a standard protocol that allows AI models and agents to securely connect with local or remote resources. Learn how to build your own [extension as an MCP server](https://modelcontextprotocol.io/quickstart/server).
