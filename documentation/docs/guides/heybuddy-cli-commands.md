...
heybuddy provides a command-line interface (CLI) with commands for managing sessions, configurations and extensions. This guide covers the main CLI commands and interactive session features.

## Flag Naming Conventions

heybuddy CLI follows consistent patterns for flag naming to make commands intuitive and predictable:
...
#### configure
Configure heybuddy settings - providers, extensions, etc.
...
:::tip Type to Filter
When selecting from menus in `heybuddy configure`, start typing to filter options in real-time. This works for lists of providers, extensions, and tools.
:::

---

#### info [options]
Shows heybuddy information, including the version, configuration file location, session storage, and logs.
...
#### version
Check the current heybuddy version you have installed.
...
#### update [options]
Update the heybuddy CLI to a newer version.

**Options:**
- **`--canary, -c`**: Update to the canary (development) version instead of the stable version
- **`--reconfigure, -r`**: Forces heybuddy to reset configuration settings during the update process
...
#### completion
Generate shell-specific scripts to enable tab completion of heybuddy commands, subcommands, and options. The script is printed to stdout, so you need to redirect it to the appropriate location for your shell and then reload or source your shell configuration.
...
heybuddy completion bash
heybuddy completion zsh
heybuddy completion fish
heybuddy completion nu
```
...
```bash
heybuddy completion fish > ~/.config/fish/completions/heybuddy.fish
```
...
mkdir $autoload_dir
heybuddy completion nu | save --force ($autoload_dir | path join "heybuddy.nu")
```

Then restart Nushell or run:
```nu
source (($nu.user-autoload-dirs | first) | path join "heybuddy.nu")
```
...
```powershell
heybuddy completion powershell | Out-String | Invoke-Expression
```
...
:::tip Testing
After installing and reloading your shell, test completion by typing `heybuddy ` and pressing Tab to see available commands, or `heybuddy session --` and Tab to see available options.
:::
...
Starting with version 1.10.0, heybuddy uses a SQLite database (`sessions.db`) instead of individual `.jsonl` files.
Your existing sessions are automatically imported to the database. Legacy `.jsonl` files remain on disk but are no longer managed by heybuddy.
:::
...
- **`--container <container_id>`**: Run extensions inside a [Docker container](/docs/tutorials/heybuddy-in-docker#running-extensions-in-docker-containers).
...
heybuddy session -n my-project

# Resume a previous session
heybuddy session --resume -n my-project
heybuddy session --resume --session-id 20251108_2
heybuddy session --resume --path ./session.json    # exported session
heybuddy session --resume --path ./session.jsonl   # legacy session storage

# Fork a specific session by name
heybuddy session --resume --fork --name my-project

# Fork the most recent session and show message history
heybuddy session --resume --fork --history

# Edit a session's conversation in your editor
heybuddy session --resume --session-id 20251108_2 --edit

# Edit and fork — create a new session from the edited conversation
heybuddy session --resume --session-id 20251108_2 --fork --edit --history

# Start with extensions
heybuddy session --with-extension "npx -y @modelcontextprotocol/server-memory"

# Name an extension explicitly (tools become memory__*, not npx__*)
heybuddy session --with-extension "memory:npx -y @modelcontextprotocol/server-memory"
heybuddy session --with-builtin developer
heybuddy session --with-streamable-http-extension "http://localhost:8080/mcp"
...
heybuddy session -n my-session --debug --max-turns 25
```
...
heybuddy session list

# List sessions in JSON format
heybuddy session list --format json

# Sort sessions by date in ascending order
heybuddy session list --ascending

# Filter sessions by working directory
heybuddy session list -w ~/projects/myapp

# List only the 10 most recent sessions
heybuddy session list --limit 10
```
...
heybuddy session remove

# Remove a specific session by ID
heybuddy session remove --session-id 20251108_3

# Remove a specific session by name
heybuddy session remove -n my-project

# Remove all sessions starting with "project-"
heybuddy session remove -r "project-.*"

# Remove all sessions containing "migration"
heybuddy session remove -r ".*migration.*"
```

:::caution
Session removal is permanent and cannot be undone. heybuddy will show which sessions will be removed and ask for confirmation before deleting.
:::
...
```bash
# Interactive export
heybuddy session export

# Export specific session as JSON for backup
heybuddy session export -n my-session --format json -o session-backup.json

# Export specific session as readable markdown
heybuddy session export -n my-session -o session.md

# Export to stdout in different formats
heybuddy session export --session-id 20251108_4 --format json
heybuddy session export -n my-session --format yaml

# Export session by path (legacy)
heybuddy session export --path ./my-session.jsonl -o exported.md
```
...
heybuddy session diagnostics --session-id 20251108_5

# Generate diagnostics for a session by name
heybuddy session diagnostics -n my-project-session

# Save diagnostics to a custom location
heybuddy session diagnostics --session-id 20251108_5 -o /path/to/my-diagnostics.json

# Interactive selection (prompts you to choose a session)
heybuddy session diagnostics
```
...
- **`-t, --text <TEXT>`**: Input text to provide to heybuddy directly
...
- **`--container <container_id>`**: Run extensions [inside a Docker container](/docs/tutorials/heybuddy-in-docker#running-extensions-in-docker-containers).
- **`--no-session`**: Run heybuddy commands without creating or storing a session file
...
heybuddy run --instructions plan.md

# Load a recipe with a prompt that heybuddy executes and then exits  
...
heybuddy run --recipe recipe.yaml --debug

# Show recipe details
heybuddy run --recipe recipe.yaml --explain

# Run a recipe with parameters
heybuddy run --recipe recipe.yaml --params environment=production --params region=us-west-2

# Run instructions from a file without session storage
heybuddy run --no-session -i instructions.txt

# Run with a specified provider and model
heybuddy run --provider anthropic --model claude-4-sonnet -t "initial prompt"

# Run with limited turns before prompting user
heybuddy run --recipe recipe.yaml --max-turns 10
```

---

#### review [options] [range]
Review the current git diff using heybuddy. By default, `heybuddy review` reviews the working tree against `HEAD`; pass a range such as `main...HEAD` to review a specific diff.

`heybuddy review` can discover review checks from `.agents/checks/*.md` and scoped review instructions from `.agents/REVIEW.md`.
...
heybuddy review main...HEAD

# Add review intent
heybuddy review --instructions "This is a refactor; flag behavior changes"

# Review only selected files
heybuddy review --files crates/heybuddy/src/agents/agent.rs documentation/docs/guides/heybuddy-cli-commands.md

# Preview the assembled prompt and discovered checks
heybuddy review --dry-run

# Run only named checks
heybuddy review --check-filter security performance --checks-only
```

---

#### recipe
Used to validate recipe files, manage recipe sharing, list available recipes, and open recipes in heybuddy desktop.
...
- **`open <RECIPE_NAME>`**: Open a recipe file directly in heybuddy desktop
  - **`-p, --param <KEY=VALUE>`**: Pre-fill recipe parameter (can be specified multiple times)
...
heybuddy recipe deeplink my-recipe.yaml

# Generate a deeplink and provide parameter values
heybuddy recipe deeplink my-recipe.yaml -p environment=production -p region=us-west-2
...
# Open a recipe in heybuddy desktop
heybuddy recipe open my-recipe.yaml

# Open a recipe by name
heybuddy recipe open my-recipe

# Open a recipe and provide parameter value
heybuddy recipe open my-recipe --param name=myproject

# Validate a recipe file
heybuddy recipe validate my-recipe.yaml

# Get help about recipe commands
heybuddy recipe help
```
...
heybuddy plugin install https://github.com/example/my-goose-plugin.git

# Install a plugin and enable automatic update checks
heybuddy plugin install --auto-update https://github.com/example/my-goose-plugin.git

# Update an installed plugin manually
heybuddy plugin update my-plugin
```
...
#### skills
List skills available to the heybuddy agent.
...
:::info
This command is available in heybuddy builds that include local inference support.
:::
...
heybuddy local-models search qwen --limit 5

# Download a model from a search result
heybuddy local-models download 'user/repo:Q4_K_M'

# List downloaded models
heybuddy local-models list

# Delete a downloaded model
heybuddy local-models delete user/repo:Q4_K_M
```
...
**Commands:**
- `add <OPTIONS>`: Create a new scheduled job. Copies the current version of the recipe to `~/.local/share/heybuddy/scheduled_recipes`
...
heybuddy schedule list

# List the 10 most recent heybuddy sessions created by a scheduled job
heybuddy schedule sessions --schedule-id daily-report -l 10

# Run a recipe immediately
heybuddy schedule run-now --schedule-id daily-report

# Remove a scheduled job
heybuddy schedule remove --schedule-id daily-report
```
...
#### acp
Run heybuddy as an Agent Client Protocol (ACP) agent server over stdio. This enables heybuddy to work with ACP-compatible clients like Zed.
...
:::info
This command is automatically invoked by ACP-compatible clients and is not typically run directly by users. The client manages the lifecycle of the `heybuddy acp` process. See [Using heybuddy in ACP Clients](/docs/guides/acp-clients) for details.
:::

---

#### serve [options]
Start heybuddy as an Agent Client Protocol (ACP) server over HTTP and WebSocket.
...
export HEYBUDDY_SERVER__SECRET_KEY=$(openssl rand -hex 32)

# Start the ACP server on localhost:3284
heybuddy serve

# Bind to a different host and port
heybuddy serve --host 0.0.0.0 --port 3284

# Start with specific built-in extensions
heybuddy serve --with-builtin developer,memory
```

:::warning
`heybuddy serve` requires `HEYBUDDY_SERVER__SECRET_KEY` unless you pass `--dangerously-unauthenticated`. Only use `--dangerously-unauthenticated` with local trusted clients.
:::
...
#### term
Set up and use terminal-integrated sessions. Terminal integration gives each shell a persistent heybuddy session through `AGENT_SESSION_ID`, and can create the `@heybuddy` and `@g` aliases.
...
- **`--default`** - Ask heybuddy to handle unknown commands in supported shells
...
# Set up zsh integration and ask heybuddy about unknown commands
eval "$(heybuddy term init zsh --default)"

# Set up nushell integration
let init = ($nu.cache-dir | path join "heybuddy-term-init.nu")
heybuddy term init nu | save --force $init
source $init

# Send a prompt to the current terminal session
heybuddy term run why did the last command fail
...
#### @heybuddy / @g
Ask heybuddy questions directly from your shell prompt, with command history included in the context. These aliases are created by `heybuddy term init` when you set up [terminal integration](/docs/guides/terminal-integration.md).

**Examples:**
```bash
# Ask questions with command history context
@heybuddy create a python script to process these files
@heybuddy create a PR description summarizing these changes
...
Once you're in an interactive session (via `heybuddy session` or `heybuddy run --interactive`), you can use these slash commands. All commands support tab completion. Press `/ + <Tab>` to cycle through available commands.

**Available Commands:**
- **`/?` or `/help`** - Display the help menu
- **`/builtin <names>`** - Add builtin extensions by name (comma-separated)
- **`/clear`** - Clear the current chat history
- **`/endplan`** - Exit plan mode and return to 'normal' heybuddy mode
- **`/exit` or `/quit`** - Exit the session
- **`/extension <command>`** - Add a stdio extension (format: ENV1=val1 command args...)
- **`/mode <name>`** - Set the heybuddy mode to use ('auto', 'approve', 'chat', 'smart_approve')
...
/clear
```
You can also create [custom slash commands for running recipes](/docs/guides/context-engineering/slash-commands) in heybuddy Desktop or the CLI. 

---

### Themes

The `/t` command controls the syntax highlighting theme for markdown content in heybuddy CLI responses. This affects the styles used for headers, code blocks, bold/italic text, and other markdown elements in the response output.
...
- The theme setting is saved to the [configuration file](/docs/guides/config-files) as `HEYBUDDY_CLI_THEME` and persists between sessions
- The saved configuration can be overridden for the session using the `HEYBUDDY_CLI_THEME` [environment variable](/docs/guides/environment-variables#session-management)

**Custom Syntax Highlighting:**

You can customize the underlying syntax highlighting theme used for code blocks by setting:
- `HEYBUDDY_CLI_LIGHT_THEME` - Theme used when in light mode (default: "GitHub")
- `HEYBUDDY_CLI_DARK_THEME` - Theme used when in dark mode (default: "zenburn")
...
The heybuddy CLI theme is independent from the heybuddy Desktop theme.
:::
...
heybuddy session --name use-custom-theme
...
- **`Ctrl+J`** - Add a newline. Can customize the character via `HEYBUDDY_CLI_NEWLINE_KEY` in the [config file](/docs/guides/config-files) (e.g. `HEYBUDDY_CLI_NEWLINE_KEY: n`) or as an [environment variable](/docs/guides/environment-variables#session-management). Avoid "c" and common terminal shortcuts like "r", "w", "z".
...
For composing longer prompts or working with complex code snippets, you can configure heybuddy to use your preferred text editor instead of CLI input. This replaces the standard CLI input and keyboard shortcuts for the entire session.

**How it works:**
1. heybuddy opens your configured editor with a template file
2. Type your prompt after the `# Your prompt:` heading (conversation history is shown below for context)
3. Save the file and close/exit the editor to send your prompt to heybuddy
4. heybuddy processes your prompt and reopens the editor with the response added to the conversation history
...
<Tabs>
  ...
  export HEYBUDDY_PROMPT_EDITOR="code --wait"
  ```
  ...
  2. Add `HEYBUDDY_PROMPT_EDITOR` and set it to your preferred editor:
  
  ```yaml
  # For terminal editors like vim or nano
  HEYBUDDY_PROMPT_EDITOR: vim

  # Or for GUI editors like VS Code (use --wait flag)
  HEYBUDDY_PROMPT_EDITOR: code --wait
  ```
...
GUI editors require a `--wait` or equivalent flag to ensure heybuddy waits for you to finish editing before continuing. Without this flag, the editor opens but heybuddy immediately proceeds as if you're done. Terminal editors like vim and nano don't need this flag.

---

### Command History Search

The `Ctrl+R` shortcut provides interactive search through your stored CLI [command history](/docs/guides/logs#command-history). This feature makes it easy to find and reuse recent commands without retyping them. When you type a search term, heybuddy searches backwards through your history for matches.

**How it works:**
1. Press `Ctrl+R` in your heybuddy CLI session
