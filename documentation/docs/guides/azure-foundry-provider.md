...
The `azure_foundry` provider connects heybuddy to Azure AI Foundry deployments. It supports two endpoint types:
...
For project endpoints, heybuddy discovers deployments with `GET /deployments`. Deployment names can be customized; heybuddy uses the returned `modelPublisher` to select the protocol and `modelName` to resolve model metadata such as the context window.

Resource endpoints do not expose project deployment discovery. heybuddy routes recognizable model or deployment names by family: `gpt-5*` and supported o-series models use Responses, `claude-*` uses Anthropic Messages, and other names use Chat Completions. Use a project endpoint when aliases do not identify their underlying model family.
...
Run `heybuddy configure`, select **Configure Providers**, and choose **Azure AI Foundry**. You can also set the variables before starting heybuddy:
...
When neither token nor key is configured, sign in with Azure CLI before starting heybuddy:
...
For a project endpoint, heybuddy routes each deployment using metadata returned by Azure:
...
The deployments API provides the deployment name and underlying `modelName`, `modelVersion`, and `modelPublisher`. heybuddy uses the underlying model name to look up a context window in its bundled model catalog. An explicit `HEYBUDDY_CONTEXT_LIMIT` or session override still takes precedence.
...
Refresh the provider model list so heybuddy can retrieve `modelPublisher`. Without deployment metadata, routing can only use recognizable model-name prefixes.
