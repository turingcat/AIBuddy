...
aibuddy provides a command-line interface (CLI) with commands for managing sessions, configurations and extensions. This guide covers the main CLI commands and interactive session features.

## Flag Naming Conventions

aibuddy CLI follows consistent patterns for flag naming to make commands intuitive and predictable:
...
#### configure
Configure aibuddy settings - providers, extensions, etc.
...
:::tip Type to Filter
When selecting from menus in `aibuddy configure`, start typing to filter options in real-time. This works for lists of providers, extensions, and tools.
:::

---

#### info [options]
Shows aibuddy information, including the version, configuration file location, session storage, and logs.
...
#### version
Check the current aibuddy version you have installed.
...
#### update [options]
Update the aibuddy CLI to a newer version.

**Options:**
- **`--canary, -c`**: Update to the canary (development) version instead of the stable version
- **`--reconfigure, -r`**: Forces aibuddy to reset configuration settings during the update process
...
#### completion
Generate shell-specific scripts to enable tab completion of aibuddy commands, subcommands, and options. The script is printed to stdout, so you need to redirect it to the appropriate location for your shell and then reload or source your shell configuration.
...
aibuddy completion bash
aibuddy completion zsh
aibuddy completion fish
aibuddy completion nu
```
...
```bash
aibuddy completion fish > ~/.config/fish/completions/aibuddy.fish
```
...
mkdir $autoload_dir
aibuddy completion nu | save --force ($autoload_dir | path join "aibuddy.nu")
```

Then restart Nushell or run:
```nu
source (($nu.user-autoload-dirs | first) | path join "aibuddy.nu")
```
...
```powershell
aibuddy completion powershell | Out-String | Invoke-Expression
```
...
:::tip Testing
After installing and reloading your shell, test completion by typing `aibuddy ` and pressing Tab to see available commands, or `aibuddy session --` and Tab to see available options.
:::
...
Starting with version 1.10.0, aibuddy uses a SQLite database (`sessions.db`) instead of individual `.jsonl` files.
Your existing sessions are automatically imported to the database. Legacy `.jsonl` files remain on disk but are no longer managed by aibuddy.
:::
...
- **`--container <container_id>`**: Run extensions inside a [Docker container](/docs/tutorials/aibuddy-in-docker#running-extensions-in-docker-containers).
...
aibuddy session -n my-project

# Resume a previous session
aibuddy session --resume -n my-project
aibuddy session --resume --session-id 20251108_2
aibuddy session --resume --path ./session.json    # exported session
aibuddy session --resume --path ./session.jsonl   # legacy session storage

# Fork a specific session by name
aibuddy session --resume --fork --name my-project

# Fork the most recent session and show message history
aibuddy session --resume --fork --history

# Edit a session's conversation in your editor
aibuddy session --resume --session-id 20251108_2 --edit

# Edit and fork — create a new session from the edited conversation
aibuddy session --resume --session-id 20251108_2 --fork --edit --history

# Start with extensions
aibuddy session --with-extension "npx -y @modelcontextprotocol/server-memory"

# Name an extension explicitly (tools become memory__*, not npx__*)
aibuddy session --with-extension "memory:npx -y @modelcontextprotocol/server-memory"
aibuddy session --with-builtin developer
aibuddy session --with-streamable-http-extension "http://localhost:8080/mcp"
...
aibuddy session -n my-session --debug --max-turns 25
```
...
aibuddy session list --format json

# Sort sessions by date in ascending order
aibuddy session list --ascending

# Filter sessions by working directory
aibuddy session list -w ~/projects/myapp

# List only the 10 most recent sessions
aibuddy session list --limit 10
```
...
aibuddy session remove

# Remove a specific session by ID
aibuddy session remove --session-id 20251108_3

# Remove a specific session by name
aibuddy session remove -n my-project

# Remove all sessions starting with "project-"
aibuddy session remove -r "project-.*"

# Remove all sessions containing "migration"
aibuddy session remove -r ".*migration.*"
```

:::caution
Session removal is permanent and cannot be undone. aibuddy will show which sessions will be removed and ask for confirmation before deleting.
:::
...
```bash
# Interactive export
aibuddy session export

# Export specific session as JSON for backup
aibuddy session export -n my-session --format json -o session-backup.json

# Export specific session as readable markdown
aibuddy session export -n my-session -o session.md

# Export to stdout in different formats
aibuddy session export --session-id 20251108_4 --format json
aibuddy session export -n my-session --format yaml

# Export session by path (legacy)
aibuddy session export --path ./my-session.jsonl -o exported.md
```
...
aibuddy session diagnostics --session-id 20251108_5

# Generate diagnostics for a session by name
aibuddy session diagnostics -n my-project-session

# Save diagnostics to a custom location
aibuddy session diagnostics --session-id 20251108_5 -o /path/to/my-diagnostics.json
...
- **`-t, --text <TEXT>`**: Input text to provide to aibuddy directly
...
- **`--container <container_id>`**: Run extensions [inside a Docker container](/docs/tutorials/aibuddy-in-docker#running-extensions-in-docker-containers).
- **`--no-session`**: Run aibuddy commands without creating or storing a session file
...
aibuddy run --instructions plan.md

# Load a recipe with a prompt that aibuddy executes and then exits  
aibuddy run --recipe recipe.yaml

# Load a recipe and stay in an interactive session
aibuddy run --recipe recipe.yaml --interactive

# Load a recipe in debug mode
aibuddy run --recipe recipe.yaml --debug

# Show recipe details
aibuddy run --recipe recipe.yaml --explain

# Run a recipe with parameters
aibuddy run --recipe recipe.yaml --params environment=production --params region=us-west-2

# Run instructions from a file without session storage
aibuddy run --no-session -i instructions.txt
...
aibuddy run --recipe recipe.yaml --max-turns 10
```

---

#### review [options] [range]
Review the current git diff using aibuddy. By default, `aibuddy review` reviews the working tree against `HEAD`; pass a range such as `main...HEAD` to review a specific diff.

`aibuddy review` can discover review checks from `.agents/checks/*.md` and scoped review instructions from `.agents/REVIEW.md`.
...
aibuddy review main...HEAD

# Add review intent
aibuddy review --instructions "This is a refactor; flag behavior changes"

# Review only selected files
aibuddy review --files crates/aibuddy/src/agents/agent.rs documentation/docs/guides/aibuddy-cli-commands.md

# Preview the assembled prompt and discovered checks
aibuddy review --dry-run

# Run only named checks
aibuddy review --check-filter security performance --checks-only
```

---

#### recipe
Used to validate recipe files, manage recipe sharing, list available recipes, and open recipes in aibuddy desktop.
...
- **`open <RECIPE_NAME>`**: Open a recipe file directly in aibuddy desktop
  - **`-p, --param <KEY=VALUE>`**: Pre-fill recipe parameter (can be specified multiple times)
...
aibuddy recipe deeplink my-recipe.yaml

# Generate a deeplink and provide parameter values
aibuddy recipe deeplink my-recipe.yaml -p environment=production -p region=us-west-2
...
# Open a recipe in aibuddy desktop
aibuddy recipe open my-recipe.yaml

# Open a recipe by name
aibuddy recipe open my-recipe

# Open a recipe and provide parameter value
aibuddy recipe open my-recipe --param name=myproject

# Validate a recipe file
aibuddy recipe validate my-recipe.yaml
...
aibuddy plugin install https://github.com/example/my-goose-plugin.git

# Install a plugin and enable automatic update checks
aibuddy plugin install --auto-update https://github.com/example/my-goose-plugin.git

# Update an installed plugin manually
aibuddy plugin update my-plugin
```
...
#### skills
List skills available to the aibuddy agent.
...
:::info
This command is available in aibuddy builds that include local inference support.
:::
...
aibuddy local-models search qwen --limit 5

# Download a model from a search result
aibuddy local-models download 'user/repo:Q4_K_M'

# List downloaded models
aibuddy local-models list

# Delete a downloaded model
aibuddy local-models delete user/repo:Q4_K_M
```
...
**Commands:**
- `add <OPTIONS>`: Create a new scheduled job. Copies the current version of the recipe to `~/.local/share/aibuddy/scheduled_recipes`
...
**Usage:**
```bash
aibuddy schedule <COMMAND>

# Add a new scheduled recipe which runs every day at 9 AM
aibuddy schedule add --schedule-id daily-report --cron "0 0 9 * * *" --recipe-source ./recipes/daily-report.yaml

# List all scheduled jobs
aibuddy schedule list

# List the 10 most recent aibuddy sessions created by a scheduled job
aibuddy schedule sessions --schedule-id daily-report -l 10

# Run a recipe immediately
aibuddy schedule run-now --schedule-id daily-report

# Remove a scheduled job
aibuddy schedule remove --schedule-id daily-report
```
...
#### acp
Run aibuddy as an Agent Client Protocol (ACP) agent server over stdio. This enables aibuddy to work with ACP-compatible clients like Zed.
...
:::info
This command is automatically invoked by ACP-compatible clients and is not typically run directly by users. The client manages the lifecycle of the `aibuddy acp` process. See [Using aibuddy in ACP Clients](/docs/guides/acp-clients) for details.
:::

---

#### serve [options]
Start aibuddy as an Agent Client Protocol (ACP) server over HTTP and WebSocket.
...
export AIBUDDY_SERVER__SECRET_KEY=$(openssl rand -hex 32)

# Start the ACP server on localhost:3284
aibuddy serve

# Bind to a different host and port
aibuddy serve --host 0.0.0.0 --port 3284

# Start with specific built-in extensions
aibuddy serve --with-builtin developer,memory
```

:::warning
`aibuddy serve` requires `AIBUDDY_SERVER__SECRET_KEY` unless you pass `--dangerously-unauthenticated`. Only use `--dangerously-unauthenticated` with local trusted clients.
:::
...
#### term
Set up and use terminal-integrated sessions. Terminal integration gives each shell a persistent aibuddy session through `AGENT_SESSION_ID`, and can create the `@aibuddy` and `@g` aliases.
...
- **`--default`** - Ask aibuddy to handle unknown commands in supported shells
...
# Set up zsh integration and ask aibuddy about unknown commands
eval "$(aibuddy term init zsh --default)"

# Set up nushell integration
let init = ($nu.cache-dir | path join "aibuddy-term-init.nu")
aibuddy term init nu | save --force $init
source $init

# Send a prompt to the current terminal session
aibuddy term run why did the last command fail
...
#### @aibuddy / @g
Ask aibuddy questions directly from your shell prompt, with command history included in the context. These aliases are created by `aibuddy term init` when you set up [terminal integration](/docs/guides/terminal-integration.md).

**Examples:**
```bash
# Ask questions with command history context
@aibuddy create a python script to process these files
@aibuddy create a PR description summarizing these changes
...
Once you're in an interactive session (via `aibuddy session` or `aibuddy run --interactive`), you can use these slash commands. All commands support tab completion. Press `/ + <Tab>` to cycle through available commands.

**Available Commands:**
- **`/?` or `/help`** - Display the help menu
- **`/builtin <names>`** - Add builtin extensions by name (comma-separated)
- **`/clear`** - Clear the current chat history
- **`/endplan`** - Exit plan mode and return to 'normal' aibuddy mode
- **`/exit` or `/quit`** - Exit the session
- **`/extension <command>`** - Add a stdio extension (format: ENV1=val1 command args...)
- **`/mode <name>`** - Set the aibuddy mode to use ('auto', 'approve', 'chat', 'smart_approve')
...
/clear
```
You can also create [custom slash commands for running recipes](/docs/guides/context-engineering/slash-commands) in aibuddy Desktop or the CLI. 

---

### Themes

The `/t` command controls the syntax highlighting theme for markdown content in aibuddy CLI responses. This affects the styles used for headers, code blocks, bold/italic text, and other markdown elements in the response output.
...
- The theme setting is saved to the [configuration file](/docs/guides/config-files) as `AIBUDDY_CLI_THEME` and persists between sessions
- The saved configuration can be overridden for the session using the `AIBUDDY_CLI_THEME` [environment variable](/docs/guides/environment-variables#session-management)

**Custom Syntax Highlighting:**

You can customize the underlying syntax highlighting theme used for code blocks by setting:
- `AIBUDDY_CLI_LIGHT_THEME` - Theme used when in light mode (default: "GitHub")
- `AIBUDDY_CLI_DARK_THEME` - Theme used when in dark mode (default: "zenburn")
...
The aibuddy CLI theme is independent from the aibuddy Desktop theme.
:::
...
aibuddy session --name use-custom-theme
...
- **`Ctrl+J`** - Add a newline. Can customize the character via `AIBUDDY_CLI_NEWLINE_KEY` in the [config file](/docs/guides/config-files) (e.g. `AIBUDDY_CLI_NEWLINE_KEY: n`) or as an [environment variable](/docs/guides/environment-variables#session-management). Avoid "c" and common terminal shortcuts like "r", "w", "z".
...
For composing longer prompts or working with complex code snippets, you can configure aibuddy to use your preferred text editor instead of CLI input. This replaces the standard CLI input and keyboard shortcuts for the entire session.

**How it works:**
1. aibuddy opens your configured editor with a template file
2. Type your prompt after the `# Your prompt:` heading (conversation history is shown below for context)
3. Save the file and close/exit the editor to send your prompt to aibuddy
4. aibuddy processes your prompt and reopens the editor with the response added to the conversation history
...
<Tabs>
  ...
  export AIBUDDY_PROMPT_EDITOR="code --wait"
  ```
  ...
  1. Navigate to the aibuddy [configuration file](/docs/guides/config-files). For example, navigate to `~/.config/aibuddy/config.yaml` on macOS.
  2. Add `AIBUDDY_PROMPT_EDITOR` and set it to your preferred editor:
  
  ```yaml
  # For terminal editors like vim or nano
  AIBUDDY_PROMPT_EDITOR: vim

  # Or for GUI editors like VS Code (use --wait flag)
  AIBUDDY_PROMPT_EDITOR: code --wait
  ```
...
GUI editors require a `--wait` or equivalent flag to ensure aibuddy waits for you to finish editing before continuing. Without this flag, the editor opens but aibuddy immediately proceeds as if you're done. Terminal editors like vim and nano don't need this flag.

---

### Command History Search

The `Ctrl+R` shortcut provides interactive search through your stored CLI [command history](/docs/guides/logs#command-history). This feature makes it easy to find and reuse recent commands without retyping them. When you type a search term, aibuddy searches backwards through your history for matches.

**How it works:**
1. Press `Ctrl+R` in your aibuddy CLI session
