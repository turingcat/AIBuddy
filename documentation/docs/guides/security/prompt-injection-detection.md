...
Prompt injection happens when malicious instructions are hidden inside executable content. In the world of AI, prompt injection can be used to nudge AI agents (like heybuddy) to run unsafe commands that compromise your environment or data.

You can help protect your heybuddy workflows by enabling prompt injection detection. This feature uses pattern matching to detect common attack techniques, including:
...
5. **You choose** whether to proceed or cancel after reviewing the alert details. Note that:
   - Each decision is logged with its finding ID in the [heybuddy system logs](/docs/guides/logs#system-logs)
...
<Tabs groupId="interface">
  ...
  <TabItem value="config" label="heybuddy config file">
...
Beyond prompt injection detection, heybuddy automatically:
