...
A good plan keeps everyone on track and helps measure progress. That's why the heybuddy CLI includes the `/plan` prompt completion command to help break down your projects into clear, manageable steps.
 
:::tip Plans in the heybuddy Desktop
The heybuddy Desktop doesn't have a `plan` keyword. If you want heybuddy Desktop to create a plan for you, you need to use a prompt like:

```
"Hey heybuddy, can you create a plan to convert my CLI project into a locally hosted web page that gives me input fields for each CLI command I can run? Please don't start the actual work"
```
Unless you ask heybuddy to "create a plan", it might just start into the project work. 
:::

The heybuddy CLI's plan mode is interactive, asking clarifying questions to understand your project before creating a plan. If you can provide thoughtful and informative answers to those questions, heybuddy can generate a really useful and actionable plan.
...
The heybuddy CLI plan mode uses two configuration values:

- `HEYBUDDY_PLANNER_PROVIDER`: Which provider to use for planning
- `HEYBUDDY_PLANNER_MODEL`: Which model to use for planning
...
You can also customize how heybuddy creates plans by editing the `plan.md` [prompt template](/docs/guides/context-engineering/prompt-templates).
:::

### Set heybuddy planner environment variables
You might add these lines to your bash shell config file (.bashrc) to add the planner environment variables:
```bash
export HEYBUDDY_PLANNER_PROVIDER=<my-chosen-provider>
export HEYBUDDY_PLANNER_MODEL=<my-chosen-model>
```
After you save your changes to the config file, you need to re-start your heybuddy session so that heybuddy can use the variables.

If these aren't set, heybuddy will use your default provider and model settings. You might want to set different planning models if you find certain models are better at breaking down tasks into clear steps. However, your default model configuration is usually sufficient.
...
heybuddy Locations:
  Config file:      /Users/alincoln/.config/heybuddy/config.yaml
  Sessions dir:     /Users/alincoln/.local/share/heybuddy/sessions
  Logs dir:         /Users/alincoln/.local/state/heybuddy/logs

heybuddy Configuration:
  HEYBUDDY_PROVIDER: anthropic
  HEYBUDDY_MODEL: claude-3.5-sonnet
  HEYBUDDY_PLANNER_PROVIDER: openai
  HEYBUDDY_MODE: smart_approve
  HEYBUDDY_PLANNER_MODEL: gpt-4.1
```

 If either `HEYBUDDY_PLANNER_PROVIDER` or `HEYBUDDY_PLANNER_MODEL` are not set, `HEYBUDDY_PROVIDER` and `HEYBUDDY_MODEL`are used to build your plan.  

## Describe your project
While heybuddy can handle complex project descriptions, it works best with clear, concise ones. Focus on stating your project's purpose and desired outcomes. If these aren't clear, heybuddy will ask clarifying questions until it fully understands your goals. 

## A simple construction plan example
heybuddy can produce good plans for relatively simple projects such as the home construction example:
...
```
The home construction plan remains high-level because heybuddy's current models specialize in technology and software development rather than construction. This is why projects like our first example—building a web application—receive more detailed planning and specific guidance.
...
- **Architectural Design**: Hire an architect to design the house, ensuring it meets your space requirements and is energy efficient. Consider factors like natural lighting, insulation, and window placement.
```
If you exit plan mode while reviewing your construction plan, you can always resume it to continue working with heybuddy. 
...
After gathering information through clarifying questions, heybuddy creates a detailed plan for hiring an architect. This sub-plan integrates with the larger home construction project, with steps that reflect and support the overall construction context.

## A development project example
In this example, a developer has written a CLI in Python that interacts with the Contentful CMS to let a user search for strings and replace them with new strings. As a website, the search/replace feature would be more usable and also allow for a larger set of features. The developer is using the heybuddy CLI to plan the conversion project.

If heybuddy believes the project can be completed in many different ways and using a wide variety of components, it will ask you a clarifying question for each of these decision points. For example, if you start a plan like this:
...
```
heybuddy parses your project description, consults with the LLM mode you've configured, and then if it needs more information, starts a round of clarifying questions.

## Clarifying questions
Converting a Python CLI into a website seems simple enough but heybuddy will have questions about things like styling, authentication, features, technology stack, and more. You might see questions like this:
...
:::tip
When heybuddy requests a project artifact like source code during plan mode, you'll need to paste the content directly into the chat. Simply copying the file contents and prefixing it with a brief description like 'Here's the requested code:' is sufficient. Note that providing just a file path won't work in plan mode.
:::

When answering multiple questions, number your responses to match each question. For example, instead of answering with a simple 'no' or 'don't remember', provide context like '2. Do not store my preferences.' This helps heybuddy track which questions have been answered and prevents repeated questions.

In complex projects like converting a CLI to a website, heybuddy may ask multiple rounds of clarifying questions. Each round typically stems from new information in your previous answers or when additional details are needed about specific aspects of your project.

If you've answered _all_ of heybuddy's questions and it has no more questions, heybuddy will generate the plan. Other times, you might think heybuddy will never run out of questions. If you want your plan and don't want to answer more questions, you can simply ask for a "generic" plan:
...
```
While heybuddy creates a standardized plan format, it customizes the content based on your answers. heybuddy can generate the code needed to implement the steps of the plan it produces. You should review the plan and any code that it generates before ending plan mode (`/endplan`) and asking heybuddy to implement the plan. 
...
## Basic usage
You need to have an active heybuddy session before you can put the CLI into plan mode. If you are going to dedicate a session to creating a plan, you should give your new session a name as in the following example:

```bash
~ heybuddy session -n web-project-plan
starting session | provider: databricks model: databricks-meta-llama
    session id: 20251110_5
    working directory: /Users/alincoln

heybuddy is running! Enter your instructions, or try asking what heybuddy can do.
```
...
```
 ...
 Plan mode in the CLI is a special interaction mode where heybuddy helps break down tasks into manageable steps.  If you want to close the plan mode and return to the active session, type `/endplan`.
