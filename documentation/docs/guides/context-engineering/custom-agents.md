...
Custom agents are reusable aibuddy configurations for specific roles, behaviors, or areas of expertise. Each agent packages a name, description, optional model preference, and instructions so you can quickly ask aibuddy to work with a specialized role such as code reviewer, documentation writer, test planner, or release assistant.
...
Global agents are available across aibuddy sessions:

```text
~/.agents/agents/
```

Project agents are available when aibuddy is working in that project:
...
aibuddy also discovers agents from `.aibuddy/agents/`, `.claude/agents/`, `~/.aibuddy/agents/`, `~/.claude/agents/`, aibuddy's platform-specific config agents directory, and project-local `.agents/agents/`. New shared agents should use `.agents/agents/` for project agents or `~/.agents/agents/` for global agents.
:::
...
After you create an agent file, start aibuddy from a working directory where the agent can be discovered. Project agents are discovered from the current working directory, while global agents are discovered from your home/config directories.

### List available agents

In a aibuddy chat session, ask aibuddy to list available sources:

```text
list available sources
```

This is a prompt to aibuddy, not a terminal command. When source loading is available, aibuddy lists discoverable agents alongside recipes and subrecipes.
...
| Use | Best fit |
|---|---|
| Change aibuddy's role, tone, or instructions for a task | Custom agent |
| Teach aibuddy a reusable workflow or domain-specific procedure it can load on demand | [Skill](/docs/guides/context-engineering/using-skills) |
| Package a repeatable task with prompts, settings, extensions, and parameters | [Recipe](/docs/guides/recipes) |
| Delegate work to another isolated aibuddy instance | [Subagent](/docs/guides/context-engineering/subagents) |

Agents define who aibuddy should be for a task. Skills and recipes define what aibuddy should know or do.
...
No. A custom agent defines who aibuddy should be for a task: its role, behavior, instructions, and optional model preference. It does not define a step-by-step workflow. Use a recipe when you need repeatable steps, parameters, extension configuration, or scheduled execution.

### Can custom agents use skills?

Yes. A custom agent can use skills that are available in the session. Skills are still discovered and loaded through aibuddy's normal skill behavior, so the agent can use them when your request matches a skill or when you explicitly ask for one.

### Can custom agents run recipes?

A custom agent does not contain or automatically run a recipe. You can start aibuddy with a recipe, ask aibuddy to load or delegate a recipe when the relevant tools are available, or create a recipe that uses custom-agent instructions as part of its workflow.
...
Yes, when delegation tools are available in the session. A custom agent can ask aibuddy to delegate work to a subagent just like the default aibuddy agent can. Delegated subagents run in isolated sessions and do not automatically inherit the full parent conversation.
