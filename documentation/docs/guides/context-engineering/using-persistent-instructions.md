...
Persistent instructions let you inject text into aibuddy's working memory every turn. Unlike [`.aibuddyhints`](/docs/guides/context-engineering/using-aibuddyhints), which are loaded at session start and can expand later when aibuddy discovers nested hint files, persistent instructions are re-read and injected fresh with every interaction. This makes them ideal for behavioral guardrails that must always be enforced, regardless of how the conversation evolves.

## How It Works

aibuddy has a component called MOIM (Model-Observed Internal Memory) that provides contextual information to the model every turn. This includes things like the current timestamp, working directory, and your todo list. Persistent instructions are injected into this same context, placing your reminders in the model's immediate attention window.
...
|----------|---------|---------|
| [`AIBUDDY_MOIM_MESSAGE_TEXT`](/docs/guides/environment-variables#session-management) | Literal text injected into working memory each turn | Not set |
| [`AIBUDDY_MOIM_MESSAGE_FILE`](/docs/guides/environment-variables#session-management) | Path to a file whose contents are injected each turn. Supports `~/` | Not set |
...
For short, single-purpose reminders, use `AIBUDDY_MOIM_MESSAGE_TEXT`:
...
```bash
export AIBUDDY_MOIM_MESSAGE_TEXT="CRITICAL: This is a production environment. Be extra careful."
export AIBUDDY_MOIM_MESSAGE_FILE="~/.aibuddy/guardrails.md"
```
...
```bash
export AIBUDDY_MOIM_MESSAGE_TEXT="SECURITY: Do not upload code to external services, create public gists, or share sensitive data. All code in this repository is confidential."
```
...
export AIBUDDY_MOIM_MESSAGE_TEXT="⚠️ PRODUCTION: Double-check all commands. Prefer read-only operations. Always create backups before modifications."

# Development environment  
export AIBUDDY_MOIM_MESSAGE_TEXT="Development environment. Feel free to experiment, but run tests before committing."
```
...
```bash
export AIBUDDY_MOIM_MESSAGE_TEXT="This project uses pnpm, not npm. Always use 'pnpm' for package management commands."
```
...
export AIBUDDY_MOIM_MESSAGE_TEXT="Current focus: Refactoring the authentication module. Don't get sidetracked."

# Clear it when done
unset AIBUDDY_MOIM_MESSAGE_TEXT
```

## Persistent Instructions vs aibuddyhints

| Feature | Persistent Instructions | [aibuddyhints](/docs/guides/context-engineering/using-aibuddyhints) |
|---------|------------------------|-------------|
...
You can use both together: aibuddyhints for project context and persistent instructions for critical guardrails.
...
4. **Use files for complex rules**: If you have multiple guidelines, organize them in a file rather than cramming everything into `AIBUDDY_MOIM_MESSAGE_TEXT`.

5. **Test your guardrails**: After setting up persistent instructions, test that aibuddy respects them by asking it to do something that should be blocked.
