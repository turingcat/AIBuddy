...
heybuddy can autonomously decide to use subagents when it determines they would be beneficial for your task - you don't always need to explicitly request them. This happens automatically in autonomous [permission mode](/docs/guides/managing-tools/heybuddy-permissions) (the default). Subagents are disabled in manual approval, smart approval, and chat-only modes.
:::

To use subagents, ask heybuddy to delegate tasks using natural language. heybuddy automatically decides when to spawn subagents and handles their lifecycle. You can:
...
When heybuddy delegates work to a subagent, you can see the subagent's tool calls in real-time. This transparency helps you understand what the subagent is doing and verify it's on the right track.
...
Internal subagents spawn heybuddy instances to handle tasks using your current session's context and extensions. There are two ways to configure and execute internal subagents:
...
**Place your recipe file where heybuddy can find it**
- Set [`HEYBUDDY_RECIPE_PATH`](/docs/guides/recipes/recipe-reference#recipe-location) environment variable to your recipe directory
...
External subagents let you bring in AI agents from other providers and platforms, enabling heybuddy to coordinate and integrate your workflow with the broader ecosystem. In the below example, we use Codex as a subagent by running it as an MCP server:

**[heybuddy Configuration File](/docs/guides/config-files)** (`.~/.config/heybuddy/config.yaml `):
```yaml
subagent:
  args:
  - mcp-server
...
1. **Core Agent System** (`crates/heybuddy/src/agents/`)
   - Agent orchestration and session management
   - Tool execution framework
   - Extension system integration

2. **CLI Interface** (`crates/heybuddy-cli/`)
   - Command-line interface and session handling
   - Configuration management

3. **ACP Server** (`crates/heybuddy/src/acp/`)
...
|-----------|---------|------------------|
| **Max Turns** | 25 | Use natural language, set `HEYBUDDY_SUBAGENT_MAX_TURNS`, or configure `settings.max_turns` in [recipes](/docs/guides/recipes/recipe-reference#settings) or subagent tool calls |
...
**Environment variable:** Set `HEYBUDDY_SUBAGENT_MAX_TURNS` to change the default max turns for all subagents.
...
### Return Mode Control
Choose how much information heybuddy provides from its subagents in your main session.
...
<ContentCardCarousel
  items={[
    ...
    },
    {
      ...
      description: 'Deep dive into heybuddy subagents. Walk through building an app using 6 specialized AI agents for advanced workflow automation and development.',
    ...
    },
    {
      ...
      description: 'Discover the differences between subagents and subrecipes for efficient task execution in heybuddy. Learn which approach is best for your workflow.',
    ...
    {
      type: 'blog',
      title: 'How to Choose Between Subagents and Subrecipes in heybuddy',
      description: 'Detailed guide to subagents and subrecipes in heybuddy. Compare reusability, setup complexity, and get practical advice for choosing the right approach.',
