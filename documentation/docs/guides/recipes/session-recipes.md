...
description: "Share a aibuddy session setup (including tools, goals, and instructions) as a reusable recipe that others can launch with a single click"
---
...
Sometimes you finish a task in aibuddy and realize, "Hey, this setup could be useful again." Maybe you have curated a great combination of tools, defined a clear goal, and want to preserve that flow. Or maybe you're trying to help someone else replicate what you just did without walking them through it step by step. 

You can turn your current aibuddy session into a reusable recipe that includes the tools, goals, and setup you're using right now and package it into a new Agent that others (or future you) can launch with a single click.
...
<Tabs groupId="interface">
  ...
  <TabItem value="cli" label="aibuddy CLI">
   ...
   settings:                      # Additional settings
     aibuddy_provider: $provider    # Provider to use for this recipe
     aibuddy_model: $model          # Specific model to use for this recipe
   ...
   settings:                     
     ...
     aibuddy_model: "claude-3-7-sonnet-latest"          
     temperature: 0.7 
   parameters:
   - key: project_name
     input_type: string
...
You can customize how aibuddy generates recipes by editing the `recipe.md` [prompt template](/docs/guides/context-engineering/prompt-templates).
:::
...
<Tabs groupId="interface">
  ...
  1. Open the recipe using a direct link or manual URL entry, or from your Recipe library:
     ...
     **Manual URL Entry:**
         ...
         2. Press `Enter` and click the `Open AIBuddy.app` prompt
     ...
     **Slash Command:**
         ...
         1. Enter a [custom slash command](/docs/guides/context-engineering/slash-commands) in any aibuddy chat session
  ...
  4. The recipe automatically submits and aibuddy begins execution. If the recipe includes a [prompt](#core-components), it's sent as the first message. If not, you can click an activity bubble or send a prompt to get started.
  ...
  Using a recipe with the aibuddy CLI might involve the following tasks:
  ...
  Recipes can be stored locally on your device or in a GitHub repository. Configure your recipe repository using either the `aibuddy configure` command or [config file](/docs/guides/config-files#global-settings).
  ...
  :::
   ...
   <Tabs>
     <TabItem value="configure" label="Using aibuddy configure" default>
       ...
       │  ○ Toggle Experiment 
       // highlight-start
       │  ● aibuddy recipe github repo (aibuddy will pull recipes from this repo if not found locally.)
       // highlight-end
       └  
       ┌  aibuddy-configure 
       │
       ...
       │
       ◆  Enter your aibuddy recipe GitHub repo (owner/repo): eg: my_org/aibuddy-recipes
       // highlight-start
       │  squareup/aibuddy-recipes (default)
       // highlight-end
       └  
       ```
     ...
     <TabItem value="config" label="Using config file">
       ...
       ```yaml title="~/.config/aibuddy/config.yaml"
       AIBUDDY_RECIPE_GITHUB_REPO: "owner/repo"
       ```
   ...
   <Tabs groupId="interface">
     <TabItem value="local" label="Local Recipe" default>
       ...
       **Basic Usage** - Run once and exit (see [run options](/docs/guides/aibuddy-cli-commands#run-options) and [recipe commands](/docs/guides/aibuddy-cli-commands#recipe) for more):
       ```sh
       # Using recipe file in current directory or [`AIBUDDY_RECIPE_PATH`](/docs/guides/environment-variables#recipe-configuration) directories
       aibuddy run --recipe recipe.yaml

       # Using full path
       aibuddy run --recipe ./recipes/my-recipe.yaml
       ```

       **Preview Recipe** - Use the [`explain`](/docs/guides/aibuddy-cli-commands#run-options) command to view details before running:
       ...
       **With Parameters** - Supply parameter values when running recipes. See the [`run` command documentation](/docs/guides/aibuddy-cli-commands#run-options) for detailed examples and options.

       Basic example:
       ```sh
       aibuddy run --recipe recipe.yaml --params language=Python
       ```

       **Slash Command** - Enter a [custom slash command](/docs/guides/context-engineering/slash-commands) in any aibuddy chat session
     ...
     <TabItem value="github" label="GitHub Recipe">
       ...
       **Basic Usage** - Run recipes from your configured repo using the recipe name that matches its directory (see [run options](/docs/guides/aibuddy-cli-commands#run-options) and [recipe commands](/docs/guides/aibuddy-cli-commands#recipe) for more):

       ```sh
       aibuddy run --recipe recipe-name
       ```
       ...
       ```sh
       aibuddy run --recipe code-review
       ```
      ...
      **Preview Recipe** - Use the [`explain`](/docs/guides/aibuddy-cli-commands#run-options) command to view details before running:
       ...
       **Interactive Mode** - With parameter prompts:
       ```sh
       aibuddy run --recipe code-review --interactive
       ```
       The interactive mode will prompt for required values:
       ```sh
...
<Tabs groupId="interface">
  ...
  <TabItem value="cli" label="aibuddy CLI">
   ...
   ```sh
   aibuddy recipe validate recipe.yaml
   ```

   :::info
   If you want to validate a recipe you just created, you need to [exit the session](/docs/guides/sessions/session-management#exit-session) before running the [`validate` subcommand](/docs/guides/aibuddy-cli-commands#recipe).
   :::
...
## Share Recipe
Share your recipe with aibuddy users using a recipe link or recipe file.
...
<Tabs groupId="interface">
  ...
  <TabItem value="cli" label="aibuddy CLI">
    ...
    ```sh
    aibuddy recipe deeplink <FILE>
    ```

    You can also provide parameter values to pre-fill the `Recipe Parameters` dialog:
    ```sh
    aibuddy recipe deeplink <FILE> --param key1=value1 --param key2=value2
    ```
  </TabItem>
</Tabs>

When someone clicks the link, it will open aibuddy Desktop with your recipe configuration. They can also use your recipe link to [import a recipe](/docs/guides/recipes/storing-recipes#importing-recipes) for future use.
...
<Tabs groupId="interface">
  ...
  In aibuddy Desktop, you can export a recipe file or copy its content to share with others.
  ...
  CLI users can run a shared recipe file using `aibuddy run --recipe <FILE>` or open it directly in aibuddy Desktop with `aibuddy recipe open <FILE>`. See the [CLI Commands guide](/docs/guides/aibuddy-cli-commands#recipe) for details.
...
<Tabs groupId="interface">
  <TabItem value="ui" label="aibuddy Desktop" default>
Automate aibuddy recipes by running them on a schedule. When creating a schedule, you'll configure:
- **Name**: A descriptive name for the schedule
- **Source**: The recipe to run
- **Execution mode**: Whether the recipe runs in the background (no window, results saved) or foreground (opens window if aibuddy Desktop is running, otherwise runs in background)
- **Frequency and time**: When to run the recipe (e.g. every 20 minutes, weekly at 10 AM on Friday). Your selection is converted into a [cron expression](https://en.wikipedia.org/wiki/Cron#Cron_expression) used by aibuddy.
...
At the bottom of the `Schedule Details` page you can view the list of sessions created by the scheduled recipe and open or restore each session.
  ...
  <TabItem value="cli" label="aibuddy CLI">
  Automate aibuddy recipes by scheduling them to run with a [cron expression](https://en.wikipedia.org/wiki/Cron#Cron_expression).
  ...
  See the [`schedule` command documentation](/docs/guides/aibuddy-cli-commands.md#schedule) for detailed examples and options.
</TabItem>
</Tabs>
...
2. aibuddy calls a `final_output` tool with JSON matching your schema
3. Output is validated against the JSON schema
4. If validation fails, aibuddy receives error details and must correct the output
...
aibuddy run --recipe analysis.yaml --params project_path=./src > output.log
...
:::info
Structured output is supported in recipes run in both the aibuddy CLI and aibuddy Desktop. However, creating and editing the `json_schema` configuration must be done manually in the recipe file.
:::
...
To protect your privacy and system integrity, aibuddy excludes:

- Global and local memory  
- API keys and personal credentials  
- System-level aibuddy settings  
