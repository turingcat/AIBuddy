...
When working with [Large Language Models (LLMs)](/docs/getting-started/providers), there are limits to how much conversation history they can process at once. aibuddy provides smart context management features to help handle context and conversation limits so you can maintain productive sessions. Here are some key concepts:
...
- **Context Management**: How aibuddy handles conversations approaching these limits
- **Turn**: One complete prompt-response interaction between aibuddy and the LLM

## How aibuddy Manages Context
aibuddy uses a two-tiered approach to context management:
...
This layered approach lets aibuddy handle token and context limits gracefully.

## Automatic Compaction
aibuddy automatically compacts older parts of your conversation into a summary when approaching token limits, allowing you to maintain long-running sessions without manual intervention.
Auto-compaction is triggered by default when you reach 80% of the token limit in aibuddy Desktop and the aibuddy CLI.

Control the auto-compaction behavior with the `AIBUDDY_AUTO_COMPACT_THRESHOLD` [environment variable](/docs/guides/environment-variables.md#session-management). 
...
When you reach the auto-compaction threshold:
  1. aibuddy will automatically start compacting the conversation to make room.
  2. Once complete, you'll see a confirmation message that the conversation was compacted and summarized.
  3. Continue the session. Your previous conversation remains visible, but only the compacted conversation is included in the active context for aibuddy.

:::tip Customize Compaction
You can customize how aibuddy summarizes conversations during compaction by editing the `compaction.md` [prompt template](/docs/guides/context-engineering/prompt-templates).
:::

:::tip Tool Output Summarization
To help maintain efficient context usage, aibuddy summarizes older tool call outputs in the background while keeping recent calls in full detail. By default, aibuddy computes the cutoff from the model context limit and auto-compaction threshold. For advanced tuning, see [`AIBUDDY_TOOL_CALL_CUTOFF`](/docs/guides/environment-variables#session-management).
:::
...
<Tabs groupId="interface">
  ...
  4. Continue the session. Your previous conversation remains visible, but only the compacted conversation is included in the active context for aibuddy.
...
When auto-compaction is disabled, or if a conversation still exceeds the context limit, aibuddy offers different ways to handle it:
...
aibuddy Desktop exclusively uses summarization by compacting the conversation to manage context, preserving key information while reducing size.
...
- **Headless mode** (`aibuddy run`): Automatically summarizes (equivalent to `summarize`)

You can configure how aibuddy handles context limits by setting the `AIBUDDY_CONTEXT_STRATEGY` environment variable:

```bash
# Set automatic strategy (choose one)
export AIBUDDY_CONTEXT_STRATEGY=summarize  # Automatically summarize (recommended)
export AIBUDDY_CONTEXT_STRATEGY=truncate   # Automatically remove oldest messages
export AIBUDDY_CONTEXT_STRATEGY=clear      # Automatically clear session

# Set to prompt the user
export AIBUDDY_CONTEXT_STRATEGY=prompt
```

When you hit the context limit, the behavior depends on your configuration:

**With default settings (no `AIBUDDY_CONTEXT_STRATEGY` set)**, you'll see this prompt to choose a management option:
...
Context maxed out
--------------------------------------------------
aibuddy summarized messages for you.
```

**With `AIBUDDY_CONTEXT_STRATEGY` configured**, aibuddy will automatically apply your chosen strategy:

```sh
# Example with AIBUDDY_CONTEXT_STRATEGY=summarize
Context maxed out - automatically summarized messages.
--------------------------------------------------
aibuddy automatically summarized messages for you.

# Example with AIBUDDY_CONTEXT_STRATEGY=truncate
Context maxed out - automatically truncated messages.
--------------------------------------------------
aibuddy tried its best to truncate messages for you.

# Example with AIBUDDY_CONTEXT_STRATEGY=clear
...
## Maximum Turns
The `Max Turns` limit is the maximum number of consecutive turns that aibuddy can take without user input (default: 1000). When the limit is reached, aibuddy stops and prompts: "I've reached the maximum number of actions I can do without user input. Would you like me to continue?" If the user answers in the affirmative, aibuddy continues until the limit is reached and then prompts again.
...
This setting is stored as the `AIBUDDY_MAX_TURNS` environment variable in your [config.yaml file](/docs/guides/config-files). You can configure it using the Desktop app or CLI.
...
<Tabs groupId="interface">
    ...
    <TabItem value="cli" label="aibuddy CLI">
      ...
      │  ○ Remove Extension
      // highlight-start
      │  ● aibuddy settings (Set the aibuddy mode, Tool Output, Tool Permissions, Experiment, aibuddy recipe github repo and more)
      // highlight-end
      └ 
      ```
      ...
      │  10
        // highlight-end
      │
      └  Set maximum turns to 10 - aibuddy will ask for input after 10 consecutive actions
      ```

      :::tip
      In addition to the persistent `Max Turns` setting, you can provide a runtime override for a specific session or task via the `aibuddy session --max-turns` and `aibuddy run --max-turns` [CLI commands](/docs/guides/aibuddy-cli-commands).
      :::
...
- **100+ turns**: More suitable for complex, multi-step automation where you trust aibuddy to work independently, like "migrate this entire project from React 16 to React 18" or "implement comprehensive test coverage for this service"

Remember that even simple-seeming tasks often require multiple turns. For example, asking aibuddy to "fix the failing tests" might involve analyzing test output (1 turn), identifying the root cause (1 turn), making code changes (1 turn), and verifying the fix (1 turn).

## Token Usage
After sending your first message, aibuddy Desktop and aibuddy CLI display token usage.
...
Context limits are automatically detected based on your model name, but aibuddy provides settings to override the default limits:

| Model | Description | Best For | Setting |
|-------|-------------|----------|---------|
| **Main** | Set context limit for the main model (also serves as fallback for other models) | LiteLLM proxies, custom models with non-standard names | `AIBUDDY_CONTEXT_LIMIT` |
...
- **LiteLLM Proxy Models**: When using LiteLLM with custom model names that don't match aibuddy's patterns
...
aibuddy resolves context limits with the following precedence (highest to lowest):

1. Global environment variable (`AIBUDDY_CONTEXT_LIMIT`)
...
<Tabs groupId="interface">
  <TabItem value="ui" label="aibuddy Desktop" default>
     ...
     Model context limit overrides are not yet available in the aibuddy Desktop app.
  ...
  <TabItem value="cli" label="aibuddy CLI">
    ...
    ```bash
    export AIBUDDY_CONTEXT_LIMIT=1000
    aibuddy session
    ```
...
export AIBUDDY_PROVIDER="openai"
export AIBUDDY_MODEL="my-custom-gpt4-proxy"
export AIBUDDY_CONTEXT_LIMIT=200000  # Override the 32k default
```

## Credit Balance Monitoring

aibuddy monitors your API provider balance and warns you when credits are running low or exhausted. When this happens, you'll see an **Insufficient Credits** notification.
...
:::tip
aibuddy detects low balance conditions automatically, so you won't lose your conversation context—just add credits and pick up where you left off.
:::
...
These costs are estimates only, and not connected to your actual provider bill. The cost shown is an approximation based on token counts and public pricing data.
    ...
    <TabItem value="cli" label="aibuddy CLI">
    Show estimated cost in the aibuddy CLI by setting the `AIBUDDY_CLI_SHOW_COST` [environment variable](/docs/guides/environment-variables.md#session-management) or including it in the [configuration file](/docs/guides/config-files.md).
