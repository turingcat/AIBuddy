...
Tool permissions provide fine-grained control over how aibuddy uses different tools within extensions. This guide will help you understand and configure these permissions effectively.

## Understanding Tools and Extensions

Before diving into permissions, let's clarify the key components:

- **Extensions** are packages that add functionality to aibuddy (like Developer, Google Drive, etc.)
- **Tools** are specific functions within each extension that aibuddy can use
...
aibuddy performs best with fewer than 25 total tools enabled across all extensions. Consider enabling only the extensions you need for your current task.
:::

## Permission Levels

Tool permissions work alongside [aibuddy permission modes](/docs/guides/managing-tools/aibuddy-permissions). The mode sets the default behavior, while tool permissions let you override the behavior of specific tools.
...
<Tabs groupId="interface">
  ...
  <TabItem value="cli" label="aibuddy CLI">
    ...
    2. Select `aibuddy settings` from the menu
    ```sh
    ┌ aibuddy-configure
    │
...
3. **Task Focus**
   ...
   - Help aibuddy make better tool choices
