...
Tool permissions provide fine-grained control over how heybuddy uses different tools within extensions. This guide will help you understand and configure these permissions effectively.

## Understanding Tools and Extensions

Before diving into permissions, let's clarify the key components:

- **Extensions** are packages that add functionality to heybuddy (like Developer, Google Drive, etc.)
- **Tools** are specific functions within each extension that heybuddy can use
...
heybuddy performs best with fewer than 25 total tools enabled across all extensions. Consider enabling only the extensions you need for your current task.
:::

## Permission Levels

Tool permissions work alongside [heybuddy permission modes](/docs/guides/managing-tools/heybuddy-permissions). The mode sets the default behavior, while tool permissions let you override the behavior of specific tools.
...
<Tabs groupId="interface">
  ...
  <TabItem value="cli" label="heybuddy CLI">
    ...
    2. Select `heybuddy settings` from the menu
    ```sh
    ┌ heybuddy-configure
    │
...
3. **Task Focus**
   ...
   - Help heybuddy make better tool choices
