...
Error handling is a key performance-driving part of aibuddy. There are many ways that the non-determinism
in the LLM can introduce an error that it can in turn recover from. In a typical aibuddy session, it's expected for there
