...
description: Add OpenMetadata MCP Server as a aibuddy Extension
---
...
The [OpenMetadata MCP Server](https://open-metadata.org/mcp) extension allows aibuddy to interact directly with your OpenMetadata, enabling operations for OpenMetadata assets, glossaries, and lineage. This makes it easy to work with your metadata stored in OpenMetadata through natural language interactions.
...
Just describe what you want to accomplish in natural language, and aibuddy will help you execute the appropriate OpenMetadata operations safely and efficiently.

## Example Usage

In this example, aibuddy looks at the metadata for a Redshift table named `customers` stored in OpenMetadata, particularly its lineage. If the lineage does not show that a Databricks table named `customerinfo` is used to build `customers`, aibuddy will add it.
