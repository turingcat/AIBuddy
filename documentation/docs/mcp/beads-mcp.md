...
description: Add Beads MCP Server as a aibuddy Extension
---
...
This tutorial covers how to add the [Beads MCP Server](https://github.com/steveyegge/beads) as a aibuddy extension. Beads is a distributed, git-backed issue tracker designed for AI agents. It enables persistent task tracking with dependency management, so multiple aibuddy sessions can coordinate work on complex projects.
...
In this example, we'll use Beads to coordinate building an expense tracker web app across **multiple parallel sessions**. This demonstrates how Beads enables multiple aibuddy instances to work on the same project without conflicts.

### Overview

We'll run **4 aibuddy sessions**:
...
Create a new directory and start a aibuddy session.
...
Open a **new terminal** in the same directory and start a new aibuddy session.
...
Open a **third terminal** in the same directory and start another aibuddy session. Run this in parallel with Step 2.
