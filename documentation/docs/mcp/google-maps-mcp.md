...
description: Add Google Maps MCP Server as a heybuddy Extension
unlisted: true
---
...
This tutorial covers how to add the [Google Maps MCP Server](https://www.pulsemcp.com/servers/google-maps-docs) as a heybuddy extension to enable geocoding, place searching, distance calculations, elevation data retrieval, and directions.
...
heybuddy acts as an autonomous agent that tracks a delivery driver’s location, updates the customer in real-time, and adjusts ETAs dynamically based on Google Maps traffic data.

heybuddy can:

**Monitor Driver Location**
* heybuddy gets the driver’s GPS coordinates every few seconds.
* If the driver’s location is significantly behind schedule, heybuddy recalculates the ETA.

**Traffic-Aware ETA Adjustments**
* heybuddy queries Google Maps for live traffic conditions.
* If delays are detected, heybuddy updates the customer’s tracker with a new estimated time.

**Dynamic Customer Notifications**
* If the driver is stuck in traffic, heybuddy sends an alert to the customer: “Your driver is running late due to traffic. Updated ETA: 7:25 PM.”
* If the driver is ahead of schedule, heybuddy notifies the restaurant to prepare the food earlier.
