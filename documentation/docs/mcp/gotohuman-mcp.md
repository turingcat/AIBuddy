...
description: Add gotoHuman MCP Server as a aibuddy Extension
---
...
This tutorial covers how to add the [gotoHuman MCP Server](https://github.com/gotohuman/gotohuman-mcp-server) as a aibuddy extension to bring **human-in-the-loop approvals** into your AI workflows. With gotoHuman, aibuddy can pause and request a review before continuing, perfect for blog drafts, code reviews,compliance checks, etc.  
...
In this example, aibuddy sends a LinkedIn post draft to gotoHuman for approval using the `n8n news to post` template that was created.
...
### aibuddy Prompt
```
Send this blog draft about aibuddy to gotoHuman for review using my `n8n news to post` form. 

Include today’s date as the timestamp, these links: 
[aibuddy Docs: https://goose-docs.ai/, gotoHuman: https://gotohuman.com/], 
summarize it as ‘Introducing aibuddy’s integration with gotoHuman for human approvals,’ 
and here’s the draft:

aibuddy is an open-source AI agent that runs locally on your machine, right in your terminal or IDE. 
It connects to MCP servers like gotoHuman to extend its abilities. While aibuddy can automate tasks, 
...
Once approved, aibuddy can continue where it left off. 
...
Perfect! I've successfully sent your aibuddy blog draft to gotoHuman for review using your "n8n news to post" form. Here's what was submitted:
...
- Summary: Introducing aibuddy's integration with gotoHuman for human approvals
- Links:
    - aibuddy Docs
    - gotoHuman
- Draft: Your complete blog post about aibuddy's integration with gotoHuman
...
Now if you open the **Review Link** from aibuddy’s output, you’ll see the request waiting in your gotoHuman dashboard:  
