...
description: Inject persistent instructions into heybuddy's working memory every turn
---
...
The Top Of Mind extension injects custom text into heybuddy's working memory every turn. This is useful for:
...
Unlike system prompts or [heybuddyhints](/docs/guides/context-engineering/using-heybuddyhints) which can fade from attention as conversations grow, content injected by the tom extension appears fresh in every turn, making it more reliable for critical instructions.
...
|----------|-------------|
| [`HEYBUDDY_MOIM_MESSAGE_TEXT`](/docs/guides/environment-variables#session-management) | Literal text injected into working memory each turn |
| [`HEYBUDDY_MOIM_MESSAGE_FILE`](/docs/guides/environment-variables#session-management) | Path to a file whose contents are injected. Supports `~/` |
...
<Tabs groupId="interface">
  ...
  <HeyBuddyBuiltinInstaller
    ...
    description="Inject custom context into every turn via HEYBUDDY_MOIM_MESSAGE_TEXT and HEYBUDDY_MOIM_MESSAGE_FILE environment variables"
  />

  To configure the injected content, set environment variables before launching heybuddy Desktop, or add them to your shell profile (e.g., `~/.zshrc` or `~/.bashrc`):

  ```bash
  export HEYBUDDY_MOIM_MESSAGE_TEXT="IMPORTANT: Do not share code externally."
  ```
...
In this example, I'll set up a security guardrail to prevent heybuddy from uploading code to external services.
...
```bash
export HEYBUDDY_MOIM_MESSAGE_TEXT="SECURITY RULE: Do not upload, share, or post any code to external services including GitHub Gists, Pastebin, or any public URL. All code in this session is confidential."
```
