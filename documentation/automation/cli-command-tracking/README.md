...
Automated pipeline for detecting and documenting CLI command changes between heybuddy releases.
...
1. **Extracting** CLI structure from heybuddy binary using `--help` output (deterministic)
...
```bash
# Set the heybuddy repository path
export HEYBUDDY_REPO=/path/to/heybuddy
...
cd output && heybuddy run --recipe ../recipes/synthesize-cli-changes.yaml

# 4. Update heybuddy-cli-commands.md
cd output && heybuddy run --recipe ../recipes/update-cli-commands.yaml
```
...
│  ↓                                                               │
│  heybuddy-cli-commands.md (updated) + update-summary.md             │
└─────────────────────────────────────────────────────────────────┘
```
...
- Building heybuddy from specific git tags
...
|----------|----------|---------|-------------|
| `HEYBUDDY_REPO` | Yes (local) | - | Path to heybuddy repository root |
| `CLI_COMMANDS_PATH` | No | `$HEYBUDDY_REPO/documentation/docs/guides/heybuddy-cli-commands.md` | Full path to target doc file |
| `RELEASE_TAG` | No | - | Used by GitHub Actions to specify the new version |

**Example:**
```bash
export HEYBUDDY_REPO=/Users/you/heybuddy
# CLI_COMMANDS_PATH is auto-constructed from HEYBUDDY_REPO
```
...
```json
{
  ...
  "skip_commands": [
    {
      "name": "term",
      "reason": "Terminal integration documented via @heybuddy/@g aliases"
    }
  ]
}
```
...
Builds heybuddy from a specific git tag and extracts CLI structure using `--help` output.
...
```bash
cd output
heybuddy run --recipe ../recipes/synthesize-cli-changes.yaml
```
...
- `heybuddy-cli-commands.md` - Target documentation file (path from `CLI_COMMANDS_PATH` or `HEYBUDDY_REPO` env var)

**Outputs:**
- Updated `heybuddy-cli-commands.md` with changes applied
- `output/update-summary.md` - Summary of changes for review

**Usage:**
```bash
export CLI_COMMANDS_PATH=/path/to/heybuddy-cli-commands.md
cd output
heybuddy run --recipe ../recipes/update-cli-commands.yaml
```
...
│   ├── extract-cli-structure.sh        # Wrapper that builds heybuddy and runs Python
...
- **Process**: Builds heybuddy for both versions, extracts CLI structures, detects changes, updates documentation
- **Output**: Creates a PR with updated `heybuddy-cli-commands.md` if changes detected
...
- [CLI Commands Guide](../../docs/guides/heybuddy-cli-commands.md) - Target documentation
