...
const SCHEMA_FILE = path.join(ROOT, 'crates', 'aibuddy', 'acp-schema.json');
const META_FILE = path.join(ROOT, 'crates', 'aibuddy', 'acp-meta.json');
...
function renderDocumentation(schemas, meta, aibuddyVersion = 'Preview') {
  ...
  const output = [
    '---',
    'title: aibuddy ACP Reference',
    'sidebar_label: Reference',
    'sidebar_position: 2',
    '---',
    '',
    '# aibuddy ACP Reference',
    '',
    'This reference documents aibuddy-specific Agent Client Protocol methods. Standard ACP methods are documented by the [Agent Client Protocol specification](https://agentclientprotocol.com/).',
    '',
    `**aibuddy version:** ${code(aibuddyVersion)}`,
    '',
    '> This file is generated from `crates/aibuddy/acp-schema.json` and `crates/aibuddy/acp-meta.json`. Do not edit it manually.',
    '',
...
function main() {
  ...
  if (args.length !== 0 && args.length !== 3 && args.length !== 4) {
    throw new Error(
      'Usage: generate-acp-docs.js [schema-file meta-file output-file [aibuddy-version]]'
    );
  }
  ...
  const [
    ...
    aibuddyVersion = 'Preview',
  ] = args;
  const schemas = JSON.parse(fs.readFileSync(schemaFile, 'utf8'));
  const meta = JSON.parse(fs.readFileSync(metaFile, 'utf8'));
  const output = renderDocumentation(schemas, meta, aibuddyVersion);
