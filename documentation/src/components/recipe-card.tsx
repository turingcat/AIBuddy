...
export function RecipeCard({ recipe }: { recipe: Recipe }) {
  ...
  const handleCopyCLI = () => {
    ...
    const command = `aibuddy run --recipe documentation/src/pages/recipes/data/recipes/${recipe.id}.yaml`;
  ...
  const handleSubmitParams = () => {
    const filledParams = Object.entries(paramValues)
      .map(([key, val]) => `${key}=${val}`)
      .join(" ");
    const command = `aibuddy run --recipe documentation/src/pages/recipes/data/recipes/${recipe.id}.yaml --params ${filledParams}`;
    navigator.clipboard.writeText(command);
