#!/usr/bin/env bash
# v8-aibuddy downloads a prebuilt librusty_v8 archive into target/<profile>/gn_out
...
find target -type f -path '*/build/v8-aibuddy-*/output' -print0 |
  while IFS= read -r -d '' output; do
    while IFS= read -r dir; do
      [ -n "$dir" ] || continue
      [ -d "$dir" ] && continue
      echo "v8-aibuddy build output references missing $dir; forcing a rebuild"
