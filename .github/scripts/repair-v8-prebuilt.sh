#!/usr/bin/env bash
# v8-heybuddy downloads a prebuilt librusty_v8 archive into target/<profile>/gn_out
...
find target -type f -path '*/build/v8-heybuddy-*/output' -print0 |
  while IFS= read -r -d '' output; do
    while IFS= read -r dir; do
      [ -n "$dir" ] || continue
      [ -d "$dir" ] && continue
      echo "v8-heybuddy build output references missing $dir; forcing a rebuild"
