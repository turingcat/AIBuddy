# heybuddy-roaming trust-model audit
...
registers the `heybuddy-acp/1` protocol handler — and the only callers are
`heybuddy roam share` and `heybuddy serve --roam`. Pure clients (the browser
...
An accepted peer gets heybuddy's **full ACP surface** with a fresh agent per
connection (`heybuddy-cli/src/commands/roam_full_bridge.rs`), backed by the
