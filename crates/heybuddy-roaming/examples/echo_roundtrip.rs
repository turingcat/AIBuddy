//! Minimal end-to-end example of the `heybuddy-roaming` library API.
//!
...
//! authorized stream — without any dependency on heybuddy's agent internals. The
//! "agent" here is a trivial echo server plugged in via the [`AcpStreamServer`]
//! trait; a real consumer would call heybuddy's ACP `serve` instead (see
//! `heybuddy-cli`'s bridge).
//!
//! Run it:
//!
//! ```bash
//! cargo run -p heybuddy-roaming --example echo_roundtrip
//! ```
//!
//! It runs both ends in one process over loopback (relays disabled), so it
//! needs no network. For a real two-machine test, use the `heybuddy roam` CLI.
