//! Peer-to-peer roaming transport for heybuddy agents.
//!
//! This crate lets a heybuddy agent expose itself over the internet via
...
//!   no bearer token. An accepted peer gets heybuddy's full ACP surface.
//! * [`RoamingNode`] — owns the iroh endpoint + router, hosts agents over the
//!   `heybuddy-acp/1` ALPN, and dials remote agents.
//!
//! The crate deliberately knows nothing about heybuddy's agent internals: hosting
//! is driven through the [`AcpStreamServer`] trait, which the integration layer
//! implements by calling heybuddy's generic `acp::server::serve`. This keeps the
//! heavy iroh dependency out of the `heybuddy` core crate entirely.
