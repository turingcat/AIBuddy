...
//! remote agent. This gives `heybuddy roam list`-style visibility without any
...
/// process (e.g. `heybuddy roam list`) can read what a running `share` has seen.
...
impl Directory {
    ...
    /// flushed straight back to disk, making `heybuddy roam connections` stop
