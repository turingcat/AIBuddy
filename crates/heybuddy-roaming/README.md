...
Peer-to-peer transport for heybuddy agents, built on
[iroh](https://iroh.computer) (QUIC, using iroh's public relays for NAT
traversal).

It lets a heybuddy agent accept connections from a remote ACP client (another
heybuddy, or any other ACP client) that drives it, and lets a client
...
This crate is a **standalone library** with no dependency on `heybuddy` core (so
...
(`cargo run -p heybuddy-roaming --example echo_roundtrip`). The code that bridges
the transport to heybuddy's agent machinery lives in `heybuddy-cli` behind an optional
...
The host runs heybuddy's real ACP server; the connecting side is an ACP client.
...
- You **swap cards** and each side **accepts** the other's key. A connection
  ...
  gets heybuddy's full ACP surface. A leaked card lets no one in; there is no bearer
  token that works by possession.

## Concepts

- **`ConnectionCard`** — the shareable, non-secret identity + reachability string
  (`heybuddy+roam://…`). Encodes public key + relay URLs; exposes `fingerprint()`.
...
An iroh bidirectional stream is the byte transport for heybuddy's existing
transport-agnostic ACP `serve` / `ByteStreams` seam, so hosting reuses the ACP
server and the client reuses the ACP client.

## CLI

Exposed via `heybuddy roam` (in `heybuddy-cli`, feature `roaming`):
...
Build both with the roaming feature (`cargo build -p heybuddy-cli --features
roaming`) — no shared network, VPN, or port-forwarding needed; the public n0
relays bridge them. On **each** machine run `heybuddy roam id` and send the printed
`heybuddy+roam://…` card to the other out of band (paste it in chat, etc.).

On **machine A** (the host): `heybuddy roam peers accept '<B's card>'` then
`heybuddy roam share` (optionally `--cwd <dir>`; it defaults to where `share`
started, and the connector's own path is always ignored). On **machine B**:
`heybuddy roam peers add '<A's card>' boxA`, then either drive A interactively with
`heybuddy roam connect boxA` (a prompt that runs on A's agent — its tools, files,
shell), hand it a one-shot task with `heybuddy roam delegate boxA "what is 2+2?"`,
or `heybuddy roam delegate boxA --list-sessions` / `--session <id> "<task>"` to
...
`heybuddy roam connections` shows who connected. If session creation hangs on
macOS, prefix with `HEYBUDDY_DISABLE_KEYRING=1`.
...
heybuddy's real `serve`). `connect` is a thin ACP client UI — not a provider
...
**Acceptance is all-or-nothing.** An accepted peer gets heybuddy's full ACP
...
(`roam-delegate`) documents how to call `heybuddy roam delegate <peer> "<task>"` via
...
`heybuddy roam share` agent — iroh compiled to wasm runs *inside the browser tab*,
...
It is fully decoupled from this crate: the `heybuddy-roaming-web` wasm crate there
...
wire format here, update heybuddy-mobile in the same change — a drift will not fail
