...
```rust
use heybuddy_context_management::{summarize, Templates};
...
- `CompactionModel` — the model abstraction compaction runs against, with
  `ProviderModel` adapting any [`heybuddy-providers`](../heybuddy-providers) provider.
...
Python and Kotlin reach compaction through [`heybuddy-sdk`](../heybuddy-sdk), which
