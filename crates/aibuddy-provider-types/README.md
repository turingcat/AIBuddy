# aibuddy-provider-types

The provider contract and the conversation types that flow through it. This is the
crate to depend on if you want to implement a provider, or to work with aibuddy
messages without pulling in the whole agent.

Provider implementations live in [`aibuddy-providers`](../aibuddy-providers), which
...
| `retry` | `RetryConfig` |
| `permission`, `aibuddy_mode` | Tool-approval modes |
