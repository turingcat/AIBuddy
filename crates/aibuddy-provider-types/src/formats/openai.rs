...
use crate::model::{is_aibuddy_internal_request_param, ModelConfig};
use crate::thinking::{
...
pub fn create_request_for_model_with_options(
    ...
    if model_config.model_name.starts_with("o1-mini") {
        return Err(anyhow!(
            "o1-mini model is not currently supported since aibuddy uses tool calling and o1-mini does not support it. Please use o1 or o3 models instead."
        ));
    }
    ...
    // AIBUDDY_MAX_TOKENS) or a canonical model record has supplied a value.
    ...
    if let Some(params) = &model_config.request_params {
        if let Some(obj) = payload.as_object_mut() {
            for (key, value) in params {
                if !is_aibuddy_internal_request_param(key) && !is_reserved_request_param_key(key) {
...
/// Maps AIBuddy's effort levels to values accepted by xAI Chat Completions.
...
mod tests {
    ...
    fn test_create_request_omits_max_tokens_when_unset() -> anyhow::Result<()> {
        ...
        // lmstudio) have no canonical record and no AIBUDDY_MAX_TOKENS, so the
        ...
        fn turn_context(time: &str, turn_budget: &str) -> String {
            format!(
                "<turn-context>\n\
                 ...
                 <working-directory>/Users/me/code/aibuddy</working-directory>\n\
