...
/// Request params aibuddy consumes itself: formats that forward unknown params into
/// the payload must skip these, or the provider gets an unrecognized wire parameter.
pub fn is_aibuddy_internal_request_param(key: &str) -> bool {
    matches!(
        key,
        "thinking_effort"
...
mod tests {
    ...
    mod thinking_effort_tests {
        ...
        fn xhigh_suffix_stripped_from_model_name() {
            let _guard = env_lock::lock_env([
                ("AIBUDDY_THINKING_EFFORT", Some("low")),
                ("AIBUDDY_MAX_TOKENS", None::<&str>),
                ("AIBUDDY_TEMPERATURE", None::<&str>),
                ("AIBUDDY_CONTEXT_LIMIT", None::<&str>),
                ("AIBUDDY_TOOLSHIM", None::<&str>),
    ...
    mod with_canonical_limits {
        ...
        fn resolves_gpt_6_astra_limits_for_databricks_model_service() {
            ...
            let config = ModelConfig::new("data_workflow_tools.aibuddy.aibuddy-gpt-6-astra")
                .with_canonical_limits("databricks_v2");

            let canonical = crate::canonical::maybe_get_canonical_model(
                "databricks_v2",
                "data_workflow_tools.aibuddy.aibuddy-gpt-6-astra",
            )
    ...
    mod is_openai_reasoning_model {
        ...
        const ENV_LOCK_KEYS: [(&str, Option<&str>); 5] = [
            ("AIBUDDY_MAX_TOKENS", None),
            ("AIBUDDY_TEMPERATURE", None),
            ("AIBUDDY_CONTEXT_LIMIT", None),
            ("AIBUDDY_TOOLSHIM", None),
            ("AIBUDDY_TOOLSHIM_OLLAMA_MODEL", None),
        ];
