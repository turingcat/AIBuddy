...
The bindings layer for heybuddy, published as the heybuddy Development Kit (GDK). It
houses the shared types used for both ACP and GDK access, and exposes a
cross-language version of the heybuddy API.

With `--features uniffi` the crate compiles to native bindings for Python and
Kotlin (namespace `heybuddy` / `io.github.aaif_heybuddy`). The UniFFI surface lets
...
`HeyBuddyStreamError`), `durationMs`, and token `usage`. All three events share a
...
The PyPI package is published as `heybuddy-sdk` and imports as `heybuddy`.
...
into the package, and writes the wheel to `crates/heybuddy-sdk/python/dist/`.

## Maven package

The Maven Central artifact is published as `io.github.aaif-heybuddy:gdk` and uses
the Rust crate version from `crates/heybuddy-sdk/Cargo.toml`.
