//! ACP Client Example
//!
//! Spawns `heybuddy acp` as a child process and sends it a completion request
//! using the Agent Client Protocol over stdio.
//!
//! # Prerequisites
//!
//! You must have heybuddy built and a provider configured (`heybuddy configure`).
//!
//! # Usage
//!
//! ```bash
//! cargo run -p heybuddy-sdk --example acp_client -- "What is 2 + 2?"
//! ```
//!
//! Or with a custom heybuddy binary path:
//!
//! ```bash
//! cargo run -p heybuddy-sdk --example acp_client -- --heybuddy-bin ./target/debug/heybuddy "Explain Rust's ownership model in one sentence"
//! ```
...
use heybuddy_sdk::custom_requests::GetExtensionsRequest;
use std::path::PathBuf;
use tokio_util::compat::{TokioAsyncReadCompatExt, TokioAsyncWriteCompatExt};

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    // Parse args: [--heybuddy-bin PATH] PROMPT
    let args: Vec<String> = std::env::args().skip(1).collect();
    let (heybuddy_bin, prompt) = parse_args(&args)?;

    eprintln!("🚀 Spawning: {} acp", heybuddy_bin.display());

    let mut child = tokio::process::Command::new(&heybuddy_bin)
        .arg("acp")
        .stdin(std::process::Stdio::piped())
        .stdout(std::process::Stdio::piped())
        .stderr(std::process::Stdio::inherit())
        .spawn()
        .map_err(|e| format!("Failed to spawn '{}': {e}", heybuddy_bin.display()))?;
...
fn parse_args(args: &[String]) -> Result<(PathBuf, String), String> {
    let mut heybuddy_bin = PathBuf::from("heybuddy");
    let mut i = 0;
    ...
    while i < args.len() {
        match args[i].as_str() {
            "--heybuddy-bin" => {
                i += 1;
                heybuddy_bin = PathBuf::from(args.get(i).ok_or("--heybuddy-bin requires a value")?);
            }
            _ => break,
        }
        i += 1;
    ...
    if prompt.is_empty() {
        return Err("Usage: acp_client [--heybuddy-bin PATH] PROMPT".into());
    }

    Ok((heybuddy_bin, prompt))
}
