...
native_lib="$1"
resource_prefix="${2:-$(crates/heybuddy-sdk/scripts/maven-resource-prefix.sh)}"

case "$resource_prefix" in
  darwin-aarch64|linux-x86-64|linux-aarch64|win32-x86-64) ;;
...
bindgen="target/release/heybuddy-uniffi-bindgen"
if [ ! -x "$bindgen" ]; then
  cargo build -p heybuddy-sdk --features uniffi --release -q
fi

maven_dir="crates/heybuddy-sdk/maven"
...
rm -rf "$kotlin_dir/io/github/aaif_heybuddy" "$resources_dir/$resource_prefix"
mkdir -p "$kotlin_dir" "$resources_dir/$resource_prefix" "$resources_dir/META-INF"
cp LICENSE "$resources_dir/META-INF/LICENSE"

"$bindgen" generate \
  --library "$native_lib" \
  --config crates/heybuddy-sdk/uniffi.toml \
  --language kotlin \
  --no-format \
  --out-dir "$kotlin_dir" 2>/dev/null

python3 - "$kotlin_dir/io/github/aaif_heybuddy/heybuddy.kt" <<'PY'
import sys
