package io.github.aaif_heybuddy
...
internal object NativeLibraryLoader {
    init {
        ...
        if (System.getProperty("uniffi.component.$componentName.libraryOverride") == null) {
            ...
            val stream = NativeLibraryLoader::class.java.classLoader.getResourceAsStream(resource)
                ?: error("HeyBuddy SDK native library resource not found: $resource")
            val library = Files.createTempFile("heybuddy-sdk-", nativeLibraryFileName()).toFile()
    ...
    private fun nativeLibraryFileName(): String = when (osName()) {
        "darwin" -> "libheybuddy_sdk.dylib"
        "linux" -> "libheybuddy_sdk.so"
        "win32" -> "heybuddy_sdk.dll"
