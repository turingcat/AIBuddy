...
use crate::bindings::{HeyBuddyError, HeyBuddyStreamError, Usage};
...
pub enum RequestOutcome {
    Success,
    Failure { error: HeyBuddyStreamError },
}
...
impl RequestObserver {
    ...
    pub(crate) fn fail(&self, error: HeyBuddyError) -> HeyBuddyError {
        self.end(
            RequestOutcome::Failure {
                error: HeyBuddyStreamError::from(&error),
            },
            None,
            None,
        );
    ...
    pub(crate) fn fail_stream(&self, error: HeyBuddyStreamError) {
...
mod tests {
    ...
    fn failure_reports_typed_error() {
        let registered = RegisteredRecorder::new(Recorder::default(), false);
        let observer = RequestObserver::start(descriptor());
        observer.fail(HeyBuddyError::RateLimited {
        ...
        assert!(matches!(
            error.kind,
            crate::bindings::HeyBuddyStreamErrorKind::RateLimited
        ));
    ...
    fn end_is_emitted_at_most_once() {
        let registered = RegisteredRecorder::new(Recorder::default(), false);
        let observer = RequestObserver::start(descriptor());
        observer.fail(HeyBuddyError::Timeout {
            details: "request timed out after 5ms".to_string(),
        });
        observer.succeeded(Some(usage()), None);
        observer.fail_stream(HeyBuddyStreamError {
            kind: crate::bindings::HeyBuddyStreamErrorKind::Generic,
        ...
        assert!(matches!(
            error.kind,
            crate::bindings::HeyBuddyStreamErrorKind::Timeout
        ));
    }
