...
//! HeyBuddy providers and mirrors the provider message/tool/streaming model closely
...
use heybuddy_providers::{
    anthropic::AnthropicProviderBuilder,
    api_client::{ApiClient, AuthMethod},
    base::{MessageStream, Provider as HeyBuddyProvider},
    conversation::{
        message::{Message, MessageContent as HeyBuddyMessageContent},
        token_usage::ProviderUsage,
    },
    databricks::DatabricksProvider as HeyBuddyDatabricksProvider,
    databricks_auth::DatabricksAuth,
    databricks_v2::DatabricksV2Provider as HeyBuddyDatabricksV2Provider,
...
impl From<anyhow::Error> for HeyBuddyError {
    fn from(error: anyhow::Error) -> Self {
        Self::generic(error)
    }
}

impl From<heybuddy_providers::errors::ProviderError> for HeyBuddyError {
    fn from(error: heybuddy_providers::errors::ProviderError) -> Self {
        match error {
            heybuddy_providers::errors::ProviderError::Authentication(message) => {
                Self::Authentication { details: message }
            }
            heybuddy_providers::errors::ProviderError::ContextLengthExceeded(message) => {
                Self::ContextLengthExceeded { details: message }
            }
            heybuddy_providers::errors::ProviderError::RateLimitExceeded {
                retry_delay, ..
            } => {
            ...
            heybuddy_providers::errors::ProviderError::ServerError(message)
            | heybuddy_providers::errors::ProviderError::EndpointNotFound(message)
            | heybuddy_providers::errors::ProviderError::CreditsExhausted {
                details: message,
                ..
            } => Self::ProviderUnavailable { details: message },
            heybuddy_providers::errors::ProviderError::NetworkError(message)
                if is_timeout(&message) =>
            {
                Self::Timeout { details: message }
            }
            heybuddy_providers::errors::ProviderError::RequestFailed(message)
                if is_timeout(&message) =>
            {
                Self::Timeout { details: message }
            }
            heybuddy_providers::errors::ProviderError::ExecutionError(message)
...
impl From<serde_json::Error> for HeyBuddyError {
...
pub trait RequestLogger: Send + Sync {
    fn start(&self) -> Result<u64, HeyBuddyError>;
    fn write(&self, request_id: u64, record: String) -> Result<(), HeyBuddyError>;
}
...
impl heybuddy_providers::request_log::RequestLogger for RequestLoggerAdapter {
    fn start(
        &self,
    ) -> Result<
        Box<dyn heybuddy_providers::request_log::RequestLogHandle>,
...
impl heybuddy_providers::request_log::RequestLogHandle for RequestLogHandleAdapter {
...
#[uniffi::export]
pub fn install_request_logger(logger: Box<dyn RequestLogger>) -> Result<(), HeyBuddyError> {
    heybuddy_providers::request_log::install_logger(RequestLoggerAdapter {
        logger: Arc::from(logger),
    })
    .map_err(|error| HeyBuddyError::generic(error.to_string()))
}
...
impl ProviderMessage {
    fn to_heybuddy_message(&self) -> Result<Option<Message>, HeyBuddyError> {
        ...
        for content in &self.content {
            message = message.with_content(content.to_heybuddy_content()?);
        }
        Ok(Some(message))
    }
}
...
impl MessageContent {
    fn to_heybuddy_content(&self) -> Result<HeyBuddyMessageContent, HeyBuddyError> {
        match self {
            MessageContent::Text { text } => {
                Ok(HeyBuddyMessageContent::text(sanitize_unicode_tags(text)))
            }
            MessageContent::Image { mime_type, data } => Ok(HeyBuddyMessageContent::image(
            ...
            MessageContent::ToolRequest {
                ...
                Ok(HeyBuddyMessageContent::tool_request_with_metadata(
                    id.clone(),
                    tool_call,
                    metadata.as_ref(),
                ))
            ...
            MessageContent::ToolResult {
                ...
                Ok(HeyBuddyMessageContent::tool_response(
                    id.clone(),
                    tool_result,
                ))
            }
            MessageContent::Thinking {
                thinking,
                signature,
            } => Ok(HeyBuddyMessageContent::thinking(
                thinking.clone(),
                signature.clone(),
            )),
            MessageContent::RedactedThinking { data } => {
                Ok(HeyBuddyMessageContent::redacted_thinking(data.clone()))
            }
        }
    }
    ...
    fn from_heybuddy_content(content: &HeyBuddyMessageContent) -> Option<Self> {
        match content {
            HeyBuddyMessageContent::Text(text) => Some(MessageContent::Text {
                text: text.text.clone(),
            }),
            HeyBuddyMessageContent::Image(image) => Some(MessageContent::Image {
                mime_type: image.mime_type.clone(),
                data: base64::engine::general_purpose::STANDARD
                    .decode(&image.data)
                    .ok()?,
            }),
            HeyBuddyMessageContent::ToolRequest(request) => {
            ...
            HeyBuddyMessageContent::Thinking(thinking) => Some(MessageContent::Thinking {
                thinking: thinking.thinking.clone(),
                signature: thinking.signature.clone(),
            }),
            HeyBuddyMessageContent::RedactedThinking(redacted) => {
...
impl ProviderTool {
    fn to_heybuddy_tool(&self) -> Result<Tool, HeyBuddyError> {
...
fn parse_json_object(json: &str) -> Result<serde_json::Map<String, Value>, HeyBuddyError> {
    match serde_json::from_str(json)? {
        Value::Object(object) => Ok(object),
        _ => Err(HeyBuddyError::generic("expected a JSON object")),
    }
}
...
impl ProviderModelConfig {
    fn to_heybuddy_model_config(&self) -> Result<ModelConfig, HeyBuddyError> {
...
impl Usage {
    fn from_provider_usage(usage: &ProviderUsage) -> Result<Self, HeyBuddyError> {
        Ok(Self {
...
pub enum StreamChunk {
    ...
    ErrorChunk {
        error: HeyBuddyStreamError,
    },
}

#[derive(Debug, Clone, uniffi::Record)]
pub struct HeyBuddyStreamError {
    pub kind: HeyBuddyStreamErrorKind,
    pub message: String,
    pub retry_after_ms: Option<u64>,
}

#[derive(Debug, Clone, uniffi::Enum)]
pub enum HeyBuddyStreamErrorKind {
    RateLimited,
...
impl From<&HeyBuddyError> for HeyBuddyStreamError {
    fn from(error: &HeyBuddyError) -> Self {
        match error {
            HeyBuddyError::RateLimited {
                retry_after_ms,
                retry_after_suffix,
            } => Self {
                kind: HeyBuddyStreamErrorKind::RateLimited,
                message: format!("Rate limit exceeded{retry_after_suffix}"),
                retry_after_ms: *retry_after_ms,
            },
            HeyBuddyError::OutputTokenLimitExceeded { details } => Self {
                kind: HeyBuddyStreamErrorKind::OutputTokenLimitExceeded,
                message: details.clone(),
                retry_after_ms: None,
            },
            HeyBuddyError::ContextLengthExceeded { details } => Self {
                kind: HeyBuddyStreamErrorKind::ContextLengthExceeded,
                message: details.clone(),
                retry_after_ms: None,
            },
            HeyBuddyError::Authentication { details } => Self {
                kind: HeyBuddyStreamErrorKind::Authentication,
                message: details.clone(),
                retry_after_ms: None,
            },
            HeyBuddyError::Timeout { details } => Self {
                kind: HeyBuddyStreamErrorKind::Timeout,
                message: details.clone(),
                retry_after_ms: None,
            },
            HeyBuddyError::ProviderUnavailable { details } => Self {
                kind: HeyBuddyStreamErrorKind::ProviderUnavailable,
                message: details.clone(),
                retry_after_ms: None,
            },
            HeyBuddyError::Generic { details } => Self {
                kind: HeyBuddyStreamErrorKind::Generic,
                message: details.clone(),
                retry_after_ms: None,
            },
        }
...
fn runtime() -> Result<&'static tokio::runtime::Runtime, HeyBuddyError> {
    ...
    T: Send + 'static,
{
    ...
    (&mut task.handle).await.map_err(|error| {
        if error.is_cancelled() {
            HeyBuddyError::Timeout {
                details: "runtime task was cancelled".to_string(),
            }
        } else {
            HeyBuddyError::generic(error)
        }
    })
}

struct ProviderHandle {
    provider: Arc<dyn HeyBuddyProvider>,
}

impl ProviderHandle {
    fn new(provider: Box<dyn HeyBuddyProvider>) -> Self {
        Self {
    ...
    async fn context_limit(&self, model: ProviderModelConfig) -> Result<usize, HeyBuddyError> {
    ...
    async fn stream(
    ...
    ) -> Result<Arc<ProviderStream>, HeyBuddyError> {
        let timeout_ms = model.timeout_ms;
        let model = model.to_heybuddy_model_config()?;
        ...
        {
            Ok(Ok(stream)) => stream,
            Ok(Err(error)) => return Err(observer.fail(HeyBuddyError::from(error))),
    ...
    async fn complete(
        ...
        messages: Vec<ProviderMessage>,
        tools: Vec<ProviderTool>,
    ) -> Result<ProviderCompletion, HeyBuddyError> {
        let timeout_ms = model.timeout_ms;
        let model = model.to_heybuddy_model_config()?;
        let messages = convert_messages(messages)?;
        let tools = convert_tools(tools)?;
        ...
        let completion = ProviderCompletion {
            ...
            content: message
                .content
                .iter()
                .filter_map(MessageContent::from_heybuddy_content)
                .collect(),
    ...
    run_on_runtime(async move {
        if let Some(timeout_ms) = timeout_ms {
            tokio::time::timeout(Duration::from_millis(timeout_ms), future)
                .await
                .map_err(|_| HeyBuddyError::Timeout {
...
fn convert_messages(messages: Vec<ProviderMessage>) -> Result<Vec<Message>, HeyBuddyError> {
    messages
        .iter()
        .filter_map(|message| message.to_heybuddy_message().transpose())
        .collect()
}

fn convert_tools(tools: Vec<ProviderTool>) -> Result<Vec<Tool>, HeyBuddyError> {
    tools.iter().map(ProviderTool::to_heybuddy_tool).collect()
}
...
impl Provider {
    fn new(provider: Box<dyn HeyBuddyProvider>) -> Arc<Self> {
        Arc::new(Self {
...
impl Provider {
    ...
    pub async fn context_limit(&self, model: ProviderModelConfig) -> Result<u64, HeyBuddyError> {
    ...
    ) -> Result<CompactionSummary, HeyBuddyError> {
        let messages: Vec<Message> = messages
            .iter()
            .map(CompactionMessage::to_heybuddy_message)
            .collect();
        let templates = templates.map(Into::into).unwrap_or_default();
        let model = heybuddy_context_management::ProviderModel::new(
            self.handle.provider.clone(),
            ModelConfig::new(&model_name),
        );

        let summary = run_on_runtime(async move {
            heybuddy_context_management::summarize(&model, None, &templates, &messages).await
        })
        .await?
        .map_err(HeyBuddyError::generic)?;
...
impl CompactionMessage {
    fn to_heybuddy_message(&self) -> Message {
        let role = match self.role {
            MessageRole::User | MessageRole::Tool => Role::User,
            MessageRole::Assistant => Role::Assistant,
        };
...
impl From<CompactionTemplates> for heybuddy_context_management::Templates {
...
pub fn default_compaction_templates() -> CompactionTemplates {
    let templates = heybuddy_context_management::Templates::default();
    CompactionTemplates {
...
#[uniffi::export]
pub fn declarative_provider_from_json(json: String) -> Result<Arc<Provider>, HeyBuddyError> {
    let provider = heybuddy_providers::declarative::from_json(&json, None, EnvKeyResolver {})?;
    Ok(Provider::new(provider))
}

#[uniffi::export]
pub fn openai_default_model() -> String {
    heybuddy_providers::openai::OPEN_AI_DEFAULT_MODEL.to_string()
}

#[uniffi::export]
pub fn openai_provider(api_key: String) -> Result<Arc<Provider>, HeyBuddyError> {
...
pub fn anthropic_default_model() -> String {
    heybuddy_providers::anthropic::ANTHROPIC_DEFAULT_MODEL.to_string()
}
...
pub fn anthropic_provider(
...
) -> Result<Arc<Provider>, HeyBuddyError> {
    ...
    .with_header(
        "anthropic-version",
        heybuddy_providers::anthropic::ANTHROPIC_API_VERSION,
    )?;
...
#[uniffi::export]
pub fn groq_provider(api_key: String) -> Result<Arc<Provider>, HeyBuddyError> {
    let json = heybuddy_providers::groq::JSON.replace("${GROQ_API_KEY}", &api_key);
    let provider = heybuddy_providers::declarative::from_json(
        &json,
        None,
        StaticKeyResolver {
...
fn groq_config() -> DeclarativeProviderConfig {
    heybuddy_providers::declarative::deserialize_provider_config(heybuddy_providers::groq::JSON)
...
impl heybuddy_providers::declarative::KeyResolver for StaticKeyResolver {
...
pub fn databricks_default_model() -> String {
    heybuddy_providers::databricks::DATABRICKS_DEFAULT_MODEL.to_string()
}

#[uniffi::export]
pub fn databricks_provider(host: String, token: String) -> Result<Arc<Provider>, HeyBuddyError> {
    let retry_config = HeyBuddyDatabricksProvider::load_retry_config(|key| std::env::var(key).ok());
    let provider = HeyBuddyDatabricksProvider::new(
        host,
...
pub fn databricks_v2_default_model() -> String {
    heybuddy_providers::databricks_v2::DATABRICKS_V2_DEFAULT_MODEL.to_string()
}

#[uniffi::export]
pub fn databricks_v2_provider(host: String, token: String) -> Result<Arc<Provider>, HeyBuddyError> {
    let retry_config =
        HeyBuddyDatabricksV2Provider::load_retry_config(|key| std::env::var(key).ok());
    let provider = HeyBuddyDatabricksV2Provider::new(
        host,
        DatabricksAuth::token(token),
        retry_config,
        None,
...
#[uniffi::export]
impl ProviderStream {
    pub async fn next_chunk(&self) -> Result<Option<StreamChunk>, HeyBuddyError> {
            ...
            loop {
                ...
                let next = if let Some(timeout_ms) = timeout_ms {
                    ...
                    .await
                    {
                        Ok(next) => next,
                        Err(_) => {
                            return Err(observer.fail(HeyBuddyError::Timeout {
                                details: format!("request timed out after {timeout_ms}ms"),
                            }))
                        }
                    }
                ...
                match next {
                    ...
                    Some(Err(error)) => {
                        state.ended = true;
                        let error = HeyBuddyStreamError::from(&HeyBuddyError::from(error));
...
fn message_to_chunks(message: Message) -> Vec<StreamChunk> {
    message
        ...
        .filter_map(|content| match content {
            HeyBuddyMessageContent::Text(text) if !text.text.is_empty() => {
            ...
            HeyBuddyMessageContent::Thinking(thinking) => Some(StreamChunk::ThinkingChunk {
...
mod tests {
    ...
    fn model_config_rejects_invalid_request_params_json() {
        ...
        assert!(config.to_heybuddy_model_config().is_err());
    }
    ...
    fn provider_message_converts_document_to_base64_content() {
        ...
        let HeyBuddyMessageContent::Document(document) = &message.content[0] else {
    ...
    fn provider_message_converts_document_serializes_for_anthropic() {
        ...
        let spec = heybuddy_providers::formats::anthropic::format_messages(&[message]);
    ...
    fn usage_exposes_provider_additional_data() {
        let mut provider_usage = ProviderUsage::new(
            ...
            heybuddy_providers::conversation::token_usage::Usage::new(Some(10), Some(5), None),
        );
    ...
    fn tool_result_content_converts() {
        ...
        let HeyBuddyMessageContent::ToolResponse(response) = content else {
    ...
    fn provider_usage_with_cache(
        ...
        use heybuddy_providers::conversation::token_usage::Usage as HeyBuddyUsage;

        ProviderUsage::new(
            "test-model".to_string(),
            HeyBuddyUsage::new(Some(100), Some(20), Some(120))
    ...
    fn thinking_content_round_trips_with_signature() {
        ...
        let heybuddy = original.to_heybuddy_content().unwrap();
        let HeyBuddyMessageContent::Thinking(thinking) = &heybuddy else {
            panic!("expected thinking content");
        };
        assert_eq!(thinking.thinking, "step one, then step two");
        assert_eq!(thinking.signature, "ErUBCkYIBRgCIkAe0pAQ==");

        let round_tripped = MessageContent::from_heybuddy_content(&heybuddy).unwrap();
    ...
    fn redacted_thinking_content_round_trips_opaque_data() {
        ...
        let heybuddy = original.to_heybuddy_content().unwrap();
        let HeyBuddyMessageContent::RedactedThinking(redacted) = &heybuddy else {
    ...
    fn tool_request_round_trips_provider_metadata() {
        ...
        let heybuddy = original.to_heybuddy_content().unwrap();
        let HeyBuddyMessageContent::ToolRequest(request) = &heybuddy else {
        ...
        .unwrap();
        let spec = heybuddy_providers::formats::openai::format_messages(
            &messages,
            &heybuddy_providers::images::ImageFormat::OpenAi,
        );
        assert_eq!(
    ...
    fn completion_content_preserves_malformed_tool_requests() {
        ...
        .unwrap();
        let HeyBuddyMessageContent::ToolRequest(request) = &replayed[0].content[0] else {
            panic!("expected tool request");
        };
    ...
    fn streaming_tool_chunks_carry_provider_metadata() {
        let mut metadata = heybuddy_providers::conversation::message::ProviderMetadata::new();
        metadata.insert(
    ...
    fn thinking_blocks_survive_multi_turn_replay() {
        ...
        assert!(matches!(
            ...
            HeyBuddyMessageContent::Thinking(_)
        ));
        assert!(matches!(
            messages[1].content[1],
            HeyBuddyMessageContent::RedactedThinking(_)
        ));

        let spec = heybuddy_providers::formats::anthropic::format_messages(&messages);
