...
#[notification(method = "_aibuddy/unstable/session/update")]
#[serde(rename_all = "camelCase")]
pub struct AIBuddySessionNotification {
    pub session_id: String,
    pub update: AIBuddySessionUpdate,
}
...
#[derive(Debug, Default, Clone, Serialize, Deserialize, JsonSchema, JsonRpcNotification)]
#[notification(method = "_aibuddy/unstable/providers/authentication/device-code")]
#[serde(rename_all = "camelCase")]
...
impl Default for AIBuddySessionUpdate {
    fn default() -> Self {
        AIBuddySessionUpdate::UsageUpdate(SessionUsageUpdate::default())
    }
}
...
/// Schemas for every aibuddy-custom outbound notification. To register a new
...
pub fn custom_notification_schemas(generator: &mut SchemaGenerator) -> Vec<CustomMethodSchema> {
    vec![
        notification_schema::<AIBuddySessionNotification>(generator),
...
mod tests {
    ...
    fn status_message_serializes_to_expected_wire_shape() {
        let notification = AIBuddySessionNotification {
    ...
    fn message_usage_serializes_to_expected_wire_shape() {
        let notification = AIBuddySessionNotification {
            session_id: "s1".to_string(),
            update: AIBuddySessionUpdate::MessageUsage(MessageUsageUpdate {
