# aibuddy-local-inference

On-device model inference for aibuddy. Runs GGUF models through `llama.cpp` (via
`llama-cpp-2`), with an optional MLX backend on Apple silicon.

Reach it through [`aibuddy-providers`](../aibuddy-providers) with the
...
Models loaded this way remain user-owned: AIBuddy can load and evict them from
...
```bash
cargo build -p aibuddy-local-inference
cargo build -p aibuddy-local-inference --features hf-hub
cargo build -p aibuddy-local-inference --features mlx
```
