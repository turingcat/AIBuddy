...
#[cfg(feature = "hf-hub")]
pub use aibuddy_download_manager as download_manager;
#[cfg(feature = "hf-hub")]
...
use aibuddy_provider_types::base::{MessageStream, Provider, ProviderDescriptor, ProviderMetadata};
use aibuddy_provider_types::conversation::message::{
    Message, MessageContent, SystemNotificationType,
};
use aibuddy_provider_types::conversation::token_usage::{ProviderUsage, Usage};
use aibuddy_provider_types::errors::ProviderError;
use aibuddy_provider_types::images::ImageFormat;
use aibuddy_provider_types::model::ModelConfig;
use aibuddy_provider_types::request_log::{start_log, LoggerHandleExt, RequestLogHandle};
...
fn configured_draft_model(
    ...
    settings
        ...
        .clone()
        .or_else(|| {
            config_resolver::string_param("AIBUDDY_LOCAL_DRAFT_MODEL")
                .ok()
                .flatten()
        })
...
fn build_openai_messages_json(
    ...
    use aibuddy_provider_types::formats::openai::format_messages;
...
impl Provider for LocalInferenceProvider {
    ...
    async fn get_context_limit(&self, model: &str, override_limit: Option<usize>) -> usize {
        aibuddy_provider_types::context_limit::ContextLimitResolver::new(&self.name)
    ...
    async fn stream(
        ...
        if let Some(false) = model_config
            ...
            .or_else(|| {
                config_resolver::bool_param("AIBUDDY_LOCAL_ENABLE_THINKING")
                    .ok()
                    .flatten()
            })
        {
