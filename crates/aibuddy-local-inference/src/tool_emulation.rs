...
//! The parser converts those patterns into AIBuddy tool-call messages.
...
pub(crate) fn load_tiny_model_prompt() -> String {
    ...
    crate::prompt_template::render_template("tiny_model_system.md", &context).unwrap_or_else(|e| {
        ...
        "You are AIBuddy, an AI assistant. You can execute shell commands by starting lines with $."
            .to_string()
    })
}
