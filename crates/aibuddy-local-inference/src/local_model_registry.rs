...
pub struct LocalModelEntry {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub backend_id: Option<String>,
    /// Where the model artifacts are stored. AIBuddy keeps this registry as a
    /// lightweight settings overlay; Hugging Face cache entries are not deleted
    /// by AIBuddy because they may be shared with other tools.
    #[serde(default)]
...
impl LocalModelEntry {
    ...
    /// Returns all local paths owned by AIBuddy for this model.
    ...
    pub fn all_local_paths(&self) -> impl Iterator<Item = &std::path::Path> {
        let aibuddy_managed = self.storage == LocalModelStorage::AIBuddyManaged;
        std::iter::once(self.local_path.as_path())
            .chain(self.shard_files.iter().map(|s| s.local_path.as_path()))
            .filter(move |path| aibuddy_managed && !path.is_dir())
    }
...
impl LocalModelRegistry {
    ...
    pub fn delete_model(&mut self, id: &str) -> Result<()> {
        ...
        if is_featured_model(id) {
            if let Some(entry) = self.models.iter_mut().find(|m| m.id == id) {
                ...
                entry.storage = LocalModelStorage::AIBuddyManaged;
    ...
    fn deletion_plan(&self, id: &str) -> Result<ModelDeletionPlan> {
        ...
        Ok(ModelDeletionPlan {
            ...
            mmproj_path,
            delete_mmproj: entry.storage == LocalModelStorage::AIBuddyManaged && !other_uses_mmproj,
        })
    }
...
mod tests {
    ...
    fn test_entry(id: &str) -> LocalModelEntry {
        LocalModelEntry {
            ...
            backend_id: None,
            storage: LocalModelStorage::AIBuddyManaged,
    ...
    fn delete_featured_model_preserves_size_bytes() {
        ...
        let _guard =
            env_lock::lock_env([("AIBUDDY_PATH_ROOT", Some(root.path().to_str().unwrap()))]);
