...
pub struct OllamaOptions {
    /// Explicit context window override from `AIBUDDY_INPUT_LIMIT`.
    ...
    /// `OLLAMA_STREAM_TIMEOUT` > `AIBUDDY_STREAM_TIMEOUT` > `OLLAMA_TIMEOUT` >
    /// default (120s).
...
fn apply_ollama_options(payload: &mut Value, options: &OllamaOptions, _model_config: &ModelConfig) {
    if let Some(obj) = payload.as_object_mut() {
        ...
        // Only an explicit AIBUDDY_INPUT_LIMIT overrides Ollama's num_ctx.
...
/// Configurable via OLLAMA_STREAM_TIMEOUT, AIBUDDY_STREAM_TIMEOUT, or falls back
                ...
                Err(_) => {
                    Err::<(), anyhow::Error>(anyhow::anyhow!(
                        "Ollama stream stalled: no data received for {}s. \
                         ...
                         timeout via OLLAMA_STREAM_TIMEOUT, AIBUDDY_STREAM_TIMEOUT, or \
