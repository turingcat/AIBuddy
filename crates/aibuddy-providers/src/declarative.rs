...
pub struct AuthConfig {
    ...
    /// path is resolved against. Defaults to aibuddy's current directory.
    #[serde(default)]
...
pub struct DeclarativeProviderConfig {
    ...
    #[serde(default)]
    pub setup: Option<aibuddy_provider_types::canonical::catalog::ProviderSetupMetadata>,
}
...
mod tests {
    ...
    #[test]
    fn aibuddy_provider_is_not_bundled() {
        let providers = fixed_provider_config_entries();
        assert!(providers.iter().all(|(name, _)| *name != "aibuddy.json"));
    }
