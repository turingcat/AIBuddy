...
Provider implementations for aibuddy. The trait they implement and the conversation
types they exchange live in [`aibuddy-provider-types`](../aibuddy-provider-types),
...
```bash
cargo run -p aibuddy-providers --example declarative
cargo run -p aibuddy-providers --example streaming
```
...
- `local-inference` — pulls in [`aibuddy-local-inference`](../aibuddy-local-inference);
