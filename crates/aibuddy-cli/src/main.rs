...
async fn run() -> Result<()> {
    if let Err(e) = aibuddy_cli::logging::setup_logging(None) {
    ...
    #[cfg(feature = "otel")]
    if aibuddy::otel::otlp::is_otlp_initialized() {
        aibuddy::otel::otlp::shutdown_otlp();
    }
...
fn main() -> Result<()> {
    ...
    aibuddy::config::legacy::bootstrap_legacy_environment();

    let handle = std::thread::Builder::new()
        .name("aibuddy-cli-main".to_string())
        ...
        .map_err(|e| anyhow::anyhow!("Failed to spawn aibuddy-cli main thread: {}", e))?;

    handle
        .join()
        .map_err(|_| anyhow::anyhow!("aibuddy-cli main thread panicked"))?
}
