use aibuddy::providers::utils::init_aibuddy_request_log;
use anyhow::Result;
...
pub fn setup_logging(name: Option<&str>) -> &'static Result<()> {
    INIT.get_or_init(|| {
        ...
        init_aibuddy_request_log()?;
        let config = aibuddy::logging::LoggingConfig {
            component: "cli",
            name,
            extra_directives: &["aibuddy_cli=info"],
            console: false,
            json: true,
        };
        let subscriber = aibuddy::logging::build_logging_subscriber(&config)?;
...
#[cfg(test)]
mod tests {
    use aibuddy::tracing::langfuse_layer;
    use std::env;
    ...
    fn test_log_directory_creation() {
        ...
        let log_dir = aibuddy::logging::prepare_log_directory("cli", true).unwrap();
    ...
    async fn test_default_filter_avoids_debug_by_default() {
        ...
        assert!(aibuddy::logging::build_logging_subscriber(&config).is_ok());
    }
}
