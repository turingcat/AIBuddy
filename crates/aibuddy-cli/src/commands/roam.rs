//! `aibuddy roam` — peer-to-peer agent access over iroh.
//!
...
//! accepted peer gets aibuddy's full ACP surface.
//!
//! Subcommands:
...
use aibuddy::config::{Config, ConfigError};
use aibuddy_roaming::{
...
const CARD_SCHEME: &str = "aibuddy+roam://";
...
/// Config key (or `AIBUDDY_ROAM_RELAYS` env) overriding the relay URLs the
/// roaming endpoint uses. When unset, roaming uses the managed relays below —
/// never iroh's shared public n0 relays.
const CONFIG_ROAM_RELAYS_KEY: &str = "AIBUDDY_ROAM_RELAYS";
...
const CONFIG_ROAM_RELAY_TOKEN_KEY: &str = "AIBUDDY_ROAM_RELAY_TOKEN";
...
///
/// Override with `AIBUDDY_ROAM_RELAYS` to point at other relays (e.g. a Block-run
/// deployment).
...
///
/// Uses `AIBUDDY_ROAM_RELAYS` (env or config file) when set — optionally
/// authenticated with the `AIBUDDY_ROAM_RELAY_TOKEN` secret applied to each —
/// otherwise the default managed relays. Never iroh's public n0 relays.
///
/// Fails when `AIBUDDY_ROAM_RELAYS` is set but unreadable: a deployment that
...
pub enum RoamCommand {
    ...
    /// Each connected peer gets aibuddy's full ACP surface — it drives its own
    ...
    Connect {
        /// A saved peer nickname (see `roam peers`) or a `aibuddy+roam://...` card.
        target: String,
    ...
    ///
    /// Defaults to stdio (for a client that spawns `aibuddy roam bridge ...` as a
...
pub enum PeersCommand {
    ...
    Add {
        /// The peer's `aibuddy+roam://...` card.
        card: String,
        /// Friendly nickname (defaults to a short id if omitted).
        name: Option<String>,
    },
    /// Accept inbound connections from a peer's key. The target is a saved
    /// nickname or a `aibuddy+roam://...` card (which is also saved to the address
    /// book). An accepted peer gets aibuddy's full ACP surface.
    Accept {
        /// A saved nickname or a `aibuddy+roam://...` card.
        target: String,
...
async fn handle_id(qr: bool) -> Result<()> {
    ...
    eprintln!();
    eprintln!("the peer adds it with:  aibuddy roam peers add '<card>' <name>");
    eprintln!("and accepts you with:   aibuddy roam peers accept <name>");
    node.shutdown().await?;
    Ok(())
}
...
async fn handle_pair(name: Option<String>) -> Result<()> {
    ...
    let decoded = ConnectionCard::decode(device_card)
        .context("that does not look like a aibuddy+roam:// connection card")?;
    let name =
    ...
    aibuddy_roaming::PeerBook::update(peerbook_path(), |book| {
...
async fn handle_peers(command: PeersCommand) -> Result<()> {
    let book = aibuddy_roaming::PeerBook::load(peerbook_path())?;
    match command {
        PeersCommand::Add { card, name } => {
            ...
            eprintln!("accept connections from it with: aibuddy roam peers accept {name}");
            Ok(())
        }
        PeersCommand::Accept { target, name } => {
            ...
            eprintln!("a running `aibuddy roam share` picks this up on the next connection");
            Ok(())
        }
        ...
        PeersCommand::Remove { name } => {
            let existed =
                aibuddy_roaming::PeerBook::update(peerbook_path(), |book| book.remove(&name))?;
            if existed {
        ...
        PeersCommand::Rename { from, to } => {
            aibuddy_roaming::PeerBook::update(peerbook_path(), |book| book.rename(&from, &to))?;
        ...
        PeersCommand::List => {
            ...
            if peers.is_empty() && accepted.is_empty() {
                eprintln!("no saved peers; add one with `aibuddy roam peers add '<card>' <name>`");
                return Ok(());
            }
...
fn resolve_key(book: &aibuddy_roaming::PeerBook, target: &str) -> Result<EndpointId> {
...
async fn handle_list() -> Result<()> {
    ...
    for e in entries {
        ...
        let dir = match e.direction {
            aibuddy_roaming::Direction::Inbound => "inbound",
            aibuddy_roaming::Direction::Outbound => "outbound",
        };
...
/// advisory lock decides ownership across all aibuddy processes (desktop
...
pub(crate) fn try_acquire_roam_lock() -> Result<std::fs::File> {
    try_acquire_roam_lock_owner()?.ok_or_else(|| {
        anyhow::anyhow!("another aibuddy process is already running the roaming endpoint")
    })
}
    ...
    if accepted_count == 0 {
        eprintln!(
            "warning: no peers are accepted yet — no one can connect.\n\
             Accept a peer's key first: aibuddy roam peers accept <name|card>"
        );
    }
...
fn resolve_card(target: &str) -> Result<ConnectionCard> {
    ...
    match book.get(target) {
        ...
        None => anyhow::bail!(
            "no saved peer named `{target}` (and it is not a card); see `aibuddy roam peers`"
        ),
    }
}
...
async fn dial_target(
    ...
    aibuddy_roaming::RoamingClientStream,
)> {
...
async fn handle_bridge(
    ...
    // outlive the splice.
    let aibuddy_roaming::RoamingClientStream {
        conn,
        send: remote_send,
        recv: remote_recv,
        ..
