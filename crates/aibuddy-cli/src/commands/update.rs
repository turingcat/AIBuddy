...
fn asset_name() -> &'static str {
    ...
    {
        "aibuddy-aarch64-apple-darwin.tar.bz2"
    }

    #[cfg(all(target_os = "linux", target_arch = "x86_64", target_env = "gnu"))]
    {
        "aibuddy-x86_64-unknown-linux-gnu.tar.bz2"
    }
    #[cfg(all(target_os = "linux", target_arch = "aarch64", target_env = "gnu"))]
    {
        "aibuddy-aarch64-unknown-linux-gnu.tar.bz2"
    }
    #[cfg(all(target_os = "linux", target_arch = "x86_64", target_env = "musl"))]
    {
        "aibuddy-x86_64-unknown-linux-musl.tar.bz2"
    }
    #[cfg(all(target_os = "linux", target_arch = "aarch64", target_env = "musl"))]
    {
        "aibuddy-aarch64-unknown-linux-musl.tar.bz2"
    }
    #[cfg(all(target_os = "windows", target_arch = "x86_64", feature = "cuda"))]
    {
        "aibuddy-x86_64-pc-windows-msvc-cuda.zip"
    }
    #[cfg(all(target_os = "windows", target_arch = "x86_64", not(feature = "cuda")))]
    {
        "aibuddy-x86_64-pc-windows-msvc.zip"
    }
}
...
fn sha256_hex(data: &[u8]) -> String {
    ...
    hasher.update(data);
    aibuddy::utils::bytes_to_hex(hasher.finalize())
}
...
const RELEASE_REPOSITORY: &str = "turingcat/AIBuddy";
...
/// Update the aibuddy binary to the latest release.
///
...
pub async fn update(canary: bool, reconfigure: bool) -> Result<()> {
    ...
    #[cfg(not(feature = "disable-update"))]
    {
        ...
        println!("aibuddy updated successfully (verified with Sigstore SLSA provenance).");

        // --- Reconfigure if requested -------------------------------------------
        if reconfigure {
            println!("Running aibuddy configure...");
            let status = Command::new(current_exe)
                .arg("configure")
                .status()
                .context("Failed to run aibuddy configure")?;
            if !status.success() {
                eprintln!("Warning: aibuddy configure exited with {status}");
            }
        }
...
///   1. A `aibuddy-package/` subdirectory (Windows releases)
...
fn find_binary(extract_dir: &Path, binary_name: &str) -> Option<PathBuf> {
    // 1. Check aibuddy-package subdir (matches download_cli.sh / download_cli.ps1)
    let package_dir = extract_dir.join("aibuddy-package");
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        if old_exe.exists() {
            fs::remove_file(&old_exe).with_context(|| {
                format!(
                    "Failed to remove old backup {}. Is another aibuddy process running?",
                    old_exe.display()
                )
            })?;
        }
        ...
        fs::rename(current_exe, &old_exe).with_context(|| {
            format!(
                "Failed to rename running binary to {}. Try closing AIBuddy Desktop if it's open.",
                old_exe.display()
            )
        })?;
...
mod tests {
    ...
    #[test]
    fn release_urls_use_the_aibuddy_repository() {
        assert_eq!(
            release_download_url("stable", "aibuddy-aarch64-apple-darwin.tar.bz2"),
            "https://github.com/turingcat/AIBuddy/releases/download/stable/aibuddy-aarch64-apple-darwin.tar.bz2"
        );
        assert_eq!(
            attestation_url("abc123"),
            "https://api.github.com/repos/turingcat/AIBuddy/attestations/sha256:abc123?per_page=30&predicate_type=https://slsa.dev/provenance/v1"
        );
    }
    ...
    fn test_asset_name_valid() {
        ...
        assert!(name.starts_with("aibuddy-"));
        #[cfg(target_os = "windows")]
    ...
    fn test_binary_name() {
        ...
        #[cfg(target_os = "windows")]
        assert_eq!(name, "aibuddy.exe");
        #[cfg(not(target_os = "windows"))]
        assert_eq!(name, "aibuddy");
    }

    #[test]
    fn test_find_binary_in_package_subdir() {
        let tmp = tempdir().unwrap();
        let pkg = tmp.path().join("aibuddy-package");
    ...
    fn test_replace_binary_basic() {
        let tmp = tempdir().unwrap();
        let new_bin = tmp.path().join("new_aibuddy");
        let current = tmp.path().join("current_aibuddy");
    ...
    fn test_replace_binary_windows_rename_away() {
        let tmp = tempdir().unwrap();
        let current = tmp.path().join("aibuddy.exe");
        let new_bin = tmp.path().join("new_aibuddy.exe");
    ...
    fn test_extract_zip_with_package_dir() {
        ...
        // Create a zip in memory with aibuddy-package/ structure
        let mut buf = Vec::new();
        {
            ...
            writer.add_directory("aibuddy-package/", options).unwrap();
            writer
                .start_file("aibuddy-package/aibuddy.exe", options)
                .unwrap();
            writer.write_all(b"fake aibuddy binary").unwrap();
            writer
                .start_file("aibuddy-package/libtest.dll", options)
                .unwrap();
        ...
        let binary = find_binary(tmp.path(), "aibuddy.exe");
        assert!(binary.is_some());

        let content = fs::read_to_string(binary.unwrap()).unwrap();
        assert_eq!(content, "fake aibuddy binary");

        // DLL should be in aibuddy-package too
        assert!(tmp.path().join("aibuddy-package/libtest.dll").exists());
    }
    ...
    fn test_validate_entry_path_accepts_safe_paths() {
        assert!(validate_entry_path(Path::new("aibuddy")).is_ok());
        assert!(validate_entry_path(Path::new("aibuddy-package/aibuddy")).is_ok());
    ...
    fn test_extract_tar_bz2_safe_archive() {
        ...
        let mut builder_buf = Vec::new();
        {
            ...
            let data = b"aibuddy binary content";
            let mut header = tar::Header::new_gnu();
            header.set_size(data.len() as u64);
            header.set_mode(0o755);
            header.set_cksum();
            builder
                .append_data(&mut header, "aibuddy-package/aibuddy", &data[..])
                .unwrap();
        ...
        let extracted = tmp.path().join("aibuddy-package/aibuddy");
        assert!(extracted.exists());
        assert_eq!(
            fs::read_to_string(extracted).unwrap(),
            "aibuddy binary content"
        );
    }
