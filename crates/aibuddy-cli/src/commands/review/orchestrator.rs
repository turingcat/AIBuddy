//! Deterministic, Rust-driven orchestration for `aibuddy review`.
//!
...
//!
//! - One subprocess per check (`aibuddy run -q -t <prompt>`)
...
use aibuddy::checks::{Check, DEFAULT_CHECK_TURN_LIMIT};
...
/// Run all discovered checks concurrently as `aibuddy run` subprocesses.
///
...
pub async fn run_checks_in_parallel(
    ...
    while let Some(joined) = set.join_next().await {
        let (idx, check, result, quiet) = match joined {
            Ok(v) => v,
            Err(e) => {
                eprintln!("aibuddy review: check task panicked: {e}");
                continue;
            }
        };
        ...
        match result {
            Ok(findings) => {
                if !quiet {
                    eprintln!(
                        "aibuddy review: check '{}' completed: {} finding(s)",
                        check.name,
                        findings.len()
                    );
                }
            ...
            Err(e) => {
                ...
                eprintln!("aibuddy review: check '{}' failed: {e}", check.name);
                results[idx] = Vec::new();
            }
        }
    }
...
///    asks for `aibuddy-claude-4-sonnet` would 404 against Google's API),
...
///
/// Uses `aibuddy review --turn-limit` when set, otherwise
...
/// Spawn a single `aibuddy run` subprocess for one check and parse its
...
/// Generic `aibuddy run` subprocess that hands a prompt to the model
...
async fn run_subprocess_for_findings(
    ...
    let aibuddy_bin = std::env::current_exe().context("locate current aibuddy binary")?;

    let mut cmd = Command::new(&aibuddy_bin);
    cmd.arg("run")
        .arg("--no-session")
        .arg("--quiet")
        .arg("--no-profile")
    ...
    if let Some(mut stdin) = child.stdin.take() {
        ...
        // Closing stdin signals EOF to `aibuddy run -i -`.
        drop(stdin);
    }
...
pub async fn run_main_pass_in_parallel(
    ...
    while let Some(joined) = set.join_next().await {
        let (idx, path, result, quiet) = match joined {
            Ok(v) => v,
            Err(e) => {
                eprintln!("aibuddy review: main-pass task panicked: {e}");
                continue;
            }
        };
        match result {
            Ok(raw) => {
                ...
                if !quiet {
                    eprintln!(
                        "aibuddy review: main pass on '{}' completed: {} finding(s)",
                        path,
                        findings.len()
                    );
                }
            ...
            Err(e) => {
                ...
                eprintln!("aibuddy review: main pass on '{}' failed: {e}", path);
...
/// cap enforced by aibuddy. Without this, models routinely burn turns on
...
fn build_subprocess_turn_budget_section(max_turns: usize) -> String {
    format!(
        "## Turn budget\n\n\
         ...
         aibuddy enforces this via `--max-turns`; when you exhaust it, the session stops and \
...
mod tests {
    ...
    fn resolve_check_model_cli_provider_wins_over_per_check_model() {
        let mut c = ck("perf");
        c.model = Some("aibuddy-claude-4-sonnet".into()); // wrong provider
        let opts = ReviewOptions {
    ...
    fn resolve_check_model_cli_provider_alone_drops_per_check_model() {
        ...
        let mut c = ck("perf");
        c.model = Some("aibuddy-claude-4-sonnet".into());
        let opts = ReviewOptions {
            provider: Some("google".into()),
            default_model: None,
            ..ReviewOptions::default()
