...
use aibuddy::checks::{discover, DiscoveredReview};
use aibuddy::subprocess::git_command;
...
/// Entry point for the `aibuddy review` subcommand.
pub async fn handle_review(opts: ReviewOptions) -> Result<()> {
    ...
    if diff.trim().is_empty() {
        eprintln!("aibuddy review: no changes to review");
        return Ok(());
    }
    ...
    if !use_orchestrator {
        ...
        if opts.checks_only {
            ...
            if !opts.quiet {
                ...
                eprintln!(
                    "aibuddy review: emitted {total_emitted} finding(s) from {} check(s) ({suppressed} hidden below severity={:?})",
                    discovered.checks.len(),
                    min_sev
                );
            }
    ...
    if !opts.quiet {
        ...
        if suppressed == 0 {
            eprintln!(
                "aibuddy review: orchestrator emitted {total_emitted} finding(s) from {} check(s) (main: {main_pass_label}, {} finding(s))",
                discovered.checks.len(),
                main_findings.len()
            );
        } else {
            eprintln!(
                "aibuddy review: orchestrator emitted {total_emitted} finding(s) from {} check(s) (main: {main_pass_label}, {} finding(s); {suppressed} hidden below severity={:?})",
                discovered.checks.len(),
                main_findings.len(),
                min_sev
            );
...
fn print_discovered_summary(d: &DiscoveredReview) {
    if d.checks.is_empty() {
        eprintln!("aibuddy review: no checks or REVIEW.md rules discovered");
        return;
    }
    eprintln!("aibuddy review: discovered {} check(s):", d.checks.len());
    for c in &d.checks {
...
/// `discovery_root` so [`aibuddy::checks::discover`] doesn't double-
