...
pub fn handle_plugin_install(url: &str, auto_update: bool) -> Result<()> {
    let install = aibuddy::plugins::install_plugin_with_options(
        url,
        aibuddy::plugins::PluginInstallOptions { auto_update },
    )?;
...
pub fn handle_plugin_update(name: &str) -> Result<()> {
    let install = aibuddy::plugins::update_plugin(name)?;
...
fn print_plugin_install(install: &aibuddy::plugins::PluginInstall) {
