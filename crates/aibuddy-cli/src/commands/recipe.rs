use aibuddy::recipe::validate_recipe::validate_recipe_template_from_file;
use anyhow::Result;
use console::style;
    ...
    match generate_deeplink(recipe_name, params_map) {
        Ok((deeplink_url, recipe)) => match opener(&deeplink_url) {
            Ok(_) => {
                writeln!(
                    out,
                    "{} Opened recipe '{}' in AIBuddy Desktop",
                    style("✓").green().bold(),
                    recipe.title
                )?;
                Ok(())
            ...
            Err(err) => {
                writeln!(
                    out,
                    "{} Failed to open recipe in AIBuddy Desktop: {}",
                    style("✗").red().bold(),
                    err
                )?;
                writeln!(out, "Generated deeplink: {}", deeplink_url)?;
                writeln!(out, "You can manually copy and open the URL above, or ensure AIBuddy Desktop is installed.")?;
...
"#;
    ...
    fn test_handle_deeplink_valid_recipe() {
        ...
        assert!(url.starts_with("aibuddy://recipe?config="));
        let encoded_part = url.strip_prefix("aibuddy://recipe?config=").unwrap();
    ...
    fn test_handle_open_opener_fails() {
        ...
        assert!(result.is_err());
        assert!(output.contains("Failed to open recipe in AIBuddy Desktop"));
