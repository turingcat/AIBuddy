...
use aibuddy_roaming::RoamingClientStream;
use anyhow::Result;
...
fn prompt_permission(request: &RequestPermissionRequest) -> RequestPermissionOutcome {
    ...
    // Fail closed: option 1 is allow-always for aibuddy hosts, so EOF, an empty
