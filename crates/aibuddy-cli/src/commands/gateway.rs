use aibuddy::execution::manager::AgentManager;
use aibuddy::gateway::manager::GatewayManager;
use anyhow::Result;
use std::sync::Arc;
...
pub async fn handle_gateway_start(
    ...
    let mut config = aibuddy::gateway::GatewayConfig {
        gateway_type,
        platform_config,
        max_sessions: 0,
    };

    let gw = aibuddy::gateway::create_gateway(&mut config)?;
