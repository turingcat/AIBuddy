use crate::recipes::github_recipe::AIBUDDY_RECIPE_GITHUB_REPO_CONFIG_KEY;
use aibuddy::agents::extension::{ToolInfo, PLATFORM_EXTENSIONS};
use aibuddy::agents::extension_manager::get_parameter_names;
use aibuddy::agents::Agent;
use aibuddy::agents::{extension::Envs, ExtensionConfig};
use aibuddy::config::declarative_providers::{
    create_custom_provider, remove_custom_provider, AuthConfig, CreateCustomProviderParams,
};
use aibuddy::config::extensions::{
    get_all_extension_names, get_all_extensions, get_enabled_extensions, get_extension_by_name,
    name_to_key, remove_extension, set_extension, set_extension_enabled,
};
use aibuddy::config::paths::Paths;
use aibuddy::config::permission::PermissionLevel;
use aibuddy::config::signup_tetrate::TetrateAuth;
use aibuddy::config::{
    configure_tetrate, AIBuddyMode, Config, ConfigError, ExperimentManager, ExtensionEntry,
    PermissionManager,
};
use aibuddy::providers::base::ConfigKey;
use aibuddy::providers::provider_test::test_provider_configuration;
use aibuddy::providers::{create, providers, retry_operation, RetryConfig};
use aibuddy::session::SessionType;
...
pub async fn handle_configure() -> anyhow::Result<()> {
    if !std::io::stdin().is_terminal() {
        anyhow::bail!(
            "aibuddy configure requires an interactive terminal.\n\
             If you installed via 'curl ... | bash', run 'aibuddy configure' separately after installation."
        );
    }
...
async fn handle_first_time_setup(config: &Config) -> anyhow::Result<()> {
    ...
    println!(
        "{}",
        style("Welcome to aibuddy! Let's get you set up.").dim()
    );
    println!(
        "{}",
    ...
    println!();
    cliclack::intro(style(" aibuddy-configure ").on_cyan().black())?;
...
async fn handle_existing_config() -> anyhow::Result<()> {
    ...
    let action = cliclack::select("What would you like to configure?")
        ...
        .item(
            "settings",
            "aibuddy settings",
            "Set the aibuddy mode, Tool Output, Tool Permissions, Experiment, aibuddy recipe github repo and more",
        )
        .interact()?;
...
fn interactive_model_search(
    models: &[String],
    provider_meta: &aibuddy::providers::base::ProviderMetadata,
    ...
    match config.set_secret(key_name, &value) {
        ...
        Err(e) => {
            cliclack::outro(style(format!(
                "Failed to store {} securely: {}. Please ensure your system's secure storage is accessible. Alternatively you can run with AIBUDDY_DISABLE_KEYRING=true or set the key in your environment variables",
                key_name, e
            )).on_red().white())?;
            Ok(false)
        }
...
pub async fn configure_provider_dialog() -> anyhow::Result<bool> {
    ...
    let current_provider: Option<String> = config.get_aibuddy_provider().ok();
    ...
    let models_res = retry_operation(&RetryConfig::default(), || async {
        temp_provider
            .fetch_recommended_models(aibuddy::model_config::global_toolshim())
            .await
    })
    .await;
    ...
    let model: String = match models_res {
        ...
        Ok(_) => {
            let default_model =
                std::env::var("AIBUDDY_MODEL").unwrap_or(provider_meta.default_model.clone());
...

    {
        let supports_thinking = match temp_provider.fetch_model_info(&model).await {
            ...
            Err(_) => aibuddy_providers::model::ModelConfig::new(&model).is_reasoning_model(),
        };
        ...
        if supports_thinking {
            ...
            config.set_aibuddy_thinking_effort(effort)?;
        }
    }
    ...
    let toolshim_enabled = std::env::var("AIBUDDY_TOOLSHIM")
        .map(|val| val == "1" || val.to_lowercase() == "true")
        .unwrap_or(false);
    let toolshim_model = std::env::var("AIBUDDY_TOOLSHIM_OLLAMA_MODEL").ok();

    match test_provider_configuration(&provider_name, &model, toolshim_enabled, toolshim_model)
        .await
    {
        Ok(()) => {
            aibuddy::config::set_active_provider(config, &provider_name, &model)?;
...
fn prompt_extension_timeout() -> anyhow::Result<u64> {
    Ok(
        cliclack::input("Please set the timeout for this tool (in secs):")
            .placeholder(&aibuddy::config::DEFAULT_EXTENSION_TIMEOUT.to_string())
...
fn configure_stdio_extension() -> anyhow::Result<()> {
    ...
    let mut parts = aibuddy::utils::split_command_args(&command_str)?;
...
pub fn configure_extensions_dialog() -> anyhow::Result<()> {
    let extension_type = cliclack::select("What type of extension would you like to add?")
        .item(
            "built-in",
            "Built-in Extension",
            "Use an extension that comes with aibuddy",
        )
        .item(
            "stdio",
...
pub async fn configure_settings_dialog() -> anyhow::Result<()> {
    ...
    let setting_type = setting_select
        ...
        .item(
            ...
            "aibuddy will pull recipes from this repo if not found locally.",
        )
        .interact()?;
...
pub fn configure_aibuddy_mode_dialog() -> anyhow::Result<()> {
    let config = Config::global();

    if std::env::var("AIBUDDY_MODE").is_ok() {
        let _ = cliclack::log::info(
            "Notice: AIBUDDY_MODE environment variable is set and will override the configuration here.",
        );
    }

    let mode = cliclack::select("Which aibuddy mode would you like to configure?")
        .item(
            AIBuddyMode::Auto,
            "Auto Mode",
    ...
    let msg = match mode {
        AIBuddyMode::Auto => "Set to Auto Mode - full file modification enabled",
        AIBuddyMode::Approve => {
            "Set to Approve Mode - all tools and modifications require approval"
        }
        AIBuddyMode::SmartApprove => "Set to Smart Approve Mode - modifications require approval",
        AIBuddyMode::Chat => "Set to Chat Mode - no tools or modifications enabled",
    };
    cliclack::outro(msg)?;
    Ok(())
}
...
pub fn configure_tool_output_dialog() -> anyhow::Result<()> {
    ...
    if std::env::var("AIBUDDY_CLI_MIN_PRIORITY").is_ok() {
        let _ = cliclack::log::info(
            "Notice: AIBUDDY_CLI_MIN_PRIORITY environment variable is set and will override the configuration here.",
        );
    }
    ...
    match tool_log_level {
        "high" => {
            config.set_param("AIBUDDY_CLI_MIN_PRIORITY", 0.8)?;
            cliclack::outro("Showing tool output of high importance only.")?;
        }
        "medium" => {
            config.set_param("AIBUDDY_CLI_MIN_PRIORITY", 0.2)?;
            cliclack::outro("Showing tool output of medium importance.")?;
        }
        "all" => {
            config.set_param("AIBUDDY_CLI_MIN_PRIORITY", 0.0)?;
...
pub fn configure_keyring_dialog() -> anyhow::Result<()> {
    ...
    if std::env::var("AIBUDDY_DISABLE_KEYRING").is_ok() {
        let _ = cliclack::log::info(
            "Notice: AIBUDDY_DISABLE_KEYRING environment variable is set and will override the configuration here.",
        );
    }

    let currently_disabled = config
        .get_param::<String>("AIBUDDY_DISABLE_KEYRING")
        .is_ok();
    ...
    match storage_option {
        "keyring" => {
            ...
            config.set_param("AIBUDDY_DISABLE_KEYRING", Value::String("".to_string()))?;
            cliclack::outro("Secret storage set to system keyring (secure)")?;
            let _ = cliclack::log::info(
                "You may need to restart aibuddy for this change to take effect",
            );
        }
        "file" => {
            // Set the disable flag to use file storage
            config.set_param("AIBUDDY_DISABLE_KEYRING", Value::String("true".to_string()))?;
            cliclack::outro(format!(
...
/// Configure experiment features that can be used with aibuddy
...
pub async fn configure_tool_permissions_dialog() -> anyhow::Result<()> {
    ...
    let model_config =
        aibuddy::model_config::model_config_from_user_config(&provider_name, &model)?;
...
fn configure_recipe_dialog() -> anyhow::Result<()> {
    let key_name = AIBUDDY_RECIPE_GITHUB_REPO_CONFIG_KEY;
    let config = Config::global();
...
pub fn configure_max_turns_dialog() -> anyhow::Result<()> {
    ...
    let current_max_turns: u32 = config.get_param("AIBUDDY_MAX_TURNS").unwrap_or(1000);
    ...
    config.set_param("AIBUDDY_MAX_TURNS", max_turns)?;

    cliclack::outro(format!(
        "Set maximum turns to {} - aibuddy will ask for input after {} consecutive actions",
        max_turns, max_turns
    ))?;
...
pub async fn handle_openrouter_auth() -> anyhow::Result<()> {
    use aibuddy::config::{configure_openrouter, signup_openrouter::OpenRouterAuth};
    ...
    let configured_model: String = config.get_aibuddy_model()?;
    let model_config =
        match aibuddy::model_config::model_config_from_user_config("openrouter", &configured_model)
        {
            Ok(config) => config,
            Err(e) => {
    ...
    match create("openrouter", Vec::new()).await {
        Ok(provider) => {
            let test_result = provider
                .complete(
                    &model_config,
                    "You are aibuddy, an AI assistant.",
            ...
            match test_result {
                Ok(_) => {
                    ...
                    if !has_developer {
                        set_extension(ExtensionEntry {
                            ...
                            config: ExtensionConfig::Platform {
                                ...
                                display_name: Some(
                                    aibuddy::config::DEFAULT_DISPLAY_NAME.to_string(),
                                ),
                                bundled: Some(true),
                                available_tools: Vec::new(),
                            },
                    ...
                    cliclack::outro("OpenRouter setup complete! You can now use aibuddy.")?;
                }
                Err(e) => {
...
pub async fn handle_tetrate_auth() -> anyhow::Result<()> {
    ...
    let configured_model: String = config.get_aibuddy_model()?;
    if let Err(e) =
        aibuddy::model_config::model_config_from_user_config("tetrate", &configured_model)
    {
        eprintln!("⚠️  Invalid model configuration: {}", e);
        eprintln!("Your settings have been saved. Please check your model configuration.");
        return Ok(());
        ...
        Ok(provider) => {
            ...
            match test_result {
                Ok(_) => {
                    ...
                    cliclack::outro(
                        "Tetrate Agent Router Service setup complete! You can now use aibuddy.",
                    )?;
                }
                Err(e) => {
                    eprintln!("⚠️  Configuration test failed: {}", e);
...
fn add_provider() -> anyhow::Result<()> {
    ...
    if requires_auth {
        let auth_mode =
            cliclack::select("How should aibuddy obtain credentials for this provider?")
    ...
    let models: Vec<aibuddy_providers::base::ModelInfo> = models_input
        .split(',')
        .map(str::trim)
        .filter(|name| !name.is_empty())
        .map(aibuddy_providers::base::ModelInfo::new)
        .collect();
    ...
    if !provider_config.models.is_empty() {
        ...
        if let Ok(model) = cliclack::select("Which model should be the default?")
            ...
            .interact()
        {
            config.set_aibuddy_provider(&provider_config.name)?;
            config.set_aibuddy_model(model)?;
        }
    }
...
async fn remove_provider() -> anyhow::Result<()> {
    let custom_providers_dir = aibuddy::config::declarative_providers::custom_providers_dir();
    let custom_providers = if custom_providers_dir.exists() {
        aibuddy::config::declarative_providers::load_custom_providers(&custom_providers_dir)?
    } else {
        Vec::new()
    };
    ...
    if let Err(e) = aibuddy::providers::cleanup_provider(selected_id).await {
