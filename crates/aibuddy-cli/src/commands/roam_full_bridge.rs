//! The roaming host adapter: serve aibuddy's **full** ACP surface to each
//! connecting peer.
//!
//! Roaming is just an authenticated p2p ACP transport. This adapter is the thin
//! seam that hands an authorized iroh stream straight to aibuddy's real
...
use aibuddy::acp::server::serve;
use aibuddy::acp::server_factory::AcpServer;
use aibuddy_roaming::{AcpStreamServer, EndpointId};

/// An [`AcpStreamServer`] that serves aibuddy's full ACP surface, a fresh agent
/// per connection.
pub struct FullAcpBridge {
    ...
    /// `AcpServer` (e.g. `aibuddy serve`) leaves `session_cwd` unset for its
    /// local clients.
