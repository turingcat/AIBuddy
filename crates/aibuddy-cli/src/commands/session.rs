...
#[cfg(feature = "nostr")]
use aibuddy::session::nostr_share;
use aibuddy::session::{
...
pub async fn handle_session_export(
    ...
    if nostr {
        return Err(anyhow::anyhow!("aibuddy was not built with nostr support"));
    }
...
fn should_import_from_nostr(input: &str, nostr: bool) -> bool {
    nostr
        || input.starts_with("aibuddy://sessions/nostr")
        || input.starts_with("aibuddy://sessions/nostr")
}
...
pub async fn handle_session_import(input: String, nostr: bool) -> Result<()> {
    ...
    let format = aibuddy::session::import_formats::detect_format(&json);
    let label = match format {
        aibuddy::session::import_formats::ImportFormat::AIBuddy => "aibuddy",
        aibuddy::session::import_formats::ImportFormat::ClaudeCode => "Claude Code",
        aibuddy::session::import_formats::ImportFormat::Codex => "Codex",
        aibuddy::session::import_formats::ImportFormat::Pi => "Pi",
    };
...
mod diagnostics_output_tests {
    ...
    fn recognizes_canonical_and_legacy_nostr_import_inputs() {
        assert!(should_import_from_nostr(
            "aibuddy://sessions/nostr?event=canonical",
            false
        ));
        assert!(should_import_from_nostr(
            "aibuddy://sessions/nostr?event=legacy",
            false
        ));
