//! Generate manpages for the aibuddy CLI.
//!
...
//! offline documentation accessible via the `man` command (e.g., `man aibuddy`).
//!
//! When aibuddy is packaged for Linux distributions (deb, rpm, etc.), the generated
...
//!
//! Output: target/man/aibuddy.1, target/man/aibuddy-session.1, etc.
...
fn generate_see_also(
    ...
    // Always reference the main aibuddy command if we're not it
    if current_name != "aibuddy" {
        references.push("aibuddy".to_string());
    }

    // Reference parent command if exists and not already added
    if let Some(parent) = parent_name {
        if parent != "aibuddy" && !references.contains(&parent.to_string()) {
    ...
    if current_name == "aibuddy" {
        ...
        for subcmd in cmd.get_subcommands() {
            ...
            if subcmd_name != "help" && !subcmd.is_hide_set() {
                let full_name = format!("aibuddy-{}", subcmd_name);
