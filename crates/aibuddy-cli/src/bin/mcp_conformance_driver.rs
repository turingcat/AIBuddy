...
fn main() {
    ...
    let aibuddy =
        std::env::var("AIBUDDY_BIN").unwrap_or_else(|_| "target/debug/aibuddy".to_string());
    let path_root = tempfile::Builder::new()
        .prefix("aibuddy-mcp-conformance-")
        .tempdir()
        .unwrap_or_else(|err| {
            eprintln!("failed to create temporary AIBUDDY_PATH_ROOT: {err}");
            std::process::exit(1);
        });
    let mut child = Command::new(&aibuddy)
        .args(["mcp-probe", target, "--script", "-"])
        .env("AIBUDDY_OAUTH_AUTOMATIC_CALLBACK", "1")
        .env("AIBUDDY_DISABLE_KEYRING", "1")
        .env("AIBUDDY_PATH_ROOT", path_root.path())
        .stdin(Stdio::piped())
        .spawn()
        .unwrap_or_else(|err| {
            eprintln!("failed to spawn {aibuddy}: {err}");
            std::process::exit(1);
        });
    ...
    child
        ...
        .expect("write probe script to aibuddy stdin");

    let status = child.wait().expect("wait for aibuddy");
