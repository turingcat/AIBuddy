...
use aibuddy::agents::{Agent, Container, ExtensionError};
use aibuddy::config::extensions::name_to_key;
use aibuddy::config::resolve_extensions_for_new_session;
use aibuddy::config::{AIBuddyMode, Config, ExtensionConfig};
use aibuddy::model_config::model_config_from_user_config;
use aibuddy::providers::create;
use aibuddy::recipe::Recipe;
use aibuddy::session::session_manager::SessionType;
use aibuddy::session::EnabledExtensionsState;
use console::style;
...
/// Configuration for building a new AIBuddy session
///
...
async fn resolve_provider_and_model(
    ...
    saved_model_config: Option<aibuddy_providers::model::ModelConfig>,
) -> ResolvedProviderConfig {
    let recipe_settings = session_config
        .recipe
        .as_ref()
        .and_then(|r| r.settings.as_ref());
    let configured_provider = config.get_aibuddy_provider().ok();

    let provider_name = session_config
        .provider
        .clone()
        .or_else(|| saved_provider.clone())
        .or_else(|| recipe_settings.and_then(|s| s.aibuddy_provider.clone()))
        .or_else(|| configured_provider.clone())
        .unwrap_or_else(|| {
            output::render_error("No provider configured. Run 'aibuddy configure' first.");
            process::exit(1);
        });
    ...
    let matching_environment_model =
        if provider_overridden && configured_provider.as_deref() == Some(provider_name.as_str()) {
            std::env::var("AIBUDDY_MODEL").ok()
        } else {
            None
        };
    let matching_config_model =
        if provider_overridden && configured_provider.as_deref() == Some(provider_name.as_str()) {
            config.get_aibuddy_model().ok()
        } else {
            None
        };
    let configured_provider_model = session_config.provider.as_ref().and_then(|_| {
        aibuddy::config::get_provider_entry(config, &provider_name)
    ...
    {
        Some(
            aibuddy::providers::get_from_registry(&provider_name)
                .await
                .unwrap_or_else(|e| {
    ...
    let model_name = session_config
        ...
        .or_else(|| {
            if provider_overridden {
                ...
                recipe_settings.and_then(|s| s.aibuddy_model.clone())
            }
        })
        .or_else(|| {
            if provider_overridden {
        ...
        .unwrap_or_else(|| {
            output::render_error("No model configured. Run 'aibuddy configure' first.");
            process::exit(1);
        });
            ...
            .is_some_and(|mc| mc.model_name == model_name)
    {
        ...
        config = aibuddy::model_config::with_rederived_cache_ttl(config).unwrap_or_else(|e| {
...
async fn resolve_session_id(
    session_config: &SessionBuilderConfig,
    session_manager: &aibuddy::session::session_manager::SessionManager,
...
async fn collect_extension_configs(
    ...
    if !session_config.no_profile && !session_config.resume && recipe_extensions.is_none() {
        ...
        all.extend(
            aibuddy::plugins::mcp_servers::enabled_plugin_mcp_servers(project_root.as_deref())
                .into_iter()
                .map(|config| (config.name(), config)),
        );
    }
...
async fn configure_session_prompts(
    ...
    let system_prompt_file: Option<String> =
        config.get_param("AIBUDDY_SYSTEM_PROMPT_FILE_PATH").ok();
...
pub async fn build_session(session_config: SessionBuilderConfig) -> CliSession {
    ...
    let session_id =
        resolve_session_id(&session_config, &session_manager, agent.config.aibuddy_mode).await;
    ...
    let (new_provider, effective_provider_name, effective_model_name, effective_model_config) =
        match create(&resolved.provider_name, extensions_for_provider.clone()).await {
            ...
            {
                let fallback_provider = config.get_aibuddy_provider().unwrap_or_else(|_| {
                    output::render_error("No provider configured. Run 'aibuddy configure' first.");
                    process::exit(1);
                });
                let fallback_model = config.get_aibuddy_model().unwrap_or_else(|_| {
                    output::render_error("No model configured. Run 'aibuddy configure' first.");
                    process::exit(1);
                });
                eprintln!(
    ...
    agent
        .update_aibuddy_mode(agent.config.aibuddy_mode, &session_id)
        .await
        .unwrap_or_else(|e| {
    ...
    let debug_mode = session_config.debug || config.get_param("AIBUDDY_DEBUG").unwrap_or(false);
...
mod tests {
    use super::*;
    use aibuddy::config::{set_provider_entry, ProviderEntry};
    ...
    fn clear_provider_env() -> env_lock::EnvGuard<'static> {
        env_lock::lock_env([
            ("AIBUDDY_PROVIDER", None::<&str>),
            ("AIBUDDY_MODEL", None::<&str>),
        ])
    }

    fn saved_model_config(model_name: &str) -> aibuddy_providers::model::ModelConfig {
        aibuddy_providers::model::ModelConfig::new(model_name).with_merged_request_params(
    ...
    fn test_session_builder_config_creation() {
        let config = SessionBuilderConfig {
            ...
            streamable_http_extensions: vec![StreamableHttpOptions {
                ...
                timeout: aibuddy::config::DEFAULT_EXTENSION_TIMEOUT,
            }],
    ...
    async fn resume_provider_override_uses_target_provider_model() {
        ...
        let resolved = resolve_provider_and_model(
            ...
            Some(aibuddy_providers::model::ModelConfig::new(
                "claude-sonnet-4-6",
            )),
        )
        .await;
    ...
    async fn matching_provider_override_preserves_configured_model() {
        ...
        let temp_dir = TempDir::new().unwrap();
        let config = test_config(&temp_dir);
        config.set_param("AIBUDDY_PROVIDER", "openai").unwrap();
        config
            .set_param("AIBUDDY_MODEL", "my-custom-model")
            .unwrap();
    ...
    async fn matching_environment_model_overrides_saved_model() {
        let _guard = env_lock::lock_env([
            ("AIBUDDY_PROVIDER", Some("openai")),
            ("AIBUDDY_MODEL", Some("environment-model")),
        ]);
        let temp_dir = TempDir::new().unwrap();
        let config = test_config(&temp_dir);
        set_provider_entry(
        ...
        let resolved = resolve_provider_and_model(
            ...
            Some(aibuddy_providers::model::ModelConfig::new("saved-model")),
        )
        .await;
    ...
    async fn matching_provider_override_preserves_recipe_model() {
        ...
        config.set_param("AIBUDDY_PROVIDER", "openai").unwrap();
        config
            .set_param("AIBUDDY_MODEL", "configured-model")
            .unwrap();
    ...
    async fn resume_provider_override_uses_target_provider_default_instead_of_active_model() {
        ...
        let temp_dir = TempDir::new().unwrap();
        let config = test_config(&temp_dir);
        config.set_param("AIBUDDY_PROVIDER", "anthropic").unwrap();
        config
            .set_param("AIBUDDY_MODEL", "claude-sonnet-4-6")
            .unwrap();
        let expected_model = aibuddy::providers::get_from_registry("openai")
            .await
            .unwrap()
            .metadata()
            .default_model
    ...
    async fn resume_rederives_cache_ttl_from_config_not_session() {
        let _guard = env_lock::lock_env([
            ("AIBUDDY_PROVIDER", None::<&str>),
            ("AIBUDDY_MODEL", None::<&str>),
            ("AIBUDDY_CACHE_TTL", Some("1h")),
        ]);
        let temp_dir = TempDir::new().unwrap();
        let config = test_config(&temp_dir);
        let saved =
            aibuddy_providers::model::ModelConfig::new("claude-sonnet-4-6").with_cache_ttl("5m");
    ...
    async fn resume_drops_saved_cache_ttl_when_config_absent() {
        let _guard = env_lock::lock_env([
            ("AIBUDDY_PROVIDER", None::<&str>),
            ("AIBUDDY_MODEL", None::<&str>),
            ("AIBUDDY_CACHE_TTL", None::<&str>),
        ]);
        let temp_dir = TempDir::new().unwrap();
        let config = test_config(&temp_dir);
        let saved =
            aibuddy_providers::model::ModelConfig::new("claude-sonnet-4-6").with_cache_ttl("1h");
    ...
    async fn headless_resume_clamps_rederived_cache_ttl() {
        ...
        let temp_dir = TempDir::new().unwrap();
        let config = test_config(&temp_dir);
        let saved = aibuddy_providers::model::ModelConfig::new("claude-sonnet-4-6");
    ...
    async fn test_implicit_resume_ignores_newer_scheduled_sessions() {
        ...
        let aibuddy_mode = AIBuddyMode::default();
