...
use aibuddy::providers::canonical_cost::estimate_model_cost;
#[cfg(target_os = "windows")]
use aibuddy::subprocess::SubprocessExt;
    ...
    fn as_str(&self) -> String {
        match self {
            Theme::Light => Config::global()
                .get_param::<String>("AIBUDDY_CLI_LIGHT_THEME")
                .unwrap_or(DEFAULT_CLI_LIGHT_THEME.to_string()),
            Theme::Dark => Config::global()
                .get_param::<String>("AIBUDDY_CLI_DARK_THEME")
...
thread_local! {
    static CURRENT_THEME: RefCell<Theme> = RefCell::new(
        std::env::var("AIBUDDY_CLI_THEME").ok()
            .map(|val| Theme::from_config_str(&val))
            .unwrap_or_else(||
                Config::global().get_param::<String>("AIBUDDY_CLI_THEME").ok()
                    .map(|val| Theme::from_config_str(&val))
                    .unwrap_or(Theme::Ansi)
            )
    );
    static SHOW_FULL_TOOL_OUTPUT: RefCell<bool> = RefCell::new(
        Config::global().get_param::<bool>("AIBUDDY_SHOW_FULL_OUTPUT").unwrap_or(false)
    );
}
...
pub fn set_theme(theme: Theme) {
    ...
    config
        .set_param("AIBUDDY_CLI_THEME", theme.as_config_string())
    ...
    if let Err(e) = config.set_param("AIBUDDY_CLI_THEME", theme_str) {
        ...
        match failure.label.as_deref() {
            ...
            Some(label) => {
                ...
                eprintln!(
                    "{}",
                    style(format!(
                        "    Hint: ask aibuddy to help debug the '{}' extension",
                        label
                    ))
                    .dim()
                );
...
pub fn run_status_hook(status: &str) {
    if let Ok(hook) = Config::global().get_param::<String>("AIBUDDY_STATUS_HOOK") {
...
pub fn aibuddy_mode_message(text: &str) {
    println!("\n{} {}", accent("mode:"), text);
}

fn should_show_thinking() -> bool {
    Config::global()
        .get_param::<bool>("AIBUDDY_CLI_SHOW_THINKING")
        .unwrap_or(false)
...
pub fn display_session_info(
    ...
    // ASCII art aibuddy with session info on the right
    println!();
    println!(
        "  {}  {} {} {} {} {}",
        style("  __( O)>").white(),
...
mod tests {
    ...
    fn terminal_line_sanitizer_preserves_plain_unicode_text() {
        assert_eq!(
            sanitize_terminal_line("aibuddy 🪿\t日本語"),
            "aibuddy 🪿\t日本語"
        );
    }
    ...
    fn tool_confirmation_renders_authoritative_details_on_stderr() {
        const CHILD_ENV: &str = "AIBUDDY_TEST_TOOL_CONFIRMATION_STDERR_CHILD";
