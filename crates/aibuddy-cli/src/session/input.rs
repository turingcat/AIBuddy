use super::completion::AIBuddyCompleter;
use super::paste::{
...
/// prompt. Configurable via `AIBUDDY_CLI_NEWLINE_KEY`, defaulting to `j` (Ctrl+J).
...
pub fn get_newline_key() -> char {
    Config::global()
        .get_param::<String>("AIBUDDY_CLI_NEWLINE_KEY")
        .ok()
...
///
/// When `aibuddy_prompt_editor` is configured, defaults to `true` (backward compat).
/// Users can override by explicitly setting `aibuddy_prompt_editor_always` to `false`.
...
pub fn get_input(
    editor: &mut Editor<AIBuddyCompleter, rustyline::history::DefaultHistory>,
    conversation_messages: Option<&Vec<String>>,
) -> Result<InputResult> {
    let config = Config::global();
    let prompt_editor = config.get_aibuddy_prompt_editor().ok().flatten();
    let editor_always_override = config.get_aibuddy_prompt_editor_always().ok().flatten();
    let editor_always = should_use_editor_always(prompt_editor.as_deref(), editor_always_override);

    if editor_always {
        if let Ok(Some(editor_cmd)) = config.get_aibuddy_prompt_editor() {
...
fn handle_slash_command(input: &str) -> Option<InputResult> {
    ...
    match input {
        ...
        s if s.starts_with(CMD_MODE) => Some(InputResult::AIBuddyMode(
...
fn help_text() -> String {
    let modes = AIBuddyMode::VARIANTS.join(", ");
...
/mode <name> - Set the aibuddy mode to use ({modes})
...
/plan <message_text> -  Enters 'plan' mode with optional message. Create a plan based on the current messages and asks user if they want to act on it.
                        If user acts on the plan, aibuddy mode is set to 'auto' and returns to 'normal' aibuddy mode.
                        To warm up aibuddy before using '/plan', we recommend setting '/mode approve' & putting appropriate context into aibuddy.
                        The model is used based on $AIBUDDY_PLANNER_PROVIDER and $AIBUDDY_PLANNER_MODEL environment variables.
...
/edit [text] - Open your prompt editor to compose a message. Optionally pre-fill with text.
               Uses $AIBUDDY_PROMPT_EDITOR, $VISUAL, or $EDITOR (in that order).
...
Navigation:
Enter - Send message
Ctrl+{newline_key} - Add a newline (configurable via AIBUDDY_CLI_NEWLINE_KEY)
...
fn additional_builtin_help() -> String {
    ...
    aibuddy::agents::execute_commands::list_commands()
        .iter()
...
fn print_editor_help() {
  ...
  Configure editor: aibuddy configure set aibuddy_prompt_editor \"vim\"
  Falls back to $VISUAL or $EDITOR if aibuddy_prompt_editor is not set.
  When aibuddy_prompt_editor is set, the editor is used for every prompt by default.
  To use inline prompts with on-demand /edit: aibuddy configure set aibuddy_prompt_editor_always false"
    );
}
...
mod tests {
    ...
    fn help_lists_builtin_agent_commands() {
        ...
        for command in aibuddy::agents::execute_commands::list_commands() {
            assert!(
