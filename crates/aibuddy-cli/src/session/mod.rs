...
use aibuddy::agents::platform_extensions::developer::shell::{
    parse_shell_output_notification, ShellOutputNotificationParams, ShellOutputStream,
};
use aibuddy::agents::AgentEvent;
use aibuddy::agents::SUBAGENT_TOOL_REQUEST_TYPE;
use aibuddy::permission::Permission;
use aibuddy::providers::base::Provider;
use aibuddy::providers::base::ProviderUsage;
use aibuddy::utils::safe_truncate;

use aibuddy::agents::extension::{Envs, ExtensionConfig, PLATFORM_EXTENSIONS};
use aibuddy::agents::types::RetryConfig;
use aibuddy::agents::{
    context_management_unsupported_message, Agent, SessionConfig, COMPACT_TRIGGERS,
};
use aibuddy::config::extensions::name_to_key;
use aibuddy::config::{AIBuddyMode, Config};
use anyhow::{Context, Result};
use completion::AIBuddyCompleter;
...
impl HistoryManager {
    ...
    fn load(
        &self,
        editor: &mut rustyline::Editor<AIBuddyCompleter, rustyline::history::DefaultHistory>,
    ) {
...
pub async fn classify_planner_response(
    ...
    model_config: aibuddy_providers::model::ModelConfig,
    ...
    let (result, _usage) = aibuddy::session_context::with_session_id(
...
impl CliSession {
    ...
    pub fn parse_stdio_extension(extension_command: &str) -> Result<ExtensionConfig> {
        ...
        let mut parts = aibuddy::utils::split_command_args(command)?;
        ...
        Ok(ExtensionConfig::Stdio {
            ...
            envs: Envs::new(envs),
            env_keys: Vec::new(),
            description: aibuddy::config::DEFAULT_EXTENSION_DESCRIPTION.to_string(),
            timeout: Some(aibuddy::config::DEFAULT_EXTENSION_TIMEOUT),
            cwd: None,
            bundled: None,
    ...
    pub async fn interactive(&mut self, prompt: Option<String>) -> Result<()> {
        let banners = self
            .agent
            .emit_hook_with_banners(aibuddy::hooks::HookEvent::SessionStart, &self.session_id)
            .await;
        if !banners.is_empty() {
        ...
        self.agent
            .emit_hook(aibuddy::hooks::HookEvent::SessionEnd, &self.session_id)
            .await;
    ...
    fn create_editor(
        &self,
    ) -> Result<rustyline::Editor<AIBuddyCompleter, rustyline::history::DefaultHistory>> {
        let builder =
        ...
        let mut editor =
            rustyline::Editor::<AIBuddyCompleter, rustyline::history::DefaultHistory>::with_config(
                config,
            )?;
        let completer = AIBuddyCompleter::new(self.completion_cache.clone());
    ...
    async fn handle_input(
        ...
        match input {
            ...
            InputResult::AIBuddyMode(mode) => {
                history.save(editor);
                self.handle_aibuddy_mode(&mode).await?;
            }
                    ...
                    None => {
                        output::render_error(
                            "No editor found. Set one with:\n  \
                                 aibuddy configure set aibuddy_prompt_editor \"vim\"\n  \
    ...
    async fn handle_aibuddy_mode(&self, mode: &str) -> Result<()> {
        let config = Config::global();
        let mode = match AIBuddyMode::from_str(&mode.to_lowercase()) {
            Ok(mode) => mode,
            Err(_) => {
                output::render_error(&format!(
                    "Invalid mode '{mode}'. Mode must be one of: {}",
                    AIBuddyMode::VARIANTS.join(", ")
                ));
                return Ok(());
            }
        };
        self.agent
            .update_aibuddy_mode(mode, &self.session_id)
            .await?;
        config.set_aibuddy_mode(mode)?;
        output::aibuddy_mode_message(&format!("AIBuddy mode set to '{mode}'"));
        Ok(())
    }
    ...
    async fn handle_model(&mut self, options: input::ModelCommandOptions) -> Result<()> {
        ...
        if options.provider.is_none() && options.model.is_none() {
            output::aibuddy_mode_message(&format!(
        ...
        let target_entry = match aibuddy::providers::get_from_registry(target_provider_name).await {
            Ok(entry) => entry,
            Err(_) => {
                output::render_error(&format!(
        ...
        let configured_effort = Config::global().get_aibuddy_thinking_effort();
        ...
        let current_context_limit = aibuddy::context_limit::get_context_limit(
            provider.as_ref(),
        ...
        let new_provider = match aibuddy::providers::create(target_provider_name, extensions).await
        {
            Ok(p) => p,
            Err(e) => {
                output::render_error(&format!(
                    "Cannot switch to provider '{}': {}\n\
                         Set credentials via `aibuddy configure` or the appropriate environment variable.\n\
        ...
        let new_context_limit = aibuddy::context_limit::get_context_limit(
            new_provider.as_ref(),
        ...
        let mode = self.agent.aibuddy_mode().await;
        self.agent
            .update_aibuddy_mode(mode, &self.session_id)
            .await?;
    ...
    async fn handle_clear(&mut self) -> Result<()> {
        ...
        if let Err(e) = self
            ...
            .usage(aibuddy_providers::conversation::token_usage::Usage::new(
                Some(0),
                Some(0),
                Some(0),
            ))
    ...
    async fn handle_new(&mut self) -> Result<()> {
        ...
        if let Err(e) = self
            .agent
            .update_aibuddy_mode(self.agent.aibuddy_mode().await, &self.session_id)
            .await
        {
        ...
        if !extension_configs.is_empty() {
            output::aibuddy_mode_message("Restarting extensions for the new session...");
        }
    ...
    async fn handle_list_skills(&mut self) -> Result<()> {
        use aibuddy::custom_requests::SourceType;
        use aibuddy::skills::list_installed_skills;
    ...
    async fn plan_with_reasoner_model(
        ...
        output::show_thinking();
        let (plan_response, _usage) = aibuddy::session_context::with_session_id(
        ...
        match planner_response_type {
            PlannerResponseType::Plan => {
                ...
                if should_act {
                    ...
                    self.run_mode = RunMode::Normal;
                    // set aibuddy mode: auto if that isn't already the case
                    let config = Config::global();
                    let curr_aibuddy_mode = config.get_aibuddy_mode().unwrap_or_default();
                    if curr_aibuddy_mode != AIBuddyMode::Auto {
                        config.set_aibuddy_mode(AIBuddyMode::Auto).unwrap();
                    }
                    ...
                    // Reset run & aibuddy mode
                    if curr_aibuddy_mode != AIBuddyMode::Auto {
                        config.set_aibuddy_mode(curr_aibuddy_mode)?;
                    }
                } else {
                        ...
                        Some(Ok(AgentEvent::Message(message))) => {
                            ...
                            if let Some((id, security_prompt)) = find_tool_confirmation(&message) {
                                let selected_permission = if interactive {
                                    ...
                                    let config = Config::global();
                                    let aibuddy_mode = config.get_aibuddy_mode().unwrap_or(AIBuddyMode::Auto);
                                    if aibuddy_mode == AIBuddyMode::Approve || aibuddy_mode == AIBuddyMode::SmartApprove {
                                        cancel_token_clone.cancel();
                                        drop(stream);
                                        return Err(anyhow::anyhow!(
                                            "Tool approval required in non-interactive mode with AIBuddyMode::{aibuddy_mode}. \
                                             This is an invalid configuration — Approve/SmartApprove modes require an \
                                             interactive terminal. Use AIBuddyMode::Auto for headless sessions."
                                        ));
                                    }
                                    tracing::warn!(
    ...
    async fn refresh_completion_cache(
        ...
        let all_providers = aibuddy::providers::providers().await;
    ...
    pub async fn get_session(&self) -> Result<aibuddy::session::Session> {
        self.agent
            .config
            .session_manager
    ...
    pub async fn display_context_usage(&self) -> Result<()> {
        ...
        let model_config = self
            ...
            .await?;
        let context_limit =
            aibuddy::context_limit::get_context_limit(provider.as_ref(), &model_config.model_name)
                .await?;

        let config = Config::global();
        let show_cost = config
            .get_param::<bool>("AIBUDDY_CLI_SHOW_COST")
            .unwrap_or(false);

        let provider_name = config
            .get_aibuddy_provider()
...
async fn create_successor_session(
    ...
    old_session: &aibuddy::session::Session,
                ...
                let formatted = match notification_type {
                    ...
                    Some("response_generated") => {
                        ...
                        let min_priority = config
                            .get_param::<f32>("AIBUDDY_CLI_MIN_PRIORITY")
                            .ok()
    ...
    for content in &message.content {
        if let MessageContent::ToolRequest(tool_request) = content {
            if let Ok(tool_call) = &tool_request.tool_call {
                tracing::info!(
                    monotonic_counter.aibuddy.tool_calls = 1,
        ...
        if let MessageContent::ToolResponse(tool_response) = content {
            ...
            tracing::info!(
                monotonic_counter.aibuddy.tool_completions = 1,
                tool_name = %tool_name,
...
fn handle_agent_error(e: &anyhow::Error, is_stream_json_mode: bool) {
    ...
    if e.downcast_ref::<aibuddy_providers::errors::ProviderError>()
        .map(|provider_error| {
            matches!(
                provider_error,
                aibuddy_providers::errors::ProviderError::ContextLengthExceeded(_)
            )
        })
        .unwrap_or(false)
    {
...
) -> Result<(Arc<dyn Provider>, aibuddy_providers::model::ModelConfig), anyhow::Error> {
    use aibuddy::providers::create;

    let config = Config::global();

    // Try planner-specific provider first, fall back to default provider
    let provider = if let Ok(provider) = config.get_param::<String>("AIBUDDY_PLANNER_PROVIDER") {
        provider
    } else {
        println!("WARNING: AIBUDDY_PLANNER_PROVIDER not found. Using default provider...");
        config
            .get_aibuddy_provider()
            .expect("No provider configured. Run 'aibuddy configure' first")
    };

    // Try planner-specific model first, fall back to default model
    let model = if let Ok(model) = config.get_param::<String>("AIBUDDY_PLANNER_MODEL") {
        model
    } else {
        println!("WARNING: AIBUDDY_PLANNER_MODEL not found. Using default model...");
        config
            .get_aibuddy_model()
            .expect("No model configured. Run 'aibuddy configure' first")
    };

    let model_config =
        aibuddy::model_config::model_config_from_user_config(&provider, model.as_str())?;
    let extensions = aibuddy::config::extensions::get_enabled_extensions_with_config(config);
...
fn build_switched_model_config(
    provider_name: &str,
    model_name: &str,
    current_model_config: &aibuddy_providers::model::ModelConfig,
) -> Result<aibuddy_providers::model::ModelConfig> {
    aibuddy::model_config::model_config_from_user_config(provider_name, model_name)
        .map(|config| {
            config
...
mod tests {
    ...
    fn planner_history_excludes_turn_context_events() {
        use aibuddy::conversation::message::MessageMetadata;
    ...
    fn test_build_switched_model_config_rebuilds_target_model_settings() {
        let _guard = env_lock::lock_env([
            ("AIBUDDY_MAX_TOKENS", None::<&str>),
            ("AIBUDDY_TEMPERATURE", None::<&str>),
            ("AIBUDDY_CONTEXT_LIMIT", None::<&str>),
            ("AIBUDDY_TOOLSHIM", None::<&str>),
            ("AIBUDDY_TOOLSHIM_OLLAMA_MODEL", None::<&str>),
        ]);

        let current_model_config = aibuddy_providers::model::ModelConfig {
        ...
        let expected = aibuddy_providers::model::ModelConfig::new("gpt-5.4")
    ...
    fn test_build_switched_model_config_detects_effort_suffix_change() {
        ...
        let current = aibuddy_providers::model::ModelConfig::new("gpt-5.4-high")
            .with_canonical_limits("openai");
        assert_eq!(current.model_name, "gpt-5.4");
        assert_eq!(
            current.thinking_effort(),
            Some(aibuddy_providers::thinking::ThinkingEffort::High)
        );
    ...
    fn test_split_command_args_windows_paths() {
        assert_eq!(
            aibuddy::utils::split_command_args(r"C:\tools\mcp.exe --arg value").unwrap(),
            vec![r"C:\tools\mcp.exe", "--arg", "value"]
        );
        assert_eq!(
            aibuddy::utils::split_command_args(r#""C:\Program Files\server\mcp.exe" --arg"#)
                .unwrap(),
    ...
    fn test_split_command_args_unmatched_quote() {
        assert!(aibuddy::utils::split_command_args(r#""unmatched"#).is_err());
    }
    ...
    async fn new_session_inherits_provider_model_and_working_dir() {
        ...
        let old = sm
            .create_session(
                ...
                "CLI Session".to_string(),
                aibuddy::session::SessionType::User,
                AIBuddyMode::Auto,
            )
            .await
            .unwrap();
        ...
        sm.update(&old.id)
            ...
            .model_config(aibuddy_providers::model::ModelConfig::new("test-model"))
            .accumulated_usage(aibuddy_providers::conversation::token_usage::Usage::new(
                Some(100),
                Some(50),
                Some(150),
            ))
        ...
        let mut extension_data = aibuddy::session::ExtensionData::new();
        ...
        let new_id = create_successor_session(&sm, &old, AIBuddyMode::Chat)
            .await
            .unwrap();
        ...
        assert_eq!(new_session.aibuddy_mode, AIBuddyMode::Chat);
    ...
    impl Provider for StubProvider {
        ...
        async fn stream(
            &self,
            _model_config: &aibuddy_providers::model::ModelConfig,
            _system: &str,
            _messages: &[Message],
            _tools: &[rmcp::model::Tool],
        ) -> std::result::Result<
            aibuddy::providers::base::MessageStream,
            aibuddy_providers::errors::ProviderError,
        > {
            Ok(aibuddy::providers::base::stream_from_single_message(
                Message::assistant().with_text("stub reply"),
                ProviderUsage::new(
                    "stub".to_string(),
                    aibuddy_providers::conversation::token_usage::Usage::default(),
                ),
            ))
        }
    }
    ...
    async fn session_with_loader(
        ...
        let agent = aibuddy::agents::Agent::with_config(aibuddy::agents::AgentConfig::new(
            Arc::new(session_manager),
            Arc::new(aibuddy::config::PermissionManager::new(
                temp_dir.path().to_path_buf(),
            )),
            None,
            AIBuddyMode::default(),
            ...
            true,
            aibuddy::agents::AIBuddyPlatform::AIBuddyCli,
        ));
        agent
            .update_provider(
                Arc::new(StubProvider),
                aibuddy_providers::model::ModelConfig::new("stub-model"),
                &session.id,
            )
            .await
            .unwrap();
