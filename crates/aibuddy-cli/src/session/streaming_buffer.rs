...
//!
//! ```
//! use aibuddy_cli::session::streaming_buffer::MarkdownBuffer;
//!
...
fn max_code_block_lines() -> Option<usize> {
    static VALUE: LazyLock<Option<usize>> = LazyLock::new(|| {
        if std::env::var("AIBUDDY_NO_CODE_TRUNCATION")
        ...
        Some(
            std::env::var("AIBUDDY_MAX_CODE_BLOCK_LINES")
                .ok()
...
fn truncated_show_lines() -> usize {
    static VALUE: LazyLock<usize> = LazyLock::new(|| {
        std::env::var("AIBUDDY_TRUNCATED_SHOW_LINES")
            .ok()
            .and_then(|v| parse_positive_lines(&v))
...
mod tests {
    ...
    ///   cargo test -p aibuddy-cli --release -- --ignored large_code_block --nocapture
    #[test]
    #[ignore]
