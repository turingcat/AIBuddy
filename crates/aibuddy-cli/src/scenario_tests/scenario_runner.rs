...
use aibuddy::agents::{AIBuddyPlatform, Agent, AgentConfig};
use aibuddy::config::permission::PermissionManager;
use aibuddy::config::AIBuddyMode;
use aibuddy::providers::{create, testprovider::TestProvider};
...
{
    if let Ok(only_provider) = std::env::var("AIBUDDY_TEST_PROVIDER") {
    ...
    F: Fn(&ScenarioResult) -> Result<()>,
{
    ...
    aibuddy::agents::moim::SKIP.with(|f| f.set(true));
    ...
    let scenario_model_config =
        aibuddy::model_config::model_config_from_user_config(&factory_name, config.model_name)?;
    agent
        .update_provider(
            provider_arc as Arc<dyn aibuddy::providers::base::Provider>,
