use aibuddy::agents::AIBuddyPlatform;
use aibuddy::builtin_extension::register_builtin_extensions;
use aibuddy::config::legacy::migrate_config_content;
use aibuddy::config::{AIBuddyMode, Config};
use aibuddy::recipe::Recipe;
use aibuddy::source_roots::SourceRoot;
use aibuddy_mcp::mcp_server_runner::{serve, McpCommand};
use aibuddy_mcp::{AutoVisualiserRouter, ComputerControllerServer, MemoryServer, TutorialServer};
use anyhow::Result;
...
use aibuddy::agents::Container;
use aibuddy::session::session_manager::SessionType;
use aibuddy::session::SessionManager;
use std::io::{Read, Write};
use std::path::{Path, PathBuf};
const AIBUDDY_SERVER_SECRET_KEY_ENV: &str = "AIBUDDY_SERVER__SECRET_KEY";
...
impl From<ServePlatform> for AIBuddyPlatform {
    fn from(platform: ServePlatform) -> Self {
        match platform {
            ServePlatform::Cli => AIBuddyPlatform::AIBuddyCli,
            ServePlatform::Desktop => AIBuddyPlatform::AIBuddyDesktop,
        }
    }
}

#[derive(Parser)]
#[command(name = "aibuddy", author, version, display_name = "", about, long_about = None)]
pub struct Cli {
    #[command(subcommand)]
...
pub struct SessionOptions {
    ...
    #[arg(
        ...
        long_help = "Run extensions (stdio and built-in) inside the specified container. The extension must exist in the container. For built-in extensions, aibuddy must be installed inside the container."
    )]
...
fn parse_streamable_http_extension(input: &str) -> Result<StreamableHttpOptions, String> {
    ...
    let (mut url, mut timeout) = (String::new(), aibuddy::config::DEFAULT_EXTENSION_TIMEOUT);
...
pub struct ExtensionOptions {
    ...
    #[arg(
        ...
        long_help = "Add one or more builtin extensions that are bundled with aibuddy by specifying their names, comma-separated",
        value_delimiter = ','
    )]
...
pub struct InputOptions {
    ...
    #[arg(
        ...
        long = "text",
        value_name = "TEXT",
        help = "Input text to provide to aibuddy directly",
        long_help = "Input text containing commands for aibuddy. Use this in lieu of the instructions argument.",
    ...
    #[arg(
        ...
        help = "Dynamic parameters (e.g., --params username=alice --params channel_name=aibuddy-channel)",
...
pub struct ModelOptions {
    ...
    #[arg(
        ...
        long_help = "Override the AIBUDDY_PROVIDER environment variable for this run. Available providers include openai, anthropic, ollama, databricks, gemini-cli, claude-code, and others."
    )]
    ...
    #[arg(
        ...
        long_help = "Override the AIBUDDY_MODEL environment variable for this run. The model must be supported by the specified provider."
    )]
...
enum SessionCommand {
    ...
    Export {
        ...
        #[arg(
            long = "nostr",
            help = "Publish the JSON session export as an encrypted Nostr event and print a AIBuddy share link"
        )]
        nostr: bool,
    ...
    Import {
        #[arg(
            help = "Path to a aibuddy session export, a Claude Code, Codex, or Pi .jsonl transcript, or a aibuddy://sessions/nostr share link"
        )]
        input: String,
...
#[derive(Subcommand)]
enum SkillsCommand {
    /// List all skills available to the aibuddy agent
    #[command(about = "List all skills available to the aibuddy agent")]
    List,
}
...
enum RecipeCommand {
    ...
    #[command(about = "Open a recipe in AIBuddy Desktop")]
    Open {
...
#[derive(Subcommand)]
enum Command {
    /// Migrate a legacy YAML config to canonical AIBuddy keys
    #[command(about = "Migrate legacy config keys without changing the source file")]
    MigrateConfig(MigrateConfigArgs),

    /// Configure aibuddy settings
    #[command(about = "Configure aibuddy settings")]
    Configure {},

    /// Display aibuddy configuration information
    #[command(about = "Display aibuddy information")]
    Info {
    ...
    #[command(about = "Check that your AIBuddy setup is working")]
    Doctor {},

    /// Manage system prompts and behaviors
    #[command(about = "Run one of the mcp servers bundled with aibuddy")]
    Mcp {
        #[arg(value_parser = clap::value_parser!(McpCommand))]
        server: McpCommand,
    },

    /// Run aibuddy as an ACP (Agent Client Protocol) agent
    #[command(about = "Run aibuddy as an ACP agent server on stdio")]
    Acp {
    ...
    Serve {
        ...
        #[arg(
            ...
            help = "Start the ACP endpoint without requiring AIBUDDY_SERVER__SECRET_KEY"
        )]
        ...
        /// Also expose this server over aibuddy roam (p2p) so paired devices can connect remotely
        #[cfg(feature = "roaming")]
        #[arg(
            long,
            help = "Also expose this server over aibuddy roam (p2p) so paired devices can connect remotely"
        )]
        roam: bool,
    },
    ...
    /// Update the aibuddy CLI version
    #[cfg(feature = "update")]
    #[command(about = "Update the aibuddy CLI version")]
    Update {
        ...
        #[arg(
            ...
            long_help = "Update to the latest canary version of the aibuddy CLI, otherwise updates to the latest stable version."
        )]
        canary: bool,

        /// Enforce to re-configure aibuddy during update
        #[arg(short, long, help = "Enforce to re-configure aibuddy during update")]
        reconfigure: bool,
    },

    /// Terminal-integrated session (one session per terminal)
    #[command(
        about = "Terminal-integrated aibuddy session",
        long_about = "Runs a aibuddy session tied to your terminal window.\n\
                      Each terminal maintains its own persistent session that resumes automatically.\n\n\
                      Setup:\n  \
                        eval \"$(aibuddy term init zsh)\"  # zsh/bash\n  \
                        let init = ($nu.cache-dir | path join \"aibuddy-term-init.nu\"); ^aibuddy term init nu | save --force $init; source $init\n\n\
                      Usage:\n  \
                        aibuddy term run \"list files in this directory\"\n  \
                        @aibuddy \"create a python script\"  # using alias\n  \
    ...
    Completion {
        ...
        #[arg(long, default_value = "aibuddy", help = "Provide a custom binary name")]
        bin_name: String,
    },
    ...
    /// runs the review through aibuddy.
    #[command(about = "Review the current diff using aibuddy")]
    Review {
        ...
        /// orchestrator dispatches one `aibuddy run` subprocess per check
    ...
    #[command(
        name = "mcp-probe",
        about = "Start a AIBuddy MCP session without an LLM and inspect a stdio MCP server",
        hide = true
    )]
    McpProbe {
...
enum TermCommand {
    ...
    #[command(
        ...
        long_about = "Prints shell configuration to set up terminal-integrated sessions.\n\
                      Each terminal gets a persistent aibuddy session that automatically resumes.\n\n\
                      Setup:\n  \
                        echo 'eval \"$(aibuddy term init zsh)\"' >> ~/.zshrc\n  \
                        source ~/.zshrc\n\n\
                        Nushell:\n  \
                        let init = ($nu.cache-dir | path join \"aibuddy-term-init.nu\")\n  \
                        ^aibuddy term init nu | save --force $init\n  \
                        source $init\n\n\
                      With --default (anything typed that isn't a command goes to aibuddy):\n  \
                        echo 'eval \"$(aibuddy term init zsh --default)\"' >> ~/.zshrc\n  \
                        ^aibuddy term init nu --default | save --force $init"
    )]
    Init {
        ...
        /// Make aibuddy the default handler for unknown commands
        #[arg(
            long = "default",
            help = "Make aibuddy the default handler for unknown commands",
            long_help = "When enabled, anything you type that isn't a valid command will be sent to aibuddy. Supported for zsh, bash, and nu."
        )]
        default: bool,
    },
    ...
    #[command(
        ...
        long_about = "Run a prompt in the terminal-integrated session.\n\n\
                      Examples:\n  \
                        aibuddy term run list files in this directory\n  \
                        @aibuddy list files  # using alias\n  \
                        @g why did that fail  # short alias"
    )]
    Run {
        /// The prompt to send to aibuddy (multiple words allowed without quotes)
...
struct McpProbeScript {
    ...
    #[serde(default)]
    oauth: aibuddy::oauth::OAuthFlowConfig,
...
async fn handle_mcp_probe(extension_command: String, script_path: Option<String>) -> Result<()> {
    use aibuddy::agents::{Agent, AgentConfig, ToolCallContext};
    ...
    let script = if let Some(path) = script_path {
        ...
        McpProbeScript {
            ...
            elicitation: None,
            oauth: aibuddy::oauth::OAuthFlowConfig::default(),
        ...
        .is_ok_and(|url| matches!(url.scheme(), "http" | "https"))
    {
        crate::session::CliSession::parse_streamable_http_extension(
            &extension_command,
            aibuddy::config::DEFAULT_EXTENSION_TIMEOUT,
        )
    } else {
    ...
    if let Some(client_id) = &script.oauth.client_id {
        std::env::set_var("AIBUDDY_MCP_OAUTH_CLIENT_ID", client_id);
    }
    if let Some(client_secret) = &script.oauth.client_secret {
        std::env::set_var("AIBUDDY_MCP_OAUTH_CLIENT_SECRET", client_secret);
    }
    if let Some(client_metadata_url) = &script.oauth.client_metadata_url {
        std::env::set_var("AIBUDDY_MCP_OAUTH_CLIENT_METADATA_URL", client_metadata_url);
    }

    let config = aibuddy::config::Config::global();
    let mut agent_config = AgentConfig::new(
        std::sync::Arc::new(SessionManager::instance()),
        aibuddy::config::permission::PermissionManager::instance(),
        None,
        config.get_aibuddy_mode().unwrap_or_default(),
        true,
        AIBuddyPlatform::AIBuddyCli,
    );
    ...
    let session = agent
        ...
        .create_session(
            std::env::current_dir()?,
            "MCP Probe".to_string(),
            aibuddy::session::session_manager::SessionType::Hidden,
            agent.config.aibuddy_mode,
        )
        .await?;
...
#[cfg(feature = "roaming")]
type RoamShareSlot =
    std::sync::Arc<tokio::sync::RwLock<Option<std::sync::Arc<aibuddy_roaming::RoamingNode>>>>;

#[cfg(feature = "roaming")]
fn spawn_roam_share(
    server: std::sync::Arc<aibuddy::acp::server_factory::AcpServer>,
) -> RoamShareSlot {
            ...
            match try_acquire_roam_lock_owner() {
                ...
                Ok(None) => {
                    if !standing_by {
                        standing_by = true;
                        eprintln!(
                            "another aibuddy process owns the roaming endpoint; standing by to take over if it exits"
                        );
                    }
                }
...
async fn start_roam_share(
    server: std::sync::Arc<aibuddy::acp::server_factory::AcpServer>,
) -> Result<std::sync::Arc<aibuddy_roaming::RoamingNode>> {
    ...
    use aibuddy_roaming::{RoamingConfig, RoamingNode, TrustBook};
    use std::sync::Arc;
    ...
    let node = RoamingNode::bind(RoamingConfig {
        ...
        directory: aibuddy_roaming::Directory::persistent_owned(directory_path()),
        bind_addr: None,
        relay_tls: None,
    })
    .await?;
    ...
    // Roaming sessions run where `aibuddy serve` was started: the connector's
...
async fn handle_serve_command(args: ServeCommandArgs) -> Result<()> {
    use aibuddy::acp::server::AcpBuiltinSelection;
    use aibuddy::acp::server_factory::{AcpServer, AcpServerFactoryConfig};
    use aibuddy::acp::transport::create_router;
    use aibuddy::config::paths::Paths;
    ...
    let server = Arc::new(AcpServer::new(AcpServerFactoryConfig {
        ...
        aibuddy_platform: platform.into(),
        additional_source_roots,
        session_cwd: None,
        enable_scheduler,
    }));
    let env_secret = std::env::var(AIBUDDY_SERVER_SECRET_KEY_ENV)
        .ok()
    ...
    if !require_token && !dangerously_unauthenticated {
        anyhow::bail!(
            "{AIBUDDY_SERVER_SECRET_KEY_ENV} must be set to start `aibuddy serve`; pass --dangerously-unauthenticated to run without ACP authentication"
        );
    }
    if dangerously_unauthenticated && !require_token {
        warn!(
            "{AIBUDDY_SERVER_SECRET_KEY_ENV} is not set and --dangerously-unauthenticated was passed; the ACP endpoint will accept unauthenticated connections"
        );
    }
    ...
    let tls_cert_path =
        tls_cert_path.or_else(|| config.get_param::<String>("AIBUDDY_TLS_CERT_PATH").ok());
    let tls_key_path =
        tls_key_path.or_else(|| config.get_param::<String>("AIBUDDY_TLS_KEY_PATH").ok());
    let tls = tls
        || config.get_param::<bool>("AIBUDDY_TLS").unwrap_or(false)
    ...
    if tls {
        ...
        {
            let tls_setup = aibuddy::acp::transport::tls::setup_tls(
...
async fn handle_interactive_session(args: InteractiveSessionArgs) -> Result<()> {
    ...
    tracing::info!(
        monotonic_counter.aibuddy.session_starts = 1,
        session_type,
        interactive = true,
        "Session started"
    );
    ...
    let aibuddy_mode = Config::global().get_aibuddy_mode().unwrap_or_default();
    let mut session_id = get_or_create_session_id(identifier, resume, false, aibuddy_mode).await?;
...
async fn log_session_completion(
    ...
    tracing::info!(
        monotonic_counter.aibuddy.session_completions = 1,
        session_type,
        exit_type,
    ...
    tracing::info!(
        monotonic_counter.aibuddy.session_duration_ms = session_duration.as_millis() as u64,
        session_type,
        "Session duration"
    );
    ...
    if total_tokens > 0 {
        tracing::info!(
            monotonic_counter.aibuddy.session_tokens = total_tokens,
            session_type,
            "Session tokens"
        );
    }
    ...
    match (
        ...
        (Some(file), _, _) => {
            let contents = std::fs::read_to_string(file).unwrap_or_else(|err| {
                eprintln!(
                    "Instruction file not found — did you mean to use aibuddy run --text?\n{}",
                    err
                );
                std::process::exit(1);
            });
        ...
        (_, _, Some(recipe_name)) => {
            ...
            let recipe_version = crate::recipes::search_recipe::load_recipe_file(recipe_name)
                .ok()
                .and_then(|rf| {
                    aibuddy::recipe::template_recipe::parse_recipe_content(
                        &rf.content,
            ...
            tracing::info!(
                monotonic_counter.aibuddy.recipe_runs = 1,
...
#[cfg(feature = "local-inference")]
fn print_download_progress(manager: &aibuddy::download_manager::DownloadManager) {
    let Some(progress) = manager
        .list_progress()
        .into_iter()
        .find(|progress| progress.status == aibuddy::download_manager::DownloadStatus::Downloading)
    else {
        return;
    };
...
fn local_search_memory_limit(ram_gb: Option<f64>) -> Result<u64> {
    ...
    match aibuddy::providers::local_inference::InferenceRuntime::get_or_init() {
        Ok(runtime) => Ok(
            aibuddy::providers::local_inference::available_inference_memory_bytes(runtime.as_ref()),
        ),
...
fn recommended_variant(
    model: &aibuddy::providers::local_inference::hf_models::HfModelInfo,
    available_memory: u64,
) -> Option<&aibuddy::providers::local_inference::hf_models::HfModelVariant> {
    use aibuddy::providers::local_inference::hf_models::{recommend_variant, HfQuantVariant};
...
async fn handle_local_models_command(command: LocalModelsCommand) -> Result<()> {
    use aibuddy::providers::local_inference::hf_models;
    use aibuddy::providers::local_inference::local_model_registry::get_registry;

    aibuddy::providers::local_inference::configure_huggingface_auth();
    ...
    match command {
        LocalModelsCommand::Search {
            ...
            for model in &results {
                ...
                if let Some(variant) = recommended_variant(model, available_memory) {
                    ...
                    println!(
                        "    Download: aibuddy local-models download '{}'",
                        variant.download_id
                    );
                } else {
                    println!(
        ...
        LocalModelsCommand::Download { spec } => {
            ...
            let manager = aibuddy::download_manager::get_download_manager();
...
async fn handle_default_session() -> Result<()> {
    ...
    let aibuddy_mode = Config::global().get_aibuddy_mode().unwrap_or_default();
    let session_id = get_or_create_session_id(None, false, false, aibuddy_mode).await?;
...
pub async fn cli() -> anyhow::Result<()> {
    register_builtin_extensions(aibuddy_mcp::BUILTIN_EXTENSIONS.clone());

    let cli = Cli::parse();

    let command_name = get_command_name(&cli.command);
    tracing::info!(
        monotonic_counter.aibuddy.cli_commands = 1,
    ...
    match cli.command {
        ...
        Some(Command::Acp {
            builtins,
            enable_scheduler,
        }) => aibuddy::acp::server::run(builtins, enable_scheduler).await,
        #[cfg(feature = "roaming")]
        ...
        Some(Command::ValidateExtensions { file }) => {
            use aibuddy::agents::validate_extensions::validate_bundled_extensions;
...
mod tests {
    ...
    fn completion_command_accepts_nushell_alias() {
        let cli = Cli::try_parse_from(["aibuddy", "completion", "nushell"]).expect("parse failed");
    ...
    fn session_accepts_provider_override_without_resume() {
        let cli = Cli::try_parse_from(["aibuddy", "session", "--provider", "openai"])
    ...
    fn session_accepts_model_override_without_resume() {
        let cli = Cli::try_parse_from(["aibuddy", "session", "--model", "gpt-5.4"])
    ...
    fn session_accepts_system_prompt() {
        let cli = Cli::try_parse_from(["aibuddy", "session", "--system", "extra instructions"])
    ...
    fn nushell_completion_generation_emits_module() {
        ...
        CompletionShell::Nu.generate(&mut cmd, "aibuddy", &mut buffer);

        let script = String::from_utf8(buffer).expect("utf8");
        assert!(script.contains("module completions"));
        assert!(script.contains("export extern aibuddy"));
    ...
    fn term_init_help_mentions_nushell() {
        ...
        assert!(help.contains("aibuddy term init nu"));
    ...
    fn skills_command_accepts_list_subcommand() {
        let cli = Cli::try_parse_from(["aibuddy", "skills", "list"]).expect("parse failed");
