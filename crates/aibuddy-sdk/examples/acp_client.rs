//! ACP Client Example
//!
//! Spawns `aibuddy acp` as a child process and sends it a completion request
//! using the Agent Client Protocol over stdio.
//!
//! # Prerequisites
//!
//! You must have aibuddy built and a provider configured (`aibuddy configure`).
//!
//! # Usage
//!
//! ```bash
//! cargo run -p aibuddy-sdk --example acp_client -- "What is 2 + 2?"
//! ```
//!
//! Or with a custom aibuddy binary path:
//!
//! ```bash
//! cargo run -p aibuddy-sdk --example acp_client -- --aibuddy-bin ./target/debug/aibuddy "Explain Rust's ownership model in one sentence"
//! ```
...
use aibuddy_sdk::custom_requests::GetExtensionsRequest;
use std::path::PathBuf;
use tokio_util::compat::{TokioAsyncReadCompatExt, TokioAsyncWriteCompatExt};

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    // Parse args: [--aibuddy-bin PATH] PROMPT
    let args: Vec<String> = std::env::args().skip(1).collect();
    let (aibuddy_bin, prompt) = parse_args(&args)?;

    eprintln!("🚀 Spawning: {} acp", aibuddy_bin.display());

    let mut child = tokio::process::Command::new(&aibuddy_bin)
        .arg("acp")
        .stdin(std::process::Stdio::piped())
        .stdout(std::process::Stdio::piped())
        .stderr(std::process::Stdio::inherit())
        .spawn()
        .map_err(|e| format!("Failed to spawn '{}': {e}", aibuddy_bin.display()))?;
...
fn parse_args(args: &[String]) -> Result<(PathBuf, String), String> {
    let mut aibuddy_bin = PathBuf::from("aibuddy");
    let mut i = 0;
    ...
    while i < args.len() {
        match args[i].as_str() {
            "--aibuddy-bin" => {
                i += 1;
                aibuddy_bin = PathBuf::from(args.get(i).ok_or("--aibuddy-bin requires a value")?);
            }
            _ => break,
        }
        i += 1;
    ...
    if prompt.is_empty() {
        return Err("Usage: acp_client [--aibuddy-bin PATH] PROMPT".into());
    }

    Ok((aibuddy_bin, prompt))
}
