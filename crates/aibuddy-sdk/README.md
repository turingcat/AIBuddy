...
The bindings layer for aibuddy, published as the aibuddy Development Kit (GDK). It
houses the shared types used for both ACP and GDK access, and exposes a
cross-language version of the aibuddy API.

With `--features uniffi` the crate compiles to native bindings for Python and
Kotlin (namespace `aibuddy` / `io.github.aaif_aibuddy`). The UniFFI surface lets
...
`AIBuddyStreamError`), `durationMs`, and token `usage`. All three events share a
...
The PyPI package is published as `aibuddy-sdk` and imports as `aibuddy`.
...
into the package, and writes the wheel to `crates/aibuddy-sdk/python/dist/`.

## Maven package

The Maven Central artifact is published as `io.github.aaif-aibuddy:gdk` and uses
the Rust crate version from `crates/aibuddy-sdk/Cargo.toml`.
