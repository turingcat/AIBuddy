...
//! AIBuddy providers and mirrors the provider message/tool/streaming model closely
...
use aibuddy_providers::{
    anthropic::AnthropicProviderBuilder,
    api_client::{ApiClient, AuthMethod},
    base::{MessageStream, Provider as AIBuddyProvider},
    conversation::{
        message::{Message, MessageContent as AIBuddyMessageContent},
        token_usage::ProviderUsage,
    },
    databricks::DatabricksProvider as AIBuddyDatabricksProvider,
    databricks_auth::DatabricksAuth,
    databricks_v2::DatabricksV2Provider as AIBuddyDatabricksV2Provider,
...
impl From<anyhow::Error> for AIBuddyError {
    fn from(error: anyhow::Error) -> Self {
        Self::generic(error)
    }
}

impl From<aibuddy_providers::errors::ProviderError> for AIBuddyError {
    fn from(error: aibuddy_providers::errors::ProviderError) -> Self {
        match error {
            aibuddy_providers::errors::ProviderError::Authentication(message) => {
                Self::Authentication { details: message }
            }
            aibuddy_providers::errors::ProviderError::ContextLengthExceeded(message) => {
                Self::ContextLengthExceeded { details: message }
            }
            aibuddy_providers::errors::ProviderError::RateLimitExceeded { retry_delay, .. } => {
            ...
            aibuddy_providers::errors::ProviderError::ServerError(message)
            | aibuddy_providers::errors::ProviderError::EndpointNotFound(message)
            | aibuddy_providers::errors::ProviderError::CreditsExhausted {
                details: message, ..
            } => Self::ProviderUnavailable { details: message },
            aibuddy_providers::errors::ProviderError::NetworkError(message)
                if is_timeout(&message) =>
            {
                Self::Timeout { details: message }
            }
            aibuddy_providers::errors::ProviderError::RequestFailed(message)
                if is_timeout(&message) =>
            {
                Self::Timeout { details: message }
            }
            aibuddy_providers::errors::ProviderError::ExecutionError(message)
...
impl From<serde_json::Error> for AIBuddyError {
...
pub trait RequestLogger: Send + Sync {
    fn start(&self) -> Result<u64, AIBuddyError>;
    fn write(&self, request_id: u64, record: String) -> Result<(), AIBuddyError>;
}
...
impl aibuddy_providers::request_log::RequestLogger for RequestLoggerAdapter {
    fn start(
        &self,
    ) -> Result<
        Box<dyn aibuddy_providers::request_log::RequestLogHandle>,
...
impl aibuddy_providers::request_log::RequestLogHandle for RequestLogHandleAdapter {
...
#[uniffi::export]
pub fn install_request_logger(logger: Box<dyn RequestLogger>) -> Result<(), AIBuddyError> {
    aibuddy_providers::request_log::install_logger(RequestLoggerAdapter {
        logger: Arc::from(logger),
    })
    .map_err(|error| AIBuddyError::generic(error.to_string()))
}
...
impl ProviderMessage {
    fn to_aibuddy_message(&self) -> Result<Option<Message>, AIBuddyError> {
        ...
        for content in &self.content {
            message = message.with_content(content.to_aibuddy_content()?);
        }
        Ok(Some(message))
    }
}
...
impl MessageContent {
    fn to_aibuddy_content(&self) -> Result<AIBuddyMessageContent, AIBuddyError> {
        match self {
            MessageContent::Text { text } => {
                Ok(AIBuddyMessageContent::text(sanitize_unicode_tags(text)))
            }
            MessageContent::Image { mime_type, data } => Ok(AIBuddyMessageContent::image(
            ...
            MessageContent::ToolRequest {
                ...
                Ok(AIBuddyMessageContent::tool_request_with_metadata(
                    id.clone(),
                    tool_call,
                    metadata.as_ref(),
                ))
            ...
            MessageContent::ToolResult {
                ...
                Ok(AIBuddyMessageContent::tool_response(
                    id.clone(),
                    tool_result,
                ))
            }
            MessageContent::Thinking {
                thinking,
                signature,
            } => Ok(AIBuddyMessageContent::thinking(
                thinking.clone(),
                signature.clone(),
            )),
            MessageContent::RedactedThinking { data } => {
                Ok(AIBuddyMessageContent::redacted_thinking(data.clone()))
            }
        }
    }
    ...
    fn from_aibuddy_content(content: &AIBuddyMessageContent) -> Option<Self> {
        match content {
            AIBuddyMessageContent::Text(text) => Some(MessageContent::Text {
                text: text.text.clone(),
            }),
            AIBuddyMessageContent::Image(image) => Some(MessageContent::Image {
                mime_type: image.mime_type.clone(),
                data: base64::engine::general_purpose::STANDARD
                    .decode(&image.data)
                    .ok()?,
            }),
            AIBuddyMessageContent::ToolRequest(request) => {
            ...
            AIBuddyMessageContent::Thinking(thinking) => Some(MessageContent::Thinking {
                thinking: thinking.thinking.clone(),
                signature: thinking.signature.clone(),
            }),
            AIBuddyMessageContent::RedactedThinking(redacted) => {
...
impl ProviderTool {
    fn to_aibuddy_tool(&self) -> Result<Tool, AIBuddyError> {
...
fn parse_json_object(json: &str) -> Result<serde_json::Map<String, Value>, AIBuddyError> {
    match serde_json::from_str(json)? {
        Value::Object(object) => Ok(object),
        _ => Err(AIBuddyError::generic("expected a JSON object")),
    }
}
...
impl ProviderModelConfig {
    fn to_aibuddy_model_config(&self) -> Result<ModelConfig, AIBuddyError> {
...
impl Usage {
    fn from_provider_usage(usage: &ProviderUsage) -> Result<Self, AIBuddyError> {
        Ok(Self {
...
pub enum StreamChunk {
    ...
    ErrorChunk {
        error: AIBuddyStreamError,
    },
}

#[derive(Debug, Clone, uniffi::Record)]
pub struct AIBuddyStreamError {
    pub kind: AIBuddyStreamErrorKind,
    pub message: String,
    pub retry_after_ms: Option<u64>,
}

#[derive(Debug, Clone, uniffi::Enum)]
pub enum AIBuddyStreamErrorKind {
    RateLimited,
...
impl From<&AIBuddyError> for AIBuddyStreamError {
    fn from(error: &AIBuddyError) -> Self {
        match error {
            AIBuddyError::RateLimited {
                retry_after_ms,
                retry_after_suffix,
            } => Self {
                kind: AIBuddyStreamErrorKind::RateLimited,
                message: format!("Rate limit exceeded{retry_after_suffix}"),
                retry_after_ms: *retry_after_ms,
            },
            AIBuddyError::OutputTokenLimitExceeded { details } => Self {
                kind: AIBuddyStreamErrorKind::OutputTokenLimitExceeded,
                message: details.clone(),
                retry_after_ms: None,
            },
            AIBuddyError::ContextLengthExceeded { details } => Self {
                kind: AIBuddyStreamErrorKind::ContextLengthExceeded,
                message: details.clone(),
                retry_after_ms: None,
            },
            AIBuddyError::Authentication { details } => Self {
                kind: AIBuddyStreamErrorKind::Authentication,
                message: details.clone(),
                retry_after_ms: None,
            },
            AIBuddyError::Timeout { details } => Self {
                kind: AIBuddyStreamErrorKind::Timeout,
                message: details.clone(),
                retry_after_ms: None,
            },
            AIBuddyError::ProviderUnavailable { details } => Self {
                kind: AIBuddyStreamErrorKind::ProviderUnavailable,
                message: details.clone(),
                retry_after_ms: None,
            },
            AIBuddyError::Generic { details } => Self {
                kind: AIBuddyStreamErrorKind::Generic,
                message: details.clone(),
                retry_after_ms: None,
            },
        }
...
fn runtime() -> Result<&'static tokio::runtime::Runtime, AIBuddyError> {
    ...
    T: Send + 'static,
{
    ...
    (&mut task.handle).await.map_err(|error| {
        if error.is_cancelled() {
            AIBuddyError::Timeout {
                details: "runtime task was cancelled".to_string(),
            }
        } else {
            AIBuddyError::generic(error)
        }
    })
}

struct ProviderHandle {
    provider: Arc<dyn AIBuddyProvider>,
}

impl ProviderHandle {
    fn new(provider: Box<dyn AIBuddyProvider>) -> Self {
        Self {
    ...
    async fn context_limit(&self, model: ProviderModelConfig) -> Result<usize, AIBuddyError> {
    ...
    async fn stream(
    ...
    ) -> Result<Arc<ProviderStream>, AIBuddyError> {
        let timeout_ms = model.timeout_ms;
        let model = model.to_aibuddy_model_config()?;
        ...
        {
            Ok(Ok(stream)) => stream,
            Ok(Err(error)) => return Err(observer.fail(AIBuddyError::from(error))),
    ...
    async fn complete(
        ...
        messages: Vec<ProviderMessage>,
        tools: Vec<ProviderTool>,
    ) -> Result<ProviderCompletion, AIBuddyError> {
        let timeout_ms = model.timeout_ms;
        let model = model.to_aibuddy_model_config()?;
        let messages = convert_messages(messages)?;
        let tools = convert_tools(tools)?;
        ...
        let completion = ProviderCompletion {
            ...
            content: message
                .content
                .iter()
                .filter_map(MessageContent::from_aibuddy_content)
                .collect(),
    ...
    run_on_runtime(async move {
        if let Some(timeout_ms) = timeout_ms {
            tokio::time::timeout(Duration::from_millis(timeout_ms), future)
                .await
                .map_err(|_| AIBuddyError::Timeout {
...
fn convert_messages(messages: Vec<ProviderMessage>) -> Result<Vec<Message>, AIBuddyError> {
    messages
        .iter()
        .filter_map(|message| message.to_aibuddy_message().transpose())
        .collect()
}

fn convert_tools(tools: Vec<ProviderTool>) -> Result<Vec<Tool>, AIBuddyError> {
    tools.iter().map(ProviderTool::to_aibuddy_tool).collect()
}
...
impl Provider {
    fn new(provider: Box<dyn AIBuddyProvider>) -> Arc<Self> {
        Arc::new(Self {
...
impl Provider {
    ...
    pub async fn context_limit(&self, model: ProviderModelConfig) -> Result<u64, AIBuddyError> {
    ...
    ) -> Result<CompactionSummary, AIBuddyError> {
        let messages: Vec<Message> = messages
            .iter()
            .map(CompactionMessage::to_aibuddy_message)
            .collect();
        let templates = templates.map(Into::into).unwrap_or_default();
        let model = aibuddy_context_management::ProviderModel::new(
            self.handle.provider.clone(),
            ModelConfig::new(&model_name),
        );

        let summary = run_on_runtime(async move {
            aibuddy_context_management::summarize(&model, None, &templates, &messages).await
        })
        .await?
        .map_err(AIBuddyError::generic)?;
...
impl CompactionMessage {
    fn to_aibuddy_message(&self) -> Message {
        let role = match self.role {
            MessageRole::User | MessageRole::Tool => Role::User,
            MessageRole::Assistant => Role::Assistant,
        };
...
impl From<CompactionTemplates> for aibuddy_context_management::Templates {
...
pub fn default_compaction_templates() -> CompactionTemplates {
    let templates = aibuddy_context_management::Templates::default();
    CompactionTemplates {
...
#[uniffi::export]
pub fn declarative_provider_from_json(json: String) -> Result<Arc<Provider>, AIBuddyError> {
    let provider = aibuddy_providers::declarative::from_json(&json, None, EnvKeyResolver {})?;
    Ok(Provider::new(provider))
}

#[uniffi::export]
pub fn openai_default_model() -> String {
    aibuddy_providers::openai::OPEN_AI_DEFAULT_MODEL.to_string()
}

#[uniffi::export]
pub fn openai_provider(api_key: String) -> Result<Arc<Provider>, AIBuddyError> {
...
pub fn anthropic_default_model() -> String {
    aibuddy_providers::anthropic::ANTHROPIC_DEFAULT_MODEL.to_string()
}
...
pub fn anthropic_provider(
    ...
    .with_header(
        "anthropic-version",
        aibuddy_providers::anthropic::ANTHROPIC_API_VERSION,
    )?;
...
#[uniffi::export]
pub fn groq_provider(api_key: String) -> Result<Arc<Provider>, AIBuddyError> {
    let json = aibuddy_providers::groq::JSON.replace("${GROQ_API_KEY}", &api_key);
    let provider = aibuddy_providers::declarative::from_json(
        &json,
        None,
        StaticKeyResolver {
...
fn groq_config() -> DeclarativeProviderConfig {
    aibuddy_providers::declarative::deserialize_provider_config(aibuddy_providers::groq::JSON)
...
impl aibuddy_providers::declarative::KeyResolver for StaticKeyResolver {
...
pub fn databricks_default_model() -> String {
    aibuddy_providers::databricks::DATABRICKS_DEFAULT_MODEL.to_string()
}

#[uniffi::export]
pub fn databricks_provider(host: String, token: String) -> Result<Arc<Provider>, AIBuddyError> {
    let retry_config = AIBuddyDatabricksProvider::load_retry_config(|key| std::env::var(key).ok());
    let provider = AIBuddyDatabricksProvider::new(
        host,
...
#[uniffi::export]
impl ProviderStream {
    pub async fn next_chunk(&self) -> Result<Option<StreamChunk>, AIBuddyError> {
            ...
            loop {
                ...
                let next = if let Some(timeout_ms) = timeout_ms {
                    ...
                    .await
                    {
                        Ok(next) => next,
                        Err(_) => {
                            return Err(observer.fail(AIBuddyError::Timeout {
                                details: format!("request timed out after {timeout_ms}ms"),
                            }))
                        }
                    }
                ...
                match next {
                    ...
                    Some(Err(error)) => {
                        state.ended = true;
                        let error = AIBuddyStreamError::from(&AIBuddyError::from(error));
...
fn message_to_chunks(message: Message) -> Vec<StreamChunk> {
    message
        ...
        .filter_map(|content| match content {
            AIBuddyMessageContent::Text(text) if !text.text.is_empty() => {
            ...
            AIBuddyMessageContent::Thinking(thinking) => Some(StreamChunk::ThinkingChunk {
...
mod tests {
    ...
    fn model_config_rejects_invalid_request_params_json() {
        ...
        assert!(config.to_aibuddy_model_config().is_err());
    }
    ...
    fn provider_message_converts_document_to_base64_content() {
        ...
        let AIBuddyMessageContent::Document(document) = &message.content[0] else {
    ...
    fn provider_message_converts_document_serializes_for_anthropic() {
        ...
        let spec = aibuddy_providers::formats::anthropic::format_messages(&[message]);
    ...
    fn usage_exposes_provider_additional_data() {
        let mut provider_usage = ProviderUsage::new(
            ...
            aibuddy_providers::conversation::token_usage::Usage::new(Some(10), Some(5), None),
        );
    ...
    fn tool_result_content_converts() {
        ...
        let AIBuddyMessageContent::ToolResponse(response) = content else {
    ...
    fn provider_usage_with_cache(
        ...
        use aibuddy_providers::conversation::token_usage::Usage as AIBuddyUsage;

        ProviderUsage::new(
            "test-model".to_string(),
            AIBuddyUsage::new(Some(100), Some(20), Some(120))
    ...
    fn thinking_content_round_trips_with_signature() {
        ...
        let aibuddy = original.to_aibuddy_content().unwrap();
        let AIBuddyMessageContent::Thinking(thinking) = &aibuddy else {
            panic!("expected thinking content");
        };
        assert_eq!(thinking.thinking, "step one, then step two");
        assert_eq!(thinking.signature, "ErUBCkYIBRgCIkAe0pAQ==");

        let round_tripped = MessageContent::from_aibuddy_content(&aibuddy).unwrap();
    ...
    fn redacted_thinking_content_round_trips_opaque_data() {
        ...
        let aibuddy = original.to_aibuddy_content().unwrap();
        let AIBuddyMessageContent::RedactedThinking(redacted) = &aibuddy else {
    ...
    fn tool_request_round_trips_provider_metadata() {
        ...
        let aibuddy = original.to_aibuddy_content().unwrap();
        let AIBuddyMessageContent::ToolRequest(request) = &aibuddy else {
        ...
        .unwrap();
        let spec = aibuddy_providers::formats::openai::format_messages(
            &messages,
            &aibuddy_providers::images::ImageFormat::OpenAi,
        );
        assert_eq!(
    ...
    fn completion_content_preserves_malformed_tool_requests() {
        ...
        .unwrap();
        let AIBuddyMessageContent::ToolRequest(request) = &replayed[0].content[0] else {
            panic!("expected tool request");
        };
    ...
    fn streaming_tool_chunks_carry_provider_metadata() {
        let mut metadata = aibuddy_providers::conversation::message::ProviderMetadata::new();
        metadata.insert(
    ...
    fn thinking_blocks_survive_multi_turn_replay() {
        ...
        assert!(matches!(
            ...
            AIBuddyMessageContent::Thinking(_)
        ));
        assert!(matches!(
            messages[1].content[1],
            AIBuddyMessageContent::RedactedThinking(_)
        ));

        let spec = aibuddy_providers::formats::anthropic::format_messages(&messages);
