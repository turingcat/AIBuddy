...
use crate::bindings::{AIBuddyError, AIBuddyStreamError, Usage};
...
pub enum RequestOutcome {
    Success,
    Failure { error: AIBuddyStreamError },
}
...
impl RequestObserver {
    ...
    pub(crate) fn fail(&self, error: AIBuddyError) -> AIBuddyError {
        self.end(
            RequestOutcome::Failure {
                error: AIBuddyStreamError::from(&error),
            },
            None,
            None,
        );
    ...
    pub(crate) fn fail_stream(&self, error: AIBuddyStreamError) {
...
mod tests {
    ...
    fn failure_reports_typed_error() {
        let registered = RegisteredRecorder::new(Recorder::default(), false);
        let observer = RequestObserver::start(descriptor());
        observer.fail(AIBuddyError::RateLimited {
        ...
        assert!(matches!(
            error.kind,
            crate::bindings::AIBuddyStreamErrorKind::RateLimited
        ));
    ...
    fn end_is_emitted_at_most_once() {
        let registered = RegisteredRecorder::new(Recorder::default(), false);
        let observer = RequestObserver::start(descriptor());
        observer.fail(AIBuddyError::Timeout {
            details: "request timed out after 5ms".to_string(),
        });
        observer.succeeded(Some(usage()), None);
        observer.fail_stream(AIBuddyStreamError {
            kind: crate::bindings::AIBuddyStreamErrorKind::Generic,
        ...
        assert!(matches!(
            error.kind,
            crate::bindings::AIBuddyStreamErrorKind::Timeout
        ));
    }
