...
native_lib="$1"
resource_prefix="${2:-$(crates/aibuddy-sdk/scripts/maven-resource-prefix.sh)}"
...
bindgen="target/release/aibuddy-uniffi-bindgen"
if [ ! -x "$bindgen" ]; then
  cargo build -p aibuddy-sdk --features uniffi --release -q
fi

maven_dir="crates/aibuddy-sdk/maven"
...
rm -rf "$kotlin_dir/io/github/aaif_aibuddy" "$resources_dir/$resource_prefix"
mkdir -p "$kotlin_dir" "$resources_dir/$resource_prefix" "$resources_dir/META-INF"
cp LICENSE "$resources_dir/META-INF/LICENSE"

"$bindgen" generate \
  --library "$native_lib" \
  --config crates/aibuddy-sdk/uniffi.toml \
  --language kotlin \
  --no-format \
  --out-dir "$kotlin_dir" 2>/dev/null

python3 - "$kotlin_dir/io/github/aaif_aibuddy/aibuddy.kt" <<'PY'
import sys
