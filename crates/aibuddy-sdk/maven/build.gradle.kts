...
group = "io.github.aaif-aibuddy"
version = aibuddySdkVersion()

fun aibuddySdkVersion(): String {
    val cargoToml = file("../Cargo.toml").readText()
    return Regex("(?m)^version\\s*=\\s*\"([^\"]+)\"")
        .find(cargoToml)
        ?.groupValues
        ?.get(1)
        ?: error("Could not find aibuddy-sdk version in ../Cargo.toml")
}
...
tasks.jar {
    manifest {
        attributes(
            "Implementation-Title" to "AIBuddy GDK",
...
mavenPublishing {
    ...
    coordinates(
        groupId = "io.github.aaif-aibuddy",
        artifactId = "gdk",
        version = project.version.toString(),
    )

    pom {
        name.set("AIBuddy GDK")
        description.set("Kotlin/JVM bindings for the aibuddy Development Kit (GDK)")
