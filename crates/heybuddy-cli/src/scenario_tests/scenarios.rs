...
//! You can set the HEYBUDDY_TEST_PROVIDER to just run a specific provider.
    ...
    async fn test_what_is_your_name() -> Result<()> {
        run_scenario(
            ...
            |result| {
                ...
                assert!(
                    result.last_message()?.to_lowercase().contains("heybuddy"),
                    "Response should contain 'heybuddy': {}",
                    result.last_message()?
                );
                Ok(())
            },
