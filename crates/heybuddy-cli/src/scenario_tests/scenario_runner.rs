...
use anyhow::Result;
use heybuddy::agents::{Agent, AgentConfig, HeyBuddyPlatform};
use heybuddy::config::permission::PermissionManager;
use heybuddy::config::HeyBuddyMode;
use heybuddy::providers::{create, testprovider::TestProvider};
...
{
    if let Ok(only_provider) = std::env::var("HEYBUDDY_TEST_PROVIDER") {
    ...
    F: Fn(&ScenarioResult) -> Result<()>,
{
    ...
    heybuddy::agents::moim::SKIP.with(|f| f.set(true));
    ...
    let scenario_model_config =
        heybuddy::model_config::model_config_from_user_config(&factory_name, config.model_name)?;
    agent
        .update_provider(
            provider_arc as Arc<dyn heybuddy::providers::base::Provider>,
