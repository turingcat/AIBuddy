...
use console::style;
use heybuddy::recipe::template_recipe::parse_recipe_content;
use heybuddy::recipe::RECIPE_FILE_EXTENSIONS;
use serde::{Deserialize, Serialize};

use heybuddy::recipe::read_recipe_file_content::RecipeFile;
use heybuddy::subprocess::{git_command, SubprocessExt};
use std::env;
use std::fs;
...
pub const HEYBUDDY_RECIPE_GITHUB_REPO_CONFIG_KEY: &str = "HEYBUDDY_RECIPE_GITHUB_REPO";
...
fn discover_github_recipes(repo: &str) -> Result<Vec<RecipeInfo>> {
    ...
    let output = Command::new("gh")
        ...
        .set_no_window()
        .output()
        .map_err(|e| anyhow!("Failed to fetch repository contents using 'gh api' command (executed when HEYBUDDY_RECIPE_GITHUB_REPO is configured). This requires GitHub CLI (gh) to be installed and authenticated. Error: {}", e))?;
...
mod tests {
    ...
    fn local_repo_path_includes_owner_to_avoid_collisions() {
        let parent = Path::new("heybuddy-recipes");
