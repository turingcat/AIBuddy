...
use heybuddy::recipe::{Recipe, SubRecipe};
...
mod tests {
    ...
    fn test_extract_recipe_info_from_cli_basic() {
        ...
        assert_eq!(
            settings.heybuddy_provider,
            Some("test_provider".to_string())
        );
        assert_eq!(settings.heybuddy_model, Some("test_model".to_string()));
...
settings:
  heybuddy_provider: test_provider
