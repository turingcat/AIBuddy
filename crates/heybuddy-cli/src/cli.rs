...
use heybuddy::agents::HeyBuddyPlatform;
use heybuddy::builtin_extension::register_builtin_extensions;
use heybuddy::config::legacy::migrate_config_content;
use heybuddy::config::{Config, HeyBuddyMode};
use heybuddy::recipe::Recipe;
use heybuddy::source_roots::SourceRoot;
use heybuddy_mcp::mcp_server_runner::{serve, McpCommand};
use heybuddy_mcp::{AutoVisualiserRouter, ComputerControllerServer, MemoryServer, TutorialServer};
...
use heybuddy::agents::Container;
use heybuddy::session::session_manager::SessionType;
use heybuddy::session::SessionManager;
use std::io::{Read, Write};
use std::path::{Path, PathBuf};
const HEYBUDDY_SERVER_SECRET_KEY_ENV: &str = "HEYBUDDY_SERVER__SECRET_KEY";
...
impl From<ServePlatform> for HeyBuddyPlatform {
    fn from(platform: ServePlatform) -> Self {
        match platform {
            ServePlatform::Cli => HeyBuddyPlatform::HeyBuddyCli,
            ServePlatform::Desktop => HeyBuddyPlatform::HeyBuddyDesktop,
        }
    }
}

#[derive(Parser)]
#[command(name = "heybuddy", author, version, display_name = "", about, long_about = None)]
pub struct Cli {
    #[command(subcommand)]
...
pub struct SessionOptions {
    ...
    #[arg(
        ...
        long_help = "Run extensions (stdio and built-in) inside the specified container. The extension must exist in the container. For built-in extensions, heybuddy must be installed inside the container."
    )]
...
fn parse_streamable_http_extension(input: &str) -> Result<StreamableHttpOptions, String> {
    ...
    let (mut url, mut timeout) = (String::new(), heybuddy::config::DEFAULT_EXTENSION_TIMEOUT);
...
pub struct ExtensionOptions {
    ...
    #[arg(
        ...
        long_help = "Add one or more builtin extensions that are bundled with heybuddy by specifying their names, comma-separated",
        value_delimiter = ','
    )]
...
pub struct InputOptions {
    ...
    #[arg(
        ...
        long = "text",
        value_name = "TEXT",
        help = "Input text to provide to heybuddy directly",
        long_help = "Input text containing commands for heybuddy. Use this in lieu of the instructions argument.",
    ...
    #[arg(
        ...
        help = "Dynamic parameters (e.g., --params username=alice --params channel_name=heybuddy-channel)",
...
pub struct ModelOptions {
    ...
    #[arg(
        ...
        long_help = "Override the HEYBUDDY_PROVIDER environment variable for this run. Available providers include openai, anthropic, ollama, databricks, gemini-cli, claude-code, and others."
    )]
    ...
    #[arg(
        ...
        long_help = "Override the HEYBUDDY_MODEL environment variable for this run. The model must be supported by the specified provider."
    )]
...
enum SessionCommand {
    ...
    Export {
        ...
        #[arg(
            long = "nostr",
            help = "Publish the JSON session export as an encrypted Nostr event and print a HeyBuddy share link"
        )]
        nostr: bool,
    ...
    Import {
        #[arg(
            help = "Path to a heybuddy session export, a Claude Code, Codex, or Pi .jsonl transcript, or a heybuddy://sessions/nostr share link"
        )]
        input: String,
...
#[derive(Subcommand)]
enum SkillsCommand {
    /// List all skills available to the heybuddy agent
    #[command(about = "List all skills available to the heybuddy agent")]
    List,
}
...
enum RecipeCommand {
    ...
    #[command(about = "Open a recipe in HeyBuddy Desktop")]
    Open {
...
#[derive(Subcommand)]
enum Command {
    /// Migrate a legacy YAML config to canonical HeyBuddy keys
    #[command(about = "Migrate legacy config keys without changing the source file")]
    MigrateConfig(MigrateConfigArgs),

    /// Configure heybuddy settings
    #[command(about = "Configure heybuddy settings")]
    Configure {},

    /// Display heybuddy configuration information
    #[command(about = "Display heybuddy information")]
    Info {
    ...
    #[command(about = "Check that your HeyBuddy setup is working")]
    Doctor {},

    /// Manage system prompts and behaviors
    #[command(about = "Run one of the mcp servers bundled with heybuddy")]
    Mcp {
        #[arg(value_parser = clap::value_parser!(McpCommand))]
        server: McpCommand,
    },

    /// Run heybuddy as an ACP (Agent Client Protocol) agent
    #[command(about = "Run heybuddy as an ACP agent server on stdio")]
    Acp {
    ...
    Serve {
        ...
        #[arg(
            ...
            help = "Start the ACP endpoint without requiring HEYBUDDY_SERVER__SECRET_KEY"
        )]
        ...
        /// Also expose this server over heybuddy roam (p2p) so paired devices can connect remotely
        #[cfg(feature = "roaming")]
        #[arg(
            long,
            help = "Also expose this server over heybuddy roam (p2p) so paired devices can connect remotely"
        )]
        roam: bool,
    },
    ...
    /// Update the heybuddy CLI version
    #[cfg(feature = "update")]
    #[command(about = "Update the heybuddy CLI version")]
    Update {
        ...
        #[arg(
            ...
            long_help = "Update to the latest canary version of the heybuddy CLI, otherwise updates to the latest stable version."
        )]
        canary: bool,

        /// Enforce to re-configure heybuddy during update
        #[arg(short, long, help = "Enforce to re-configure heybuddy during update")]
        reconfigure: bool,
    },

    /// Terminal-integrated session (one session per terminal)
    #[command(
        about = "Terminal-integrated heybuddy session",
        long_about = "Runs a heybuddy session tied to your terminal window.\n\
                      Each terminal maintains its own persistent session that resumes automatically.\n\n\
                      Setup:\n  \
                        eval \"$(heybuddy term init zsh)\"  # zsh/bash\n  \
                        let init = ($nu.cache-dir | path join \"heybuddy-term-init.nu\"); ^heybuddy term init nu | save --force $init; source $init\n\n\
                      Usage:\n  \
                        heybuddy term run \"list files in this directory\"\n  \
                        @heybuddy \"create a python script\"  # using alias\n  \
    ...
    /// runs the review through heybuddy.
    #[command(about = "Review the current diff using heybuddy")]
    Review {
        ...
        /// orchestrator dispatches one `heybuddy run` subprocess per check
    ...
    #[command(
        name = "mcp-probe",
        about = "Start a HeyBuddy MCP session without an LLM and inspect a stdio MCP server",
        hide = true
    )]
    McpProbe {
...
struct MigrateConfigArgs {
    /// Existing legacy config file to read
    #[arg(long, value_name = "FILE")]
    source: PathBuf,

    /// New canonical config file to create
    #[arg(long, value_name = "FILE")]
    destination: PathBuf,

    /// Create the backup and destination; otherwise only report key names
    #[arg(long)]
    apply: bool,
}
...
enum TermCommand {
    ...
    #[command(
        ...
        long_about = "Prints shell configuration to set up terminal-integrated sessions.\n\
                      Each terminal gets a persistent heybuddy session that automatically resumes.\n\n\
                      Setup:\n  \
                        echo 'eval \"$(heybuddy term init zsh)\"' >> ~/.zshrc\n  \
                        source ~/.zshrc\n\n\
                        Nushell:\n  \
                        let init = ($nu.cache-dir | path join \"heybuddy-term-init.nu\")\n  \
                        ^heybuddy term init nu | save --force $init\n  \
                        source $init\n\n\
                      With --default (anything typed that isn't a command goes to heybuddy):\n  \
                        echo 'eval \"$(heybuddy term init zsh --default)\"' >> ~/.zshrc\n  \
                        ^heybuddy term init nu --default | save --force $init"
    )]
    Init {
        ...
        /// Make heybuddy the default handler for unknown commands
        #[arg(
            long = "default",
            help = "Make heybuddy the default handler for unknown commands",
            long_help = "When enabled, anything you type that isn't a valid command will be sent to heybuddy. Supported for zsh, bash, and nu."
        )]
        default: bool,
    },
    ...
    #[command(
        ...
        long_about = "Run a prompt in the terminal-integrated session.\n\n\
                      Examples:\n  \
                        heybuddy term run list files in this directory\n  \
                        @heybuddy list files  # using alias\n  \
                        @g why did that fail  # short alias"
    )]
    Run {
        /// The prompt to send to heybuddy (multiple words allowed without quotes)
...
fn get_command_name(command: &Option<Command>) -> &'static str {
    match command {
        Some(Command::MigrateConfig(_)) => "migrate-config",
...
struct McpProbeScript {
    ...
    #[serde(default)]
    oauth: heybuddy::oauth::OAuthFlowConfig,
...
async fn handle_mcp_probe(extension_command: String, script_path: Option<String>) -> Result<()> {
    use heybuddy::agents::{Agent, AgentConfig, ToolCallContext};
    use heybuddy::config::ExtensionConfig;
    ...
    let script = if let Some(path) = script_path {
        ...
        McpProbeScript {
            ...
            elicitation: None,
            oauth: heybuddy::oauth::OAuthFlowConfig::default(),
        ...
        .is_ok_and(|url| matches!(url.scheme(), "http" | "https"))
    {
        crate::session::CliSession::parse_streamable_http_extension(
            &extension_command,
            heybuddy::config::DEFAULT_EXTENSION_TIMEOUT,
        )
    } else {
    ...
    if let Some(client_id) = &script.oauth.client_id {
        std::env::set_var("HEYBUDDY_MCP_OAUTH_CLIENT_ID", client_id);
    }
    if let Some(client_secret) = &script.oauth.client_secret {
        std::env::set_var("HEYBUDDY_MCP_OAUTH_CLIENT_SECRET", client_secret);
    }
    ...
    let config = heybuddy::config::Config::global();
    let mut agent_config = AgentConfig::new(
        std::sync::Arc::new(SessionManager::instance()),
        heybuddy::config::permission::PermissionManager::instance(),
        None,
        config.get_heybuddy_mode().unwrap_or_default(),
        true,
        HeyBuddyPlatform::HeyBuddyCli,
    );
    ...
    let session = agent
        ...
        .create_session(
            std::env::current_dir()?,
            "MCP Probe".to_string(),
            heybuddy::session::session_manager::SessionType::Hidden,
            agent.config.heybuddy_mode,
        )
        .await?;
...
#[cfg(feature = "roaming")]
type RoamShareSlot =
    std::sync::Arc<tokio::sync::RwLock<Option<std::sync::Arc<heybuddy_roaming::RoamingNode>>>>;

#[cfg(feature = "roaming")]
fn spawn_roam_share(
    server: std::sync::Arc<heybuddy::acp::server_factory::AcpServer>,
) -> RoamShareSlot {
            ...
            match try_acquire_roam_lock_owner() {
                ...
                Ok(None) => {
                    if !standing_by {
                        standing_by = true;
                        eprintln!(
                            "another heybuddy process owns the roaming endpoint; standing by to take over if it exits"
                        );
                    }
                }
...
async fn start_roam_share(
    server: std::sync::Arc<heybuddy::acp::server_factory::AcpServer>,
) -> Result<std::sync::Arc<heybuddy_roaming::RoamingNode>> {
    ...
    use heybuddy::config::paths::Paths;
    use heybuddy_roaming::{RoamingConfig, RoamingNode, TrustBook};
    use std::sync::Arc;
    ...
    let node = RoamingNode::bind(RoamingConfig {
        ...
        directory: heybuddy_roaming::Directory::persistent_owned(directory_path()),
        bind_addr: None,
        relay_tls: None,
    })
    .await?;
    ...
    // Roaming sessions run where `heybuddy serve` was started: the connector's
...
async fn handle_serve_command(args: ServeCommandArgs) -> Result<()> {
    ...
    use heybuddy::acp::server::AcpBuiltinSelection;
    use heybuddy::acp::server_factory::{AcpServer, AcpServerFactoryConfig};
    use heybuddy::acp::transport::create_router;
    use heybuddy::config::paths::Paths;
    ...
    let server = Arc::new(AcpServer::new(AcpServerFactoryConfig {
        ...
        heybuddy_platform: platform.into(),
        additional_source_roots,
        session_cwd: None,
        enable_scheduler,
    }));
    let env_secret = std::env::var(HEYBUDDY_SERVER_SECRET_KEY_ENV)
        .ok()
    ...
    if !require_token && !dangerously_unauthenticated {
        anyhow::bail!(
            "{HEYBUDDY_SERVER_SECRET_KEY_ENV} must be set to start `heybuddy serve`; pass --dangerously-unauthenticated to run without ACP authentication"
        );
    }
    if dangerously_unauthenticated && !require_token {
        warn!(
            "{HEYBUDDY_SERVER_SECRET_KEY_ENV} is not set and --dangerously-unauthenticated was passed; the ACP endpoint will accept unauthenticated connections"
        );
    }
    ...
    let tls_cert_path =
        tls_cert_path.or_else(|| config.get_param::<String>("HEYBUDDY_TLS_CERT_PATH").ok());
    let tls_key_path =
        tls_key_path.or_else(|| config.get_param::<String>("HEYBUDDY_TLS_KEY_PATH").ok());
    let tls = tls
        || config.get_param::<bool>("HEYBUDDY_TLS").unwrap_or(false)
    ...
    if tls {
        ...
        {
            let tls_setup = heybuddy::acp::transport::tls::setup_tls(
...
async fn handle_interactive_session(args: InteractiveSessionArgs) -> Result<()> {
    ...
    tracing::info!(
        monotonic_counter.heybuddy.session_starts = 1,
        session_type,
        interactive = true,
        "Session started"
    );
    ...
    let heybuddy_mode = Config::global().get_heybuddy_mode().unwrap_or_default();
    let mut session_id = get_or_create_session_id(identifier, resume, false, heybuddy_mode).await?;
...
async fn log_session_completion(
    ...
    tracing::info!(
        monotonic_counter.heybuddy.session_completions = 1,
        session_type,
        exit_type,
    ...
    tracing::info!(
        monotonic_counter.heybuddy.session_duration_ms = session_duration.as_millis() as u64,
        session_type,
        "Session duration"
    );
    ...
    if total_tokens > 0 {
        tracing::info!(
            monotonic_counter.heybuddy.session_tokens = total_tokens,
            session_type,
            "Session tokens"
        );
    }
    ...
    match (
        ...
        (Some(file), _, _) => {
            let contents = std::fs::read_to_string(file).unwrap_or_else(|err| {
                eprintln!(
                    "Instruction file not found — did you mean to use heybuddy run --text?\n{}",
                    err
                );
                std::process::exit(1);
            });
        ...
        (_, _, Some(recipe_name)) => {
            ...
            let recipe_version = crate::recipes::search_recipe::load_recipe_file(recipe_name)
                .ok()
                .and_then(|rf| {
                    heybuddy::recipe::template_recipe::parse_recipe_content(
                        &rf.content,
            ...
            tracing::info!(
                monotonic_counter.heybuddy.recipe_runs = 1,
...
#[cfg(feature = "local-inference")]
fn print_download_progress(manager: &heybuddy::download_manager::DownloadManager) {
    let Some(progress) = manager.list_progress().into_iter().find(|progress| {
        progress.status == heybuddy::download_manager::DownloadStatus::Downloading
    }) else {
        return;
    };
...
fn local_search_memory_limit(ram_gb: Option<f64>) -> Result<u64> {
    ...
    match heybuddy::providers::local_inference::InferenceRuntime::get_or_init() {
        Ok(runtime) => Ok(
            heybuddy::providers::local_inference::available_inference_memory_bytes(
                runtime.as_ref(),
            ),
        ),
...
fn recommended_variant(
    model: &heybuddy::providers::local_inference::hf_models::HfModelInfo,
    available_memory: u64,
) -> Option<&heybuddy::providers::local_inference::hf_models::HfModelVariant> {
    use heybuddy::providers::local_inference::hf_models::{recommend_variant, HfQuantVariant};
...
async fn handle_local_models_command(command: LocalModelsCommand) -> Result<()> {
    use heybuddy::providers::local_inference::hf_models;
    use heybuddy::providers::local_inference::local_model_registry::get_registry;

    heybuddy::providers::local_inference::configure_huggingface_auth();
    ...
    match command {
        LocalModelsCommand::Search {
            ...
            for model in &results {
                ...
                if let Some(variant) = recommended_variant(model, available_memory) {
                    ...
                    println!(
                        "    Download: heybuddy local-models download '{}'",
                        variant.download_id
                    );
                } else {
                    println!(
        ...
        LocalModelsCommand::Download { spec } => {
            ...
            let manager = heybuddy::download_manager::get_download_manager();
...
async fn handle_default_session() -> Result<()> {
    ...
    let heybuddy_mode = Config::global().get_heybuddy_mode().unwrap_or_default();
    let session_id = get_or_create_session_id(None, false, false, heybuddy_mode).await?;
...
fn migration_backup_path(source: &Path) -> PathBuf {
    PathBuf::from(format!("{}.bak", source.display()))
}

fn require_regular_file(path: &Path, label: &str) -> Result<()> {
    let metadata = std::fs::symlink_metadata(path)
        .map_err(|e| anyhow::anyhow!("failed to inspect {label} {:?}: {e}", path))?;
    if metadata.file_type().is_symlink() {
        anyhow::bail!("{label} must not be a symlink: {:?}", path);
    }
    if !metadata.is_file() {
        anyhow::bail!("{label} must be a regular file: {:?}", path);
    }
    Ok(())
}

fn require_new_path(path: &Path, label: &str) -> Result<()> {
    match std::fs::symlink_metadata(path) {
        Ok(metadata) if metadata.file_type().is_symlink() => {
            anyhow::bail!("{label} must not be a symlink: {:?}", path)
        }
        Ok(_) => anyhow::bail!("{label} already exists: {:?}", path),
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => Ok(()),
        Err(error) => Err(error.into()),
    }
}

fn write_new_file_atomically(path: &Path, content: &[u8]) -> Result<()> {
    let parent = path
        .parent()
    ...
    if std::fs::symlink_metadata(parent)
        ...
        .unwrap_or(false)
    {
        anyhow::bail!("destination directory must not be a symlink: {:?}", parent);
    }

    let mut temp = tempfile::Builder::new()
        .prefix(".heybuddy-migrate-")
        .tempfile_in(parent)?;
    ...
    temp.persist_noclobber(path)
        .map(|_| ())
        .map_err(|error| anyhow::anyhow!("failed to publish {:?}: {}", path, error.error))
}
...
fn migrate_config_file(
    ...
    let source_content = std::str::from_utf8(&source_bytes)
        .map_err(|e| anyhow::anyhow!("source config is not valid UTF-8: {e}"))?;
    let (migrated_content, changes) = migrate_config_content(source_content)
        .map_err(|_| anyhow::anyhow!("failed to parse source config YAML"))?;

    if apply {
        require_new_path(destination, "destination")?;
        let backup = migration_backup_path(source);
        require_new_path(&backup, "backup")?;
        if destination == backup {
            anyhow::bail!("destination must differ from backup: {:?}", destination);
        }
        write_new_file_atomically(&backup, &source_bytes)?;
        write_new_file_atomically(destination, migrated_content.as_bytes())?;
    }

    Ok(changes)
}

fn handle_migrate_config(args: MigrateConfigArgs) -> Result<()> {
    let changes = migrate_config_file(&args.source, &args.destination, args.apply)?;
    if changes.is_empty() {
        println!("No legacy config keys found.");
    } else {
        ...
        println!("{action} {} config key(s):", changes.len());
        for (legacy, canonical) in changes {
            println!("- {legacy} -> {canonical}");
        }
    }
    if args.apply {
        println!("Backup: {:?}", migration_backup_path(&args.source));
        println!("Destination: {:?}", args.destination);
    } else {
        println!("Dry run only; use --apply to create the backup and destination.");
    }
    Ok(())
}

pub async fn cli() -> anyhow::Result<()> {
    register_builtin_extensions(heybuddy_mcp::BUILTIN_EXTENSIONS.clone());

    let cli = Cli::parse();

    let command_name = get_command_name(&cli.command);
    tracing::info!(
        monotonic_counter.heybuddy.cli_commands = 1,
        command = command_name,
        "CLI command executed"
    );

    match cli.command {
        Some(Command::MigrateConfig(args)) => handle_migrate_config(args),
        ...
        Some(Command::Acp {
            builtins,
            enable_scheduler,
        }) => heybuddy::acp::server::run(builtins, enable_scheduler).await,
        #[cfg(feature = "roaming")]
        ...
        Some(Command::ValidateExtensions { file }) => {
            use heybuddy::agents::validate_extensions::validate_bundled_extensions;
...
mod tests {
    ...
    fn completion_command_accepts_nushell_alias() {
        let cli = Cli::try_parse_from(["heybuddy", "completion", "nushell"]).expect("parse failed");
    ...
    #[test]
    fn migrate_config_command_defaults_to_dry_run() {
        ...
        match cli.command {
            Some(Command::MigrateConfig(args)) => assert!(!args.apply),
            _ => panic!("expected migrate-config command"),
        }
    }

    #[test]
    fn config_migration_apply_preserves_source_bytes_and_writes_canonical_output() {
        let dir = tempdir().unwrap();
        let source = dir.path().join("config.yaml");
        let destination = dir.path().join("migrated.yaml");
        let source_bytes =
            b"GOOSE_MODEL: secret-model\nprovider:\n  GOOSE_API_KEY: nested-secret\n";
        fs::write(&source, source_bytes).unwrap();

        let changes = migrate_config_file(&source, &destination, true).unwrap();

        assert_eq!(changes, vec![("GOOSE_MODEL", "HEYBUDDY_MODEL")]);
        assert_eq!(fs::read(&source).unwrap(), source_bytes);
        assert_eq!(
            fs::read(migration_backup_path(&source)).unwrap(),
            source_bytes
        );
        let migrated = fs::read_to_string(&destination).unwrap();
        ...
        #[cfg(unix)]
        for path in [migration_backup_path(&source), destination] {
            use std::os::unix::fs::PermissionsExt;
    ...
    #[test]
    fn config_migration_refuses_existing_destination() {
        let dir = tempdir().unwrap();
        let source = dir.path().join("config.yaml");
        let destination = dir.path().join("migrated.yaml");
        fs::write(&source, "GOOSE_MODEL: model\n").unwrap();
        fs::write(&destination, "keep me\n").unwrap();

        let error = migrate_config_file(&source, &destination, true).unwrap_err();

        assert!(error.to_string().contains("destination already exists"));
        assert_eq!(fs::read_to_string(&destination).unwrap(), "keep me\n");
        assert!(!migration_backup_path(&source).exists());
    }
    ...
    fn session_accepts_provider_override_without_resume() {
        let cli = Cli::try_parse_from(["heybuddy", "session", "--provider", "openai"])
    ...
    fn session_accepts_model_override_without_resume() {
        let cli = Cli::try_parse_from(["heybuddy", "session", "--model", "gpt-5.4"])
    ...
    fn session_accepts_system_prompt() {
        let cli = Cli::try_parse_from(["heybuddy", "session", "--system", "extra instructions"])
    ...
    fn nushell_completion_generation_emits_module() {
        ...
        CompletionShell::Nu.generate(&mut cmd, "heybuddy", &mut buffer);

        let script = String::from_utf8(buffer).expect("utf8");
        assert!(script.contains("module completions"));
        assert!(script.contains("export extern heybuddy"));
    ...
    fn term_init_help_mentions_nushell() {
        ...
        assert!(help.contains("heybuddy term init nu"));
    ...
    fn skills_command_accepts_list_subcommand() {
        let cli = Cli::try_parse_from(["heybuddy", "skills", "list"]).expect("parse failed");
