...
use anyhow::Result;
use heybuddy_roaming::RoamingClientStream;
...
fn prompt_permission(request: &RequestPermissionRequest) -> RequestPermissionOutcome {
    ...
    // Fail closed: option 1 is allow-always for heybuddy hosts, so EOF, an empty
