...
#[cfg(feature = "nostr")]
use heybuddy::session::nostr_share;
...
pub async fn handle_session_export(
    ...
    if nostr {
        return Err(anyhow::anyhow!("heybuddy was not built with nostr support"));
    }
...
pub async fn handle_session_import(input: String, nostr: bool) -> Result<()> {
    ...
    let format = heybuddy::session::import_formats::detect_format(&json);
    let label = match format {
        heybuddy::session::import_formats::ImportFormat::HeyBuddy => "heybuddy",
        heybuddy::session::import_formats::ImportFormat::ClaudeCode => "Claude Code",
        heybuddy::session::import_formats::ImportFormat::Codex => "Codex",
        heybuddy::session::import_formats::ImportFormat::Pi => "Pi",
    };
