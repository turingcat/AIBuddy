...
fn asset_name() -> &'static str {
    ...
    {
        "heybuddy-aarch64-apple-darwin.tar.bz2"
    }

    #[cfg(all(target_os = "linux", target_arch = "x86_64", target_env = "gnu"))]
    {
        "heybuddy-x86_64-unknown-linux-gnu.tar.bz2"
    }
    #[cfg(all(target_os = "linux", target_arch = "aarch64", target_env = "gnu"))]
    {
        "heybuddy-aarch64-unknown-linux-gnu.tar.bz2"
    }
    #[cfg(all(target_os = "linux", target_arch = "x86_64", target_env = "musl"))]
    {
        "heybuddy-x86_64-unknown-linux-musl.tar.bz2"
    }
    #[cfg(all(target_os = "linux", target_arch = "aarch64", target_env = "musl"))]
    {
        "heybuddy-aarch64-unknown-linux-musl.tar.bz2"
    }
    #[cfg(all(target_os = "windows", target_arch = "x86_64", feature = "cuda"))]
    {
        "heybuddy-x86_64-pc-windows-msvc-cuda.zip"
    }
    #[cfg(all(target_os = "windows", target_arch = "x86_64", not(feature = "cuda")))]
    {
        "heybuddy-x86_64-pc-windows-msvc.zip"
    }
}
...
fn sha256_hex(data: &[u8]) -> String {
    ...
    hasher.update(data);
    heybuddy::utils::bytes_to_hex(hasher.finalize())
}
...
async fn fetch_bundle(client: &reqwest::Client, url: &str) -> Result<serde_json::Value> {
    let resp = client
        .get(url)
        .header("User-Agent", "heybuddy-cli")
        .send()
        .await
...
/// Update the heybuddy binary to the latest release.
///
...
pub async fn update(canary: bool, reconfigure: bool) -> Result<()> {
    ...
    #[cfg(not(feature = "disable-update"))]
    {
        ...
        println!("heybuddy updated successfully (verified with Sigstore SLSA provenance).");

        // --- Reconfigure if requested -------------------------------------------
        if reconfigure {
            println!("Running heybuddy configure...");
            let status = Command::new(current_exe)
                .arg("configure")
                .status()
                .context("Failed to run heybuddy configure")?;
            if !status.success() {
                eprintln!("Warning: heybuddy configure exited with {status}");
            }
        }
...
///   1. A `heybuddy-package/` subdirectory (Windows releases)
...
fn find_binary(extract_dir: &Path, binary_name: &str) -> Option<PathBuf> {
    // 1. Check heybuddy-package subdir (matches download_cli.sh / download_cli.ps1)
    let package_dir = extract_dir.join("heybuddy-package");
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        if old_exe.exists() {
            fs::remove_file(&old_exe).with_context(|| {
                format!(
                    "Failed to remove old backup {}. Is another heybuddy process running?",
                    old_exe.display()
                )
            })?;
        }
        ...
        fs::rename(current_exe, &old_exe).with_context(|| {
            format!(
                "Failed to rename running binary to {}. Try closing HeyBuddy Desktop if it's open.",
                old_exe.display()
            )
        })?;
...
mod tests {
    ...
    fn test_asset_name_valid() {
        ...
        assert!(name.starts_with("heybuddy-"));
        #[cfg(target_os = "windows")]
    ...
    fn test_binary_name() {
        ...
        #[cfg(target_os = "windows")]
        assert_eq!(name, "heybuddy.exe");
        #[cfg(not(target_os = "windows"))]
        assert_eq!(name, "heybuddy");
    }

    #[test]
    fn test_find_binary_in_package_subdir() {
        let tmp = tempdir().unwrap();
        let pkg = tmp.path().join("heybuddy-package");
    ...
    fn test_replace_binary_basic() {
        let tmp = tempdir().unwrap();
        let new_bin = tmp.path().join("new_heybuddy");
        let current = tmp.path().join("current_heybuddy");
    ...
    fn test_replace_binary_windows_rename_away() {
        let tmp = tempdir().unwrap();
        let current = tmp.path().join("heybuddy.exe");
        let new_bin = tmp.path().join("new_heybuddy.exe");
    ...
    fn test_extract_zip_with_package_dir() {
        ...
        // Create a zip in memory with heybuddy-package/ structure
        let mut buf = Vec::new();
        {
            ...
            writer.add_directory("heybuddy-package/", options).unwrap();
            writer
                .start_file("heybuddy-package/heybuddy.exe", options)
                .unwrap();
            writer.write_all(b"fake heybuddy binary").unwrap();
            writer
                .start_file("heybuddy-package/libtest.dll", options)
                .unwrap();
        ...
        let binary = find_binary(tmp.path(), "heybuddy.exe");
        assert!(binary.is_some());

        let content = fs::read_to_string(binary.unwrap()).unwrap();
        assert_eq!(content, "fake heybuddy binary");

        // DLL should be in heybuddy-package too
        assert!(tmp.path().join("heybuddy-package/libtest.dll").exists());
    }
    ...
    fn test_validate_entry_path_accepts_safe_paths() {
        assert!(validate_entry_path(Path::new("heybuddy")).is_ok());
        assert!(validate_entry_path(Path::new("heybuddy-package/heybuddy")).is_ok());
    ...
    fn test_extract_tar_bz2_safe_archive() {
        ...
        let mut builder_buf = Vec::new();
        {
            ...
            let data = b"heybuddy binary content";
            let mut header = tar::Header::new_gnu();
            header.set_size(data.len() as u64);
            header.set_mode(0o755);
            header.set_cksum();
            builder
                .append_data(&mut header, "heybuddy-package/heybuddy", &data[..])
                .unwrap();
        ...
        let extracted = tmp.path().join("heybuddy-package/heybuddy");
        assert!(extracted.exists());
        assert_eq!(
            fs::read_to_string(extracted).unwrap(),
            "heybuddy binary content"
        );
    }
