use anyhow::Result;
use heybuddy::execution::manager::AgentManager;
use heybuddy::gateway::manager::GatewayManager;
use std::sync::Arc;
...
pub async fn handle_gateway_start(
    ...
    let mut config = heybuddy::gateway::GatewayConfig {
        gateway_type,
        platform_config,
        max_sessions: 0,
    };

    let gw = heybuddy::gateway::create_gateway(&mut config)?;
