    ...
    match scheduler.add_scheduled_job(job, true).await {
        Ok(_) => {
            ...
            let scheduled_recipes_dir = get_default_scheduled_recipes_dir()
                .unwrap_or_else(|_| Path::new("./.heybuddy_scheduled_recipes").to_path_buf()); // Fallback for display
...
pub async fn handle_schedule_services_status() -> Result<()> {
    ...
    println!(
        "The built-in scheduler runs within the heybuddy process and requires no external services."
    );
    Ok(())
}
...
pub async fn handle_schedule_cron_help() -> Result<()> {
    println!("📅 Cron Expression Guide for heybuddy Scheduler");
    println!("===========================================\\n");
    ...
    println!("💡 EXAMPLES:");
    println!(
        "  heybuddy schedule add --schedule-id hourly-report --cron \"0 * * * *\" --recipe-source report.yaml"
    );
    println!(
        "  heybuddy schedule add --schedule-id daily-backup --cron \"@daily\" --recipe-source backup.yaml"
    );
    println!("  heybuddy schedule add --schedule-id weekly-summary --cron \"0 9 * * 1\" --recipe-source summary.yaml");
