...
use heybuddy::conversation::message::{Message, MessageContent, MessageMetadata};
...
alias @heybuddy='{heybuddy_bin} term run'
alias @g='{heybuddy_bin} term run'

heybuddy_preexec() {
    [[ "$1" =~ ^heybuddy\ term ]] && return
    [[ "$1" =~ ^(@heybuddy|@g)($|[[:space:]]) ]] && return
    ('{heybuddy_bin}' term log "$1" &) 2>/dev/null
}

if [[ -z "$heybuddy_preexec_installed" ]]; then
    heybuddy_preexec_installed=1
    trap 'heybuddy_preexec "$BASH_COMMAND"' DEBUG
fi{command_not_found_handler}"#,
    command_not_found: Some(
        r#"

command_not_found_handle() {
    echo "🪿 Command '$1' not found. Asking heybuddy..."
    '{heybuddy_bin}' term run "$@"
    return 0
}"#,
    ),
};
...
add-zsh-hook preexec heybuddy_preexec{command_not_found_handler}"#,
    command_not_found: Some(
        r#"
...
function @heybuddy; {heybuddy_bin} term run $argv; end
function @g; {heybuddy_bin} term run $argv; end

function heybuddy_preexec --on-event fish_preexec
    string match -q -r '^heybuddy term' -- $argv[1]; and return
    string match -q -r '^(@heybuddy|@g)($|\s)' -- $argv[1]; and return
    {heybuddy_bin} term log "$argv[1]" 2>/dev/null &
end"#,
...
def --wrapped @heybuddy [...args] { run-external "{heybuddy_bin}" "term" "run" ...$args }
def --wrapped @g [...args] { run-external "{heybuddy_bin}" "term" "run" ...$args }

if (($env | get -o HEYBUDDY_NU_PREEXEC_INSTALLED | default false) != true) {
    $env.HEYBUDDY_NU_PREEXEC_INSTALLED = true
    $env.config.hooks.pre_execution = (
        ...
        | append {||
            ...
            if ($line =~ '^heybuddy term(\s|$)') {
                return
            }
            if ($line =~ '^(@heybuddy|@g)(\s|$)') {
                return
            }
            job spawn { run-external "{heybuddy_bin}" "term" "log" $line | complete | ignore } | ignore
        }
    )
}
{command_not_found_handler}"#,
...
$env.config.hooks.command_not_found = {|command_name|
    ...
    print $"🪿 Command '($command_name)' not found. Asking heybuddy..."
    run-external "{heybuddy_bin}" "term" "run" $prompt | complete | ignore
    null
}"#,
    ),
};
...
static POWERSHELL_CONFIG: ShellConfig = ShellConfig {
    script_template: r#"$env:AGENT_SESSION_ID = "{session_id}"
function @heybuddy {{ & '{heybuddy_bin}' term run @args }}
function @g {{ & '{heybuddy_bin}' term run @args }}

Set-PSReadLineKeyHandler -Chord Enter -ScriptBlock {{
    $line = $null
    [Microsoft.PowerShell.PSConsoleReadLine]::GetBufferState([ref]$line, [ref]$null)
    if ($line -notmatch '^heybuddy term' -and $line -notmatch '^(@heybuddy|@g)($|\s)') {{
        Start-Job -ScriptBlock {{ & '{heybuddy_bin}' term log $using:line }} | Out-Null
    }}
...
fn render_term_init_script(
    ...
    let command_not_found_handler = if with_command_not_found {
        config
            .command_not_found
            .map(|handler| handler.replace("{heybuddy_bin}", heybuddy_bin))
            .unwrap_or_default()
    } else {
        String::new()
    };
    ...
    config
        ...
        .replace("{heybuddy_bin}", heybuddy_bin)
...
pub async fn handle_term_init(
    ...
    let session = match named_session {
        ...
        None => {
            let session = session_manager
                .create_session(
                    working_dir,
                    "HeyBuddy Term Session".to_string(),
    ...
    let heybuddy_bin = std::env::current_exe()
        .map(|p| p.to_string_lossy().into_owned())
        .unwrap_or_else(|_| "heybuddy".to_string());

    println!(
        "{}",
        render_term_init_script(shell, &session.id, &heybuddy_bin, with_command_not_found)
    );
    Ok(())
}
...
pub async fn handle_term_log(command: String) -> Result<()> {
    let session_id = std::env::var("AGENT_SESSION_ID").map_err(|_| {
        anyhow!(
            "AGENT_SESSION_ID not set. Initialize terminal integration with `heybuddy term init <shell>` and reload your shell first."
        )
    })?;
    ...
    let session_id = std::env::var("AGENT_SESSION_ID").map_err(|_| {
        anyhow!(
            "AGENT_SESSION_ID not set.\n\n\
             Initialize terminal integration with `heybuddy term init <shell>` in your shell profile, \
...
/// Handle `heybuddy term info` - print compact session info for prompt integration
pub async fn handle_term_info() -> Result<()> {
    ...
    let model_name = session
        ...
        .or_else(|| config.get_heybuddy_model().ok())
        .map(|name| {
            let short = name.rsplit('/').next().unwrap_or(&name);
            if let Some(stripped) = short.strip_prefix("heybuddy-") {
                stripped.to_string()
            } else {
                short.to_string()
            }
    ...
    let context_limit = session
        ...
        .and_then(|session| {
            ...
            heybuddy::context_limit::get_local_context_limit(provider_name, &model.model_name).ok()
        })
...
mod tests {
    ...
    fn render_term_init_script_includes_nushell_hooks() {
        let script = render_term_init_script(Shell::Nu, "session-123", "/tmp/heybuddy", false);

        assert!(script.contains("$env.AGENT_SESSION_ID = \"session-123\""));
        assert!(script.contains("def --wrapped @heybuddy [...args]"));
        assert!(script.contains("def --wrapped @g [...args]"));
        assert!(script.contains("HEYBUDDY_NU_PREEXEC_INSTALLED"));
        assert!(script.contains("$env.config.hooks.pre_execution"));
        assert!(script.contains("job spawn { run-external \"/tmp/heybuddy\" \"term\" \"log\" $line | complete | ignore } | ignore"));
        assert!(!script.contains("command_not_found = {|command_name|"));
    }

    #[test]
    fn render_term_init_script_includes_nushell_default_handler() {
        let script = render_term_init_script(Shell::Nu, "session-123", "/tmp/heybuddy", true);

        assert!(script.contains("$env.config.hooks.command_not_found = {|command_name|"));
        assert!(script.contains(
            "run-external \"/tmp/heybuddy\" \"term\" \"run\" $prompt | complete | ignore"
        ));
    }

    #[test]
    fn render_term_init_script_skips_unsupported_default_handler() {
        let script = render_term_init_script(Shell::Fish, "session-123", "/tmp/heybuddy", true);
