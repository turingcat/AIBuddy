//! `heybuddy roam` — peer-to-peer agent access over iroh.
//!
...
//! accepted peer gets heybuddy's full ACP surface.
//!
//! Subcommands:
...
use heybuddy::config::{Config, ConfigError};
...
const CARD_SCHEME: &str = "heybuddy+roam://";
...
/// Config key (or `HEYBUDDY_ROAM_RELAYS` env) overriding the relay URLs the
/// roaming endpoint uses. When unset, roaming uses the managed relays below —
/// never iroh's shared public n0 relays.
const CONFIG_ROAM_RELAYS_KEY: &str = "HEYBUDDY_ROAM_RELAYS";
...
const CONFIG_ROAM_RELAY_TOKEN_KEY: &str = "HEYBUDDY_ROAM_RELAY_TOKEN";
...
///
/// Override with `HEYBUDDY_ROAM_RELAYS` to point at other relays (e.g. a Block-run
/// deployment).
...
///
/// Uses `HEYBUDDY_ROAM_RELAYS` (env or config file) when set — optionally
/// authenticated with the `HEYBUDDY_ROAM_RELAY_TOKEN` secret applied to each —
/// otherwise the default managed relays. Never iroh's public n0 relays.
///
/// Fails when `HEYBUDDY_ROAM_RELAYS` is set but unreadable: a deployment that
...
pub enum RoamCommand {
    ...
    /// Each connected peer gets heybuddy's full ACP surface — it drives its own
    ...
    Connect {
        /// A saved peer nickname (see `roam peers`) or a `heybuddy+roam://...` card.
        target: String,
    ...
    ///
    /// Defaults to stdio (for a client that spawns `heybuddy roam bridge ...` as a
...
pub enum PeersCommand {
    ...
    Add {
        /// The peer's `heybuddy+roam://...` card.
        card: String,
        /// Friendly nickname (defaults to a short id if omitted).
        name: Option<String>,
    },
    /// Accept inbound connections from a peer's key. The target is a saved
    /// nickname or a `heybuddy+roam://...` card (which is also saved to the address
    /// book). An accepted peer gets heybuddy's full ACP surface.
    Accept {
        /// A saved nickname or a `heybuddy+roam://...` card.
        target: String,
...
async fn handle_id(qr: bool) -> Result<()> {
    ...
    eprintln!();
    eprintln!("the peer adds it with:  heybuddy roam peers add '<card>' <name>");
    eprintln!("and accepts you with:   heybuddy roam peers accept <name>");
    node.shutdown().await?;
    Ok(())
}
...
async fn handle_pair(name: Option<String>) -> Result<()> {
    ...
    let decoded = ConnectionCard::decode(device_card)
        .context("that does not look like a heybuddy+roam:// connection card")?;
    let name =
    ...
    heybuddy_roaming::PeerBook::update(peerbook_path(), |book| {
...
async fn handle_peers(command: PeersCommand) -> Result<()> {
    let book = heybuddy_roaming::PeerBook::load(peerbook_path())?;
    match command {
        PeersCommand::Add { card, name } => {
            ...
            eprintln!("accept connections from it with: heybuddy roam peers accept {name}");
            Ok(())
        }
        PeersCommand::Accept { target, name } => {
            ...
            eprintln!("a running `heybuddy roam share` picks this up on the next connection");
            Ok(())
        }
        ...
        PeersCommand::Remove { name } => {
            let existed =
                heybuddy_roaming::PeerBook::update(peerbook_path(), |book| book.remove(&name))?;
            if existed {
        ...
        PeersCommand::Rename { from, to } => {
            heybuddy_roaming::PeerBook::update(peerbook_path(), |book| book.rename(&from, &to))?;
        ...
        PeersCommand::List => {
            ...
            if peers.is_empty() && accepted.is_empty() {
                eprintln!("no saved peers; add one with `heybuddy roam peers add '<card>' <name>`");
                return Ok(());
            }
...
fn resolve_key(book: &heybuddy_roaming::PeerBook, target: &str) -> Result<EndpointId> {
...
async fn handle_list() -> Result<()> {
    ...
    for e in entries {
        ...
        let dir = match e.direction {
            heybuddy_roaming::Direction::Inbound => "inbound",
            heybuddy_roaming::Direction::Outbound => "outbound",
        };
...
/// advisory lock decides ownership across all heybuddy processes (desktop
...
pub(crate) fn try_acquire_roam_lock() -> Result<std::fs::File> {
    try_acquire_roam_lock_owner()?.ok_or_else(|| {
        anyhow::anyhow!("another heybuddy process is already running the roaming endpoint")
    })
}
    ...
    if accepted_count == 0 {
        eprintln!(
            "warning: no peers are accepted yet — no one can connect.\n\
             Accept a peer's key first: heybuddy roam peers accept <name|card>"
        );
    }
...
fn resolve_card(target: &str) -> Result<ConnectionCard> {
    ...
    match book.get(target) {
        ...
        None => anyhow::bail!(
            "no saved peer named `{target}` (and it is not a card); see `heybuddy roam peers`"
        ),
    }
}
...
async fn dial_target(
    ...
    heybuddy_roaming::RoamingClientStream,
)> {
...
async fn handle_bridge(
    ...
    // outlive the splice.
    let heybuddy_roaming::RoamingClientStream {
        conn,
        send: remote_send,
        recv: remote_recv,
        ..
