...
use heybuddy::checks::{discover, DiscoveredReview};
use heybuddy::subprocess::git_command;
...
/// Entry point for the `heybuddy review` subcommand.
pub async fn handle_review(opts: ReviewOptions) -> Result<()> {
    ...
    if diff.trim().is_empty() {
        eprintln!("heybuddy review: no changes to review");
        return Ok(());
    }
    ...
    if !use_orchestrator {
        ...
        if opts.checks_only {
            ...
            if !opts.quiet {
                ...
                eprintln!(
                    "heybuddy review: emitted {total_emitted} finding(s) from {} check(s) ({suppressed} hidden below severity={:?})",
                    discovered.checks.len(),
                    min_sev
                );
            }
    ...
    if !opts.quiet {
        ...
        if suppressed == 0 {
            eprintln!(
                "heybuddy review: orchestrator emitted {total_emitted} finding(s) from {} check(s) (main: {main_pass_label}, {} finding(s))",
                discovered.checks.len(),
                main_findings.len()
            );
        } else {
            eprintln!(
                "heybuddy review: orchestrator emitted {total_emitted} finding(s) from {} check(s) (main: {main_pass_label}, {} finding(s); {suppressed} hidden below severity={:?})",
                discovered.checks.len(),
                main_findings.len(),
                min_sev
            );
...
fn print_discovered_summary(d: &DiscoveredReview) {
    if d.checks.is_empty() {
        eprintln!("heybuddy review: no checks or REVIEW.md rules discovered");
        return;
    }
    eprintln!("heybuddy review: discovered {} check(s):", d.checks.len());
    for c in &d.checks {
...
/// `discovery_root` so [`heybuddy::checks::discover`] doesn't double-
