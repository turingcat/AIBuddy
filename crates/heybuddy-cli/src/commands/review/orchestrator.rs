//! Deterministic, Rust-driven orchestration for `heybuddy review`.
//!
...
//!
//! - One subprocess per check (`heybuddy run -q -t <prompt>`)
...
use heybuddy::checks::{Check, DEFAULT_CHECK_TURN_LIMIT};
...
/// Run all discovered checks concurrently as `heybuddy run` subprocesses.
///
...
pub async fn run_checks_in_parallel(
    ...
    while let Some(joined) = set.join_next().await {
        let (idx, check, result, quiet) = match joined {
            Ok(v) => v,
            Err(e) => {
                eprintln!("heybuddy review: check task panicked: {e}");
                continue;
            }
        };
        ...
        match result {
            Ok(findings) => {
                if !quiet {
                    eprintln!(
                        "heybuddy review: check '{}' completed: {} finding(s)",
                        check.name,
                        findings.len()
                    );
                }
            ...
            Err(e) => {
                ...
                eprintln!("heybuddy review: check '{}' failed: {e}", check.name);
                results[idx] = Vec::new();
            }
        }
    }
...
///    asks for `heybuddy-claude-4-sonnet` would 404 against Google's API),
...
///
/// Uses `heybuddy review --turn-limit` when set, otherwise
...
/// Spawn a single `heybuddy run` subprocess for one check and parse its
...
/// Generic `heybuddy run` subprocess that hands a prompt to the model
...
async fn run_subprocess_for_findings(
    ...
    let heybuddy_bin = std::env::current_exe().context("locate current heybuddy binary")?;

    let mut cmd = Command::new(&heybuddy_bin);
    cmd.arg("run")
        .arg("--no-session")
        .arg("--quiet")
        .arg("--no-profile")
    ...
    if let Some(mut stdin) = child.stdin.take() {
        ...
        // Closing stdin signals EOF to `heybuddy run -i -`.
        drop(stdin);
    }
...
pub async fn run_main_pass_in_parallel(
    ...
    while let Some(joined) = set.join_next().await {
        let (idx, path, result, quiet) = match joined {
            Ok(v) => v,
            Err(e) => {
                eprintln!("heybuddy review: main-pass task panicked: {e}");
                continue;
            }
        };
        match result {
            Ok(raw) => {
                ...
                if !quiet {
                    eprintln!(
                        "heybuddy review: main pass on '{}' completed: {} finding(s)",
                        path,
                        findings.len()
                    );
                }
            ...
            Err(e) => {
                ...
                eprintln!("heybuddy review: main pass on '{}' failed: {e}", path);
...
/// cap enforced by heybuddy. Without this, models routinely burn turns on
...
fn build_subprocess_turn_budget_section(max_turns: usize) -> String {
    format!(
        "## Turn budget\n\n\
         ...
         heybuddy enforces this via `--max-turns`; when you exhaust it, the session stops and \
...
mod tests {
    ...
    fn resolve_check_model_cli_provider_wins_over_per_check_model() {
        let mut c = ck("perf");
        c.model = Some("heybuddy-claude-4-sonnet".into()); // wrong provider
        let opts = ReviewOptions {
    ...
    fn resolve_check_model_cli_provider_alone_drops_per_check_model() {
        ...
        let mut c = ck("perf");
        c.model = Some("heybuddy-claude-4-sonnet".into());
        let opts = ReviewOptions {
            provider: Some("google".into()),
            default_model: None,
            ..ReviewOptions::default()
