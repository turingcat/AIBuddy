...
use heybuddy::session::session_manager::{DB_NAME, SESSIONS_FOLDER};
...
async fn check_provider(
    ...
    let (provider, model) = match (config.get_heybuddy_provider(), config.get_heybuddy_model()) {
    ...
    let model_config = heybuddy::model_config::model_config_from_user_config(&provider, &model)
        .map_err(|e| ProviderCheckError::InvalidModel(e.to_string()))?;

    let provider_client = heybuddy::providers::create(&provider, Vec::new())
        .await
        .map_err(|e| {
    ...
    heybuddy::session_context::with_session_id(
...
pub async fn handle_info(verbose: bool, check: bool) -> Result<()> {
    ...
    println!("{}", style("heybuddy Version:").cyan().bold());
    ...
    if verbose {
        println!("\n{}", style("heybuddy Configuration:").cyan().bold());
        let values = config.all_values()?;
        if values.is_empty() {
            println!("  No configuration values set");
            println!(
                "  Run '{}' to configure heybuddy",
    ...
    if check {
        ...
        match &result {
            ...
            Err(ProviderCheckError::NotConfigured { label, error }) => {
                ...
                print_aligned(
                    "Hint:",
                    &format!("Run '{}'", style("heybuddy configure").cyan()),
                    label_padding,
                );
            }
        ...
        // checks, health probes) can rely on `heybuddy info --check` as a
        // pre-flight verifier.
        if result.is_err() {
