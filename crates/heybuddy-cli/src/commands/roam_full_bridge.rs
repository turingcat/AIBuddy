//! The roaming host adapter: serve heybuddy's **full** ACP surface to each
//! connecting peer.
//!
//! Roaming is just an authenticated p2p ACP transport. This adapter is the thin
//! seam that hands an authorized iroh stream straight to heybuddy's real
...
use heybuddy::acp::server::serve;
use heybuddy::acp::server_factory::AcpServer;
use heybuddy_roaming::{AcpStreamServer, EndpointId};

/// An [`AcpStreamServer`] that serves heybuddy's full ACP surface, a fresh agent
/// per connection.
pub struct FullAcpBridge {
    ...
    /// `AcpServer` (e.g. `heybuddy serve`) leaves `session_cwd` unset for its
    /// local clients.
