use super::completion::HeyBuddyCompleter;
use super::paste::{
...
/// prompt. Configurable via `HEYBUDDY_CLI_NEWLINE_KEY`, defaulting to `j` (Ctrl+J).
...
pub fn get_newline_key() -> char {
    Config::global()
        .get_param::<String>("HEYBUDDY_CLI_NEWLINE_KEY")
        .ok()
...
///
/// When `heybuddy_prompt_editor` is configured, defaults to `true` (backward compat).
/// Users can override by explicitly setting `heybuddy_prompt_editor_always` to `false`.
...
pub fn get_input(
    editor: &mut Editor<HeyBuddyCompleter, rustyline::history::DefaultHistory>,
    conversation_messages: Option<&Vec<String>>,
) -> Result<InputResult> {
    let config = Config::global();
    let prompt_editor = config.get_heybuddy_prompt_editor().ok().flatten();
    let editor_always_override = config.get_heybuddy_prompt_editor_always().ok().flatten();
    let editor_always = should_use_editor_always(prompt_editor.as_deref(), editor_always_override);

    if editor_always {
        if let Ok(Some(editor_cmd)) = config.get_heybuddy_prompt_editor() {
...
fn handle_slash_command(input: &str) -> Option<InputResult> {
    ...
    match input {
        ...
        s if s.starts_with(CMD_MODE) => Some(InputResult::HeyBuddyMode(
...
fn help_text() -> String {
    let modes = HeyBuddyMode::VARIANTS.join(", ");
...
/mode <name> - Set the heybuddy mode to use ({modes})
...
/plan <message_text> -  Enters 'plan' mode with optional message. Create a plan based on the current messages and asks user if they want to act on it.
                        If user acts on the plan, heybuddy mode is set to 'auto' and returns to 'normal' heybuddy mode.
                        To warm up heybuddy before using '/plan', we recommend setting '/mode approve' & putting appropriate context into heybuddy.
                        The model is used based on $HEYBUDDY_PLANNER_PROVIDER and $HEYBUDDY_PLANNER_MODEL environment variables.
...
/edit [text] - Open your prompt editor to compose a message. Optionally pre-fill with text.
               Uses $HEYBUDDY_PROMPT_EDITOR, $VISUAL, or $EDITOR (in that order).
...
Navigation:
Enter - Send message
Ctrl+{newline_key} - Add a newline (configurable via HEYBUDDY_CLI_NEWLINE_KEY)
...
fn additional_builtin_help() -> String {
    ...
    heybuddy::agents::execute_commands::list_commands()
        .iter()
...
fn print_editor_help() {
  ...
  Configure editor: heybuddy configure set heybuddy_prompt_editor \"vim\"
  Falls back to $VISUAL or $EDITOR if heybuddy_prompt_editor is not set.
  When heybuddy_prompt_editor is set, the editor is used for every prompt by default.
  To use inline prompts with on-demand /edit: heybuddy configure set heybuddy_prompt_editor_always false"
    );
}
...
mod tests {
    ...
    fn help_lists_builtin_agent_commands() {
        ...
        for command in heybuddy::agents::execute_commands::list_commands() {
            assert!(
