...
use heybuddy::conversation::message::{
    ActionRequiredData, Message, MessageContent, SystemNotificationContent, SystemNotificationType,
    ToolNameParts, ToolRequest, ToolResponse,
};
use heybuddy::providers::canonical_cost::estimate_model_cost;
#[cfg(target_os = "windows")]
use heybuddy::subprocess::SubprocessExt;
    ...
    fn as_str(&self) -> String {
        match self {
            Theme::Light => Config::global()
                .get_param::<String>("HEYBUDDY_CLI_LIGHT_THEME")
                .unwrap_or(DEFAULT_CLI_LIGHT_THEME.to_string()),
            Theme::Dark => Config::global()
                .get_param::<String>("HEYBUDDY_CLI_DARK_THEME")
...
thread_local! {
    static CURRENT_THEME: RefCell<Theme> = RefCell::new(
        std::env::var("HEYBUDDY_CLI_THEME").ok()
            .map(|val| Theme::from_config_str(&val))
            .unwrap_or_else(||
                Config::global().get_param::<String>("HEYBUDDY_CLI_THEME").ok()
                    .map(|val| Theme::from_config_str(&val))
                    .unwrap_or(Theme::Ansi)
            )
    );
    static SHOW_FULL_TOOL_OUTPUT: RefCell<bool> = RefCell::new(
        Config::global().get_param::<bool>("HEYBUDDY_SHOW_FULL_OUTPUT").unwrap_or(false)
    );
}
...
pub fn set_theme(theme: Theme) {
    ...
    config
        .set_param("HEYBUDDY_CLI_THEME", theme.as_config_string())
    ...
    if let Err(e) = config.set_param("HEYBUDDY_CLI_THEME", theme_str) {
        ...
        match failure.label.as_deref() {
            ...
            Some(label) => {
                ...
                eprintln!(
                    "{}",
                    style(format!(
                        "    Hint: ask heybuddy to help debug the '{}' extension",
                        label
                    ))
                    .dim()
                );
...
pub fn run_status_hook(status: &str) {
    if let Ok(hook) = Config::global().get_param::<String>("HEYBUDDY_STATUS_HOOK") {
...
pub fn heybuddy_mode_message(text: &str) {
    println!("\n{} {}", accent("mode:"), text);
}

fn should_show_thinking() -> bool {
    Config::global()
        .get_param::<bool>("HEYBUDDY_CLI_SHOW_THINKING")
        .unwrap_or(false)
...
pub fn display_session_info(
    ...
    // ASCII art heybuddy with session info on the right
    println!();
    println!(
        "  {}  {} {} {} {} {}",
        style("  __( O)>").white(),
...
mod tests {
    ...
    fn terminal_line_sanitizer_preserves_plain_unicode_text() {
        assert_eq!(
            sanitize_terminal_line("heybuddy 🪿\t日本語"),
            "heybuddy 🪿\t日本語"
        );
    }
