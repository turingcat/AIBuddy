...
use heybuddy::agents::platform_extensions::developer::shell::{
    parse_shell_output_notification, ShellOutputNotificationParams, ShellOutputStream,
};
use heybuddy::agents::AgentEvent;
use heybuddy::agents::SUBAGENT_TOOL_REQUEST_TYPE;
use heybuddy::permission::Permission;
use heybuddy::providers::base::Provider;
use heybuddy::providers::base::ProviderUsage;
use heybuddy::utils::safe_truncate;

use anyhow::{Context, Result};
use completion::HeyBuddyCompleter;
use heybuddy::agents::extension::{Envs, ExtensionConfig, PLATFORM_EXTENSIONS};
use heybuddy::agents::types::RetryConfig;
use heybuddy::agents::{
    context_management_unsupported_message, Agent, SessionConfig, COMPACT_TRIGGERS,
};
use heybuddy::config::extensions::name_to_key;
...
use heybuddy::config::providers;
use heybuddy::conversation::message::{ActionRequiredData, Message, MessageContent};
use heybuddy::providers::inventory::ProviderInventoryService;
...
impl HistoryManager {
    ...
    fn load(
        &self,
        editor: &mut rustyline::Editor<HeyBuddyCompleter, rustyline::history::DefaultHistory>,
    ) {
...
pub async fn classify_planner_response(
    ...
    model_config: heybuddy_providers::model::ModelConfig,
    ...
    let (result, _usage) = heybuddy::session_context::with_session_id(
...
impl CliSession {
    ...
    pub fn parse_stdio_extension(extension_command: &str) -> Result<ExtensionConfig> {
        ...
        let mut parts = heybuddy::utils::split_command_args(command)?;
        ...
        Ok(ExtensionConfig::Stdio {
            ...
            envs: Envs::new(envs),
            env_keys: Vec::new(),
            description: heybuddy::config::DEFAULT_EXTENSION_DESCRIPTION.to_string(),
            timeout: Some(heybuddy::config::DEFAULT_EXTENSION_TIMEOUT),
            cwd: None,
            bundled: None,
    ...
    pub async fn interactive(&mut self, prompt: Option<String>) -> Result<()> {
        let banners = self
            .agent
            .emit_hook_with_banners(heybuddy::hooks::HookEvent::SessionStart, &self.session_id)
            .await;
        if !banners.is_empty() {
        ...
        self.agent
            .emit_hook(heybuddy::hooks::HookEvent::SessionEnd, &self.session_id)
            .await;
    ...
    fn create_editor(
        &self,
    ) -> Result<rustyline::Editor<HeyBuddyCompleter, rustyline::history::DefaultHistory>> {
        let builder =
        ...
        let mut editor =
            rustyline::Editor::<HeyBuddyCompleter, rustyline::history::DefaultHistory>::with_config(
                config,
            )?;
        let completer = HeyBuddyCompleter::new(self.completion_cache.clone());
    ...
    async fn handle_input(
        ...
        match input {
            ...
            InputResult::HeyBuddyMode(mode) => {
                history.save(editor);
                self.handle_heybuddy_mode(&mode).await?;
            }
                    ...
                    None => {
                        output::render_error(
                            "No editor found. Set one with:\n  \
                                 heybuddy configure set heybuddy_prompt_editor \"vim\"\n  \
    ...
    async fn handle_heybuddy_mode(&self, mode: &str) -> Result<()> {
        let config = Config::global();
        let mode = match HeyBuddyMode::from_str(&mode.to_lowercase()) {
            Ok(mode) => mode,
            Err(_) => {
                output::render_error(&format!(
                    "Invalid mode '{mode}'. Mode must be one of: {}",
                    HeyBuddyMode::VARIANTS.join(", ")
                ));
                return Ok(());
            }
        };
        self.agent
            .update_heybuddy_mode(mode, &self.session_id)
            .await?;
        config.set_heybuddy_mode(mode)?;
        output::heybuddy_mode_message(&format!("HeyBuddy mode set to '{mode}'"));
        Ok(())
    }
    ...
    async fn handle_model(&mut self, options: input::ModelCommandOptions) -> Result<()> {
        ...
        if options.provider.is_none() && options.model.is_none() {
            output::heybuddy_mode_message(&format!(
        ...
        let target_entry = match heybuddy::providers::get_from_registry(target_provider_name).await
        {
            Ok(entry) => entry,
            Err(_) => {
                output::render_error(&format!(
        ...
        let configured_effort = Config::global().get_heybuddy_thinking_effort();
        ...
        let current_context_limit = heybuddy::context_limit::get_context_limit(
            provider.as_ref(),
        ...
        let new_provider = match heybuddy::providers::create(target_provider_name, extensions).await
        {
            Ok(p) => p,
            Err(e) => {
                output::render_error(&format!(
                    "Cannot switch to provider '{}': {}\n\
                         Set credentials via `heybuddy configure` or the appropriate environment variable.\n\
        ...
        let new_context_limit = heybuddy::context_limit::get_context_limit(
            new_provider.as_ref(),
        ...
        let mode = self.agent.heybuddy_mode().await;
        self.agent
            .update_heybuddy_mode(mode, &self.session_id)
            .await?;
    ...
    async fn handle_clear(&mut self) -> Result<()> {
        ...
        if let Err(e) = self
            ...
            .usage(heybuddy_providers::conversation::token_usage::Usage::new(
                Some(0),
                Some(0),
                Some(0),
            ))
    ...
    async fn handle_new(&mut self) -> Result<()> {
        ...
        if let Err(e) = self
            .agent
            .update_heybuddy_mode(self.agent.heybuddy_mode().await, &self.session_id)
            .await
        {
        ...
        if !extension_configs.is_empty() {
            output::heybuddy_mode_message("Restarting extensions for the new session...");
        }
    ...
    async fn prepare_successor_session(&self) -> Result<String> {
        ...
        let new_session_id = create_successor_session(
            session_manager,
            &old_session,
            self.agent.heybuddy_mode().await,
        )
        .await?;
    ...
    async fn handle_list_skills(&mut self) -> Result<()> {
        ...
        use heybuddy::custom_requests::SourceType;
        use heybuddy::skills::list_installed_skills;
    ...
    async fn plan_with_reasoner_model(
        ...
        output::show_thinking();
        let (plan_response, _usage) = heybuddy::session_context::with_session_id(
        ...
        match planner_response_type {
            PlannerResponseType::Plan => {
                ...
                if should_act {
                    ...
                    self.run_mode = RunMode::Normal;
                    // set heybuddy mode: auto if that isn't already the case
                    let config = Config::global();
                    let curr_heybuddy_mode = config.get_heybuddy_mode().unwrap_or_default();
                    if curr_heybuddy_mode != HeyBuddyMode::Auto {
                        config.set_heybuddy_mode(HeyBuddyMode::Auto).unwrap();
                    }
                    ...
                    // Reset run & heybuddy mode
                    if curr_heybuddy_mode != HeyBuddyMode::Auto {
                        config.set_heybuddy_mode(curr_heybuddy_mode)?;
                    }
                } else {
                        ...
                        Some(Ok(AgentEvent::Message(message))) => {
                            ...
                            if let Some((id, security_prompt)) = find_tool_confirmation(&message) {
                                let selected_permission = if interactive {
                                    ...
                                    if heybuddy_mode == HeyBuddyMode::Approve || heybuddy_mode == HeyBuddyMode::SmartApprove {
                                        cancel_token_clone.cancel();
                                        drop(stream);
                                        return Err(anyhow::anyhow!(
                                            "Tool approval required in non-interactive mode with HeyBuddyMode::{heybuddy_mode}. \
                                             This is an invalid configuration — Approve/SmartApprove modes require an \
                                             interactive terminal. Use HeyBuddyMode::Auto for headless sessions."
                                        ));
                                    }
                                    tracing::warn!(
    ...
    async fn refresh_completion_cache(
        ...
        let all_providers = heybuddy::providers::providers().await;
    ...
    pub async fn get_session(&self) -> Result<heybuddy::session::Session> {
        self.agent
            .config
            .session_manager
    ...
    pub async fn display_context_usage(&self) -> Result<()> {
        ...
        let model_config = self
            ...
            .await?;
        let context_limit =
            heybuddy::context_limit::get_context_limit(provider.as_ref(), &model_config.model_name)
                .await?;

        let config = Config::global();
        let show_cost = config
            .get_param::<bool>("HEYBUDDY_CLI_SHOW_COST")
            .unwrap_or(false);

        let provider_name = config
            .get_heybuddy_provider()
...
async fn create_successor_session(
    ...
    old_session: &heybuddy::session::Session,
...
fn prompt_tool_confirmation(security_prompt: &Option<String>) -> Result<Permission> {
    ...
    let prompt = if let Some(security_message) = security_prompt {
        ...
        "HeyBuddy would like to call the above tool, do you allow?".to_string()
    };
                ...
                let formatted = match notification_type {
                    ...
                    Some("response_generated") => {
                        ...
                        let min_priority = config
                            .get_param::<f32>("HEYBUDDY_CLI_MIN_PRIORITY")
                            .ok()
    ...
    for content in &message.content {
        if let MessageContent::ToolRequest(tool_request) = content {
            if let Ok(tool_call) = &tool_request.tool_call {
                tracing::info!(
                    monotonic_counter.heybuddy.tool_calls = 1,
        ...
        if let MessageContent::ToolResponse(tool_response) = content {
            ...
            tracing::info!(
                monotonic_counter.heybuddy.tool_completions = 1,
                tool_name = %tool_name,
...
fn handle_agent_error(e: &anyhow::Error, is_stream_json_mode: bool) {
    ...
    if e.downcast_ref::<heybuddy_providers::errors::ProviderError>()
        .map(|provider_error| {
            matches!(
                provider_error,
                heybuddy_providers::errors::ProviderError::ContextLengthExceeded(_)
            )
        })
        .unwrap_or(false)
    {
...
) -> Result<(Arc<dyn Provider>, heybuddy_providers::model::ModelConfig), anyhow::Error> {
    use heybuddy::providers::create;

    let config = Config::global();

    // Try planner-specific provider first, fall back to default provider
    let provider = if let Ok(provider) = config.get_param::<String>("HEYBUDDY_PLANNER_PROVIDER") {
        provider
    } else {
        println!("WARNING: HEYBUDDY_PLANNER_PROVIDER not found. Using default provider...");
        config
            .get_heybuddy_provider()
            .expect("No provider configured. Run 'heybuddy configure' first")
    };

    // Try planner-specific model first, fall back to default model
    let model = if let Ok(model) = config.get_param::<String>("HEYBUDDY_PLANNER_MODEL") {
        model
    } else {
        println!("WARNING: HEYBUDDY_PLANNER_MODEL not found. Using default model...");
        config
            .get_heybuddy_model()
            .expect("No model configured. Run 'heybuddy configure' first")
    };

    let model_config =
        heybuddy::model_config::model_config_from_user_config(&provider, model.as_str())?;
    let extensions = heybuddy::config::extensions::get_enabled_extensions_with_config(config);
...
fn build_switched_model_config(
    provider_name: &str,
    model_name: &str,
    current_model_config: &heybuddy_providers::model::ModelConfig,
) -> Result<heybuddy_providers::model::ModelConfig> {
    heybuddy::model_config::model_config_from_user_config(provider_name, model_name)
        .map(|config| {
            config
...
mod tests {
    use super::*;
    use heybuddy::agents::extension::Envs;
    ...
    fn planner_history_excludes_turn_context_events() {
        use heybuddy::conversation::message::MessageMetadata;
    ...
    fn test_build_switched_model_config_rebuilds_target_model_settings() {
        let _guard = env_lock::lock_env([
            ("HEYBUDDY_MAX_TOKENS", None::<&str>),
            ("HEYBUDDY_TEMPERATURE", None::<&str>),
            ("HEYBUDDY_CONTEXT_LIMIT", None::<&str>),
            ("HEYBUDDY_TOOLSHIM", None::<&str>),
            ("HEYBUDDY_TOOLSHIM_OLLAMA_MODEL", None::<&str>),
        ]);

        let current_model_config = heybuddy_providers::model::ModelConfig {
        ...
        let expected = heybuddy_providers::model::ModelConfig::new("gpt-5.4")
    ...
    fn test_build_switched_model_config_detects_effort_suffix_change() {
        ...
        let current = heybuddy_providers::model::ModelConfig::new("gpt-5.4-high")
            .with_canonical_limits("openai");
        assert_eq!(current.model_name, "gpt-5.4");
        assert_eq!(
            current.thinking_effort(),
            Some(heybuddy_providers::thinking::ThinkingEffort::High)
        );
    ...
    fn test_split_command_args_windows_paths() {
        assert_eq!(
            heybuddy::utils::split_command_args(r"C:\tools\mcp.exe --arg value").unwrap(),
            vec![r"C:\tools\mcp.exe", "--arg", "value"]
        );
        assert_eq!(
            heybuddy::utils::split_command_args(r#""C:\Program Files\server\mcp.exe" --arg"#)
                .unwrap(),
    ...
    fn test_split_command_args_unmatched_quote() {
        assert!(heybuddy::utils::split_command_args(r#""unmatched"#).is_err());
    }
    ...
    async fn new_session_inherits_provider_model_and_working_dir() {
        ...
        let old = sm
            .create_session(
                ...
                "CLI Session".to_string(),
                heybuddy::session::SessionType::User,
                HeyBuddyMode::Auto,
            )
            .await
            .unwrap();
        ...
        sm.update(&old.id)
            ...
            .model_config(heybuddy_providers::model::ModelConfig::new("test-model"))
            .accumulated_usage(heybuddy_providers::conversation::token_usage::Usage::new(
                Some(100),
                Some(50),
                Some(150),
            ))
        ...
        let mut extension_data = heybuddy::session::ExtensionData::new();
        ...
        let new_id = create_successor_session(&sm, &old, HeyBuddyMode::Chat)
            .await
            .unwrap();
        ...
        assert_eq!(new_session.heybuddy_mode, HeyBuddyMode::Chat);
    ...
    impl Provider for StubProvider {
        ...
        async fn stream(
            &self,
            _model_config: &heybuddy_providers::model::ModelConfig,
            _system: &str,
            _messages: &[Message],
            _tools: &[rmcp::model::Tool],
        ) -> std::result::Result<
            heybuddy::providers::base::MessageStream,
            heybuddy_providers::errors::ProviderError,
        > {
            Ok(heybuddy::providers::base::stream_from_single_message(
                Message::assistant().with_text("stub reply"),
                ProviderUsage::new(
                    "stub".to_string(),
                    heybuddy_providers::conversation::token_usage::Usage::default(),
                ),
            ))
        }
    }
    ...
    async fn session_with_loader(
        ...
        let agent = heybuddy::agents::Agent::with_config(heybuddy::agents::AgentConfig::new(
            Arc::new(session_manager),
            Arc::new(heybuddy::config::PermissionManager::new(
                temp_dir.path().to_path_buf(),
            )),
            None,
            HeyBuddyMode::default(),
            ...
            true,
            heybuddy::agents::HeyBuddyPlatform::HeyBuddyCli,
        ));
        agent
            .update_provider(
                Arc::new(StubProvider),
                heybuddy_providers::model::ModelConfig::new("stub-model"),
                &session.id,
            )
            .await
            .unwrap();
