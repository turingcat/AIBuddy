...
/// Checks HEYBUDDY_PROMPT_EDITOR, then $VISUAL, then $EDITOR.
pub fn resolve_editor_command() -> Option<String> {
    let config = Config::global();
    let config_editor = config.get_heybuddy_prompt_editor().ok().flatten();
...
fn build_template(messages: &[&str], prefill: Option<&str>) -> String {
    let mut content = String::from("# HeyBuddy Prompt Editor\n\n");
...
mod tests {
    ...
    fn test_extract_user_input_with_editor_output() {
        let content = r#"# HeyBuddy Prompt Editor
...
"#;
    ...
    fn test_create_temp_file_with_messages() {
        ...
        assert!(path.exists());
        assert!(path.to_str().unwrap().contains("heybuddy_prompt_"));
        assert!(path.to_str().unwrap().ends_with(".md"));

        let content = fs::read_to_string(path).unwrap();
        assert!(content.contains("# HeyBuddy Prompt Editor"));
    ...
    fn test_create_temp_file_with_prefix_suffix() {
        let temp_file = Builder::new()
            .prefix("heybuddy_test_")
            .suffix(".md")
            .tempfile()
            .unwrap();

        let name = temp_file.path().file_name().unwrap().to_str().unwrap();
        assert!(name.starts_with("heybuddy_test_"));
...
"#;
    ...
    fn test_build_template_no_prefill_no_messages() {
        ...
        assert_eq!(content, "# HeyBuddy Prompt Editor\n\n# Your prompt:\n\n");
    }
    ...
    fn test_editor_uses_random_tempfile_and_preserves_cwd() {
        let script = Builder::new()
            .prefix("heybuddy_editor_test_")
            .tempfile()
            .unwrap();
        let path_report = Builder::new()
            .prefix("heybuddy_editor_path_")
            .tempfile()
            .unwrap();
        let cwd_report = Builder::new()
            .prefix("heybuddy_editor_cwd_")
            .tempfile()
            .unwrap();
        let secret = Builder::new()
            .prefix("heybuddy_editor_secret_")
            .tempfile()
            .unwrap();
        ...
        let predictable_path = Path::new(".heybuddy_prompt_temp.md");
        ...
        fs::write(
            ...
            r#"printf '%s' "$4" > "$1"
pwd > "$2"
printf '# HeyBuddy Prompt Editor\n\n# Your prompt:\n\nupdated prompt\n' > "$4"
ln -sf "$3" .heybuddy_prompt_temp.md
"#,
        )
        .unwrap();
        ...
        assert!(edited_path
            ...
            .unwrap()
            .to_string_lossy()
            .starts_with("heybuddy_prompt_"));
        assert_ne!(edited_path, Path::new(".heybuddy_prompt_temp.md"));
