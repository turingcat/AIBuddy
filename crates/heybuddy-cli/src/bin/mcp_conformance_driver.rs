...
fn main() {
    ...
    let heybuddy =
        std::env::var("HEYBUDDY_BIN").unwrap_or_else(|_| "target/debug/heybuddy".to_string());
    let path_root = tempfile::Builder::new()
        .prefix("heybuddy-mcp-conformance-")
        .tempdir()
        .unwrap_or_else(|err| {
            eprintln!("failed to create temporary HEYBUDDY_PATH_ROOT: {err}");
            std::process::exit(1);
        });
    let mut child = Command::new(&heybuddy)
        .args(["mcp-probe", target, "--script", "-"])
        .env("HEYBUDDY_OAUTH_AUTOMATIC_CALLBACK", "1")
        .env("HEYBUDDY_DISABLE_KEYRING", "1")
        .env("HEYBUDDY_PATH_ROOT", path_root.path())
        .stdin(Stdio::piped())
        .spawn()
        .unwrap_or_else(|err| {
            eprintln!("failed to spawn {heybuddy}: {err}");
            std::process::exit(1);
        });
    ...
    child
        ...
        .expect("write probe script to heybuddy stdin");

    let status = child.wait().expect("wait for heybuddy");
