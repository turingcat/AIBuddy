//! Generate manpages for the heybuddy CLI.
//!
...
//! offline documentation accessible via the `man` command (e.g., `man heybuddy`).
//!
//! When heybuddy is packaged for Linux distributions (deb, rpm, etc.), the generated
...
//!
//! Output: target/man/heybuddy.1, target/man/heybuddy-session.1, etc.

use clap::CommandFactory;
use clap_mangen::Man;
use heybuddy_cli::Cli;
use std::env;
use std::fs;
use std::io::Result;
...
fn generate_see_also(
    ...
    // Always reference the main heybuddy command if we're not it
    if current_name != "heybuddy" {
        references.push("heybuddy".to_string());
    }

    // Reference parent command if exists and not already added
    if let Some(parent) = parent_name {
        if parent != "heybuddy" && !references.contains(&parent.to_string()) {
    ...
    if current_name == "heybuddy" {
        ...
        for subcmd in cmd.get_subcommands() {
            ...
            if subcmd_name != "help" && !subcmd.is_hide_set() {
                let full_name = format!("heybuddy-{}", subcmd_name);
