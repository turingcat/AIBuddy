...
async fn run() -> Result<()> {
    if let Err(e) = heybuddy_cli::logging::setup_logging(None) {
    ...
    #[cfg(feature = "otel")]
    if heybuddy::otel::otlp::is_otlp_initialized() {
        heybuddy::otel::otlp::shutdown_otlp();
    }
...
fn main() -> Result<()> {
    ...
    heybuddy::config::legacy::bootstrap_legacy_environment();

    let handle = std::thread::Builder::new()
        .name("heybuddy-cli-main".to_string())
        ...
        .map_err(|e| anyhow::anyhow!("Failed to spawn heybuddy-cli main thread: {}", e))?;

    handle
        .join()
        .map_err(|_| anyhow::anyhow!("heybuddy-cli main thread panicked"))?
}
