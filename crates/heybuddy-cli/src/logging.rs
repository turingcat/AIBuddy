use anyhow::Result;
use heybuddy::providers::utils::init_heybuddy_request_log;
...
pub fn setup_logging(name: Option<&str>) -> &'static Result<()> {
    INIT.get_or_init(|| {
        ...
        init_heybuddy_request_log()?;
        let config = heybuddy::logging::LoggingConfig {
            component: "cli",
            name,
            extra_directives: &["heybuddy_cli=info"],
            console: false,
            json: true,
        };
        let subscriber = heybuddy::logging::build_logging_subscriber(&config)?;
...
#[cfg(test)]
mod tests {
    use heybuddy::tracing::langfuse_layer;
    use std::env;
    ...
    fn test_log_directory_creation() {
        ...
        let log_dir = heybuddy::logging::prepare_log_directory("cli", true).unwrap();
    ...
    async fn test_default_filter_avoids_debug_by_default() {
        ...
        assert!(heybuddy::logging::build_logging_subscriber(&config).is_ok());
    }
}
