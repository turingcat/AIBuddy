...
/// Request params heybuddy consumes itself: formats that forward unknown params into
/// the payload must skip these, or the provider gets an unrecognized wire parameter.
pub fn is_heybuddy_internal_request_param(key: &str) -> bool {
    matches!(
        key,
        "thinking_effort"
...
mod tests {
    ...
    mod thinking_effort_tests {
        ...
        fn xhigh_suffix_stripped_from_model_name() {
            let _guard = env_lock::lock_env([
                ("HEYBUDDY_THINKING_EFFORT", Some("low")),
                ("HEYBUDDY_MAX_TOKENS", None::<&str>),
                ("HEYBUDDY_TEMPERATURE", None::<&str>),
                ("HEYBUDDY_CONTEXT_LIMIT", None::<&str>),
                ("HEYBUDDY_TOOLSHIM", None::<&str>),
    ...
    mod is_openai_reasoning_model {
        ...
        const ENV_LOCK_KEYS: [(&str, Option<&str>); 5] = [
            ("HEYBUDDY_MAX_TOKENS", None),
            ("HEYBUDDY_TEMPERATURE", None),
            ("HEYBUDDY_CONTEXT_LIMIT", None),
            ("HEYBUDDY_TOOLSHIM", None),
            ("HEYBUDDY_TOOLSHIM_OLLAMA_MODEL", None),
        ];
        ...
        #[test]
        fn heybuddy_prefixed_reasoning_models() {
            let _guard = env_lock::lock_env(ENV_LOCK_KEYS);
            assert!(ModelConfig::new("heybuddy-o3-mini").is_openai_reasoning_model());
            assert!(ModelConfig::new("heybuddy-o4-mini").is_openai_reasoning_model());
            assert!(ModelConfig::new("heybuddy-gpt-5").is_openai_reasoning_model());
        }
        ...
        fn non_reasoning_models() {
            ...
            assert!(!ModelConfig::new("heybuddy-claude-sonnet-4").is_openai_reasoning_model());
