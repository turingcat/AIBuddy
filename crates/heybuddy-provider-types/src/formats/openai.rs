...
use crate::model::{is_heybuddy_internal_request_param, ModelConfig};
use crate::thinking::{
...
pub fn create_request_for_model_with_options(
    ...
    if model_config.model_name.starts_with("o1-mini") {
        return Err(anyhow!(
            "o1-mini model is not currently supported since heybuddy uses tool calling and o1-mini does not support it. Please use o1 or o3 models instead."
        ));
    }
    ...
    // HEYBUDDY_MAX_TOKENS) or a canonical model record has supplied a value.
    ...
    if let Some(params) = &model_config.request_params {
        if let Some(obj) = payload.as_object_mut() {
            for (key, value) in params {
                if !is_heybuddy_internal_request_param(key) && !is_reserved_request_param_key(key) {
...
/// hosted aliases like `databricks-gpt-5.4`, `heybuddy-o3-mini`, or
/// `headless-heybuddy-o3-mini` work without provider-specific normalization.
...
/// Maps HeyBuddy's effort levels to values accepted by xAI Chat Completions.
...
mod tests {
    ...
    fn test_create_request_omits_max_tokens_when_unset() -> anyhow::Result<()> {
        ...
        // lmstudio) have no canonical record and no HEYBUDDY_MAX_TOKENS, so the
...
data: [DONE]"#;
    ...
    fn test_extract_reasoning_effort_for_responses_models() {
        for (model, expected_name, expected_effort) in [
            ...
            ("heybuddy-gpt-5-high", "heybuddy-gpt-5", Some("high")),
            ("gpt-4o", "gpt-4o", None),
        ] {
        ...
        fn turn_context(time: &str, turn_budget: &str) -> String {
            format!(
                "<turn-context>\n\
                 ...
                 <working-directory>/Users/me/code/heybuddy</working-directory>\n\
