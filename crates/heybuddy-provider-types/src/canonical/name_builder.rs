...
pub fn map_provider_name(provider: &str) -> &str {
    match provider {
        // HeyBuddy provider names that differ from models.dev names
        "xai" | "xai_oauth" => "x-ai",
...
mod tests {
    ...
    fn test_map_to_canonical_model() {
        ...
        assert_eq!(
            map_to_canonical_model("databricks", "heybuddy-claude-sonnet-4-5-bedrock", r),
            Some("anthropic/claude-sonnet-4.5".to_string())
        );
        assert_eq!(
        ...
        assert_eq!(
            map_to_canonical_model("databricks", "kheybuddy-gpt-4o", r),
            Some("openai/gpt-4o".to_string())
        );
        assert_eq!(
        ...
        // === OpenAI O-series ===
        assert_eq!(
            map_to_canonical_model("databricks", "heybuddy-o1", r),
            Some("openai/o1".to_string())
        );
        assert_eq!(
            map_to_canonical_model("databricks", "kheybuddy-o3", r),
            Some("openai/o3".to_string())
        );
        assert_eq!(
            map_to_canonical_model("databricks", "headless-heybuddy-o3-mini", r),
        ...
        assert_eq!(
            map_to_canonical_model("databricks", "kheybuddy-grok-4.3", r),
            Some("x-ai/grok-4.3".to_string())
        );
        ...
        assert_eq!(
            map_to_canonical_model("databricks", "heybuddy-command-r-08-2024", r),
