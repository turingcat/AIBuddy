...
1. **Builds canonical models** - Fetches from OpenRouter API and updates the registry
   - Writes to: `crates/heybuddy-provider-types/src/canonical/data/canonical_models.json`
2. **Checks model mappings** (unless `--no-check` is passed) - Tests provider mappings and tracks changes over time
   ...
   - Writes to: `crates/heybuddy-provider-types/src/canonical/data/canonical_mapping_report.json`

The script is currently built from `crates/heybuddy/src/bin/build_canonical_models.rs` and writes into this crate's `src/canonical/data` directory.
