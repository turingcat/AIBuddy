# heybuddy-provider-types

The provider contract and the conversation types that flow through it. This is the
crate to depend on if you want to implement a provider, or to work with heybuddy
messages without pulling in the whole agent.

Provider implementations live in [`heybuddy-providers`](../heybuddy-providers), which
...
| `retry` | `RetryConfig` |
| `permission`, `heybuddy_mode` | Tool-approval modes |
