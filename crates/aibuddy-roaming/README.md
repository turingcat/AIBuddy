...
Peer-to-peer transport for aibuddy agents, built on
[iroh](https://iroh.computer) (QUIC, using iroh's public relays for NAT
traversal).

It lets a aibuddy agent accept connections from a remote ACP client (another
aibuddy, or any other ACP client) that drives it, and lets a client
...
This crate is a **standalone library** with no dependency on `aibuddy` core (so
...
(`cargo run -p aibuddy-roaming --example echo_roundtrip`). The code that bridges
the transport to aibuddy's agent machinery lives in `aibuddy-cli` behind an optional
...
The host runs aibuddy's real ACP server; the connecting side is an ACP client.
...
- You **swap cards** and each side **accepts** the other's key. A connection
  ...
  gets aibuddy's full ACP surface. A leaked card lets no one in; there is no bearer
  token that works by possession.

## Concepts

- **`ConnectionCard`** — the shareable, non-secret identity + reachability string
  (`aibuddy+roam://…`). Encodes public key + relay URLs; exposes `fingerprint()`.
...
An iroh bidirectional stream is the byte transport for aibuddy's existing
transport-agnostic ACP `serve` / `ByteStreams` seam, so hosting reuses the ACP
server and the client reuses the ACP client.

## CLI

Exposed via `aibuddy roam` (in `aibuddy-cli`, feature `roaming`):
...
Build both with the roaming feature (`cargo build -p aibuddy-cli --features
roaming`) — no shared network, VPN, or port-forwarding needed; the public n0
relays bridge them. On **each** machine run `aibuddy roam id` and send the printed
`aibuddy+roam://…` card to the other out of band (paste it in chat, etc.).

On **machine A** (the host): `aibuddy roam peers accept '<B's card>'` then
`aibuddy roam share` (optionally `--cwd <dir>`; it defaults to where `share`
started, and the connector's own path is always ignored). On **machine B**:
`aibuddy roam peers add '<A's card>' boxA`, then either drive A interactively with
`aibuddy roam connect boxA` (a prompt that runs on A's agent — its tools, files,
shell), hand it a one-shot task with `aibuddy roam delegate boxA "what is 2+2?"`,
or `aibuddy roam delegate boxA --list-sessions` / `--session <id> "<task>"` to
...
`aibuddy roam connections` shows who connected. If session creation hangs on
macOS, prefix with `AIBUDDY_DISABLE_KEYRING=1`.
...
aibuddy's real `serve`). `connect` is a thin ACP client UI — not a provider
...
**Acceptance is all-or-nothing.** An accepted peer gets aibuddy's full ACP
...
(`roam-delegate`) documents how to call `aibuddy roam delegate <peer> "<task>"` via
...
`aibuddy roam share` agent — iroh compiled to wasm runs *inside the browser tab*,
...
It is fully decoupled from this crate: the `aibuddy-roaming-web` wasm crate there
...
wire format here, update aibuddy-mobile in the same change — a drift will not fail
