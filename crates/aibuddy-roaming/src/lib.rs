//! Peer-to-peer roaming transport for aibuddy agents.
//!
//! This crate lets a aibuddy agent expose itself over the internet via
...
//!   no bearer token. An accepted peer gets aibuddy's full ACP surface.
//! * [`RoamingNode`] — owns the iroh endpoint + router, hosts agents over the
//!   `aibuddy-acp/1` ALPN, and dials remote agents.
//!
//! The crate deliberately knows nothing about aibuddy's agent internals: hosting
//! is driven through the [`AcpStreamServer`] trait, which the integration layer
//! implements by calling aibuddy's generic `acp::server::serve`. This keeps the
//! heavy iroh dependency out of the `aibuddy` core crate entirely.
