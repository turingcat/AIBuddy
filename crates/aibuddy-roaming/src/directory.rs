...
//! remote agent. This gives `aibuddy roam list`-style visibility without any
...
/// process (e.g. `aibuddy roam list`) can read what a running `share` has seen.
...
impl Directory {
    ...
    /// flushed straight back to disk, making `aibuddy roam connections` stop
