//! Minimal end-to-end example of the `aibuddy-roaming` library API.
//!
...
//! authorized stream — without any dependency on aibuddy's agent internals. The
//! "agent" here is a trivial echo server plugged in via the [`AcpStreamServer`]
//! trait; a real consumer would call aibuddy's ACP `serve` instead (see
//! `aibuddy-cli`'s bridge).
//!
//! Run it:
//!
//! ```bash
//! cargo run -p aibuddy-roaming --example echo_roundtrip
//! ```
//!
//! It runs both ends in one process over loopback (relays disabled), so it
//! needs no network. For a real two-machine test, use the `aibuddy roam` CLI.
