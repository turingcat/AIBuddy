# aibuddy-roaming trust-model audit
...
registers the `aibuddy-acp/1` protocol handler — and the only callers are
`aibuddy roam share` and `aibuddy serve --roam`. Pure clients (the browser
...
An accepted peer gets aibuddy's **full ACP surface** with a fresh agent per
connection (`aibuddy-cli/src/commands/roam_full_bridge.rs`), backed by the
