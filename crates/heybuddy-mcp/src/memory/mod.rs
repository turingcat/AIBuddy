...
impl MemoryServer {
    pub fn new() -> Self {
        let instructions = formatdoc! {r#"
             ...
             Storage:
             - Local: .heybuddy/memory/ (project-specific)
             - Global: ~/.config/heybuddy/memory/ (user-wide)
        ...
        let global_memory_dir = choose_app_strategy(crate::APP_STRATEGY.clone())
            ...
            .unwrap_or_else(|_| PathBuf::from(".config/heybuddy/memory"));
    ...
    fn get_memory_file(
        ...
        let base_dir = if is_global {
            ...
            local_base.join(".heybuddy").join("memory")
        };
...
mod tests {
    ...
    fn test_lazy_directory_creation() {
        ...
        let local_memory_dir = working_dir.join(".heybuddy").join("memory");
    ...
    fn test_memory_operations_reject_escape_capable_categories() {
        ...
        assert!(!working_dir.join(".heybuddy").exists());
    }
    ...
    fn test_memory_category_allows_safe_filename_characters() {
        ...
        assert!(working_dir
            .join(".heybuddy/memory/project notes_2026.txt")
            .is_file());
    }
    ...
    fn test_retrieve_all_skips_invalid_legacy_categories() {
        let temp_dir = tempdir().unwrap();
        let working_dir = temp_dir.path().join("working");
        let memory_dir = working_dir.join(".heybuddy/memory");
