...
Instruct users to start a heybuddy session with their extension.

**Important**: You cannot start the heybuddy session for them, as it is interactive. You will have to let them
know to start it in a terminal. Make sure you include instructions on how to set up the environment

```bash
# Python example
heybuddy session --with-extension "python server.py"

# TypeScript example
heybuddy session --with-extension "node server.js"

# Kotlin example
heybuddy session --with-extension "java -jar build/libs/extension.jar"
```
...
Note:
You can run a feedback loop using a headless heybuddy session, however if the process hangs you get into a stuck action.
Ask the user if they want you to do that, and let them know they will manually need to kill any stuck processes.

```bash
# Python example
heybuddy run --with-extension "python server.py" --text "EXAMPLE PROMPT HERE"

# TypeScript example
heybuddy run --with-extension "node server.js" --text "EXAMPLE PROMPT HERE"

# Kotlin example
heybuddy run --with-extension "java -jar build/libs/extension.jar" --text "EXAMPLE PROMPT HERE"
```
...
- For tools, they should ask heybuddy to use the tool directly
- For resources, they should ask heybuddy to access the relevant data
...
1. First, check if there are any immediate error messages in the heybuddy session
