...
async fn main() -> Result<()> {
    ...
    // Create providers
    let providers: Vec<(
        Arc<dyn aibuddy::providers::base::Provider>,
        aibuddy_providers::model::ModelConfig,
    )> = vec![
        (
            create_with_named_model("databricks", Vec::new()).await?,
            aibuddy::model_config::model_config_from_user_config(
                "databricks",
        ...
        (
            ...
            aibuddy::model_config::model_config_from_user_config("openai", OPEN_AI_DEFAULT_MODEL)?,
        ),
        (
    ...
    for (provider, model_config) in providers {
        ...
        let image_data = fs::read("crates/aibuddy/examples/test_assets/test_image.png")?;
