...
use aibuddy::agents::state_machine::{
    yielded_with, AIBuddyEffect, Emitter, Inference, InferenceInput, Operation, OperationResult,
    StateMachine, Step,
};
...
use aibuddy::session::session_manager::token_state_from_session_and_totals;
use aibuddy::session::{Session, SessionManager, SessionType};
...
#[async_trait]
impl Operation<Session, AIBuddyEffect> for PromptPart {
...
#[async_trait]
impl Operation<Session, AIBuddyEffect> for TestInference {
    fn name(&self) -> &'static str {
        "test_inference"
    }
}
...
#[async_trait]
impl Inference<Session, AIBuddyEffect> for TestInference {
    ...
    async fn infer(
        ...
        let message = emit
            ...
            .await;
        yielded_with([
            AIBuddyEffect::from(message),
            AIBuddyEffect::RecordUsage(ProviderUsage::new(
...
async fn custom_pipeline_supports_step_apply_run_and_usage() -> Result<()> {
    let _env = aibuddy_test_support::otel::clear_otel_env(&[(
    ...
    assert!(matches!(
        ...
        Some(AIBuddyEffect::Conversation(
            aibuddy::agents::state_machine::ConversationEffect::AppendMessage(message)
