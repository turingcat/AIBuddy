...
use aibuddy::conversation::message::{Message, MessageContent, ToolRequest};
use aibuddy::security::adversary_inspector::AdversaryInspector;
use aibuddy::tool_inspection::ToolInspector;
...
async fn test_adversary_disabled_without_config_file() {
    ...
    let inspector = AdversaryInspector::with_config_dir(
        provider,
        Arc::new(aibuddy::session::SessionManager::new(
...
async fn test_adversary_enabled_default_tools() {
    ...
    assert!(matches!(
        results[0].action,
        aibuddy::tool_inspection::InspectionAction::Allow
    ));
