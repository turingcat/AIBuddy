...
use aibuddy::config::declarative_providers::load_provider;
use aibuddy::config::paths::Paths;
use aibuddy::config::{Config, ConfigError, DeclarativeProviderConfig};
use aibuddy_test_support::EnforceSessionId;
...
fn acp_catalog_and_custom_provider_methods_use_core_provider_store() {
    let _env = env_lock::lock_env([
        ("AIBUDDY_DISABLE_KEYRING", Some("1")),
        ("XAI_API_KEY", None),
        ("XAI_HOST", None),
    ...
    run_test(async move {
        ...
        assert!(
            ...
            "custom provider should be saved in AIBuddy's declarative provider store"
        );
        ...
        assert_eq!(
            Config::global()
                ...
                .unwrap(),
            "created-custom-key",
            "custom provider create should write through AIBuddy's config store"
        );
        assert!(
        ...
        assert_eq!(
            Config::global()
                ...
                .unwrap(),
            "updated-custom-key",
            "custom provider update should write through AIBuddy's config store"
        );
        ...
        for valid_id in ["custom_openai", "openai-compat", "a1"] {
            assert!(
                aibuddy::config::declarative_providers::validate_provider_id(valid_id).is_ok(),
