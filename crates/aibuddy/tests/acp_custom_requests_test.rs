...
use aibuddy::acp::server::AcpProviderFactory;
use aibuddy::providers::base::{MessageStream, Provider};
use aibuddy_providers::errors::ProviderError;
use aibuddy_providers::model::ModelConfig;
use aibuddy_test_support::{EnforceSessionId, IgnoreSessionId, McpFixture, FAKE_CODE};
...
fn write_acp_global_config(contents: &str) -> PathBuf {
    std::env::set_var("AIBUDDY_PATH_ROOT", ACP_CONFIG_ROOT.path());
    std::env::set_var("AIBUDDY_DISABLE_KEYRING", "1");
    let config_dir = aibuddy::config::paths::Paths::config_dir();
    std::fs::create_dir_all(&config_dir).unwrap();
    let mut contents = contents.to_string();
    if !contents.contains("AIBUDDY_DISABLE_KEYRING") {
        contents.push_str("AIBUDDY_DISABLE_KEYRING: true\n");
    }
    std::fs::write(
        config_dir.join(aibuddy::config::base::CONFIG_YAML_NAME),
        contents,
    )
    .unwrap();
    config_dir
...
impl Provider for MockProvider {
    ...
    async fn stream(
        ...
        _system: &str,
        _messages: &[aibuddy::conversation::message::Message],
...
fn steer_chunk_message_ids(updates: &[SessionUpdate]) -> Vec<String> {
    updates
        ...
        .filter_map(|update| {
            ...
            let aibuddy = chunk.meta.as_ref()?.get("aibuddy")?;
            aibuddy.get("steer")?.as_bool().filter(|b| *b)?;
            aibuddy.get("messageId")?.as_str().map(ToString::to_string)
        })
        .collect()
}
...
fn test_custom_get_extensions() {
    ...
    run_test(async move {
        ...
        let add_result = send_custom(
            conn.cx(),
            "_aibuddy/unstable/config/extensions/add",
            serde_json::json!({
                "enabled": true,
                "extension": {
                    "type": "mcp",
        ...
        let stored_inline_token = aibuddy::config::Config::global()
...
fn test_steer_session_adds_input_to_active_prompt() {
    ...
    run_test(async move {
        ...
        assert!(
            ...
            "expected a chunk marked _meta.aibuddy.steer with the steer text, got: {steer_chunks:?}"
        );
...
fn test_custom_list_builtin_skill_sources() {
    ...
    run_test(async move {
        ...
        let builtin = sources
            .iter()
            .find(|source| source.get("name") == Some(&serde_json::json!("aibuddy-doc-guide")))
            .expect("expected aibuddy-doc-guide builtin skill");
        ...
        assert_eq!(
            builtin.get("path"),
            Some(&serde_json::json!("builtin://skills/aibuddy-doc-guide"))
        );
    });
}
...
fn test_custom_preferences_read_save() {
    let config_dir = write_acp_global_config(
        "AIBUDDY_MODEL: gpt-4o\nAIBUDDY_PROVIDER: openai\nAIBUDDY_AUTO_COMPACT_THRESHOLD: 0.7\nAIBUDDY_THINKING_EFFORT: high\nVOICE_AUTO_SUBMIT_PHRASES: send it\n",
    );
    ...
    run_test(async move {
        ...
        assert_eq!(
            ...
            Some(&serde_json::json!([
                ...
                { "key": "aibuddyThinkingEffort", "value": "high" },
        ...
        send_custom(
            conn.cx(),
            "_aibuddy/unstable/preferences/save",
            serde_json::json!({
                "values": [
                    { "key": "aibuddyThinkingEffort", "value": "disabled" },
        ...
        let response = send_custom(
            ...
            "_aibuddy/unstable/preferences/read",
            serde_json::json!({
                "keys": ["aibuddyThinkingEffort", "voiceDictationProvider", "voiceDictationPreferredMic"],
            }),
        )
        .await
        ...
        assert_eq!(
            response.get("values"),
            Some(&serde_json::json!([
                { "key": "aibuddyThinkingEffort", "value": "off" },
                { "key": "voiceDictationProvider", "value": "__disabled__" },
                { "key": "voiceDictationPreferredMic", "value": "mic-1" },
            ]))
        );
...
fn test_custom_preferences_save_rejects_invalid_values() {
    ...
    run_test(async {
        ...
        let invalid_payloads = [
            ...
            serde_json::json!({
                "values": [{ "key": "aibuddyThinkingEffort", "value": "bogus" }],
            }),
            serde_json::json!({
                "values": [{ "key": "aibuddyThinkingEffort", "value": ["high"] }],
            }),
            serde_json::json!({
        ...
        for payload in invalid_payloads {
            let result =
                send_custom(conn.cx(), "_aibuddy/unstable/preferences/save", payload).await;
...
fn test_custom_defaults_read() {
    let config_dir = write_acp_global_config(
        "AIBUDDY_MODEL: claude-3-5-haiku-latest\nAIBUDDY_PROVIDER: anthropic\n",
    );
...
fn test_raw_config_and_secret_methods_are_removed() {
    ...
    run_test(async {
        ...
        for method in [
            "_aibuddy/config/read",
            "_aibuddy/config/upsert",
            "_aibuddy/config/remove",
            "_aibuddy/secret/check",
            "_aibuddy/secret/upsert",
            "_aibuddy/secret/remove",
        ] {
...
fn test_provider_switching_updates_session_state() {
    ...
    run_test(async {
        ...
        conn.set_config_option(&session_id, "provider", "aibuddy")
            .await
            .expect("provider reset to aibuddy should succeed");
    });
}
                ...
                Box::pin(async move {
                    Ok(Arc::new(MockProvider {
                        ...
                        supported_models: Ok(vec![
                            "aibuddy-claude-opus-4-8".to_string(),
