use aibuddy::acp::ACP_CURRENT_MODEL;
use aibuddy::agents::{
    AIBuddyPlatform, Agent, AgentConfig, AgentEvent, PromptManager, SessionConfig,
};
...
use aibuddy::providers::anthropic::ANTHROPIC_DEFAULT_MODEL;
use aibuddy::providers::azure::AZURE_DEFAULT_MODEL;
...
use aibuddy::providers::openai::OPEN_AI_DEFAULT_MODEL;
#[cfg(feature = "aws-providers")]
use aibuddy::providers::sagemaker_tgi::SAGEMAKER_TGI_DEFAULT_MODEL;
use aibuddy::providers::snowflake::SNOWFLAKE_DEFAULT_MODEL;
use aibuddy::providers::xai::XAI_DEFAULT_MODEL;
...
use aibuddy_providers::thinking::ThinkingEffortSupport;
use aibuddy_test_support::{
...
impl ProviderFixture {
    async fn setup(config: &ProviderTestConfig, mode: AIBuddyMode) -> Result<Self> {
        let mut env_vars: Vec<(&'static str, Option<&str>)> =
            vec![("AIBUDDY_MODE", Some(<&str>::from(mode)))];
    ...
    async fn tool_roundtrip(
        &self,
        prompt: &str,
        model_config: Option<aibuddy_providers::model::ModelConfig>,
    ) -> Result<Message> {
        let tools = self
            .agent
            .extension_manager
        ...
        let (response1, _) = aibuddy::session_context::with_session_id(
        ...
        let ctx = aibuddy::agents::ToolCallContext::new(
        ...
        let (response2, _) = aibuddy::session_context::with_session_id(
            Some(self.session_id.clone()),
            self.provider.complete(
                &model_config,
                &system,
    ...
    async fn test_basic_response(&self) -> Result<()> {
        ...
        let (response, _) = aibuddy::session_context::with_session_id(
            Some(self.session_id.clone()),
            self.provider.complete(
                &model_config,
    ...
    async fn test_context_length_exceeded_error(&self) -> Result<()> {
        ...
        let result = aibuddy::session_context::with_session_id(
            Some(self.session_id.clone()),
            self.provider.complete(
                &model_config,
                "You are a helpful assistant.",
    ...
    async fn test_image_content_support(&self) -> Result<()> {
        let image_config = self.image_model.as_ref().map(|model| {
            aibuddy_providers::model::ModelConfig::new(model).with_canonical_limits(&self.name)
        });
        let response = self
            .tool_roundtrip(
        ...
        assert!(
            text.contains("hello aibuddy") || text.contains("test image"),
            "{text}"
        );
    ...
    async fn test_model_switch(&self) -> Result<()> {
        ...
        let alt_config =
            aibuddy_providers::model::ModelConfig::new(alt).with_canonical_limits(&self.name);
    ...
    async fn test_mode_update(&self) -> Result<()> {
        ...
        self.agent
            .update_aibuddy_mode(AIBuddyMode::Approve, &self.session_id)
            .await?;
...
async fn test_provider(config: ProviderTestConfig) -> Result<()> {
    ...
    let run_test = |mode: AIBuddyMode| ProviderFixture::setup(&config, mode);

    if run_test(AIBuddyMode::Auto).await.is_err() {
    ...
    let result: Result<()> = async {
        ...
        if config.test_mcp_tools {
            run_test(AIBuddyMode::Auto).await?.test_tool_usage().await?;
            run_test(AIBuddyMode::Auto)
                .await?
        ...
        if config.test_permissions {
            ...
            if config.test_smart_approve {
                run_test(AIBuddyMode::SmartApprove)
                    .await?
