...
use aibuddy::conversation::message::{Message, MessageMetadata};
use aibuddy::custom_requests::{
    GetSessionInfoRequest, GetSessionInfoResponse, UpdateSessionProjectRequest,
};
use aibuddy::recipe::{Recipe, Settings};
use aibuddy::recipe_deeplink;
...
fn include_last_message_snippet_meta(
    ...
    aibuddy.insert("includeLastMessageSnippet".to_string(), value);
...
fn test_update_session_project_rejects_unknown_persisted_session_type() {
    run_test(async {
        ...
        let db_path = data_root
            .path()
            .join(aibuddy::session::session_manager::SESSIONS_FOLDER)
            .join(aibuddy::session::session_manager::DB_NAME);
...
fn test_legacy_client_receives_legacy_custom_notification_namespace() {
    run_test(async {
        ...
        let (transport, _handle, _permission_manager) = spawn_acp_server_in_process(
            ...
            AIBuddyMode::default(),
            None,
            aibuddy_test_support::TEST_MODEL,
            true,
        )
        .await;
    ...
    run_test(async {
        ...
        let recipe = Recipe::builder()
            ...
            .settings(Settings {
                ...
                aibuddy_model: Some(recipe_model.to_string()),
                temperature: None,
                max_turns: None,
            })
            .build()
...
fn test_new_session_prefers_recipe_title_over_meta_session_title() {
    run_test(async {
        ...
        // aibuddy's own recipe-title naming path still applies.
        assert_eq!(
            session_title(&conn, &session_id).await,
