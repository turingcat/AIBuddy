...
use aibuddy::config::base::CONFIG_YAML_NAME;
use aibuddy::config::AIBuddyMode;
use aibuddy::session::{EnabledExtensionsState, SessionManager};
use aibuddy_test_support::{McpFixture, FAKE_CODE, TEST_IMAGE_B64, TEST_MODEL};
use fixtures::{
...
pub async fn run_load_mode<C: Connection>() {
    ...
    assert_eq!(
        ...
        SessionModeId::new(<&str>::from(AIBuddyMode::default()))
    );
    let session_id = session.session_id().0.to_string();
    conn.set_mode(&session_id, <&str>::from(AIBuddyMode::Approve))
        .await
        .unwrap();
    ...
    assert_eq!(
        modes.unwrap().current_mode_id,
        SessionModeId::new(<&str>::from(AIBuddyMode::Approve))
    );
...
pub async fn run_load_session_replays_image_attachment<C: Connection>() {
    ...
    let output = session
        ...
        .await
        .unwrap();
    assert!(output.text.contains("Hello AIBuddy!"));
...
async fn run_mode_set_impl<C: Connection>(via: SetModeVia) {
    ...
    let approve = <&str>::from(AIBuddyMode::Approve);
    match via {
...
pub async fn run_new_session_uses_current_config_mode<C: Connection>() {
    ...
    let config_path = temp_dir
        .path()
        .join(aibuddy::config::base::CONFIG_YAML_NAME);
    fs::write(
        &config_path,
        format!("AIBUDDY_MODEL: {TEST_MODEL}\nAIBUDDY_PROVIDER: openai\nAIBUDDY_MODE: approve\n"),
    )
    .unwrap();
    ...
    let global_config_path =
        aibuddy::config::paths::Paths::config_dir().join(aibuddy::config::base::CONFIG_YAML_NAME);
    fs::write(
        &global_config_path,
        format!("AIBUDDY_MODEL: {TEST_MODEL}\nAIBUDDY_PROVIDER: openai\nAIBUDDY_MODE: auto\n"),
    )
...
pub async fn run_prompt_basic<C: Connection>() {
    ...
    assert_eq!(output.text, "2");
    let updates = session.session_updates();
    let (standard_message_id, aibuddy_message_id) = updates
        .iter()
        .find_map(|update| {
            ...
            let aibuddy_message_id = chunk
                ...
                .as_str()?
                .to_string();
            Some((standard_message_id, aibuddy_message_id))
        })
        .expect("expected live agent message chunk with standard and aibuddy message IDs");
    assert!(!standard_message_id.is_empty());
    assert_eq!(standard_message_id, aibuddy_message_id);
    assert_notifications(
        &fixtures::to_notifications(&updates),
        &[Notification::AgentMessage],
    );
...
pub async fn run_prompt_image<C: Connection>() {
    ...
    let output = session
        ...
        .await
        .unwrap();
    assert_eq!(output.text, "Hello AIBuddy!\nThis is a test image.");
    assert_notifications(
        &session.notifications(),
        &[
            Notification::ToolCall,
