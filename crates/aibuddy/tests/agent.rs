...
use aibuddy::agents::{AIBuddyPlatform, Agent, AgentEvent};
use aibuddy::config::extensions::{set_extension, ExtensionEntry};
use anyhow::Result;
...
mod tests {
    ...
    mod schedule_tool_tests {
        use super::*;
        use aibuddy::agents::platform_extensions::scheduler::{
            EXTENSION_NAME as SCHEDULER_EXTENSION_NAME, MANAGE_SCHEDULE_TOOL_NAME_COMPLETE,
        };
        use aibuddy::agents::ExtensionConfig;
        use aibuddy::agents::{AgentConfig, ScheduleTool};
        use aibuddy::config::permission::PermissionManager;
        use aibuddy::config::AIBuddyMode;
        use aibuddy::scheduler::{ScheduledJob, SchedulerError, ValidatedScheduleRecipe};
        use aibuddy::scheduler_trait::SchedulerTrait;
        use aibuddy::session::{Session, SessionManager};
        ...
        async fn test_schedule_management_tool_list() {
            ...
            assert!(tool
                ...
                .clone()
                .unwrap_or_default()
                .contains("Manage aibuddy's internal scheduled recipe execution"));
        }
    ...
    mod retry_tests {
        use super::*;
        use aibuddy::agents::types::{RetryConfig, SuccessCheck};

        #[tokio::test]
        async fn test_retry_success_check_execution() -> Result<()> {
            use aibuddy::agents::retry::execute_success_checks;
    ...
    mod max_turns_tests {
        use super::*;
        use aibuddy::agents::SessionConfig;
        use aibuddy::config::AIBuddyMode;
        use aibuddy::conversation::message::{Message, MessageContent};
        use aibuddy::providers::base::{
        ...
        impl aibuddy::providers::base::ProviderDescriptor for MockToolProvider {
        ...
        impl ProviderDef for MockToolProvider {
            ...
            fn from_env(
                _extensions: Vec<aibuddy::config::ExtensionConfig>,
                _tls_config: Option<aibuddy::providers::api_client::TlsConfig>,
            ...
            while let Some(response_result) = reply_stream.next().await {
                match response_result {
                    Ok(AgentEvent::Message(response)) => {
                        ...
                        {
                            if let aibuddy::conversation::message::ActionRequiredData::ToolConfirmation { id, .. } = &action.data {
                                agent
                                    .submit_tool_confirmation(
                                        &session_id,
                                        id,
                                        aibuddy::permission::Permission::AllowOnce,
                                    )
                                    .await?;
                            }
                        }
    ...
    mod unparseable_tool_call_tests {
        use super::*;
        use aibuddy::agents::{AgentConfig, SessionConfig};
        use aibuddy::config::permission::PermissionManager;
        use aibuddy::config::AIBuddyMode;
        use aibuddy::conversation::message::{Message, MessageContent};
        use aibuddy::providers::base::{
        ...
        impl aibuddy::providers::base::ProviderDescriptor for UnparseableToolProvider {
            fn metadata() -> ProviderMetadata {
                ProviderMetadata {
    ...
    mod tool_pair_summarization_tests {
        use super::*;
        use aibuddy::agents::{AgentConfig, SessionConfig};
        use aibuddy::config::base::Config;
        use aibuddy::config::permission::PermissionManager;
        use aibuddy::config::AIBuddyMode;
        ...
        use aibuddy::session::{SessionManager, SessionType};
        use aibuddy_providers::conversation::token_usage::{ProviderUsage, Usage};
        use aibuddy_providers::errors::ProviderError;
        use aibuddy_providers::model::ModelConfig;
        use async_trait::async_trait;
        ...
        impl aibuddy::providers::base::ProviderDescriptor for SummarizationTestProvider {
            fn metadata() -> ProviderMetadata {
                ProviderMetadata {
        ...
        async fn test_batch_summarization_preserves_all_summaries() -> Result<()> {
            ...
            Config::global()
                .set_param("AIBUDDY_TOOL_CALL_CUTOFF", 2)
                .unwrap();
            ...
            Config::global().delete("AIBUDDY_TOOL_CALL_CUTOFF").unwrap();
    ...
    mod extension_manager_tests {
        use super::*;
        use aibuddy::agents::extension::ExtensionConfig;
        use aibuddy::agents::platform_extensions::{
            MANAGE_EXTENSIONS_TOOL_NAME, SEARCH_AVAILABLE_EXTENSIONS_TOOL_NAME,
        };
        use aibuddy::agents::AgentConfig;
        use aibuddy::config::permission::PermissionManager;
        use aibuddy::config::AIBuddyMode;
        use aibuddy::session::SessionManager;
        ...
        async fn setup_agent_with_extension_manager() -> (Agent, String) {
            ...
            let todo_extension_entry = ExtensionEntry {
                ...
                config: ExtensionConfig::Platform {
                    name: "todo".to_string(),
                    description:
                        "Enable a todo list for aibuddy so it can keep track of what it is doing"
                            .to_string(),
    ...
    mod streaming_persistence_tests {
        ...
        use aibuddy::config::AIBuddyMode;
        use aibuddy::conversation::message::Message;
        use aibuddy::providers::base::{MessageStream, Provider, ProviderDef, ProviderMetadata};
        use aibuddy::session::session_manager::SessionType;
        use aibuddy::session::SessionManager;
        use aibuddy_providers::conversation::token_usage::{ProviderUsage, Usage};
        use aibuddy_providers::errors::ProviderError;
        ...
        impl aibuddy::providers::base::ProviderDescriptor for MultiStepProvider {
            fn metadata() -> ProviderMetadata {
                ProviderMetadata {
    ...
    mod thinking_preservation_tests {
        ...
        impl aibuddy::providers::base::ProviderDescriptor for ThinkingStreamProvider {
            fn metadata() -> ProviderMetadata {
                ProviderMetadata {
        ...
        fn assert_formatter_adds_reasoning_to_tool_calls(messages: &[Message], provider: &str) {
            use aibuddy_providers::formats::openai::{
                format_messages_with_options, OpenAiFormatOptions,
            };
            use aibuddy_providers::images::ImageFormat;
        ...
        impl aibuddy::providers::base::ProviderDescriptor for CombinedThinkingToolProvider {
            fn metadata() -> ProviderMetadata {
                ProviderMetadata {
        ...
        impl aibuddy::providers::base::ProviderDescriptor for MultiToolThinkingProvider {
            fn metadata() -> ProviderMetadata {
                ProviderMetadata {
        ...
        async fn test_signed_thinking_not_duplicated_for_anthropic() -> Result<()> {
            use aibuddy_providers::formats::anthropic::format_messages as anthropic_format;
    ...
    mod goal_checking_tests {
        ...
        impl aibuddy::providers::base::ProviderDescriptor for GoalTextProvider {
            fn metadata() -> ProviderMetadata {
                ProviderMetadata {
    ...
    mod cumulative_token_tests {
        ...
        use aibuddy::config::AIBuddyMode;
        use aibuddy::conversation::message::Message;
        use aibuddy::providers::base::{stream_from_single_message, MessageStream, Provider};
        use aibuddy::session::session_manager::SessionType;
        use aibuddy::session::SessionManager;
        use aibuddy_providers::conversation::token_usage::{ProviderUsage, Usage};
        use aibuddy_providers::errors::ProviderError;
    ...
    mod add_extensions_bulk_tests {
        ...
        use aibuddy::agents::{AgentConfig, ExtensionConfig};
        use aibuddy::config::permission::PermissionManager;
        use aibuddy::config::AIBuddyMode;
        use aibuddy::session::session_manager::SessionType;
        use aibuddy::session::{
        ...
        fn unloadable_stdio_extension(name: &str) -> ExtensionConfig {
            ExtensionConfig::Stdio {
                ...
                cmd: "aibuddy-test-definitely-missing-binary".to_string(),
                args: vec![],
                envs: Envs::default(),
                env_keys: vec![],
                timeout: Some(1),
    ...
    mod audience_tool_result_tests {
        use super::*;
        use aibuddy::agents::{AgentConfig, SessionConfig};
        use aibuddy::config::{AIBuddyMode, ExtensionConfig, PermissionManager};
        use aibuddy::conversation::message::{Message, MessageContent};
        use aibuddy::providers::base::{stream_from_single_message, MessageStream, Provider};
        use aibuddy::session::{SessionManager, SessionType};
        use aibuddy_providers::conversation::token_usage::{ProviderUsage, Usage};
        use aibuddy_providers::errors::ProviderError;
        use aibuddy_providers::model::ModelConfig;
        use aibuddy_test_support::McpFixture;
        use async_trait::async_trait;
        use rmcp::model::{CallToolRequestParams, Tool};
        use std::path::PathBuf;
        use std::sync::atomic::{AtomicUsize, Ordering};
    ...
    mod empty_turn_tests {
        use super::*;
        use aibuddy::agents::final_output_tool::FINAL_OUTPUT_TOOL_NAME;
        use aibuddy::agents::{AIBuddyPlatform, AgentConfig, AgentEvent, SessionConfig};
        use aibuddy::config::permission::PermissionManager;
        use aibuddy::config::AIBuddyMode;
        use aibuddy::conversation::message::{Message, MessageContent};
        use aibuddy::conversation::Conversation;
        use aibuddy::providers::base::{
            stream_from_single_message, MessageStream, Provider, ProviderDef, ProviderMetadata,
        };
        use aibuddy::session::session_manager::SessionType;
        ...
        impl aibuddy::providers::base::ProviderDescriptor for AssistantOnlyProvider {
            fn metadata() -> ProviderMetadata {
                ProviderMetadata {
        ...
        impl aibuddy::providers::base::ProviderDescriptor for EmptyThenTextProvider {
            fn metadata() -> ProviderMetadata {
                ProviderMetadata {
        ...
        async fn test_empty_turn_with_final_output_tool_nudges() -> Result<()> {
            use aibuddy::agents::final_output_tool::FINAL_OUTPUT_CONTINUATION_MESSAGE;
            use aibuddy::recipe::Response;
