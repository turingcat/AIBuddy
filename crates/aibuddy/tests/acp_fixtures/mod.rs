...
use aibuddy::acp::server::{serve, AIBuddyAcpAgent, AIBuddyAcpAgentOptions, AcpProviderFactory};
pub use aibuddy::acp::{map_permission_response, PermissionDecision};
...
use aibuddy::config::{AIBuddyMode, PermissionManager};
use aibuddy::providers::api_client::{ApiClient, AuthMethod as ApiAuthMethod};
use aibuddy::providers::base::Provider;
use aibuddy::providers::openai::OpenAiProvider;
use aibuddy::scheduler::{ScheduledJob, SchedulerError, ValidatedScheduleRecipe};
use aibuddy::scheduler_trait::SchedulerTrait;
use aibuddy::session::Session as AIBuddySession;
use aibuddy::session_context::SESSION_ID_HEADER;
use aibuddy_test_support::{ExpectedSessionId, TEST_MODEL};
...
impl SchedulerTrait for FixtureScheduler {
    ...
    async fn sessions(
        ...
        sched_id: &str,
        _limit: usize,
    ) -> Result<Vec<(String, AIBuddySession)>, SchedulerError> {
...
fn write_global_test_config(config_path: &Path, openai_base_url: &str) {
    ...
    let global_config_path = global_config_dir.join(aibuddy::config::base::CONFIG_YAML_NAME);
...
pub async fn spawn_acp_server_in_process(
    ...
    let config_path = data_root.join(aibuddy::config::base::CONFIG_YAML_NAME);
    if !config_path.exists() {
        fs::write(
            &config_path,
            format!(
                "AIBUDDY_MODEL: {current_model}\nAIBUDDY_PROVIDER: openai\nAIBUDDY_MODE: {}\nAIBUDDY_TOOL_PAIR_SUMMARIZATION: false\n",
                aibuddy_mode
            ),
        )
        .unwrap();
    ...
    let agent = AIBuddyAcpAgent::new(AIBuddyAcpAgentOptions {
        provider_factory,
        builtin_selection: aibuddy::acp::server::AcpBuiltinSelection {
    ...
    F: Future<Output = ()> + Send + 'static,
{
    ...
    if std::env::var_os("AIBUDDY_PATH_ROOT").is_none() {
