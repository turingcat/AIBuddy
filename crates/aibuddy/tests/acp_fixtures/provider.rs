...
use aibuddy::acp::{AcpProvider, AcpProviderConfig};
...
use aibuddy::permission::permission_confirmation::PrincipalType;
use aibuddy::permission::{Permission, PermissionConfirmation};
use aibuddy::providers::base::Provider;
use aibuddy_providers::model::ModelConfig;
use aibuddy_test_support::{ExpectedSessionId, IgnoreSessionId, TEST_MODEL};
...
impl AcpProviderSession {
    ...
    async fn send_message(
        ...
        let mut stream = aibuddy::session_context::with_session_id(
...
impl Connection for AcpProviderConnection {
    ...
    async fn new(config: TestConnectionConfig, openai: OpenAiFixture) -> Self {
        ...
        let aibuddy_mode = config.aibuddy_mode;
        ...
        let provider_config = AcpProviderConfig {
            ...
            mode_mapping: AIBuddyMode::VARIANTS
                .iter()
                .map(|v| {
                    let mode = AIBuddyMode::from_str(v).unwrap();
    ...
    async fn new_session(&mut self) -> anyhow::Result<SessionData<AcpProviderSession>> {
        ...
        let aibuddy_id = format!("test-session-{}", self.session_counter);
        ...
        let session = AcpProviderSession {
            ...
            session_id: agent_client_protocol::schema::v1::SessionId::new(aibuddy_id),
