...
use aibuddy::agents::extension_manager::{ExtensionManager, ExtensionManagerCapabilities};
use aibuddy::agents::{AIBuddyPlatform, MCP_PROTOCOL_VERSION};
...
impl aibuddy::providers::base::ProviderDescriptor for MockProvider {
...
async fn test_replayed_session(
    ...
    const TEST_WORKING_DIR: &str = "/tmp/aibuddy_test";
    fs::create_dir_all(TEST_WORKING_DIR).ok();

    let _env = env_lock::lock_env([
        ("AIBUDDY_MCP_CLIENT_VERSION", Some("0.0.0")),
        ("AIBUDDY_PROVIDER", Some("openai")),
        ("AIBUDDY_MODEL", Some("gpt-4o")),
        ("AIBUDDY_WORKING_DIR", Some(TEST_WORKING_DIR)),
    ]);

    // Setup test file for developer extension tests
    let test_file_path = "/tmp/aibuddy_test/aibuddy.txt";
    fs::write(test_file_path, "# aibuddy\n").ok();
    ...
    let mode = if env::var("AIBUDDY_RECORD_MCP").is_ok() {
        TestMode::Record
    } else {
    ...
    let session_manager = Arc::new(aibuddy::session::SessionManager::new(
