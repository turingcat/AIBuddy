use crate::agents::mcp_client::AIBuddyMcpHostInfo;
use crate::agents::{AIBuddyPlatform, Agent, AgentConfig, ExtensionLoadResult};
...
impl AgentManager {
        ...
        AGENT_MANAGER
            .get_or_try_init(|| async {
                ...
                let max_sessions = config
                    .get_aibuddy_max_active_agents()
                    .unwrap_or(DEFAULT_MAX_SESSION);
                let default_mode = config.get_aibuddy_mode().unwrap_or_default();
    ...
    async fn create_agent_locked(
        ...
        let mut mode = self.agent_config.aibuddy_mode;
        if let Ok(session) = self
            ...
            .await
        {
            mode = session.aibuddy_mode;
            info!(aibuddy_mode = %mode, session_id = %session_id, "Session loaded");
        }

        let mut config = self.agent_config.clone();
        config.aibuddy_mode = mode;
        ...
        if agent.provider().await.is_err() {
            if let Some(provider) = &*self.default_provider.read().await {
                ...
                let model_config = config
                    ...
                    .ok()
                    .zip(config.get_aibuddy_model().ok())
                    ...
                    .unwrap_or_else(|| aibuddy_providers::model::ModelConfig::new("unknown"));
                agent
...
mod tests {
    ...
    use crate::agents::{AIBuddyPlatform, AgentConfig};
    use crate::config::permission::PermissionManager;
    ...
    #[test_case(AIBuddyMode::Approve ; "approve")]
    #[test_case(AIBuddyMode::Chat ; "chat")]
    #[test_case(AIBuddyMode::SmartApprove ; "smart_approve")]
    #[tokio::test]
    async fn test_agent_inherits_session_mode(mode: AIBuddyMode) {
        let temp_dir = TempDir::new().unwrap();
        let manager = create_test_manager(&temp_dir).await;
        ...
        assert_eq!(agent.aibuddy_mode().await, mode);
    }
    ...
    async fn test_session_mode_isolation() {
        ...
        assert_eq!(a1.aibuddy_mode().await, AIBuddyMode::Approve);
        assert_eq!(a2.aibuddy_mode().await, AIBuddyMode::Auto);
    }
}
