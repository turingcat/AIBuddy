...
/// ```yaml
/// AIBUDDY_PROVIDER: claude-acp
/// AIBUDDY_MODEL: current
...
fn migrate_provider_config(config: &mut Mapping) -> bool {
    ...
    if config.contains_key(&providers_key) {
        ...
        if !config.contains_key(&ap_key) {
            if let Some(legacy) = config
                .get(serde_yaml::Value::String("AIBUDDY_PROVIDER".to_string()))
                .and_then(|v| v.as_str())
            {
    ...
    let active_model = config
        .get(serde_yaml::Value::String("AIBUDDY_MODEL".to_string()))
        .and_then(|v| v.as_str())
        .map(|s| s.to_string())
        .unwrap_or_default();
    ...
    config.shift_remove(serde_yaml::Value::String("AIBUDDY_PROVIDER".to_string()));
    config.shift_remove(serde_yaml::Value::String("AIBUDDY_MODEL".to_string()));
    for name in &discovered_providers {
...
mod tests {
    ...
    fn test_migrate_provider_config_basic() {
        let mut config = Mapping::new();
        config.insert(
            serde_yaml::Value::String("AIBUDDY_PROVIDER".to_string()),
            serde_yaml::Value::String("claude-acp".to_string()),
        );
        config.insert(
            serde_yaml::Value::String("AIBUDDY_MODEL".to_string()),
        ...
        assert!(!config.contains_key(serde_yaml::Value::String("AIBUDDY_PROVIDER".to_string())));
        assert!(!config.contains_key(serde_yaml::Value::String("AIBUDDY_MODEL".to_string())));
    ...
    fn test_cleanup_legacy_keys_when_providers_exists() {
        ...
        // active_provider should be backfilled from legacy AIBUDDY_PROVIDER
        assert_eq!(
            config
                .get(serde_yaml::Value::String("active_provider".to_string()))
                .and_then(|v| v.as_str()),
