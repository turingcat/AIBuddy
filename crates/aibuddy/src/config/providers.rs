...
pub fn get_active_provider(config: &Config) -> Option<String> {
    if let Ok(val) = env::var("AIBUDDY_PROVIDER") {
        return Some(val);
    }
    if let Ok(val) = config.get_param::<String>(ACTIVE_PROVIDER_KEY) {
        return Some(val);
    }
    config.get_param::<String>("AIBUDDY_PROVIDER").ok()
}

pub fn get_active_model(config: &Config) -> Option<String> {
    if let Ok(val) = env::var("AIBUDDY_MODEL") {
        return Some(val);
    }
    ...
    config.get_param::<String>("AIBUDDY_MODEL").ok()
}
...
pub fn clear_active_provider(config: &Config) -> Result<(), ConfigError> {
    for key in [ACTIVE_PROVIDER_KEY, "AIBUDDY_PROVIDER", "AIBUDDY_MODEL"] {
...
mod tests {
    ...
    fn test_clear_active_provider_removes_legacy_keys() {
        ...
        config.set_param("AIBUDDY_MODEL", "claude").unwrap();
