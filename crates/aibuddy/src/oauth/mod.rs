...
pub use persist::AIBuddyCredentialStore;
...
const OAUTH_CALLBACK_TIMEOUT_ENV: &str = "AIBUDDY_OAUTH_CALLBACK_TIMEOUT_SECONDS";
...
async fn complete_automatic_authorization(
    ...
    if std::env::var_os("AIBUDDY_OAUTH_AUTOMATIC_CALLBACK").is_none() {
        return Ok(None);
    }
...
/// drive the flow without an extension config (`aibuddy mcp-probe`, conformance
/// driver).
fn env_static_oauth_client() -> Option<StaticOAuthClientConfig> {
    Some(StaticOAuthClientConfig {
        client_id: std::env::var("AIBUDDY_MCP_OAUTH_CLIENT_ID").ok()?,
        client_secret: std::env::var("AIBUDDY_MCP_OAUTH_CLIENT_SECRET").ok(),
        scopes: Vec::new(),
    })
}

fn client_metadata_url() -> String {
    std::env::var("AIBUDDY_MCP_OAUTH_CLIENT_METADATA_URL")
...
fn build_authorization_request(
    ...
    let mut request = AuthorizationRequest::new(redirect_uri).with_client_name("aibuddy");
    match static_client {
        Some(client) => {
...
pub async fn oauth_flow_with_challenge(
    ...
    let credential_store = AIBuddyCredentialStore::new(name.clone());
    ...
    let port = std::env::var("AIBUDDY_OAUTH_CALLBACK_PORT")
        .ok()
