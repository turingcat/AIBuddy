...
fn project_settings_path(project_root: &Path, local: bool) -> PathBuf {
    ...
    project_root.join(".config").join("aibuddy").join(file)
}
...
mod tests {
    ...
    fn test_config(dir: &Path) -> Config {
        Config::new(dir.join("config.yaml"), "aibuddy-discovery-test").unwrap()
    }
    ...
    fn disabled_in_project_settings_drops_plugin() {
        ...
        write_settings(
            &project.join(".config").join("aibuddy"),
    ...
    fn project_scope_overrides_user_scope() {
        ...
        write_settings(
            &fake_home.path().join(".config").join("aibuddy"),
            r#"{"disabledPlugins":["demo"]}"#,
        );
