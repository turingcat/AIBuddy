...
use crate::config::{resolve_extensions_for_new_session, AIBuddyMode, Config};
...
async fn execute_job(
    ...
    agent
        ...
        .await?;
    agent
        .update_aibuddy_mode(AIBuddyMode::Auto, &session.id)
        .await?;
...
mod tests {
    ...
    async fn validated_recipe_bytes_and_base_are_persisted_after_source_replacement() {
        let temp_dir = tempdir().unwrap();
        let _guard =
            env_lock::lock_env([("AIBUDDY_PATH_ROOT", Some(temp_dir.path().to_str().unwrap()))]);
    ...
    async fn test_job_runs_on_schedule() {
        ...
        assert_eq!(sessions[0].aibuddy_mode, AIBuddyMode::Auto);
    }
