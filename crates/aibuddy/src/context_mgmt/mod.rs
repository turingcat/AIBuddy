pub use aibuddy_context_management::structured;
...
pub use aibuddy_context_management::DEFAULT_COMPACTION_THRESHOLD;

pub(crate) const TOOLCALL_SUMMARIZATION_BATCH_SIZE: usize = 10;

pub(crate) fn tool_pair_summarization_enabled() -> bool {
    Config::global()
        .get_param::<bool>("AIBUDDY_TOOL_PAIR_SUMMARIZATION")
        .unwrap_or(true)
}
...
struct AIBuddyCompactionModel<'a> {
...
impl aibuddy_context_management::CompactionModel for AIBuddyCompactionModel<'_> {
    async fn complete(
        &self,
        system: &str,
        messages: &[Message],
...
struct AIBuddyTokenEstimator;

#[async_trait::async_trait]
impl aibuddy_context_management::TokenEstimator for AIBuddyTokenEstimator {
...
fn compaction_templates() -> Result<aibuddy_context_management::Templates> {
    Ok(aibuddy_context_management::Templates {
...
async fn do_compact(
    ...
    let model = AIBuddyCompactionModel {
        provider,
        model_config,
        session_id,
    };
    let summary = aibuddy_context_management::summarize(
        &model,
        Some(&AIBuddyTokenEstimator),
...
pub use aibuddy_context_management::format_message_for_compacting;
    ...
    async fn carried_turn_context_stays_last_after_persist_and_reload() {
        ...
        let session = manager
            .create_session(
                ...
                crate::config::AIBuddyMode::default(),
            )
            .await
            .unwrap();
        manager
