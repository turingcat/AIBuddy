...
pub const CONFIG_RELAYS_KEY: &str = "AIBUDDY_NOSTR_RELAYS";
...
pub fn build_deeplink(nevent: &str, decryption_key: &str) -> String {
    format!(
        "aibuddy://sessions/nostr?nevent={}&key={}",
...
pub fn parse_deeplink(deeplink: &str) -> Result<ParsedShareLink> {
    ...
    if !matches!(parsed.scheme(), "aibuddy" | "aibuddy")
        ...
        || parsed.path() != "/nostr"
    {
        return Err(anyhow!("Invalid AIBuddy Nostr session share link"));
    }
...
mod tests {
    ...
    async fn publish_builds_deeplink_and_encrypted_kind_30278_event() {
        ...
        assert!(share.deeplink.starts_with("aibuddy://sessions/nostr?"));
    ...
    fn parses_canonical_deeplink() {
        let parsed = parse_deeplink("aibuddy://sessions/nostr?nevent=abc&key=def").unwrap();
