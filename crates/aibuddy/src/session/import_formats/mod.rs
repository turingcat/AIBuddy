//! Importers for non-aibuddy session formats.
//!
//! AIBuddy's native session export is a JSON-serialized [`crate::session::Session`].
...
//!
//! The strategy is to convert any supported foreign format into aibuddy's
...
/// the aibuddy-native session JSON handed to `SessionManager::import_session`.
...
pub(crate) fn build_session_json(session: ImportedSession) -> Value {
    ...
    obj.insert("aibuddy_mode".into(), json!("auto"));
...
pub enum ImportFormat {
    /// Native aibuddy session export — a JSON object representing a `Session`.
    AIBuddy,
...
///   and a `conversation` (or `messages`) field, it's aibuddy.
...
pub fn detect_format(content: &str) -> ImportFormat {
    ...
    if let Ok(v) = serde_json::from_str::<serde_json::Value>(first_line) {
        ...
        // Claude Code lines always include a sessionId; aibuddy's native JSON is
    ...
    // AIBuddy's pretty-printed export starts with `{` and *eventually* contains
    ...
    ImportFormat::AIBuddy
}

/// Convert any supported foreign format to a aibuddy-native session JSON string.
pub fn convert_to_aibuddy_session_json(content: &str) -> Result<String> {
    match detect_format(content) {
        ImportFormat::AIBuddy => Ok(upgrade_legacy_token_fields(content)),
...
/// Exports from aibuddy versions before `usage`/`accumulated_usage` existed
