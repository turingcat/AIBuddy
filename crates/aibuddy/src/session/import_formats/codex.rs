...
use aibuddy_providers::formats::openai_responses::{ResponseOutputItem, ResponsesApiResponse};
...
pub fn convert(content: &str) -> Result<String> {
    ...
    for (line_idx, line) in lines.iter().enumerate() {
        ...
        if let Ok(item) = serde_json::from_value::<ResponseOutputItem>(payload.clone()) {
            ...
            if let Ok(decoded) =
                aibuddy_providers::formats::openai_responses::responses_api_to_message(&stub)
            {
