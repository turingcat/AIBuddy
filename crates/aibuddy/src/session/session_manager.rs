...
use aibuddy_providers::conversation::token_usage::Usage;
...
pub struct SessionUpdateBuilder<'a> {
    ...
    aibuddy_mode: Option<AIBuddyMode>,
...
impl<'a> SessionUpdateBuilder<'a> {
    ...
    pub fn aibuddy_mode(mut self, mode: AIBuddyMode) -> Self {
        self.aibuddy_mode = Some(mode);
        self
    }
...
impl SessionManager {
    ...
    pub async fn create_session(
        ...
        self.storage
            .create_session(working_dir, name, session_type, aibuddy_mode)
            .await
    }
...
impl Default for Session {
    fn default() -> Self {
        Self {
            ...
            provider_name: None,
            model_config: None,
            aibuddy_mode: AIBuddyMode::default(),
            archived_at: None,
            project_id: None,
            parent_session_id: None,
...
fn deserialize_session_model_config(
    ...
    if provider_name == Some(aibuddy_providers::azure_foundry::AZURE_FOUNDRY_PROVIDER_NAME) {
        #[derive(Deserialize)]
...
impl SessionStorage {
    ...
    async fn create_schema(pool: &Pool<Sqlite>) -> Result<()> {
        ...
        sqlx::query(
            ...
            CREATE TABLE IF NOT EXISTS sessions (
                ...
                aibuddy_mode TEXT NOT NULL DEFAULT 'auto',
    ...
    async fn import_legacy_session(pool: &Pool<Sqlite>, session: &Session) -> Result<()> {
        ...
        INSERT INTO sessions (
            ...
            provider_name, model_config_json, aibuddy_mode
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        "#,
        )
        .bind(&session.id)
        ...
        .bind(session.aibuddy_mode.to_string())
        .execute(&mut *tx)
        .await?;
    ...
    async fn migrate_legacy_session_columns(tx: &mut sqlx::Transaction<'_, Sqlite>) -> Result<()> {
        let columns: Vec<String> = sqlx::query_scalar(
            "SELECT name FROM pragma_table_info('sessions') WHERE name IN ('goose_mode', 'aibuddy_mode')",
        )
        .fetch_all(&mut **tx)
        .await?;
        ...
        match version {
            ...
            8 => {
                let has_mode: bool = sqlx::query_scalar(
                    "SELECT EXISTS(SELECT 1 FROM pragma_table_info('sessions') WHERE name = 'aibuddy_mode')",
                )
                .fetch_one(&mut **tx)
                .await?;
                if !has_mode {
    ...
    async fn create_session(
        ...
        let session = sqlx::query_as(
            r#"
                INSERT INTO sessions (id, name, user_set_name, session_type, working_dir, extension_data, aibuddy_mode)
                VALUES (
                    ? || '_' || CAST(COALESCE((
    ...
    async fn apply_update(&self, builder: SessionUpdateBuilder<'_>) -> Result<()> {
        ...
        add_update!(builder.aibuddy_mode, "aibuddy_mode");
        ...
        if let Some(aibuddy_mode) = builder.aibuddy_mode {
            q = q.bind(aibuddy_mode.to_string());
        }
    ...
    async fn list_sessions_matching(&self, query: SessionListQuery<'_>) -> Result<Vec<Session>> {
        ...
        let sql = format!(
            ...
            SELECT s.id, s.working_dir, s.name, s.description, s.user_set_name, s.session_type, s.created_at, s.updated_at, s.extension_data,
                   ...
                   s.provider_name, s.model_config_json, s.aibuddy_mode,
    ...
    async fn import_session(
        ...
        let normalized = super::import_formats::convert_to_aibuddy_session_json(json)?;
    ...
    async fn copy_session(
        ...
        builder = builder.aibuddy_mode(original_session.aibuddy_mode);
...
mod tests {
    ...
    use aibuddy_providers::conversation::token_usage::{CostSource, ProviderUsage};
    ...
    async fn legacy_brand_database() -> (TempDir, Session) {
        ...
        sqlx::query("ALTER TABLE sessions RENAME COLUMN aibuddy_mode TO goose_mode")
            .execute(pool)
            .await
            .unwrap();
        pool.close().await;
    ...
    async fn legacy_brand_column_migrates_at_current_version_without_losing_sessions() {
        ...
        let created = manager
            ...
            .await
            .unwrap();
        assert_eq!(created.aibuddy_mode, AIBuddyMode::Chat);
        let pool = manager.storage().pool().await.unwrap();
    ...
    async fn legacy_brand_column_migration_serializes_concurrent_startup() {
        ...
        assert_eq!(left.unwrap().aibuddy_mode, AIBuddyMode::Approve);
        assert_eq!(right.unwrap().aibuddy_mode, AIBuddyMode::Approve);
    }
    ...
    async fn legacy_brand_column_conflict_leaves_both_values_untouched() {
        ...
        assert!(error
            .to_string()
            .contains("both goose_mode and aibuddy_mode"));
    ...
    fn azure_session_model_config_preserves_suffixed_deployment_id() {
        ...
        let config = deserialize_session_model_config(
            Some(aibuddy_providers::azure_foundry::AZURE_FOUNDRY_PROVIDER_NAME),
            &json,
        )
        .unwrap();
    ...
    fn azure_session_model_config_preserves_explicit_thinking_effort() {
        ...
        assert_eq!(
            ...
            Some(aibuddy_providers::thinking::ThinkingEffort::Low)
        );
    }
    ...
    fn non_azure_session_model_config_keeps_suffix_normalization() {
        let config = deserialize_session_model_config(
            Some(aibuddy_providers::openai::OPEN_AI_PROVIDER_NAME),
    ...
    impl Provider for NamingTestProvider {
        ...
        ) -> std::result::Result<MessageStream, aibuddy_providers::errors::ProviderError> {
    ...
    #[tokio::test]
    async fn test_aibuddy_mode_persists(mode: AIBuddyMode) {
        let temp_dir = TempDir::new().unwrap();
        let sm = SessionManager::new(temp_dir.path().to_path_buf());
        ...
        let reloaded = sm.get_session(&session.id, false).await.unwrap();
        assert_eq!(reloaded.aibuddy_mode, mode);
    }

    #[tokio::test]
    async fn test_aibuddy_mode_update() {
        let temp_dir = TempDir::new().unwrap();
        let sm = SessionManager::new(temp_dir.path().to_path_buf());
        ...
        sm.update(&session.id)
            .aibuddy_mode(AIBuddyMode::Approve)
            .apply()
            .await
            .unwrap();

        let reloaded = sm.get_session(&session.id, false).await.unwrap();
        assert_eq!(reloaded.aibuddy_mode, AIBuddyMode::Approve);
    }

    #[tokio::test]
    async fn test_aibuddy_mode_malformed_defaults_to_auto() {
        let temp_dir = TempDir::new().unwrap();
        let sm = SessionManager::new(temp_dir.path().to_path_buf());
        ...
        let reloaded = sm.get_session(&session.id, false).await.unwrap();
        assert_eq!(reloaded.aibuddy_mode, AIBuddyMode::default());
    }
