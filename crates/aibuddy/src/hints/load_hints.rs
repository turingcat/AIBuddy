...
pub const AIBUDDY_HINTS_FILENAME: &str = ".aibuddyhints";
...
pub fn get_context_filenames() -> Vec<String> {
    ...
    Config::global()
        ...
        .unwrap_or_else(|_| {
            vec![
                AIBUDDY_HINTS_FILENAME.to_string(),
...
pub fn load_hint_files(
    ...
    if !global_hints_contents.is_empty() {
        hints.push_str("\n### Global Hints\nThese are my global aibuddy hints.\n");
...
mod tests {
    ...
    #[test]
    fn test_aibuddyhints_when_present() {
        let dir = TempDir::new().unwrap();

        fs::write(dir.path().join(AIBUDDY_HINTS_FILENAME), "Test hint content").unwrap();
    ...
    fn test_global_agents_md_in_agents_home() {
        ...
        std::env::set_var("AIBUDDY_PATH_ROOT", root.path());
        ...
        std::env::remove_var("AIBUDDY_PATH_ROOT");
    ...
    #[test]
    fn test_aibuddyhints_when_missing() {
        let dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_aibuddyhints_multiple_filenames() {
        let dir = TempDir::new().unwrap();
        ...
        .unwrap();
        fs::write(
            dir.path().join(AIBUDDY_HINTS_FILENAME),
            "Custom hints file content from .aibuddyhints",
        )
        .unwrap();
        ...
        let hints = load_hint_files(
            dir.path(),
            &["CLAUDE.md".to_string(), AIBUDDY_HINTS_FILENAME.to_string()],
            &gitignore,
        );

        assert!(hints.contains("Custom hints file content from CLAUDE.md"));
        assert!(hints.contains("Custom hints file content from .aibuddyhints"));
    }

    #[test]
    fn test_aibuddyhints_configurable_filename() {
        let dir = TempDir::new().unwrap();
        ...
        assert!(!hints.contains(".aibuddyhints")); // Make sure it's not loading the default
    }

    #[test]
    fn test_nested_aibuddyhints_with_git_root() {
        let temp_dir = TempDir::new().unwrap();
        let project_root = temp_dir.path();

        fs::create_dir(project_root.join(".git")).unwrap();
        fs::write(
            project_root.join(AIBUDDY_HINTS_FILENAME),
            "Root hints content",
        )
        .unwrap();
        ...
        fs::write(subdir.join(AIBUDDY_HINTS_FILENAME), "Subdir hints content").unwrap();
        let current_dir = subdir.join("current_dir");
        fs::create_dir(&current_dir).unwrap();
        fs::write(
            current_dir.join(AIBUDDY_HINTS_FILENAME),
    ...
    #[test]
    fn test_nested_aibuddyhints_without_git_root() {
        let temp_dir = TempDir::new().unwrap();
        let base_dir = temp_dir.path();

        fs::write(base_dir.join(AIBUDDY_HINTS_FILENAME), "Base hints content").unwrap();
    ...
    #[test]
    fn test_nested_aibuddyhints_mixed_filenames() {
        let temp_dir = TempDir::new().unwrap();
        let project_root = temp_dir.path();
        ...
        fs::create_dir(&subdir).unwrap();
        fs::write(
            subdir.join(AIBUDDY_HINTS_FILENAME),
            "Subdir .aibuddyhints content",
        )
        .unwrap();
        ...
        assert!(hints.contains("Subdir .aibuddyhints content"));
    }
...
Additional instructions here."#;
        fs::write(project_root.join(AIBUDDY_HINTS_FILENAME), hints_content).unwrap();
...
End of nested hints"#;
        fs::write(
            components_dir.join(AIBUDDY_HINTS_FILENAME),
...
@../parent.md
End of hints"#;
        fs::write(current_dir.join(AIBUDDY_HINTS_FILENAME), hints_content).unwrap();
...
@../root_file.md
End of hints"#;
        fs::write(subdir.join(AIBUDDY_HINTS_FILENAME), hints_content).unwrap();
        let gitignore = create_dummy_gitignore();

        let hints = load_hint_files(&subdir, &[AIBUDDY_HINTS_FILENAME.to_string()], &gitignore);
    ...
    fn tracker_deduplicates_directories() {
        ...
        let subdir = project_root.join("nested");
        fs::create_dir_all(&subdir).unwrap();
        fs::write(subdir.join(AIBUDDY_HINTS_FILENAME), "nested hints").unwrap();
    ...
    fn tracker_rejects_parent_traversal_outside_working_directory() {
        ...
        fs::write(outside.join(AIBUDDY_HINTS_FILENAME), "outside hints").unwrap();
    ...
    fn tracker_preserves_in_boundary_symlinks_and_deduplicates_aliases() {
        ...
        fs::write(real.join(AIBUDDY_HINTS_FILENAME), "real hints").unwrap();
    ...
    fn tracker_retries_directory_after_parent_is_created() {
        ...
        fs::write(future.join(AIBUDDY_HINTS_FILENAME), "future hints").unwrap();
        tracker.record_tool_arguments(&Some(args), &project_root);
