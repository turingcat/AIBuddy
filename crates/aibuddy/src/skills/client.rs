...
impl SkillsClient {
    ...
    /// Controls whether AIBuddy's bundled skills are exposed by this client.
...
mod tests {
    ...
    fn test_client(project: &Path, plugin_name: &str, enabled: bool) -> SkillsClient {
        let config = Box::leak(Box::new(
            Config::new(project.join("test-config.yaml"), "aibuddy-skills-test").unwrap(),
        ));
    ...
    async fn test_load_filesystem_skill_without_builtin_skills() {
        ...
        let skill_dir = temp_dir.path().join(".aibuddy/skills/my-skill");
        fs::create_dir_all(&skill_dir).unwrap();
        fs::write(
            skill_dir.join("SKILL.md"),
