...
const DEFAULT_AIBUDDY_DOCS_ROOT: &str = "https://goose-docs.ai";
const AIBUDDY_DOCS_ROOT_PLACEHOLDER: &str = "{{AIBUDDY_DOCS_ROOT}}";

/// Substitute the `{{AIBUDDY_DOCS_ROOT}}` placeholder in the builtin
/// `aibuddy-doc-guide` skill with the resolved docs root. Resolution is
/// deterministic: the configured `AIBUDDY_DOCS_ROOT` if set, otherwise the
/// canonical online docs root.
fn resolve_docs_root_placeholder(skill: &SourceEntry, content: &str, docs_root: &str) -> String {
    if skill.name != "aibuddy-doc-guide" || skill.source_type != SourceType::BuiltinSkill {
        return content.to_string();
    }

    content.replace(AIBUDDY_DOCS_ROOT_PLACEHOLDER, docs_root)
}

fn loaded_skill_context(skill: &SourceEntry, content: &str) -> Result<String> {
    let docs_root = Config::global()
        .get_aibuddy_docs_root()?
        .unwrap_or_else(|| DEFAULT_AIBUDDY_DOCS_ROOT.to_string());
    ...
    canonical_path.ancestors().find_map(|ancestor| {
        ...
        let is_project_skills_root = ancestor.file_name().and_then(|name| name.to_str())
            ...
            && matches!(
                ...
                Some(".aibuddy") | Some(".claude") | Some(".agents")
            );
...
mod tests {
    ...
    fn builtin_aibuddy_doc_guide_skill() -> SourceEntry {
        SourceEntry {
            source_type: SourceType::BuiltinSkill,
            name: "aibuddy-doc-guide".to_string(),
            description: "Test docs skill".to_string(),
            content: "Docs root: {{AIBUDDY_DOCS_ROOT}}.".to_string(),
            path: "builtin://skills/aibuddy-doc-guide".to_string(),
            global: true,
            writable: true,
            supporting_files: Vec::new(),
    ...
    fn loaded_skill_context_shows_resolved_paths_for_supporting_files() {
        let skill_dir = std::env::temp_dir().join("aibuddy-test-skill");
    ...
    #[test]
    fn resolve_docs_root_placeholder_substitutes_builtin_aibuddy_doc_guide_root() {
        let skill = builtin_aibuddy_doc_guide_skill();

        let rendered =
            resolve_docs_root_placeholder(&skill, &skill.content, "/tmp/aibuddy docs/root");

        assert_eq!(rendered, "Docs root: /tmp/aibuddy docs/root.");
    }

    #[test]
    fn resolve_docs_root_placeholder_ignores_non_builtin_aibuddy_doc_guide_skills() {
        let mut skill = builtin_aibuddy_doc_guide_skill();
        skill.source_type = SourceType::Skill;

        let rendered = resolve_docs_root_placeholder(&skill, &skill.content, "/tmp/aibuddy-docs");
    ...
    fn project_plugin_skill_precedes_global_skill_with_same_name() {
        ...
        let _guard = env_lock::lock_env([
            ("AIBUDDY_PATH_ROOT", path_root.path().to_str()),
            ("PLUGINS", None),
        ]);
    ...
    fn settings_disabled_project_plugin_canonical_path_is_rejected() {
        ...
        std::fs::create_dir_all(plugin_link.parent().unwrap()).unwrap();
        std::os::unix::fs::symlink(external_root.path(), &plugin_link).unwrap();
        let settings_dir = project.path().join(".config/aibuddy");
    ...
    fn symlinked_user_plugin_skill_remains_writable_for_source_crud() {
        ...
        {
            let _guard = env_lock::lock_env([
                ("AIBUDDY_PATH_ROOT", project_path_root.path().to_str()),
                ("PLUGINS", None),
            ]);
