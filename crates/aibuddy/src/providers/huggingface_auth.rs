...
pub fn oauth_client_id() -> &'static str {
    option_env!("AIBUDDY_HUGGINGFACE_OAUTH_CLIENT_ID")
...
<html>
  <head>
    <title>aibuddy - Hugging Face Authorization Successful</title>
...
<html>
  <head>
    <title>aibuddy - Hugging Face Authorization Failed</title>
    <style>
      body {{
        font-family: system-ui, -apple-system, sans-serif;
        display: flex; justify-content: center; align-items: center;
...
async fn perform_loopback_oauth_flow(client_id: &str) -> Result<HuggingFaceTokenData> {
    ...
    if webbrowser::open(&auth_url).is_err() {
        tracing::info!(
            "Please open this URL in your browser to authorize aibuddy with Hugging Face:\n{}",
            auth_url
        );
    }
...
mod tests {
    ...
    fn oauth_client_id_defaults_to_cimd_metadata_url() {
        if option_env!("AIBUDDY_HUGGINGFACE_OAUTH_CLIENT_ID").is_none() {
