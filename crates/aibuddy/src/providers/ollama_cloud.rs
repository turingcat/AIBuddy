...
use aibuddy_providers::api_client::{ApiClient, AuthMethod, TlsConfig};
use aibuddy_providers::base::{ModelInfo, ProviderDescriptor};
use aibuddy_providers::errors::ProviderError;
use aibuddy_providers::model::ModelConfig;
use aibuddy_providers::ollama::fetch_ollama_model_names;
use aibuddy_providers::openai::OpenAiProvider;
use anyhow::Result;
...
fn build_ollama_api_client(
    ...
    let normalized_base_url = aibuddy_providers::openai::ensure_url_scheme(&config.base_url);
        ...
        Box::pin(async {
            anyhow::bail!(
                "Ollama Cloud must be configured as a declarative provider. \
                 Run `aibuddy configure` to set it up."
            )
        })
    }
}
