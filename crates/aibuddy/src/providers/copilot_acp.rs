...
impl aibuddy_providers::base::ProviderDescriptor for CopilotAcpProvider {
    fn metadata() -> ProviderMetadata {
        ProviderMetadata::new(
            COPILOT_ACP_PROVIDER_NAME,
            "GitHub Copilot CLI (ACP)",
            "Use aibuddy with your GitHub Copilot subscription via the Copilot CLI.",
            ACP_CURRENT_MODEL,
            vec![],
            COPILOT_ACP_DOC_URL,
            vec![],
    ...
    fn create(
        ...
        Box::pin(async move {
            ...
            let mode_mapping = HashMap::from([
                (AIBuddyMode::Auto, vec![MODE_AGENT.to_string()]),
                (AIBuddyMode::Approve, vec![MODE_AGENT.to_string()]),
                (AIBuddyMode::SmartApprove, vec![MODE_AGENT.to_string()]),
                (AIBuddyMode::Chat, vec![MODE_PLAN.to_string()]),
            ]);
