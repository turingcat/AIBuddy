...
pub fn options_from_config() -> OllamaOptions {
    ...
    let input_limit = match config.get_param::<usize>("AIBUDDY_INPUT_LIMIT") {
        Ok(limit) if limit > 0 => Some(limit),
        Ok(_) => None,
        Err(crate::config::ConfigError::NotFound(_)) => None,
        Err(e) => {
            tracing::warn!("Invalid AIBUDDY_INPUT_LIMIT value: {}", e);
            None
        }
    };
...
/// Priority: OLLAMA_STREAM_TIMEOUT > AIBUDDY_STREAM_TIMEOUT > OLLAMA_TIMEOUT > default (120s).
...
fn resolve_ollama_chunk_timeout(config: &crate::config::Config) -> u64 {
    ...
    if let Ok(val) = config.get_param::<u64>("AIBUDDY_STREAM_TIMEOUT") {
        if val > 0 {
            return val;
        }
    }
...
mod tests {
    ...
    fn test_resolve_ollama_chunk_timeout_defaults_to_ollama_timeout() {
        let _guard = env_lock::lock_env([
            ...
            ("AIBUDDY_STREAM_TIMEOUT", None::<&str>),
    ...
    fn test_resolve_ollama_chunk_timeout_prefers_stream_override() {
        let _guard = env_lock::lock_env([
            ...
            ("AIBUDDY_STREAM_TIMEOUT", Some("90")),
            ("OLLAMA_TIMEOUT", Some("300")),
        ]);
        let config = crate::config::Config::global();
    ...
    #[test]
    fn test_resolve_ollama_chunk_timeout_uses_aibuddy_stream_fallback() {
        let _guard = env_lock::lock_env([
            ("OLLAMA_STREAM_TIMEOUT", None::<&str>),
            ("AIBUDDY_STREAM_TIMEOUT", Some("90")),
            ("OLLAMA_TIMEOUT", Some("300")),
