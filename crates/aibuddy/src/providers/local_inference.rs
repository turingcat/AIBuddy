pub use aibuddy_providers::local_inference::*;
...
const LOCAL_MODEL_SETTINGS_KEY: &str = "AIBUDDY_LOCAL_MODEL_SETTINGS";
...
fn resolve_model_settings(
    model_id: &str,
) -> Result<Option<aibuddy_providers::local_inference::model::ModelSettings>> {
    let settings = crate::config::Config::global()
        .get_param::<HashMap<String, aibuddy_providers::local_inference::model::ModelSettings>>(
...
fn write_model_settings(
    model_id: &str,
    settings: &aibuddy_providers::local_inference::model::ModelSettings,
) -> Result<()> {
    crate::config::Config::global().update_param::<HashMap<
        String,
        aibuddy_providers::local_inference::model::ModelSettings,
