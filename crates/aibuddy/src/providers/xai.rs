...
pub fn xai_known_model_info() -> Vec<aibuddy_providers::base::ModelInfo> {
    XAI_KNOWN_MODELS
        .iter()
        .map(|model_name| {
            aibuddy_providers::base::model_info_for_provider_model(XAI_PROVIDER_NAME, model_name)
        })
        .collect()
}

impl aibuddy_providers::base::ProviderDescriptor for XaiProvider {
