...
pub struct ClaudeCodeProvider {
    ...
    #[serde(skip)]
    initial_mode: tokio::sync::Mutex<Option<AIBuddyMode>>,
}
...
impl ClaudeCodeProvider {
    ...
    fn build_stream_json_command(&self) -> Command {
        ...
        // Allow aibuddy to run inside a Claude Code session.
    ...
    fn apply_permission_flags(cmd: &mut Command) -> Result<bool, ProviderError> {
        ...
        match aibuddy_mode {
            ...
            AIBuddyMode::SmartApprove | AIBuddyMode::Approve => {
                cmd.arg("--permission-prompt-tool").arg("stdio");
                Ok(true)
            }
            AIBuddyMode::Chat => Ok(false),
        }
    }
...
impl aibuddy_providers::base::ProviderDescriptor for ClaudeCodeProvider {
...
impl Provider for ClaudeCodeProvider {
    ...
    async fn update_mode(&self, _session_id: &str, mode: AIBuddyMode) -> Result<(), ProviderError> {
...
mod tests {
    ...
    use aibuddy_test_support::session::TEST_SESSION_ID;
    use chrono::Utc;
    use serde_json::json;
    use std::collections::HashMap;
    use std::fs;
