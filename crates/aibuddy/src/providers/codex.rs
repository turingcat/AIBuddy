...
pub struct CodexProvider {
    ...
    #[serde(skip)]
    mode_by_session: tokio::sync::RwLock<HashMap<String, AIBuddyMode>>,
}
...
impl CodexProvider {
    ...
    /// Apply permission flags based on AIBuddyMode
    ...
    async fn execute_command(
        ...
        if std::env::var("AIBUDDY_CODEX_DEBUG").is_ok() {
        ...
        // This allows users to set AIBUDDY_PROVIDER=codex without needing to specify a model
        ...
        Self::apply_permission_flags(&mut cmd, aibuddy_mode)?;
...
impl aibuddy_providers::base::ProviderDescriptor for CodexProvider {
...
impl Provider for CodexProvider {
    ...
    async fn stream(
        ...
        let lines = self
            .execute_command(model_config, system, messages, tools, aibuddy_mode)
            .await?;
...
mod tests {
    ...
    use aibuddy_providers::base::ProviderDescriptor as _;
    use aibuddy_test_support::TEST_IMAGE_B64;
    use std::collections::HashMap;
    #[cfg(unix)]
    use std::fs;
    #[cfg(unix)]
...
"#,
    ...
    #[test_case(AIBuddyMode::Auto, &["--yolo"] ; "auto_yolo")]
    #[test_case(AIBuddyMode::SmartApprove, &["--full-auto"] ; "smart_approve_full_auto")]
    #[test_case(AIBuddyMode::Approve, &[] as &[&str] ; "approve_no_flags")]
    #[test_case(AIBuddyMode::Chat, &["--sandbox", "read-only"] ; "chat_read_only")]
    fn test_apply_permission_flags(mode: AIBuddyMode, expected: &[&str]) {
