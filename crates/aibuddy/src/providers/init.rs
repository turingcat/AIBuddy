...
mod tests {
    ...
    async fn test_custom_provider_context_limit_is_applied_from_file() {
        let _guard = env_lock::lock_env([("AIBUDDY_PATH_ROOT", None::<&str>)]);
        let temp_dir = tempfile::tempdir().expect("tempdir should be created");
        std::env::set_var("AIBUDDY_PATH_ROOT", temp_dir.path());
  ...
  "requires_auth": false
    ...
    #[tokio::test]
    async fn test_aibuddy_context_limit_overrides_known_models_and_defaults() {
        let _guard = env_lock::lock_env([
            ("AIBUDDY_PATH_ROOT", None::<&str>),
            ("AIBUDDY_CONTEXT_LIMIT", Some("1000000")),
