pub use aibuddy_providers::canonical::catalog::{
...
pub async fn get_providers_by_format(format: ProviderFormat) -> Vec<ProviderCatalogEntry> {
    ...
    aibuddy_providers::canonical::catalog::get_providers_by_format(format, &native_provider_ids)
}

pub async fn get_setup_catalog_entries() -> Vec<ProviderSetupCatalogEntry> {
    aibuddy_providers::canonical::catalog::get_setup_catalog_entries(
        super::providers()
            .await
            .into_iter()
...
pub fn get_provider_template(provider_id: &str) -> Option<ProviderTemplate> {
    aibuddy_providers::canonical::catalog::get_provider_template(provider_id)
}
...
mod tests {
    ...
    #[tokio::test]
    async fn setup_catalog_includes_aibuddy_and_curated_fields() {
        let entries = get_setup_catalog_entries().await;

        let aibuddy = entries
            .iter()
            .find(|entry| entry.provider_id == "aibuddy")
            .expect("setup catalog should include synthetic aibuddy");
        assert_eq!(aibuddy.category, ProviderSetupCategory::Agent);
        assert!(!aibuddy.acp);
        assert_eq!(aibuddy.setup_method, ProviderSetupMethod::None);
        assert!(aibuddy.fields.is_empty());
    ...
    async fn setup_catalog_preserves_curated_providers_and_hides_deprecated_ones() {
        ...
        for provider_id in ["aibuddy", "claude-acp", "openai"] {
