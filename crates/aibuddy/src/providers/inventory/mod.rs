...
fn enrich_model_ids_with_canonical(
    ...
    // For Databricks providers, prefer aibuddy- prefixed model_ids when there are duplicates.
    // Re-scan: if a later model_id with "aibuddy-" prefix maps to the same display name,
    // swap it in.
...
mod tests {
    ...
    #[test]
    fn databricks_v2_inventory_prefers_aibuddy_model_ids_for_duplicate_names() {
        let models = enrich_model_ids_with_canonical(
            "databricks_v2",
            &[
                "databricks-gpt-5-5".to_string(),
                "aibuddy-gpt-5-5".to_string(),
            ],
        );

        assert!(
            models.iter().any(|model| model.id == "aibuddy-gpt-5-5"),
            "expected aibuddy-gpt-5-5 to win duplicate canonical-name tie, got {models:?}"
        );
        assert!(
            !models.iter().any(|model| model.id == "databricks-gpt-5-5"),
            "expected databricks-gpt-5-5 to be replaced by aibuddy-gpt-5-5, got {models:?}"
        );
    }
