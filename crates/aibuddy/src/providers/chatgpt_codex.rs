...
use aibuddy_providers::formats::openai_responses::responses_api_to_streaming_message;
...
<html>
  <head>
    <title>aibuddy - ChatGPT Authorization Successful</title>
    <style>
      body {
  ...
  <body>
    <div class="container">
      ...
      <p>You can close this window and return to aibuddy.</p>
    </div>
...
<html>
  <head>
    <title>aibuddy - ChatGPT Authorization Failed</title>
    <style>
      body {{
        font-family: system-ui, -apple-system, sans-serif;
        display: flex;
...
impl ChatGptCodexProvider {
    ...
    async fn post_streaming(&self, payload: &Value) -> Result<reqwest::Response, ProviderError> {
        ...
        let client = aibuddy_providers::provider_reqwest_builder()
            .build()
            .map_err(|e| ProviderError::ExecutionError(e.to_string()))?;
        let request = client
...
impl aibuddy_providers::base::ProviderDescriptor for ChatGptCodexProvider {
...
mod tests {
    ...
    fn inventory_configured_uses_oauth_token_cache() {
        ...
        let _guard = env_lock::lock_env([("AIBUDDY_PATH_ROOT", Some(root_path.as_str()))]);
