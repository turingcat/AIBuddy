...
use aibuddy_providers::formats::anthropic::AnthropicFormatOptions;
use anyhow::Result;
...
impl KimiCodeProvider {
    ...
    pub async fn from_env(
        ...
        let client = aibuddy_providers::provider_reqwest_builder().build()?;
    ...
    async fn post(&self, payload: &Value) -> Result<reqwest::Response, ProviderError> {
        ...
        aibuddy_providers::http_status::send_bounded(
            request,
...
impl aibuddy_providers::base::ProviderDescriptor for KimiCodeProvider {
    fn metadata() -> ProviderMetadata {
        ...
        .with_setup_steps(vec![
            "Run `aibuddy configure` and select 'Kimi Code'",
            "A browser window will open — log in to kimi.com and enter the displayed code",
            "Once authorized, AIBuddy will save your token automatically",
        ])
    }
}
        ...
        KimiCodeProvider {
            ...
            token_cache: TokenCache {
                path: std::env::temp_dir()
                    .join(format!("aibuddy-kimicode-test-{}.json", Uuid::new_v4())),
            },
            cached_token: tokio::sync::Mutex::new(None),
