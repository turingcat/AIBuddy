...
use crate::config::{AIBuddyMode, PermissionManager};
    ...
    async fn inspect(
        ...
        for request in tool_requests {
            if let Ok(tool_call) = &request.tool_call {
                ...
                let action = match aibuddy_mode {
                    AIBuddyMode::Chat => continue,
                    AIBuddyMode::Auto => InspectionAction::Allow,
                    AIBuddyMode::Approve | AIBuddyMode::SmartApprove => {
                        ...
                        } else if aibuddy_mode == AIBuddyMode::SmartApprove
                ...
                let reason = match &action {
                    InspectionAction::Allow => {
                        if aibuddy_mode == AIBuddyMode::Auto {
...
mod tests {
    ...
    async fn inspect_tool(
        ...
        let mut results = inspector
            .inspect(aibuddy_test_support::TEST_SESSION_ID, &[req], &[], mode)
            .await
            .unwrap();
    ...
    #[test_case(AIBuddyMode::Auto, false, None, InspectionAction::Allow; "auto_allows")]
    #[test_case(AIBuddyMode::SmartApprove, true, None, InspectionAction::Allow; "smart_approve_annotation_allows")]
    #[test_case(AIBuddyMode::SmartApprove, false, Some(PermissionLevel::AlwaysAllow), InspectionAction::RequireApproval(None); "smart_approve_ignores_legacy_cached_allow")]
    #[test_case(AIBuddyMode::SmartApprove, false, Some(PermissionLevel::AskBefore), InspectionAction::RequireApproval(None); "smart_approve_cached_ask")]
    #[test_case(AIBuddyMode::SmartApprove, false, None, InspectionAction::RequireApproval(None); "smart_approve_unknown_defers")]
    #[test_case(AIBuddyMode::Approve, false, None, InspectionAction::RequireApproval(None); "approve_requires_approval")]
    #[test_case(AIBuddyMode::Approve, false, Some(PermissionLevel::AlwaysAllow), InspectionAction::RequireApproval(None); "approve_ignores_cache")]
    #[test_case(AIBuddyMode::Approve, true, None, InspectionAction::RequireApproval(None); "approve_ignores_annotation")]
    #[tokio::test]
