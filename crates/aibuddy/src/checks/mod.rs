...
//! projects; `aibuddy review` consumes [`Check`] and [`discover`] directly.
...
///
/// Mirrors `aibuddy::agents::subagent_task_config::DEFAULT_SUBAGENT_MAX_TURNS`,
...
pub fn global_checks_dirs() -> Vec<PathBuf> {
    ...
    if let Some(home) = dirs_home() {
        dirs.push(home.join(".config").join("aibuddy").join("checks"));
    ...
    for entry in entries {
        ...
        match parsed {
            ...
            Err(e) => match mode {
                ...
                LoadMode::Lenient => {
                    eprintln!("aibuddy review: skipping {}: {e}", path.display());
                }
            },
        }
    }
