...
impl<'a> SystemPromptBuilder<'a, PromptManager> {
    ...
    pub fn with_aibuddy_mode(mut self, mode: AIBuddyMode) -> Self {
    ...
    pub fn build(self) -> String {
        ...
        let aibuddy_mode = self
            .aibuddy_mode
            .unwrap_or_else(|| Config::global().get_aibuddy_mode().unwrap_or_default());

        let context = SystemPromptContext {
            extensions: sanitized_extensions_info,
            current_date_time: self.manager.current_date_timestamp.clone(),
            aibuddy_mode,
            is_autonomous: aibuddy_mode == AIBuddyMode::Auto,
        ...
        .unwrap_or_else(|_| {
            "你是 AIBuddy，由广林数科创建的通用 AI 助手。当用户问候你或询问你的身份时，你的回复必须以\"我是AIBuddy\"开头。默认使用中文进行可见的推理、工具调用说明和最终回复；代码、命令、文件路径、协议标识以及工具和扩展提供的上下文字段保持原样。".to_string()
        });
...
mod tests {
    ...
    #[test]
    fn shared_system_prompt_uses_aibuddy_chinese_identity() {
        ...
        assert!(!prompt.contains("called aibuddy"));
    }
    ...
    async fn test_all_platform_extensions() {
        ...
        let _guard = env_lock::lock_env([
            ...
            ("AIBUDDY_PATH_ROOT", Some(temp_root.as_str())),
        ]);
