...
use crate::agents::state_machine::{AIBuddyEffect, ConversationEffect};
use crate::conversation::message::MessageUsage;
use crate::conversation::Conversation;
use crate::session::{Session, SessionManager};

fn attach_to_last_assistant(effects: &mut [AIBuddyEffect], usage: &ProviderUsage) {
...
pub(super) fn enrich(session: &Session, effects: &mut [AIBuddyEffect]) {
    for index in 0..effects.len() {
        let (usage, replaces_conversation) = match &effects[index] {
            AIBuddyEffect::RecordUsage(usage) => (usage.clone(), false),
        ...
        match &mut effects[index] {
            AIBuddyEffect::RecordUsage(usage) => *usage = enriched,
            AIBuddyEffect::ReplaceConversation { usage, .. } => *usage = Some(enriched),
            _ => {}
        }
    }
}
