...
//! `StateMachine::run`. AIBuddy's concrete operations remain internal because their
...
mod session;
pub(crate) use session::run as run_aibuddy;
...
pub use effects::AIBuddyEffect;
pub(crate) use tool_confirmation::{
    has_unapplied_tool_confirmation_response, pending_tool_confirmations,
    persist_tool_confirmation_decision,
};

pub(super) use inference_preparation::AIBuddyInferenceRequestPreparer;
...
pub(super) use ops_llm::{AIBuddyInferenceProvider, InferenceRunner};
...
pub fn enabled() -> bool {
    std::env::var("AIBUDDY_STATE_MACHINE")
