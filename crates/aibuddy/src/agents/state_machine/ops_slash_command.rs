...
pub struct SlashCommandOperation<'a> {
    operations: Vec<Arc<dyn Operation<Session, AIBuddyEffect> + 'a>>,
}
...
impl<'a> SlashCommandOperation<'a> {
    pub fn new(operations: Vec<Arc<dyn Operation<Session, AIBuddyEffect> + 'a>>) -> Self {
        Self { operations }
    }
}

#[async_trait]
impl Operation<Session, AIBuddyEffect> for SlashCommandOperation<'_> {
