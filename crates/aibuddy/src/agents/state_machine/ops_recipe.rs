...
use crate::agents::state_machine::{
    ...
    AIBuddyEffect, ConversationEffect, Emitter, Operation, OperationResult, SlashCommand,
};
...
#[async_trait]
impl Operation<Session, AIBuddyEffect> for RecipeOperation {
    ...
    async fn run_command(
        ...
        applied([
            ...
            .into(),
            AIBuddyEffect::SetRecipe(Box::new(Some(recipe))),
            Message::user()
                .with_text(prompt)
    ...
    async fn run(
        ...
        if let Some((request, _)) = pending {
            if session.aibuddy_mode == AIBuddyMode::Chat {
