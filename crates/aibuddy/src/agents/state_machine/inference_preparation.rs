//! AIBuddy-specific inference request preparation.
...
use aibuddy_agent::inference::{InferenceRequestPreparer, PreparedInferenceRequest};
use aibuddy_agent::operation::{messages_since_kickoff, InferenceInput};
use aibuddy_providers::conversation::message::Message;
use aibuddy_providers::conversation::Conversation;
use anyhow::Result;
...
pub struct AIBuddyInferenceRequestPreparer<'a> {
    #[cfg(feature = "code-mode")]
    pub(crate) extension_manager: Arc<ExtensionManager>,
    pub(crate) aibuddy_mode: &'a Mutex<AIBuddyMode>,
...
#[async_trait]
impl InferenceRequestPreparer<Session> for AIBuddyInferenceRequestPreparer<'_> {
    async fn prepare(
        &self,
        session: &Session,
        ...
        let aibuddy_mode = *self.aibuddy_mode.lock().await;
