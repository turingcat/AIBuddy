...
use crate::agents::state_machine::{
    ...
    yielded_with, AIBuddyEffect, ConversationEffect, Emitter, Operation, OperationResult,
    SlashCommand,
};
...
const COMPACTION_THINKING_TEXT: &str = "aibuddy is compacting the conversation...";
...
#[async_trait]
impl Operation<Session, AIBuddyEffect> for CompactionOperation {
    ...
    async fn run(
        ...
        .await
        {
            Ok(result) => {
                ...
                .await;
                applied([AIBuddyEffect::ReplaceConversation {
                    conversation: compacted,
                    usage: Some(usage),
                }])
            }
