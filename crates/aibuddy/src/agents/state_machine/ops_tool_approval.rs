...
use crate::agents::state_machine::{
    applied, messages_since_kickoff, not_applicable, AIBuddyEffect, ConversationEffect, Emitter,
...
pub const TOOL_EXECUTABLE_KEY: &str = "aibuddy.executable";

pub struct ToolApprovalOperation<'a> {
    aibuddy_mode: &'a Mutex<AIBuddyMode>,
...
#[async_trait]
impl Operation<Session, AIBuddyEffect> for ToolApprovalOperation<'_> {
    ...
    async fn run(
        ...
        if !pending_requests.is_empty() {
            ...
            for request in permission_check_result.needs_approval {
                ...
                {
                    tracing::info!(
                        monotonic_counter.aibuddy.prompt_injection_user_decisions = 1,
...
fn mark_executable(tool_call_id: &str, executable: bool) -> AIBuddyEffect {
