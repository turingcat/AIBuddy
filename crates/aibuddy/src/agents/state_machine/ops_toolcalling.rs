...
use crate::agents::state_machine::{
    applied, messages_since_kickoff, not_applicable, yielded_with, AIBuddyEffect,
    ConversationEffect, Emitter, Operation, OperationResult, SlashCommand,
};
...
impl<'a> ToolExecutionOperation<'a> {
    ...
    async fn extension_state_effect(&self, session: &Session) -> Result<AIBuddyEffect> {
        ...
        Ok(AIBuddyEffect::SetExtensionData(extension_data))
    }
...
#[async_trait]
impl Operation<Session, AIBuddyEffect> for ToolExecutionOperation<'_> {
    ...
    async fn run(
        ...
        if *self.aibuddy_mode.lock().await == AIBuddyMode::Chat {
