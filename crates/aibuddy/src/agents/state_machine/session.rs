...
use aibuddy_agent::machine::{EffectHandler, EffectUsage, MachineSession, SessionLoader};
use aibuddy_agent::operation::{ConversationEffect, Emitter, MachineEffect};
...
#[async_trait]
impl EffectHandler<Session, AIBuddyEffect> for SessionManager {
    async fn apply_effects(
        &self,
        session: &Session,
        effects: &mut [AIBuddyEffect],
        emit: &Emitter,
    ) -> Result<()> {
        ...
        for effect in effects.iter_mut() {
            match effect {
                AIBuddyEffect::Conversation(ConversationEffect::AppendMessage(message)) => {
                    self.add_message(&session.id, message).await?;
                }
                AIBuddyEffect::Conversation(ConversationEffect::ReplaceConversation(
                    conversation,
                )) => {
                ...
                AIBuddyEffect::ReplaceConversation {
                    conversation,
                ...
                AIBuddyEffect::Conversation(ConversationEffect::PatchToolRequestMeta {
                    tool_call_id,
                    patch,
                }) => {
                ...
                AIBuddyEffect::Conversation(ConversationEffect::SetMessageVisibility {
                    message_id,
                    user_visible,
                    agent_visible,
                }) => {
                ...
                AIBuddyEffect::SetRecipe(recipe) => {
                    self.update(&session.id)
                        .recipe(recipe.as_ref().clone())
                        .apply()
                        .await?;
                }
                AIBuddyEffect::SetExtensionData(extension_data) => {
                    self.update(&session.id)
                        .extension_data(extension_data.clone())
                        .apply()
                        .await?;
                }
                AIBuddyEffect::RecordUsage(provider_usage) => {
        ...
        for effect in effects {
            match effect {
                ...
                AIBuddyEffect::RecordUsage(usage) => {
...
impl EffectUsage<AIBuddyEffect> for SessionManager {
    fn usage(
        &self,
        effect: &AIBuddyEffect,
    ) -> Option<aibuddy_providers::conversation::token_usage::Usage> {
        match effect {
            AIBuddyEffect::RecordUsage(usage)
            | AIBuddyEffect::ReplaceConversation {
                usage: Some(usage), ..
...
pub(crate) async fn run(
    machine: &crate::agents::state_machine::StateMachine<'_, Session, AIBuddyEffect>,
    ...
    let mut turn_usage = aibuddy_providers::conversation::token_usage::Usage::default();
    loop {
        let session = runtime.load(session_id).await?;
        let Some(mut result) = machine.step(&session, emit).await? else {
            break;
        };
        tracing::debug!(target: "aibuddy::state_machine", step = result.applied_step, "applied step");
