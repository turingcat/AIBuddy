//! AIBuddy integration for the reusable inference operation.

use std::sync::Arc;

use aibuddy_agent::inference::InferenceEffect;
pub use aibuddy_agent::inference::InferenceRunner;
use aibuddy_providers::base::{MessageStream, ModelInfo, Provider};
...
use crate::agents::state_machine::AIBuddyEffect;

pub(super) use aibuddy_agent::inference::{chat_span, record_chat_usage};

pub(super) const ADVERTISED_TOOLS_NOTE: &str = "advertised_tools";
pub(super) const LLM_OPERATION_NAME: &str = "llm";

pub struct AIBuddyInferenceProvider {
    inner: Arc<dyn Provider>,
}

impl AIBuddyInferenceProvider {
    pub fn new(inner: Arc<dyn Provider>) -> Self {
        Self { inner }
    }
}

impl InferenceEffect for AIBuddyEffect {
...
fn enrich_unclaimed_tool_errors(messages: &[Message], tools: &[rmcp::model::Tool]) -> Vec<Message> {
    ...
    for message in &mut messages {
        for content in &mut message.content {
            let aibuddy_providers::conversation::message::MessageContent::ToolResponse(response) =
                content
            else {
                continue;
            };
...
fn canonicalize_tool_request_names(
    ...
    for content in &mut message.content {
        let aibuddy_providers::conversation::message::MessageContent::ToolRequest(request) =
            content
        else {
            continue;
        };
...
#[async_trait]
impl Provider for AIBuddyInferenceProvider {
