...
use aibuddy_providers::base::Provider;
use aibuddy_providers::model::ModelConfig;
use anyhow::{anyhow, Result};
use async_trait::async_trait;

use crate::agents::state_machine::{
    messages_since_kickoff, not_applicable, yielded_with, AIBuddyEffect, ConversationEffect,
    Emitter, Operation, OperationResult, SlashCommand,
};
...
#[async_trait]
impl Operation<Session, AIBuddyEffect> for StatusOperation {
    ...
    async fn run_command(
        ...
        let response = Message::assistant().with_text(format!("**Session status**\n\n- Model: {}\n- Provider: {}\n- Mode: {}\n- Tokens (lifetime): {}\n- Context: {} / {} tokens ({})", self.model_config.model_name, self.provider.get_name(), session.aibuddy_mode, lifetime_tokens, context_tokens, context_limit, context_pct)).with_visibility(true, false);
