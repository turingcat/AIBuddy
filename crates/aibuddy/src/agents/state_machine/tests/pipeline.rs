...
use crate::agents::state_machine::{
    AIBuddyEffect, AIBuddyInferenceProvider, AIBuddyInferenceRequestPreparer, BangShellOperation,
...
impl Provider for FeatureProvider {
    ...
    ) -> Result<crate::providers::base::MessageStream, aibuddy_providers::errors::ProviderError>
    {
        self.inner
    ...
    async fn fetch_model_info(
        &self,
        model_name: &str,
    ) -> Result<aibuddy_providers::base::ModelInfo, aibuddy_providers::errors::ProviderError> {
...
pub(super) struct TestPipeline {
    ...
    aibuddy_mode: TokioMutex<AIBuddyMode>,
...
impl TestPipeline {
    pub(super) fn machine(
        ...
        let request_preparer = AIBuddyInferenceRequestPreparer {
            ...
            aibuddy_mode: &self.aibuddy_mode,
    ...
    pub(super) async fn with_aibuddy_mode(self, mode: AIBuddyMode) -> Self {
        *self.aibuddy_mode.lock().await = mode;
...
async fn test_pipeline_with_components(
    ...
    let session = session_manager
        ...
        .await?;
    let model_config = ModelConfig::new(aibuddy_providers::openai::OPEN_AI_DEFAULT_MODEL)
...
async fn build_test_pipeline(
    ...
    let api_client = aibuddy_providers::api_client::ApiClient::new_with_tls(
        api.uri(),
        aibuddy_providers::api_client::AuthMethod::NoAuth,
        None,
    )?;
    let provider: Arc<dyn Provider> = Arc::new(
        aibuddy_providers::openai::OpenAiProviderBuilder::new(api_client)
            .name(provider_name)
    ...
    let pipeline = TestPipeline {
        ...
        model_config,
        extension_manager,
        aibuddy_mode: TokioMutex::new(session.aibuddy_mode),
