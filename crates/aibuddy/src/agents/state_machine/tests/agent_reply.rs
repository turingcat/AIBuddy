...
use crate::acp::server::AIBuddyAcpAgent;
use crate::agents::extension::ExtensionConfig;
use crate::agents::mcp_client::McpClientTrait;
use crate::agents::{AIBuddyPlatform, Agent, AgentConfig, AgentEvent, SessionConfig};
...
async fn agent_with_dummy_api() -> Result<(Agent, Arc<DummyApi>, String, tempfile::TempDir)> {
    ...
    agent
        .update_provider(
            provider,
            ModelConfig::new(aibuddy_providers::openai::OPEN_AI_DEFAULT_MODEL)
...
async fn agent_with_calculator() -> Result<(
    ...
    agent
        .update_aibuddy_mode(AIBuddyMode::Approve, &session_id)
        .await?;
...
async fn state_machine_confirmation_through_agent_resumes_tool_call() -> Result<()> {
    let _guard = env_lock::lock_env([("AIBUDDY_STATE_MACHINE", Some("1"))]);
...
async fn assert_bang_shell_uses_only_user_visible_content() -> Result<()> {
    ...
    let hidden_text_prefix = AIBuddyAcpAgent::convert_acp_prompt_to_message(&[
    ...
    api.on("benign visible input")
        .reply("handled as ordinary input");
    let empty_audience_text = AIBuddyAcpAgent::convert_acp_prompt_to_message(&[
    ...
    let (agent, api, session_id, _temp_dir) = agent_with_dummy_api().await?;
    let hidden_text_suffix = AIBuddyAcpAgent::convert_acp_prompt_to_message(&[
    ...
    api.on("benign visible input")
        .reply("handled as ordinary input");
    let hidden_resource_prefix = AIBuddyAcpAgent::convert_acp_prompt_to_message(&[
    ...
    api.on("benign visible input")
        .reply("handled as ordinary input");
    let empty_audience_resource = AIBuddyAcpAgent::convert_acp_prompt_to_message(&[
    ...
    let hidden_link_suffix = AIBuddyAcpAgent::convert_acp_prompt_to_message(&[
        AcpContentBlock::Text(AcpTextContent::new("!echo visible")),
        hidden_link,
    ]);
