use aibuddy_providers::conversation::token_usage::{ProviderUsage, Usage as ProviderTokenUsage};
use anyhow::Result;
...
async fn proactive_and_manual_compaction_continue_with_replaced_usage() -> Result<()> {
    ...
    let apply = async |effects: Vec<state_machine::AIBuddyEffect>| -> Result<()> {
    ...
    apply(vec![
        replacement.into(),
        state_machine::AIBuddyEffect::RecordUsage(usage),
        Message::assistant()
...
async fn tokenless_provider_compacts_estimated_context() -> Result<()> {
    ...
    let pipeline = pipeline
        .with_model_config(
            aibuddy_providers::model::ModelConfig::new("gpt-4.1").with_context_limit(Some(200)),
        )
        .await;
