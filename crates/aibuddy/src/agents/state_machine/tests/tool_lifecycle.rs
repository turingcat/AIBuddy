...
async fn toolshim_provider_turn_executes_advertised_tools() -> Result<()> {
    let (pipeline, api) = test_pipeline().await?;
    let model_config = aibuddy_providers::model::ModelConfig::new(
        aibuddy_providers::openai::OPEN_AI_DEFAULT_MODEL,
    )
...
async fn approvals_and_per_tool_permissions() -> Result<()> {
    let (pipeline, api) = test_pipeline().await?;
    let pipeline = pipeline.with_aibuddy_mode(AIBuddyMode::Approve).await;
    ...
    let pipeline = pipeline.with_aibuddy_mode(AIBuddyMode::Auto).await;
...
async fn tool_availability_tracks_mode_and_extension_removal() -> Result<()> {
    ...
    let pipeline = pipeline.with_aibuddy_mode(AIBuddyMode::Chat).await;
