...
use crate::agents::mcp_client::{
    AIBuddyMcpClientCapabilities, AIBuddyMcpHostInfo, McpClient, McpClientTrait,
};
...
use crate::oauth::{
    oauth_flow, oauth_flow_with_challenge, AIBuddyCredentialStore, StaticOAuthClientConfig,
};
...
fn resolve_timeout(timeout: Option<u64>) -> u64 {
    timeout.unwrap_or_else(|| {
        Config::global()
            .get_aibuddy_default_extension_timeout()
...
pub struct ExtensionManagerCapabilities {
    pub mcpui: bool,
    pub host_info: Option<AIBuddyMcpHostInfo>,
...
pub struct AIBuddyMcpAppToolAttachment {
...
pub(crate) const TRUSTED_TOOL_UPDATE_META_KEY: &str = "__aibuddy_tool_update_meta";

/// Manages aibuddy extensions / MCP clients and their interactions
...
const TOOL_EXTENSION_META_KEY: &str = "aibuddy_extension";
...
pub(crate) fn recover_mangled_tool_name<'a>(
    ...
    for (name, owner) in tools {
        // Prefixed tools: the model turns AIBuddy's "__" separator into a dot
...
fn remove_untrusted_mcp_app_meta(result: &mut CallToolResult) {
    ...
    let remove_aibuddy = meta
        .0
        .get_mut("aibuddy")
        .and_then(Value::as_object_mut)
        .map(|aibuddy_meta| {
            aibuddy_meta.remove("mcpApp");
            aibuddy_meta.is_empty()
        })
        .unwrap_or(false);

    if remove_aibuddy {
        meta.0.remove("aibuddy");
    }
...
fn insert_trusted_tool_update_meta(
    ...
    attachment: &AIBuddyMcpAppToolAttachment,
) {
...
async fn child_process_client(
    ...
    client_name: String,
    capabilities: AIBuddyMcpClientCapabilities,
...
const AIBUDDY_USER_AGENT: reqwest::header::HeaderValue =
    reqwest::header::HeaderValue::from_static(concat!("aibuddy/", env!("CARGO_PKG_VERSION")));
...
async fn connect_with_auth(
    ...
    auth_headers.insert(reqwest::header::USER_AGENT, AIBUDDY_USER_AGENT);
...
async fn create_streamable_http_client(
    ...
    default_headers.insert(reqwest::header::USER_AGENT, AIBUDDY_USER_AGENT);
...
async fn create_unix_socket_http_client(
    ...
    custom_headers.insert(
        ...
        AIBUDDY_USER_AGENT
            ...
            .unwrap_or("aibuddy")
            .parse()
            .unwrap_or_else(|_| HeaderValue::from_static("aibuddy")),
    );
...
impl ExtensionManager {
    fn mcp_client_capabilities(&self) -> AIBuddyMcpClientCapabilities {
        AIBuddyMcpClientCapabilities {
    ...
    pub async fn add_extension(
        ...
        let effective_working_dir = working_dir
            .clone()
            .or_else(|| std::env::var("AIBUDDY_WORKING_DIR").ok().map(PathBuf::from))
            ...
            ExtensionConfig::StreamableHttp {
                ...
                create_streamable_http_client(
                    ...
                    *timeout,
                    ...
                    static_oauth_client,
                    Box::new(AIBuddyCredentialStore::new(name.to_string())),
                    self.provider.clone(),
    ...
    async fn hydrate_mcp_app_attachment(
        ...
        cancellation_token: CancellationToken,
    ) -> Option<AIBuddyMcpAppToolAttachment> {
        let resource_uri = resolved_tool.resource_uri.clone()?;

        let mut attachment = AIBuddyMcpAppToolAttachment {
...
mod tests {
    ...
    fn test_insert_trusted_tool_update_meta_stores_backend_payload() {
        let mut result = CallToolResult::success(vec![]);
        let attachment = AIBuddyMcpAppToolAttachment {
    ...
    async fn test_invalid_header_name_returns_config_error() {
        ...
        let capabilities = AIBuddyMcpClientCapabilities {
            mcpui: false,
            host_info: None,
            elicitation_handler: None,
            protocol_version: None,
