...
/// respecting `AIBUDDY_SHELL`.
///
/// Defaults to `cmd` when `AIBUDDY_SHELL` is unset. The invocation flags are
...
fn windows_shell() -> String {
    std::env::var("AIBUDDY_SHELL").unwrap_or_else(|_| "cmd".to_string())
}
...
fn unix_shell() -> String {
    if let Ok(shell) = std::env::var("AIBUDDY_SHELL") {
        return shell;
    }
...
///
/// When aibuddyd is launched from a desktop app (e.g. Electron), it may inherit
...
pub(crate) fn resolve_login_shell_path() -> Option<String> {
    ...
    // (TIOCSPGRP) cannot steal the terminal foreground from aibuddy, which
    // would cause aibuddy to receive SIGTTIN and be suspended on startup.
        ...
        if shell_basename(&windows_shell()) == "cmd" && params.command.contains(['\n', '\r']) {
            return Self::error_result(
                "cmd.exe silently truncates commands at newlines — only the first line executes, \
                 ...
                 (e.g. `echo a & echo b`), or set AIBUDDY_SHELL=powershell.",
                None,
            );
        }
...
/// name, so setting `AIBUDDY_SHELL` to a bare executable is enough — users never
