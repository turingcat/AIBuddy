...
use crate::aibuddy_apps::McpAppResource;
use crate::aibuddy_apps::{AIBuddyApp, McpAppCache, WindowProps};
...
const CLOCK_HTML: &str = include_str!("../../aibuddy_apps/clock.html");
...
impl AppsManagerClient {
    ...
    fn ensure_default_apps(&self) -> Result<(), String> {
        ...
        if !clock_path.exists() {
            ...
            let clock_app = AIBuddyApp::from_html(CLOCK_HTML)?;
    ...
    fn load_app(&self, name: &str) -> Result<AIBuddyApp, String> {
        let path = self.app_path(name)?;
        if name == CLOCK_APP_NAME {
            return AIBuddyApp::from_html(CLOCK_HTML);
        }

        let html =
            fs::read_to_string(&path).map_err(|e| format!("Failed to read app file: {}", e))?;

        AIBuddyApp::from_html(&html)
    }

    fn load_editable_app(&self, name: &str) -> Result<AIBuddyApp, String> {
        if name == CLOCK_APP_NAME {
    ...
    fn save_app(&self, app: &AIBuddyApp) -> Result<(), String> {
    ...
    fn create_app_content_tool() -> rmcp::model::Tool {
        rmcp::model::Tool::new(
            ...
            "Generate content for a new AIBuddy app. Returns the HTML code, app name, description, and window properties.".to_string(),
    ...
    fn update_app_content_tool() -> rmcp::model::Tool {
        rmcp::model::Tool::new(
            ...
            "Generate updated content for an existing AIBuddy app. Returns the improved HTML code, updated description, and optionally updated window properties.".to_string(),
...
impl McpClientTrait for AppsManagerClient {
    async fn list_tools(
        ...
        let tools = vec![
            McpTool::new(
                "list_apps".to_string(),
                "List all available AIBuddy apps with their names and descriptions. Use this to see what apps exist before creating or modifying apps.".to_string(),
                schema::<ListAppsParams>(),
            ),
            model_only_tool(McpTool::new(
                "create_app".to_string(),
                "Create a new AIBuddy app based on a description or PRD. The extension will use an LLM to generate the HTML/CSS/JavaScript. Apps are sandboxed and run in standalone windows.".to_string(),
...
mod tests {
    ...
    fn test_app(name: &str) -> AIBuddyApp {
        AIBuddyApp {
            resource: McpAppResource {
    ...
    async fn bundled_default_apps_cannot_be_modified_or_deleted() {
        ...
        let compiled_clock = AIBuddyApp::from_html(CLOCK_HTML).unwrap();
