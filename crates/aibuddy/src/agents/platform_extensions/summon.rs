...
use aibuddy_agent::operation::messages_since_kickoff;
use aibuddy_sdk_types::custom_requests::{SourceEntry, SourceType};
use anyhow::Result;
...
pub fn discover_filesystem_sources(working_dir: &Path) -> Vec<SourceEntry> {
    ...
    let local_recipe_dirs: Vec<PathBuf> = vec![
        working_dir.join(".aibuddy/recipes"),
        working_dir.join(".agents/recipes"),
    ];

    let global_recipe_dirs: Vec<PathBuf> = std::env::var("AIBUDDY_RECIPE_PATH")
        .ok()
        .into_iter()
        .flat_map(|p| {
        ...
        .chain(
            [
                home.as_ref().map(|h| h.join(".aibuddy/recipes")),
...
fn max_background_tasks() -> usize {
    Config::global()
        .get_param::<usize>("AIBUDDY_MAX_BACKGROUND_TASKS")
        .unwrap_or(5)
}

fn completed_task_ttl() -> Duration {
    let secs = Config::global()
        .get_param::<u64>("AIBUDDY_COMPLETED_TASK_TTL_SECS")
        .unwrap_or(600);
...
impl SummonClient {
        ...
        let schema = serde_json::json!({
            ...
            "properties": {
                ...
                "max_turns": {
                    "type": "integer",
                    "minimum": 1,
                    "description": "Maximum turns for this delegate. Overrides recipe settings.max_turns and AIBUDDY_SUBAGENT_MAX_TURNS."
                },
                "context": {
                    "type": "string",
        ...
        if sources.is_empty() && completed.is_empty() {
            return Ok(vec![ContentBlock::text(
                "No sources available for load/delegate.\n\n\
                 ...
                 • AIBUDDY_RECIPE_PATH directories",
            )]);
        }
    ...
    async fn handle_delegate(
        ...
        let mut agent_config = AgentConfig::new(
            ...
            crate::agents::AIBuddyPlatform::AIBuddyCli,
        )
    ...
    fn build_recipe_from_agent(
        ...
        let settings = model.map(|m| Settings {
            aibuddy_model: Some(m),
            aibuddy_provider: params.provider.clone(),
    ...
    fn resolve_model_config(
    ...
    ) -> Result<aibuddy_providers::model::ModelConfig, anyhow::Error> {
        let env_model = std::env::var("AIBUDDY_SUBAGENT_MODEL").ok();
        let recipe_settings = recipe.settings.as_ref();
        let configured = Config::global().all_values().ok();
        let configured_provider = configured
            .as_ref()
            .and_then(|values| values.get("AIBUDDY_SUBAGENT_PROVIDER"))
            .and_then(serde_json::Value::as_str);
        let configured_model = configured
            .as_ref()
            .and_then(|values| values.get("AIBUDDY_SUBAGENT_MODEL"))
            .and_then(serde_json::Value::as_str);
        ...
        let model = env_model
            ...
            .or_else(|| {
                recipe_settings
                    .and_then(|settings| settings.aibuddy_model.clone())
                    .filter(|_| {
                        matches_provider(
                            recipe_settings
                                .and_then(|settings| settings.aibuddy_provider.as_deref()),
                        )
                    })
            })
            .or_else(|| {
            ...
            .ok_or_else(|| {
                anyhow::anyhow!(
                    "No model configured for provider '{}'; set AIBUDDY_SUBAGENT_MODEL",
                    provider_name
                )
            })?;
    ...
    async fn resolve_provider(
        ...
        anyhow::Error,
    > {
        let env_provider = std::env::var("AIBUDDY_SUBAGENT_PROVIDER").ok();
        let provider_name = env_provider
            ...
            .or_else(|| {
                recipe
                    .settings
                    .as_ref()
                    .and_then(|s| s.aibuddy_provider.clone())
            })
            .or_else(|| {
                Config::global()
                    .get_param::<String>("AIBUDDY_SUBAGENT_PROVIDER")
                    .ok()
            })
    ...
    fn resolve_max_turns(&self, session: &crate::session::Session) -> usize {
        session
            ...
            .or_else(|| {
                std::env::var("AIBUDDY_SUBAGENT_MAX_TURNS")
                    .ok()
                    .and_then(|v| v.parse().ok())
            })
            .or_else(|| {
                Config::global()
                    .get_param::<usize>("AIBUDDY_SUBAGENT_MAX_TURNS")
                    .ok()
            })
...
You review code."#;
    ...
    async fn test_discover_recipes_and_agents() {
        ...
        let recipes = temp_dir.path().join(".aibuddy/recipes");
        ...
        let agents = temp_dir.path().join(".aibuddy/agents");
    ...
    async fn test_recipe_deduplication_local_wins() {
        ...
        let local = temp_dir.path().join(".aibuddy/recipes");
    ...
    fn test_resolve_max_turns_recipe_overrides_env_var() {
        ...
        std::env::set_var("AIBUDDY_SUBAGENT_MAX_TURNS", "99");
        let result = client.resolve_max_turns(&session);
        std::env::remove_var("AIBUDDY_SUBAGENT_MAX_TURNS");
    ...
    fn test_resolve_max_turns_falls_back_to_env_var() {
        ...
        std::env::set_var("AIBUDDY_SUBAGENT_MAX_TURNS", "7");
        let result = client.resolve_max_turns(&session);
        std::env::remove_var("AIBUDDY_SUBAGENT_MAX_TURNS");

        assert_eq!(
            result, 7,
            "should fall back to AIBUDDY_SUBAGENT_MAX_TURNS env var"
        );
    }
    ...
    async fn test_resolve_provider_reuses_unregistered_parent_provider() {
        ...
        *extension_manager.get_provider().lock().await = Some(Arc::clone(&parent_provider));
        ...
        let session = crate::session::Session {
            ...
            model_config: Some(aibuddy_providers::model::ModelConfig::new("test-model")),
            ..Default::default()
        };
    ...
    fn session_with(parent: aibuddy_providers::model::ModelConfig) -> crate::session::Session {
    ...
    fn resolve_with_override(
        model: Option<&str>,
        parent: aibuddy_providers::model::ModelConfig,
    ) -> aibuddy_providers::model::ModelConfig {
        let client = SummonClient::new(create_test_context()).unwrap();
        let params = DelegateParams {
    ...
    fn parent_config() -> aibuddy_providers::model::ModelConfig {
        aibuddy_providers::model::ModelConfig::new(PARENT_MODEL).with_canonical_limits(PROVIDER)
    }
    ...
    async fn test_resolve_model_config_applies_canonical_limits_to_overridden_model() {
        let _env = env_lock::lock_env([
            ...
            ("AIBUDDY_SUBAGENT_MODEL", None::<&str>),
        ]);

        let parent = parent_config();
        let overridden = aibuddy_providers::model::ModelConfig::new(OVERRIDE_MODEL)
    ...
    async fn test_resolve_model_config_env_var_overrides_params_model() {
        let _env = env_lock::lock_env([
            ("AIBUDDY_CONTEXT_LIMIT", None::<&str>),
            ("AIBUDDY_MAX_TOKENS", None::<&str>),
            ("AIBUDDY_SUBAGENT_MODEL", Some(OVERRIDE_MODEL)),
        ]);
        ...
        assert_eq!(
            ...
            "AIBUDDY_SUBAGENT_MODEL must take priority over params.model"
        );
    }
    ...
    async fn test_resolve_model_config_env_var_overrides_recipe_model() {
        ...
        recipe.settings = Some(crate::recipe::Settings {
            aibuddy_provider: None,
            aibuddy_model: Some("recipe-model".to_string()),
            temperature: None,
            max_turns: None,
        });
        let result = client
        ...
        assert_eq!(
            result.model_name, OVERRIDE_MODEL,
            "AIBUDDY_SUBAGENT_MODEL must take priority over recipe settings"
        );
    }
    ...
    async fn test_resolve_model_config_env_provider_uses_provider_default_model() {
        let _env = env_lock::lock_env([
            ("AIBUDDY_CONTEXT_LIMIT", None::<&str>),
            ("AIBUDDY_MAX_TOKENS", None::<&str>),
            ("AIBUDDY_SUBAGENT_PROVIDER", Some(PROVIDER)),
            ("AIBUDDY_SUBAGENT_MODEL", None::<&str>),
        ...
        let mut recipe = empty_recipe();
        recipe.settings = Some(crate::recipe::Settings {
            aibuddy_provider: Some("openai".to_string()),
            aibuddy_model: Some("recipe-model-for-another-provider".to_string()),
            temperature: None,
            max_turns: None,
        });
        ...
        let session = crate::session::Session {
            ...
            model_config: Some(aibuddy_providers::model::ModelConfig::new(
                "parent-openai-model",
            )),
            ..Default::default()
        };
