...
use super::mcp_client::AIBuddyMcpHostInfo;
...
use crate::agents::state_machine::{
    ...
    persist_tool_confirmation_decision, run_aibuddy, AIBuddyEffect, AIBuddyInferenceProvider,
    AIBuddyInferenceRequestPreparer, BangShellOperation, CompactionOperation, DoctorOperation,
    Emitter, EntryHookOperation, ExitOnErrorOperation, InferenceRunner, MaxTurnsOperation,
...
use crate::config::{get_enabled_extensions, AIBuddyMode, Config};
...
use aibuddy_providers::thinking::{ThinkingEffort, ThinkingEffortSupport};
use regex::Regex;
use rmcp::model::{
...
const COMPACTION_PROGRESS_TEXT: &str = "aibuddy is compacting the conversation...";
...
fn normalize_legacy_provider_thinking_effort(
    mut model_config: aibuddy_providers::model::ModelConfig,
    effort_support: &ThinkingEffortSupport,
) -> aibuddy_providers::model::ModelConfig {
    ...
    model_config.with_default_thinking_effort(Config::global().get_aibuddy_thinking_effort())
}
...
pub(crate) fn stop_hook_block_cap_warning(plugin: &str, cap: u32) -> Message {
    Message::assistant().with_system_notification(
        SystemNotificationType::InlineMessage,
        format!(
            "Stop hook `{plugin}` blocked the turn from ending more than {cap} consecutive times — overriding and ending turn to avoid an infinite loop. Set AIBUDDY_STOP_HOOK_BLOCK_CAP to raise this limit."
        ),
    )
}
...
pub struct ReplyContext {
    ...
    pub model_config: aibuddy_providers::model::ModelConfig,
}
...
#[derive(Clone, Debug)]
pub enum AIBuddyPlatform {
    AIBuddyDesktop,
    AIBuddyCli,
}

impl fmt::Display for AIBuddyPlatform {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            AIBuddyPlatform::AIBuddyCli => write!(f, "aibuddy-cli"),
            AIBuddyPlatform::AIBuddyDesktop => write!(f, "aibuddy-desktop"),
        }
    }
}
...
pub struct AgentConfig {
    ...
    pub mcp_host_info: Option<AIBuddyMcpHostInfo>,
...
impl AgentConfig {
    pub fn new(
        ...
        aibuddy_platform: AIBuddyPlatform,
    ) -> Self {
        Self {
            session_manager,
            permission_manager,
    ...
    pub fn with_mcp_host_info(mut self, mcp_host_info: Option<AIBuddyMcpHostInfo>) -> Self {
    ...
    fn resolve_use_login_shell_path(&self) -> bool {
        resolve_use_login_shell_path(self.use_login_shell_path, &self.aibuddy_platform)
    }
}

fn resolve_use_login_shell_path(explicit: Option<bool>, platform: &AIBuddyPlatform) -> bool {
    explicit.unwrap_or(matches!(platform, AIBuddyPlatform::AIBuddyDesktop))
}

/// The main aibuddy Agent
pub struct Agent {
    pub(super) provider: SharedProvider,
    pub config: AgentConfig,
    pub(super) current_aibuddy_mode: Mutex<AIBuddyMode>,
...
impl Agent {
    pub fn new() -> Self {
        ...
        Self::with_config(AgentConfig::new(
            ...
            config.get_aibuddy_disable_session_naming().unwrap_or(false),
    ...
    pub fn with_config(config: AgentConfig) -> Self {
        ...
        let aibuddy_platform = config.aibuddy_platform.clone();
        let initial_mode = config.aibuddy_mode;
        ...
        let mcpui = explicit_mcp_host_info
            ...
            .map(AIBuddyMcpHostInfo::mcpui_enabled)
            .unwrap_or_else(|| match config.aibuddy_platform {
                AIBuddyPlatform::AIBuddyDesktop => true,
                AIBuddyPlatform::AIBuddyCli => false,
            });
        ...
        let client_name = explicit_mcp_host_info
            ...
            .unwrap_or_else(|| aibuddy_platform.to_string());
        ...
        Self {
            ...
            config,
            current_aibuddy_mode: Mutex::new(initial_mode),
    ...
    pub(crate) fn stop_hook_block_cap(&self) -> u32 {
        ...
        Config::global()
            .get_param::<u32>("AIBUDDY_STOP_HOOK_BLOCK_CAP")
    ...
    fn create_tool_inspection_manager(
        ...
        // Add adversary inspector (LLM-based review, enabled by ~/.config/aibuddy/adversary.md)
    ...
    async fn prepare_reply_context(
        ...
        let aibuddy_mode = *self.current_aibuddy_mode.lock().await;

        let tool_call_cut_off = match Config::global()
            .get_param::<usize>("AIBUDDY_TOOL_CALL_CUTOFF")
        {
            Ok(v) => v,
            Err(_) => {
                let context_limit = match self.provider().await {
                    ...
                    .await
                    .unwrap_or(aibuddy_providers::model::DEFAULT_CONTEXT_LIMIT),
                    Err(_) => aibuddy_providers::model::DEFAULT_CONTEXT_LIMIT,
                };
                let compaction_threshold = Config::global()
                    .get_param::<f64>("AIBUDDY_AUTO_COMPACT_THRESHOLD")
    ...
    pub(super) fn create_state_machine(
    ...
    ) -> StateMachine<'_, Session, AIBuddyEffect> {
        let max_turns = max_turns.unwrap_or_else(|| {
            Config::global()
                .get_param::<u32>("AIBUDDY_MAX_TURNS")
                .unwrap_or(DEFAULT_MAX_TURNS)
        });
        let retry_timeout = Config::global()
            .get_param::<u64>("AIBUDDY_RECIPE_RETRY_TIMEOUT_SECONDS")
            .unwrap_or(DEFAULT_RETRY_TIMEOUT_SECONDS);
        let on_failure_timeout = Config::global()
            .get_param::<u64>("AIBUDDY_RECIPE_ON_FAILURE_TIMEOUT_SECONDS")
        ...
        let mut operations: Vec<Arc<dyn Operation<Session, AIBuddyEffect> + '_>> = vec![
        ...
        let remaining_operations: Vec<Arc<dyn Operation<Session, AIBuddyEffect> + '_>> = vec![
        ...
        let request_preparer = AIBuddyInferenceRequestPreparer {
            #[cfg(feature = "code-mode")]
            extension_manager: self.extension_manager.clone(),
            aibuddy_mode: &self.current_aibuddy_mode,
        ...
        let inference_provider = Arc::new(AIBuddyInferenceProvider::new(provider));
        ...
        let command_operation: Arc<dyn Operation<Session, AIBuddyEffect> + '_> =
        ...
        let operations: Vec<_> =
            std::iter::once(Arc::new(EntryHookOperation::new(self.hook_manager.clone()))
                as Arc<dyn Operation<Session, AIBuddyEffect> + '_>)
            ...
            async_stream::try_stream! {
                ...
                let result = {
                    let run = crate::session_context::with_session_id(
                        ...
                        run_aibuddy(&machine, session_manager.as_ref(), &session_id, &emit),
                    );
                    tokio::pin!(run);
                    loop {
                        tokio::select! {
                    ...
                    match next {
                        Ok((response, usage)) => {
                            ...
                            if let Some(response) = response {
                                ...
                                if aibuddy_mode == AIBuddyMode::Chat {
                                    for request in &tool_requests {
    ...
    pub async fn update_aibuddy_mode(&self, mode: AIBuddyMode, session_id: &str) -> Result<()> {
        if let Some(provider) = self.provider.lock().await.as_ref() {
            provider
        ...
        *self.current_aibuddy_mode.lock().await = mode;
        self.config
            .session_manager
            .clone()
            .update(session_id)
            ...
            .apply()
            .await
            .context("Failed to persist aibuddy_mode to session")
    }

    pub async fn aibuddy_mode(&self) -> AIBuddyMode {
        *self.current_aibuddy_mode.lock().await
    }
    ...
    pub async fn recreate_provider_for_session(
        ...
        let mode = self.aibuddy_mode().await;
        self.update_aibuddy_mode(mode, session_id).await
    }
    ...
    pub async fn restore_provider_from_session(&self, session: &Session) -> Result<bool> {
        ...
        let provider_name = session
            .provider_name
            .clone()
            .or_else(|| config.get_aibuddy_provider().ok())
        ...
        let (provider, active_model_config, provider_changed) =
                ...
                .is_ok()
            {
                ...
                let fallback_model_name = config.get_aibuddy_model().ok().ok_or_else(|| {
        ...
        if let Some(provider) = self.provider.lock().await.as_ref() {
            provider
                .update_mode(&session.id, session.aibuddy_mode)
                .await
                .map_err(|e| anyhow!("Failed to propagate mode to provider: {}", e))?;
        }
        *self.current_aibuddy_mode.lock().await = session.aibuddy_mode;
        Ok(provider_changed)
    }
    ...
    pub async fn create_recipe(
        ...
        let system_prompt = prompt_manager
            ...
            .with_aibuddy_mode(aibuddy_mode)
            .build();
        ...
        let settings = Settings {
            aibuddy_provider: Some(provider_name.clone()),
            aibuddy_model: Some(model_name.clone()),
...
mod tests {
    ...
    async fn capture_tool_dispatch_fields(
        ...
        use aibuddy_test_support::otel::clear_otel_env;
        use rmcp::object;
    ...
    #[tokio::test]
    #[serial_test::serial(tracing_capture)]
    ...
    fn resolve_use_login_shell_path_defaults_by_platform() {
        assert!(resolve_use_login_shell_path(
            None,
            &AIBuddyPlatform::AIBuddyDesktop
        ));
        assert!(!resolve_use_login_shell_path(
            None,
            &AIBuddyPlatform::AIBuddyCli
    ...
    impl crate::providers::base::Provider for ActionRequiredProvider {
        ...
        async fn stream(
            &self,
            _: &aibuddy_providers::model::ModelConfig,
            _: &str,
        ...
        fn thinking_effort_support(&self) -> ThinkingEffortSupport {
            if self.applies_effort {
                ThinkingEffortSupport::Options(
                    aibuddy_providers::thinking::ThinkingEffortCapability {
                        option_id: "effort".to_string(),
                        values: vec![aibuddy_providers::thinking::ThinkingEffortOption {
    ...
    async fn effort_test_agent(
        ...
        agent
            .update_provider(
                provider.clone(),
                aibuddy_providers::model::ModelConfig::new("mock-model"),
                &session.id,
            )
            .await
            .unwrap();
    ...
    async fn update_provider_replaces_harness_only_effort_for_legacy_provider() {
        ...
        let model_config =
            aibuddy_providers::model::ModelConfig::new("mock-model").with_merged_request_params(
...
"#,
    ...
    async fn capture_legacy_reply_fields(
        ...
        let mut overrides = vec![("AIBUDDY_STATE_MACHINE", "0")];
    ...
    async fn legacy_provider_request_uses_aibuddy_chinese_identity() -> Result<()> {
        let _guard = env_lock::lock_env([("AIBUDDY_STATE_MACHINE", None::<&str>)]);
        let temp_dir = tempfile::tempdir()?;
        let hook_manager = crate::hooks::HookManager::from_plugins_for_test(vec![]);
                ...
                matches!(
                    ...
                    MessageContent::SystemNotification(notification)
                        if notification.msg.contains("more than 2 consecutive times")
                            && notification.msg.contains("AIBUDDY_STOP_HOOK_BLOCK_CAP")
                )
            })
        }));
    ...
    async fn test_add_final_output_tool() -> Result<()> {
        ...
        let system_prompt = prompt_manager
            .builder()
            .with_aibuddy_mode(AIBuddyMode::default())
            .build();
