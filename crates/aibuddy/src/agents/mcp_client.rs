...
/// MCP client implementation for AIBuddy
#[expect(deprecated)]
...
fn resolve_sampling_model_config() -> anyhow::Result<aibuddy_providers::model::ModelConfig> {
    let config = crate::config::Config::global();
    let provider_name = config.get_aibuddy_provider()?;
    let model_name = config.get_aibuddy_model()?;
...
impl AIBuddyClient {
    ...
    fn resolved_client_info(&self) -> Implementation {
        ...
        let version = self
            ...
            .unwrap_or_else(|| {
                std::env::var("AIBUDDY_MCP_CLIENT_VERSION")
...
impl ClientHandler for AIBuddyClient {
    #[expect(deprecated)]
    async fn list_roots(
        &self,
    ...
    async fn create_message(
        ...
        let system_prompt = params
            .system_prompt
            .as_deref()
            .unwrap_or("You are a general-purpose AI agent called aibuddy");
...
pub struct AIBuddyMcpClientCapabilities {
    pub mcpui: bool,
...
impl std::fmt::Debug for AIBuddyMcpClientCapabilities {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter
            .debug_struct("AIBuddyMcpClientCapabilities")
...
pub struct McpClient {
    client: Mutex<Arc<RunningService<RoleClient, AIBuddyClient>>>,
...
impl McpClient {
        ...
        E: std::error::Error + From<std::io::Error> + Send + Sync + 'static,
    {
        ...
        let client: rmcp::service::RunningService<rmcp::RoleClient, AIBuddyClient> =
...
mod tests {
    ...
    use crate::agents::AIBuddyPlatform;
    use rmcp::model::Tool;
    use serde_json::json;
    ...
    fn new_client(platform: AIBuddyPlatform) -> AIBuddyClient {
        let capabilities = match platform {
            AIBuddyPlatform::AIBuddyDesktop => AIBuddyMcpClientCapabilities {
                mcpui: true,
                host_info: None,
                elicitation_handler: None,
                protocol_version: None,
            },
            AIBuddyPlatform::AIBuddyCli => AIBuddyMcpClientCapabilities {
                mcpui: false,
                host_info: None,
                elicitation_handler: None,
                protocol_version: None,
    ...
    async fn tool_list_changed_during_fetch_prevents_stale_cache() {
        ...
        let aibuddy_client = AIBuddyClient::new(
            Arc::new(Mutex::new(Vec::new())),
            Arc::new(Mutex::new(None)),
        ...
        aibuddy_client.handle_tool_list_changed().await;
    ...
    fn test_resolve_session_id(
        ...
        runtime.block_on(async {
            let client = new_client(AIBuddyPlatform::AIBuddyCli);
    ...
    fn test_request_injects_session(request_builder: fn(Extensions) -> ClientRequest) {
        ...
        extensions.insert(
            serde_json::from_value::<MetaObject>(json!({
                "AIBuddy-Session-Id": "old-session-id",
    ...
    fn test_client_info_advertises_mcp_apps_ui_extension() {
        let client = new_client(AIBuddyPlatform::AIBuddyDesktop);
    ...
    fn test_explicit_host_info_passes_through_client_identity() {
        let client = AIBuddyClient::new(
            Arc::new(Mutex::new(Vec::new())),
            Arc::new(Mutex::new(None)),
            AIBuddyPlatform::AIBuddyDesktop.to_string(),
            AIBuddyMcpClientCapabilities {
                mcpui: true,
                host_info: Some(AIBuddyMcpHostInfo {
                    explicit_extensions: true,
                    extensions: ExtensionCapabilities::new(),
                    client_name: Some("aibuddy2".to_string()),
        ...
        let info = ClientHandler::get_info(&client);
        assert_eq!(info.client_info.name, "aibuddy2");
    ...
    fn test_explicit_host_extensions_override_platform_fallback() {
        let client = AIBuddyClient::new(
            Arc::new(Mutex::new(Vec::new())),
            Arc::new(Mutex::new(None)),
            AIBuddyPlatform::AIBuddyCli.to_string(),
            AIBuddyMcpClientCapabilities {
                mcpui: false,
                host_info: Some(AIBuddyMcpHostInfo {
                    explicit_extensions: true,
