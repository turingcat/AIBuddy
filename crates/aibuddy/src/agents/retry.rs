...
const AIBUDDY_RECIPE_RETRY_TIMEOUT_SECONDS: &str = "AIBUDDY_RECIPE_RETRY_TIMEOUT_SECONDS";

/// Environment variable for configuring on_failure timeout globally
const AIBUDDY_RECIPE_ON_FAILURE_TIMEOUT_SECONDS: &str = "AIBUDDY_RECIPE_ON_FAILURE_TIMEOUT_SECONDS";
...
fn get_retry_timeout(retry_config: &RetryConfig) -> Duration {
    let timeout_seconds = retry_config
        ...
        .or_else(|| {
            ...
            config.get_param(AIBUDDY_RECIPE_RETRY_TIMEOUT_SECONDS).ok()
        })
...
fn get_on_failure_timeout(retry_config: &RetryConfig) -> Duration {
    let timeout_seconds = retry_config
        ...
        .or_else(|| {
            let config = Config::global();
            config
                .get_param(AIBUDDY_RECIPE_ON_FAILURE_TIMEOUT_SECONDS)
                .ok()
        })
...
pub async fn execute_shell_command(
    ...
    let future = async {
        let mut cmd = if cfg!(target_os = "windows") {
            ...
            cmd.args(["/C", command]);
            cmd.env("AIBUDDY_TERMINAL", "1");
            cmd.env("AGENT", "aibuddy");
            cmd
        } else {
