...
static TEMPLATE_REGISTRY: &[(&str, &str)] = &[
    (
        "system.md",
        "Main system prompt that defines aibuddy's personality and behavior",
    ),
    (
        "compaction.md",
    ...
    (
        "apps_create.md",
        "Prompt for generating new AIBuddy apps based on the user instructions",
    ),
    (
        "apps_iterate.md",
        "Prompt for updating existing AIBuddy apps based on feedback",
    ),
    (
        "permission_judge.md",
    ...
    (
        "plan.md",
        "Prompt used when aibuddy creates step-by-step plans. CLI only",
    ),
    (
...
fn builtin_content(name: &str) -> Option<String> {
    CORE_PROMPTS_DIR
        ...
        .or_else(|| aibuddy_context_management::templates::builtin_template(name))
}
