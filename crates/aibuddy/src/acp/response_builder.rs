...
///
/// Replaces the removed `SessionModelState` ACP schema type; aibuddy now surfaces
...
pub(super) fn build_mode_state(
    current_mode: AIBuddyMode,
) -> Result<SessionModeState, agent_client_protocol::Error> {
    let mut available = Vec::with_capacity(AIBuddyMode::VARIANTS.len());
    for &name in AIBuddyMode::VARIANTS {
        let aibuddy_mode: AIBuddyMode = name.parse().map_err(|_| {
            agent_client_protocol::Error::internal_error() // impossible but satisfy linters
                .data(format!("Failed to parse AIBuddyMode variant: {}", name))
        })?;
        let mut mode = SessionMode::new(SessionModeId::new(name), name);
        mode.description = aibuddy_mode.get_message().map(Into::into);
        available.push(mode);
    }
...
/// The provider decides whether aibuddy owns the effort menu; a session without a
...
pub(super) async fn build_session_setup_config(
    ...
    let mode_state = build_mode_state(session.aibuddy_mode)?;
...
/// aibuddy fall back to the model-name based values, which the ACP sentinel model
...
/// The aibuddy-side value that will actually be sent wins — `resolve_effort_value`
/// is shared with the provider's send path — then whatever the agent currently
/// has, which is what it keeps when aibuddy sends nothing.
...
fn current_thinking_effort_value(model_config: &ModelConfig) -> String {
    if model_config.is_reasoning_model() {
        model_config
            .thinking_effort()
            .or_else(|| Config::global().get_aibuddy_thinking_effort())
...
mod tests {
    ...
    use aibuddy_providers::thinking::ThinkingEffortOption;
    ...
    #[test_case(
        build_mode_state(AIBuddyMode::Auto).unwrap(),
        "openai",
        vec![
    ...
    #[test_case(
        build_mode_state(AIBuddyMode::Approve).unwrap(),
        "openai",
    ...
    fn test_build_config_options_uses_current_thinking_effort() {
        let mode_state = build_mode_state(AIBuddyMode::Auto).unwrap();
