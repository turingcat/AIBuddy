...
use crate::agents::mcp_client::{AIBuddyMcpHostInfo, McpClientTrait};
use crate::agents::platform_extensions::developer::DeveloperClient;
use crate::agents::state_machine::{
    has_unapplied_tool_confirmation_response, pending_tool_confirmations,
};
use crate::agents::{
    AIBuddyPlatform, Agent, AgentConfig, ExtensionConfig, ExtensionLoadResult, SessionConfig,
};
...
use crate::config::{AIBuddyMode, Config};
...
pub(super) const DEFAULT_PROVIDER_ID: &str = "aibuddy";
pub(super) const DEFAULT_PROVIDER_LABEL: &str = "AIBuddy (Default)";
...
/// a durable, append-only exchange of messages. Internally, aibuddy also has a concept
...
/// Per-session active-run registry, shared by every `AIBuddyAcpAgent` created
...
pub struct AIBuddyAcpAgent {
    sessions: Arc<Mutex<HashMap<String, AIBuddyAcpSession>>>,
    ...
    client_mcp_host_info: OnceCell<AIBuddyMcpHostInfo>,
    client_supports_acp_elicitation: OnceCell<bool>,
    client_supports_aibuddy_custom_notifications: OnceCell<bool>,
...
fn agent_capabilities_meta() -> Option<Meta> {
    let mut aibuddy = serde_json::Map::new();
    aibuddy.insert("recipeParameterScopes".to_string(), serde_json::json!({}));
    if cfg!(feature = "local-inference") {
        aibuddy.insert("localInference".to_string(), serde_json::json!({}));
    }
...
#[derive(Debug, Default, Deserialize)]
struct AIBuddyClientCapabilities {
    #[serde(rename = "mcpHostCapabilities", default)]
    mcp_host_capabilities: Option<AIBuddyMcpHostCapabilities>,
...
#[derive(Debug, Default, Deserialize)]
struct AIBuddyMcpHostCapabilities {
    #[serde(default)]
...
) -> (CustomMethodNamespace, Option<AIBuddyClientCapabilities>) {
    let Some(meta) = extract_client_capabilities_meta(args) else {
        return (CustomMethodNamespace::AIBuddy, None);
    };
...
impl JsonRpcMessage for LegacyAIBuddySessionNotification {
...
impl JsonRpcNotification for LegacyAIBuddySessionNotification {}
...
pub(super) fn send_custom_session_notification(
    ...
    notification: AIBuddySessionNotification,
...
fn extract_client_mcp_host_info(
    args: &InitializeRequest,
    aibuddy_client_capabilities: Option<&AIBuddyClientCapabilities>,
) -> AIBuddyMcpHostInfo {
    let host_capabilities =
        aibuddy_client_capabilities.and_then(|aibuddy| aibuddy.mcp_host_capabilities.as_ref());
...
fn extract_use_login_shell_path(args: &InitializeRequest) -> bool {
    args.meta
        .as_ref()
        .and_then(|meta| meta.get("aibuddy/useLoginShellPath"))
...
fn initial_session_extensions(
    ...
    project_root: &Path,
    mcp_servers: Vec<McpServer>,
    aibuddy_extensions: Option<Vec<AIBuddyExtension>>,
    ...
    } else if let Some(aibuddy_extensions) = aibuddy_extensions {
        for extension in extensions::aibuddy_extensions_to_configs(aibuddy_extensions)? {
...
fn resolve_default_provider_model_config(
    config: &Config,
) -> Result<(String, aibuddy_providers::model::ModelConfig), agent_client_protocol::Error> {
    let resolved_provider = config.get_aibuddy_provider().map_err(|error| {
        agent_client_protocol::Error::internal_error()
            .data(format!("Failed to resolve provider: {}", error))
    })?;
    let resolved_model = config.get_aibuddy_model().map_err(|error| {
        agent_client_protocol::Error::internal_error()
...
async fn resolve_provider_default_model_config(
    provider_name: &str,
) -> Result<aibuddy_providers::model::ModelConfig, agent_client_protocol::Error> {
...
pub(super) struct UsageUpdates {
    pub(super) custom: AIBuddySessionNotification,
...
pub(super) fn build_usage_updates(
    ...
    UsageUpdates {
        custom: AIBuddySessionNotification {
            session_id: session.id.clone(),
            update: AIBuddySessionUpdate::UsageUpdate(SessionUsageUpdate {
                used,
...
impl AIBuddyAcpAgent {
    ...
    pub(super) fn supports_aibuddy_custom_notifications(&self) -> bool {
        self.client_supports_aibuddy_custom_notifications
            .get()
            .copied()
            .unwrap_or(false)
    }
    ...
    // TODO: aibuddy reads Paths::in_state_dir globally (e.g. RequestLog), ignoring this data_dir.
    pub async fn new(options: AIBuddyAcpAgentOptions) -> Result<Self> {
        ...
        let agent_config = AgentConfig::new(
            ...
            options.scheduler,
            Config::global().get_aibuddy_mode().unwrap_or_default(),
            options.disable_session_naming,
            options.aibuddy_platform.clone(),
        );
        ...
        Ok(Self {
            ...
            client_supports_aibuddy_custom_notifications: OnceCell::new(),
    ...
    fn spawn_provider_inventory_refresh(&self, aibuddy_session: &Session, agent: &Arc<Agent>) {
        let Some(provider_name) = aibuddy_session.provider_name.clone() else {
            return;
        };
        let inventory_service = self.provider_inventory.clone();
        let agent = agent.clone();
        let session_id = aibuddy_session.id.clone();
        tokio::spawn(async move {
    ...
    async fn register_acp_session(&self, session_id: String, agent: Arc<Agent>) {
        let acp_session = AIBuddyAcpSession {
            agent: agent.clone(),
        };
        self.sessions
            .lock()
        ...
        match content_item {
            ...
            MessageContent::SystemNotification(notification) => {
                send_status_message_update(
                    cx,
                    self.supports_aibuddy_custom_notifications(),
...
fn extract_client_supports_aibuddy_custom_notifications(
    aibuddy_client_capabilities: Option<&AIBuddyClientCapabilities>,
) -> bool {
    aibuddy_client_capabilities
        .and_then(|aibuddy| aibuddy.custom_notifications)
        .unwrap_or(false)
}
...
fn extract_client_supports_recipe_param_requests(
    ...
    aibuddy_client_capabilities
        .and_then(|aibuddy| aibuddy.recipe_parameter_requests)
        .unwrap_or(false)
}
...
fn send_status_message_update(
    cx: &ConnectionTo<Client>,
    supports_aibuddy_custom_notifications: bool,
    custom_method_namespace: CustomMethodNamespace,
    session_id: &str,
    notification: &SystemNotificationContent,
) -> Result<(), agent_client_protocol::Error> {
    if let Some(status) = status_message_from_system_notification(notification) {
        if supports_aibuddy_custom_notifications {
            send_custom_session_notification(
                cx,
                custom_method_namespace,
                AIBuddySessionNotification {
                    session_id: session_id.to_string(),
                    update: AIBuddySessionUpdate::StatusMessage(StatusMessageUpdate { status }),
                },
            )?;
        }
    }
...
fn send_progress_message_update(
    ...
    if supports_aibuddy_custom_notifications {
        send_custom_session_notification(
            ...
            AIBuddySessionNotification {
                session_id: session_id.to_string(),
                update: AIBuddySessionUpdate::StatusMessage(StatusMessageUpdate {
...
impl AIBuddyAcpAgent {
    async fn on_initialize(
        ...
        let (custom_method_namespace, aibuddy_client_capabilities) =
        ...
        let _ = self.client_mcp_host_info.set(extract_client_mcp_host_info(
            &args,
            aibuddy_client_capabilities.as_ref(),
        ));
        let _ = self.client_supports_aibuddy_custom_notifications.set(
            extract_client_supports_aibuddy_custom_notifications(
                aibuddy_client_capabilities.as_ref(),
            ),
        );
        let _ = self.client_supports_recipe_param_requests.set(
            extract_client_supports_recipe_param_requests(aibuddy_client_capabilities.as_ref()),
        );
        let client_requests_tool_call_label_enrichment = aibuddy_client_capabilities
            .as_ref()
            .and_then(|aibuddy| aibuddy.tool_call_label_enrichment)
            .unwrap_or(false);
        let _ = self
        ...
        Ok(InitializeResponse::new(args.protocol_version)
            .agent_info(Implementation::new("aibuddy", env!("CARGO_PKG_VERSION")))
            .agent_capabilities(capabilities)
            .auth_methods(vec![AuthMethod::Agent(
                AuthMethodAgent::new("aibuddy-provider", "Configure Provider")
                    .description("Run `aibuddy configure` to set up your AI provider and API key"),
            )]))
    }
    ...
    async fn start_active_run(
        ...
        if let Some(active_run) = active_prompt_runs.get(session_id) {
            return Err(agent_client_protocol::Error::invalid_params().data(format!(
                "session already has active run `{}`; use _aibuddy/unstable/session/steer",
    ...
    fn active_run_meta(active_run_id: Option<&str>) -> Meta {
        ...
        let mut meta = serde_json::Map::new();
        meta.insert("aibuddy".to_string(), serde_json::Value::Object(aibuddy));
        meta
    }
    ...
    async fn send_session_usage_updates(
        ...
        if self.supports_aibuddy_custom_notifications() {
                ...
                Ok(crate::agents::AgentEvent::MessageUsage { message_id, usage }) => {
                    if self.supports_aibuddy_custom_notifications() {
                        send_custom_session_notification(
                            ...
                            AIBuddySessionNotification {
                                session_id: session_id.to_string(),
                                update: AIBuddySessionUpdate::MessageUsage(message_usage_update(
                                    message_id, &usage,
                                )),
                            },
                        )?;
    ...
    async fn build_config_update(
        ...
        let aibuddy_mode = agent.aibuddy_mode().await;
        let inventory = self
            .provider_inventory
        ...
        let mode_state = build_mode_state(aibuddy_mode)?;
    ...
    async fn on_set_mode(
        ...
        let mode = mode_id.parse::<AIBuddyMode>().map_err(|_| {
            agent_client_protocol::Error::invalid_params()
        ...
        let agent = self.get_session_agent(session_id).await?;
        agent
            .update_aibuddy_mode(mode, session_id)
            .await
            .internal_err_ctx("Failed to update mode")?;

        // aibuddy_mode is already updated on the session above.
...
pub struct AIBuddyAcpHandler {
    pub agent: Arc<AIBuddyAcpAgent>,
}

pub fn serve<R, W>(
    agent: Arc<AIBuddyAcpAgent>,
    read: R,
    write: W,
...
{
    Box::pin(async move {
        let handler = AIBuddyAcpHandler { agent };
...
pub async fn run(builtins: Vec<String>, enable_scheduler: bool) -> Result<()> {
    ...
    let server = crate::acp::server_factory::AcpServer::new(
        crate::acp::server_factory::AcpServerFactoryConfig {
            ...
            aibuddy_platform: AIBuddyPlatform::AIBuddyCli,
...
mod tests {
    ...
    use aibuddy_providers::conversation::token_usage::Usage as TokenUsage;
...
```",
    ...
    fn convert_acp_prompt_preserves_audience_for_converted_blocks() {
        ...
        let message = AIBuddyAcpAgent::convert_acp_prompt_to_message(&prompt);
    ...
    fn test_build_usage_update_clamps_negative_used_to_zero() {
        ...
        session.model_config = Some(
            aibuddy_providers::model::ModelConfig::new("test-model")
        ...
        let usage = match updates.custom.update {
            AIBuddySessionUpdate::UsageUpdate(usage) => usage,
    ...
    #[test]
    fn test_aibuddy_custom_notifications_capability_defaults_to_false() {
        let request = InitializeRequest::new(agent_client_protocol::schema::ProtocolVersion::V1);
        let aibuddy_client_capabilities =
            extract_client_capabilities_meta(&request).and_then(|meta| meta.aibuddy);
    ...
    #[test]
    fn test_aibuddy_custom_notifications_capability_reads_client_meta() {
    ...
    fn test_tool_call_label_enrichment_capability() {
        ...
        let aibuddy_client_capabilities =
            extract_client_capabilities_meta(&request).and_then(|meta| meta.aibuddy);
        assert!(!aibuddy_client_capabilities
            .and_then(|aibuddy| aibuddy.tool_call_label_enrichment)
            .unwrap_or(false));
    ...
    async fn asynchronous_provider_effort_update_is_forwarded_to_client() {
        ...
        let server = Arc::new(
            AIBuddyAcpAgent::new(AIBuddyAcpAgentOptions {
                provider_factory,
        ...
        session_agent
            .update_provider(
                provider.clone(),
                aibuddy_providers::model::ModelConfig::new("gpt-4o").with_merged_request_params(
