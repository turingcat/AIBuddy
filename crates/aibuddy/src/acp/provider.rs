...
use aibuddy_providers::conversation::token_usage::{ProviderUsage, Usage};
...
use crate::config::{AIBuddyMode, Config, ExtensionConfig};
...
use aibuddy_providers::errors::ProviderError;
use aibuddy_providers::model::ModelConfig;
use aibuddy_providers::thinking::{
...
pub struct AcpProviderConfig {
    ...
    pub mode_mapping: HashMap<AIBuddyMode, Vec<String>>,
...
pub struct AcpProvider {
    name: String,
    aibuddy_mode: Arc<Mutex<AIBuddyMode>>,
    mode_mapping: HashMap<AIBuddyMode, Vec<String>>,
    ...
    /// redundant-send guard: unlike a aibuddy-side cache of the last applied
...
impl AcpProvider {
    ...
    async fn start(
        ...
        let aibuddy_mode_shared = Arc::new(Mutex::new(aibuddy_mode));
        ...
        Ok(Self {
            name,
            aibuddy_mode: aibuddy_mode_shared,
            mode_mapping,
    ...
    pub(crate) async fn send_set_mode(&self, _aibuddy_id: &str, mode_id: String) -> Result<()> {
    ...
    /// aibuddy-side record of the last sent value: the agent's `SetConfigOption`
    ...
    async fn set_effort_option(
        ...
        self.send_set_config_option(aibuddy_id, option_id.to_string(), value)
            .await
    }
...
impl Provider for AcpProvider {
    ...
    async fn get_context_limit(&self, model: &str, override_limit: Option<usize>) -> usize {
        aibuddy_providers::context_limit::ContextLimitResolver::new(self.get_name())
    ...
    async fn update_mode(&self, session_id: &str, mode: AIBuddyMode) -> Result<(), ProviderError> {
        ...
        if let Ok(mut guard) = self.aibuddy_mode.lock() {
            *guard = mode;
        }
        Ok(())
    }
    ...
    async fn stream(
        ...
        if claim.include_context && memo.is_none() {
            // Nothing fit beside this turn's prompt, so the context never left aibuddy. Give
        ...
        let aibuddy_mode = *self
            .aibuddy_mode
            .lock()
            .map_err(|_| ProviderError::RequestFailed("aibuddy_mode lock poisoned".into()))?;

        let reject_all_tools = aibuddy_mode == AIBuddyMode::Chat;
                ...
                match update {
                    ...
                    AcpUpdate::ToolCallStart { id, name, kind, raw_input } => {
                        ...
                        if reject_all_tools {
                            ...
                            // call. aibuddy.acp.kind preserves ACP's stable categorization for
                    ...
                    AcpUpdate::PermissionRequest { request, response_tx } => {
                        text_run = None;
                        thought_run = None;
                        if let Some(decision) = permission_decision_from_mode(aibuddy_mode) {
            ...
            .on_receive_notification(
                {
                    ...
                    let aibuddy_mode = aibuddy_mode.clone();
                        ...
                        match &notification.update {
                            SessionUpdate::CurrentModeUpdate(update) => {
                                if let Some(mode) = resolve_mode(
                                    ...
                                    if let Ok(mut guard) = aibuddy_mode.lock() {
                                        *guard = mode;
                                    }
                                }
                            }
                            ...
                            .and_then(|g| g.as_ref())
                        {
                            match notification.update {
                                ...
                                SessionUpdate::ToolCall(tool_call) => {
                                    ...
                                    // aibuddy.external_dispatch marker keeps `name` off the
...
async fn forward_child_stderr(mut stderr: tokio::process::ChildStderr) {
    ...
    loop {
        match stderr.read(&mut chunk).await {
            ...
            Err(e) => {
                tracing::debug!(target: "aibuddy::acp::child::stderr", error = %e, "stderr read error");
                break;
            }
        }
    }
...
fn emit_stderr_line(line: &mut Vec<u8>) {
    ...
    tracing::info!(target: "aibuddy::acp::child::stderr", "{}", String::from_utf8_lossy(trimmed));
    line.clear();
}
    ...
    while let Some(request) = rx.recv().await {
        match request {
                ...
                let result = match session {
                    Ok(session) => {
                        *session_state.active_id.lock().unwrap() = Some(session.session_id.clone());
                        ...
                        .await?;
                        apply_session_mode(&config, &aibuddy_mode, &cx, session).await
                    }
            ...
            ClientRequest::SetConfigOption {
                ...
                // so it is the freshest source after a aibuddy-initiated switch.
                let result: Result<()> = cx
                    .send_request(req)
                    .block_task()
                    .await
...
async fn apply_session_mode(
    ...
    let current_mode = aibuddy_mode.lock().ok().map(|mode| *mode);
...
fn initial_mode_candidates(
    config: &AcpProviderConfig,
    current_mode: Option<AIBuddyMode>,
) -> Vec<String> {
    current_mode
...
/// Convert ACP `ToolCallContent` blocks into the rmcp `Content` shape aibuddy's
...
/// Map a aibuddy effort value onto the agent's advertised vocabulary. Values aibuddy
/// and the agent share pass through; aibuddy's own enum values map onto their
...
/// Resolve the aibuddy-side thinking effort to send to the agent: the session's
...
/// dead subprocess or dropped connection, and aibuddy's own send failures carry no
/// ACP error at all.
...
fn reverse_mode_mapping(
    mode_mapping: &HashMap<AIBuddyMode, Vec<String>>,
) -> HashMap<String, Vec<AIBuddyMode>> {
    let mut reverse: HashMap<String, Vec<AIBuddyMode>> = HashMap::new();
...
fn resolve_mode(
    reverse_modes: &HashMap<String, Vec<AIBuddyMode>>,
    mode_id: &str,
    current: &Arc<Mutex<AIBuddyMode>>,
) -> Option<AIBuddyMode> {
...
fn permission_decision_from_mode(aibuddy_mode: AIBuddyMode) -> Option<PermissionDecision> {
    match aibuddy_mode {
        AIBuddyMode::Auto => Some(PermissionDecision::AllowOnce),
        AIBuddyMode::Chat => Some(PermissionDecision::RejectOnce),
        AIBuddyMode::Approve | AIBuddyMode::SmartApprove => None,
    }
}
...
mod tests {
    ...
    fn test_provider_with_tx(
        ...
        (
            AcpProvider {
                ...
                aibuddy_mode: Arc::new(Mutex::new(AIBuddyMode::Auto)),
    ...
    async fn messages_to_prompt_prepends_handoff_context_before_latest_user() {
        ...
        assert!(memo.starts_with(
            "Conversation context from aibuddy before this ACP provider session was created:"
        ));
    ...
    async fn chat_mode_denied_tool_messages_get_distinct_provider_ids() {
        ...
        let (tx, mut rx) = mpsc::channel(1);
        let (provider, model) = test_provider_with_tx(Some(tx));
        *provider.aibuddy_mode.lock().unwrap() = AIBuddyMode::Chat;
    ...
    fn lock_default_context_limit() -> env_lock::EnvGuard<'static> {
        env_lock::lock_env([("AIBUDDY_CONTEXT_LIMIT", None::<&str>)])
    }
    ...
    async fn get_context_limit_surfaces_captured_context_size() {
        ...
        assert_eq!(
            ...
            aibuddy_providers::model::DEFAULT_CONTEXT_LIMIT
        );
    ...
    async fn handoff_budget_honors_global_context_limit_override() {
        let _guard = env_lock::lock_env([("AIBUDDY_CONTEXT_LIMIT", Some("64"))]);
        let (provider, model) = test_provider();
        provider.context_size.store(200_000, Ordering::Relaxed);
        let messages = vec![
            Message::assistant().with_text("prior answer"),
    ...
    async fn a_budget_too_small_for_a_memo_keeps_the_claim() {
        ...
        assert!(
            ...
            "context that never left aibuddy must still be handed off later"
        );
    }
    ...
    fn map_effort_value_maps_aibuddy_values_onto_agent_values(value: &str) -> Option<String> {
        map_effort_value(
    ...
    /// what aibuddy last sent: the persisted value gets re-applied.
    #[tokio::test]
    ...
    async fn apply_effort_if_changed_falls_back_to_the_global_default() {
        let _guard = env_lock::lock_env([("AIBUDDY_THINKING_EFFORT", Some("high"))]);
        let (tx, mut rx) = mpsc::channel(1);
        let provider =
            test_provider_with_effort(tx, Some(effort_capability(&["default", "high"], "default")));
    ...
    async fn apply_effort_if_changed_skips_unmapped_value() {
        ...
        let _guard = env_lock::lock_env([("AIBUDDY_THINKING_EFFORT", Some("medium"))]);
        let (tx, mut rx) = mpsc::channel(1);
        let provider =
            test_provider_with_effort(tx, Some(effort_capability(&["default", "high"], "default")));
    ...
    #[test_case(AIBuddyMode::Auto)]
    #[test_case(AIBuddyMode::Approve)]
    #[test_case(AIBuddyMode::SmartApprove)]
    #[test_case(AIBuddyMode::Chat)]
    fn initial_mode_candidates_empty_when_mode_negotiation_disabled(mode: AIBuddyMode) {
    ...
    fn initial_mode_candidates_prefer_mapping_then_fallback() {
        let mapping = HashMap::from([(AIBuddyMode::Auto, vec!["bypassPermissions".to_string()])]);
        let config = test_acp_config(mapping, Some("default".to_string()));

        assert_eq!(
            initial_mode_candidates(&config, Some(AIBuddyMode::Auto)),
            vec!["bypassPermissions".to_string()]
        );
        assert_eq!(
            initial_mode_candidates(&config, Some(AIBuddyMode::Chat)),
    ...
    async fn update_mode_without_mapping_skips_acp_request_but_tracks_mode() {
        ...
        provider
            .update_mode("session", AIBuddyMode::Chat)
            .await
            .unwrap();

        assert!(rx.try_recv().is_err());
        assert_eq!(*provider.aibuddy_mode.lock().unwrap(), AIBuddyMode::Chat);
    }
    ...
    async fn update_mode_with_mapping_sends_set_mode() {
        let (tx, mut rx) = mpsc::channel(1);
        let (mut provider, _) = test_provider_with_tx(Some(tx));
        provider.mode_mapping = HashMap::from([(AIBuddyMode::Chat, vec!["plan".to_string()])]);
    ...
    async fn update_mode_sends_candidate_offered_by_agent() {
        ...
        let handle = tokio::spawn(async move {
            provider
                .update_mode("session", AIBuddyMode::Auto)
                .await
                .unwrap();
            provider
        });
        ...
        let provider = handle.await.unwrap();
        assert_eq!(*provider.aibuddy_mode.lock().unwrap(), AIBuddyMode::Auto);
    }
    ...
    async fn update_mode_errors_when_no_candidate_offered() {
        let (tx, mut rx) = mpsc::channel(1);
        let (mut provider, _) = test_provider_with_tx(Some(tx));
        provider.mode_mapping = HashMap::from([(AIBuddyMode::Chat, vec!["read-only".to_string()])]);
        provider.session.lock().unwrap().response = NewSessionResponse::new("test-session")
            .modes(mode_state("agent", &["agent", "agent-full-access"]));

        let result = provider.update_mode("session", AIBuddyMode::Chat).await;
    ...
    #[test_case(AIBuddyMode::Auto => Some(PermissionDecision::AllowOnce) ; "auto allows")]
    #[test_case(AIBuddyMode::Chat => Some(PermissionDecision::RejectOnce) ; "chat rejects")]
    #[test_case(AIBuddyMode::Approve => None ; "approve defers")]
    #[test_case(AIBuddyMode::SmartApprove => None ; "smart_approve defers")]
    fn test_permission_decision_from_mode(mode: AIBuddyMode) -> Option<PermissionDecision> {
        permission_decision_from_mode(mode)
    }

    #[test_case(
        HashMap::from([
            (AIBuddyMode::Auto, vec!["yolo".to_string()]),
            (AIBuddyMode::Approve, vec!["default".to_string()]),
            (AIBuddyMode::SmartApprove, vec!["auto_edit".to_string()]),
            (AIBuddyMode::Chat, vec!["plan".to_string()]),
        ]),
        HashMap::from([
            ("yolo".to_string(), vec![AIBuddyMode::Auto]),
            ("default".to_string(), vec![AIBuddyMode::Approve]),
            ("auto_edit".to_string(), vec![AIBuddyMode::SmartApprove]),
            ("plan".to_string(), vec![AIBuddyMode::Chat]),
        ])
        ; "gemini provider mapping"
    )]
    #[test_case(
        HashMap::from([
            (AIBuddyMode::Auto, vec!["bypassPermissions".to_string()]),
            (AIBuddyMode::Approve, vec!["default".to_string()]),
            (AIBuddyMode::SmartApprove, vec!["acceptEdits".to_string()]),
            (AIBuddyMode::Chat, vec!["plan".to_string()]),
        ]),
        HashMap::from([
            ("bypassPermissions".to_string(), vec![AIBuddyMode::Auto]),
            ("default".to_string(), vec![AIBuddyMode::Approve]),
            ("acceptEdits".to_string(), vec![AIBuddyMode::SmartApprove]),
            ("plan".to_string(), vec![AIBuddyMode::Chat]),
        ])
    ...
    #[test_case(
        HashMap::from([
            (AIBuddyMode::Auto, vec!["full-access".to_string(), "agent-full-access".to_string()]),
            (AIBuddyMode::Approve, vec!["read-only".to_string()]),
            (AIBuddyMode::SmartApprove, vec!["auto".to_string(), "agent".to_string()]),
            (AIBuddyMode::Chat, vec!["read-only".to_string()]),
        ]),
        HashMap::from([
            ("full-access".to_string(), vec![AIBuddyMode::Auto]),
            ("agent-full-access".to_string(), vec![AIBuddyMode::Auto]),
            ("read-only".to_string(), vec![AIBuddyMode::Approve, AIBuddyMode::Chat]),
            ("auto".to_string(), vec![AIBuddyMode::SmartApprove]),
            ("agent".to_string(), vec![AIBuddyMode::SmartApprove]),
        ])
        ; "codex candidates for both bridge generations"
    )]
    fn test_reverse_mode_mapping(
        forward: HashMap<AIBuddyMode, Vec<String>>,
        expected: HashMap<String, Vec<AIBuddyMode>>,
    ) {
    ...
    fn duplicate_read_only_reverse_modes() -> HashMap<String, Vec<AIBuddyMode>> {
        HashMap::from([
            ("full-access".to_string(), vec![AIBuddyMode::Auto]),
            (
                "read-only".to_string(),
                vec![AIBuddyMode::Approve, AIBuddyMode::Chat],
            ),
            ("auto".to_string(), vec![AIBuddyMode::SmartApprove]),
        ])
    }
    ...
    #[test_case(
        "full-access", AIBuddyMode::Auto, Some(AIBuddyMode::Auto)
        ; "unique mapping returns the only candidate"
    )]
    #[test_case(
        "read-only", AIBuddyMode::Approve, Some(AIBuddyMode::Approve)
        ; "duplicate prefers current when current is Approve"
    )]
    #[test_case(
        "read-only", AIBuddyMode::Chat, Some(AIBuddyMode::Chat)
        ; "duplicate prefers current when current is Chat"
    )]
    #[test_case(
        "read-only", AIBuddyMode::Auto, Some(AIBuddyMode::Approve)
        ; "duplicate falls back to first when current not in candidates"
    )]
    #[test_case(
        "unknown-id", AIBuddyMode::Auto, None
        ; "unknown mode id returns None"
    )]
    fn test_resolve_mode(mode_id: &str, current: AIBuddyMode, expected: Option<AIBuddyMode>) {
        ...
        if mode_id == "read-only" && expected == Some(AIBuddyMode::Approve) {
            assert!(result == Some(AIBuddyMode::Approve) || result == Some(AIBuddyMode::Chat));
        } else {
    ...
    fn tool_meta_pairs_external_dispatch_marker_with_acp_kind() {
        ...
        for (kind, expected) in cases {
            ...
            assert_eq!(
                tool_meta["aibuddy.acp.kind"],
                serde_json::Value::String(expected.to_string()),
                "aibuddy.acp.kind serialized wrong for kind={kind:?}"
            );
        }
    }
}
