...
use crate::acp::server::AIBuddyAgentConnection;
use crate::acp::server_factory::AcpServer;

// The upstream ACP HTTP server only supports exact origin allowlists for
// WebSocket upgrades; AIBuddy applies its richer loopback predicate before this.
...
/// `aibuddy serve`. This allows the `x-secret-key` auth header the proxy routes
/// rely on.
...
fn create_acp_router_inner(server: Arc<AcpServer>, policy: AcpOriginPolicy) -> Router {
    AcpHttpServer::new(move || AIBuddyAgentConnection::new(server.clone()))
...
/// or aibuddy-specific auxiliary routes.
...
/// The full standalone ACP server router used by `aibuddy serve`: ACP transport,
