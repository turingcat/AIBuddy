use crate::acp::server::{
    AIBuddyAcpAgent, AIBuddyAcpAgentOptions, AcpBuiltinSelection, AcpProviderFactory,
    ActiveRunRegistry,
};
...
impl AcpServer {
    ...
    /// headless `aibuddy serve` runs scheduled jobs; on failure `create_agent`
    ...
    pub async fn create_agent(&self) -> Result<Arc<AIBuddyAcpAgent>> {
    ...
    /// server's configured default. Used by the roaming bridge on `aibuddy
    ...
    pub async fn create_agent_with_session_cwd(
    ...
    ) -> Result<Arc<AIBuddyAcpAgent>> {
        let config = crate::config::Config::global();
        let disable_session_naming = config.get_aibuddy_disable_session_naming().unwrap_or(false);
        ...
        let agent = AIBuddyAcpAgent::new(AIBuddyAcpAgentOptions {
            ...
            aibuddy_platform: self.config.aibuddy_platform.clone(),
