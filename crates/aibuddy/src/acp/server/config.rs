...
impl AIBuddyAcpAgent {
    ...
    pub(super) async fn on_config_read(
        ...
        if req.key == "AIBUDDY_PROVIDER" || req.key == "active_provider" {
            let value = config
    ...
    pub(super) async fn on_config_upsert(
        ...
        if req.key == "AIBUDDY_MODEL" {
            if let Some(model) = req.value.as_str() {
                if let Ok(provider) = config.get_aibuddy_provider() {
    ...
    pub(super) async fn on_config_remove(
        ...
        } else if req.key == "AIBUDDY_PROVIDER" || req.key == "active_provider" {
            config.delete("active_provider").internal_err()?;
            config.delete("AIBUDDY_PROVIDER").internal_err()?;
        } else if req.key == "AIBUDDY_MODEL" {
            if let Ok(provider) = config.get_aibuddy_provider() {
                crate::config::set_active_provider(config, &provider, "").internal_err()?;
            }
            config.delete("AIBUDDY_MODEL").internal_err()?;
        } else {
    ...
    pub(super) async fn on_defaults_read(
        ...
        Ok(DefaultsReadResponse {
            provider_id: config.get_aibuddy_provider().ok(),
            model_id: config.get_aibuddy_model().ok(),
        })
    }
...
const PREFERENCE_DEFS: &[PreferenceDef] = &[
    PreferenceDef {
        ...
        config_key: "AIBUDDY_AUTO_COMPACT_THRESHOLD",
        prepare: prepare_auto_compact_threshold,
    },
    PreferenceDef {
        key: PreferenceKey::AIBuddyThinkingEffort,
        config_key: "AIBUDDY_THINKING_EFFORT",
...
fn prepare_thinking_effort(
    ...
    let Some(value) = value.as_str() else {
        return Err(agent_client_protocol::Error::invalid_params()
            .data("aibuddyThinkingEffort must be a string"));
    };
    let effort = value.parse::<ThinkingEffort>().map_err(|err| {
        agent_client_protocol::Error::invalid_params()
            .data(format!("Invalid aibuddyThinkingEffort: {err}"))
    })?;
