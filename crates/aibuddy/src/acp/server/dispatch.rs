...
impl HandleDispatchFrom<Client> for AIBuddyAcpHandler {
    ...
    ) -> impl std::future::Future<Output = Result<Handled<Dispatch>, agent_client_protocol::Error>> + Send
    {
        ...
        Box::pin(async move {
            ...
            MatchDispatchFrom::new(message, &cx)
                                ...
                                Ok(response) => {
                                    ...
                                    if let Err(error) = session_setup.and_then(|(session, totals, context_limit)| {
                                        send_session_setup_notifications(
                                            ...
                                            &totals,
                                            context_limit,
                                            agent.supports_aibuddy_custom_notifications(),
                ...
                .await
                // set_config_option (SACP 11) and set_mode; custom _aibuddy/* in otherwise.
                .if_request({
                    let agent = agent.clone();
                    let cx = cx.clone();
