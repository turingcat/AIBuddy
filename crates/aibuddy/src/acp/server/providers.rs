...
use aibuddy_providers::base::ModelInfo;
use std::str::FromStr;
...
impl AIBuddyAcpAgent {
    ...
    pub(super) async fn on_list_provider_supported_models(
        ...
        let models = match provider.fetch_supported_models().await {
            Ok(models) => models,
            Err(aibuddy_providers::errors::ProviderError::Authentication(error)) => {
                return Err(agent_client_protocol::Error::auth_required().data(error));
            }
            Err(aibuddy_providers::errors::ProviderError::NotConfigured) => {
                return Err(agent_client_protocol::Error::invalid_params()
    ...
    pub(super) async fn on_delete_custom_provider(
        ...
        if Config::global().get_aibuddy_provider().ok().as_deref() == Some(req.provider_id.as_str())
        {
            return Err(agent_client_protocol::Error::invalid_params().data(format!(
