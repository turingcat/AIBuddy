...
impl AIBuddyAcpAgent {
    pub(super) async fn on_add_session_extension(
        ...
        let config = aibuddy_extension_to_config_without_secrets(req.extension)?;
    ...
    pub(super) async fn on_get_config_extensions(
        ...
        let extensions = extensions
            .into_iter()
            .map(config_entry_to_aibuddy_entry)
    ...
    pub(super) async fn on_add_config_extension(
        ...
        let conversion = aibuddy_extension_to_config(req.extension)?;
...
fn session_configs_to_entries(
    ...
    for config in configs {
        ...
        if let Some(extension) = config_to_aibuddy_extension(&config)? {
...
fn config_to_aibuddy_extension(
    config: &ExtensionConfig,
) -> Result<Option<AIBuddyExtension>, agent_client_protocol::Error> {
...
fn aibuddy_extension_to_config(
    extension: AIBuddyExtension,
...
fn aibuddy_extension_to_config_without_secrets(
    extension: AIBuddyExtension,
) -> Result<ExtensionConfig, agent_client_protocol::Error> {
    let conversion = aibuddy_extension_to_config(extension)?;
...
pub(super) fn aibuddy_extensions_to_configs(
    extensions: Vec<AIBuddyExtension>,
) -> Result<Vec<ExtensionConfig>, agent_client_protocol::Error> {
    extensions
        .into_iter()
        .map(aibuddy_extension_to_config_without_secrets)
        .collect()
}

fn config_entry_to_aibuddy_entry(
    entry: ExtensionEntry,
) -> Result<Option<AIBuddyExtensionEntry>, agent_client_protocol::Error> {
    let config_key = entry.config.key();
    let Some(extension) = config_to_aibuddy_extension(&entry.config)? else {
        return Ok(None);
    };
    Ok(Some(AIBuddyExtensionEntry {
        extension,
...
mod tests {
    ...
    #[test]
    fn builtin_config_converts_to_aibuddy_builtin_extension() {
        ...
        let extension = config_to_aibuddy_extension(&config)
            .expect("conversion should succeed")
            .expect("builtin should be supported");

        let AIBuddyExtension::Builtin {
            name,
            description,
            display_name,
            timeout,
    ...
    #[test]
    fn platform_config_converts_to_aibuddy_platform_extension() {
    ...
    #[test]
    fn stdio_config_converts_to_aibuddy_mcp_extension_without_literal_envs() {
    ...
    #[test]
    fn streamable_http_config_converts_to_aibuddy_mcp_extension_without_literal_envs() {
    ...
    #[test]
    fn aibuddy_mcp_stdio_extension_converts_to_config_without_literal_envs() {
        let extension = AIBuddyExtension::Mcp {
            server: Box::new(McpServer::Stdio(
        ...
        let conversion = aibuddy_extension_to_config(extension).expect("conversion should succeed");
    ...
    #[test]
    fn aibuddy_mcp_stdio_extension_extracts_literal_envs_for_config_add() {
        let extension = AIBuddyExtension::Mcp {
            server: Box::new(McpServer::Stdio(
    ...
    #[test]
    fn aibuddy_mcp_streamable_http_extension_converts_to_config_without_literal_envs() {
        let extension = AIBuddyExtension::Mcp {
            server: Box::new(McpServer::Http(
    ...
    #[test]
    fn aibuddy_builtin_extension_converts_to_config() {
        let builtin = AIBuddyExtension::Builtin {
            name: "developer".to_string(),
        ...
        let conversion = aibuddy_extension_to_config(builtin).expect("conversion should succeed");
        assert!(conversion.secret_updates.is_empty());
    ...
    #[test]
    fn aibuddy_platform_extension_converts_to_config() {
        let platform = AIBuddyExtension::Platform {
            name: "todo".to_string(),
        ...
        let conversion = aibuddy_extension_to_config(platform).expect("conversion should succeed");
        assert!(conversion.secret_updates.is_empty());
    ...
    #[test]
    fn aibuddy_mcp_sse_extension_is_rejected_for_config_add() {
        let extension = AIBuddyExtension::Mcp {
        ...
        assert!(aibuddy_extension_to_config(extension).is_err());
    }
}
