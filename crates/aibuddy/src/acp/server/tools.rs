...
use aibuddy_sdk_types::custom_requests::{ToolListItem, ToolPermissionLevel};
...
impl AIBuddyAcpAgent {
            ...
            .map(|tool| {
                let permission = permission_manager
                    ...
                    .or_else(|| {
                        ...
                        } else if aibuddy_mode == AIBuddyMode::Approve {
    ...
    pub(super) async fn on_call_tool(
        &self,
        req: AIBuddyToolCallRequest,
    ) -> Result<AIBuddyToolCallResponse, agent_client_protocol::Error> {
        let session_id = &req.session_id;
        let agent = self.get_session_agent(&req.session_id).await?;
        ...
        if agent.aibuddy_mode().await != AIBuddyMode::Auto {
            return Err(agent_client_protocol::Error::invalid_params()
