use crate::acp::custom_requests::AIBuddyExtension;
...
struct NewSessionMetaFields {
    ...
    /// Client-supplied title, recorded as user-set so aibuddy's own name
...
impl AIBuddyAcpAgent {
    pub(super) async fn handle_new_session(
        ...
        let current_mode: AIBuddyMode = config.get_aibuddy_mode().unwrap_or_default();
    ...
    async fn configure_new_session(
        ...
        let aibuddy_extensions = meta_aibuddy_extensions(args.meta.as_ref())?;
    ...
    async fn resolve_provider_and_model(
        ...
        let recipe_provider = recipe_settings.and_then(|s| s.aibuddy_provider.clone());
        let recipe_model = recipe_settings.and_then(|s| s.aibuddy_model.clone());
            ...
            None => match meta_string(meta, "provider")? {
                ...
                None => {
                    if let Some(model) = recipe_model.as_deref() {
                        let provider = config.get_aibuddy_provider().map_err(|error| {
...
fn meta_aibuddy_extensions(
    meta: Option<&Meta>,
) -> Result<Option<Vec<AIBuddyExtension>>, agent_client_protocol::Error> {
