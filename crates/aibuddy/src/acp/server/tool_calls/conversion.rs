...
pub(crate) fn aibuddy_tool_call_meta(tool_request: &ToolRequest) -> Option<Meta> {
...
fn build_initial_tool_call(tool_request: &ToolRequest, include_generated_title: bool) -> ToolCall {
    ...
    let aibuddy_meta = aibuddy_tool_call_meta(tool_request);
...
pub(crate) fn trusted_update_meta(tool_response: &ToolResponse) -> Option<Meta> {
    ...
    meta_map.insert("aibuddy".to_string(), aibuddy_meta);
    Some(meta_map)
}
...
mod tests {
    ...
    mod build_initial_tool_call {
        ...
        fn uses_default_title_and_preserves_request_data() {
            ...
            let request = ToolRequest {
                ...
                metadata: None,
                tool_meta: Some(serde_json::json!({"aibuddy_extension": "developer"})),
            };
            ...
            assert_eq!(
                tool_call.meta.as_ref().and_then(|meta| meta.get("aibuddy")),
        ...
        fn merges_message_and_tool_metadata() {
            ...
            assert_eq!(
                limited_tool_call
                    .meta
                    .as_ref()
                    .and_then(|meta| meta.get("aibuddy")),
                Some(&serde_json::json!({
                    "created": 1_700_000_000,
                    "messageId": "msg_live",
    ...
    mod aibuddy_tool_call_meta {
        use super::*;

        #[test]
        fn prefers_aibuddy_extension_metadata_over_name_prefix() {
            let request = ToolRequest {
                id: "req_1".to_string(),
                tool_call: Ok(CallToolRequestParams::new("other__query-docs")),
                metadata: None,
                tool_meta: Some(serde_json::json!({"aibuddy_extension": "context7"})),
            };

            let meta = aibuddy_tool_call_meta(&request).expect("expected metadata");
    ...
    mod extract_tool_locations_from_request {
        ...
        fn accepts_unprefixed_developer_tool_from_metadata() {
            ...
            request.tool_meta = Some(serde_json::json!({"aibuddy_extension": "developer"}));
    ...
    mod trusted_update_meta {
        ...
        #[test]
        fn ignores_untrusted_aibuddy_meta() {
