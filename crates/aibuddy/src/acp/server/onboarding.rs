...
const AIBUDDY_CONFIG_PREFIX: &str = "aibuddy_config:";
...
impl AIBuddyAcpAgent {
    pub(super) async fn on_onboarding_import_scan(
        ...
        if source_filter.contains(&OnboardingImportSourceKind::AIBuddyConfig) {
            for path in aibuddy_config_candidate_paths(&self.config_dir) {
                if let Some(candidate) = scan_aibuddy_config_candidate(&path) {
...
fn apply_onboarding_import_candidates(
    ...
    for candidate_id in &req.candidate_ids {
        match parse_candidate_id(candidate_id) {
            Some((OnboardingImportSourceKind::AIBuddyConfig, path)) => {
                match apply_aibuddy_config_candidate(config, target_config_dir, &path) {
                    Ok(result) => {
                    ...
                    Err(error) => warnings.push(import_failure_warning(
                        OnboardingImportSourceKind::AIBuddyConfig,
                        &path,
                        &error,
                    )),
                }
...
fn import_failure_warning(
    ...
    let source_name = match source_kind {
        OnboardingImportSourceKind::AIBuddyConfig => "AIBuddy configuration",
...
fn aibuddy_config_candidate_paths(config_dir: &Path) -> Vec<PathBuf> {
    let mut paths = vec![config_dir.join(CONFIG_YAML_NAME)];
    if let Some(home) = dirs::home_dir() {
        paths.push(home.join(".config").join("aibuddy").join(CONFIG_YAML_NAME));
    }
    dedupe_paths(paths)
}
...
fn scan_aibuddy_config_candidate(path: &Path) -> Option<OnboardingImportCandidate> {
    if !path.exists() {
        return None;
    }
    ...
    if mapping_contains_string(&mapping, "AIBUDDY_PROVIDER")
        || mapping_contains_string(&mapping, "AIBUDDY_MODEL")
    {
        counts.providers = 1;
    }
    ...
    if counts.sessions > 0 {
        warnings.push("Sessions are already shared through AIBuddy's data store.".to_string());
    }

    Some(OnboardingImportCandidate {
        id: candidate_id(AIBUDDY_CONFIG_PREFIX, path),
        source_kind: OnboardingImportSourceKind::AIBuddyConfig,
        display_name: "Existing AIBuddy configuration".to_string(),
...
fn parse_candidate_id(id: &str) -> Option<(OnboardingImportSourceKind, PathBuf)> {
    if let Some(path) = id.strip_prefix(AIBUDDY_CONFIG_PREFIX) {
        return Some((
            OnboardingImportSourceKind::AIBuddyConfig,
            PathBuf::from(path),
        ));
...
fn apply_aibuddy_config_candidate(
    ...
    let provider = yaml_string(&source, "AIBUDDY_PROVIDER");
    let model = yaml_string(&source, "AIBUDDY_MODEL");
    ...
    let extension_result = import_aibuddy_config_extensions(target_config, &source)?;
    ...
    result.warnings.push(
        "Session history already lives in the AIBuddy data store when available.".to_string(),
    );
    Ok(result)
}
...
fn import_aibuddy_config_extensions(
    target_config: &Config,
    source: &Mapping,
...
mod tests {
    ...
    fn apply_onboarding_imports_continues_after_candidate_failure() {
        ...
        let missing_aibuddy_config = source.path().join("missing-config.yaml");
        ...
        let req = OnboardingImportApplyRequest {
            candidate_ids: vec![
                candidate_id(AIBUDDY_CONFIG_PREFIX, &missing_aibuddy_config),
        ...
        assert!(response
            .warnings
            .iter()
            .any(|warning| warning.starts_with("Skipped AIBuddy configuration import at ")));
    ...
    #[test]
    fn apply_aibuddy_config_imports_defaults_extensions_and_skills() {
        let source = TempDir::new().unwrap();
        let target = TempDir::new().unwrap();
...
"#,
        ...
        let result =
            apply_aibuddy_config_candidate(&target_config, target.path(), &source_config).unwrap();

        assert_eq!(result.imported.providers, 1);
        assert_eq!(result.imported.extensions, 1);
        assert_eq!(result.imported.skills, 1);
        assert_eq!(target_config.get_aibuddy_provider().unwrap(), "openai");
        assert!(target.path().join("skills").join("reviewer").exists());
    }

    #[test]
    fn apply_aibuddy_config_model_only_skips_provider_activation() {
        let source = TempDir::new().unwrap();
        let target = TempDir::new().unwrap();
        let source_config = source.path().join(CONFIG_YAML_NAME);
        fs::write(&source_config, "AIBUDDY_MODEL: gpt-5.1\n").unwrap();
        ...
        assert!(target_config.get_aibuddy_provider().is_err());
    }
