...
fn aibuddy_message_meta(
    message: &Message,
    steer: bool,
) -> serde_json::Map<String, serde_json::Value> {
    let message_meta = AIBuddyMessageMeta {
...
fn extend_message_meta(meta: &mut Meta, message: &Message, steer: bool) {
    let message_aibuddy = aibuddy_message_meta(message, steer);
    let aibuddy_value = meta
        .entry("aibuddy".to_string())
        .or_insert_with(|| serde_json::Value::Object(serde_json::Map::new()));

    if let serde_json::Value::Object(aibuddy) = aibuddy_value {
        aibuddy.extend(message_aibuddy);
    } else {
        *aibuddy_value = serde_json::Value::Object(message_aibuddy);
    }
}
...
mod tests {
    ...
    fn message_meta_serializes_message_fields() {
        ...
        assert_eq!(
            message_meta(&message).get("aibuddy"),
        ...
        assert_eq!(
            message_meta(&steer_message).get("aibuddy"),
            Some(&serde_json::json!({
                "created": 1_700_000_000,
                "messageId": "msg_live",
                "steer": true,
        ...
        assert_eq!(
            message_meta_without_steer(&steer_message).get("aibuddy"),
            Some(&serde_json::json!({
                "created": 1_700_000_000,
                "messageId": "msg_live",
            })),
        ...
        assert_eq!(
            message_meta(&message_without_id).get("aibuddy"),
            Some(&serde_json::json!({
                "created": 1_700_000_000,
            })),
        );
        ...
        assert_eq!(
            message_meta(&limited_message).get("aibuddy"),
            Some(&serde_json::json!({
                "created": 1_700_000_000,
                "messageId": "msg_live",
    ...
    fn content_chunk_carries_message_id_and_metadata() {
        ...
        assert_eq!(
            chunk.meta.as_ref().and_then(|meta| meta.get("aibuddy")),
            Some(&serde_json::json!({
                "created": 1_700_000_000,
                "messageId": "msg_live",
                "outputTokenLimitReached": true,
    ...
    #[test]
    fn merge_message_meta_replaces_non_object_aibuddy_metadata() {
