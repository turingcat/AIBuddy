...
pub(crate) fn configured_model_for_provider(
    ...
    if config.get_aibuddy_provider().ok().as_deref() == Some(provider_name) {
        config
            .get_aibuddy_model()
...
mod tests {
    ...
    fn configured_model_is_not_reused_for_another_provider() {
        ...
        config.set_aibuddy_provider("openai").unwrap();
        config.set_aibuddy_model("gpt-5").unwrap();
    ...
    fn configured_model_is_used_for_the_active_provider() {
        ...
        let config =
            crate::config::Config::new(directory.path().join("config.yaml"), "test").unwrap();
        config.set_aibuddy_provider("pi-acp").unwrap();
        config
            .set_aibuddy_model("anthropic/claude-sonnet-4")
            .unwrap();
