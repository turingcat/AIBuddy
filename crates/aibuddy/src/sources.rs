...
fn is_global_agent_file(path: &Path) -> bool {
    ...
    if let Some(home) = dirs::home_dir() {
        ...
        global_roots.push(home.join(".aibuddy").join("agents"));
...
fn list_agent_dirs(
    ...
    let mut dirs = Vec::new();
    {
        ...
        if let Some(working_dir) = working_dir {
            ...
            push(working_dir.join(".aibuddy").join("agents"), false);
        ...
        if let Some(home) = dirs::home_dir() {
            ...
            push(home.join(".aibuddy").join("agents"), true);
...
mod tests {
    ...
    fn project_agent_sources_reject_external_symlinked_roots() {
        ...
        for source_dir in [".agents", ".aibuddy", ".claude"] {
        ...
        assert!(!listed
            .iter()
            .any(|source| { matches!(source.name.as_str(), ".agents" | ".aibuddy" | ".claude") }));
    }
    ...
    fn project_agent_sources_preserve_directory_precedence() {
        ...
        for name in ["Agents unique", "AIBuddy unique", "Claude unique"] {
    ...
    fn list_sources_lists_builtin_skills() {
        ...
        let builtin = listed
            .iter()
            .find(|source| source.name == "aibuddy-doc-guide")
        ...
        assert_eq!(builtin.path, "builtin://skills/aibuddy-doc-guide");
    ...
    fn filesystem_skill_suppresses_same_named_builtin() {
        ...
        assert!(!builtins
            .iter()
            .any(|source| source.name == "aibuddy-doc-guide"));
    ...
    #[test]
    fn project_sources_prefer_agents_directory_over_legacy_aibuddy() {
        let tmp = TempDir::new().unwrap();
        ...
        std::fs::write(
            ...
            build_skill_md("shared-skill", "legacy", "AIBuddy", &HashMap::new()),
        )
        .unwrap();
    ...
    fn update_rejects_path_traversal() {
        let tmp = TempDir::new().unwrap();
        let project = tmp.path();
        let escaped_dir = project.join(".aibuddy").join("escaped");
        ...
        let attempted_escape = project.join(".aibuddy").join("escaped");
        let err = update_source_with_roots(
            SourceType::Skill,
