...
//! ```
//!
//! AIBuddy currently supports `type: "command"` actions. Unknown event names and
...
pub struct HookContext {
    ...
    /// Stable identifier for one tool call, the same value aibuddy records as
...
impl HookManager {
    ...
    async fn run_action(
        ...
        let span = tracing::info_span!(
            target: "aibuddy::hooks",
            "execute_hook",
            "gen_ai.operation.name" = "execute_hook",
            "aibuddy.hook.event" = %event,
            "aibuddy.hook.plugin" = %rule.plugin_name,
...
mod tests {
    ...
    async fn command_hooks_repair_path_when_enabled() {
        ...
        let _guard = env_lock::lock_env([
            ("AIBUDDY_SHELL", Some(fake_shell.as_str())),
            ("FAKE_LOGIN_PATH", Some(fake_login_path.as_str())),
            (
                "PATH",
                Some(
                    "/Applications/AIBuddy.app/Contents/Resources/bin:/usr/bin:/bin:/usr/sbin:/sbin",
                ),
            ),
        ]);
