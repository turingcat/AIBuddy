use crate::acp::server::{
    AcpBuiltinSelection, AcpProviderFactory, ActiveRunRegistry, HeyBuddyAcpAgent,
    HeyBuddyAcpAgentOptions,
};
...
impl AcpServer {
    ...
    /// headless `heybuddy serve` runs scheduled jobs; on failure `create_agent`
    ...
    pub async fn create_agent(&self) -> Result<Arc<HeyBuddyAcpAgent>> {
    ...
    /// server's configured default. Used by the roaming bridge on `heybuddy
    ...
    pub async fn create_agent_with_session_cwd(
        ...
        let agent = HeyBuddyAcpAgent::new(HeyBuddyAcpAgentOptions {
            ...
            heybuddy_platform: self.config.heybuddy_platform.clone(),
