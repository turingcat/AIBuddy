...
use crate::agents::mcp_client::{HeyBuddyMcpHostInfo, McpClientTrait};
use crate::agents::platform_extensions::developer::DeveloperClient;
use crate::agents::state_machine::{
    has_unapplied_tool_confirmation_response, pending_tool_confirmations,
};
use crate::agents::{
    Agent, AgentConfig, ExtensionConfig, ExtensionLoadResult, HeyBuddyPlatform, SessionConfig,
};
...
pub(super) const DEFAULT_PROVIDER_ID: &str = "heybuddy";
pub(super) const DEFAULT_PROVIDER_LABEL: &str = "HeyBuddy (Default)";
...
/// a durable, append-only exchange of messages. Internally, heybuddy also has a concept
...
/// Per-session active-run registry, shared by every `HeyBuddyAcpAgent` created
...
pub struct HeyBuddyAcpAgentOptions {
...
pub struct HeyBuddyAcpAgent {
    sessions: Arc<Mutex<HashMap<String, HeyBuddyAcpSession>>>,
...
fn agent_capabilities_meta() -> Option<Meta> {
    let mut heybuddy = serde_json::Map::new();
    heybuddy.insert("recipeParameterScopes".to_string(), serde_json::json!({}));
...
#[derive(Debug, Default, Deserialize)]
struct HeyBuddyClientCapabilities {
    #[serde(rename = "mcpHostCapabilities", default)]
    mcp_host_capabilities: Option<HeyBuddyMcpHostCapabilities>,
...
#[derive(Debug, Default, Deserialize)]
struct HeyBuddyMcpHostCapabilities {
    #[serde(default)]
...
fn extract_client_mcp_host_info(
    ...
    heybuddy_client_capabilities: Option<&HeyBuddyClientCapabilities>,
) -> HeyBuddyMcpHostInfo {
    let host_capabilities =
        heybuddy_client_capabilities.and_then(|heybuddy| heybuddy.mcp_host_capabilities.as_ref());
...
fn extract_use_login_shell_path(args: &InitializeRequest) -> bool {
    args.meta
        .as_ref()
        .and_then(|meta| meta.get("heybuddy/useLoginShellPath"))
...
fn initial_session_extensions(
    ...
    project_root: &Path,
    mcp_servers: Vec<McpServer>,
    heybuddy_extensions: Option<Vec<HeyBuddyExtension>>,
    ...
    } else if let Some(heybuddy_extensions) = heybuddy_extensions {
        for extension in extensions::heybuddy_extensions_to_configs(heybuddy_extensions)? {
...
fn resolve_default_provider_model_config(
    config: &Config,
) -> Result<(String, heybuddy_providers::model::ModelConfig), agent_client_protocol::Error> {
    let resolved_provider = config.get_heybuddy_provider().map_err(|error| {
        agent_client_protocol::Error::internal_error()
            .data(format!("Failed to resolve provider: {}", error))
    })?;
    let resolved_model = config.get_heybuddy_model().map_err(|error| {
        agent_client_protocol::Error::internal_error()
...
async fn resolve_provider_default_model_config(
    provider_name: &str,
) -> Result<heybuddy_providers::model::ModelConfig, agent_client_protocol::Error> {
...
pub(super) struct UsageUpdates {
    pub(super) custom: HeyBuddySessionNotification,
...
pub(super) fn build_usage_updates(
    ...
    UsageUpdates {
        custom: HeyBuddySessionNotification {
            session_id: session.id.clone(),
            update: HeyBuddySessionUpdate::UsageUpdate(SessionUsageUpdate {
                used,
...
impl HeyBuddyAcpAgent {
    ...
    pub(super) fn supports_heybuddy_custom_notifications(&self) -> bool {
        self.client_supports_heybuddy_custom_notifications
            .get()
            .copied()
            .unwrap_or(false)
    }
    ...
    // TODO: heybuddy reads Paths::in_state_dir globally (e.g. RequestLog), ignoring this data_dir.
    pub async fn new(options: HeyBuddyAcpAgentOptions) -> Result<Self> {
        ...
        let agent_config = AgentConfig::new(
            ...
            options.scheduler,
            Config::global().get_heybuddy_mode().unwrap_or_default(),
            options.disable_session_naming,
            options.heybuddy_platform.clone(),
        );
    ...
    fn spawn_provider_inventory_refresh(&self, heybuddy_session: &Session, agent: &Arc<Agent>) {
        let Some(provider_name) = heybuddy_session.provider_name.clone() else {
            return;
        };
        let inventory_service = self.provider_inventory.clone();
        let agent = agent.clone();
        let session_id = heybuddy_session.id.clone();
        tokio::spawn(async move {
    ...
    async fn register_acp_session(&self, session_id: String, agent: Arc<Agent>) {
        let acp_session = HeyBuddyAcpSession {
            agent: agent.clone(),
        };
        self.sessions
            .lock()
...
fn extract_client_supports_heybuddy_custom_notifications(
    heybuddy_client_capabilities: Option<&HeyBuddyClientCapabilities>,
) -> bool {
    heybuddy_client_capabilities
        .and_then(|heybuddy| heybuddy.custom_notifications)
        .unwrap_or(false)
}
...
fn extract_client_supports_recipe_param_requests(
    ...
    heybuddy_client_capabilities
        .and_then(|heybuddy| heybuddy.recipe_parameter_requests)
        .unwrap_or(false)
}
...
impl HeyBuddyAcpAgent {
    async fn on_initialize(
        ...
        let _ = self.client_supports_heybuddy_custom_notifications.set(
            extract_client_supports_heybuddy_custom_notifications(
                heybuddy_client_capabilities.as_ref(),
            ),
        );
        let _ = self.client_supports_recipe_param_requests.set(
            extract_client_supports_recipe_param_requests(heybuddy_client_capabilities.as_ref()),
        );
        let client_requests_tool_call_label_enrichment = heybuddy_client_capabilities
            .as_ref()
            .and_then(|heybuddy| heybuddy.tool_call_label_enrichment)
            .unwrap_or(false);
        let _ = self
        ...
        Ok(InitializeResponse::new(args.protocol_version)
            .agent_info(Implementation::new("heybuddy", env!("CARGO_PKG_VERSION")))
            .agent_capabilities(capabilities)
            .auth_methods(vec![AuthMethod::Agent(
                AuthMethodAgent::new("heybuddy-provider", "Configure Provider")
                    .description("Run `heybuddy configure` to set up your AI provider and API key"),
            )]))
    }
    ...
    async fn start_active_run(
        ...
        if let Some(active_run) = active_prompt_runs.get(session_id) {
            return Err(agent_client_protocol::Error::invalid_params().data(format!(
                "session already has active run `{}`; use _heybuddy/unstable/session/steer",
    ...
    async fn build_config_update(
        ...
        let heybuddy_mode = agent.heybuddy_mode().await;
        let inventory = self
            .provider_inventory
        ...
        let mode_state = build_mode_state(heybuddy_mode)?;
    ...
    async fn on_set_mode(
        ...
        let mode = mode_id.parse::<HeyBuddyMode>().map_err(|_| {
            agent_client_protocol::Error::invalid_params()
        ...
        let agent = self.get_session_agent(session_id).await?;
        agent
            .update_heybuddy_mode(mode, session_id)
            .await
            .internal_err_ctx("Failed to update mode")?;

        // heybuddy_mode is already updated on the session above.
...
pub struct HeyBuddyAcpHandler {
    pub agent: Arc<HeyBuddyAcpAgent>,
}

pub fn serve<R, W>(
    agent: Arc<HeyBuddyAcpAgent>,
    read: R,
    write: W,
...
{
    Box::pin(async move {
        let handler = HeyBuddyAcpHandler { agent };
...
pub async fn run(builtins: Vec<String>, enable_scheduler: bool) -> Result<()> {
    ...
    let server = crate::acp::server_factory::AcpServer::new(
        crate::acp::server_factory::AcpServerFactoryConfig {
            ...
            heybuddy_platform: HeyBuddyPlatform::HeyBuddyCli,
...
mod tests {
    ...
    use heybuddy_providers::conversation::token_usage::Usage as TokenUsage;
...
```",
    ...
    fn convert_acp_prompt_preserves_audience_for_converted_blocks() {
        ...
        let message = HeyBuddyAcpAgent::convert_acp_prompt_to_message(&prompt);
    ...
    fn test_build_usage_update_clamps_negative_used_to_zero() {
        ...
        session.model_config = Some(
            heybuddy_providers::model::ModelConfig::new("test-model")
        ...
        let usage = match updates.custom.update {
            HeyBuddySessionUpdate::UsageUpdate(usage) => usage,
    ...
    #[test]
    fn test_heybuddy_custom_notifications_capability_defaults_to_false() {
        let request = InitializeRequest::new(agent_client_protocol::schema::ProtocolVersion::V1);
        let heybuddy_client_capabilities =
            extract_client_capabilities_meta(&request).and_then(|meta| meta.heybuddy);
    ...
    #[test]
    fn test_heybuddy_custom_notifications_capability_reads_client_meta() {
        let mut heybuddy_meta = serde_json::Map::new();
        heybuddy_meta.insert(
        ...
        meta.insert(
            "heybuddy".to_string(),
            serde_json::Value::Object(heybuddy_meta),
        );
    ...
    fn test_tool_call_label_enrichment_capability() {
        ...
        let heybuddy_client_capabilities =
            extract_client_capabilities_meta(&request).and_then(|meta| meta.heybuddy);
        assert!(!heybuddy_client_capabilities
            .and_then(|heybuddy| heybuddy.tool_call_label_enrichment)
            .unwrap_or(false));
    ...
    async fn asynchronous_provider_effort_update_is_forwarded_to_client() {
        ...
        let server = Arc::new(
            HeyBuddyAcpAgent::new(HeyBuddyAcpAgentOptions {
                provider_factory,
        ...
        session_agent
            .update_provider(
                provider.clone(),
                heybuddy_providers::model::ModelConfig::new("gpt-4o").with_merged_request_params(
