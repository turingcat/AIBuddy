...
///
/// Replaces the removed `SessionModelState` ACP schema type; heybuddy now surfaces
...
pub(super) fn build_mode_state(
    current_mode: HeyBuddyMode,
) -> Result<SessionModeState, agent_client_protocol::Error> {
    let mut available = Vec::with_capacity(HeyBuddyMode::VARIANTS.len());
    for &name in HeyBuddyMode::VARIANTS {
        let heybuddy_mode: HeyBuddyMode = name.parse().map_err(|_| {
            agent_client_protocol::Error::internal_error() // impossible but satisfy linters
                .data(format!("Failed to parse HeyBuddyMode variant: {}", name))
        })?;
        let mut mode = SessionMode::new(SessionModeId::new(name), name);
        mode.description = heybuddy_mode.get_message().map(Into::into);
        available.push(mode);
    }
...
/// The provider decides whether heybuddy owns the effort menu; a session without a
...
pub(super) async fn build_session_setup_config(
    ...
    let mode_state = build_mode_state(session.heybuddy_mode)?;
...
/// heybuddy fall back to the model-name based values, which the ACP sentinel model
...
/// The heybuddy-side value that will actually be sent wins — `resolve_effort_value`
/// is shared with the provider's send path — then whatever the agent currently
/// has, which is what it keeps when heybuddy sends nothing.
...
fn current_thinking_effort_value(model_config: &ModelConfig) -> String {
    if model_config.is_reasoning_model() {
        model_config
            .thinking_effort()
            .or_else(|| Config::global().get_heybuddy_thinking_effort())
...
mod tests {
    ...
    use heybuddy_providers::thinking::ThinkingEffortOption;
    ...
    #[test_case(
        build_mode_state(HeyBuddyMode::Auto).unwrap(),
        "openai",
        vec![
    ...
    #[test_case(
        build_mode_state(HeyBuddyMode::Approve).unwrap(),
        "openai",
    ...
    fn test_build_config_options_uses_current_thinking_effort() {
        let mode_state = build_mode_state(HeyBuddyMode::Auto).unwrap();
