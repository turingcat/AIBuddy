...
use crate::acp::server::HeyBuddyAgentConnection;
use crate::acp::server_factory::AcpServer;

// The upstream ACP HTTP server only supports exact origin allowlists for
// WebSocket upgrades; HeyBuddy applies its richer loopback predicate before this.
...
/// `heybuddy serve`. This allows the `x-secret-key` auth header the proxy routes
/// rely on.
...
fn create_acp_router_inner(server: Arc<AcpServer>, policy: AcpOriginPolicy) -> Router {
    AcpHttpServer::new(move || HeyBuddyAgentConnection::new(server.clone()))
...
/// or heybuddy-specific auxiliary routes.
...
/// The full standalone ACP server router used by `heybuddy serve`: ACP transport,
