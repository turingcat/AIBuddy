...
pub(crate) fn configured_model_for_provider(
    ...
    if config.get_heybuddy_provider().ok().as_deref() == Some(provider_name) {
        config
            .get_heybuddy_model()
...
mod tests {
    ...
    fn configured_model_is_not_reused_for_another_provider() {
        ...
        config.set_heybuddy_provider("openai").unwrap();
        config.set_heybuddy_model("gpt-5").unwrap();
    ...
    fn configured_model_is_used_for_the_active_provider() {
        ...
        let config =
            crate::config::Config::new(directory.path().join("config.yaml"), "test").unwrap();
        config.set_heybuddy_provider("pi-acp").unwrap();
        config
            .set_heybuddy_model("anthropic/claude-sonnet-4")
            .unwrap();
