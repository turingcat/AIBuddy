...
impl HeyBuddyAcpAgent {
    ...
    pub(super) async fn handle_fork_session(
        ...
        let heybuddy_session = self
            .prepare_session_for_activation(new_session.clone(), cwd, args.mcp_servers, true)
            .await?;

        let (agent, extension_results) = self
            .prepare_acp_session_agent(cx, &heybuddy_session)
            .await?;
        self.apply_session_recipe(&agent, &heybuddy_session).await?;
        self.register_acp_session(heybuddy_session.id.clone(), agent.clone())
            .await;
        let provider = agent
            .provider()
            .await
            .internal_err_ctx("Failed to get provider while forking ACP session")?;
        resume_saved_provider_session(&provider, heybuddy_session.conversation.as_ref()).await;
        let effort_support = agent_thinking_effort_support(&agent).await;

        let acp_session_id = SessionId::new(new_session_id.clone());
        let mut meta = session_meta(&heybuddy_session);
