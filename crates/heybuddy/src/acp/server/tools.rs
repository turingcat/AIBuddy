...
use heybuddy_sdk_types::custom_requests::{ToolListItem, ToolPermissionLevel};
...
impl HeyBuddyAcpAgent {
            ...
            .map(|tool| {
                let permission = permission_manager
                    ...
                    .or_else(|| {
                        ...
                        } else if heybuddy_mode == HeyBuddyMode::Approve {
    ...
    pub(super) async fn on_call_tool(
        &self,
        req: HeyBuddyToolCallRequest,
    ) -> Result<HeyBuddyToolCallResponse, agent_client_protocol::Error> {
        let session_id = &req.session_id;
        let agent = self.get_session_agent(&req.session_id).await?;
        ...
        if agent.heybuddy_mode().await != HeyBuddyMode::Auto {
            return Err(agent_client_protocol::Error::invalid_params()
