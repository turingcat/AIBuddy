...
impl HeyBuddyAcpAgent {
    pub(super) async fn on_add_session_extension(
        ...
        let config = heybuddy_extension_to_config_without_secrets(req.extension)?;
    ...
    pub(super) async fn on_get_config_extensions(
        ...
        let extensions = extensions
            .into_iter()
            .map(config_entry_to_heybuddy_entry)
    ...
    pub(super) async fn on_add_config_extension(
        ...
        let conversion = heybuddy_extension_to_config(req.extension)?;
...
fn session_configs_to_entries(
    ...
    for config in configs {
        ...
        if let Some(extension) = config_to_heybuddy_extension(&config)? {
...
fn config_to_heybuddy_extension(
    config: &ExtensionConfig,
) -> Result<Option<HeyBuddyExtension>, agent_client_protocol::Error> {
...
fn heybuddy_extension_to_config(
...
fn heybuddy_extension_to_config_without_secrets(
    extension: HeyBuddyExtension,
) -> Result<ExtensionConfig, agent_client_protocol::Error> {
    let conversion = heybuddy_extension_to_config(extension)?;
...
pub(super) fn heybuddy_extensions_to_configs(
    extensions: Vec<HeyBuddyExtension>,
) -> Result<Vec<ExtensionConfig>, agent_client_protocol::Error> {
    extensions
        .into_iter()
        .map(heybuddy_extension_to_config_without_secrets)
        .collect()
}

fn config_entry_to_heybuddy_entry(
    entry: ExtensionEntry,
) -> Result<Option<HeyBuddyExtensionEntry>, agent_client_protocol::Error> {
    let config_key = entry.config.key();
    let Some(extension) = config_to_heybuddy_extension(&entry.config)? else {
        return Ok(None);
    };
    Ok(Some(HeyBuddyExtensionEntry {
        extension,
...
mod tests {
    ...
    #[test]
    fn builtin_config_converts_to_heybuddy_builtin_extension() {
        ...
        let extension = config_to_heybuddy_extension(&config)
            .expect("conversion should succeed")
            .expect("builtin should be supported");

        let HeyBuddyExtension::Builtin {
            name,
            description,
            display_name,
            timeout,
    ...
    #[test]
    fn platform_config_converts_to_heybuddy_platform_extension() {
        ...
        let HeyBuddyExtension::Platform {
            name,
            description,
            display_name,
            bundled,
    ...
    #[test]
    fn stdio_config_converts_to_heybuddy_mcp_extension_without_literal_envs() {
    ...
    #[test]
    fn streamable_http_config_converts_to_heybuddy_mcp_extension_without_literal_envs() {
    ...
    #[test]
    fn heybuddy_mcp_stdio_extension_converts_to_config_without_literal_envs() {
        let extension = HeyBuddyExtension::Mcp {
            server: Box::new(McpServer::Stdio(
        ...
        let conversion =
            heybuddy_extension_to_config(extension).expect("conversion should succeed");
    ...
    #[test]
    fn heybuddy_mcp_stdio_extension_extracts_literal_envs_for_config_add() {
        let extension = HeyBuddyExtension::Mcp {
            server: Box::new(McpServer::Stdio(
    ...
    #[test]
    fn heybuddy_mcp_streamable_http_extension_converts_to_config_without_literal_envs() {
        let extension = HeyBuddyExtension::Mcp {
            server: Box::new(McpServer::Http(
    ...
    #[test]
    fn heybuddy_builtin_extension_converts_to_config() {
        let builtin = HeyBuddyExtension::Builtin {
            name: "developer".to_string(),
        ...
        let conversion = heybuddy_extension_to_config(builtin).expect("conversion should succeed");
        assert!(conversion.secret_updates.is_empty());
    ...
    #[test]
    fn heybuddy_platform_extension_converts_to_config() {
        let platform = HeyBuddyExtension::Platform {
            name: "todo".to_string(),
        ...
        let conversion = heybuddy_extension_to_config(platform).expect("conversion should succeed");
        assert!(conversion.secret_updates.is_empty());
    ...
    #[test]
    fn heybuddy_mcp_sse_extension_is_rejected_for_config_add() {
        let extension = HeyBuddyExtension::Mcp {
        ...
        assert!(heybuddy_extension_to_config(extension).is_err());
    }
}
