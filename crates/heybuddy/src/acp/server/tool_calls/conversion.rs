...
pub(crate) fn heybuddy_tool_call_meta(tool_request: &ToolRequest) -> Option<Meta> {
...
fn build_initial_tool_call(tool_request: &ToolRequest, include_generated_title: bool) -> ToolCall {
    ...
    let heybuddy_meta = heybuddy_tool_call_meta(tool_request);
...
pub(crate) fn trusted_update_meta(tool_response: &ToolResponse) -> Option<Meta> {
    ...
    meta_map.insert("heybuddy".to_string(), heybuddy_meta);
    Some(meta_map)
}
...
mod tests {
    ...
    mod build_initial_tool_call {
        ...
        fn uses_default_title_and_preserves_request_data() {
            ...
            let request = ToolRequest {
                ...
                metadata: None,
                tool_meta: Some(serde_json::json!({"heybuddy_extension": "developer"})),
            };
            ...
            assert_eq!(
                tool_call
                    .meta
                    .as_ref()
                    .and_then(|meta| meta.get("heybuddy")),
    ...
    mod heybuddy_tool_call_meta {
        use super::*;

        #[test]
        fn prefers_heybuddy_extension_metadata_over_name_prefix() {
            let request = ToolRequest {
                id: "req_1".to_string(),
                tool_call: Ok(CallToolRequestParams::new("other__query-docs")),
                metadata: None,
                tool_meta: Some(serde_json::json!({"heybuddy_extension": "context7"})),
            };

            let meta = heybuddy_tool_call_meta(&request).expect("expected metadata");
    ...
    mod extract_tool_locations_from_request {
        ...
        fn accepts_unprefixed_developer_tool_from_metadata() {
            ...
            request.tool_meta = Some(serde_json::json!({"heybuddy_extension": "developer"}));
    ...
    mod trusted_update_meta {
        ...
        #[test]
        fn ignores_untrusted_heybuddy_meta() {
