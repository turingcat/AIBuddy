use super::{
    build_session_info, is_acp_visible_session_type, meta_string, HeyBuddyAcpAgent, ResultExt,
...
struct SessionListCursorToken {
    ...
    // HeyBuddy stores timestamps with second precision in common write paths, so the
...
fn include_last_message_snippet_from_meta(
    ...
    let Some(value) = meta.and_then(|meta| meta.get("heybuddy")) else {
        return Ok(false);
    };
    if value.is_null() {
        return Ok(false);
    ...
    let Some(heybuddy_meta) = value.as_object() else {
        return Err(
            agent_client_protocol::Error::invalid_params().data("heybuddy must be an object")
        );
    };
    let Some(value) = heybuddy_meta.get("includeLastMessageSnippet") else {
        return Ok(false);
    };
    if value.is_null() {
        return Ok(false);
    ...
    value.as_bool().ok_or_else(|| {
        agent_client_protocol::Error::invalid_params()
            .data("heybuddy.includeLastMessageSnippet must be a boolean")
    })
}
