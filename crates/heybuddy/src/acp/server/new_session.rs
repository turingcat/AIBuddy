use crate::acp::custom_requests::HeyBuddyExtension;
...
struct NewSessionMetaFields {
    ...
    /// Client-supplied title, recorded as user-set so heybuddy's own name
...
impl HeyBuddyAcpAgent {
    pub(super) async fn handle_new_session(
        ...
        let current_mode: HeyBuddyMode = config.get_heybuddy_mode().unwrap_or_default();
    ...
    async fn configure_new_session(
        ...
        let heybuddy_extensions = meta_heybuddy_extensions(args.meta.as_ref())?;
    ...
    async fn resolve_provider_and_model(
        ...
        let recipe_provider = recipe_settings.and_then(|s| s.heybuddy_provider.clone());
        let recipe_model = recipe_settings.and_then(|s| s.heybuddy_model.clone());
            ...
            None => match meta_string(meta, "provider")? {
                ...
                None => {
                    if let Some(model) = recipe_model.as_deref() {
                        let provider = config.get_heybuddy_provider().map_err(|error| {
...
fn meta_heybuddy_extensions(
    meta: Option<&Meta>,
) -> Result<Option<Vec<HeyBuddyExtension>>, agent_client_protocol::Error> {
