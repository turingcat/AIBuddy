...
const HEYBUDDY_CONFIG_PREFIX: &str = "heybuddy_config:";
...
impl HeyBuddyAcpAgent {
    pub(super) async fn on_onboarding_import_scan(
        ...
        if source_filter.contains(&OnboardingImportSourceKind::HeyBuddyConfig) {
            for path in heybuddy_config_candidate_paths(&self.config_dir) {
                if let Some(candidate) = scan_heybuddy_config_candidate(&path) {
...
fn apply_onboarding_import_candidates(
    ...
    for candidate_id in &req.candidate_ids {
        match parse_candidate_id(candidate_id) {
            Some((OnboardingImportSourceKind::HeyBuddyConfig, path)) => {
                match apply_heybuddy_config_candidate(config, target_config_dir, &path) {
                    Ok(result) => {
                    ...
                    Err(error) => warnings.push(import_failure_warning(
                        OnboardingImportSourceKind::HeyBuddyConfig,
                        &path,
                        &error,
                    )),
                }
...
fn import_failure_warning(
    ...
    let source_name = match source_kind {
        OnboardingImportSourceKind::HeyBuddyConfig => "HeyBuddy configuration",
...
fn heybuddy_config_candidate_paths(config_dir: &Path) -> Vec<PathBuf> {
    let mut paths = vec![config_dir.join(CONFIG_YAML_NAME)];
    if let Some(home) = dirs::home_dir() {
        paths.push(home.join(".config").join("heybuddy").join(CONFIG_YAML_NAME));
    }
    dedupe_paths(paths)
}
...
fn scan_heybuddy_config_candidate(path: &Path) -> Option<OnboardingImportCandidate> {
    if !path.exists() {
        return None;
    }
    ...
    if mapping_contains_string(&mapping, "HEYBUDDY_PROVIDER")
        || mapping_contains_string(&mapping, "HEYBUDDY_MODEL")
    {
        counts.providers = 1;
    }
    ...
    if counts.sessions > 0 {
        warnings.push("Sessions are already shared through HeyBuddy's data store.".to_string());
    }

    Some(OnboardingImportCandidate {
        id: candidate_id(HEYBUDDY_CONFIG_PREFIX, path),
        source_kind: OnboardingImportSourceKind::HeyBuddyConfig,
        display_name: "Existing HeyBuddy configuration".to_string(),
...
fn parse_candidate_id(id: &str) -> Option<(OnboardingImportSourceKind, PathBuf)> {
    if let Some(path) = id.strip_prefix(HEYBUDDY_CONFIG_PREFIX) {
        return Some((
            OnboardingImportSourceKind::HeyBuddyConfig,
            PathBuf::from(path),
        ));
...
fn apply_heybuddy_config_candidate(
    ...
    let provider = yaml_string(&source, "HEYBUDDY_PROVIDER");
    let model = yaml_string(&source, "HEYBUDDY_MODEL");
    ...
    let extension_result = import_heybuddy_config_extensions(target_config, &source)?;
    ...
    result.warnings.push(
        "Session history already lives in the HeyBuddy data store when available.".to_string(),
    );
    Ok(result)
}
...
fn import_heybuddy_config_extensions(
    target_config: &Config,
    source: &Mapping,
...
mod tests {
    ...
    fn apply_onboarding_imports_continues_after_candidate_failure() {
        ...
        let missing_heybuddy_config = source.path().join("missing-config.yaml");
        ...
        let req = OnboardingImportApplyRequest {
            candidate_ids: vec![
                candidate_id(HEYBUDDY_CONFIG_PREFIX, &missing_heybuddy_config),
        ...
        assert!(response
            .warnings
            .iter()
            .any(|warning| warning.starts_with("Skipped HeyBuddy configuration import at ")));
    ...
    #[test]
    fn apply_heybuddy_config_imports_defaults_extensions_and_skills() {
        let source = TempDir::new().unwrap();
        let target = TempDir::new().unwrap();
...
"#,
        ...
        let result =
            apply_heybuddy_config_candidate(&target_config, target.path(), &source_config).unwrap();

        assert_eq!(result.imported.providers, 1);
        assert_eq!(result.imported.extensions, 1);
        assert_eq!(result.imported.skills, 1);
        assert_eq!(target_config.get_heybuddy_provider().unwrap(), "openai");
        assert!(target.path().join("skills").join("reviewer").exists());
    }

    #[test]
    fn apply_heybuddy_config_model_only_skips_provider_activation() {
        let source = TempDir::new().unwrap();
        let target = TempDir::new().unwrap();
        let source_config = source.path().join(CONFIG_YAML_NAME);
        fs::write(&source_config, "HEYBUDDY_MODEL: gpt-5.1\n").unwrap();
        ...
        assert!(target_config.get_heybuddy_provider().is_err());
    }
