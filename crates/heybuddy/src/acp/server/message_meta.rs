...
fn heybuddy_message_meta(
    message: &Message,
    steer: bool,
) -> serde_json::Map<String, serde_json::Value> {
    let message_meta = HeyBuddyMessageMeta {
...
fn extend_message_meta(meta: &mut Meta, message: &Message, steer: bool) {
    let message_heybuddy = heybuddy_message_meta(message, steer);
    let heybuddy_value = meta
        .entry("heybuddy".to_string())
        .or_insert_with(|| serde_json::Value::Object(serde_json::Map::new()));

    if let serde_json::Value::Object(heybuddy) = heybuddy_value {
        heybuddy.extend(message_heybuddy);
    } else {
        *heybuddy_value = serde_json::Value::Object(message_heybuddy);
    }
}
...
mod tests {
    ...
    fn message_meta_serializes_message_fields() {
        ...
        assert_eq!(
            message_meta(&message).get("heybuddy"),
        ...
        assert_eq!(
            message_meta(&steer_message).get("heybuddy"),
            Some(&serde_json::json!({
                "created": 1_700_000_000,
                "messageId": "msg_live",
                "steer": true,
        ...
        assert_eq!(
            message_meta_without_steer(&steer_message).get("heybuddy"),
            Some(&serde_json::json!({
                "created": 1_700_000_000,
                "messageId": "msg_live",
            })),
        ...
        assert_eq!(
            message_meta(&message_without_id).get("heybuddy"),
            Some(&serde_json::json!({
                "created": 1_700_000_000,
            })),
        );
        ...
        assert_eq!(
            message_meta(&limited_message).get("heybuddy"),
            Some(&serde_json::json!({
                "created": 1_700_000_000,
                "messageId": "msg_live",
    ...
    fn content_chunk_carries_message_id_and_metadata() {
        ...
        assert_eq!(
            chunk.meta.as_ref().and_then(|meta| meta.get("heybuddy")),
            Some(&serde_json::json!({
                "created": 1_700_000_000,
                "messageId": "msg_live",
                "outputTokenLimitReached": true,
    ...
    #[test]
    fn merge_message_meta_replaces_non_object_heybuddy_metadata() {
