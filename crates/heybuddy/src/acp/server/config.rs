...
impl HeyBuddyAcpAgent {
    ...
    pub(super) async fn on_config_read(
        ...
        if req.key == "HEYBUDDY_PROVIDER" || req.key == "active_provider" {
            let value = config
    ...
    pub(super) async fn on_config_upsert(
        ...
        if req.key == "HEYBUDDY_MODEL" {
            if let Some(model) = req.value.as_str() {
                if let Ok(provider) = config.get_heybuddy_provider() {
    ...
    pub(super) async fn on_config_remove(
        ...
        } else if req.key == "HEYBUDDY_PROVIDER" || req.key == "active_provider" {
            config.delete("active_provider").internal_err()?;
            config.delete("HEYBUDDY_PROVIDER").internal_err()?;
        } else if req.key == "HEYBUDDY_MODEL" {
            if let Ok(provider) = config.get_heybuddy_provider() {
                crate::config::set_active_provider(config, &provider, "").internal_err()?;
            }
            config.delete("HEYBUDDY_MODEL").internal_err()?;
        } else {
    ...
    pub(super) async fn on_defaults_read(
        ...
        Ok(DefaultsReadResponse {
            provider_id: config.get_heybuddy_provider().ok(),
            model_id: config.get_heybuddy_model().ok(),
        })
    }
...
const PREFERENCE_DEFS: &[PreferenceDef] = &[
    PreferenceDef {
        ...
        config_key: "HEYBUDDY_AUTO_COMPACT_THRESHOLD",
        prepare: prepare_auto_compact_threshold,
    },
    PreferenceDef {
        key: PreferenceKey::HeyBuddyThinkingEffort,
        config_key: "HEYBUDDY_THINKING_EFFORT",
...
fn prepare_thinking_effort(
    ...
    let Some(value) = value.as_str() else {
        return Err(agent_client_protocol::Error::invalid_params()
            .data("heybuddyThinkingEffort must be a string"));
    };
    let effort = value.parse::<ThinkingEffort>().map_err(|err| {
        agent_client_protocol::Error::invalid_params()
            .data(format!("Invalid heybuddyThinkingEffort: {err}"))
    })?;
