...
use heybuddy_providers::conversation::token_usage::{ProviderUsage, Usage};
...
use crate::config::{Config, ExtensionConfig, HeyBuddyMode};
...
use heybuddy_providers::thinking::{
...
pub struct AcpProviderConfig {
    ...
    pub mode_mapping: HashMap<HeyBuddyMode, Vec<String>>,
...
pub struct AcpProvider {
    name: String,
    heybuddy_mode: Arc<Mutex<HeyBuddyMode>>,
    mode_mapping: HashMap<HeyBuddyMode, Vec<String>>,
    ...
    /// redundant-send guard: unlike a heybuddy-side cache of the last applied
...
impl AcpProvider {
    ...
    async fn start(
        ...
        let heybuddy_mode_shared = Arc::new(Mutex::new(heybuddy_mode));
        ...
        Ok(Self {
            name,
            heybuddy_mode: heybuddy_mode_shared,
            mode_mapping,
    ...
    pub(crate) async fn send_set_mode(&self, _heybuddy_id: &str, mode_id: String) -> Result<()> {
    ...
    /// heybuddy-side record of the last sent value: the agent's `SetConfigOption`
    ...
    async fn set_effort_option(
        ...
        self.send_set_config_option(heybuddy_id, option_id.to_string(), value)
            .await
    }
...
impl Provider for AcpProvider {
    ...
    async fn get_context_limit(&self, model: &str, override_limit: Option<usize>) -> usize {
        heybuddy_providers::context_limit::ContextLimitResolver::new(self.get_name())
    ...
    async fn update_mode(&self, session_id: &str, mode: HeyBuddyMode) -> Result<(), ProviderError> {
        ...
        if let Ok(mut guard) = self.heybuddy_mode.lock() {
            *guard = mode;
        }
        Ok(())
    }
    ...
    async fn stream(
        ...
        if claim.include_context && memo.is_none() {
            // Nothing fit beside this turn's prompt, so the context never left heybuddy. Give
        ...
        let heybuddy_mode = *self
            .heybuddy_mode
            .lock()
            .map_err(|_| ProviderError::RequestFailed("heybuddy_mode lock poisoned".into()))?;

        let reject_all_tools = heybuddy_mode == HeyBuddyMode::Chat;
                ...
                match update {
                    ...
                    AcpUpdate::ToolCallStart { id, name, kind, raw_input } => {
                        ...
                        if reject_all_tools {
                            ...
                            // call. heybuddy.acp.kind preserves ACP's stable categorization for
                    ...
                    AcpUpdate::PermissionRequest { request, response_tx } => {
                        text_run = None;
                        thought_run = None;
                        if let Some(decision) = permission_decision_from_mode(heybuddy_mode) {
            ...
            .on_receive_notification(
                {
                    ...
                    let heybuddy_mode = heybuddy_mode.clone();
                        ...
                        match &notification.update {
                            SessionUpdate::CurrentModeUpdate(update) => {
                                if let Some(mode) = resolve_mode(
                                    ...
                                    if let Ok(mut guard) = heybuddy_mode.lock() {
                                        *guard = mode;
                                    }
                                }
                            }
                            ...
                            .and_then(|g| g.as_ref())
                        {
                            match notification.update {
                                ...
                                SessionUpdate::ToolCall(tool_call) => {
                                    ...
                                    // heybuddy.external_dispatch marker keeps `name` off the
...
async fn forward_child_stderr(mut stderr: tokio::process::ChildStderr) {
    ...
    loop {
        match stderr.read(&mut chunk).await {
            ...
            Err(e) => {
                tracing::debug!(target: "heybuddy::acp::child::stderr", error = %e, "stderr read error");
                break;
            }
        }
    }
...
fn emit_stderr_line(line: &mut Vec<u8>) {
    ...
    tracing::info!(target: "heybuddy::acp::child::stderr", "{}", String::from_utf8_lossy(trimmed));
    line.clear();
}
    ...
    while let Some(request) = rx.recv().await {
        match request {
                ...
                let result = match session {
                    Ok(session) => {
                        *session_state.active_id.lock().unwrap() = Some(session.session_id.clone());
                        ...
                        .await?;
                        apply_session_mode(&config, &heybuddy_mode, &cx, session).await
                    }
            ...
            ClientRequest::SetConfigOption {
                ...
                // so it is the freshest source after a heybuddy-initiated switch.
                let result: Result<()> = cx
                    .send_request(req)
                    .block_task()
                    .await
...
async fn apply_session_mode(
    ...
    let current_mode = heybuddy_mode.lock().ok().map(|mode| *mode);
...
fn initial_mode_candidates(
    config: &AcpProviderConfig,
    current_mode: Option<HeyBuddyMode>,
) -> Vec<String> {
    current_mode
...
/// Convert ACP `ToolCallContent` blocks into the rmcp `Content` shape heybuddy's
...
/// Map a heybuddy effort value onto the agent's advertised vocabulary. Values heybuddy
/// and the agent share pass through; heybuddy's own enum values map onto their
...
/// Resolve the heybuddy-side thinking effort to send to the agent: the session's
...
/// dead subprocess or dropped connection, and heybuddy's own send failures carry no
/// ACP error at all.
...
fn reverse_mode_mapping(
    mode_mapping: &HashMap<HeyBuddyMode, Vec<String>>,
) -> HashMap<String, Vec<HeyBuddyMode>> {
    let mut reverse: HashMap<String, Vec<HeyBuddyMode>> = HashMap::new();
...
fn resolve_mode(
    reverse_modes: &HashMap<String, Vec<HeyBuddyMode>>,
    mode_id: &str,
    current: &Arc<Mutex<HeyBuddyMode>>,
) -> Option<HeyBuddyMode> {
...
fn permission_decision_from_mode(heybuddy_mode: HeyBuddyMode) -> Option<PermissionDecision> {
    match heybuddy_mode {
        HeyBuddyMode::Auto => Some(PermissionDecision::AllowOnce),
        HeyBuddyMode::Chat => Some(PermissionDecision::RejectOnce),
        HeyBuddyMode::Approve | HeyBuddyMode::SmartApprove => None,
    }
}
...
mod tests {
    ...
    fn test_provider_with_tx(
        ...
        (
            AcpProvider {
                ...
                heybuddy_mode: Arc::new(Mutex::new(HeyBuddyMode::Auto)),
    ...
    async fn messages_to_prompt_prepends_handoff_context_before_latest_user() {
        ...
        assert!(memo.starts_with(
            "Conversation context from heybuddy before this ACP provider session was created:"
        ));
    ...
    async fn chat_mode_denied_tool_messages_get_distinct_provider_ids() {
        ...
        let (tx, mut rx) = mpsc::channel(1);
        let (provider, model) = test_provider_with_tx(Some(tx));
        *provider.heybuddy_mode.lock().unwrap() = HeyBuddyMode::Chat;
    ...
    fn lock_default_context_limit() -> env_lock::EnvGuard<'static> {
        env_lock::lock_env([("HEYBUDDY_CONTEXT_LIMIT", None::<&str>)])
    }

    #[tokio::test]
    async fn get_context_limit_surfaces_captured_context_size() {
        let _guard = lock_default_context_limit();
        let (provider, model) = test_provider();
        assert_eq!(
            provider.get_context_limit(&model.model_name, None).await,
            heybuddy_providers::model::DEFAULT_CONTEXT_LIMIT
        );
    ...
    async fn handoff_budget_honors_global_context_limit_override() {
        let _guard = env_lock::lock_env([("HEYBUDDY_CONTEXT_LIMIT", Some("64"))]);
        let (provider, model) = test_provider();
        provider.context_size.store(200_000, Ordering::Relaxed);
        let messages = vec![
            Message::assistant().with_text("prior answer"),
    ...
    async fn a_budget_too_small_for_a_memo_keeps_the_claim() {
        ...
        assert!(
            ...
            "context that never left heybuddy must still be handed off later"
        );
    }
    ...
    fn map_effort_value_maps_heybuddy_values_onto_agent_values(value: &str) -> Option<String> {
        map_effort_value(
    ...
    /// what heybuddy last sent: the persisted value gets re-applied.
    #[tokio::test]
    ...
    async fn apply_effort_if_changed_falls_back_to_the_global_default() {
        let _guard = env_lock::lock_env([("HEYBUDDY_THINKING_EFFORT", Some("high"))]);
        let (tx, mut rx) = mpsc::channel(1);
        let provider =
            test_provider_with_effort(tx, Some(effort_capability(&["default", "high"], "default")));
    ...
    async fn apply_effort_if_changed_skips_unmapped_value() {
        ...
        let _guard = env_lock::lock_env([("HEYBUDDY_THINKING_EFFORT", Some("medium"))]);
        let (tx, mut rx) = mpsc::channel(1);
        let provider =
            test_provider_with_effort(tx, Some(effort_capability(&["default", "high"], "default")));
    ...
    #[test_case(HeyBuddyMode::Auto)]
    #[test_case(HeyBuddyMode::Approve)]
    #[test_case(HeyBuddyMode::SmartApprove)]
    #[test_case(HeyBuddyMode::Chat)]
    fn initial_mode_candidates_empty_when_mode_negotiation_disabled(mode: HeyBuddyMode) {
    ...
    fn initial_mode_candidates_prefer_mapping_then_fallback() {
        let mapping = HashMap::from([(HeyBuddyMode::Auto, vec!["bypassPermissions".to_string()])]);
        let config = test_acp_config(mapping, Some("default".to_string()));

        assert_eq!(
            initial_mode_candidates(&config, Some(HeyBuddyMode::Auto)),
            vec!["bypassPermissions".to_string()]
        );
        assert_eq!(
            initial_mode_candidates(&config, Some(HeyBuddyMode::Chat)),
    ...
    async fn update_mode_without_mapping_skips_acp_request_but_tracks_mode() {
        ...
        provider
            .update_mode("session", HeyBuddyMode::Chat)
            .await
            .unwrap();

        assert!(rx.try_recv().is_err());
        assert_eq!(*provider.heybuddy_mode.lock().unwrap(), HeyBuddyMode::Chat);
    }
    ...
    async fn update_mode_with_mapping_sends_set_mode() {
        let (tx, mut rx) = mpsc::channel(1);
        let (mut provider, _) = test_provider_with_tx(Some(tx));
        provider.mode_mapping = HashMap::from([(HeyBuddyMode::Chat, vec!["plan".to_string()])]);
    ...
    async fn update_mode_sends_candidate_offered_by_agent() {
        ...
        let handle = tokio::spawn(async move {
            provider
                .update_mode("session", HeyBuddyMode::Auto)
                .await
                .unwrap();
            provider
        });
        ...
        let provider = handle.await.unwrap();
        assert_eq!(*provider.heybuddy_mode.lock().unwrap(), HeyBuddyMode::Auto);
    }
    ...
    async fn update_mode_errors_when_no_candidate_offered() {
        let (tx, mut rx) = mpsc::channel(1);
        let (mut provider, _) = test_provider_with_tx(Some(tx));
        provider.mode_mapping =
            HashMap::from([(HeyBuddyMode::Chat, vec!["read-only".to_string()])]);
        provider.session.lock().unwrap().response = NewSessionResponse::new("test-session")
            .modes(mode_state("agent", &["agent", "agent-full-access"]));

        let result = provider.update_mode("session", HeyBuddyMode::Chat).await;
    ...
    #[test_case(HeyBuddyMode::Auto => Some(PermissionDecision::AllowOnce) ; "auto allows")]
    #[test_case(HeyBuddyMode::Chat => Some(PermissionDecision::RejectOnce) ; "chat rejects")]
    #[test_case(HeyBuddyMode::Approve => None ; "approve defers")]
    #[test_case(HeyBuddyMode::SmartApprove => None ; "smart_approve defers")]
    fn test_permission_decision_from_mode(mode: HeyBuddyMode) -> Option<PermissionDecision> {
        permission_decision_from_mode(mode)
    }

    #[test_case(
        HashMap::from([
            (HeyBuddyMode::Auto, vec!["yolo".to_string()]),
            (HeyBuddyMode::Approve, vec!["default".to_string()]),
            (HeyBuddyMode::SmartApprove, vec!["auto_edit".to_string()]),
            (HeyBuddyMode::Chat, vec!["plan".to_string()]),
        ]),
        HashMap::from([
            ("yolo".to_string(), vec![HeyBuddyMode::Auto]),
            ("default".to_string(), vec![HeyBuddyMode::Approve]),
            ("auto_edit".to_string(), vec![HeyBuddyMode::SmartApprove]),
            ("plan".to_string(), vec![HeyBuddyMode::Chat]),
        ])
        ; "gemini provider mapping"
    )]
    #[test_case(
        HashMap::from([
            (HeyBuddyMode::Auto, vec!["bypassPermissions".to_string()]),
            (HeyBuddyMode::Approve, vec!["default".to_string()]),
            (HeyBuddyMode::SmartApprove, vec!["acceptEdits".to_string()]),
            (HeyBuddyMode::Chat, vec!["plan".to_string()]),
        ]),
        HashMap::from([
            ("bypassPermissions".to_string(), vec![HeyBuddyMode::Auto]),
            ("default".to_string(), vec![HeyBuddyMode::Approve]),
            ("acceptEdits".to_string(), vec![HeyBuddyMode::SmartApprove]),
            ("plan".to_string(), vec![HeyBuddyMode::Chat]),
        ])
    ...
    #[test_case(
        HashMap::from([
            (HeyBuddyMode::Auto, vec!["full-access".to_string(), "agent-full-access".to_string()]),
            (HeyBuddyMode::Approve, vec!["read-only".to_string()]),
            (HeyBuddyMode::SmartApprove, vec!["auto".to_string(), "agent".to_string()]),
            (HeyBuddyMode::Chat, vec!["read-only".to_string()]),
        ]),
        HashMap::from([
            ("full-access".to_string(), vec![HeyBuddyMode::Auto]),
            ("agent-full-access".to_string(), vec![HeyBuddyMode::Auto]),
            ("read-only".to_string(), vec![HeyBuddyMode::Approve, HeyBuddyMode::Chat]),
            ("auto".to_string(), vec![HeyBuddyMode::SmartApprove]),
            ("agent".to_string(), vec![HeyBuddyMode::SmartApprove]),
        ])
        ; "codex candidates for both bridge generations"
    )]
    fn test_reverse_mode_mapping(
        forward: HashMap<HeyBuddyMode, Vec<String>>,
        expected: HashMap<String, Vec<HeyBuddyMode>>,
    ) {
    ...
    fn duplicate_read_only_reverse_modes() -> HashMap<String, Vec<HeyBuddyMode>> {
        HashMap::from([
            ("full-access".to_string(), vec![HeyBuddyMode::Auto]),
            (
                "read-only".to_string(),
                vec![HeyBuddyMode::Approve, HeyBuddyMode::Chat],
            ),
            ("auto".to_string(), vec![HeyBuddyMode::SmartApprove]),
        ])
    }
    ...
    #[test_case(
        "full-access", HeyBuddyMode::Auto, Some(HeyBuddyMode::Auto)
        ; "unique mapping returns the only candidate"
    )]
    #[test_case(
        "read-only", HeyBuddyMode::Approve, Some(HeyBuddyMode::Approve)
        ; "duplicate prefers current when current is Approve"
    )]
    #[test_case(
        "read-only", HeyBuddyMode::Chat, Some(HeyBuddyMode::Chat)
        ; "duplicate prefers current when current is Chat"
    )]
    #[test_case(
        "read-only", HeyBuddyMode::Auto, Some(HeyBuddyMode::Approve)
        ; "duplicate falls back to first when current not in candidates"
    )]
    #[test_case(
        "unknown-id", HeyBuddyMode::Auto, None
        ; "unknown mode id returns None"
    )]
    fn test_resolve_mode(mode_id: &str, current: HeyBuddyMode, expected: Option<HeyBuddyMode>) {
        ...
        if mode_id == "read-only" && expected == Some(HeyBuddyMode::Approve) {
            assert!(result == Some(HeyBuddyMode::Approve) || result == Some(HeyBuddyMode::Chat));
        } else {
    ...
    fn tool_meta_pairs_external_dispatch_marker_with_acp_kind() {
        ...
        for (kind, expected) in cases {
            ...
            assert_eq!(
                tool_meta["heybuddy.acp.kind"],
                serde_json::Value::String(expected.to_string()),
                "heybuddy.acp.kind serialized wrong for kind={kind:?}"
            );
        }
    }
}
