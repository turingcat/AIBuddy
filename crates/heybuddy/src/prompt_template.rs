...
static TEMPLATE_REGISTRY: &[(&str, &str)] = &[
    (
        "system.md",
        "Main system prompt that defines heybuddy's personality and behavior",
    ),
    (
        "compaction.md",
    ...
    (
        "apps_create.md",
        "Prompt for generating new HeyBuddy apps based on the user instructions",
    ),
    (
        "apps_iterate.md",
        "Prompt for updating existing HeyBuddy apps based on feedback",
    ),
    (
        "permission_judge.md",
    ...
    (
        "plan.md",
        "Prompt used when heybuddy creates step-by-step plans. CLI only",
    ),
    (
...
fn builtin_content(name: &str) -> Option<String> {
    CORE_PROMPTS_DIR
        ...
        .or_else(|| heybuddy_context_management::templates::builtin_template(name))
}
