...
use crate::config::{HeyBuddyMode, PermissionManager};
    ...
    async fn inspect(
        ...
        for request in tool_requests {
            if let Ok(tool_call) = &request.tool_call {
                ...
                let action = match heybuddy_mode {
                    HeyBuddyMode::Chat => continue,
                    HeyBuddyMode::Auto => InspectionAction::Allow,
                    HeyBuddyMode::Approve | HeyBuddyMode::SmartApprove => {
                        ...
                        } else if heybuddy_mode == HeyBuddyMode::SmartApprove
                ...
                let reason = match &action {
                    InspectionAction::Allow => {
                        if heybuddy_mode == HeyBuddyMode::Auto {
...
mod tests {
    ...
    async fn inspect_tool(
        ...
        let mut results = inspector
            .inspect(heybuddy_test_support::TEST_SESSION_ID, &[req], &[], mode)
            .await
            .unwrap();
    ...
    #[test_case(HeyBuddyMode::Auto, false, None, InspectionAction::Allow; "auto_allows")]
    #[test_case(HeyBuddyMode::SmartApprove, true, None, InspectionAction::Allow; "smart_approve_annotation_allows")]
    #[test_case(HeyBuddyMode::SmartApprove, false, Some(PermissionLevel::AlwaysAllow), InspectionAction::RequireApproval(None); "smart_approve_ignores_legacy_cached_allow")]
    #[test_case(HeyBuddyMode::SmartApprove, false, Some(PermissionLevel::AskBefore), InspectionAction::RequireApproval(None); "smart_approve_cached_ask")]
    #[test_case(HeyBuddyMode::SmartApprove, false, None, InspectionAction::RequireApproval(None); "smart_approve_unknown_defers")]
    #[test_case(HeyBuddyMode::Approve, false, None, InspectionAction::RequireApproval(None); "approve_requires_approval")]
    #[test_case(HeyBuddyMode::Approve, false, Some(PermissionLevel::AlwaysAllow), InspectionAction::RequireApproval(None); "approve_ignores_cache")]
    #[test_case(HeyBuddyMode::Approve, true, None, InspectionAction::RequireApproval(None); "approve_ignores_annotation")]
    #[tokio::test]
