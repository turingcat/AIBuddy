...
pub fn get_active_provider(config: &Config) -> Option<String> {
    if let Ok(val) = env::var("HEYBUDDY_PROVIDER") {
        return Some(val);
    }
    if let Ok(val) = config.get_param::<String>(ACTIVE_PROVIDER_KEY) {
        return Some(val);
    }
    config.get_param::<String>("HEYBUDDY_PROVIDER").ok()
}

pub fn get_active_model(config: &Config) -> Option<String> {
    if let Ok(val) = env::var("HEYBUDDY_MODEL") {
        return Some(val);
    }
    ...
    config.get_param::<String>("HEYBUDDY_MODEL").ok()
}
...
pub fn clear_active_provider(config: &Config) -> Result<(), ConfigError> {
    for key in [ACTIVE_PROVIDER_KEY, "HEYBUDDY_PROVIDER", "HEYBUDDY_MODEL"] {
...
mod tests {
    ...
    fn test_clear_active_provider_removes_legacy_keys() {
        ...
        config.set_param("HEYBUDDY_MODEL", "claude").unwrap();
