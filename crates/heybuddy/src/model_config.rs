...
pub fn model_config_from_user_config_with_session_settings(
    ...
    let model = materialize_model_config_inner(model, provider_name, false)?
        ...
        .with_default_thinking_effort(config.get_heybuddy_thinking_effort());
...
fn apply_canonical_limits(provider_name: &str, model: ModelConfig) -> ModelConfig {
    if provider_name == heybuddy_providers::azure_foundry::AZURE_FOUNDRY_PROVIDER_NAME {
        model
    } else {
...
fn materialize_model_config_inner(
    ...
    if model.temperature.is_none() {
        model = model.with_temperature(get_heybuddy_temperature(config)?);
    }

    if model.toolshim && model.toolshim_model.is_none() {
        model = model.with_toolshim_model(get_heybuddy_toolshim_model(config)?);
    }

    model = model.with_default_max_tokens(config.get_heybuddy_max_tokens()?);

    if include_default_thinking_effort {
        model = model.with_default_thinking_effort(config.get_heybuddy_thinking_effort());
    }

    if model.cache_ttl().is_none() {
        if let Some(ttl) = get_heybuddy_cache_ttl(config)? {
            model = model.with_cache_ttl(&ttl);
        }
    }

    if provider_name == heybuddy_providers::openai::OPEN_AI_PROVIDER_NAME {
...
fn configured_fast_model_name() -> Option<String> {
    Config::global()
        .get_param::<String>("HEYBUDDY_FAST_MODEL")
        .ok()
...
///   1. `HEYBUDDY_FAST_MODEL` (user override)
...
fn base_model_config_from_user_config(
    ...
    let mut model = ModelConfig {
        ...
        context_limit: None,
        temperature: get_heybuddy_temperature(config)?,
        max_tokens: None,
        toolshim: get_heybuddy_toolshim(config)?.unwrap_or(false),
        toolshim_model: get_heybuddy_toolshim_model(config)?,
        request_params: None,
        reasoning: None,
...
pub fn with_rederived_cache_ttl(model: ModelConfig) -> Result<ModelConfig> {
    ...
    if let Some(ttl) = get_heybuddy_cache_ttl(Config::global())? {
        model = model.with_cache_ttl(&ttl);
    }
    Ok(model)
}

fn get_heybuddy_cache_ttl(config: &Config) -> Result<Option<String>> {
    match config.get_param::<String>("HEYBUDDY_CACHE_TTL") {
        Ok(ttl) => {
            let ttl = ttl.trim().to_lowercase();
            match ttl.as_str() {
                "5m" | "1h" => Ok(Some(ttl)),
                other => Err(anyhow!(
                    "HEYBUDDY_CACHE_TTL must be '5m' or '1h', got '{other}'"
                )),
            }
        }
...
fn get_heybuddy_temperature(config: &Config) -> Result<Option<f32>> {
    match config.get_param::<f32>("HEYBUDDY_TEMPERATURE") {
        Ok(temp) if temp < 0.0 => Err(anyhow!(
            "Value for 'HEYBUDDY_TEMPERATURE' is out of valid range: {temp}"
        )),
...
fn get_heybuddy_toolshim(config: &Config) -> Result<Option<bool>> {
    match config.get_param::<serde_yaml::Value>("HEYBUDDY_TOOLSHIM") {
        Ok(value) => parse_yaml_bool_config("HEYBUDDY_TOOLSHIM", value).map(Some),
        Err(ConfigError::NotFound(_)) => Ok(None),
        Err(e) => Err(e.into()),
    }
}
...
pub fn global_toolshim() -> bool {
    get_heybuddy_toolshim(Config::global())
        .ok()
        .flatten()
        .unwrap_or(false)
}

fn get_heybuddy_toolshim_model(config: &Config) -> Result<Option<String>> {
    match config.get_param::<String>("HEYBUDDY_TOOLSHIM_OLLAMA_MODEL") {
        Ok(value) if value.trim().is_empty() => Err(anyhow!(
            "Invalid value for 'HEYBUDDY_TOOLSHIM_OLLAMA_MODEL': '{value}' - cannot be empty if set"
        )),
...
mod cache_ttl_tests {
    ...
    fn env_var_populates_cache_ttl() {
        let _guard = env_lock::lock_env([("HEYBUDDY_CACHE_TTL", Some("1h"))]);
    ...
    fn absent_env_var_leaves_cache_ttl_unset() {
        let _guard = env_lock::lock_env([("HEYBUDDY_CACHE_TTL", None::<&str>)]);
        let model = materialize_model_config_inner(
            ModelConfig::new("claude-sonnet-4-5"),
            "anthropic",
            false,
    ...
    fn invalid_env_var_is_rejected() {
        let _guard = env_lock::lock_env([("HEYBUDDY_CACHE_TTL", Some("2h"))]);
