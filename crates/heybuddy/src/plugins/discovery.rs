...
fn project_settings_path(project_root: &Path, local: bool) -> PathBuf {
    ...
    project_root.join(".config").join("heybuddy").join(file)
}
...
mod tests {
    ...
    fn test_config(dir: &Path) -> Config {
        Config::new(dir.join("config.yaml"), "heybuddy-discovery-test").unwrap()
    }
    ...
    fn disabled_in_project_settings_drops_plugin() {
        ...
        write_settings(
            &project.join(".config").join("heybuddy"),
    ...
    fn project_scope_overrides_user_scope() {
        ...
        write_settings(
            &fake_home.path().join(".config").join("heybuddy"),
            r#"{"disabledPlugins":["demo"]}"#,
        );
