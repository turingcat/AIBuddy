use crate::agents::mcp_client::HeyBuddyMcpHostInfo;
use crate::agents::{Agent, AgentConfig, ExtensionLoadResult, HeyBuddyPlatform};
...
impl AgentManager {
        ...
        AGENT_MANAGER
            .get_or_try_init(|| async {
                ...
                let max_sessions = config
                    .get_heybuddy_max_active_agents()
                    .unwrap_or(DEFAULT_MAX_SESSION);
                let default_mode = config.get_heybuddy_mode().unwrap_or_default();
    ...
    async fn create_agent_locked(
        ...
        let mut mode = self.agent_config.heybuddy_mode;
        if let Ok(session) = self
            ...
            .await
        {
            mode = session.heybuddy_mode;
            info!(heybuddy_mode = %mode, session_id = %session_id, "Session loaded");
        }

        let mut config = self.agent_config.clone();
        config.heybuddy_mode = mode;
        ...
        if agent.provider().await.is_err() {
            if let Some(provider) = &*self.default_provider.read().await {
                ...
                let model_config = config
                    ...
                    .ok()
                    .zip(config.get_heybuddy_model().ok())
                    ...
                    .unwrap_or_else(|| heybuddy_providers::model::ModelConfig::new("unknown"));
                agent
...
mod tests {
    ...
    use crate::agents::{AgentConfig, HeyBuddyPlatform};
    use crate::config::permission::PermissionManager;
    ...
    #[test_case(HeyBuddyMode::Approve ; "approve")]
    #[test_case(HeyBuddyMode::Chat ; "chat")]
    #[test_case(HeyBuddyMode::SmartApprove ; "smart_approve")]
    #[tokio::test]
    async fn test_agent_inherits_session_mode(mode: HeyBuddyMode) {
        let temp_dir = TempDir::new().unwrap();
        let manager = create_test_manager(&temp_dir).await;
        ...
        assert_eq!(agent.heybuddy_mode().await, mode);
    }
    ...
    async fn test_session_mode_isolation() {
        ...
        assert_eq!(a1.heybuddy_mode().await, HeyBuddyMode::Approve);
        assert_eq!(a2.heybuddy_mode().await, HeyBuddyMode::Auto);
    }
}
