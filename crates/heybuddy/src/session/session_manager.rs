...
use heybuddy_providers::conversation::token_usage::Usage;
...
pub struct Session {
    ...
    #[serde(default, alias = "goose_mode")]
...
pub struct SessionUpdateBuilder<'a> {
    ...
    heybuddy_mode: Option<HeyBuddyMode>,
...
impl<'a> SessionUpdateBuilder<'a> {
    ...
    pub fn heybuddy_mode(mut self, mode: HeyBuddyMode) -> Self {
        self.heybuddy_mode = Some(mode);
        self
    }
...
impl SessionManager {
    ...
    pub async fn create_session(
        ...
        self.storage
            .create_session(working_dir, name, session_type, heybuddy_mode)
            .await
    }
...
impl Default for Session {
    fn default() -> Self {
        Self {
            ...
            provider_name: None,
            model_config: None,
            heybuddy_mode: HeyBuddyMode::default(),
            archived_at: None,
            project_id: None,
            parent_session_id: None,
...
fn deserialize_session_model_config(
    ...
    if provider_name == Some(heybuddy_providers::azure_foundry::AZURE_FOUNDRY_PROVIDER_NAME) {
        #[derive(Deserialize)]
...
impl sqlx::FromRow<'_, sqlx::sqlite::SqliteRow> for Session {
    fn from_row(row: &sqlx::sqlite::SqliteRow) -> Result<Self, sqlx::Error> {
        ...
        Ok(Session {
            ...
            model_config,
            heybuddy_mode: row
                .try_get::<String, _>("heybuddy_mode")
                .ok()
...
impl SessionStorage {
    ...
    async fn create_schema(pool: &Pool<Sqlite>) -> Result<()> {
        ...
        sqlx::query(
            ...
            CREATE TABLE IF NOT EXISTS sessions (
                ...
                heybuddy_mode TEXT NOT NULL DEFAULT 'auto',
        ...
        Self::migrate_legacy_session_columns(&mut tx).await?;
    ...
    async fn import_legacy_session(pool: &Pool<Sqlite>, session: &Session) -> Result<()> {
        ...
        INSERT INTO sessions (
            ...
            provider_name, model_config_json, heybuddy_mode
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        "#,
        )
        .bind(&session.id)
        ...
        .bind(session.heybuddy_mode.to_string())
        .execute(&mut *tx)
        .await?;
    ...
    async fn migrate_legacy_session_columns(tx: &mut sqlx::Transaction<'_, Sqlite>) -> Result<()> {
        let columns: Vec<String> = sqlx::query_scalar(
            "SELECT name FROM pragma_table_info('sessions') WHERE name IN ('goose_mode', 'heybuddy_mode')",
        )
        .fetch_all(&mut **tx)
        .await?;
        ...
        anyhow::ensure!(
            !columns.iter().any(|column| column == "heybuddy_mode"),
            "sessions contains both goose_mode and heybuddy_mode; refusing an ambiguous mode migration"
        );

        // This fork migration must also run on databases already at the upstream schema version.
        sqlx::query("ALTER TABLE sessions RENAME COLUMN goose_mode TO heybuddy_mode")
            .execute(&mut **tx)
            .await?;
        Ok(())
    }
        ...
        match version {
            ...
            8 => {
                let has_mode: bool = sqlx::query_scalar(
                    "SELECT EXISTS(SELECT 1 FROM pragma_table_info('sessions') WHERE name = 'heybuddy_mode')",
                )
                .fetch_one(&mut **tx)
                .await?;
                if !has_mode {
                    sqlx::query(
                        r#"
                    ALTER TABLE sessions ADD COLUMN heybuddy_mode TEXT NOT NULL DEFAULT 'auto'
                "#,
                    )
                    .execute(&mut **tx)
                    .await?;
    ...
    async fn create_session(
        ...
        let session = sqlx::query_as(
            r#"
                INSERT INTO sessions (id, name, user_set_name, session_type, working_dir, extension_data, heybuddy_mode)
                VALUES (
                    ? || '_' || CAST(COALESCE((
    ...
    async fn apply_update(&self, builder: SessionUpdateBuilder<'_>) -> Result<()> {
        ...
        add_update!(builder.heybuddy_mode, "heybuddy_mode");
        ...
        if let Some(heybuddy_mode) = builder.heybuddy_mode {
            q = q.bind(heybuddy_mode.to_string());
        }
    ...
    async fn list_sessions_matching(&self, query: SessionListQuery<'_>) -> Result<Vec<Session>> {
        ...
        let sql = format!(
            ...
            SELECT s.id, s.working_dir, s.name, s.description, s.user_set_name, s.session_type, s.created_at, s.updated_at, s.extension_data,
                   ...
                   s.provider_name, s.model_config_json, s.heybuddy_mode,
    ...
    async fn import_session(
        ...
        let normalized = super::import_formats::convert_to_heybuddy_session_json(json)?;
    ...
    async fn copy_session(
        ...
        let new_session = self
            .create_session(
                ...
                original_session.heybuddy_mode,
            )
            .await?;
        ...
        builder = builder.heybuddy_mode(original_session.heybuddy_mode);
...
mod tests {
    ...
    use heybuddy_providers::conversation::token_usage::{CostSource, ProviderUsage};
    ...
    async fn legacy_brand_database() -> (TempDir, Session) {
        ...
        let session = manager
            .create_session(
                PathBuf::from("/tmp"),
                "Retained conversation".to_string(),
                SessionType::User,
            ...
            .unwrap();
        manager
            .add_message(&session.id, &Message::user().with_text("retained message"))
            .await
            .unwrap();
        let pool = manager.storage().pool().await.unwrap();
        sqlx::query("ALTER TABLE sessions RENAME COLUMN heybuddy_mode TO goose_mode")
            .execute(pool)
            .await
            .unwrap();
        pool.close().await;
    ...
    #[tokio::test]
    async fn legacy_brand_column_migrates_at_current_version_without_losing_sessions() {
        let (dir, session) = legacy_brand_database().await;
        let manager = SessionManager::new(dir.path().to_path_buf());
        let retained = manager.get_session(&session.id, true).await.unwrap();
        assert_eq!(retained.heybuddy_mode, HeyBuddyMode::Approve);
        ...
        let created = manager
            ...
            .await
            .unwrap();
        assert_eq!(created.heybuddy_mode, HeyBuddyMode::Chat);
        let pool = manager.storage().pool().await.unwrap();
    ...
    #[tokio::test]
    async fn legacy_brand_column_migration_serializes_concurrent_startup() {
        let (dir, session) = legacy_brand_database().await;
        let first = SessionManager::new(dir.path().to_path_buf());
        let second = SessionManager::new(dir.path().to_path_buf());
        let (left, right) = tokio::join!(
            first.get_session(&session.id, false),
            second.get_session(&session.id, false)
        );
        assert_eq!(left.unwrap().heybuddy_mode, HeyBuddyMode::Approve);
        assert_eq!(right.unwrap().heybuddy_mode, HeyBuddyMode::Approve);
    }

    #[tokio::test]
    async fn legacy_brand_session_json_preserves_mode() {
        let dir = TempDir::new().unwrap();
        let manager = SessionManager::new(dir.path().to_path_buf());
        let session = manager
            .create_session(
                PathBuf::from("/tmp"),
                "Legacy export".to_string(),
                SessionType::User,
                HeyBuddyMode::Approve,
            )
            .await
        ...
        let mode = object.remove("heybuddy_mode").unwrap();
        object.insert("goose_mode".to_string(), mode);
        let restored: Session = serde_json::from_value(json).unwrap();
        assert_eq!(restored.heybuddy_mode, HeyBuddyMode::Approve);
    }
    ...
    async fn legacy_brand_column_does_not_collide_with_upstream_migration_eight() {
        ...
        let retained = manager.get_session(&session.id, true).await.unwrap();
        assert_eq!(retained.heybuddy_mode, HeyBuddyMode::Approve);
        assert!(serde_json::to_string(&retained.conversation)
            .unwrap()
            .contains("retained message"));
    }
    ...
    async fn legacy_brand_column_conflict_leaves_both_values_untouched() {
        let (dir, session) = legacy_brand_database().await;
        let manager = SessionManager::new(dir.path().to_path_buf());
        sqlx::query("ALTER TABLE sessions ADD COLUMN heybuddy_mode TEXT NOT NULL DEFAULT 'auto'")
            .execute(&manager.storage().pool)
            .await
            .unwrap();
        ...
        let row = sqlx::query("SELECT goose_mode, heybuddy_mode FROM sessions WHERE id = ?")
            .bind(&session.id)
            .fetch_one(&manager.storage().pool)
            .await
            .unwrap();
        assert_eq!(row.get::<String, _>("goose_mode"), "approve");
        assert_eq!(row.get::<String, _>("heybuddy_mode"), "auto");
    }
    ...
    async fn legacy_brand_pre_mode_schema_still_adds_default_column() {
        ...
        assert_eq!(retained.heybuddy_mode, HeyBuddyMode::default());
        assert_eq!(retained.name, session.name);
    }
    ...
    fn azure_session_model_config_preserves_suffixed_deployment_id() {
        ...
        let config = deserialize_session_model_config(
            Some(heybuddy_providers::azure_foundry::AZURE_FOUNDRY_PROVIDER_NAME),
            &json,
        )
        .unwrap();
    ...
    fn azure_session_model_config_preserves_explicit_thinking_effort() {
        ...
        assert_eq!(
            ...
            Some(heybuddy_providers::thinking::ThinkingEffort::Low)
        );
    }
    ...
    fn non_azure_session_model_config_keeps_suffix_normalization() {
        let config = deserialize_session_model_config(
            Some(heybuddy_providers::openai::OPEN_AI_PROVIDER_NAME),
    ...
    impl Provider for NamingTestProvider {
        ...
        ) -> std::result::Result<MessageStream, heybuddy_providers::errors::ProviderError> {
    ...
    #[tokio::test]
    async fn test_heybuddy_mode_persists(mode: HeyBuddyMode) {
        let temp_dir = TempDir::new().unwrap();
        let sm = SessionManager::new(temp_dir.path().to_path_buf());
        ...
        let reloaded = sm.get_session(&session.id, false).await.unwrap();
        assert_eq!(reloaded.heybuddy_mode, mode);
    }

    #[tokio::test]
    async fn test_heybuddy_mode_update() {
        let temp_dir = TempDir::new().unwrap();
        let sm = SessionManager::new(temp_dir.path().to_path_buf());
        ...
        sm.update(&session.id)
            .heybuddy_mode(HeyBuddyMode::Approve)
            .apply()
            .await
            .unwrap();

        let reloaded = sm.get_session(&session.id, false).await.unwrap();
        assert_eq!(reloaded.heybuddy_mode, HeyBuddyMode::Approve);
    }

    #[tokio::test]
    async fn test_heybuddy_mode_malformed_defaults_to_auto() {
        let temp_dir = TempDir::new().unwrap();
        let sm = SessionManager::new(temp_dir.path().to_path_buf());
        ...
        sqlx::query("UPDATE sessions SET heybuddy_mode = 'garbage' WHERE id = ?")
            .bind(&session.id)
            .execute(pool)
            .await
            .unwrap();
        ...
        let reloaded = sm.get_session(&session.id, false).await.unwrap();
        assert_eq!(reloaded.heybuddy_mode, HeyBuddyMode::default());
    }
