//! Importers for non-heybuddy session formats.
//!
//! HeyBuddy's native session export is a JSON-serialized [`crate::session::Session`].
...
//!
//! The strategy is to convert any supported foreign format into heybuddy's
...
/// the heybuddy-native session JSON handed to `SessionManager::import_session`.
...
pub(crate) fn build_session_json(session: ImportedSession) -> Value {
    ...
    obj.insert("heybuddy_mode".into(), json!("auto"));
...
pub enum ImportFormat {
    /// Native heybuddy session export — a JSON object representing a `Session`.
    HeyBuddy,
...
///   and a `conversation` (or `messages`) field, it's heybuddy.
...
pub fn detect_format(content: &str) -> ImportFormat {
    ...
    if let Ok(v) = serde_json::from_str::<serde_json::Value>(first_line) {
        ...
        // Claude Code lines always include a sessionId; heybuddy's native JSON is
    ...
    // HeyBuddy's pretty-printed export starts with `{` and *eventually* contains
    ...
    ImportFormat::HeyBuddy
}

/// Convert any supported foreign format to a heybuddy-native session JSON string.
pub fn convert_to_heybuddy_session_json(content: &str) -> Result<String> {
    match detect_format(content) {
        ImportFormat::HeyBuddy => Ok(upgrade_legacy_token_fields(content)),
...
/// Exports from heybuddy versions before `usage`/`accumulated_usage` existed
