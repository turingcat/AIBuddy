...
async fn ensure_working_provider(
    ...
    let provider_name = config.get_heybuddy_provider().ok();
    let model_name = config.get_heybuddy_model().ok();
    ...
    Ok(Some(Message::assistant().with_text(format!(
        "**HeyBuddy Doctor**\n\n{}\n\n\
         No working provider found. Run `heybuddy configure` to set one up.",
        preamble,
    ))))
}
...
async fn try_create_and_test(
    provider_name: &str,
    model_name: &str,
) -> Result<(Arc<dyn Provider>, heybuddy_providers::model::ModelConfig), ProviderError> {
    let model_config =
...
async fn try_other_models(
    ...
    skip_model: &str,
    log: &mut Vec<String>,
) -> Option<(Arc<dyn Provider>, heybuddy_providers::model::ModelConfig)> {
    ...
    let toolshim = Config::global()
        .get_param::<bool>("HEYBUDDY_TOOLSHIM")
        .unwrap_or(false);
...
fn describe_error(e: &ProviderError) -> String {
    match e {
        ProviderError::NotConfigured => {
            "Provider is not configured. Run `heybuddy configure` to set it up.".to_string()
        }
        ProviderError::Authentication(_) => {
            "Authentication failed — check your API key. Run `heybuddy configure` to update it."
                .to_string()
        }
