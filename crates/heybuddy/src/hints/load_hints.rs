...
pub const HEYBUDDY_HINTS_FILENAME: &str = ".heybuddyhints";
...
pub fn get_context_filenames() -> Vec<String> {
    ...
    Config::global()
        ...
        .unwrap_or_else(|_| {
            vec![
                HEYBUDDY_HINTS_FILENAME.to_string(),
...
pub fn load_hint_files(
    ...
    if !global_hints_contents.is_empty() {
        hints.push_str("\n### Global Hints\nThese are my global heybuddy hints.\n");
...
mod tests {
    ...
    #[test]
    fn test_heybuddyhints_when_present() {
        let dir = TempDir::new().unwrap();

        fs::write(
            dir.path().join(HEYBUDDY_HINTS_FILENAME),
            "Test hint content",
        )
        .unwrap();
    ...
    fn test_global_agents_md_in_agents_home() {
        ...
        std::env::set_var("HEYBUDDY_PATH_ROOT", root.path());
        ...
        std::env::remove_var("HEYBUDDY_PATH_ROOT");
    ...
    #[test]
    fn test_heybuddyhints_when_missing() {
        let dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_heybuddyhints_multiple_filenames() {
        let dir = TempDir::new().unwrap();
        ...
        fs::write(
            dir.path().join(HEYBUDDY_HINTS_FILENAME),
            "Custom hints file content from .heybuddyhints",
        )
        .unwrap();
        ...
        let hints = load_hint_files(
            dir.path(),
            &["CLAUDE.md".to_string(), HEYBUDDY_HINTS_FILENAME.to_string()],
            &gitignore,
        );

        assert!(hints.contains("Custom hints file content from CLAUDE.md"));
        assert!(hints.contains("Custom hints file content from .heybuddyhints"));
    }

    #[test]
    fn test_heybuddyhints_configurable_filename() {
        let dir = TempDir::new().unwrap();
        ...
        assert!(!hints.contains(".heybuddyhints")); // Make sure it's not loading the default
    }

    #[test]
    fn test_nested_heybuddyhints_with_git_root() {
        let temp_dir = TempDir::new().unwrap();
        let project_root = temp_dir.path();

        fs::create_dir(project_root.join(".git")).unwrap();
        fs::write(
            project_root.join(HEYBUDDY_HINTS_FILENAME),
            "Root hints content",
        )
        .unwrap();
        ...
        fs::write(subdir.join(HEYBUDDY_HINTS_FILENAME), "Subdir hints content").unwrap();
        let current_dir = subdir.join("current_dir");
        fs::create_dir(&current_dir).unwrap();
        fs::write(
            current_dir.join(HEYBUDDY_HINTS_FILENAME),
    ...
    #[test]
    fn test_nested_heybuddyhints_without_git_root() {
        let temp_dir = TempDir::new().unwrap();
        let base_dir = temp_dir.path();

        fs::write(base_dir.join(HEYBUDDY_HINTS_FILENAME), "Base hints content").unwrap();
    ...
    #[test]
    fn test_nested_heybuddyhints_mixed_filenames() {
        let temp_dir = TempDir::new().unwrap();
        let project_root = temp_dir.path();
        ...
        fs::create_dir(&subdir).unwrap();
        fs::write(
            subdir.join(HEYBUDDY_HINTS_FILENAME),
            "Subdir .heybuddyhints content",
        )
        .unwrap();
        ...
        assert!(hints.contains("Subdir .heybuddyhints content"));
    }
...
Additional instructions here."#;
        fs::write(project_root.join(HEYBUDDY_HINTS_FILENAME), hints_content).unwrap();
...
End of nested hints"#;
        fs::write(
            components_dir.join(HEYBUDDY_HINTS_FILENAME),
...
@../parent.md
End of hints"#;
        fs::write(current_dir.join(HEYBUDDY_HINTS_FILENAME), hints_content).unwrap();
...
@../root_file.md
End of hints"#;
        fs::write(subdir.join(HEYBUDDY_HINTS_FILENAME), hints_content).unwrap();
        let gitignore = create_dummy_gitignore();

        let hints = load_hint_files(&subdir, &[HEYBUDDY_HINTS_FILENAME.to_string()], &gitignore);
    ...
    fn tracker_deduplicates_directories() {
        ...
        let subdir = project_root.join("nested");
        fs::create_dir_all(&subdir).unwrap();
        fs::write(subdir.join(HEYBUDDY_HINTS_FILENAME), "nested hints").unwrap();
    ...
    fn tracker_rejects_parent_traversal_outside_working_directory() {
        ...
        fs::write(outside.join(HEYBUDDY_HINTS_FILENAME), "outside hints").unwrap();
    ...
    fn tracker_preserves_in_boundary_symlinks_and_deduplicates_aliases() {
        ...
        fs::write(real.join(HEYBUDDY_HINTS_FILENAME), "real hints").unwrap();
    ...
    fn tracker_retries_directory_after_parent_is_created() {
        ...
        fs::write(future.join(HEYBUDDY_HINTS_FILENAME), "future hints").unwrap();
        tracker.record_tool_arguments(&Some(args), &project_root);
