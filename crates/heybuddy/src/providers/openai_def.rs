...
use heybuddy_providers::base::ProviderDescriptor;
...
use heybuddy_providers::api_client::{ApiClient, AuthMethod};
use heybuddy_providers::openai::{
...
impl ProviderDescriptor for OpenAiProviderDef {
    fn metadata() -> heybuddy_providers::base::ProviderMetadata {
...
pub async fn from_env(
    tls_config: Option<heybuddy_providers::api_client::TlsConfig>,
    ...
    //      honoured so that `OPENAI_HOST=… heybuddy` keeps working)
    //   2. OPENAI_BASE_URL (env or config) — ecosystem-standard
    //   3. OPENAI_HOST from config file — persisted by `heybuddy configure`
            ...
            match e {
                ConfigError::NotFound(_) => {
                    if config.requires_auth {
                        anyhow::bail!(
                            "Required API key {} is not set. Configure it via `heybuddy configure` or set the {} environment variable.",
                            config.api_key_env,
                            config.api_key_env
                        );
                    }
...
pub fn from_custom_config(
    ...
    heybuddy_providers::openai::from_declarative_config(
        config,
        tls_config,
