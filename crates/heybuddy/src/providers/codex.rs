...
use heybuddy_providers::request_log::{start_log, LoggerHandleExt};
use rmcp::model::Role;
use rmcp::model::Tool;
...
pub struct CodexProvider {
    ...
    #[serde(skip)]
    mode_by_session: tokio::sync::RwLock<HashMap<String, HeyBuddyMode>>,
}
...
impl CodexProvider {
    ...
    /// Apply permission flags based on HeyBuddyMode
    ...
    async fn execute_command(
        ...
        if std::env::var("HEYBUDDY_CODEX_DEBUG").is_ok() {
        ...
        // This allows users to set HEYBUDDY_PROVIDER=codex without needing to specify a model
        ...
        Self::apply_permission_flags(&mut cmd, heybuddy_mode)?;
...
impl heybuddy_providers::base::ProviderDescriptor for CodexProvider {
...
impl Provider for CodexProvider {
    ...
    async fn stream(
        ...
        let lines = self
            .execute_command(model_config, system, messages, tools, heybuddy_mode)
            .await?;
...
mod tests {
    ...
    use heybuddy_providers::base::ProviderDescriptor as _;
    use heybuddy_test_support::TEST_IMAGE_B64;
    use std::collections::HashMap;
    ...
    #[test_case(HeyBuddyMode::Auto, &["--yolo"] ; "auto_yolo")]
    #[test_case(HeyBuddyMode::SmartApprove, &["--full-auto"] ; "smart_approve_full_auto")]
    #[test_case(HeyBuddyMode::Approve, &[] as &[&str] ; "approve_no_flags")]
    #[test_case(HeyBuddyMode::Chat, &["--sandbox", "read-only"] ; "chat_read_only")]
    fn test_apply_permission_flags(mode: HeyBuddyMode, expected: &[&str]) {
