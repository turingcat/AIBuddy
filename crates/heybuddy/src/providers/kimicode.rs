...
use heybuddy_providers::formats::anthropic::AnthropicFormatOptions;
...
impl KimiCodeProvider {
    ...
    pub async fn from_env(
        ...
        let client = heybuddy_providers::provider_reqwest_builder().build()?;
    ...
    async fn post(&self, payload: &Value) -> Result<reqwest::Response, ProviderError> {
        ...
        heybuddy_providers::http_status::send_bounded(
            request,
...
impl heybuddy_providers::base::ProviderDescriptor for KimiCodeProvider {
    fn metadata() -> ProviderMetadata {
        ...
        .with_setup_steps(vec![
            "Run `heybuddy configure` and select 'Kimi Code'",
            "A browser window will open — log in to kimi.com and enter the displayed code",
            "Once authorized, HeyBuddy will save your token automatically",
        ])
    }
}
        ...
        KimiCodeProvider {
            ...
            token_cache: TokenCache {
                path: std::env::temp_dir()
                    .join(format!("heybuddy-kimicode-test-{}.json", Uuid::new_v4())),
            },
            cached_token: tokio::sync::Mutex::new(None),
