...
pub struct ClaudeCodeProvider {
    ...
    #[serde(skip)]
    initial_mode: tokio::sync::Mutex<Option<HeyBuddyMode>>,
}
...
impl ClaudeCodeProvider {
    ...
    fn build_stream_json_command(&self) -> Command {
        ...
        // Allow heybuddy to run inside a Claude Code session.
    ...
    fn apply_permission_flags(cmd: &mut Command) -> Result<bool, ProviderError> {
        ...
        match heybuddy_mode {
            ...
            HeyBuddyMode::SmartApprove | HeyBuddyMode::Approve => {
                cmd.arg("--permission-prompt-tool").arg("stdio");
                Ok(true)
            }
            HeyBuddyMode::Chat => Ok(false),
        }
    }
...
impl heybuddy_providers::base::ProviderDescriptor for ClaudeCodeProvider {
...
mod tests {
    ...
    use chrono::Utc;
    use heybuddy_test_support::session::TEST_SESSION_ID;
    use serde_json::json;
    use std::collections::HashMap;
    use std::fs;
