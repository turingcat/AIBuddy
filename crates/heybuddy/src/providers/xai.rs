...
pub fn xai_known_model_info() -> Vec<heybuddy_providers::base::ModelInfo> {
    XAI_KNOWN_MODELS
        .iter()
        .map(|model_name| {
            heybuddy_providers::base::model_info_for_provider_model(XAI_PROVIDER_NAME, model_name)
        })
        .collect()
}

impl heybuddy_providers::base::ProviderDescriptor for XaiProvider {
