...
use heybuddy_providers::formats::openai_responses::responses_api_to_streaming_message;
...
<html>
  <head>
    <title>heybuddy - ChatGPT Authorization Successful</title>
    <style>
      body {
  ...
  <body>
    <div class="container">
      ...
      <p>You can close this window and return to heybuddy.</p>
    </div>
...
<html>
  <head>
    <title>heybuddy - ChatGPT Authorization Failed</title>
    <style>
      body {{
        font-family: system-ui, -apple-system, sans-serif;
        display: flex;
...
impl ChatGptCodexProvider {
    ...
    async fn post_streaming(&self, payload: &Value) -> Result<reqwest::Response, ProviderError> {
        ...
        let client = heybuddy_providers::provider_reqwest_builder()
            .build()
            .map_err(|e| ProviderError::ExecutionError(e.to_string()))?;
        let request = client
...
impl heybuddy_providers::base::ProviderDescriptor for ChatGptCodexProvider {
...
mod tests {
    ...
    fn inventory_configured_uses_oauth_token_cache() {
        ...
        let _guard = env_lock::lock_env([("HEYBUDDY_PATH_ROOT", Some(root_path.as_str()))]);
