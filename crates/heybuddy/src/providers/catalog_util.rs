pub use heybuddy_providers::canonical::catalog::{
...
pub async fn get_providers_by_format(format: ProviderFormat) -> Vec<ProviderCatalogEntry> {
    ...
    heybuddy_providers::canonical::catalog::get_providers_by_format(format, &native_provider_ids)
}

pub async fn get_setup_catalog_entries() -> Vec<ProviderSetupCatalogEntry> {
    heybuddy_providers::canonical::catalog::get_setup_catalog_entries(
        super::providers()
            .await
            .into_iter()
...
pub fn get_provider_template(provider_id: &str) -> Option<ProviderTemplate> {
    heybuddy_providers::canonical::catalog::get_provider_template(provider_id)
}
...
mod tests {
    ...
    #[tokio::test]
    async fn setup_catalog_includes_heybuddy_and_curated_fields() {
        let entries = get_setup_catalog_entries().await;

        let heybuddy = entries
            .iter()
            .find(|entry| entry.provider_id == "heybuddy")
            .expect("setup catalog should include synthetic heybuddy");
        assert_eq!(heybuddy.category, ProviderSetupCategory::Agent);
        assert!(!heybuddy.acp);
        assert_eq!(heybuddy.setup_method, ProviderSetupMethod::None);
        assert!(heybuddy.fields.is_empty());
    ...
    async fn setup_catalog_preserves_curated_providers_and_hides_deprecated_ones() {
        ...
        for provider_id in ["heybuddy", "claude-acp", "openai"] {
