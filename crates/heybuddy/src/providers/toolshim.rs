...
use heybuddy_providers::formats::openai::create_request;
...
pub const TOOLSHIM_BACKEND_ENV_VAR: &str = "HEYBUDDY_TOOLSHIM_BACKEND";
pub const TOOLSHIM_LOCAL_MODEL_ENV_VAR: &str = "HEYBUDDY_TOOLSHIM_MODEL";
...
fn resolve_tool_name(raw_tool_name: &str, tools: &[Tool]) -> Option<String> {
    ...
    // Also try replacing dots with double-underscores (heybuddy tool name convention)
...
/// - HEYBUDDY_TOOLSHIM: When set to "true" or "1", enables using the tool shim in the standard OllamaProvider (default: false)
/// - HEYBUDDY_TOOLSHIM_OLLAMA_MODEL: Ollama model to use as the tool interpreter (default: DEFAULT_INTERPRETER_MODEL)
...
impl OllamaInterpreter {
    pub fn new() -> Result<Self, ProviderError> {
        let client = Client::builder()
            .user_agent(heybuddy_providers::PRODUCT_USER_AGENT)
...
";
        ...
        let interpreter_model = std::env::var("HEYBUDDY_TOOLSHIM_OLLAMA_MODEL")
...
mod tests {
    ...
    fn parses_tokenized_tool_call_with_windows_path_arguments() {
        ...
        let content = "<|tool_calls_section_begin|> <|tool_call_begin|> functions.tree:0 <|tool_call_argument_begin|> {\"path\": \"C:\\Users\\eugen\\programmazione\\heybuddy-fork\", \"depth\": 1} <|tool_call_end|> <|tool_calls_section_end|>";
        let calls = parse_tokenized_tool_calls(content, &tools);
        ...
        assert_eq!(
            calls[0]
                ...
                .and_then(|v| v.as_str()),
            Some("C:\\Users\\eugen\\programmazione\\heybuddy-fork")
        );
    }
