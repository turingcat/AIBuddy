...
use heybuddy_providers::api_client::{ApiClient, AuthMethod, TlsConfig};
use heybuddy_providers::base::{ModelInfo, ProviderDescriptor};
use heybuddy_providers::errors::ProviderError;
use heybuddy_providers::model::ModelConfig;
use heybuddy_providers::ollama::fetch_ollama_model_names;
use heybuddy_providers::openai::OpenAiProvider;
use rmcp::model::Tool;
...
fn build_ollama_api_client(
    ...
    let normalized_base_url = heybuddy_providers::openai::ensure_url_scheme(&config.base_url);
        ...
        Box::pin(async {
            anyhow::bail!(
                "Ollama Cloud must be configured as a declarative provider. \
                 Run `heybuddy configure` to set it up."
            )
        })
    }
}
