...
fn build_authorize_url(pkce: &PkceChallenge, state: &str, nonce: &str) -> Result<String> {
    ...
    // non-allowlisted clients. `referrer=heybuddy` lets xAI attribute
    // heybuddy-originated logins.
    let params = [
        ("response_type", "code"),
        ("client_id", CLIENT_ID),
...
async fn poll_device_code_token(device: &DeviceCodeResponse) -> Result<TokenResponse> {
    ...
    loop {
        ...
        match body.error.as_deref() {
            ...
            Some("expired_token") => {
                return Err(anyhow!(
                    "xAI device code expired - please re-run heybuddy configure"
                ));
            }
            other => {
                let detail = body
...
<html>
  <head>
    <title>heybuddy - xAI Authorization Successful</title>
    <style>
      body {
...
<html>
  <head>
    <title>heybuddy - xAI Authorization Failed</title>
    <style>
      body {{
        font-family: system-ui, -apple-system, sans-serif;
...
async fn perform_loopback_oauth_flow(auth_state: &XaiAuthState) -> Result<TokenData> {
    ...
    if webbrowser::open(&auth_url).is_err() {
        tracing::info!(
            "Please open this URL in your browser to authorize heybuddy with xAI:\n{}",
            auth_url
        );
    }
...
async fn perform_device_code_flow() -> Result<TokenData> {
    ...
    eprintln!(
        "\nTo authorize heybuddy with xAI, open this URL in any browser:\n  {}\nand enter code: {}\n",
...
impl heybuddy_providers::base::ProviderDescriptor for XaiOAuthProvider {
...
mod tests {
    ...
    fn authorize_url_contains_required_oauth_params() {
        ...
        assert!(url.contains("referrer=heybuddy"));
    ...
    #[test]
    fn token_cache_path_lives_under_heybuddy_config_dir() {
