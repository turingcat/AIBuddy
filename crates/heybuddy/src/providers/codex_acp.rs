...
use crate::config::{Config, ConfigError, HeyBuddyMode};
...
fn resolve_heybuddy_mode(
    configured_mode: Result<HeyBuddyMode, ConfigError>,
) -> Result<HeyBuddyMode, ConfigError> {
    match configured_mode {
        Ok(mode) => Ok(mode),
        Err(ConfigError::NotFound(_)) => Ok(HeyBuddyMode::Auto),
        Err(error) => Err(error),
    }
}

impl heybuddy_providers::base::ProviderDescriptor for CodexAcpProvider {
    fn metadata() -> ProviderMetadata {
        ProviderMetadata::new(
            CODEX_ACP_PROVIDER_NAME,
            "Codex ACP",
            "Use heybuddy with ChatGPT Plus/Pro or OpenAI API credits via the codex-acp adapter.",
            ACP_CURRENT_MODEL,
            vec![],
            CODEX_ACP_DOC_URL,
            vec![],
...
impl ProviderDef for CodexAcpProvider {
    ...
    fn from_env_with_working_dir(
        ...
        Box::pin(async move {
            ...
            let heybuddy_mode = resolve_heybuddy_mode(config.get_heybuddy_mode_strict())?;
            let mcp_servers = extension_configs_to_mcp_servers(&extensions);

            let mode_mapping = HashMap::from([
                (HeyBuddyMode::Auto, vec!["agent-full-access".to_string()]),
                (HeyBuddyMode::SmartApprove, vec!["agent".to_string()]),
...
mod tests {
    ...
    #[cfg(unix)]
    const CHILD_ENV: &str = "HEYBUDDY_CODEX_ACP_MODE_TEST_CHILD";
    #[cfg(unix)]
    const EXPECT_CONFIG_ERROR_ENV: &str = "HEYBUDDY_CODEX_ACP_EXPECT_CONFIG_ERROR";
    ...
    #[test]
    fn missing_heybuddy_mode_defaults_to_auto() {
        assert_eq!(
            resolve_heybuddy_mode(Err(ConfigError::NotFound("HEYBUDDY_MODE".to_string()))).unwrap(),
            HeyBuddyMode::Auto
        );
    }

    #[test]
    fn configured_heybuddy_modes_are_preserved() {
        for mode in [
            HeyBuddyMode::Auto,
            ...
            assert_eq!(resolve_heybuddy_mode(Ok(mode)).unwrap(), mode);
        }
    }

    #[test]
    fn invalid_heybuddy_mode_errors_are_preserved() {
        assert!(matches!(
            resolve_heybuddy_mode(Err(ConfigError::DeserializeError("invalid".to_string()))),
            Err(ConfigError::DeserializeError(_))
        ));
        assert!(matches!(
            resolve_heybuddy_mode(Err(ConfigError::FileError(std::io::Error::other(
                "unreadable"
            )))),
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn heybuddy_mode_validation_precedes_codex_acp_launch() {
        ...
        for (name, mode, config_fixture, should_launch) in [
            ...
            (
                "configured_file",
                None,
                ConfigFixture::File("HEYBUDDY_MODE: approve\n"),
                true,
            ),
            (
                "malformed_file",
                None,
                ConfigFixture::File("HEYBUDDY_MODE: ["),
                false,
            ),
            ...
            command
                .arg("--exact")
                .arg("providers::codex_acp::tests::heybuddy_mode_validation_precedes_codex_acp_launch")
                .arg("--nocapture")
                .env(CHILD_ENV, "1")
                .env("HEYBUDDY_SEARCH_PATHS", &search_paths)
                .env("HEYBUDDY_CODEX_ACP_MARKER", &marker)
                .env("HEYBUDDY_PATH_ROOT", &path_root)
                .env(
                    crate::config::base::TEST_SYSTEM_CONFIG_PATH_ENV,
                    path_root.join("system-config.yaml"),
                )
                .env_remove("HEYBUDDY_ADDITIONAL_CONFIG_FILES")
                .env("HEYBUDDY_DISABLE_KEYRING", "1");
            if should_launch {
            ...
            match mode {
                Some(mode) => {
                    command.env("HEYBUDDY_MODE", mode);
                }
                None => {
                    command.env_remove("HEYBUDDY_MODE");
                }
            }
            ...
            assert_eq!(
                marker.exists(),
                should_launch,
                "unexpected codex-acp launch result for {name} HEYBUDDY_MODE"
            );
        }
    }
}
