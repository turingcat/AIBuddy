...
use crate::agents::state_machine::{
    ...
    yielded_with, ConversationEffect, Emitter, HeyBuddyEffect, Operation, OperationResult,
    SlashCommand,
};
...
const COMPACTION_THINKING_TEXT: &str = "heybuddy is compacting the conversation...";
...
#[async_trait]
impl Operation<Session, HeyBuddyEffect> for CompactionOperation {
    ...
    async fn run(
        ...
        .await
        {
            Ok(result) => {
                ...
                .await;
                applied([HeyBuddyEffect::ReplaceConversation {
                    conversation: compacted,
                    usage: Some(usage),
                }])
            }
