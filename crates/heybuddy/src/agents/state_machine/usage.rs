...
use crate::agents::state_machine::{ConversationEffect, HeyBuddyEffect};
use crate::conversation::message::MessageUsage;
use crate::conversation::Conversation;
use crate::session::{Session, SessionManager};

fn attach_to_last_assistant(effects: &mut [HeyBuddyEffect], usage: &ProviderUsage) {
...
pub(super) fn enrich(session: &Session, effects: &mut [HeyBuddyEffect]) {
    for index in 0..effects.len() {
        let (usage, replaces_conversation) = match &effects[index] {
            HeyBuddyEffect::RecordUsage(usage) => (usage.clone(), false),
        ...
        match &mut effects[index] {
            HeyBuddyEffect::RecordUsage(usage) => *usage = enriched,
            HeyBuddyEffect::ReplaceConversation { usage, .. } => *usage = Some(enriched),
            _ => {}
        }
    }
}
