...
pub struct SlashCommandOperation<'a> {
    operations: Vec<Arc<dyn Operation<Session, HeyBuddyEffect> + 'a>>,
}
...
impl<'a> SlashCommandOperation<'a> {
    pub fn new(operations: Vec<Arc<dyn Operation<Session, HeyBuddyEffect> + 'a>>) -> Self {
        Self { operations }
    }
}

#[async_trait]
impl Operation<Session, HeyBuddyEffect> for SlashCommandOperation<'_> {
