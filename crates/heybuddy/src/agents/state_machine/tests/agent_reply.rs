...
use crate::acp::server::HeyBuddyAcpAgent;
use crate::agents::extension::ExtensionConfig;
use crate::agents::mcp_client::McpClientTrait;
use crate::agents::{Agent, AgentConfig, AgentEvent, HeyBuddyPlatform, SessionConfig};
...
async fn agent_with_dummy_api() -> Result<(Agent, Arc<DummyApi>, String, tempfile::TempDir)> {
    ...
    agent
        .update_provider(
            provider,
            ModelConfig::new(heybuddy_providers::openai::OPEN_AI_DEFAULT_MODEL)
...
async fn agent_with_calculator() -> Result<(
    ...
    agent
        .update_heybuddy_mode(HeyBuddyMode::Approve, &session_id)
        .await?;
...
async fn state_machine_confirmation_through_agent_resumes_tool_call() -> Result<()> {
    let _guard = env_lock::lock_env([("HEYBUDDY_STATE_MACHINE", Some("1"))]);
...
async fn assert_bang_shell_uses_only_user_visible_content() -> Result<()> {
    ...
    let hidden_text_prefix = HeyBuddyAcpAgent::convert_acp_prompt_to_message(&[
    ...
    api.on("benign visible input")
        .reply("handled as ordinary input");
    let empty_audience_text = HeyBuddyAcpAgent::convert_acp_prompt_to_message(&[
    ...
    let (agent, api, session_id, _temp_dir) = agent_with_dummy_api().await?;
    let hidden_text_suffix = HeyBuddyAcpAgent::convert_acp_prompt_to_message(&[
    ...
    api.on("benign visible input")
        .reply("handled as ordinary input");
    let hidden_resource_prefix = HeyBuddyAcpAgent::convert_acp_prompt_to_message(&[
    ...
    api.on("benign visible input")
        .reply("handled as ordinary input");
    let empty_audience_resource = HeyBuddyAcpAgent::convert_acp_prompt_to_message(&[
    ...
    let hidden_link_suffix = HeyBuddyAcpAgent::convert_acp_prompt_to_message(&[
        AcpContentBlock::Text(AcpTextContent::new("!echo visible")),
        hidden_link,
    ]);
