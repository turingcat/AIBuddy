...
//! `StateMachine::run`. HeyBuddy's concrete operations remain internal because their
...
mod session;
pub(crate) use session::run as run_heybuddy;
...
pub use effects::HeyBuddyEffect;
pub use heybuddy_agent::machine::{
    EffectHandler, EffectUsage, MachineSession, SessionLoader, StateMachine, Step,
};
pub use heybuddy_agent::operation::{
...
pub(super) use inference_preparation::HeyBuddyInferenceRequestPreparer;
...
pub(super) use ops_llm::{HeyBuddyInferenceProvider, InferenceRunner};
...
pub fn enabled() -> bool {
    std::env::var("HEYBUDDY_STATE_MACHINE")
