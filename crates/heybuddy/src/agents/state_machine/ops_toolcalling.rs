...
use crate::agents::state_machine::{
    ...
    HeyBuddyEffect, Operation, OperationResult, SlashCommand,
};
...
impl<'a> ToolExecutionOperation<'a> {
    ...
    async fn extension_state_effect(&self, session: &Session) -> Result<HeyBuddyEffect> {
        ...
        Ok(HeyBuddyEffect::SetExtensionData(extension_data))
    }
...
#[async_trait]
impl Operation<Session, HeyBuddyEffect> for ToolExecutionOperation<'_> {
    ...
    async fn run(
        ...
        if *self.heybuddy_mode.lock().await == HeyBuddyMode::Chat {
