//! HeyBuddy-specific inference request preparation.
...
use heybuddy_agent::inference::{InferenceRequestPreparer, PreparedInferenceRequest};
use heybuddy_agent::operation::{messages_since_kickoff, InferenceInput};
use heybuddy_providers::conversation::message::Message;
use heybuddy_providers::conversation::Conversation;
#[cfg(feature = "code-mode")]
use std::sync::Arc;
use tokio::sync::Mutex;

pub struct HeyBuddyInferenceRequestPreparer<'a> {
    #[cfg(feature = "code-mode")]
    pub(crate) extension_manager: Arc<ExtensionManager>,
    pub(crate) heybuddy_mode: &'a Mutex<HeyBuddyMode>,
...
#[async_trait]
impl InferenceRequestPreparer<Session> for HeyBuddyInferenceRequestPreparer<'_> {
    async fn prepare(
        &self,
        session: &Session,
        ...
        let heybuddy_mode = *self.heybuddy_mode.lock().await;
