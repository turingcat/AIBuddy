...
use crate::agents::state_machine::{
    applied, messages_since_kickoff, not_applicable, ConversationEffect, Emitter, HeyBuddyEffect,
...
pub const TOOL_EXECUTABLE_KEY: &str = "heybuddy.executable";

pub struct ToolApprovalOperation<'a> {
    heybuddy_mode: &'a Mutex<HeyBuddyMode>,
...
#[async_trait]
impl Operation<Session, HeyBuddyEffect> for ToolApprovalOperation<'_> {
    ...
    async fn run(
        ...
        if !pending_requests.is_empty() {
            ...
            for request in permission_check_result.needs_approval {
                ...
                {
                    tracing::info!(
                        monotonic_counter.heybuddy.prompt_injection_user_decisions = 1,
...
fn mark_executable(tool_call_id: &str, executable: bool) -> HeyBuddyEffect {
