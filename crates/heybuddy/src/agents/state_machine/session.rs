...
use heybuddy_agent::machine::{EffectHandler, EffectUsage, MachineSession, SessionLoader};
use heybuddy_agent::operation::{ConversationEffect, Emitter, MachineEffect};
...
#[async_trait]
impl EffectHandler<Session, HeyBuddyEffect> for SessionManager {
    async fn apply_effects(
        &self,
        session: &Session,
        effects: &mut [HeyBuddyEffect],
        emit: &Emitter,
    ) -> Result<()> {
        ...
        for effect in effects.iter_mut() {
            match effect {
                HeyBuddyEffect::Conversation(ConversationEffect::AppendMessage(message)) => {
                    self.add_message(&session.id, message).await?;
                }
                HeyBuddyEffect::Conversation(ConversationEffect::ReplaceConversation(
                    conversation,
                )) => {
                ...
                HeyBuddyEffect::ReplaceConversation {
                    conversation,
                ...
                HeyBuddyEffect::Conversation(ConversationEffect::PatchToolRequestMeta {
                    tool_call_id,
                    patch,
                }) => {
                ...
                HeyBuddyEffect::Conversation(ConversationEffect::SetMessageVisibility {
                    message_id,
                    user_visible,
                    agent_visible,
                }) => {
                ...
                HeyBuddyEffect::SetRecipe(recipe) => {
                    self.update(&session.id)
                        .recipe(recipe.as_ref().clone())
                        .apply()
                        .await?;
                }
                HeyBuddyEffect::SetExtensionData(extension_data) => {
                    self.update(&session.id)
                        .extension_data(extension_data.clone())
                        .apply()
                        .await?;
                }
                HeyBuddyEffect::RecordUsage(provider_usage) => {
        ...
        for effect in effects {
            match effect {
                ...
                HeyBuddyEffect::RecordUsage(usage) => {
...
impl EffectUsage<HeyBuddyEffect> for SessionManager {
    fn usage(
        &self,
        effect: &HeyBuddyEffect,
    ) -> Option<heybuddy_providers::conversation::token_usage::Usage> {
        match effect {
            HeyBuddyEffect::RecordUsage(usage)
            | HeyBuddyEffect::ReplaceConversation {
                usage: Some(usage), ..
...
pub(crate) async fn run(
    machine: &crate::agents::state_machine::StateMachine<'_, Session, HeyBuddyEffect>,
    ...
    let mut turn_usage = heybuddy_providers::conversation::token_usage::Usage::default();
    loop {
        let session = runtime.load(session_id).await?;
        let Some(mut result) = machine.step(&session, emit).await? else {
            break;
        };
        tracing::debug!(target: "heybuddy::state_machine", step = result.applied_step, "applied step");
