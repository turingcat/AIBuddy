//! HeyBuddy integration for the reusable inference operation.

use std::sync::Arc;

use async_trait::async_trait;
use futures::StreamExt;
use heybuddy_agent::inference::InferenceEffect;
pub use heybuddy_agent::inference::InferenceRunner;
use heybuddy_providers::base::{MessageStream, ModelInfo, Provider};
...
use crate::agents::state_machine::HeyBuddyEffect;

pub(super) use heybuddy_agent::inference::{chat_span, record_chat_usage};

pub(super) const ADVERTISED_TOOLS_NOTE: &str = "advertised_tools";
pub(super) const LLM_OPERATION_NAME: &str = "llm";

pub struct HeyBuddyInferenceProvider {
    inner: Arc<dyn Provider>,
}

impl HeyBuddyInferenceProvider {
    pub fn new(inner: Arc<dyn Provider>) -> Self {
        Self { inner }
    }
}

impl InferenceEffect for HeyBuddyEffect {
...
fn enrich_unclaimed_tool_errors(messages: &[Message], tools: &[rmcp::model::Tool]) -> Vec<Message> {
    ...
    for message in &mut messages {
        for content in &mut message.content {
            let heybuddy_providers::conversation::message::MessageContent::ToolResponse(response) =
                content
            else {
                continue;
            };
...
fn canonicalize_tool_request_names(
    ...
    for content in &mut message.content {
        let heybuddy_providers::conversation::message::MessageContent::ToolRequest(request) =
            content
        else {
            continue;
        };
...
#[async_trait]
impl Provider for HeyBuddyInferenceProvider {
