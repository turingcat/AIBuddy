...
use crate::agents::state_machine::{
    ...
    ConversationEffect, Emitter, HeyBuddyEffect, Operation, OperationResult, SlashCommand,
};
...
#[async_trait]
impl Operation<Session, HeyBuddyEffect> for RecipeOperation {
    ...
    async fn run_command(
        ...
        applied([
            ...
            .into(),
            HeyBuddyEffect::SetRecipe(Box::new(Some(recipe))),
            Message::user()
                .with_text(prompt)
    ...
    async fn run(
        ...
        if let Some((request, _)) = pending {
            if session.heybuddy_mode == HeyBuddyMode::Chat {
