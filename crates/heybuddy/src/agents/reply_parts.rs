...
use heybuddy_providers::conversation::token_usage::{
...
impl Agent {
    pub async fn prepare_tools_and_prompt(
        ...
        if heybuddy_mode == HeyBuddyMode::SmartApprove {
        ...
        if !provider.manages_own_context() {
            ...
            loop {
                match stream.next().await {
                    ...
                    Some(Err(error))
                        if heybuddy_providers::retry::should_retry(&error, &retry_config)
                            && attempts < retry_config.max_retries =>
                    {
                        ...
                        let skip_backoff = std::env::var("HEYBUDDY_PROVIDER_SKIP_BACKOFF")
                            .unwrap_or_default()
                            .parse::<bool>()
                            .unwrap_or(false);
                        if !skip_backoff {
                    ...
                    if let Ok(ref mut tool_call) = coerced_req.tool_call {
                            ...
                            .find(|t| t.name == tool_call.name)
                        {
                            ...
                            if let Some(ref meta) = tool.meta {
                                ...
                                // heybuddy.external_dispatch) survive coercion.
...
mod tests {
    ...
    use heybuddy_providers::conversation::token_usage::{ProviderStats, ProviderUsage, Usage};
    use heybuddy_providers::model::ModelConfig;
    ...
    fn categorize_tool_requests_skips_externally_dispatched_and_preserves_marker() {
        // External requests must (1) survive coercion with heybuddy.external_dispatch
        ...
        assert!(
            ...
            "heybuddy.external_dispatch marker was clobbered by coercion; merged tool_meta = {:?}",
            tool_req.tool_meta
        );
        let merged = tool_req
            .tool_meta
    ...
    fn categorize_tool_requests_canonicalizes_mangled_unprefixed_tool_name() {
        ...
        .with_meta(rmcp::model::MetaObject(
            serde_json::json!({ "heybuddy_extension": "developer" })
                .as_object()
                .unwrap()
                .clone(),
        ));
    ...
    impl Provider for SequencedProvider {
        ...
        fn retry_config(&self) -> heybuddy_providers::retry::RetryConfig {
            heybuddy_providers::retry::RetryConfig::new(2, 0, 1.0, 0)
        }
