...
use super::mcp_client::HeyBuddyMcpHostInfo;
...
use crate::agents::state_machine::{
    ...
    persist_tool_confirmation_decision, run_heybuddy, BangShellOperation, CompactionOperation,
    DoctorOperation, Emitter, EntryHookOperation, ExitOnErrorOperation, HeyBuddyEffect,
    HeyBuddyInferenceProvider, HeyBuddyInferenceRequestPreparer, InferenceRunner,
...
use crate::config::{get_enabled_extensions, Config, HeyBuddyMode};
...
use heybuddy_providers::thinking::{ThinkingEffort, ThinkingEffortSupport};
use regex::Regex;
use rmcp::model::{
...
const COMPACTION_PROGRESS_TEXT: &str = "heybuddy is compacting the conversation...";
...
fn normalize_legacy_provider_thinking_effort(
    mut model_config: heybuddy_providers::model::ModelConfig,
    effort_support: &ThinkingEffortSupport,
) -> heybuddy_providers::model::ModelConfig {
    ...
    model_config.with_default_thinking_effort(Config::global().get_heybuddy_thinking_effort())
}
...
pub(crate) fn stop_hook_block_cap_warning(plugin: &str, cap: u32) -> Message {
    Message::assistant().with_system_notification(
        SystemNotificationType::InlineMessage,
        format!(
            "Stop hook `{plugin}` blocked the turn from ending more than {cap} consecutive times — overriding and ending turn to avoid an infinite loop. Set HEYBUDDY_STOP_HOOK_BLOCK_CAP to raise this limit."
        ),
    )
}
...
pub struct ReplyContext {
    ...
    pub heybuddy_mode: HeyBuddyMode,
    pub tool_call_cut_off: usize,
    pub model_config: heybuddy_providers::model::ModelConfig,
}
...
#[derive(Clone, Debug)]
pub enum HeyBuddyPlatform {
    HeyBuddyDesktop,
    HeyBuddyCli,
}

impl fmt::Display for HeyBuddyPlatform {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            HeyBuddyPlatform::HeyBuddyCli => write!(f, "heybuddy-cli"),
            HeyBuddyPlatform::HeyBuddyDesktop => write!(f, "heybuddy-desktop"),
        }
    }
}
...
pub struct AgentConfig {
    ...
    pub heybuddy_platform: HeyBuddyPlatform,
    pub mcp_host_info: Option<HeyBuddyMcpHostInfo>,
...
impl AgentConfig {
    pub fn new(
        ...
        heybuddy_platform: HeyBuddyPlatform,
    ) -> Self {
        Self {
            session_manager,
            permission_manager,
    ...
    pub fn with_mcp_host_info(mut self, mcp_host_info: Option<HeyBuddyMcpHostInfo>) -> Self {
    ...
    fn resolve_use_login_shell_path(&self) -> bool {
        resolve_use_login_shell_path(self.use_login_shell_path, &self.heybuddy_platform)
    }
}

fn resolve_use_login_shell_path(explicit: Option<bool>, platform: &HeyBuddyPlatform) -> bool {
    explicit.unwrap_or(matches!(platform, HeyBuddyPlatform::HeyBuddyDesktop))
}

/// The main heybuddy Agent
pub struct Agent {
    pub(super) provider: SharedProvider,
    pub config: AgentConfig,
    pub(super) current_heybuddy_mode: Mutex<HeyBuddyMode>,
...
impl Agent {
    pub fn new() -> Self {
        ...
        Self::with_config(AgentConfig::new(
            ...
            config
                .get_heybuddy_disable_session_naming()
                .unwrap_or(false),
    ...
    pub fn with_config(config: AgentConfig) -> Self {
        ...
        let heybuddy_platform = config.heybuddy_platform.clone();
        let initial_mode = config.heybuddy_mode;
        ...
        let mcpui = explicit_mcp_host_info
            ...
            .map(HeyBuddyMcpHostInfo::mcpui_enabled)
            .unwrap_or_else(|| match config.heybuddy_platform {
                HeyBuddyPlatform::HeyBuddyDesktop => true,
                HeyBuddyPlatform::HeyBuddyCli => false,
            });
        ...
        let client_name = explicit_mcp_host_info
            ...
            .unwrap_or_else(|| heybuddy_platform.to_string());
        ...
        Self {
            ...
            config,
            current_heybuddy_mode: Mutex::new(initial_mode),
    ...
    pub(crate) fn stop_hook_block_cap(&self) -> u32 {
        ...
        Config::global()
            .get_param::<u32>("HEYBUDDY_STOP_HOOK_BLOCK_CAP")
    ...
    fn create_tool_inspection_manager(
        ...
        // Add adversary inspector (LLM-based review, enabled by ~/.config/heybuddy/adversary.md)
    ...
    async fn prepare_reply_context(
        ...
        let heybuddy_mode = *self.current_heybuddy_mode.lock().await;

        let tool_call_cut_off = match Config::global()
            .get_param::<usize>("HEYBUDDY_TOOL_CALL_CUTOFF")
        {
            Ok(v) => v,
            Err(_) => {
                let context_limit = match self.provider().await {
                    ...
                    .await
                    .unwrap_or(heybuddy_providers::model::DEFAULT_CONTEXT_LIMIT),
                    Err(_) => heybuddy_providers::model::DEFAULT_CONTEXT_LIMIT,
                };
                let compaction_threshold = Config::global()
                    .get_param::<f64>("HEYBUDDY_AUTO_COMPACT_THRESHOLD")
    ...
    pub(super) fn create_state_machine(
    ...
    ) -> StateMachine<'_, Session, HeyBuddyEffect> {
        let max_turns = max_turns.unwrap_or_else(|| {
            Config::global()
                .get_param::<u32>("HEYBUDDY_MAX_TURNS")
                .unwrap_or(DEFAULT_MAX_TURNS)
        });
        let retry_timeout = Config::global()
            .get_param::<u64>("HEYBUDDY_RECIPE_RETRY_TIMEOUT_SECONDS")
            .unwrap_or(DEFAULT_RETRY_TIMEOUT_SECONDS);
        let on_failure_timeout = Config::global()
            .get_param::<u64>("HEYBUDDY_RECIPE_ON_FAILURE_TIMEOUT_SECONDS")
        ...
        let mut operations: Vec<Arc<dyn Operation<Session, HeyBuddyEffect> + '_>> = vec![
        ...
        let remaining_operations: Vec<Arc<dyn Operation<Session, HeyBuddyEffect> + '_>> = vec![
        ...
        let request_preparer = HeyBuddyInferenceRequestPreparer {
            #[cfg(feature = "code-mode")]
            extension_manager: self.extension_manager.clone(),
            heybuddy_mode: &self.current_heybuddy_mode,
        ...
        let inference_provider = Arc::new(HeyBuddyInferenceProvider::new(provider));
        ...
        let command_operation: Arc<dyn Operation<Session, HeyBuddyEffect> + '_> =
        ...
        let operations: Vec<_> =
            std::iter::once(Arc::new(EntryHookOperation::new(self.hook_manager.clone()))
                as Arc<dyn Operation<Session, HeyBuddyEffect> + '_>)
            ...
            async_stream::try_stream! {
                ...
                let result = {
                    let run = crate::session_context::with_session_id(
                        ...
                        run_heybuddy(&machine, session_manager.as_ref(), &session_id, &emit),
                    );
                    tokio::pin!(run);
                    loop {
                        tokio::select! {
                    ...
                    match next {
                        Ok((response, usage)) => {
                            ...
                            if let Some(response) = response {
                                ...
                                if heybuddy_mode == HeyBuddyMode::Chat {
                                    for request in &tool_requests {
    ...
    pub async fn update_heybuddy_mode(&self, mode: HeyBuddyMode, session_id: &str) -> Result<()> {
        if let Some(provider) = self.provider.lock().await.as_ref() {
            provider
        ...
        *self.current_heybuddy_mode.lock().await = mode;
        self.config
            .session_manager
            .clone()
            .update(session_id)
            ...
            .apply()
            .await
            .context("Failed to persist heybuddy_mode to session")
    }

    pub async fn heybuddy_mode(&self) -> HeyBuddyMode {
        *self.current_heybuddy_mode.lock().await
    }
    ...
    pub async fn recreate_provider_for_session(
        ...
        let mode = self.heybuddy_mode().await;
        self.update_heybuddy_mode(mode, session_id).await
    }
    ...
    pub async fn restore_provider_from_session(&self, session: &Session) -> Result<bool> {
        ...
        let provider_name = session
            .provider_name
            .clone()
            .or_else(|| config.get_heybuddy_provider().ok())
        ...
        let (provider, active_model_config, provider_changed) =
                ...
                .is_ok()
            {
                ...
                let fallback_model_name = config.get_heybuddy_model().ok().ok_or_else(|| {
        ...
        if let Some(provider) = self.provider.lock().await.as_ref() {
            provider
                .update_mode(&session.id, session.heybuddy_mode)
                .await
                .map_err(|e| anyhow!("Failed to propagate mode to provider: {}", e))?;
        }
        *self.current_heybuddy_mode.lock().await = session.heybuddy_mode;
        Ok(provider_changed)
    }
    ...
    pub async fn create_recipe(
        ...
        let system_prompt = prompt_manager
            ...
            .with_heybuddy_mode(heybuddy_mode)
            .build();
        ...
        let settings = Settings {
            heybuddy_provider: Some(provider_name.clone()),
            heybuddy_model: Some(model_name.clone()),
...
mod tests {
    ...
    async fn tool_dispatch_records_gen_ai_span_attributes() {
        use heybuddy_test_support::otel::clear_otel_env;
        use rmcp::object;
    ...
    fn resolve_use_login_shell_path_defaults_by_platform() {
        assert!(resolve_use_login_shell_path(
            None,
            &HeyBuddyPlatform::HeyBuddyDesktop
        ));
        assert!(!resolve_use_login_shell_path(
            None,
            &HeyBuddyPlatform::HeyBuddyCli
    ...
    impl crate::providers::base::Provider for ActionRequiredProvider {
        ...
        async fn stream(
            &self,
            _: &heybuddy_providers::model::ModelConfig,
            _: &str,
        ...
        fn thinking_effort_support(&self) -> ThinkingEffortSupport {
            if self.applies_effort {
                ThinkingEffortSupport::Options(
                    heybuddy_providers::thinking::ThinkingEffortCapability {
                        option_id: "effort".to_string(),
                        values: vec![heybuddy_providers::thinking::ThinkingEffortOption {
    ...
    async fn effort_test_agent(
        ...
        agent
            .update_provider(
                provider.clone(),
                heybuddy_providers::model::ModelConfig::new("mock-model"),
                &session.id,
            )
            .await
            .unwrap();
    ...
    async fn update_provider_replaces_harness_only_effort_for_legacy_provider() {
        ...
        let model_config =
            heybuddy_providers::model::ModelConfig::new("mock-model").with_merged_request_params(
...
"#,
                ...
                matches!(
                    ...
                    MessageContent::SystemNotification(notification)
                        if notification.msg.contains("more than 2 consecutive times")
                            && notification.msg.contains("HEYBUDDY_STOP_HOOK_BLOCK_CAP")
                )
            })
        }));
    ...
    async fn test_add_final_output_tool() -> Result<()> {
        ...
        let system_prompt = prompt_manager
            .builder()
            .with_heybuddy_mode(HeyBuddyMode::default())
            .build();
