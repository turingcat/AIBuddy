...
pub type BoxedClientInitializeError = Box<ClientInitializeError>;

#[derive(Debug)]
pub struct ProcessExit {
    stderr: String,
    source: BoxedClientInitializeError,
}
...
impl fmt::Display for ProcessExit {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            formatter,
            "process quit before initialization: stderr = {}",
            self.stderr
        )
    }
}

impl StdError for ProcessExit {
...
pub enum ExtensionError {
    ...
    InitializeError(BoxedClientInitializeError),
...
impl fmt::Display for ExtensionError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Client(error) => {
                write!(formatter, "failed a client call to an MCP server: {error}")
            }
            Self::ConfigError(error) => write!(formatter, "invalid config: {error}"),
            Self::SetupError(error) => write!(formatter, "error during extension setup: {error}"),
            Self::TaskJoinError(error) => {
                write!(
                    formatter,
                    "join error occurred during task execution: {error}"
                )
            }
            Self::IoError(error) => write!(formatter, "IO error: {error}"),
            Self::InitializeError(error) => {
                write!(formatter, "failed to initialize MCP client: {error}")
            }
            Self::ProcessExit(error) => error.fmt(formatter),
        }
    }
}
...
impl StdError for ExtensionError {
    fn source(&self) -> Option<&(dyn StdError + 'static)> {
        match self {
            ...
            Self::ConfigError(_) | Self::SetupError(_) => None,
            Self::TaskJoinError(error) => Some(error),
            Self::IoError(error) => Some(error),
            Self::InitializeError(error) => Some(error.as_ref()),
            Self::ProcessExit(error) => Some(error),
        }
    }
}

impl From<ClientError> for ExtensionError {
    fn from(error: ClientError) -> Self {
        Self::Client(error)
    }
}

impl From<tokio::task::JoinError> for ExtensionError {
...
impl From<BoxedClientInitializeError> for ExtensionError {
    fn from(error: BoxedClientInitializeError) -> Self {
        Self::InitializeError(error)
    }
}
...
pub enum ExtensionConfig {
    ...
    /// Built-in extension that is part of the bundled heybuddy MCP server
