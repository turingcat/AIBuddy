...
use super::extension::{
    BoxedClientInitializeError, ExtensionConfig, ExtensionError, ExtensionInfo, ExtensionResult,
    PlatformExtensionContext, PLATFORM_EXTENSIONS,
};
...
use crate::agents::mcp_client::{
    HeyBuddyMcpClientCapabilities, HeyBuddyMcpHostInfo, McpClient, McpClientTrait,
};
...
use crate::oauth::{
    oauth_flow, oauth_flow_with_challenge, HeyBuddyCredentialStore, StaticOAuthClientConfig,
};
...
fn resolve_timeout(timeout: Option<u64>) -> u64 {
    timeout.unwrap_or_else(|| {
        Config::global()
            .get_heybuddy_default_extension_timeout()
...
pub struct ExtensionManagerCapabilities {
    pub mcpui: bool,
    pub host_info: Option<HeyBuddyMcpHostInfo>,
...
pub struct HeyBuddyMcpAppToolAttachment {
...
pub(crate) const TRUSTED_TOOL_UPDATE_META_KEY: &str = "__heybuddy_tool_update_meta";

/// Manages heybuddy extensions / MCP clients and their interactions
...
const TOOL_EXTENSION_META_KEY: &str = "heybuddy_extension";
...
pub(crate) fn recover_mangled_tool_name<'a>(
    ...
    for (name, owner) in tools {
        // Prefixed tools: the model turns HeyBuddy's "__" separator into a dot
...
fn remove_untrusted_mcp_app_meta(result: &mut CallToolResult) {
    ...
    let remove_heybuddy = meta
        .0
        .get_mut("heybuddy")
        .and_then(Value::as_object_mut)
        .map(|heybuddy_meta| {
            heybuddy_meta.remove("mcpApp");
            heybuddy_meta.is_empty()
        })
        .unwrap_or(false);

    if remove_heybuddy {
        meta.0.remove("heybuddy");
    }
...
fn insert_trusted_tool_update_meta(
    ...
    attachment: &HeyBuddyMcpAppToolAttachment,
) {
...
async fn child_process_client(
    ...
    client_name: String,
    capabilities: HeyBuddyMcpClientCapabilities,
...
fn should_attempt_oauth_fallback(res: &Result<McpClient, BoxedClientInitializeError>) -> bool {
    res.as_ref()
        .err()
        .is_some_and(|error| is_oauth_auth_failure(error.as_ref()))
}
...
fn auth_challenge_from_result(
    res: &Result<McpClient, BoxedClientInitializeError>,
) -> Option<String> {
    res.as_ref()
        .err()
        .and_then(|error| auth_challenge_from_error(error.as_ref()))
}
...
const HEYBUDDY_USER_AGENT: reqwest::header::HeaderValue =
    reqwest::header::HeaderValue::from_static(concat!("heybuddy/", env!("CARGO_PKG_VERSION")));
...
async fn connect_with_auth(
    ...
    auth_headers.insert(reqwest::header::USER_AGENT, HEYBUDDY_USER_AGENT);
...
async fn create_streamable_http_client(
    ...
    default_headers.insert(reqwest::header::USER_AGENT, HEYBUDDY_USER_AGENT);
...
async fn create_unix_socket_http_client(
    ...
    custom_headers.insert(
        ...
        HEYBUDDY_USER_AGENT
            ...
            .unwrap_or("heybuddy")
            .parse()
            .unwrap_or_else(|_| HeaderValue::from_static("heybuddy")),
    );
...
impl ExtensionManager {
    fn mcp_client_capabilities(&self) -> HeyBuddyMcpClientCapabilities {
        HeyBuddyMcpClientCapabilities {
    ...
    pub async fn add_extension(
        ...
        let effective_working_dir = working_dir
            .clone()
            .or_else(|| {
                std::env::var("HEYBUDDY_WORKING_DIR")
                    .ok()
                    .map(PathBuf::from)
            })
            ...
            ExtensionConfig::StreamableHttp {
                ...
                create_streamable_http_client(
                    ...
                    *timeout,
                    ...
                    static_oauth_client,
                    Box::new(HeyBuddyCredentialStore::new(name.to_string())),
                    self.provider.clone(),
    ...
    async fn hydrate_mcp_app_attachment(
        ...
        cancellation_token: CancellationToken,
    ) -> Option<HeyBuddyMcpAppToolAttachment> {
        let resource_uri = resolved_tool.resource_uri.clone()?;

        let mut attachment = HeyBuddyMcpAppToolAttachment {
...
mod tests {
    ...
    fn test_insert_trusted_tool_update_meta_stores_backend_payload() {
        let mut result = CallToolResult::success(vec![]);
        let attachment = HeyBuddyMcpAppToolAttachment {
    ...
    async fn test_invalid_header_name_returns_config_error() {
        ...
        let capabilities = HeyBuddyMcpClientCapabilities {
            mcpui: false,
            host_info: None,
            elicitation_handler: None,
            protocol_version: None,
