...
use rmcp::{
    ...
    service::{
        ClientLifecycleMode, ClientServiceExt, PeerRequestOptions, RequestContext, RequestHandle,
        RunningService, ServiceRole,
    },
...
fn resolve_sampling_model_config() -> anyhow::Result<heybuddy_providers::model::ModelConfig> {
    let config = crate::config::Config::global();
    let provider_name = config.get_heybuddy_provider()?;
    let model_name = config.get_heybuddy_model()?;
...
pub struct HeyBuddyMcpHostInfo {
...
impl HeyBuddyClient {
    ...
    fn resolved_client_info(&self) -> Implementation {
        ...
        let version = self
            ...
            .unwrap_or_else(|| {
                std::env::var("HEYBUDDY_MCP_CLIENT_VERSION")
...
impl ClientHandler for HeyBuddyClient {
    #[expect(deprecated)]
    async fn list_roots(
        &self,
    ...
    async fn create_message(
        ...
        let system_prompt = params
            .system_prompt
            .as_deref()
            .unwrap_or("You are a general-purpose AI agent called heybuddy");
...
pub struct HeyBuddyMcpClientCapabilities {
    pub mcpui: bool,
...
impl std::fmt::Debug for HeyBuddyMcpClientCapabilities {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter
            .debug_struct("HeyBuddyMcpClientCapabilities")
...
pub struct McpClient {
    client: Mutex<Arc<RunningService<RoleClient, HeyBuddyClient>>>,
...
impl McpClient {
    ...
    pub(crate) async fn connect<T, E, A>(
        ...
        action_required: Arc<ActionRequiredManager>,
        extension_manager: Weak<ExtensionManager>,
    ) -> Result<Self, BoxedClientInitializeError>
    where
        ...
        E: std::error::Error + From<std::io::Error> + Send + Sync + 'static,
    {
        ...
        let client: rmcp::service::RunningService<rmcp::RoleClient, HeyBuddyClient> =
...
mod tests {
    ...
    use crate::agents::HeyBuddyPlatform;
    use rmcp::model::Tool;
    use serde_json::json;
    ...
    fn new_client(platform: HeyBuddyPlatform) -> HeyBuddyClient {
        let capabilities = match platform {
            HeyBuddyPlatform::HeyBuddyDesktop => HeyBuddyMcpClientCapabilities {
                mcpui: true,
                host_info: None,
                elicitation_handler: None,
                protocol_version: None,
            },
            HeyBuddyPlatform::HeyBuddyCli => HeyBuddyMcpClientCapabilities {
                mcpui: false,
                host_info: None,
                elicitation_handler: None,
                protocol_version: None,
    ...
    async fn tool_list_changed_during_fetch_prevents_stale_cache() {
        ...
        let heybuddy_client = HeyBuddyClient::new(
            Arc::new(Mutex::new(Vec::new())),
            Arc::new(Mutex::new(None)),
        ...
        heybuddy_client.handle_tool_list_changed().await;
    ...
    fn test_resolve_session_id(
        ...
        runtime.block_on(async {
            let client = new_client(HeyBuddyPlatform::HeyBuddyCli);
    ...
    fn test_request_injects_session(request_builder: fn(Extensions) -> ClientRequest) {
        ...
        extensions.insert(
            serde_json::from_value::<MetaObject>(json!({
                "HeyBuddy-Session-Id": "old-session-id",
    ...
    fn test_client_info_advertises_mcp_apps_ui_extension() {
        let client = new_client(HeyBuddyPlatform::HeyBuddyDesktop);
    ...
    fn test_explicit_host_info_passes_through_client_identity() {
        let client = HeyBuddyClient::new(
            Arc::new(Mutex::new(Vec::new())),
            Arc::new(Mutex::new(None)),
            HeyBuddyPlatform::HeyBuddyDesktop.to_string(),
            HeyBuddyMcpClientCapabilities {
                mcpui: true,
                host_info: Some(HeyBuddyMcpHostInfo {
                    explicit_extensions: true,
                    extensions: ExtensionCapabilities::new(),
                    client_name: Some("heybuddy2".to_string()),
        ...
        let info = ClientHandler::get_info(&client);
        assert_eq!(info.client_info.name, "heybuddy2");
    ...
    fn test_explicit_host_extensions_override_platform_fallback() {
        let client = HeyBuddyClient::new(
            Arc::new(Mutex::new(Vec::new())),
            Arc::new(Mutex::new(None)),
            HeyBuddyPlatform::HeyBuddyCli.to_string(),
            HeyBuddyMcpClientCapabilities {
                mcpui: false,
                host_info: Some(HeyBuddyMcpHostInfo {
                    explicit_extensions: true,
