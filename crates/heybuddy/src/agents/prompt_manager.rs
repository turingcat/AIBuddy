...
impl<'a> SystemPromptBuilder<'a, PromptManager> {
    ...
    pub fn with_heybuddy_mode(mut self, mode: HeyBuddyMode) -> Self {
    ...
    pub fn build(self) -> String {
        ...
        let heybuddy_mode = self
            .heybuddy_mode
            .unwrap_or_else(|| Config::global().get_heybuddy_mode().unwrap_or_default());

        let context = SystemPromptContext {
            extensions: sanitized_extensions_info,
            current_date_time: self.manager.current_date_timestamp.clone(),
            heybuddy_mode,
            is_autonomous: heybuddy_mode == HeyBuddyMode::Auto,
...
mod tests {
    ...
    fn shared_system_prompt_uses_aibuddy_chinese_identity() {
        ...
        assert!(!prompt.contains("called heybuddy"));
    }
    ...
    async fn test_all_platform_extensions() {
        ...
        let _guard = env_lock::lock_env([
            ...
            ("HEYBUDDY_PATH_ROOT", Some(temp_root.as_str())),
        ]);
