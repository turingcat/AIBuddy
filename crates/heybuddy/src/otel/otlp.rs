...
/// in an environment where heybuddy was built without the `grpc-tonic`
...
fn warn_grpc_protocol_skipped_once() {
    GRPC_PROTOCOL_WARNING_EMITTED.call_once(|| {
        eprintln!(
            "heybuddy otel: OTEL_EXPORTER_OTLP_PROTOCOL is set to a gRPC \
             variant, but this heybuddy build only includes the HTTP \
...
///
/// heybuddy's `opentelemetry-otlp` build only enables the `http-proto` /
...
fn signal_protocol_is_http(signal: &str) -> bool {
    ...
    match raw.trim().to_lowercase().as_str() {
        ...
        // gRPC variants require the `grpc-tonic` feature, which heybuddy
        // does not enable.
        _ => false,
    }
}
...
/// Promotes heybuddy config-file OTel settings to env vars before exporter build.
...
fn create_resource() -> Resource {
    ...
    let mut builder = Resource::builder_empty()
        .with_attributes([
            KeyValue::new("service.name", "heybuddy"),
            KeyValue::new("service.version", env!("CARGO_PKG_VERSION")),
            KeyValue::new("service.namespace", "heybuddy"),
...
fn create_otlp_tracing_layer() -> OtlpResult<OtlpTracingLayer> {
    ...
    let tracer_provider = match exporter {
        ExporterType::Otlp => {
            if !signal_protocol_is_http("traces") {
                ...
                return Err("OTLP traces protocol is grpc but heybuddy was built without grpc-tonic; skipping traces exporter".into());
            }
    ...
    let tracer = tracer_provider.tracer("heybuddy");
    ...
    let meter_provider = match exporter {
        ExporterType::Otlp => {
            if !signal_protocol_is_http("metrics") {
                warn_grpc_protocol_skipped_once();
                return Err("OTLP metrics protocol is grpc but heybuddy was built without grpc-tonic; skipping metrics exporter".into());
            }
            let rt = get_or_create_otel_rt()?;
    ...
    let logger_provider = match exporter {
        ExporterType::Otlp => {
            if !signal_protocol_is_http("logs") {
                warn_grpc_protocol_skipped_once();
                return Err("OTLP logs protocol is grpc but heybuddy was built without grpc-tonic; skipping logs exporter".into());
            }
            let rt = get_or_create_otel_rt()?;
...
fn create_otlp_tracing_filter() -> FilterFn<impl Fn(&Metadata<'_>) -> bool> {
    FilterFn::new(|metadata: &Metadata<'_>| {
        ...
        if metadata.level() == &Level::DEBUG {
            ...
            if target.starts_with("heybuddy::")
...
fn create_otlp_metrics_filter() -> FilterFn<impl Fn(&Metadata<'_>) -> bool> {
    FilterFn::new(|metadata: &Metadata<'_>| {
        ...
        if metadata.level() == &Level::DEBUG {
            let target = metadata.target();
            if target.starts_with("heybuddy::telemetry")
                || target.starts_with("heybuddy::metrics")
...
mod tests {
    ...
    fn otlp_trace_filter_suppresses_sensitive_targets() {
        let seen = count_otlp_trace_events(|| {
            ...
            tracing::info!(target: "heybuddy::test", "allowed ordinary event");
        });
    ...
    fn test_create_resource_defaults() {
        ...
        assert_eq!(get("service.name").as_deref(), Some("heybuddy"));
        assert_eq!(
            get("service.version").as_deref(),
            Some(env!("CARGO_PKG_VERSION"))
        );
        assert_eq!(get("service.namespace").as_deref(), Some("heybuddy"));
    ...
    #[test_case(
        ...
        Resource::builder_empty()
            .with_attributes([KeyValue::new("service.name", "heybuddy"), KeyValue::new("service.version", env!("CARGO_PKG_VERSION")), KeyValue::new("service.namespace", "heybuddy"), KeyValue::new("host.name", session_host()), KeyValue::new("user.name", session_user())])
            .with_detector(Box::new(TelemetryResourceDetector))
            .build();
        "no env vars uses heybuddy defaults"
    )]
    #[test_case(
        &[("OTEL_SERVICE_NAME", "custom")],
        Resource::builder_empty()
    ...
    fn otlp_logs_filter_honors_bare_rust_log_level() {
        let seen = count_otlp_log_events(&[("RUST_LOG", "warn")], || {
            tracing::error!(target: "heybuddy::test", "error event");
            tracing::warn!(target: "heybuddy::test", "warn event");
            tracing::info!(target: "heybuddy::test", "info event");
        });
    ...
    fn otlp_logs_filter_honors_scoped_and_mixed_rust_log_directives() {
        let seen = count_otlp_log_events(
            &[(
                "RUST_LOG",
                "heybuddy=error,heybuddy::agents::retry=debug,other=warn",
            )],
            || {
                tracing::debug!(target: "heybuddy::agents::retry", "retry detail");
                tracing::info!(target: "heybuddy::providers", "provider detail");
    ...
    fn otlp_logs_filter_falls_back_for_invalid_rust_log() {
        let seen = count_otlp_log_events(
            &[
                ("RUST_LOG", "heybuddy=definitely-not-a-level"),
    ...
    fn otlp_logs_filter_uses_otel_and_default_fallbacks() {
        ...
        let default_seen = count_otlp_log_events(&[("RUST_LOG", "")], || {
            tracing::info!(target: "heybuddy::test", "info event");
            tracing::debug!(target: "heybuddy::test", "debug event");
        });
    ...
    fn otlp_logs_filter_always_suppresses_sensitive_targets() {
        let seen =
            count_otlp_log_events(&[("RUST_LOG", "trace,rmcp::service::client=trace")], || {
                tracing::info!(target: "rmcp::service", "suppressed root");
                tracing::error!(target: "rmcp::service::client", "suppressed descendant");
                tracing::info!(target: "heybuddy::test", "allowed event");
            });
    ...
    #[test]
    fn otlp_logs_filter_excludes_sensitive_info_event_for_heybuddy_error() {
        let seen = count_otlp_log_events(&[("RUST_LOG", "heybuddy=error")], || {
            tracing::info!(
                target: "heybuddy::agents::retry",
