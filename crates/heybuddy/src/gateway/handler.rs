...
/// keeps a stricter default than the global `HEYBUDDY_MAX_TURNS` ceiling.  Users
/// can override this through `HEYBUDDY_GATEWAY_MAX_TURNS` (gateway-specific) or
/// `HEYBUDDY_MAX_TURNS` (applies globally).
const DEFAULT_GATEWAY_MAX_TURNS: u32 = 5;

/// Resolve the max turns to use for a gateway session.
///
/// Precedence: `HEYBUDDY_GATEWAY_MAX_TURNS` -> `HEYBUDDY_MAX_TURNS` ->
...
impl GatewayHandler {
                ...
                if let Some(gateway_type) = self.try_consume_code(message.text.trim()).await? {
                    ...
                    self.gateway
                        .send_message(
                            &message.user,
                            OutgoingMessage::Text {
                                body: "Welcome! Enter your pairing code to connect to heybuddy."
                                    .into(),
                            },
                        )
                        .await?;
    ...
    async fn complete_pairing(&self, user: &PlatformUser) -> anyhow::Result<()> {
        ...
        let provider = config.get_heybuddy_provider().ok();
        if let Some(ref provider) = provider {
            update = update.provider_name(provider);
        }
        if let (Some(ref provider), Ok(model_name)) = (&provider, config.get_heybuddy_model()) {
        ...
        self.gateway
            .send_message(
                user,
                OutgoingMessage::Text {
                    body: "Paired! You can now chat with heybuddy.".into(),
                },
            )
            .await?;
    ...
    async fn sync_session_config(&self, session: &Session) -> anyhow::Result<bool> {
        ...
        let current_provider = config.get_heybuddy_provider().ok();
        let current_model_name = config.get_heybuddy_model().ok();
        ...
        let current_mode = config.get_heybuddy_mode().unwrap_or_default();
        ...
        let mode_changed = current_mode != session.heybuddy_mode;
        ...
        if mode_changed {
            update = update.heybuddy_mode(current_mode);
        }
    ...
    async fn relay_to_session(
        ...
        // whatever it has.
        //
        // Honors `HEYBUDDY_GATEWAY_MAX_TURNS` (gateway-specific override) and
        // `HEYBUDDY_MAX_TURNS` (global), falling back to a conservative default
        // so the limit is configurable without editing the source.
        let config = Config::global();
        let max_turns = resolve_gateway_max_turns(
            config.get_param::<u32>("HEYBUDDY_GATEWAY_MAX_TURNS").ok(),
            config.get_param::<u32>("HEYBUDDY_MAX_TURNS").ok(),
        );
