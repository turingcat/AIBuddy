...
fn cleanup_orphaned_voice_files(
    ...
    for entry in entries {
        ...
        if !is_heybuddy_voice_file_name(&entry.file_name()) {
            continue;
        }
        let path = entry.path();
...
fn is_heybuddy_voice_file_name(name: &std::ffi::OsStr) -> bool {
    let Some(name) = name.to_str() else {
        return false;
    };
    let Some(rest) = name.strip_prefix("heybuddy_voice_") else {
        return false;
    };
...
fn cleanup_legacy_voice_files(
    ...
    let root_path = parent.join("heybuddy_voice");
...
impl TelegramGateway {
    ...
    /// HeyBuddy can access them via its shell tools. The extension is derived from
    ...
    /// Build the text prompt that tells HeyBuddy about a voice message file.
    fn voice_prompt(
        path: &std::path::Path,
        duration: Option<i32>,
                    ...
                    match result {
                        Ok(updates) => {
                            for update in updates {
                                ...
                                // HeyBuddy to transcribe the file using CLI tools.
...
mod tests {
    ...
    fn voice_prompt_includes_path_and_duration() {
        let path = std::path::PathBuf::from("/tmp/heybuddy_voice/voice_test.ogg");
        let prompt = TelegramGateway::voice_prompt(&path, Some(10), Some("audio/ogg"));
        assert!(prompt.contains("/tmp/heybuddy_voice/voice_test.ogg"));
    ...
    fn voice_prompt_with_mp3_mime() {
        let path = std::path::PathBuf::from("/tmp/heybuddy_voice/voice_test.mp3");
    ...
    fn save_voice_file_contains_untrusted_mime_before_pairing() {
        ...
        assert!(filename.starts_with("heybuddy_voice_"));
    ...
    fn cleanup_reclaims_only_stale_legacy_voice_files() {
        let temp = tempfile::tempdir().unwrap();
        let legacy_root = temp.path().join("heybuddy_voice");
    ...
    fn cleanup_legacy_root_replacement_stays_anchored() {
        ...
        let temp = tempfile::tempdir().unwrap();
        let legacy_root = temp.path().join("heybuddy_voice");
        let moved_root = temp.path().join("moved-heybuddy-voice");
    ...
    fn cleanup_preserves_symlinks_and_unrelated_files() {
        ...
        let disguised_symlink = temp.path().join("heybuddy_voice_ABC123.ogg");
        std::fs::write(&unrelated, b"unrelated").unwrap();
    ...
    fn precreated_legacy_root_cannot_redirect_voice_save_or_cleanup() {
        ...
        std::os::unix::fs::symlink(&victim_dir, fake_tmp.join("heybuddy_voice")).unwrap();
    ...
    fn replaced_legacy_root_cannot_redirect_voice_save_or_cleanup() {
        ...
        std::fs::write(&victim_file, b"keep me").unwrap();
        let replaced_root = fake_tmp.join("heybuddy_voice");
