pub use heybuddy_context_management::structured;
...
pub use heybuddy_context_management::DEFAULT_COMPACTION_THRESHOLD;

pub(crate) const TOOLCALL_SUMMARIZATION_BATCH_SIZE: usize = 10;

pub(crate) fn tool_pair_summarization_enabled() -> bool {
    Config::global()
        .get_param::<bool>("HEYBUDDY_TOOL_PAIR_SUMMARIZATION")
        .unwrap_or(true)
}
...
struct HeyBuddyCompactionModel<'a> {
...
impl heybuddy_context_management::CompactionModel for HeyBuddyCompactionModel<'_> {
    async fn complete(
        &self,
        system: &str,
        messages: &[Message],
...
struct HeyBuddyTokenEstimator;

#[async_trait::async_trait]
impl heybuddy_context_management::TokenEstimator for HeyBuddyTokenEstimator {
...
fn compaction_templates() -> Result<heybuddy_context_management::Templates> {
    Ok(heybuddy_context_management::Templates {
...
async fn do_compact(
    ...
    let model = HeyBuddyCompactionModel {
        provider,
        model_config,
        session_id,
    };
    let summary = heybuddy_context_management::summarize(
        &model,
        Some(&HeyBuddyTokenEstimator),
...
pub use heybuddy_context_management::format_message_for_compacting;
    ...
    async fn carried_turn_context_stays_last_after_persist_and_reload() {
        ...
        let session = manager
            .create_session(
                ...
                crate::config::HeyBuddyMode::default(),
            )
            .await
            .unwrap();
        manager
