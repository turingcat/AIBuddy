...
use super::app::HeyBuddyApp;

static CLOCK_HTML: &str = include_str!("../heybuddy_apps/clock.html");
...
pub fn mark_deletable_apps(apps: &mut [HeyBuddyApp]) {
...
impl McpAppCache {
    ...
    fn bundled_default_app(resource_uri: &str) -> Option<HeyBuddyApp> {
        let (_, html) = DEFAULT_APPS.iter().find(|(uri, _)| *uri == resource_uri)?;
        let mut app = HeyBuddyApp::from_html(html).ok()?;
        app.mcp_servers = vec![APPS_EXTENSION_NAME.to_string()];
        Some(app)
    }

    pub fn restore_bundled_default_apps(apps: &mut [HeyBuddyApp]) {
        for app in apps {
    ...
    pub fn list_apps(&self) -> Result<Vec<HeyBuddyApp>, std::io::Error> {
            ...
            for entry in fs::read_dir(&self.cache_dir)? {
                ...
                if path.extension().and_then(|s| s.to_str()) == Some("json") {
                    match fs::read_to_string(&path) {
                        Ok(content) => match serde_json::from_str::<HeyBuddyApp>(&content) {
    ...
    pub fn store_app(&self, app: &HeyBuddyApp) -> Result<(), std::io::Error> {
    ...
    pub fn get_app(&self, extension_name: &str, resource_uri: &str) -> Option<HeyBuddyApp> {
        ...
        fs::read_to_string(&app_path)
            .ok()
            .and_then(|content| serde_json::from_str::<HeyBuddyApp>(&content).ok())
    }
        ...
        for entry in fs::read_dir(&self.cache_dir)? {
            ...
            if path.extension().and_then(|s| s.to_str()) == Some("json") {
                if let Ok(content) = fs::read_to_string(&path) {
                    if let Ok(app) = serde_json::from_str::<HeyBuddyApp>(&content) {
...
<body></body>
    ...
    fn mark_deletable_apps_protects_bundled_uri_not_name() {
        let mut bundled_clock = HeyBuddyApp::from_html(CLOCK_HTML).unwrap();
        bundled_clock.mcp_servers = vec![APPS_EXTENSION_NAME.to_string()];

        let mut user_clock = HeyBuddyApp::from_html(CUSTOM_APP_HTML).unwrap();
        ...
        let mut custom = HeyBuddyApp::from_html(CUSTOM_APP_HTML).unwrap();
        custom.mcp_servers = vec![APPS_EXTENSION_NAME.to_string()];

        let mut external = HeyBuddyApp::from_html(CUSTOM_APP_HTML).unwrap();
    ...
    fn delete_app_removes_cached_entry() {
        with_temp_config(|| {
            ...
            let mut app = HeyBuddyApp::from_html(CUSTOM_APP_HTML).unwrap();
            app.mcp_servers = vec![APPS_EXTENSION_NAME.to_string()];
    ...
    fn bundled_default_identity_cannot_be_replaced_or_deleted() {
        with_temp_config(|| {
            let cache = McpAppCache::new().unwrap();
            let expected = HeyBuddyApp::from_html(CLOCK_HTML).unwrap();
            let mut attacker_app = HeyBuddyApp::from_html(CUSTOM_APP_HTML).unwrap();
