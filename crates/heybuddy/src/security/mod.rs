...
impl SecurityManager {
    ...
    pub async fn analyze_tool_requests(
        ...
        if !self.is_prompt_injection_detection_enabled() {
            tracing::debug!(
                monotonic_counter.heybuddy.prompt_injection_scanner_disabled = 1,
        ...
        let scanner = self.scanner.get_or_init(|| {
            ...
            tracing::info!(
                monotonic_counter.heybuddy.security_command_classifier_enabled = if command_classifier_enabled { 1 } else { 0 },
                monotonic_counter.heybuddy.security_prompt_classifier_enabled = if prompt_classifier_enabled { 1 } else { 0 },
            ...
            let scanner = if ml_enabled {
                match PromptInjectionScanner::with_ml_detection() {
                    Ok(s) => {
                        tracing::info!(
                            monotonic_counter.heybuddy.prompt_injection_scanner_enabled = 1,
        ...
        for tool_request in tool_requests.iter() {
            if let Ok(tool_call) = &tool_request.tool_call {
                ...
                if analysis_result.is_malicious {
                    ...
                    tracing::warn!(
                        monotonic_counter.heybuddy.prompt_injection_finding = 1,
                    ...
                    tracing::info!(
                        monotonic_counter.heybuddy.prompt_injection_tool_call_passed = 1,
                        security.event_type = "prompt_injection_scan",
