...
use heybuddy::acp::server::AcpProviderFactory;
use heybuddy::providers::base::{MessageStream, Provider};
use heybuddy_providers::errors::ProviderError;
use heybuddy_providers::model::ModelConfig;
use heybuddy_test_support::{EnforceSessionId, IgnoreSessionId, McpFixture, FAKE_CODE};
...
fn write_acp_global_config(contents: &str) -> PathBuf {
    std::env::set_var("HEYBUDDY_PATH_ROOT", ACP_CONFIG_ROOT.path());
    std::env::set_var("HEYBUDDY_DISABLE_KEYRING", "1");
    let config_dir = heybuddy::config::paths::Paths::config_dir();
    std::fs::create_dir_all(&config_dir).unwrap();
    let mut contents = contents.to_string();
    if !contents.contains("HEYBUDDY_DISABLE_KEYRING") {
        contents.push_str("HEYBUDDY_DISABLE_KEYRING: true\n");
    }
    std::fs::write(
        config_dir.join(heybuddy::config::base::CONFIG_YAML_NAME),
        contents,
    )
    .unwrap();
    config_dir
...
impl Provider for MockProvider {
    ...
    async fn stream(
        ...
        _system: &str,
        _messages: &[heybuddy::conversation::message::Message],
...
fn steer_chunk_message_ids(updates: &[SessionUpdate]) -> Vec<String> {
    updates
        ...
        .filter_map(|update| {
            ...
            let heybuddy = chunk.meta.as_ref()?.get("heybuddy")?;
            heybuddy.get("steer")?.as_bool().filter(|b| *b)?;
            heybuddy.get("messageId")?.as_str().map(ToString::to_string)
        })
        .collect()
}
...
fn test_custom_get_extensions() {
    ...
    run_test(async move {
        ...
        let add_result = send_custom(
            conn.cx(),
            "_heybuddy/unstable/config/extensions/add",
            serde_json::json!({
                "enabled": true,
                "extension": {
                    "type": "mcp",
        ...
        let stored_inline_token = heybuddy::config::Config::global()
...
fn test_steer_session_adds_input_to_active_prompt() {
    ...
    run_test(async move {
        ...
        assert!(
            ...
            "expected a chunk marked _meta.heybuddy.steer with the steer text, got: {steer_chunks:?}"
        );
...
fn test_custom_list_builtin_skill_sources() {
    ...
    run_test(async move {
        ...
        let builtin = sources
            .iter()
            .find(|source| source.get("name") == Some(&serde_json::json!("heybuddy-doc-guide")))
            .expect("expected heybuddy-doc-guide builtin skill");
        ...
        assert_eq!(
            builtin.get("path"),
            Some(&serde_json::json!("builtin://skills/heybuddy-doc-guide"))
        );
    });
}
...
fn test_custom_preferences_read_save() {
    let config_dir = write_acp_global_config(
        "HEYBUDDY_MODEL: gpt-4o\nHEYBUDDY_PROVIDER: openai\nHEYBUDDY_AUTO_COMPACT_THRESHOLD: 0.7\nHEYBUDDY_THINKING_EFFORT: high\nVOICE_AUTO_SUBMIT_PHRASES: send it\n",
    );
    ...
    run_test(async move {
        ...
        assert_eq!(
            ...
            Some(&serde_json::json!([
                ...
                { "key": "heybuddyThinkingEffort", "value": "high" },
        ...
        send_custom(
            conn.cx(),
            "_heybuddy/unstable/preferences/save",
            serde_json::json!({
                "values": [
                    { "key": "heybuddyThinkingEffort", "value": "disabled" },
        ...
        let response = send_custom(
            ...
            "_heybuddy/unstable/preferences/read",
            serde_json::json!({
                "keys": ["heybuddyThinkingEffort", "voiceDictationProvider", "voiceDictationPreferredMic"],
            }),
        )
        .await
        ...
        assert_eq!(
            response.get("values"),
            Some(&serde_json::json!([
                { "key": "heybuddyThinkingEffort", "value": "off" },
                { "key": "voiceDictationProvider", "value": "__disabled__" },
                { "key": "voiceDictationPreferredMic", "value": "mic-1" },
            ]))
        );
...
fn test_custom_preferences_save_rejects_invalid_values() {
    ...
    run_test(async {
        ...
        let invalid_payloads = [
            ...
            serde_json::json!({
                "values": [{ "key": "heybuddyThinkingEffort", "value": "bogus" }],
            }),
            serde_json::json!({
                "values": [{ "key": "heybuddyThinkingEffort", "value": ["high"] }],
            }),
            serde_json::json!({
        ...
        for payload in invalid_payloads {
            let result =
                send_custom(conn.cx(), "_heybuddy/unstable/preferences/save", payload).await;
...
fn test_custom_defaults_read() {
    let config_dir = write_acp_global_config(
        "HEYBUDDY_MODEL: claude-3-5-haiku-latest\nHEYBUDDY_PROVIDER: anthropic\n",
    );
...
fn test_raw_config_and_secret_methods_are_removed() {
    ...
    run_test(async {
        ...
        for method in [
            "_heybuddy/config/read",
            "_heybuddy/config/upsert",
            "_heybuddy/config/remove",
            "_heybuddy/secret/check",
            "_heybuddy/secret/upsert",
            "_heybuddy/secret/remove",
        ] {
...
fn test_provider_switching_updates_session_state() {
    ...
    run_test(async {
        ...
        conn.set_config_option(&session_id, "provider", "heybuddy")
            .await
            .expect("provider reset to heybuddy should succeed");
    });
}
                ...
                Box::pin(async move {
                    Ok(Arc::new(MockProvider {
                        ...
                        supported_models: Ok(vec![
                            "heybuddy-claude-opus-4-8".to_string(),
