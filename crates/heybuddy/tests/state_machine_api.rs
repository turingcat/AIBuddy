...
use heybuddy::agents::state_machine::{
    yielded_with, Emitter, HeyBuddyEffect, Inference, InferenceInput, Operation, OperationResult,
    StateMachine, Step,
};
...
use heybuddy::session::session_manager::token_state_from_session_and_totals;
use heybuddy::session::{Session, SessionManager, SessionType};
...
#[async_trait]
impl Operation<Session, HeyBuddyEffect> for PromptPart {
...
#[async_trait]
impl Operation<Session, HeyBuddyEffect> for TestInference {
    fn name(&self) -> &'static str {
        "test_inference"
    }
}
...
#[async_trait]
impl Inference<Session, HeyBuddyEffect> for TestInference {
    ...
    async fn infer(
        ...
        let message = emit
            ...
            .await;
        yielded_with([
            HeyBuddyEffect::from(message),
            HeyBuddyEffect::RecordUsage(ProviderUsage::new(
...
async fn custom_pipeline_supports_step_apply_run_and_usage() -> Result<()> {
    let _env = heybuddy_test_support::otel::clear_otel_env(&[(
    ...
    assert!(matches!(
        ...
        Some(HeyBuddyEffect::Conversation(
            heybuddy::agents::state_machine::ConversationEffect::AppendMessage(message)
