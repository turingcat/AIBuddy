...
use heybuddy::conversation::message::{Message, MessageContent, ToolRequest};
use heybuddy::security::adversary_inspector::AdversaryInspector;
use heybuddy::tool_inspection::ToolInspector;
...
async fn test_adversary_disabled_without_config_file() {
    ...
    let inspector = AdversaryInspector::with_config_dir(
        provider,
        Arc::new(heybuddy::session::SessionManager::new(
...
async fn test_adversary_enabled_default_tools() {
    ...
    assert!(matches!(
        results[0].action,
        heybuddy::tool_inspection::InspectionAction::Allow
    ));
