...
use heybuddy::agents::{Agent, AgentEvent, HeyBuddyPlatform};
use heybuddy::config::extensions::{set_extension, ExtensionEntry};
...
mod tests {
    ...
    mod schedule_tool_tests {
        ...
        use heybuddy::agents::platform_extensions::scheduler::{
            EXTENSION_NAME as SCHEDULER_EXTENSION_NAME, MANAGE_SCHEDULE_TOOL_NAME_COMPLETE,
        };
        use heybuddy::agents::ExtensionConfig;
        use heybuddy::agents::{AgentConfig, ScheduleTool};
        use heybuddy::config::permission::PermissionManager;
        use heybuddy::config::HeyBuddyMode;
        use heybuddy::scheduler::{ScheduledJob, SchedulerError, ValidatedScheduleRecipe};
        use heybuddy::scheduler_trait::SchedulerTrait;
        use heybuddy::session::{Session, SessionManager};
        ...
        async fn test_schedule_management_tool_list() {
            ...
            assert!(tool
                ...
                .clone()
                .unwrap_or_default()
                .contains("Manage heybuddy's internal scheduled recipe execution"));
        }
    ...
    mod retry_tests {
        use super::*;
        use heybuddy::agents::types::{RetryConfig, SuccessCheck};

        #[tokio::test]
        async fn test_retry_success_check_execution() -> Result<()> {
            use heybuddy::agents::retry::execute_success_checks;
    ...
    mod max_turns_tests {
        use super::*;
        use async_trait::async_trait;
        use heybuddy::agents::SessionConfig;
        use heybuddy::config::HeyBuddyMode;
        use heybuddy::conversation::message::{Message, MessageContent};
        use heybuddy::providers::base::{
        ...
        impl heybuddy::providers::base::ProviderDescriptor for MockToolProvider {
        ...
        impl ProviderDef for MockToolProvider {
            ...
            fn from_env(
                _extensions: Vec<heybuddy::config::ExtensionConfig>,
                _tls_config: Option<heybuddy::providers::api_client::TlsConfig>,
            ...
            while let Some(response_result) = reply_stream.next().await {
                match response_result {
                    Ok(AgentEvent::Message(response)) => {
                        ...
                        {
                            if let heybuddy::conversation::message::ActionRequiredData::ToolConfirmation { id, .. } = &action.data {
                                agent
                                    .submit_tool_confirmation(
                                        &session_id,
                                        id,
                                        heybuddy::permission::Permission::AllowOnce,
                                    )
                                    .await?;
                            }
                        }
    ...
    mod unparseable_tool_call_tests {
        use super::*;
        use async_trait::async_trait;
        use heybuddy::agents::{AgentConfig, SessionConfig};
        use heybuddy::config::permission::PermissionManager;
        use heybuddy::config::HeyBuddyMode;
        use heybuddy::conversation::message::{Message, MessageContent};
        use heybuddy::providers::base::{
        ...
        impl heybuddy::providers::base::ProviderDescriptor for UnparseableToolProvider {
            fn metadata() -> ProviderMetadata {
                ProviderMetadata {
    ...
    mod tool_pair_summarization_tests {
        ...
        use async_trait::async_trait;
        use heybuddy::agents::{AgentConfig, SessionConfig};
        use heybuddy::config::base::Config;
        use heybuddy::config::permission::PermissionManager;
        use heybuddy::config::HeyBuddyMode;
        ...
        use heybuddy::session::{SessionManager, SessionType};
        use heybuddy_providers::conversation::token_usage::{ProviderUsage, Usage};
        use heybuddy_providers::errors::ProviderError;
        use heybuddy_providers::model::ModelConfig;
        ...
        impl heybuddy::providers::base::ProviderDescriptor for SummarizationTestProvider {
            fn metadata() -> ProviderMetadata {
                ProviderMetadata {
        ...
        async fn test_batch_summarization_preserves_all_summaries() -> Result<()> {
            ...
            Config::global()
                .set_param("HEYBUDDY_TOOL_CALL_CUTOFF", 2)
                .unwrap();
            ...
            Config::global()
                .delete("HEYBUDDY_TOOL_CALL_CUTOFF")
                .unwrap();
    ...
    mod extension_manager_tests {
        use super::*;
        use heybuddy::agents::extension::ExtensionConfig;
        use heybuddy::agents::platform_extensions::{
            MANAGE_EXTENSIONS_TOOL_NAME, SEARCH_AVAILABLE_EXTENSIONS_TOOL_NAME,
        };
        use heybuddy::agents::AgentConfig;
        use heybuddy::config::permission::PermissionManager;
        use heybuddy::config::HeyBuddyMode;
        use heybuddy::session::SessionManager;
        ...
        async fn setup_agent_with_extension_manager() -> (Agent, String) {
            ...
            let todo_extension_entry = ExtensionEntry {
                ...
                config: ExtensionConfig::Platform {
                    name: "todo".to_string(),
                    description:
                        "Enable a todo list for heybuddy so it can keep track of what it is doing"
                            .to_string(),
    ...
    mod streaming_persistence_tests {
        ...
        use heybuddy::config::HeyBuddyMode;
        use heybuddy::conversation::message::Message;
        use heybuddy::providers::base::{MessageStream, Provider, ProviderDef, ProviderMetadata};
        use heybuddy::session::session_manager::SessionType;
        use heybuddy::session::SessionManager;
        use heybuddy_providers::conversation::token_usage::{ProviderUsage, Usage};
        use heybuddy_providers::errors::ProviderError;
        ...
        impl heybuddy::providers::base::ProviderDescriptor for MultiStepProvider {
            fn metadata() -> ProviderMetadata {
                ProviderMetadata {
    ...
    mod thinking_preservation_tests {
        ...
        impl heybuddy::providers::base::ProviderDescriptor for ThinkingStreamProvider {
            fn metadata() -> ProviderMetadata {
                ProviderMetadata {
        ...
        fn assert_formatter_adds_reasoning_to_tool_calls(messages: &[Message], provider: &str) {
            use heybuddy_providers::formats::openai::{
                format_messages_with_options, OpenAiFormatOptions,
            };
            use heybuddy_providers::images::ImageFormat;
        ...
        impl heybuddy::providers::base::ProviderDescriptor for CombinedThinkingToolProvider {
            fn metadata() -> ProviderMetadata {
                ProviderMetadata {
        ...
        impl heybuddy::providers::base::ProviderDescriptor for MultiToolThinkingProvider {
            fn metadata() -> ProviderMetadata {
                ProviderMetadata {
        ...
        async fn test_signed_thinking_not_duplicated_for_anthropic() -> Result<()> {
            use heybuddy_providers::formats::anthropic::format_messages as anthropic_format;
    ...
    mod goal_checking_tests {
        ...
        impl heybuddy::providers::base::ProviderDescriptor for GoalTextProvider {
            fn metadata() -> ProviderMetadata {
                ProviderMetadata {
    ...
    mod cumulative_token_tests {
        ...
        use heybuddy::config::HeyBuddyMode;
        use heybuddy::conversation::message::Message;
        use heybuddy::providers::base::{stream_from_single_message, MessageStream, Provider};
        use heybuddy::session::session_manager::SessionType;
        use heybuddy::session::SessionManager;
        use heybuddy_providers::conversation::token_usage::{ProviderUsage, Usage};
        use heybuddy_providers::errors::ProviderError;
    ...
    mod add_extensions_bulk_tests {
        ...
        use heybuddy::agents::{AgentConfig, ExtensionConfig};
        use heybuddy::config::permission::PermissionManager;
        use heybuddy::config::HeyBuddyMode;
        use heybuddy::session::session_manager::SessionType;
        use heybuddy::session::{
        ...
        fn unloadable_stdio_extension(name: &str) -> ExtensionConfig {
            ExtensionConfig::Stdio {
                ...
                cmd: "heybuddy-test-definitely-missing-binary".to_string(),
                args: vec![],
                envs: Envs::default(),
                env_keys: vec![],
                timeout: Some(1),
    ...
    mod audience_tool_result_tests {
        ...
        use async_trait::async_trait;
        use heybuddy::agents::{AgentConfig, SessionConfig};
        use heybuddy::config::{ExtensionConfig, HeyBuddyMode, PermissionManager};
        use heybuddy::conversation::message::{Message, MessageContent};
        use heybuddy::providers::base::{stream_from_single_message, MessageStream, Provider};
        use heybuddy::session::{SessionManager, SessionType};
        use heybuddy_providers::conversation::token_usage::{ProviderUsage, Usage};
        use heybuddy_providers::errors::ProviderError;
        use heybuddy_providers::model::ModelConfig;
        use heybuddy_test_support::McpFixture;
        use rmcp::model::{CallToolRequestParams, Tool};
        use std::path::PathBuf;
        use std::sync::atomic::{AtomicUsize, Ordering};
    ...
    mod empty_turn_tests {
        use super::*;
        use async_trait::async_trait;
        use heybuddy::agents::final_output_tool::FINAL_OUTPUT_TOOL_NAME;
        use heybuddy::agents::{AgentConfig, AgentEvent, HeyBuddyPlatform, SessionConfig};
        use heybuddy::config::permission::PermissionManager;
        use heybuddy::config::HeyBuddyMode;
        use heybuddy::conversation::message::{Message, MessageContent};
        use heybuddy::conversation::Conversation;
        use heybuddy::providers::base::{
            stream_from_single_message, MessageStream, Provider, ProviderDef, ProviderMetadata,
        };
        use heybuddy::session::session_manager::SessionType;
        ...
        impl heybuddy::providers::base::ProviderDescriptor for AssistantOnlyProvider {
            fn metadata() -> ProviderMetadata {
                ProviderMetadata {
        ...
        impl heybuddy::providers::base::ProviderDescriptor for EmptyThenTextProvider {
            fn metadata() -> ProviderMetadata {
                ProviderMetadata {
        ...
        async fn legacy_structured_output_fails_before_provider_inference() -> Result<()> {
            use heybuddy::recipe::Response;

            let _guard = env_lock::lock_env([("HEYBUDDY_STATE_MACHINE", None::<&str>)]);
            let agent = Agent::new();
            let session = agent
                .config
                .session_manager
        ...
        async fn test_empty_turn_with_final_output_tool_nudges() -> Result<()> {
            use heybuddy::agents::final_output_tool::FINAL_OUTPUT_CONTINUATION_MESSAGE;
            use heybuddy::recipe::Response;
