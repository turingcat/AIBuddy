...
use heybuddy::acp::ACP_CURRENT_MODEL;
use heybuddy::agents::{
    Agent, AgentConfig, AgentEvent, HeyBuddyPlatform, PromptManager, SessionConfig,
};
...
use heybuddy::providers::anthropic::ANTHROPIC_DEFAULT_MODEL;
use heybuddy::providers::azure::AZURE_DEFAULT_MODEL;
use heybuddy::providers::base::Provider;
#[cfg(feature = "aws-providers")]
use heybuddy::providers::bedrock::BEDROCK_DEFAULT_MODEL;
use heybuddy::providers::claude_code::CLAUDE_CODE_DEFAULT_MODEL;
use heybuddy::providers::codex::CODEX_DEFAULT_MODEL;
use heybuddy::providers::create_with_named_model;
use heybuddy::providers::google::GOOGLE_DEFAULT_MODEL;
use heybuddy::providers::litellm::LITELLM_DEFAULT_MODEL;
use heybuddy::providers::openai::OPEN_AI_DEFAULT_MODEL;
#[cfg(feature = "aws-providers")]
use heybuddy::providers::sagemaker_tgi::SAGEMAKER_TGI_DEFAULT_MODEL;
use heybuddy::providers::snowflake::SNOWFLAKE_DEFAULT_MODEL;
use heybuddy::providers::xai::XAI_DEFAULT_MODEL;
use heybuddy::session::{SessionManager, SessionType};
use heybuddy_providers::databricks::DATABRICKS_DEFAULT_MODEL;
use heybuddy_providers::errors::ProviderError;
use heybuddy_providers::thinking::ThinkingEffortSupport;
use heybuddy_test_support::{
...
impl ProviderFixture {
    async fn setup(config: &ProviderTestConfig, mode: HeyBuddyMode) -> Result<Self> {
        let mut env_vars: Vec<(&'static str, Option<&str>)> =
            vec![("HEYBUDDY_MODE", Some(<&str>::from(mode)))];
        ...
        let model_config = heybuddy::model_config::model_config_from_user_config(
            &config.name.to_lowercase(),
            config.model_name,
        )?;
    ...
    async fn tool_roundtrip(
        &self,
        prompt: &str,
        model_config: Option<heybuddy_providers::model::ModelConfig>,
    ) -> Result<Message> {
        let tools = self
            .agent
            .extension_manager
        ...
        let (response1, _) = heybuddy::session_context::with_session_id(
        ...
        let ctx = heybuddy::agents::ToolCallContext::new(
        ...
        let (response2, _) = heybuddy::session_context::with_session_id(
            Some(self.session_id.clone()),
            self.provider.complete(
                &model_config,
                &system,
    ...
    async fn test_basic_response(&self) -> Result<()> {
        ...
        let (response, _) = heybuddy::session_context::with_session_id(
            Some(self.session_id.clone()),
            self.provider.complete(
                &model_config,
    ...
    async fn test_context_length_exceeded_error(&self) -> Result<()> {
        ...
        let result = heybuddy::session_context::with_session_id(
            Some(self.session_id.clone()),
            self.provider.complete(
                &model_config,
                "You are a helpful assistant.",
    ...
    async fn test_image_content_support(&self) -> Result<()> {
        let image_config = self.image_model.as_ref().map(|model| {
            heybuddy_providers::model::ModelConfig::new(model).with_canonical_limits(&self.name)
        });
        let response = self
            .tool_roundtrip(
        ...
        assert!(
            text.contains("hello heybuddy") || text.contains("test image"),
            "{text}"
        );
    ...
    async fn test_model_switch(&self) -> Result<()> {
        ...
        let alt_config =
            heybuddy_providers::model::ModelConfig::new(alt).with_canonical_limits(&self.name);
    ...
    async fn test_mode_update(&self) -> Result<()> {
        ...
        self.agent
            .update_heybuddy_mode(HeyBuddyMode::Approve, &self.session_id)
            .await?;
...
async fn test_provider(config: ProviderTestConfig) -> Result<()> {
    ...
    let run_test = |mode: HeyBuddyMode| ProviderFixture::setup(&config, mode);

    if run_test(HeyBuddyMode::Auto).await.is_err() {
    ...
    let result: Result<()> = async {
        ...
        if config.test_permissions {
            ...
            if config.test_smart_approve {
                run_test(HeyBuddyMode::SmartApprove)
                    .await?
