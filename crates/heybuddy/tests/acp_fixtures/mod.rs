...
use fs_err as fs;
use heybuddy::acp::server::{serve, AcpProviderFactory, HeyBuddyAcpAgent, HeyBuddyAcpAgentOptions};
pub use heybuddy::acp::{map_permission_response, PermissionDecision};
...
use heybuddy::config::{HeyBuddyMode, PermissionManager};
use heybuddy::providers::api_client::{ApiClient, AuthMethod as ApiAuthMethod};
use heybuddy::providers::base::Provider;
use heybuddy::providers::openai::OpenAiProvider;
use heybuddy::scheduler::{ScheduledJob, SchedulerError, ValidatedScheduleRecipe};
use heybuddy::scheduler_trait::SchedulerTrait;
use heybuddy::session::Session as HeyBuddySession;
use heybuddy::session_context::SESSION_ID_HEADER;
use heybuddy_test_support::{ExpectedSessionId, TEST_MODEL};
...
impl SchedulerTrait for FixtureScheduler {
    ...
    async fn sessions(
        ...
        sched_id: &str,
        _limit: usize,
    ) -> Result<Vec<(String, HeyBuddySession)>, SchedulerError> {
...
fn write_global_test_config(config_path: &Path, openai_base_url: &str) {
    ...
    let global_config_path = global_config_dir.join(heybuddy::config::base::CONFIG_YAML_NAME);
...
pub async fn spawn_acp_server_in_process(
    ...
    let config_path = data_root.join(heybuddy::config::base::CONFIG_YAML_NAME);
    if !config_path.exists() {
        fs::write(
            &config_path,
            format!(
                "HEYBUDDY_MODEL: {current_model}\nHEYBUDDY_PROVIDER: openai\nHEYBUDDY_MODE: {}\nHEYBUDDY_TOOL_PAIR_SUMMARIZATION: false\n",
                heybuddy_mode
            ),
        )
        .unwrap();
    ...
    let agent = HeyBuddyAcpAgent::new(HeyBuddyAcpAgentOptions {
        provider_factory,
        builtin_selection: heybuddy::acp::server::AcpBuiltinSelection {
    ...
    F: Future<Output = ()> + Send + 'static,
{
    ...
    if std::env::var_os("HEYBUDDY_PATH_ROOT").is_none() {
