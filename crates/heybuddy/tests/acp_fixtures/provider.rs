...
use heybuddy::acp::{AcpProvider, AcpProviderConfig};
...
use heybuddy::permission::permission_confirmation::PrincipalType;
use heybuddy::permission::{Permission, PermissionConfirmation};
use heybuddy::providers::base::Provider;
use heybuddy_providers::model::ModelConfig;
use heybuddy_test_support::{ExpectedSessionId, IgnoreSessionId, TEST_MODEL};
...
impl AcpProviderSession {
    ...
    async fn send_message(
        ...
        let mut stream = heybuddy::session_context::with_session_id(
...
impl Connection for AcpProviderConnection {
    ...
    async fn new(config: TestConnectionConfig, openai: OpenAiFixture) -> Self {
        ...
        let heybuddy_mode = config.heybuddy_mode;
        ...
        let provider_config = AcpProviderConfig {
            ...
            mode_mapping: HeyBuddyMode::VARIANTS
                .iter()
                .map(|v| {
                    let mode = HeyBuddyMode::from_str(v).unwrap();
    ...
    async fn new_session(&mut self) -> anyhow::Result<SessionData<AcpProviderSession>> {
        ...
        let heybuddy_id = format!("test-session-{}", self.session_counter);
        ...
        let session = AcpProviderSession {
            ...
            session_id: agent_client_protocol::schema::v1::SessionId::new(heybuddy_id),
