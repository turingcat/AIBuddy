...
use heybuddy::agents::extension_manager::{ExtensionManager, ExtensionManagerCapabilities};
use heybuddy::agents::{HeyBuddyPlatform, MCP_PROTOCOL_VERSION};
...
impl heybuddy::providers::base::ProviderDescriptor for MockProvider {
...
async fn test_replayed_session(
    ...
    const TEST_WORKING_DIR: &str = "/tmp/heybuddy_test";
    fs::create_dir_all(TEST_WORKING_DIR).ok();

    let _env = env_lock::lock_env([
        ("HEYBUDDY_MCP_CLIENT_VERSION", Some("0.0.0")),
        ("HEYBUDDY_PROVIDER", Some("openai")),
        ("HEYBUDDY_MODEL", Some("gpt-4o")),
        ("HEYBUDDY_WORKING_DIR", Some(TEST_WORKING_DIR)),
    ]);

    // Setup test file for developer extension tests
    let test_file_path = "/tmp/heybuddy_test/heybuddy.txt";
    fs::write(test_file_path, "# heybuddy\n").ok();
    ...
    let mode = if env::var("HEYBUDDY_RECORD_MCP").is_ok() {
        TestMode::Record
    } else {
    ...
    let session_manager = Arc::new(heybuddy::session::SessionManager::new(
