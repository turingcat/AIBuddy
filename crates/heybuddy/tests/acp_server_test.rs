...
use heybuddy::conversation::message::{Message, MessageMetadata};
use heybuddy::custom_requests::{
    GetSessionInfoRequest, GetSessionInfoResponse, UpdateSessionProjectRequest,
};
use heybuddy::recipe::{Recipe, Settings};
use heybuddy::recipe_deeplink;
...
fn include_last_message_snippet_meta(
    ...
    heybuddy.insert("includeLastMessageSnippet".to_string(), value);
...
fn test_update_session_project_rejects_unknown_persisted_session_type() {
    run_test(async {
        ...
        let db_path = data_root
            .path()
            .join(heybuddy::session::session_manager::SESSIONS_FOLDER)
            .join(heybuddy::session::session_manager::DB_NAME);
    ...
    run_test(async {
        ...
        let recipe = Recipe::builder()
            ...
            .settings(Settings {
                ...
                heybuddy_model: Some(recipe_model.to_string()),
                temperature: None,
                max_turns: None,
            })
            .build()
...
fn test_new_session_prefers_recipe_title_over_meta_session_title() {
    run_test(async {
        ...
        // heybuddy's own recipe-title naming path still applies.
        assert_eq!(
            session_title(&conn, &session_id).await,
