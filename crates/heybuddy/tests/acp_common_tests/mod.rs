...
use heybuddy::config::base::CONFIG_YAML_NAME;
use heybuddy::config::HeyBuddyMode;
use heybuddy::session::{EnabledExtensionsState, SessionManager};
use heybuddy_test_support::{McpFixture, FAKE_CODE, TEST_IMAGE_B64, TEST_MODEL};
...
pub async fn run_load_mode<C: Connection>() {
    ...
    assert_eq!(
        ...
        SessionModeId::new(<&str>::from(HeyBuddyMode::default()))
    );
    let session_id = session.session_id().0.to_string();
    conn.set_mode(&session_id, <&str>::from(HeyBuddyMode::Approve))
        .await
        .unwrap();
    ...
    assert_eq!(
        modes.unwrap().current_mode_id,
        SessionModeId::new(<&str>::from(HeyBuddyMode::Approve))
    );
...
pub async fn run_load_session_replays_image_attachment<C: Connection>() {
    ...
    let output = session
        ...
        .await
        .unwrap();
    assert!(output.text.contains("Hello HeyBuddy!"));
...
async fn run_mode_set_impl<C: Connection>(via: SetModeVia) {
    ...
    let approve = <&str>::from(HeyBuddyMode::Approve);
    match via {
...
pub async fn run_new_session_uses_current_config_mode<C: Connection>() {
    ...
    let config_path = temp_dir
        .path()
        .join(heybuddy::config::base::CONFIG_YAML_NAME);
    fs::write(
        &config_path,
        format!(
            "HEYBUDDY_MODEL: {TEST_MODEL}\nHEYBUDDY_PROVIDER: openai\nHEYBUDDY_MODE: approve\n"
        ),
    )
    .unwrap();
    ...
    let global_config_path =
        heybuddy::config::paths::Paths::config_dir().join(heybuddy::config::base::CONFIG_YAML_NAME);
    fs::write(
        &global_config_path,
        format!("HEYBUDDY_MODEL: {TEST_MODEL}\nHEYBUDDY_PROVIDER: openai\nHEYBUDDY_MODE: auto\n"),
    )
    .unwrap();
...
pub async fn run_prompt_basic<C: Connection>() {
    ...
    assert_eq!(output.text, "2");
    let updates = session.session_updates();
    let (standard_message_id, heybuddy_message_id) = updates
        .iter()
        .find_map(|update| {
            ...
            let heybuddy_message_id = chunk
                ...
                .as_str()?
                .to_string();
            Some((standard_message_id, heybuddy_message_id))
        })
        .expect("expected live agent message chunk with standard and heybuddy message IDs");
    assert!(!standard_message_id.is_empty());
    assert_eq!(standard_message_id, heybuddy_message_id);
    assert_notifications(
        &fixtures::to_notifications(&updates),
        &[Notification::AgentMessage],
    );
...
pub async fn run_prompt_image<C: Connection>() {
    ...
    let output = session
        ...
        .await
        .unwrap();
    assert_eq!(output.text, "Hello HeyBuddy!\nThis is a test image.");
    assert_notifications(
        &session.notifications(),
        &[
            Notification::ToolCall,
...
pub async fn run_prompt_model_mismatch<C: Connection>() {
    ...
    let BasicSession { .. } = new_basic_session::<C>(config).await;
}
