...
impl Paths {
    fn get_dir(dir_type: DirType) -> PathBuf {
        if let Some(base) = Self::path_root() {
            ...
            // user config/data directories (e.g. ~/Library/Application Support/Block/heybuddy/).
            // Changing this would orphan existing installations.
            let strategy = choose_app_strategy(AppStrategyArgs {
                top_level_domain: "Block".to_string(),
                author: "Block".to_string(),
                app_name: "heybuddy".to_string(),
            })
            .expect("heybuddy requires a home dir");
    ...
    fn path_root() -> Option<PathBuf> {
        Self::validated_path_root(std::env::var_os("HEYBUDDY_PATH_ROOT"))
    }
...
mod tests {
    ...
    fn path_root_requires_an_absolute_path() {
        ...
        let absolute = std::env::current_dir()
            .unwrap()
            .join("nonexistent-heybuddy-root");
        assert_eq!(
