...
pub use heybuddy_download_manager as download_manager;
...
use heybuddy_provider_types::base::{
    MessageStream, Provider, ProviderDescriptor, ProviderMetadata,
};
use heybuddy_provider_types::conversation::message::{
    Message, MessageContent, SystemNotificationType,
};
use heybuddy_provider_types::conversation::token_usage::{ProviderUsage, Usage};
use heybuddy_provider_types::errors::ProviderError;
use heybuddy_provider_types::images::ImageFormat;
use heybuddy_provider_types::model::ModelConfig;
use heybuddy_provider_types::request_log::{start_log, LoggerHandleExt, RequestLogHandle};
    ...
    if let Ok(registry) = get_registry().lock() {
        if let Some(entry) = registry.get_model(model_id) {
            ...
            let draft_model = settings
                ...
                .clone()
                .or_else(|| {
                    config_resolver::string_param("HEYBUDDY_LOCAL_DRAFT_MODEL")
                        .ok()
                        .flatten()
                })
...
fn build_openai_messages_json(
    ...
    use heybuddy_provider_types::formats::openai::format_messages;
...
impl Provider for LocalInferenceProvider {
    ...
    async fn get_context_limit(&self, model: &str, override_limit: Option<usize>) -> usize {
        heybuddy_provider_types::context_limit::ContextLimitResolver::new(&self.name)
    ...
    async fn stream(
        ...
        if let Some(false) = model_config
            ...
            .or_else(|| {
                config_resolver::bool_param("HEYBUDDY_LOCAL_ENABLE_THINKING")
                    .ok()
                    .flatten()
            })
        {
