# heybuddy-local-inference

On-device model inference for heybuddy. Runs GGUF models through `llama.cpp` (via
`llama-cpp-2`), with an optional MLX backend on Apple silicon.

Reach it through [`heybuddy-providers`](../heybuddy-providers) with the
...
- **Model lifecycle** — `is_model_loaded`, `loaded_model_ids`, and `evict_model`
  ...
  [`heybuddy-download-manager`](../heybuddy-download-manager), re-exported here as
  `download_manager`.
...
```bash
cargo build -p heybuddy-local-inference
cargo build -p heybuddy-local-inference --features mlx
```
