...
pub struct AuthConfig {
    ...
    /// path is resolved against. Defaults to heybuddy's current directory.
    #[serde(default)]
...
pub struct DeclarativeProviderConfig {
    ...
    #[serde(default)]
    pub setup: Option<heybuddy_provider_types::canonical::catalog::ProviderSetupMetadata>,
}
...
mod tests {
    ...
    #[test]
    fn heybuddy_provider_is_not_bundled() {
        let providers = fixed_provider_config_entries();
        assert!(providers.iter().all(|(name, _)| *name != "heybuddy.json"));
    }
