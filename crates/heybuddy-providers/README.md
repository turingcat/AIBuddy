...
Provider implementations for heybuddy. The trait they implement and the conversation
types they exchange live in [`heybuddy-provider-types`](../heybuddy-provider-types),
...
```bash
cargo run -p heybuddy-providers --example declarative
cargo run -p heybuddy-providers --example streaming
```
...
- `local-inference` — pulls in [`heybuddy-local-inference`](../heybuddy-local-inference);
