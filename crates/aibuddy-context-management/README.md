...
```rust
use aibuddy_context_management::{summarize, Templates};
...
- `CompactionModel` — the model abstraction compaction runs against, with
  `ProviderModel` adapting any [`aibuddy-providers`](../aibuddy-providers) provider.
...
Python and Kotlin reach compaction through [`aibuddy-sdk`](../aibuddy-sdk), which
