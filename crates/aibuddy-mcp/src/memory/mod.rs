...
impl MemoryServer {
    pub fn new() -> Self {
        let instructions = formatdoc! {r#"
             ...
             Storage:
             - Local: .aibuddy/memory/ (project-specific)
             - Global: ~/.config/aibuddy/memory/ (user-wide)
        ...
        let global_memory_dir = choose_app_strategy(crate::APP_STRATEGY.clone())
            ...
            .unwrap_or_else(|_| PathBuf::from(".config/aibuddy/memory"));
    ...
    fn get_memory_file(
        ...
        let base_dir = if is_global {
            ...
            local_base.join(".aibuddy").join("memory")
        };
...
mod tests {
    ...
    fn test_lazy_directory_creation() {
        ...
        let local_memory_dir = working_dir.join(".aibuddy").join("memory");
    ...
    fn test_memory_operations_reject_escape_capable_categories() {
        ...
        assert!(!working_dir.join(".aibuddy").exists());
    }
    ...
    fn test_memory_category_allows_safe_filename_characters() {
        ...
        assert!(working_dir
            .join(".aibuddy/memory/project notes_2026.txt")
            .is_file());
    }
    ...
    fn test_retrieve_all_skips_invalid_legacy_categories() {
        let temp_dir = tempdir().unwrap();
        let working_dir = temp_dir.path().join("working");
        let memory_dir = working_dir.join(".aibuddy/memory");
