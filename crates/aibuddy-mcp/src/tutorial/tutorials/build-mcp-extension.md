...
Instruct users to start a aibuddy session with their extension.

**Important**: You cannot start the aibuddy session for them, as it is interactive. You will have to let them
know to start it in a terminal. Make sure you include instructions on how to set up the environment

```bash
# Python example
aibuddy session --with-extension "python server.py"

# TypeScript example
aibuddy session --with-extension "node server.js"

# Kotlin example
aibuddy session --with-extension "java -jar build/libs/extension.jar"
```
...
Note:
You can run a feedback loop using a headless aibuddy session, however if the process hangs you get into a stuck action.
Ask the user if they want you to do that, and let them know they will manually need to kill any stuck processes.

```bash
# Python example
aibuddy run --with-extension "python server.py" --text "EXAMPLE PROMPT HERE"

# TypeScript example
aibuddy run --with-extension "node server.js" --text "EXAMPLE PROMPT HERE"

# Kotlin example
aibuddy run --with-extension "java -jar build/libs/extension.jar" --text "EXAMPLE PROMPT HERE"
```
...
- For tools, they should ask aibuddy to use the tool directly
- For resources, they should ask aibuddy to access the relevant data
...
1. First, check if there are any immediate error messages in the aibuddy session
