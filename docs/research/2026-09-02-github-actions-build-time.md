...
The three runs below are successful `workflow_dispatch` executions of `Bundle CLI and Desktop (Windows)` on `main`. Total wall time is the run's GitHub `created_at` to `updated_at` duration. Rust compile time is the `Build Windows executable` step in the `Build AIBuddy (Windows)` job. This is the comparison scope for the three-run baseline; it does not compare overall CI or release workflows.
