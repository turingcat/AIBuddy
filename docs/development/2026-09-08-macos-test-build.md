...
Built from the current uncommitted `feat/heybuddy-branding` worktree for user testing.
Main remains unchanged. This is a local test build, not a published or notarized release.

- Product/version: HeyBuddy 1.0.6.
...
- App: `ui/desktop/out/HeyBuddy-darwin-arm64/HeyBuddy.app`.
- ZIP: `ui/desktop/out/HeyBuddy-macOS-arm64-rebrand-test-20260908-0930.zip` (approximately 202 MiB).
...
- Playwright confirmed the HeyBuddy login heading and account/password controls;
  no old-brand text appeared on that screen. No login was submitted.
- Packaged `heybuddy migrate-config --help` ran successfully.
- ZIP integrity check passed.

Visual evidence: `/private/tmp/heybuddy-user-test-dqzlp7/login.png`.
Startup report: `/private/tmp/heybuddy-user-test-dqzlp7/report.json`.
