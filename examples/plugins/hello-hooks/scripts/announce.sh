#!/usr/bin/env bash
# Tiny hook handler that loudly announces every event heybuddy fires at it.
#
# HeyBuddy pipes the event payload as JSON on stdin and sets PLUGIN_ROOT in the
...
# Both stderr (so it shows up alongside heybuddy tracing) and a log file.
