# AIBuddy Buzz automation

These tools connect issues in `aaif-goose/goose` to the public AIBuddy Buzz
...
- AIBuddy CLI with a configured model provider
...
```text
$AIBUDDY_BUZZ_HOME/github-manager
```

`AIBUDDY_BUZZ_HOME` defaults to `$XDG_CONFIG_HOME/aibuddy/buzz`, or
`~/.config/aibuddy/buzz` when `XDG_CONFIG_HOME` is not set. The identity directory
...
```sh
export BUZZ_PRIVATE_KEY="$(cat ~/.config/aibuddy/buzz/github-manager/private-key.nsec)"
...
- Mic as a member
- lifei as a member
    - Lifei aibuddy agent as their bot
...
queue entry can contain a AIBuddy issue or pull request URL, or just `#<issue
...
keys, interests, and assignment capacity so a AIBuddy recipe can select an owner.
...
The project defaults to `aaif-aibuddy` project 1. Use `--repo`,
...
This AIBuddy recipe manages the full Inbox loop:
...
The dedicated machine must stay awake and have working AIBuddy, `gh`, and Buzz
CLI configuration. This runner does not require AIBuddy's scheduler.
...
```text
~/.config/aibuddy/buzz/github-manager
```
...
install and authenticate `gh`, install and configure AIBuddy, and install Buzz
