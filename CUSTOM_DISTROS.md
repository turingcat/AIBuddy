# Custom Distributions of heybuddy
...
This guide explains how to create custom distributions of heybuddy tailored to your organization's needs—whether that's preconfigured models, custom tools, branded interfaces, or entirely new user experiences.

## Overview

heybuddy's architecture is designed for extensibility. Organizations can create "remixed" versions that:
...
│  │  (heybuddy-cli)│  │  (Electron) │  │  (web, mobile, etc.)    │  │
│  └──────┬──────┘  └──────┬──────┘  └────────────┬────────────┘  │
└─────────┼────────────────┼──────────────────────┼───────────────┘
          │                │                      │
          ▼                ▼                      ▼
...
| Add custom AI providers | `crates/heybuddy/src/providers/declarative/` | Low |
...
| Modify system prompts | `crates/heybuddy/src/prompts/` | Low |
| Customize desktop branding | `ui/desktop/` (icons, names, colors) | Medium |
| Build a new UI (web, mobile) | Integrate with `heybuddy serve` over ACP | High |
...
heybuddy is licensed under Apache License 2.0 (ASL v2). Custom distributions must:
- Include the original license and copyright notices
- Clearly indicate any modifications made
- Not use "HeyBuddy" trademarks in ways that imply official endorsement
...
**Goal**: Ship heybuddy preconfigured to use a local Ollama model, requiring no API keys.
...
- Provider configuration: `crates/heybuddy/src/config/base.rs`
- Ollama provider implementation: `crates/heybuddy/src/providers/ollama.rs`
...
**Goal**: Distribute heybuddy internally with pre-provisioned API keys for a frontier model.

### Steps

1. **Store API keys securely** using heybuddy's secret management:
...
```bash
# Secrets are stored in system keyring or ~/.config/heybuddy/secrets.yaml
# if HEYBUDDY_DISABLE_KEYRING=1
heybuddy configure set-secret ANTHROPIC_API_KEY "your-corporate-key"
```
...
- Secret storage: `crates/heybuddy/src/config/base.rs` (SecretStorage enum)
- Keyring integration: Uses system keyring by default, file-based fallback available
- Config file location: `~/.config/heybuddy/config.yaml`
...
description: heybuddy configured for data analysis
instructions: |
...
extensions:
  - type: stdio
    name: internal-data
    cmd: python
    args: ["/opt/corp-heybuddy/internal_data_mcp.py"]
    description: Corporate data lake access
```

### Technical Details

- Extension types: `crates/heybuddy/src/agents/extension.rs` (ExtensionConfig enum)
- Built-in MCP servers: `crates/heybuddy-mcp/`
- Extension loading: `crates/heybuddy/src/agents/extension_manager.rs`
...
3. **Update the system prompt** to reflect your branding in `crates/heybuddy/src/prompts/system.md`:
...
5. **Align packaging and updater names** when rebranding:
   ...
   - Set build/release environment variables consistently:
     ...
     - `HEYBUDDY_BUNDLE_NAME` for bundle/debug scripts and updater asset naming (defaults to `HeyBuddy`)

Example:

```bash
export GITHUB_OWNER="your-org"
export GITHUB_REPO="your-heybuddy-fork"
export HEYBUDDY_BUNDLE_NAME="InsightStream-heybuddy"
```
...
- System prompts: `crates/heybuddy/src/prompts/`

---

## E. Building a New Interface (Web, Mobile, etc.)

**Goal**: Create an entirely new frontend while leveraging heybuddy's backend.

heybuddy provides two ACP transport options for building custom clients:

### Option 1: ACP HTTP/WebSocket (`heybuddy serve`)

Use `heybuddy serve` for process-separated integrations such as web apps, desktop shells, and other clients:
...
For richer local integrations (IDEs and embedded agents), run heybuddy as an ACP agent over stdio.
...
**Start heybuddy as an ACP agent**:

```bash
# Run heybuddy as an ACP server on stdio
heybuddy acp --with-builtin developer,memory

# Or programmatically
cargo run -p heybuddy-cli -- acp --with-builtin developer
```
...
class AcpClient:
    def __init__(self):
        self.process = subprocess.Popen(
            ['heybuddy', 'acp', '--with-builtin', 'developer'],
...
**ACP**:
- ACP server implementation: `crates/heybuddy/src/acp/server.rs`
- CLI integration: `crates/heybuddy-cli/src/cli.rs` (Command::Acp)
...
**Goal**: Create a version of heybuddy tailored for a specific professional audience.
...
extensions:
  ...
  - type: stdio
    name: legal-database
    cmd: python
    args: ["/opt/legal-heybuddy/legal_db_mcp.py"]
...
- Recipe format: `crates/heybuddy/src/recipe/mod.rs`
- Recipe loading: `crates/heybuddy/src/recipe/local_recipes.rs`
...
Create a JSON file in `~/.config/heybuddy/custom_providers/` or bundle in your distribution:
...
1. Create a new file in `crates/heybuddy/src/providers/`
2. Implement the `Provider` trait from `base.rs`
3. Register in `crates/heybuddy/src/providers/factory.rs`

### Technical Details

- Declarative providers: `crates/heybuddy/src/config/declarative_providers.rs`
- Provider trait: `crates/heybuddy/src/providers/base.rs`
- Provider registration: `crates/heybuddy/src/providers/factory.rs`
- Example providers: `crates/heybuddy/src/providers/declarative/*.json`
...
Recipes are YAML files that define complete heybuddy experiences—instructions, extensions, parameters, and prompts bundled together. They're ideal for custom distributions because they require no code changes and can be distributed as simple files.
...
- Recipe schema: `crates/heybuddy/src/recipe/mod.rs`
- Parameter handling: `crates/heybuddy/src/recipe/template_recipe.rs`
- Recipe validation: `crates/heybuddy/src/recipe/validate_recipe.rs`
...
For complex workflows, heybuddy supports two powerful composition mechanisms:
...
- Subagent tool: `crates/heybuddy/src/agents/subagent_tool.rs`
- Subagent execution: `crates/heybuddy/src/agents/subagent_handler.rs`
- Recipe sub_recipes field: `crates/heybuddy/src/recipe/mod.rs` (SubRecipe struct)
- Template rendering: `crates/heybuddy/src/recipe/template_recipe.rs`
