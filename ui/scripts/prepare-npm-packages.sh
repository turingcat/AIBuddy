...
output_dir="$2"
repository="${GITHUB_REPOSITORY:-turingcat/AIBuddy}"
work_dir="$(mktemp -d)"
...
download_release_binaries() {
  gh release download "$release_tag" \
    --repo "$repository" \
    --dir "$asset_dir" \
    --pattern 'aibuddy-aarch64-apple-darwin.tar.bz2' \
    --pattern 'aibuddy-aarch64-unknown-linux-gnu.tar.bz2' \
    --pattern 'aibuddy-x86_64-unknown-linux-gnu.tar.bz2' \
    --pattern 'aibuddy-x86_64-pc-windows-msvc.zip'
}
...
copy_unix_binary() {
  ...
  local destination="$repo_root/ui/aibuddy-binary/aibuddy-binary-$platform/bin/aibuddy"

  mkdir -p "$extract_dir" "$(dirname "$destination")"
  tar -xjf "$asset_dir/aibuddy-$target.tar.bz2" -C "$extract_dir"
  test -f "$extract_dir/aibuddy"
  rm -f "$destination"
  install -m 755 "$extract_dir/aibuddy" "$destination"
}
...
copy_release_binaries() {
  ...
  local destination="$repo_root/ui/aibuddy-binary/aibuddy-binary-win32-x64/bin/aibuddy.exe"
  mkdir -p "$extract_dir" "$(dirname "$destination")"
  unzip -q "$asset_dir/aibuddy-x86_64-pc-windows-msvc.zip" -d "$extract_dir"
  test -f "$extract_dir/aibuddy-package/aibuddy.exe"
  rm -f "$destination"
  install -m 755 "$extract_dir/aibuddy-package/aibuddy.exe" "$destination"
}
...
current_platform_binary() {
  local platform
  local executable="aibuddy"
  platform="$(current_platform)"

  case "$platform" in
    darwin-arm64 | linux-arm64 | linux-x64) ;;
    win32-x64) executable="aibuddy.exe" ;;
    *)
      echo "No AIBuddy npm binary is available for $platform" >&2
      return 1
      ;;
  esac

  echo "$repo_root/ui/aibuddy-binary/aibuddy-binary-$platform/bin/$executable"
}
...
pack_packages() {
  local packages=(
    ui/aibuddy-binary/aibuddy-binary-darwin-arm64
    ui/aibuddy-binary/aibuddy-binary-linux-arm64
    ui/aibuddy-binary/aibuddy-binary-linux-x64
    ui/aibuddy-binary/aibuddy-binary-win32-x64
    ui/aibuddy-acp
  ...
  tar -xzf "$output_dir/aibuddy-aibuddy-acp-$release_version.tgz" \
...
verify_packed_wrapper() {
  ...
  (
    ...
    "$repo_root/bin/pnpm" add \
      "$output_dir/aibuddy-aibuddy-binary-$platform-$release_version.tgz" \
      "$output_dir/aibuddy-aibuddy-acp-$release_version.tgz"

    assert_version "Packed wrapper" \
      "$(env -u AIBUDDY_BINARY "$repo_root/bin/pnpm" exec aibuddy --version | xargs)"
  )
}
