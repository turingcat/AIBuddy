...
function createLauncherHarness() {
  const root = mkdtempSync(join(tmpdir(), "aibuddy-acp-launcher-"));
  tempDirs.push(root);

  const scopeDir = join(root, "node_modules", "@aibuddy");
  const wrapperDir = join(scopeDir, "aibuddy-acp");
  ...
  copyFileSync(
    join(packageRoot, "bin", "aibuddy.mjs"),
    join(wrapperDir, "bin", "aibuddy.mjs"),
  );
  ...
  const platformPackageDir = join(scopeDir, packageName.slice("@aibuddy/".length));
  const platformBinDir = join(platformPackageDir, "bin");
  const executableName = process.platform === "win32" ? "aibuddy.exe" : "aibuddy";
  ...
  if (process.platform === "win32") {
    ...
    writeFileSync(
      executablePath,
      '#!/bin/sh\nexec "$AIBUDDY_ACP_TEST_NODE" "$@"\n',
    );
  }
  chmodSync(executablePath, 0o755);

  return join(wrapperDir, "bin", "aibuddy.mjs");
}
...
function runLauncher(args, input) {
  ...
  const result = spawnSync(process.execPath, [launcherPath, ...args], {
    ...
    env: {
      ...process.env,
      AIBUDDY_BINARY: "",
      AIBUDDY_ACP_TEST_NODE: process.execPath,
    },
    input,
  });
