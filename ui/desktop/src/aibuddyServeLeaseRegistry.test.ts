...
import type { AIBuddyServeResult, Logger } from './aibuddyServe';
import {
  AIBUDDY_SERVE_EXITED_USER_MESSAGE,
  AIBuddyServeLeaseRegistry,
} from './aibuddyServeLeaseRegistry';
...
function createAIBuddyServeResult(
  overrides: Partial<Pick<AIBuddyServeResult, 'cleanup' | 'hasExited' | 'getExitDetails'>> = {}
): AIBuddyServeResult {
  return {
    acpUrl: 'ws://127.0.0.1:1234/acp?token=test',
    workingDir: '/tmp',
    process: new EventEmitter() as AIBuddyServeResult['process'],
    errorLog: [],
...
describe('AIBuddyServeLeaseRegistry', () => {
  it('returns the ACP URL for an attached live lease', () => {
    const store = new AIBuddyServeLeaseRegistry(createLogger());
    const lease = store.create(createAIBuddyServeResult(), 'local-secret');
  ...
  it('throws a recovery message after the process exits', () => {
    ...
    const store = new AIBuddyServeLeaseRegistry(logger);
    const result = createAIBuddyServeResult();
  ...
  it('cleans up once after the last attached window is released', async () => {
    ...
    const store = new AIBuddyServeLeaseRegistry(createLogger());
    const lease = store.create(createAIBuddyServeResult({ cleanup }), 'local-secret');
    store.attachWindow(1, lease);
