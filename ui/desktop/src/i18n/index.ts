 ...
 *   1. HEYBUDDY_LOCALE config value (manual setting or environment variable, passed through appConfig)
...
/**
 * Product and URL scheme placeholders are supplied centrally so call sites
 * consistently render the fixed AIBuddy identity.
 */
export function useIntl(): IntlShape {
  const intl = useReactIntl();

  return useMemo(() => {
    const brandValues = { appName: appBrand.productName, protocol: appBrand.protocol };
...
export function getLocale(): { locale: string; messageLocale: string } {
  const explicit =
    typeof window !== 'undefined' && window.appConfig
      ? window.appConfig.get('HEYBUDDY_LOCALE')
      : undefined;
