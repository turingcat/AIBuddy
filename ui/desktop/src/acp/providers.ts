...
export async function acpListProviderDetails(): Promise<ProviderDetails[]> {
  ...
  const { entries } = await client.aibuddy.providersList_unstable({});
...
export async function acpGetProviderDetails(providerId: string): Promise<ProviderDetails> {
  const client = await getAcpClient();
  const { entries } = await client.aibuddy.providersList_unstable({ providerIds: [providerId] });
...
async function waitForProviderInventoryRefresh(
  ...
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    ...
    const response = await client.aibuddy.providersList_unstable({ providerIds: [providerId] });
    throwIfAborted(signal);
...
export async function acpRefreshProviderDetails(
  ...
  const client = await getAcpClient();
  throwIfAborted(signal);
  let { entries } = await client.aibuddy.providersList_unstable({ providerIds: [providerId] });
  throwIfAborted(signal);
  ...
  const readiness = await client.aibuddy.providersReadinessCheck_unstable({ providerId });
  throwIfAborted(signal);
  if (!readiness.ready) {
    return {
      provider: providerEntryToDetails(entry),
  ...
  if (entry.supportsRefresh) {
    const refresh = await client.aibuddy.providersInventoryRefresh_unstable({
...
// AIBuddyExtClient 类型未暴露该 unstable 方法，用类型断言调用 generated client。
export async function acpRefreshProviderInventory(providerId: string): Promise<void> {
  const client = await getAcpClient();
  const aibuddy = client.aibuddy as {
    providersInventoryRefresh_unstable: (params: { providerIds: string[] }) => Promise<unknown>;
  };
  await aibuddy.providersInventoryRefresh_unstable({ providerIds: [providerId] });
}
...
export async function acpListProviderCatalogEntries(
  ...
  const client = await getAcpClient();
  const { providers } = await client.aibuddy.providersCatalogList_unstable(format ? { format } : {});
  return providers;
}

export async function acpGetProviderTemplate(providerId: string): Promise<ProviderTemplateDto> {
  const client = await getAcpClient();
  const { template } = await client.aibuddy.providersCatalogTemplate_unstable({ providerId });
  return template;
}
...
export async function acpGetCustomProvider(
  ...
  const client = await getAcpClient();
  return client.aibuddy.providersCustomRead_unstable({ providerId });
}
...
export async function acpCreateCustomProviderFromRequest(
  ...
  const client = await getAcpClient();
  const response = await client.aibuddy.providersCustomCreate_unstable(
...
export async function acpUpdateCustomProviderFromRequest(
  ...
  const client = await getAcpClient();
  await client.aibuddy.providersCustomUpdate_unstable({
    providerId,
    ...updateRequestToCreate(request),
  });
}
...
export async function acpDeleteCustomProvider(providerId: string): Promise<void> {
  const client = await getAcpClient();
  await client.aibuddy.providersCustomDelete_unstable({ providerId });
}

export async function acpReadProviderConfig(providerId: string) {
  const client = await getAcpClient();
  const { fields } = await client.aibuddy.providersConfigRead_unstable({ providerId });
  return fields;
}

export async function acpDeleteProviderConfig(providerId: string): Promise<void> {
  const client = await getAcpClient();
  await client.aibuddy.providersConfigDelete_unstable({ providerId });
}
...
export async function acpSaveProviderConfig(
  ...
  const client = await getAcpClient();
  await client.aibuddy.providersConfigSave_unstable({ providerId, fields });
}
...
export async function acpEnableProvider(
  ...
  const client = await getAcpClient();
  throwIfAborted(signal);
  const { refresh } = await client.aibuddy.providersConfigSave_unstable({
    providerId,
    fields: [],
  });
  throwIfAborted(signal);
...
export async function acpAuthenticateProvider(providerId: string): Promise<void> {
  const client = await getAcpClient();
  await client.aibuddy.providersConfigAuthenticate_unstable({ providerId });
}

export async function acpListProviderSecrets(): Promise<ProviderSecretDto[]> {
  const client = await getAcpClient();
  const { secrets } = await client.aibuddy.providersSecretsList_unstable({});
  return secrets;
}

export async function acpDeleteProviderSecret(id: string): Promise<void> {
  const client = await getAcpClient();
  await client.aibuddy.providersSecretsDelete_unstable({ id });
}
...
export async function acpGetCanonicalModelInfo(
  ...
  const client = await getAcpClient();
  const { modelInfo } = await client.aibuddy.providersCanonicalModelInfo_unstable({
    provider,
    model,
  });
...
export async function acpReadDefaults(): Promise<{
  ...
  const client = await getAcpClient();
  const response = await client.aibuddy.defaultsRead_unstable({});
  return {
...
export async function acpSaveDefaults(providerId: string, modelId?: string | null): Promise<void> {
  const client = await getAcpClient();
  await client.aibuddy.defaultsSave_unstable({ providerId, modelId: modelId ?? null });
}

export async function acpClearDefaults(): Promise<void> {
  const client = await getAcpClient();
  await client.aibuddy.defaultsClear_unstable({});
}

export async function acpReadThinkingEffort(): Promise<ThinkingEffort | null> {
  const client = await getAcpClient();
  const response = await client.aibuddy.preferencesRead_unstable({ keys: ['aibuddyThinkingEffort'] });
  const value = response.values.find((v) => v.key === 'aibuddyThinkingEffort')?.value;
...
export async function acpSaveThinkingEffort(effort: ThinkingEffort): Promise<void> {
  const client = await getAcpClient();
  await client.aibuddy.preferencesSave_unstable({
    values: [{ key: 'aibuddyThinkingEffort', value: effort }],
  });
}
