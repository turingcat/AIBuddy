...
export async function encodeRecipe(recipe: RecipeDto): Promise<string> {
  try {
    ...
    const response = await client.aibuddy.recipesEncode_unstable({ recipe });
...
export async function decodeRecipe(deeplink: string): Promise<RecipeDto> {
  try {
    const client = await getAcpClient();
    const response = await client.aibuddy.recipesDecode_unstable({ deeplink });
...
export async function scanRecipe(recipe: RecipeDto): Promise<ScanRecipeResponse_unstable> {
  try {
    const client = await getAcpClient();
    return await client.aibuddy.recipesScan_unstable({ recipe });
  } catch (error) {
...
export async function parseRecipe(content: string): Promise<RecipeDto> {
  try {
    const client = await getAcpClient();
    const response = await client.aibuddy.recipesParse_unstable({ content });
    return response.recipe;
  } catch (error) {
...
export async function saveRecipe(
  ...
  try {
    const client = await getAcpClient();
    return await client.aibuddy.recipesSave_unstable({
      recipe,
      id,
    });
  } catch (error) {
...
export async function listRecipes(): Promise<RecipeListEntryDto[]> {
  ...
  const listPromise = (async () => {
    const client = await getAcpClient();
    const response = await client.aibuddy.recipesList_unstable({});
...
export async function deleteRecipe(id: string): Promise<void> {
  try {
    const client = await getAcpClient();
    await client.aibuddy.recipesDelete_unstable({ id });
  } catch (error) {
...
export async function scheduleRecipe(id: string, cronSchedule?: string | null): Promise<void> {
  try {
    const client = await getAcpClient();
    await client.aibuddy.recipesSchedule_unstable({ id, cron_schedule: cronSchedule });
  } catch (error) {
...
export async function setRecipeSlashCommand(
  ...
  try {
    const client = await getAcpClient();
    await client.aibuddy.recipesSlashCommand_unstable({ id, slash_command: slashCommand });
  } catch (error) {
...
export async function recipeToYaml(recipe: RecipeDto): Promise<string> {
  try {
    const client = await getAcpClient();
    const response = await client.aibuddy.recipesToYaml_unstable({ recipe });
