...
const mockClientFactory = vi.hoisted(() => {
  ...
  type MockClient = {
    ...
    heybuddy: Record<string, never>;
  };
  const instances: Array<{ client: MockClient; resolveClosed: () => void }> = [];
  const connectHeyBuddyAcpClient = vi.fn((_stream: MockStream): MockClient => {
  ...
  return { connectHeyBuddyAcpClient, initialize, instances };
});
...
vi.mock('@heybuddy/heybuddy-sdk', () => ({
  DEFAULT_HEYBUDDY_MCP_HOST_CAPABILITIES: {},
}));

vi.mock('../heybuddyAcpClient', () => ({
  connectHeyBuddyAcpClient: mockClientFactory.connectHeyBuddyAcpClient,
}));
...
describe('ACP connection ownership', () => {
  ...
  it('stops reconnecting when the HeyBuddy backend has exited', async () => {
    ...
    const getAcpUrl = vi
      .fn()
      .mockRejectedValue(
        new Error(`Error invoking remote method 'get-acp-url': ${HEYBUDDY_SERVE_EXITED_USER_MESSAGE}`)
      );
    window.electron.getAcpUrl = getAcpUrl;
    mockClientFactory.instances[0].resolveClosed();
    await Promise.resolve();

    const connection = expect(getAcpClient()).rejects.toThrow(HEYBUDDY_SERVE_EXITED_USER_MESSAGE);
    await vi.advanceTimersByTimeAsync(250);
    await connection;
  ...
  it('does not cache terminal recovery failures across caller retries', async () => {
    ...
    const failedRecovery = expect(getAcpClient()).rejects.toThrow(HEYBUDDY_SERVE_EXITED_USER_MESSAGE);
    await vi.advanceTimersByTimeAsync(250);
    await failedRecovery;

    await expect(getAcpClient()).rejects.toThrow(HEYBUDDY_SERVE_EXITED_USER_MESSAGE);
