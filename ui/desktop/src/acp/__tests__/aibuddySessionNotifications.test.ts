...
function aibuddyUpdate(
  update: AIBuddySessionNotification_unstable['update']
...
describe('applyAIBuddySessionNotification', () => {
  describe('message_usage', () => {
    it('attaches usage to the message matching messageId', () => {
      const state = makeState();

      const changes = applyAIBuddySessionNotification(state, messageUsageNotification('a1'));
    ...
    it.each([
      ...
      const changes = applyAIBuddySessionNotification(state, messageUsageNotification(messageId));
    ...
    it('skips client-made approval rows when falling back', () => {
      ...
      const changes = applyAIBuddySessionNotification(state, messageUsageNotification('missing'));
  ...
  describe('usage_update', () => {
    it('still maps usage updates into token state', () => {
      const changes = applyAIBuddySessionNotification(
        makeState(),
        aibuddyUpdate({
