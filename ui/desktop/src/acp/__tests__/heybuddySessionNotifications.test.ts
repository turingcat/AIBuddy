...
function heybuddyUpdate(
  update: HeyBuddySessionNotification_unstable['update']
...
describe('applyHeyBuddySessionNotification', () => {
  describe('message_usage', () => {
    it('attaches usage to the message matching messageId', () => {
      const state = makeState();

      const changes = applyHeyBuddySessionNotification(state, messageUsageNotification('a1'));
    ...
    it.each([
      ...
      const changes = applyHeyBuddySessionNotification(state, messageUsageNotification(messageId));
    ...
    it('skips client-made approval rows when falling back', () => {
      ...
      const changes = applyHeyBuddySessionNotification(state, messageUsageNotification('missing'));
  ...
  describe('usage_update', () => {
    it('still maps usage updates into token state', () => {
      const changes = applyHeyBuddySessionNotification(
        makeState(),
        heybuddyUpdate({
