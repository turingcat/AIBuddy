...
import { connectAIBuddyAcpClient, type AIBuddyAcpCallbacks } from '../aibuddyAcpClient';
...
function callbacks(): AIBuddyAcpCallbacks {
  return {
...
describe('AIBuddy ACP client composition', () => {
  it('registers standard and AIBuddy-specific handlers on a live ACP connection', async () => {
    const stream = createTestStream();
    const handlers = callbacks();
    const client = connectAIBuddyAcpClient(stream, handlers);
    ...
    const toolsRequest = client.aibuddy.toolsList_unstable({ sessionId: 'session-1' });
    await waitForWrites(stream, 3);
    const outboundRequest = stream.writes[2] as { id: number; method: string };
    expect(outboundRequest.method).toBe('_aibuddy/unstable/tools/list');
    stream.push({
      jsonrpc: '2.0',
