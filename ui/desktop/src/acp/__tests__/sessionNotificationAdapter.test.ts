...
describe('createAcpSessionNotificationAdapter', () => {
      ...
      it('preserves output-limit metadata when updating a thought message', () => {
        ...
        adapter.apply(
          acpUpdate({
            ...
            _meta: { aibuddy: { messageId: 'thought-1' } },
          } as SessionNotification['update'])
        );
  ...
  describe('applyAIBuddy', () => {
    it('maps usage updates into token state', () => {
      const adapter = createAcpSessionNotificationAdapter();

      expect(
        adapter.applyAIBuddy(
          aibuddyUpdate({
    ...
    it('maps status messages and keeps later id-less chunks separate', () => {
      ...
      const noticeStateChanges = adapter.applyAIBuddy(
        aibuddyUpdate({
      ...
      const progressStateChanges = adapter.applyAIBuddy(
        aibuddyUpdate({
          sessionUpdate: 'status_message',
