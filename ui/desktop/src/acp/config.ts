...
export async function acpReadConfig(
  ...
  const { value } = await client.aibuddy.configRead_unstable({ key, isSecret });
  if (value == null) {
    return null;
  }
  if (isSecret) {
...
export async function acpUpsertConfig(
  ...
  const client = await getAcpClient();
  await client.aibuddy.configUpsert_unstable({ key, value, isSecret });
}

export async function acpRemoveConfig(key: string, isSecret: boolean): Promise<void> {
  const client = await getAcpClient();
  await client.aibuddy.configRemove_unstable({ key, isSecret });
}

export async function acpReadAllConfig(): Promise<Record<string, unknown>> {
  const client = await getAcpClient();
  const { config } = await client.aibuddy.configReadAll_unstable({});
  return config;
}
