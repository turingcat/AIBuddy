...
interface AIBuddySessionInfoMeta {
  messageCount?: number;
  createdAt?: string;
...
function sessionInfoMeta(s: SessionInfo): AIBuddySessionInfoMeta {
  return (s._meta ?? {}) as AIBuddySessionInfoMeta;
}
...
export async function acpGetSessionListItem(sessionId: string): Promise<SessionListItem> {
  const client = await getAcpClient();
  const response = await client.aibuddy.sessionInfo_unstable({ sessionId });
...
async function loadAcpSession(sessionId: string): Promise<AcpLoadSessionResult> {
  const client = await getAcpClient();
  const initialSessionInfoResponse = await client.aibuddy.sessionInfo_unstable({ sessionId });
  ...
  const sessionInfoResponse = await client.aibuddy.sessionInfo_unstable({ sessionId });
...
export async function acpNewSession(
  ...
  const client = await getAcpClient();
  const meta: Record<string, unknown> = { client: 'aibuddy-desktop' };
  if (aibuddyExtensions.length > 0) {
    meta.enabledExtensions = aibuddyExtensions;
  }
  if (recipe?.recipeId) {
...
export async function acpRenameSession(sessionId: string, title: string): Promise<void> {
  const client = await getAcpClient();
  await client.aibuddy.sessionRename_unstable({ sessionId, title });
}

export async function acpUpdateWorkingDir(sessionId: string, workingDir: string): Promise<void> {
  const client = await getAcpClient();
  await client.aibuddy.sessionWorkingDirUpdate_unstable({ sessionId, workingDir });
}
...
export async function acpTruncateSessionConversation(
  ...
  const client = await getAcpClient();
  await client.aibuddy.sessionConversationTruncate_unstable({ sessionId, truncateFrom });
}
...
export async function acpForkSession(
  ...
  const client = await getAcpClient();
  const sessionInfo = await client.aibuddy.sessionInfo_unstable({ sessionId });
...
export async function acpExportSession(
  ...
  const client = await getAcpClient();
  const response = await client.aibuddy.sessionExport_unstable({ sessionId, format });
  return response.data;
}

export async function acpImportSession(input: string, source: SessionImportSource): Promise<void> {
  const client = await getAcpClient();
  await client.aibuddy.sessionImport_unstable({ input, source });
}

export async function acpShareSessionNostr(sessionId: string, relays: string[]) {
  const client = await getAcpClient();
  return await client.aibuddy.sessionShareNostr_unstable({ sessionId, relays });
}
