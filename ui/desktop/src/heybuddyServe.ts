...
import {
  ...
  createHeyBuddyServeStartupDiagnostics,
  type HeyBuddyServeStartupDiagnostics,
...
export const defaultLogger: Logger = {
  info: (...args) => console.log('[heybuddy-serve]', ...args),
  error: (...args) => console.error('[heybuddy-serve]', ...args),
};

export interface FindHeyBuddyBinaryOptions {
  isPackaged?: boolean;
  resourcesPath?: string;
}

type ReadinessFetchInit = Parameters<typeof globalThis.fetch>[1];
export type HeyBuddyServeExitSignal = ChildProcess['signalCode'];
type ReadinessFetch = (input: string, init?: ReadinessFetchInit) => Promise<Response>;

export interface StartHeyBuddyServeOptions extends FindHeyBuddyBinaryOptions {
  dir?: string;
  serverSecret: string;
  tls?: boolean;
  env?: Record<string, string | undefined>;
  /** PATH from the user's login shell, appended so heybuddyd can find CLI providers. */
...
export interface HeyBuddyServeResult {
  acpUrl: string;
  workingDir: string;
  ...
  hasExited: () => boolean;
  getExitDetails: () => { code: number | null; signal: HeyBuddyServeExitSignal };
  startupDiagnosticsPath: string | null;
  getStartupDiagnostics: () => HeyBuddyServeStartupDiagnostics | null;
...
export const findHeyBuddyBinaryPath = (options: FindHeyBuddyBinaryOptions = {}): string => {
  const { isPackaged = false, resourcesPath } = options;
  const pathFromEnv = process.env.HEYBUDDY_BINARY;
  if (pathFromEnv) {
    if (isPackaged) {
      throw new Error('HEYBUDDY_BINARY is only supported in development builds');
    }
    ...
    throw new Error(`Invalid HEYBUDDY_BINARY path: ${pathFromEnv} (pwd is ${process.cwd()})`);
  }
  ...
  throw new Error(
    `heybuddy binary not found in any of the possible paths: ${possiblePaths.join(', ')}`
  );
};
...
const CERT_FINGERPRINT_PREFIX = 'HEYBUDDYD_CERT_FINGERPRINT=';
...
const waitForHeyBuddyServeReady = async (
  statusUrl: string,
  errorLog: string[],
...
const buildHeyBuddyServeEnv = (
  ...
  env.HEYBUDDY_SERVER__SECRET_KEY = serverSecret;

  return env;
};

export const startHeyBuddyServe = async ({
  dir,
  serverSecret,
  tls = false,
  env: additionalEnv = {},
...
}: StartHeyBuddyServeOptions): Promise<HeyBuddyServeResult> => {
  const workingDir = dir || process.cwd();
  const startupTrace = createHeyBuddyServeStartupDiagnostics(diagnosticsDir, workingDir);
  const startupDiagnosticsPath = startupTrace?.diagnosticsPath ?? null;
  const secretKey = serverSecret.trim();
  if (!secretKey) {
    const message = 'HEYBUDDY_SERVER__SECRET_KEY is required for heybuddy serve';
    startupTrace?.record('configuration_error', { message });
    throw new Error(withStartupDiagnosticsPath(message, startupDiagnosticsPath));
  }

  let heybuddyPath: string;
  try {
    heybuddyPath = findHeyBuddyBinaryPath({ isPackaged, resourcesPath });
  } catch (error) {
  ...
  logger.info(`Starting heybuddy serve from: ${heybuddyPath} on port ${port} in dir ${workingDir}`);
  if (startupTrace) {
    startupTrace.diagnostics.binaryPath = heybuddyPath;
  ...
  const spawnOptions = {
    env: buildHeyBuddyServeEnv(secretKey, heybuddyPath, additionalEnv, loginShellPath),
    cwd: workingDir,
    windowsHide: true,
  ...
  const heybuddyProcess = spawn(heybuddyPath, args, spawnOptions);
  if (startupTrace) {
    startupTrace.diagnostics.pid = heybuddyProcess.pid ?? null;
    startupTrace.record('spawn_success', { pid: heybuddyProcess.pid ?? null });
  }

  let exited = false;
  let spawnFailed = false;
  let exitCode: number | null = null;
  let exitSignal: HeyBuddyServeExitSignal = null;
  ...
  const stopStdoutCollection = () => {
    ...
    heybuddyProcess.stdout?.off('data', onStdoutData);
    heybuddyProcess.stdout?.resume();
  };
  ...
  heybuddyProcess.stdout?.on('data', onStdoutData);
  ...
  const onStderrData = (data: Buffer) => {
    ...
    for (const line of lines) {
      if (line.trim() && isFatalError(line)) {
        logger.error(`heybuddy serve stderr for port ${port} and dir ${workingDir}: ${line}`);
      }
    }
  };

  heybuddyProcess.stderr?.on('data', onStderrData);

  heybuddyProcess.on('exit', (code, signal) => {
    exited = true;
    exitCode = code;
    exitSignal = signal;
    logger.info(
      `heybuddy serve process exited with code ${code} and signal ${signal} for port ${port} and dir ${workingDir}`
    );
    if (startupTrace) {
  ...
  heybuddyProcess.on('error', (error) => {
    spawnFailed = true;
    errorLog.push(error.message);
    logger.error(`Failed to start heybuddy serve on port ${port} and dir ${workingDir}`, error);
    startupTrace?.record('spawn_error', { message: error.message, name: error.name });
  });

  const cleanup = async (): Promise<void> => {
    return new Promise<void>((resolve) => {
      if (exited || heybuddyProcess.killed) {
        resolve();
        return;
      }
      ...
      heybuddyProcess.once('close', finish);

      logger.info('Terminating heybuddy serve');
      try {
        if (process.platform === 'win32') {
          if (heybuddyProcess.pid) {
            spawn('taskkill', ['/pid', heybuddyProcess.pid.toString(), '/f', '/t']);
          }
        } else {
          heybuddyProcess.kill('SIGTERM');
        }
      } catch (error) {
        logger.error('Error while terminating heybuddy serve process:', error);
      }

      setTimeout(() => {
        if (!exited && !heybuddyProcess.killed && process.platform !== 'win32') {
          heybuddyProcess.kill('SIGKILL');
        }
        finish();
      }, 5000);
    });
  ...
  const ready = await waitForHeyBuddyServeReady(statusUrl, errorLog, () => exited || spawnFailed, {
    healthUrl,
    readinessFetch,
  ...
  const stopOutputCollection = () => {
    stopStdoutCollection();
    heybuddyProcess.stderr?.off('data', onStderrData);
    heybuddyProcess.stderr?.resume();
  };
  ...
  if (!ready) {
    ...
    throw new Error(
      withStartupDiagnosticsPath(
        `heybuddy serve did not become ready on ${statusUrl}.${exitDetails}${stderrDetails}`,
  ...
  if (tls) {
    ...
    if (!fingerprint) {
      ...
      throw new Error(
        withStartupDiagnosticsPath(
          `heybuddy serve did not emit TLS certificate fingerprint on ${statusUrl}.${exitDetails}${stderrDetails}`,
          startupDiagnosticsPath
        )
      );
    }
