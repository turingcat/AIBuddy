...
const builtIns = [
  ...
  },
  {
    name: 'memory',
    display_name: 'Memory',
    description: 'Teach aibuddy your preferences as you go.',
    enabled: false,
    type: 'builtin',
  },
  {
...
describe('ExtensionList', () => {
  it('localizes headings and built-in extension copy by stable ID', () => {
    ...
    expect(screen.getByText('让 AIBuddy 在使用过程中记住你的偏好。')).toBeInTheDocument();
  ...
  it('uses Chinese extension-view and configuration copy', () => {
    ...
    expect(
      ...
      '这些扩展使用模型上下文协议（MCP）。它们可以通过三大组件扩展 AIBuddy 的能力：提示词、资源和工具。按 Cmd+K 搜索。'
    );
