...
export default function AppsView() {
  const intl = useIntl();
  const [apps, setApps] = useState<HeyBuddyApp[]>([]);
  ...
  const refreshAppsExtensionList = useCallback(async (activeSessionId?: string) => {
    ...
    try {
      ...
      const merged = new Map<string, HeyBuddyApp>();
  ...
  const handleLaunchApp = async (app: HeyBuddyApp) => {
    try {
  ...
  const handleDeleteApp = async (app: HeyBuddyApp) => {
    if (
  ...
  const handleDownloadApp = async (app: HeyBuddyApp) => {
    try {
          ...
          {loading ? (
            ...
            <GridLayout>
              {apps.map((app) => {
                ...
                const retiredChatApp = isRetiredHeyBuddyChatApp(app);
