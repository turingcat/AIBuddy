...
describe('deriveMessageRowContexts', () => {
  it('only cancels older requests without a response', () => {
    const request = message('request', 'assistant', [toolRequest('tool-1')]);
    expect(deriveMessageRowContexts([request])[0].toolStates[0].isCancelled).toBe(false);

    const later = message('later', 'user', [{ type: 'text', text: 'continue' }]);
    expect(deriveMessageRowContexts([request, later])[0].toolStates[0].isCancelled).toBe(true);

    const response = message('response', 'user', [toolResponse('tool-1', 'done')]);
    expect(deriveMessageRowContexts([request, response])[0].toolStates[0].isCancelled).toBe(false);
  });
