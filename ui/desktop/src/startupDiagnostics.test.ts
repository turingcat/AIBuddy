...
import { createHeyBuddyServeStartupDiagnostics } from './startupDiagnostics';
...
describe('startup diagnostics', () => {
  ...
  it('writes serve startup diagnostics with serve-specific fields', () => {
    ...
    const trace = createHeyBuddyServeStartupDiagnostics(diagnosticsDir, '/tmp/project');

    expect(trace).not.toBeNull();
    trace!.diagnostics.binaryPath = '/bin/heybuddy';
    ...
    expect(path.basename(trace!.diagnosticsPath)).toMatch(/^heybuddy-serve-startup-.*\.json$/);
    const saved = JSON.parse(fs.readFileSync(trace!.diagnosticsPath, 'utf8'));
    expect(saved).toMatchObject({
      binaryPath: '/bin/heybuddy',
    ...
    expect(saved).not.toHaveProperty('heybuddydPath');
