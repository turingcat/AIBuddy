export const getInitialWorkingDir = (): string => {
  ...
  return (window.appConfig?.get('AIBUDDY_WORKING_DIR') as string) ?? '';
};

/**
 * Resolve the working directory for a new chat in the current window.
 *
 * AIBUDDY_WORKING_DIR is fixed when the window is created, so it goes stale when
 ...
 * via the aibuddyServeLeases) and that backend still matches the current
 ...
 * chats in the same window. Env-mode backends (AIBUDDY_EXTERNAL_BACKEND) always
...
export const getEffectiveWorkingDir = async (): Promise<string> => {
  ...
  const boundUrl = window.appConfig?.get('AIBUDDY_EXTERNAL_BACKEND_URL') as string | undefined;
  const source = window.appConfig?.get('AIBUDDY_EXTERNAL_BACKEND_SOURCE') as string | undefined;
  if (window.appConfig?.get('AIBUDDY_EXTERNAL_BACKEND') !== true || !boundUrl) {
    return initial;
  }
  try {
    const external = await window.electron.getSetting('externalAIBuddyd');
    const remote = external?.workingDir?.trim();
    if (!remote) {
      return initial;
    }
    // Env-mode backends use settings.externalAIBuddyd.workingDir regardless of the
