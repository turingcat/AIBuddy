export const getInitialWorkingDir = (): string => {
  ...
  return (window.appConfig?.get('HEYBUDDY_WORKING_DIR') as string) ?? '';
};

/**
 * Resolve the working directory for a new chat in the current window.
 *
 * HEYBUDDY_WORKING_DIR is fixed when the window is created, so it goes stale when
 ...
 * via the heybuddyServeLeases) and that backend still matches the current
 ...
 * chats in the same window. Env-mode backends (HEYBUDDY_EXTERNAL_BACKEND) always
...
export const getEffectiveWorkingDir = async (): Promise<string> => {
  ...
  const boundUrl = window.appConfig?.get('HEYBUDDY_EXTERNAL_BACKEND_URL') as string | undefined;
  const source = window.appConfig?.get('HEYBUDDY_EXTERNAL_BACKEND_SOURCE') as string | undefined;
  if (window.appConfig?.get('HEYBUDDY_EXTERNAL_BACKEND') !== true || !boundUrl) {
    return initial;
  }
  try {
    const external = await window.electron.getSetting('externalHeyBuddyd');
    const remote = external?.workingDir?.trim();
    if (!remote) {
      return initial;
    }
    // Env-mode backends use settings.externalHeyBuddyd.workingDir regardless of the
