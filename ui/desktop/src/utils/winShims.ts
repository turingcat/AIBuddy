...
/**
 * Ensures Windows shims are available in %LOCALAPPDATA%\AIBuddy\bin
 * This allows the bundled executables to be found via PATH regardless of where AIBuddy is installed
 */
export async function ensureWinShims(): Promise<void> {
  ...
  try {
    ...
    // Copy command-line tools only; the aibuddy binary is never shimmed
    ...
    if (!currentPath.toLowerCase().includes(tgtDir.toLowerCase())) {
      ...
      log.info(`Added ${tgtDir} to PATH for AIBuddy processes only`);
    } else {
      ...
      if (binDirIndex > 0) {
        ...
        log.info(`Moved ${tgtDir} to beginning of PATH for AIBuddy processes only`);
      }
    }
  } catch (error) {
