...
import type { HeyBuddyServeResult, Logger } from './heybuddyServe';
import {
  HEYBUDDY_SERVE_EXITED_USER_MESSAGE,
  HeyBuddyServeLeaseRegistry,
} from './heybuddyServeLeaseRegistry';
...
function createHeyBuddyServeResult(
  overrides: Partial<Pick<HeyBuddyServeResult, 'cleanup' | 'hasExited' | 'getExitDetails'>> = {}
): HeyBuddyServeResult {
  return {
    acpUrl: 'ws://127.0.0.1:1234/acp?token=test',
    workingDir: '/tmp',
    process: new EventEmitter() as HeyBuddyServeResult['process'],
    errorLog: [],
...
describe('HeyBuddyServeLeaseRegistry', () => {
  it('returns the ACP URL for an attached live lease', () => {
    const store = new HeyBuddyServeLeaseRegistry(createLogger());
    const lease = store.create(createHeyBuddyServeResult(), 'local-secret');
  ...
  it('throws a recovery message after the process exits', () => {
    ...
    const store = new HeyBuddyServeLeaseRegistry(logger);
    const result = createHeyBuddyServeResult();
    const lease = store.create(result, 'local-secret');
    store.attachWindow(1, lease);

    result.process.emit('exit', 1, null);

    expect(() => store.getAcpUrl(1)).toThrow(HEYBUDDY_SERVE_EXITED_USER_MESSAGE);
    expect(() => store.getSecretKey(1)).toThrow(HEYBUDDY_SERVE_EXITED_USER_MESSAGE);
  ...
  it('cleans up once after the last attached window is released', async () => {
    ...
    const store = new HeyBuddyServeLeaseRegistry(createLogger());
    const lease = store.create(createHeyBuddyServeResult({ cleanup }), 'local-secret');
    store.attachWindow(1, lease);
