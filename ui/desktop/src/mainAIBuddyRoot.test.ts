...
describe('main embedded AIBuddy root', () => {
  it('passes the AIBuddy-owned AIBuddy root from main to the embedded backend', async () => {
    vi.resetModules();
    ...
    const buildEnv = vi.fn((_credentials, aibuddyPathRoot: string) => ({
    ...
    vi.doMock('./aibuddyServeEnv', () => ({ buildAIBuddyServeEnv: buildEnv }));
    vi.doMock('./aibuddyServe', () => ({ startAIBuddyServe: start }));
    ...
    vi.doMock('./utils/settings', () => ({
      defaultSettings: {
        ...
        externalAIBuddyd: { enabled: false, secret: '', url: '' },
        keyboardShortcuts: {},
        language: 'system',
        recentModels: [],
    ...
    try {
      ...
      const expectedRoot = '/tmp/Application Support/AIBuddy/aibuddy';
      expect(buildEnv).toHaveBeenCalledWith(null, expectedRoot);
      expect(start).toHaveBeenCalledWith(
        expect.objectContaining({
          env: expect.objectContaining({ AIBUDDY_PATH_ROOT: expectedRoot }),
        })
      );
    } finally {
      ...
      vi.doUnmock('./aibuddyServeEnv');
      vi.doUnmock('./aibuddyServe');
