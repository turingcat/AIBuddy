...
export function resolveAuthApiBaseUrl(
  ...
  edition: AppEdition = 'aibuddy',
  fallback?: string
): string {
  return (
    environment.AIBUDDY_AUTH_API_BASE_URL?.trim() || fallback || brands[edition].authApiBaseUrl
  );
}
...
const edition = process.env.APP_EDITION ? getAppEdition() : 'aibuddy';
