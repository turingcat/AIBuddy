...
import { buildLocalServeUrls, findAIBuddyBinaryPath, startAIBuddyServe } from './aibuddyServe';
...
function makeTempDir(): string {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'aibuddy-serve-test-'));
...
describe('findAIBuddyBinaryPath', () => {
  afterEach(() => {
    vi.unstubAllEnvs();
  ...
  it('uses AIBUDDY_BINARY in development builds', () => {
    const tempDir = makeTempDir();
    const overridePath = makeFile(path.join(tempDir, 'override-aibuddy'));
    vi.stubEnv('AIBUDDY_BINARY', overridePath);

    expect(findAIBuddyBinaryPath({ isPackaged: false })).toBe(overridePath);
  });

  it('rejects AIBUDDY_BINARY in packaged builds', () => {
    const tempDir = makeTempDir();
    ...
    expect(() => findAIBuddyBinaryPath({ isPackaged: true, resourcesPath })).toThrow(
      'AIBUDDY_BINARY is only supported in development builds'
    );
  });
  ...
  it('prefers the staged binary over target builds in development builds', () => {
    ...
    const resolvedPath = findAIBuddyBinaryPath({ isPackaged: false });
  ...
  it('uses the bundled aibuddy binary in packaged builds', () => {
    const tempDir = makeTempDir();
    const resourcesPath = path.join(tempDir, 'resources');
    const bundledPath = makeFile(path.join(resourcesPath, 'bin', binaryName));

    expect(findAIBuddyBinaryPath({ isPackaged: true, resourcesPath })).toBe(bundledPath);
  });
});
...
describe('startAIBuddyServe', () => {
  it.skipIf(process.platform === 'win32')(
    'passes the owned AIBuddy data root to the child process',
    async () => {
      const tempDir = makeTempDir();
      const envPath = path.join(tempDir, 'aibuddy-path-root.txt');
      const aibuddyPath = makeExecutable(
        path.join(tempDir, 'aibuddy'),
        [
          '#!/usr/bin/env sh',
          'printf "%s\\n" "$AIBUDDY_PATH_ROOT" > "$TEST_AIBUDDY_PATH_ROOT_FILE"',
      ...
      const result = await startAIBuddyServe({
        ...
        dir: tempDir,
        env: {
          TEST_AIBUDDY_PATH_ROOT_FILE: envPath,
      ...
      try {
        expect(await waitForFileLines(envPath)).toEqual(['/tmp/Application Support/AIBuddy/aibuddy']);
        expect(process.env.AIBUDDY_PATH_ROOT).toBe('/shared/aibuddy');
      } finally {
  ...
  it.skipIf(process.platform === 'win32')('captures the TLS fingerprint from stdout', async () => {
    ...
    const aibuddyPath = makeExecutable(
      ...
      [
        '#!/usr/bin/env sh',
        'printf "AIBUDDYD_CERT_FINGERPRINT=AA:BB:CC\\n"',
        'while true; do sleep 1; done',
        '',
      ].join('\n')
    );
  ...
  it.skipIf(process.platform === 'win32')('uses TLS URLs and args when TLS is enabled', async () => {
    ...
    const aibuddyPath = makeExecutable(
      ...
      [
        '#!/usr/bin/env sh',
        'printf "%s\\n" "$@" > "$TEST_ARGS_PATH"',
        'printf "AIBUDDYD_CERT_FINGERPRINT=DD:EE:FF\\n"',
        'while true; do sleep 1; done',
        '',
      ].join('\n')
    );
  ...
  it.skipIf(process.platform === 'win32')('waits for TLS fingerprint after readiness succeeds', async () => {
    ...
    const aibuddyPath = makeExecutable(
      ...
      [
        '#!/usr/bin/env sh',
        'sleep 0.1',
        'printf "AIBUDDYD_CERT_FINGERPRINT=11:22:33\\n"',
        'while true; do sleep 1; done',
        '',
      ].join('\n')
    );
