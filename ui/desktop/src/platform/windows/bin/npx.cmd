...
SET "NODE_ZIP=%TEMP%\goose-node-%NODE_VERSION%-%NODE_ARCH%.zip"
SET "NODE_EXTRACT=%TEMP%\goose-node-extract-%NODE_ARCH%"

powershell -NoProfile -Command "$ProgressPreference='SilentlyContinue'; try { [Net.ServicePointManager]::SecurityProtocol=[Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri 'https://nodejs.org/dist/v%NODE_VERSION%/%NODE_PACKAGE%.zip' -OutFile '%NODE_ZIP%' -UseBasicParsing; Expand-Archive -Path '%NODE_ZIP%' -DestinationPath '%NODE_EXTRACT%' -Force; exit 0 } catch { Write-Error $_.Exception.Message; exit 1 }"
if errorlevel 1 (
...
xcopy /s /e /q /y "%NODE_EXTRACT%\%NODE_PACKAGE%\*" "%GOOSE_NODE_DIR%\" >nul 2>&1
