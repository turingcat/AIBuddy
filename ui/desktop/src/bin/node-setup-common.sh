...
if [ -n "${AIBUDDY_PATH_ROOT:-}" ]; then
    RESOLVED_AIBUDDY_CONFIG_DIR="${AIBUDDY_PATH_ROOT}/config"
elif [ -n "${AIBUDDY_CONFIG_DIR:-}" ]; then
    log "AIBUDDY_CONFIG_DIR is deprecated for desktop shims; prefer AIBUDDY_PATH_ROOT."
    RESOLVED_AIBUDDY_CONFIG_DIR="${AIBUDDY_CONFIG_DIR}"
else
    RESOLVED_AIBUDDY_CONFIG_DIR="${HOME}/.config/aibuddy"
fi
MCP_HERMIT_DIR="${RESOLVED_AIBUDDY_CONFIG_DIR}/mcp-hermit"
mkdir -p "${RESOLVED_AIBUDDY_CONFIG_DIR}"
HERMIT_SETUP_LOCK_DIR="${RESOLVED_AIBUDDY_CONFIG_DIR}/.mcp-hermit-setup.lock"
...
CLEANUP_MARKER="${RESOLVED_AIBUDDY_CONFIG_DIR}/.mcp-hermit-cleanup-v1"
...
log "Checking for AIBUDDY_NPM_REGISTRY and AIBUDDY_NPM_CERT environment variables for custom npm registry setup..."
# Check if AIBUDDY_NPM_REGISTRY is set and accessible
if [ -n "${AIBUDDY_NPM_REGISTRY:-}" ] && curl -s --head --fail "${AIBUDDY_NPM_REGISTRY}" > /dev/null; then
    log "Checking custom aibuddy registry availability: ${AIBUDDY_NPM_REGISTRY}"
    log "${AIBUDDY_NPM_REGISTRY} is accessible. Using it for npm registry."
    export NPM_CONFIG_REGISTRY="${AIBUDDY_NPM_REGISTRY}"

    # Check if AIBUDDY_NPM_CERT is set and accessible
    if [ -n "${AIBUDDY_NPM_CERT:-}" ] && curl -s --head --fail "${AIBUDDY_NPM_CERT}" > /dev/null; then
        log "Downloading certificate from: ${AIBUDDY_NPM_CERT}"
        curl -sSL -o "${MCP_HERMIT_DIR}/cert.pem" "${AIBUDDY_NPM_CERT}"
        if [ $? -eq 0 ]; then
        ...
        log "AIBUDDY_NPM_CERT is either not set or not accessible. Skipping certificate setup."
    fi

else
    log "AIBUDDY_NPM_REGISTRY is either not set or not accessible. Falling back to default npm registry."
