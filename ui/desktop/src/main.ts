...
import { expandTilde, sanitizeAIBuddyPathRoot } from './utils/pathUtils';
...
let appConfig = {
  ...
  AIBUDDY_PATH_ROOT: sanitizeAIBuddyPathRoot(process.env),
  AIBUDDY_WORKING_DIR: '',
...
const createChat = async (
  ...
  if (externalBackend) {
    ...
    const heyBuddyEnv = buildAIBuddyEnv(readCredentials(CREDENTIALS_FILE, getCredentialsCodec()));
    ...
    try {
      aibuddyServeResult = await startAIBuddyServe({
        ...
        tls: true,
        env: {
          AIBUDDY_PATH_ROOT: appConfig.AIBUDDY_PATH_ROOT as string | undefined,
          ...heyBuddyEnv,
        },
        loginShellPath,
  ...
  // AIBuddy's react app uses HashRouter, so the path + search params follow a #/
...
ipcMain.handle(
  ...
  async (_event, amount: number): Promise<WechatPayOrderResult> => {
    ...
    if (!result.ok) {
      log.error(
        `[AIBuddy] 微信充值下单失败 kind=${result.kind} amount=${amount}: ${result.message}`
      );
    }
...
// 模型列表走主进程 fetch new-api /v1/models：绕开 aibuddy inventory refresh 依赖 + renderer CSP
// @author logic
// @date 2026-08-12
