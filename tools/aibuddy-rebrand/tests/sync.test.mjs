...
const RULES_V1 = {
  renames: {
    'src/goose-core.txt': 'src/aibuddy-core.txt',
    'docs/goose-note.txt': 'docs/aibuddy-note.txt',
  },
};
...
const RULES_V2 = {
  renames: {
    'src/goose-core.txt': 'src/aibuddy-core.txt',
    'docs/goose-note.txt': 'docs/aibuddy-reference.txt',
  },
};
...
const H0_SOURCE = without(
  {
    ...
    'desktop/theme.txt': file(
      'Goose desktop baseline\nAIBuddy desktop customization\n',
    ),
    'product-only.txt': file('AIBuddy product-only behavior\n'),
  },
...
function fixtureOnlyDeterministicRename(snapshot, rules) {
  ...
  for (const [sourcePath, sourceFile] of Object.entries(snapshot)) {
    ...
    result[targetPath] = {
      ...sourceFile,
      content: sourceFile.content.replaceAll('Goose', 'AIBuddy'),
    };
  }
  return result;
}
...
async function createFixtureRepo({ ambientEnv = process.env } = {}) {
  const repo = await mkdtemp(path.join(os.tmpdir(), 'aibuddy-sync-fixture-'));
...
test('keeps transformed mirror snapshots as an independent M0 to M1 linear graph', async () => {
  ...
  try {
    ...
    assert.equal(commitFile(state.fixture, state.m0, 'src/aibuddy-core.txt'), 'AIBuddy core v0\n');
    assert.equal(commitFile(state.fixture, state.m1, 'desktop/theme.txt'), 'AIBuddy desktop baseline\n');
    assert.equal(commitFile(state.fixture, state.m1, 'docs/aibuddy-note.txt'), 'AIBuddy note\n');
    assert.deepEqual(commitPaths(state.fixture, state.m1), [
      'desktop/legacy-panel.txt',
      'desktop/theme.txt',
      'docs/aibuddy-note.txt',
      'src/aibuddy-core.txt',
  ...
  try {
    const productFeature = commitSnapshot(
      state.fixture,
      {
        ...
        'product-feature.txt': file('AIBuddy product feature\n'),
      },
      {
    ...
    assert.equal(
      ...
      'AIBuddy product feature\n',
    );
    assert.equal(
      commitFile(state.fixture, integrated, 'src/aibuddy-core.txt'),
      'AIBuddy core v1 upstream increment\n',
    );
    assert.equal(
...
test('merges a same-file nonoverlapping desktop update without losing customization', async () => {
  ...
  try {
    ...
    assert.equal(
      commitFile(state.fixture, integrated, 'desktop/theme.txt'),
      'AIBuddy desktop baseline refreshed upstream\nAIBuddy desktop stable option\nAIBuddy desktop stable detail\nAIBuddy desktop stable footer\nAIBuddy desktop customization\n',
    );
  } finally {
    await removeFixtureRepo(state.fixture);
  }
...
test('stops on an overlapping semantic desktop conflict without choosing a side', async () => {
  ...
  try {
    const productSemanticChange = commitSnapshot(
      ...
      {
        ...fixtureOnlyDeterministicRename(H0_SOURCE, RULES_V1),
        'desktop/theme.txt': file('AIBuddy desktop product behavior\n'),
      },
      {
...
test('integrates a rule migration before merging the next upstream snapshot', async () => {
  ...
  try {
    ...
    assert.equal(commitPaths(state.fixture, ruleIntegrated).includes('docs/aibuddy-note.txt'), false);
    assert.equal(
      commitFile(state.fixture, ruleIntegrated, 'docs/aibuddy-reference.txt'),
      'AIBuddy note\n',
    );
    assert.equal(
    ...
    assert.equal(commitPaths(state.fixture, integratedNext).includes('docs/aibuddy-note.txt'), false);
    assert.equal(
      commitFile(state.fixture, integratedNext, 'docs/aibuddy-reference.txt'),
      'AIBuddy note\n',
    );
    assert.equal(
      commitFile(state.fixture, integratedNext, 'src/aibuddy-core.txt'),
      'AIBuddy core v1 upstream increment\n',
    );
    assert.equal(
...
test('keeps a rule migration as a separate mirror candidate from an upstream increment', async () => {
  ...
  try {
    ...
    assert.deepEqual(commitParents(state.fixture, state.m1), [state.m0]);
    assert.equal(
      commitFile(state.fixture, ruleMigration, 'docs/aibuddy-reference.txt'),
      'AIBuddy note\n',
    );
    assert.equal(commitPaths(state.fixture, ruleMigration).includes('src/upstream-feature.txt'), false);
    assert.equal(commitPaths(state.fixture, state.m1).includes('docs/aibuddy-reference.txt'), false);
    assert.equal(
      commitFile(state.fixture, ruleMigration, 'src/aibuddy-core.txt'),
      commitFile(state.fixture, state.m0, 'src/aibuddy-core.txt'),
    );
...
test('isolates fixture Git commands from ambient config, environment, and hooks', async () => {
  const ambientRoot = await mkdtemp(path.join(os.tmpdir(), 'aibuddy-sync-ambient-'));
