...
test('plans literal edits with documented UTF-16 JavaScript offsets', () => {
  ...
  assert.deepEqual(result, {
    'src/example.txt': '😀 const aibuddy = "aibuddy";\n',
  });
...
test('plans and applies a safe path move together with content edits', () => {
  const pathRule = {
    ...
    to: 'crates/aibuddy-cli/Cargo.toml',
  ...
  assert.deepEqual(result, {
    'crates/aibuddy-cli/Cargo.toml': 'name = "aibuddy-cli"\n',
  });
});
...
test('rejects case-insensitive path collisions and unsafe targets', () => {
  ...
  assert.throws(
    () => validateRuleSet(
      ruleSet({ ...pathRule, id: 'test.unsafe', to: '../AIBuddy.txt' }),
    ),
