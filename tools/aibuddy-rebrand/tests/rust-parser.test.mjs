...
const PRODUCT_SOURCE_COMMIT = 'e8da4298445d851dc7f07cdc10eda636246c5f01';
...
test('accepts valid let-else Rust and edits tokens without changing formatting', () => {
  ...
  assert.equal(text, [
    'pub fn run(value: Option<&str>) -> Option<&str> {',
    '    let Some(aibuddy) = value else {',
    '        // AIBuddy stays in this comment.',
    '        return None;',
    '    };',
    '    Some(aibuddy)',
    '}',
...
test('renames macro definitions, macro inputs, and raw or byte string literals', () => {
  ...
  assert.equal(text, [
    'macro_rules! AIBuddiesFactory {',
    '    ($value:expr) => { AIBuddy::new($value) };',
    '}',
    '',
    'fn example() {',
    '    let _ = AIBuddiesFactory!(r###"AIBuddy"###);',
    '    let _bytes = br###"AIBuddies"###;',
    '}',
    '',
  ].join('\n'));
});
...
test('maps Rust UTF-8 byte spans to JavaScript UTF-16 offsets', () => {
  ...
  assert.equal(text, 'fn main() { let greeting = "中文"; let aibuddy = "AIBuddy"; }\n');
...
test('renames comments and preserves URL/runtime identity references', () => {
  ...
  assert.equal(text, [
    '// AIBuddy comment',
    '/* AIBuddies block comment */',
    '// URL https://github.com/aaif-goose/goose stays exact.',
    'fn choose() {',
    '    let args = choose_app_strategy(AppStrategyArgs {',
    '        app_name: "goose".to_string(),',
    '        display_name: "aibuddy".to_string(),',
    '    });',
    '    let name = "aibuddy";',
    '    const KEYRING_SERVICE: &str = "goose";',
    '    let _ = (args, name, KEYRING_SERVICE);',
...
test('preserves URL spans while renaming adjacent brands in normal, raw, and byte strings', () => {
  ...
  assert.equal(text, [
    'fn example() {',
    '    let normal = "https://github.com/aaif-goose/goose AIBuddy";',
    '    let raw = r###"https://github.com/aaif-goose/goose AIBuddies"###;',
    '    let bytes = br###"https://github.com/aaif-goose/goose aibuddy"###;',
    '    let _ = (normal, raw, bytes);',
    '}',
    '',
  ].join('\n'));
...
test('parses every Rust file in the recorded H0 snapshot', async () => {
  const snapshot = readSnapshot({
    cwd: process.cwd(),
    sourceRef: PRODUCT_SOURCE_COMMIT,
  });
