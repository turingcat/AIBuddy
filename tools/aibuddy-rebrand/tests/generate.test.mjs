...
function createFixture({ marker = false, submodule = false, trailingSpace = false } = {}) {
  let root = mkdtempSync(join(tmpdir(), 'aibuddy-rebrand-generate-'));
  if (trailingSpace) {
  ...
  if (marker) {
    writeFixtureFile(root, '.aibuddy-rebrand.json', '{"valid":true}\n');
  }

  git(root, ['init', '--quiet', '--initial-branch=fixture']);
  const hooksPath = mkdtempSync(join(tmpdir(), 'aibuddy-rebrand-hooks-'));
  ...
  if (submodule) {
    const nested = mkdtempSync(join(tmpdir(), 'aibuddy-rebrand-submodule-'));
...
function createOutputLocation(label) {
  const parent = mkdtempSync(join(tmpdir(), `aibuddy-rebrand-output-${label}-`));
  return {
    cleanup() {
...
test('repeated generation into fresh destinations is byte-for-byte reproducible', async (t) => {
  ...
  assert.deepEqual(
    JSON.parse(readFileSync(join(firstDir, '.aibuddy-rebrand.json'), 'utf8')),
    JSON.parse(readFileSync(join(secondDir, '.aibuddy-rebrand.json'), 'utf8')),
  );
});
...
test('rejects destinations inside the source worktree and unsafe parent symlinks', async (t) => {
  ...
  const outside = mkdtempSync(join(tmpdir(), 'aibuddy-rebrand-parent-'));
