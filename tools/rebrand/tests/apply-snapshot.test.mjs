...
const BRANCH = 'feat/heybuddy-branding';
...
function createFixture({ branch = BRANCH, controlCommit = false, nestedWorkspaceFile = false } = {}) {
  const root = mkdtempSync(join(tmpdir(), 'heybuddy-rebrand-apply-'));
...
function createOutputLocation() {
  const parent = mkdtempSync(join(tmpdir(), 'heybuddy-rebrand-output-'));
  return {
...
async function createSnapshot(fixture, { targetPath = 'src/heybuddy.txt', deletePaths = [] } = {}) {
    ...
    transform: async (entries) => {
      ...
      for (const entry of entries) {
        ...
        if (entry.path === 'src/goose.txt') {
          content = Buffer.from('HeyBuddy source\n');
...
test('dry-run reports changes without touching the feature worktree', async (t) => {
  ...
  assert.deepEqual(result.added, ['src/heybuddy.txt']);
  ...
  assert.equal(existsSync(join(fixture.root, 'src/heybuddy.txt')), false);
...
test('applies output files atomically and preserves unrelated product files and symlinks', async (t) => {
  ...
  assert.deepEqual(
    JSON.parse(readFileSync(join(fixture.root, '.heybuddy-rebrand.json'), 'utf8')),
    JSON.parse(readFileSync(join(output.outputDir, '.heybuddy-rebrand.json'), 'utf8')),
  );
...
test('rejects an existing non-baseline destination and symlink ancestors', async (t) => {
  ...
  const symlinkOutput = await createSnapshot(fixture, { targetPath: 'new/nested/heybuddy.txt' });
  const outside = mkdtempSync(join(tmpdir(), 'heybuddy-rebrand-outside-'));
  t.after(() => {
  ...
  writeFixtureFile(fixture.root, 'src/heybuddy.txt', 'unrelated collision\n');
  await assert.rejects(
    applySnapshot({ cwd: fixture.root, outputDir: collisionOutput.outputDir }),
    /collision|destination/i,
  );
  assert.equal(readFileSync(join(fixture.root, 'keep.txt'), 'utf8'), 'keep source\n');
  rmSync(join(fixture.root, 'src/heybuddy.txt'));
  ...
  assert.equal(existsSync(join(fixture.root, 'new/nested/heybuddy.txt')), false);
});
...
test('rejects main even when it is supplied as the expected branch', async (t) => {
  ...
  await assert.rejects(
    ...
    /expectedBranch must be a feat\/\* branch/i,
  );
...
test('rejects a competing cooperative apply lock', async (t) => {
  const fixture = createFixture();
  const output = await createSnapshot(fixture);
  const lockPath = join(fixture.root, '.heybuddy-rebrand.apply.lock');
  t.after(() => {
  ...
  await assert.rejects(
    applySnapshot({
      ...
      beforeMutation: ({ kind, path }) => {
        if (kind === 'write' && path === 'src/heybuddy.txt') {
          writeFileSync(
...
test('rejects a destination that appears after the initial destination check', async (t) => {
  ...
  const collisionPath = join(fixture.root, 'src/heybuddy.txt');
  await assert.rejects(
    applySnapshot({
      cwd: fixture.root,
      outputDir: output.outputDir,
