...
function createRepo() {
  const repo = mkdtempSync(join(tmpdir(), 'heybuddy-mirror-test-'));
  git(repo, [
    '-c',
    `core.hooksPath=${nullDevice}`,
    'init',
    '--quiet',
    '--initial-branch=feat/heybuddy-branding',
    '--template=',
  ]);
...
async function generateUpstreamSnapshot(repo, sourceCommit, label) {
  const outputDir = mkdtempSync(join(tmpdir(), `heybuddy-mirror-${label}-`));
...
function cleanupRepository({ repo, ...rest }) {
  ...
  for (const directory of Object.values(rest)) {
    if (typeof directory === 'string' && directory.startsWith('/tmp/heybuddy-mirror-')) {
...
test('prepareMirror dry-run validates an upstream snapshot without creating its ref', async (t) => {
  ...
  assert.equal(result.branch, 'upstream/heybuddy-mirror');
  assert.match(result.message, /Upstream-Commit:/);
  assert.equal(output(fixture.repo, ['count-objects', '-v']), objectCountBefore);
  assert.equal(
    git(fixture.repo, ['show-ref', '--verify', '--quiet', 'refs/heads/upstream/heybuddy-mirror'], {
      allowFailure: true,
    }).status,
    1,
  );
...
test('prepareMirror creates a pure transformed root commit and excludes provenance metadata', async (t) => {
  ...
  assert.equal(output(fixture.repo, ['rev-parse', 'refs/heads/upstream/heybuddy-mirror']), result.mirrorCommit);
  assert.deepEqual(parents(fixture.repo, result.mirrorCommit), []);
  assert.equal(lsTree(fixture.repo, result.mirrorCommit).includes('.heybuddy-rebrand.json'), false);
...
test('prepareMirror refuses an existing branch and appendMirror uses CAS against the previous mirror', async (t) => {
  ...
  assert.equal(output(fixture.repo, ['rev-parse', 'refs/heads/upstream/heybuddy-mirror']), m1.mirrorCommit);
...
test('appendMirror preserves the transformed mirror merge-base for a reviewed bridge', async (t) => {
  ...
  try {
    ...
    writeFileSync(join(fixture.repo, 'product-only.txt'), 'HeyBuddy product patch\n');
    ...
    assert.equal(output(fixture.repo, ['show', `${bridge}:product-only.txt`]), 'HeyBuddy product patch');
  } finally {
...
test('establishBridge binds an external proof artifact and only creates an explicitly approved bridge', async (t) => {
  const fixture = makeRepositoryWithSnapshots();
  t.after(() => cleanupRepository(fixture));
  const proofDirectory = mkdtempSync(join(tmpdir(), 'heybuddy-mirror-proof-'));
  ...
  git(fixture.repo, ['checkout', '-B', 'feat/heybuddy-branding']);
