...
test('formats selected Rust through rustfmt and reports precise change metadata', () => {
  const source = Buffer.from([
    ...
    '}',
    '',
    'fn example(heybuddy_extremely_long_identifier_that_exceeds_the_default_formatting_width: usize) {',
    '    let _value = calculate_value(heybuddy_extremely_long_identifier_that_exceeds_the_default_formatting_width, 123);',
    '}',
    '',
  ].join('\n'));
  ...
  assert.match(formatted.toString('utf8'), /heybuddy_extremely_long_identifier_that_exceeds_the_default_formatting_width/u);
...
test('does not use the caller directory rustfmt configuration or write there', () => {
  const callerDirectory = mkdtempSync(join(tmpdir(), 'heybuddy-format-caller-'));
