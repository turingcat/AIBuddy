...
const DEFAULT_BRANCH = 'feat/heybuddy-branding';
const APPLY_LOCK_PATH = '.heybuddy-rebrand.apply.lock';
...
function temporaryName(parent, name) {
  return join(parent, `.${name}.heybuddy-rebrand-${randomBytes(8).toString('hex')}`);
}
...
export async function applySnapshot({
  ...
  if (!expectedBranch.startsWith('feat/') || expectedBranch.length <= 'feat/'.length) {
    throw new Error('expectedBranch must be a feat/* branch');
  }
  ...
  try {
    try {
      ...
      if (!actualBranch.startsWith('feat/')) {
        throw new Error(`applySnapshot refuses non-feature branch ${actualBranch}`);
      }
