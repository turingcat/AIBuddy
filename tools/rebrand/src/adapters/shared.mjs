const BRAND_PATTERN = /(GEESE|Geese|geese|GOOSE|Goose|goose)/g;
const BRAND_OCCURRENCE_PATTERN = /GEESE|Geese|geese|GOOSE|Goose|goose/g;
...
export function canonicalBrand(value) {
  return value
    .replace(/GEESE/g, 'HEYBUDDIES')
    .replace(/Geese/g, 'HeyBuddies')
    .replace(/geese/g, 'heybuddies')
    .replace(/GOOSE/g, 'HEYBUDDY')
    .replace(/Goose/g, 'HeyBuddy')
    .replace(/goose/g, 'heybuddy');
}
...
export function renameBrandSegments(value) {
  ...
  return renamed.replace(/@aaif\/(heybuddy(?:-|$))/gu, '@heybuddy/$1');
}
...
export function textHasBrand(value) {
  return /(?:goose|geese)/iu.test(value);
}
