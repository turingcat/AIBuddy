...
const PROVENANCE_PATH = '.heybuddy-rebrand.json';
const DEFAULT_BRANCH = 'upstream/heybuddy-mirror';
const BRIDGE_BRANCH = 'feat/heybuddy-branding';
...
function writeSnapshotTree(cwd, snapshot, { writeObjects }) {
  ...
  const blobDirectory = mkdtempSync(join(tmpdir(), 'heybuddy-mirror-blobs-'));
  const indexDirectory = mkdtempSync(join(tmpdir(), 'heybuddy-mirror-index-'));
